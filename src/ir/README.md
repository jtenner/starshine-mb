...
- `hot_query.mbt`: canonical read-only node-family, exact-instruction, type, branch, simple-value, repeated-root / repeated-child, trailing-payload-split, span, local-metadata, and tombstone queries so analyses and passes stop decoding raw storage layout directly.
