...
- `hot_query.mbt`: canonical read-only node-family, exact-instruction, type, branch, simple-value, single-root wrapper, single-`if`, holder-block, repeated-root / repeated-child, repeated-wrapper, trailing-payload-split, span, local-metadata, and tombstone queries so analyses and passes stop decoding raw storage layout directly.
