...
- Verify parity at `10000` GenValid comparisons with an explicit prebuilt native Starshine binary and parallel workers: first run `moon build --target native --release src/cmd`, then use `bun fuzz compare-pass ... --jobs auto --max-subprocesses 8 --max-mismatch-artifacts 20 --starshine-bin _build/native/release/build/cmd/cmd.exe` (or the equivalent `bun scripts/pass-fuzz-compare.ts ... --jobs auto --max-subprocesses 8 --max-mismatch-artifacts 20 --starshine-bin _build/native/release/build/cmd/cmd.exe`). Use `_build/native/...` after current Moon builds; treat any `target/native/...` binary as stale unless its timestamp/hash proves it was refreshed. Run the separate external-generator lane only when explicitly requested by adding `--wasm-smith`. When a pass has a documented aggregate GenValid profile under `docs/wiki/binaryen/passes/<pass>/fuzzing.md`, use that aggregate for ordinary dedicated-profile signoff and keep singleton profiles for targeted repro. The compare-pass harness uses `.tmp/pass-fuzz-cache` by default to reuse deterministic `wasm-smith` inputs when `--wasm-smith` is selected and Binaryen oracle outputs/failures for all lanes; use `--cache-dir <dir>` for shared or isolated caches and `--no-cache` only when debugging cache behavior. Starshine outputs are never cached.
