...
## Build Note
- `npm run build` refreshes the checked-in WASI CLI artifact under `node/internal/starshine.wasm-wasi.wasm` from the live `src/cmd` build.
- The checked-in JS/TS wrappers and `node/internal/starshine.wasm-gc.wasm` adapter artifact are not regenerated by `npm run build` today.
- `npm run generate` is intentionally disabled while the old `src/node_api` adapter flow is inactive.
