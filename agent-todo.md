...
- Both-canonical diagnostic helper: `scripts/self-optimize-compare.ts --canonicalize-binaryen-output --dae-optimizing` last completed at `defined=509 abs=526` in `.tmp/dae006-next-frontier-20260526`. Research notes `0576` through `0584` reduce the saved Func505 body from unknown/risky output-shape drift to representation-only control/local-carrier, branch-polarity, temp-local, and dropped-debris drift; `0585` adds a diagnostic-only canonical-function normalizer for that inspected Func505 parser-loop family; `0586` records the successor Func509 frontier as a semantic-safe, size-losing dead-return-suffix cleanup gap.
- Latest diagnostic timing is over the 2x target for the current Func509 artifact: `2923.619ms` Starshine pass versus `891.787ms` Binaryen pass for `.tmp/dae006-next-frontier-20260526`. Per user direction, keep prioritizing correctness/frontier classification over pass time for now.
...
- [DAE]006 - Both-Canonical Frontier Advancement
  ...
  - Current classification:
    ...
    - `0587-2026-05-26-dae-func509-return-suffix-cleanup-attempt.md` added focused root-return and terminal-wrapper cleanup tests plus narrow DAE cleanup helpers. A recovery follow-up fixed the pending lowered-return helper tuple bug and added a block-wrapped terminal-wrapper regression, but the latest replay `.tmp/dae006-return-suffix-recovery-20260526` still first-diffs at `defined=509 abs=526` (`2870.440ms` Starshine pass versus `867.327ms` Binaryen pass). Trace spot-checking shows no final-return-suffix cleanup hit on the debug artifact, so the current helpers still miss the actual pre-encode Func509 shape. A later recovery probe rebuilt the native CLI and printed `--dae-optimizing --print-func 526` to `.tmp/dae-print-func526.err`, confirming the pass-side shape is an outer `block I64` containing an inner `block (Void)` with `br Label 1` exits, an `unreachable`, a fallthrough `local.get $20; return`, then post-inner `local.set $2` plus the 8-byte wrapper suffix.
  - Next action:
    - Build a focused white-box reduction for the exact pre-encode outer-block shape: outer `block I64`, inner `block (Void)`, `br Label 1` exits, fallthrough `local.get $20; return`, then `local.set $2` and the wrapper allocation/store/result suffix.
    - Prove whether the `Label 1` exits can reach the wrapper suffix with a live value that must be wrapped, or whether every real exit returns the earlier object before the suffix. Do not add a pass rewrite until this proof is recorded.
    - If the proof is safe, extend `dae_strip_root_return_suffix_instrs` with a narrow regression for that exact outer-block shape and replay the both-canonical artifact. If not safe, document the frontier as non-normalizable/currently unsafe and choose the next `[DAE]006` subfrontier.
    - Preserve the landed root-return and simple wrapper-suffix tests/helpers if they remain valid, but do not claim Func509 closure until the both-canonical artifact compare moves beyond `defined=509 abs=526`.
  - Deliverables:
