...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [CP]002 - Rewrite Coverage and Artifact Validation - Implement the move and test it against branchy, trap-sensitive fixtures plus the MoonBit debug artifact.
     ...
     - Current blocker: focused pass/registry/CLI coverage is in-tree for direct `--code-pushing`; `moon test src/passes`, `moon test src/cmd`, `moon info`, full `moon test`, a `1000`-case wasm-smith compare lane, and `.tmp/pass-fuzz-code-pushing-genvalid-1000` (`1000/1000`, `0` mismatches, `0` validation/generator/command failures) are green. The earlier `CannotEncodeNonStandardTypeIndex` generator/invalid-binary failure is fixed by making descriptor metadata use absolute type indexes, and `--emit-gen-valid-batch` now uses a portable coverage-forced profile that external `wasm-tools`/Binaryen compare lanes can validate. A prior `10000`-case wasm-smith lane reached `9952/10000` comparable cases with `9951` matches, `0` validation failures, and `1` known non-local mismatch in Binaryen's typed-unreachable block canonicalization. The checked debug WASI artifact is still rejected by `wasm-tools validate` for too many locals. The optimized WASI artifact compare is semantically green (`Normalized WAT equal: yes`, `Canonical function compare equal: yes`) but still much slower than Binaryen, so the pass is not parity-complete or ready for public optimize preset scheduling.
     - Next tasks:
       1. Run the long direct `code-pushing` gen-valid compare at `10000` cases and save the result path in this backlog item.
       2. Investigate `.tmp/pass-fuzz-code-pushing-wasmsmith-10000/failures/case-009332-wasm-smith`, but keep it separate from local-set motion unless evidence shows `code-pushing` should own typed-unreachable block canonicalization; current trace showed `pass[code-pushing]:done changed=false` for the mismatching function.
       3. Repair or replace the debug WASI artifact compare input because `tests/node/dist/starshine-debug-wasi.wasm` currently fails `wasm-tools validate` with `too many locals`.
       4. Once the lanes are comparable, rerun the `10000` wasm-smith direct compare, optimized/debug artifact compare where valid, and only then decide whether to expand the implemented Binaryen surface or keep the pass direct-only.
       5. If correctness is green, profile the optimized-artifact lane; current direct `code-pushing` optimized-WASI compare is semantically green but much slower than Binaryen.
     - Doc: [0066#L213](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L213)
3. Do work.
   - Land the slices above in dependency order in the implementing file(s) and any required scheduler, preset, or dispatcher surfaces.
   - Wire the pass into the exact top-level slot(s) and nested rerun sites documented in the research doc before calling the work done.
