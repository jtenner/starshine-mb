...
- The current unreduced artifact frontier still starts at absolute `Func 71`, `_M0FP37jtenner9starshine3cmd31run__cmd__with__adapter_2einner` (WAT `$50`) in `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-3006682`. The newer raw-skip cleanup work trims more no-op families and improves the sibling lane from Starshine `4867.572ms` total / `2616.343ms` pass at `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-2819119` to `4663.826ms` total / `2530.639ms` pass, but Binaryen is still folding more nested same-arm call-result and load/store temporaries there than Starshine.
- The next parity and performance work should stay focused on two remaining buckets instead of broader cleanup passes: the unreduced non-adjacent exact-expression `Func 71` drift, and the internal branchy no-op cluster now led by `Func 395`, `473`, `308`, `1488`, `430`, and `2098` after the `Func 1800` straight-line builder family was removed from the hotspot list.
