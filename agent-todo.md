...
- **Status (2026-07-22 partial):** the arbitrary `touched_count > 8` and module `>100` full-roster skips are removed. Ordinary nested replay derives a Binaryen-shaped worth set from changed function signatures, result-change callers in both original and current call graphs, and exact caller local-count growth for transitional scratch cleanup; plain callsite-only edits and touched callers with unchanged preexisting locals are not replayed. Local `>128` and instruction `>1000` do not suppress semantic nested replay: those checks remain isolated to the earlier broad changed-boundary fast-skip policy, preserving its authoritative 129-local transaction before the unconditional per-function roster. The selected literal/global owner-order blocker was resolved by common epoch-bearing uniform work items. A fresh 101-definition red-first replay is green, and the two formerly blocked literal/global fixtures now match explicit Binaryen v131 output after nested cleanup. Material-caller admission, the earlier broad-cleanup discriminator, and exact convergence feedback remain open.
- **Deliverables:**
  ...
  - [ ] Replace `touched_count > 8`, module `>100`, local `>128`, instruction `>1000`, and material-caller thresholds with source-matched per-function profitability or explicitly non-parity budgets. *(`touched_count`, module size, and semantic nested local/instruction skips are removed; the earlier broad-cleanup local/instruction discriminator and material-caller threshold remain.)*
