...
- [FUZ]1021 - Invalid Binary Malformed-Byte Matrix
  - Status: IN PROGRESS (cron 23, 2026-05-20). Latest section-limit LEB slice landed: `malformed-memory-min-uleb` and `malformed-table-min-uleb` are decode-rejected, non-smoke binary-invalid strategies that emit memory/table sections whose limits minimum-size fields contain unterminated ULEBs, with registry entries, focused replay coverage, LEB/fuzz docs, wiki log entry, and `.mbti` public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on the missing limit-min strategies, then passed after implementation. Earlier slices also landed SIMD lane-overflow coverage through `simd-extract-lane-index-overflow`, the bulk-init secondary-index LEB strategies, import descriptor type-index LEB strategies, export descriptor index-LEB strategies, bulk-copy index-LEB strategies, `invalid-element-kind-byte` for passive/declarative function-index elemkind corruption, and duplicate known-section decode rejection for import/export/table/memory/global/function/code/element/data/tag sections. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon test`, and `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`. Remaining blocker: the task is intentionally broader than this slice; next run should continue with another malformed-byte family such as malformed/overwide LEB carriers in more contexts, more UTF-8/name payload corruption, invalid heap/ref encodings, shape-specific binary-origin SIMD lane validation beyond the current coarse decode-overflow fixtures, or additional section descriptor bytes without weakening stage-aware accounting.
