...
- [O4Z-AUDIT-GSI] - Widen `global-struct-inference` beyond subset signoff
  - Status: reopened v0.1.0 release-gating audit. Current direct compare is green for the documented Starshine subset, and the 2026-06-08 nested-aggregate / un-nesting-size-gate and aggregate-array un-nesting slices are green. Evidence lives in `docs/wiki/raw/research/0718-2026-06-08-global-struct-inference-behavior-gap-inventory.md`: latest focused tests `55/55`, `moon test src/passes` `2000/2000`, full `moon test` `5192/5192`, native `src/cmd` build passed with pre-existing warnings, and direct 10000-case compare `.tmp/pass-fuzz-global-struct-inference-array-unnesting-10000` compared `7608/10000` with `7608` normalized matches, `0` mismatches, and `20` Binaryen/tool command failures. The requested final-style 100000-case lane `.tmp/pass-fuzz-global-struct-inference-final-100000` compared `99751/100000` with `3` agent-classified semantic-safe dropped-unreachable debris mismatches and `249` Binaryen/tool command failures; the follow-up shared cleanup added GSI coverage for those shapes and the direct 10000-case compare `.tmp/pass-fuzz-global-struct-inference-debris-cleanup-10000` compared `7609/10000` with `7609` normalized matches, `0` mismatches, and `20` Binaryen/tool command failures. `moon info` still reproduces the known Moon tool panic.
  - Scope: open-world direct immutable-global reads, closed-world candidate-global reasoning over locals and params, subtype propagation, one/two unique-value selection, non-constant un-nesting, packed fields, atomic gets, `ref.get_desc`, sibling `global-struct-inference-desc-cast`, aggregate array un-nesting, carrier breadth, larger decision trees, and Binaryen-style refinalization triggers.
  - Deliverables: apply the common checklist; refresh the exact Binaryen `version_130` source/lit surface; keep deferrals narrow and evidence-backed; implement remaining Binaryen-positive carrier/refinalization families where local representation supports them; keep sibling `gsi-desc-cast` rejected until its own real pass surface is safe.
