...
- **Latest bounded iteration (2026-07-15):** commits `c6181e26d` and `0a415161f` completed the required two-code performance budget. The first red-first slice built one immutable exact label-to-branch-node index, deduplicated repeated/default `br_table` labels, derived `label_used` from the same scan, and removed scalar target admission's full live-node rescan. The second red-first slice replaced one-for-one scalar and multivalue block/if/loop/legacy-try result-tail splices with exact HOT region-root writes that preserve siblings, old-value ownership, one revision advance, and analysis invalidation. A same-session reconstructed 120-function chain improved `1,197.5 -> 1,092.5 -> 1,030 us` (`13.99%` total); exact-shape code-2 samples were `1,033.5 us` and `1,047 us`, still roughly `3.9x` Binaryen v130's `266.05 us`. Focused flatten is `245/245`, private flatten `146/146`, passes `5,721/5,721`, the full suite `9,181/9,181`, and `moon info` is green. Typed catches/payloads, nested-pop repair, `rethrow`, `delegate`, structured suffix control-plus-label deletion, broader behavior, a GenValid profile, four-lane signoff, performance, and public wiring remain open. Source: `docs/wiki/raw/binaryen/2026-07-15-flatten-version-130-nonthrowing-bridge-suffix-cache-impact.md`.
- **Deliverables:**
