...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [RUN]002 - Pass-Fuzz Command-Failure Classification And Replay - Keep the `remove-unused-names` differential-fuzz command-failure corpus visible as a distinct follow-up item so Binaryen parser and canonicalization gaps are tracked separately from Starshine semantic regressions.
     - Deliverables: preserve the saved `wasm-smith` corpus under `.tmp/pass-fuzz-run-smith`; replay the Binaryen-only failures independently of Starshine pass work; keep the harness-level `failureClass` buckets current as new command-failure families appear.
