...
- [O4Z-AUDIT-SSA] - Deep audit `ssa-nomerge`
  ...
  - Remaining deliverables: classify or reduce the remaining exact artifact first diff after `.tmp/self-ssa-nomerge-debug-wasi-canonical-local` (`defined=0 abs=27`, currently representation/local-numbering drift and not proven corruption); sample the remaining `17` huge `large-structured-local-writes` functions now traced with `func`/`instrs`/`locals` payloads before any further threshold change; replay early `SSA` slot / artifact neighborhoods; decide whether the current O4z `ssa-nomerge` no-op remains intentional or can be narrowed; resolve or route the current `-O4z` debug-artifact abort in a later `remove-unused-names` `Func 2560` neighborhood before using preset evidence to re-enable SSA; file pass-local runtime fixes here and whole-command residuals under `[WALL]001`.
