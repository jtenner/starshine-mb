...
- [CF]002 - Late-Slot Regression and Artifact Compare
  - Current status: runtime recovered on 2026-05-10 after narrowing embedded branch/drop payload scans to small, likely-profitable control payloads and delaying expensive exiting-block checks until a direct `unreachable` sentinel is visible. Direct `code-folding` still covers typed/value `if` suffix hoists, branch-to-outer-label full-tail hoists, branch-free structured suffix hoists, unprofitable full-void `if` skips, conservative full-`if` terminal suffix sharing for empty-payload `return` / `unreachable` endings, conservative block-exit/fallthrough tail hoists, and branch-payload embedded block-exit hoists, with safety guards for live block labels / result-region trailing `unreachable`. Direct fuzz remains semantic-green at `.tmp/pass-fuzz-code-folding-cf002-terminal-if` (`6759` compared, `0` mismatches, `20` Binaryen command failures). The debug artifact `--code-folding` command still differs first at `defined=220 abs=237`, but pass-local timing remains inside the repo floor (`334.711ms` Starshine vs `176.295ms` Binaryen on `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-1680352`). A reduced nested value-branch helper-wrapper fixture now confirms Binaryen and Starshine share the same duplicated-call payoff while preserving different wrapper placement; the artifact function has equal `call $28` counts and smaller Starshine focused WAT, so this specific remaining diff is classified as representation drift rather than a current code-quality blocker.
  - Remaining deliverables: implement function-ending helper-label sharing as needed; keep branch-scope, embedded-payload budget, and EH movement guards conservative; rerun late-pipeline slot and `--code-folding` artifact parity or record explicit representation-drift acceptance for direct pass signoff.
