...
- **Latest bounded iteration (2026-07-15):** commits `d20ebd21c` and `e9cb90b84` plus the current cache slice remove nonthrowing synthetic catch-all bridge scaffolding, admit direct `i32.mul`/`i32.and` call roots, consolidate suffix ownership onto one run-wide use-def snapshot, cache exact owner-region/node vectors during admission, and refuse cache misses after mutation starts. The refreshed three-probe matrix validates and returns the expected results with `204` input, `212` Starshine direct, `212` Starshine cleanup, `275` Binaryen direct, and `236` Binaryen cleanup bytes; this narrow bridge/control/local family is a measured 24-byte post-cleanup Starshine win. Representative pass-only timing improves from `3,682.5 us` to `2,345.5 us`, but remains `8.82x` Binaryen v130's `266.05 us`. Typed catches/payloads, nested-pop repair, `rethrow`, `delegate`, structured suffix control deletion, broader behavior, a GenValid profile, four-lane signoff, the `<=2x` performance target, and public wiring remain open. Source: `docs/wiki/raw/binaryen/2026-07-15-flatten-version-130-nonthrowing-bridge-suffix-cache-impact.md`.
- **Deliverables:**
