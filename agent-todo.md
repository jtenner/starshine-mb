...
- [RSE]002 - CFG Value Flow And Refined Local Gets
  - Current status: 2026-05-10 added branch-disagreement merge identities for self-set folding, body-local default identities for redundant default writes, raw strict-subtype equivalent-local `local.get` retargeting, raw preservation of fallthrough facts after one-armed terminating `if`s, paired vacuum cleanup for nested pure `drop` / `nop` debris, vacuum empty-then/live-else inversion, raw structured block/if label-exit merge tracking with unreachable-tail skipping so `br_if` block exits no longer misfold final const sets, identity-preserving refinement wrappers for raw `ref.as_non_null` / `ref.cast` / `ref.cast_desc_eq` so concrete-heap refined gets now retarget through straight-line and branch-merge shapes, programmatic lowered-instruction coverage for `ref.cast` / `ref.cast_desc_eq`, and an explicit conservative loop-exit skip regression where a post-loop const set survives an outer loop exit. Direct compare-pass stayed semantic-green at `.tmp/pass-fuzz-rse-rse002-cast-loop-coverage` (`6759/10000`, `0` mismatches, `20` Binaryen/tool command failures).
  - Remaining deliverables: replace the documented conservative loop-exit skip with fixed-point loop convergence where parity needs it, extend the same structured label start/end identities through the remaining HOT/control families, and broaden `rse-gc.wast`-style type-refinement coverage beyond the reduced abs-heap plus concrete-heap `ref.as_non_null` / `ref.cast` / `ref.cast_desc_eq` wrapper fixtures.
  ...
  - Tests: branch-join positives/negatives, raw block/loop-exit negatives, broader `rse-gc.wast`-style type-refinement tests, direct compare-pass parity, and eventual late-tail preset replay.
