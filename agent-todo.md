...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [TO]005 - Exact Slot Gate And Oracle Proof - Keep `tuple-optimization` active on the explicit hot-pass surface, but finish the Binaryen-matching preset slot, feature gate, and parity proof in a follow-up slice.
     ...
     - Current blocker: the explicit pass surface is ready, but the surrounding Binaryen neighbors are still not representable in the preserved public preset order, so the next keepable work is proof and exact-slot preparation rather than approximate scheduling. The tuple pass itself now recognizes one-hop scalar-forward copy bridges from direct non-`Block` multivalue spill producers, the compare-shaped host-`local.tee` seed bridge, the raw decoded one-hop carrier shape, the narrowed no-host root-carrier family, and the new overlap-aware exact-copy family where a copy group writes back into one of its source lane locals. New white-box regressions in `src/passes/tuple_optimization_wbtest.mbt` now lock the extra synthetic copy-group analysis, the no-host source/copy root-carrier path, and the new single-use / overlap-aware exact-copy rewrites. Hot lowering also now keeps typed terminal `if` carriers that feed `return`. The latest kept-tree debug-artifact compare is `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-149131`, and it is still red: canonical wasm and normalized WAT differ, Starshine is still over budget (`4939.432 ms` wall, `830.388 ms` pass) versus Binaryen (`318.315 ms` wall, `2.850 ms` pass), and the first substantive diff is still the same earlier source/copy rematerialization family in canonical `func $440`. The overlap-aware exact-copy fix landed, but it did not eliminate that older surviving artifact hunk.
