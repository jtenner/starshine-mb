...
- **Exit criteria / suggested tests:** retain open parity work until the 1,253 GTO, 2,216 shared-object, 2,632 CA, and 1,080 OI v133 output differences are classified with semantic and size evidence; the smaller generic control/representation differences in the other lanes also remain open. GTO's 935 canonical size losses and shared objects' 86 losses especially need reduction. The 167 v133 shared-object oracle command failures comprise released `try_table` and intrinsic validation bugs; avoid treating them as Starshine comparisons. Complete the remaining released exception, descriptor, subtype, atomic, `struct.wait`, and i64 wasm2js variants with red fixtures before full release signoff. The i64 implementation currently covers only the four new saturating conversions. Keep the ordinary baseline at verified v132.
