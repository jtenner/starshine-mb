...
- **Current progress:** analysis-only Flat IR classification, scalar function-result materialization, root `local.tee` lowering, ordered scalar ordinary-operand preludes, and branch-free defaultable scalar `block`/`if` result routing are implemented with focused verifier-backed tests; condition work stays in the owning prelude and arm work stays inside each arm. Branch-targeted control, loop/try values, branch payloads, nested tees, unreachable placeholders, EH, profiles, and public wiring remain open.
- **Deliverables:**
