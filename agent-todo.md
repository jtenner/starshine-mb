...
| `remove-unused-brs` | Post-repair v131 matrix completed on 2026-07-26. Regular `100000` and wasm-smith are cleanup-normalized green. A red-first follow-up now retires all `181` previously open random-all selectification/result-refinalization/scratch-wrapper cases when replaying the original `870` residuals; the remaining `689` are the already measured smaller Starshine cleanups. The dedicated aggregate still has `10000/10000` raw mismatches, although its aggregate canonical size loss improved from `+538137` to `+496291` bytes. | **Open dedicated parity repair, fresh four-lane renewal, and scheduler reconciliation:** `[AUDIT-CORRECTNESS]001`; Starshine also has one extra slot. |
...
- **Deliverables:**
  ...
  - [x] Run the required four-lane v131 matrix for `remove-unused-brs` (2026-07-26: regular `100000/100000` cleanup-normalized green; wasm-smith `9956` comparable with `9954` direct plus `2` cleanup-normalized and `44` Binaryen-only failures; dedicated `10000/10000` mismatches, every Starshine output canonically larger by `18..140` bytes; random-all `7281` direct + `1849` cleanup-normalized + `870` residuals, of which `689` are measured smaller Starshine dead-write/terminal-return cleanups and `181` were parity gaps). Follow-up replay now matches those `181` cases and leaves only the `689` measured wins from the old random residual set; dedicated replay remains `10000/10000` mismatches with a smaller `+496291`-byte aggregate loss. Keep this item open for dedicated implementation repair and a fresh four-lane renewal.
