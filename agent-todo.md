...
1. Canonical correctness landed.
   - [RUME]003 - Oracle-stable fast-path pass completed - Direct pass execution, CLI flag and registry wiring, focused section/index rewrite coverage, explicit memarg-index rewrites, and canonical compare-harness artifact parity are in place; `remove-unused-module-elements` is now canonically correct against the oracle on the debug artifact.
     ...
     - Remaining blocker after semantic cleanup: the post-fix `wasm-smith` command-failure corpus is still coverage-only work, and the later non-constant-initializer validator failure is now closed. After landing explicit shared-memory support, the memory64 validator fix, and extended-const arithmetic validation, `.tmp/pass-fuzz-rume-constexpr-rerun` reached `153/154` normalized matches with `19` Binaryen command failures and `1` later semantic mismatch before the `20`-failure cutoff. Starshine no longer contributes command failures in that slice. Binaryen still accounts for the `19/20` failure slots: fourteen `invalid type index` cases (`000009`, `000021`, `000024`, `000031`, `000047`, `000050`, `000052`, `000054`, `000056`, `000079`, `000095`, `000124`, `000141`, `000143`), one `Recursion groups of size zero not supported` case (`000029`), two `invalid wasm type: -64` cases (`000081`, `000083`), and two later parser failures at `000162` and `000167`. The new semantic frontier is `.tmp/pass-fuzz-rume-constexpr-rerun/failures/case-000173-wasm-smith`, where Binaryen drops imported-table active elem segments that only write `nullref` while Starshine still roots the imported table through those no-op initializers.
...
2. Do work.
   ...
   - The remaining post-fix work is no longer validator cleanup; it is one later semantic RUME mismatch on no-op active `nullref` elem initializers plus Binaryen parser-compatibility blockers.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [RUN]001 - Triple-Slot Replay and Ordered-Prefix Parity - The core function pass is landed with exact single-pass oracle parity for same-typed block peeling and loop demotion, but the Binaryen default optimize path still needs the repeated RUN slots replayed in the ordered prefix once surrounding passes are in place.
     ...
     - Current blocker: standalone `--remove-unused-names` parity is exact, but Starshine pass time is still roughly `82.722-87.984ms` versus Binaryen at roughly `14.014-14.700ms` on `tests/node/dist/starshine-debug-wasi.wasm`, so the remaining work is now ordered replay plus more serial throughput / GC-churn reduction rather than missing core semantics.
