...
- `RemoveUnusedModuleElements`: closed-world typed `struct.new` / `struct.new_desc`, ref-bearing `array.new` / `array.new_fixed` / `array.new_elem`, mutable `array.set` / `array.init_elem`, ref-bearing `array.fill`, and ref-bearing `array.copy` payloads now defer unread funcref/global/elem references until a live `struct.get*` or `array.get*` read occurs; remaining GC precision is hardening around reference-only non-function retention and any future GC consumer expansion beyond the current struct/array surface.
