...
- The definitive fresh original-input WAGO checkpoint is now `.tmp/wago-o4z-pass8-final14-safe-global-prune-fresh-20260821/`, covering all 1,330 tracked fixtures at or below 2 MiB and the unchanged 766-file stable cohort with native SHA-256 `c0c6ad0b45afa89a286d0adf375bea70f9cde7b3ac1d108dfda26bc17178d67e`. Starshine emits 413,311 bytes versus pinned Binaryen v131 at 415,485, a fresh 2,174-byte / 0.523% win with 110/599/57 smaller/equal/larger, 3,036 positive residual bytes, 46,819 bytes removed from the 460,130-byte baseline, and 104.87% original-gap closure. Final14 applies narrow non-function RUME pruning only to the already-materialized original-simplify-locals scalar-final candidate when a cheap recursive initializer/export/code scan finds an unreferenced defined global. Exactly two stable artifacts improve without regressions: fuzzcase 1820 820 → 814 and fuzzcase 1825 620 → 594. The 1825 result also repairs the prior external stack-validation failure, increasing independently valid Starshine outputs from 1,274 to 1,275; only typed-function-reference `elem.82` remains externally rejected among successful outputs. Instrumented fresh-instance probes expose the internal i64/i32/v128 accumulators and prove 1825 matches input and Binaryen for three repeated 16-result calls, exact zero return bits, alternating accumulator states, and SIMD lanes. Fuzzcase 1820 preserves three repeated returns and exact exported i32/i64/v128 state. Exact final13-vs-final14 100-pair medians are -0.31% for 1820 and -2.07% for 1825. A validating seven-byte 2031 reduction was rejected and removed because it regressed median wall time by 1.46%; the final output remains 547 bytes and measures -0.36% versus final13. The runtime-safe retained projection remains distinct and now projects 412,175 versus 415,485, a 3,310-byte / 0.797% win with 112/598/56 smaller/equal/larger, 1,997 positive residual bytes, 47,955 bytes removed, and 107.41% gap closure. Full validation is 10,601/10,601; all passes 6,951/6,951; command 268/268; formatting, interfaces, native release, API sync, diff checks, external validation, fresh sweep, runtime probes, and exact performance A/Bs are green. Remaining fresh leaders are ifs +1,434, JSON-SIMD +397, primes +378, fannkuch +170, fasta +129, scalar JSON +86, startup JSON +60, urem +38, issue1809 +35, deeply nested struct +25, bump-pointer GC +24, return-call-indirect +21 each, array-copy-inline +19, and fuzzcase 1820/1825 +8/+7. Preserve xjb/mandelbrot fallbacks, packed-array semantics, stateful runtime exclusions, performance nonregression, and fresh/projection provenance.
