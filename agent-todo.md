...
- [FUZ]1010 - GenValid SIMD Exact Opcode And Relaxed-SIMD Widening
  - Status: IN PROGRESS (cron 23, 2026-05-21): representative exact standard-SIMD opcode counters are landed for phase-1/phase-2/phase-3 coverage, the phase-1 counter set has been widened to all currently emitted splats, lane extract/replace families, and core bitwise ops, and the phase-2/phase-3 counter sets now cover the currently emitted standard-SIMD matrix while preserving coarse SIMD phase facts for exact-counted instructions. Latest TDD evidence: `moon test src/validate` first failed on missing phase-2/phase-3 exact fields such as `i64x2_ge_s`, `v128_load8x8_s`, and `f64x2_promote_low_f32x4`; after implementation `moon fmt` and `moon test src/validate` passed. Next run should add relaxed-SIMD emission behind an explicit non-Binaryen-oracle toggle/profile, then add validation fixtures and exact counters for the emitted relaxed-SIMD subgroups.
