...
1. Canonical correctness landed.
   - [RUME]003 - Oracle-stable fast-path pass completed - Direct pass execution, CLI flag and registry wiring, focused section/index rewrite coverage, explicit memarg-index rewrites, and canonical compare-harness artifact parity are in place; `remove-unused-module-elements` is now canonically correct against the oracle on the debug artifact.
     ...
     - Remaining blocker after semantic cleanup: the post-fix `wasm-smith` command-failure corpus is still coverage-only work, but the shared-memory decode family is now closed. After landing explicit shared-memory support, `.tmp/pass-fuzz-rume-sharedmem-rerun` reached `121/121` normalized matches with `0` mismatches before the `20` command-failure cutoff. Starshine now accounts for `5/20` cases: four final-module validation failures `Invalid range for limits` (`000073`, `000117`, `000124`, `000134`) plus one later final-module validation failure `initializer expression is not constant` (`000103`). Binaryen accounts for `15/20` parser failures: twelve `invalid type index` cases (`000009`, `000021`, `000024`, `000031`, `000047`, `000050`, `000052`, `000054`, `000056`, `000079`, `000095`, `000141`), one `Recursion groups of size zero not supported` case (`000029`), and two `invalid wasm type: -64` cases (`000081`, `000083`).
...
2. Do work.
   ...
   - The remaining post-fix work is no longer semantic RUME cleanup; it is upstream fuzz-coverage work on later final-module validation blockers (`Invalid range for limits`, non-constant initializers) and Binaryen parser-compatibility blockers.
