...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [SL]004 - Slot Validation and Artifact Replay - Lock the completed pass into the late slot and prove parity against Binaryen on both focused and large-artifact lanes.
     ...
     - Current blocker: the late-slot regression work is now in-tree and the focused Binaryen oracle lane is green. [`src/passes/registry_test.mbt`](./src/passes/registry_test.mbt) now locks `reorder-locals` as boundary-only, [`src/passes/optimize_test.mbt`](./src/passes/optimize_test.mbt) locks the modeled late `simplify-locals` slot plus `simplify-locals -> vacuum` cleanup, and [`src/passes/simplify_locals_test.mbt`](./src/passes/simplify_locals_test.mbt) now covers overwritten dead `tee` values and sinking across empty linear `block` wrappers. The fresh compare evidence is `.tmp/pass-fuzz-sl-genvalid-10000-sl004`, which completed `10000/10000` compared cases with `10000` normalized matches, `0` mismatches, `0` validation failures, `0` generator failures, and `0` command failures. The remaining blocker is the large-artifact lane on [`tests/node/dist/starshine-debug-wasi.wasm`](./tests/node/dist/starshine-debug-wasi.wasm), not known small-case semantic mismatches.
     ...
     - Reproduce debug-artifact lane: `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --simplify-locals`
