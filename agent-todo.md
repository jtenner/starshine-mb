...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [CP]002 - Rewrite Coverage and Artifact Validation - Implement the move and test it against branchy, trap-sensitive fixtures plus the MoonBit debug artifact.
     ...
     - Current blocker: focused pass/registry/CLI coverage is in-tree for direct `--code-pushing`; `moon test src/passes`, `moon test src/cmd`, `moon info`, and a `1000`-case wasm-smith compare lane are green, and a `10000`-case wasm-smith lane reached `9952/10000` comparable cases with `9951` matches, `0` validation failures, and `1` known non-local mismatch in Binaryen's typed-unreachable block canonicalization. Gen-valid compare lanes and full `moon test` remain blocked by the existing `CannotEncodeNonStandardTypeIndex` generator/invalid-binary failures, and the checked debug WASI artifact is currently rejected by `wasm-tools validate` for too many locals. The optimized WASI artifact compare is semantically green (`Normalized WAT equal: yes`, `Canonical function compare equal: yes`) but still much slower than Binaryen, so the pass is not parity-complete or ready for public optimize preset scheduling.
     - Next tasks:
       1. Fix the shared gen-valid/invalid-binary `Encode(CannotEncodeNonStandardTypeIndex)` failure first so `moon test` and gen-valid compare lanes are usable again.
       2. Re-run direct `code-pushing` gen-valid compare after that fix and save the result path in this backlog item.
       3. Investigate `.tmp/pass-fuzz-code-pushing-wasmsmith-10000/failures/case-009332-wasm-smith`, but keep it separate from local-set motion unless evidence shows `code-pushing` should own typed-unreachable block canonicalization; current trace showed `pass[code-pushing]:done changed=false` for the mismatching function.
       4. Repair or replace the debug WASI artifact compare input because `tests/node/dist/starshine-debug-wasi.wasm` currently fails `wasm-tools validate` with `too many locals`.
       5. Once the lanes are comparable, run `10000` gen-valid plus `10000` wasm-smith direct compare, optimized/debug artifact compare where valid, and only then decide whether to expand the implemented Binaryen surface or keep the pass direct-only.
       6. If correctness is green, profile the optimized-artifact lane; current direct `code-pushing` optimized-WASI compare is semantically green but much slower than Binaryen.
     - Doc: [0066#L213](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L213)
3. Do work.
   - Land the slices above in dependency order in the implementing file(s) and any required scheduler, preset, or dispatcher surfaces.
   - Wire the pass into the exact top-level slot(s) and nested rerun sites documented in the research doc before calling the work done.
