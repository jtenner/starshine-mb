...
- The definitive fresh original-input WAGO checkpoint is now `.tmp/wago-o4z-pass8-final17-empty-void-else-fresh-20260821/`, covering all 1,330 tracked fixtures at or below 2 MiB and the unchanged 766-file stable cohort with native SHA-256 `b02cacf4c9a673339a89f3bf1d0c8470187ca07dc61126d444514d878bbbf92d`. Starshine emits 413,297 bytes versus pinned Binaryen v131 at 415,485, a fresh 2,188-byte / 0.527% win with 110/603/53 smaller/equal/larger, 3,022 positive residual bytes, 46,833 bytes removed from the 460,130-byte baseline, and 104.90% original-gap closure. Final17 recursively omits explicit empty `else` arms only from void `if` instructions during final candidate canonicalization; conditions, then bodies, nested control, labels, effects, and traps are unchanged. Exactly three stable artifacts improve with zero regressions: `many_table_gets_lead_to_gc` 103 → 102 and byte-identical to Binaryen, fannkuch 23,332 → 23,331, and fasta 23,721 → 23,720. A fourth broad-only artifact, `try_table.1`, falls 1,180 → 1,176 after the no-else form enables an exact `if br` → `br_if` canonicalization; it is excluded from stable totals because pinned Binaryen v131 times out. Thirty fresh-instance table/event probes cover five externref values and counts through 100,000. Stateful fannkuch and fasta preserve allocator/event ordering, 131,072-byte memory, and exact full-memory SHA-256 values `b46c752a789beced575e808e38bc59fed14b584a257f686338849381b77c0ea0` and `c5f619ac224a8890782b031dfa981bfcbf7957c9a1726dc4fee33f2750c2b464`. Focused EH calls preserve `simple-throw-catch` returns 23/42 across zero, positive, negative, and max-i32 inputs. WAT diffs for all four artifacts contain only empty-else omission and the resulting exact `br_if` form. Performance is neutral or better: many-table 1,000-pair median -0.78%, fannkuch +0.08%, fasta +0.01%, and try-table +0.09%. Final16’s transitive DFE type-remap repair, final15’s abstract-bottom `ref.test` repair, and final14’s candidate-local global pruning remain unchanged. The runtime-safe retained projection remains distinct and now projects 412,161 versus 415,485, a 3,324-byte / 0.800% win with 112/602/52 smaller/equal/larger, 1,983 positive residual bytes, 47,969 bytes removed, and 107.45% gap closure. Full validation is 10,605/10,605; all passes 6,953/6,953; command 270/270. Remaining fresh leaders are ifs +1,434, JSON-SIMD +397, primes +378, fannkuch +169, fasta +128, scalar JSON +86, startup JSON +60, urem +38, issue1809 +35, deeply nested struct +25, bump-pointer GC +24, return-call-indirect +21 each, array-copy-inline +19, and fuzzcase 1820/1825 +8/+7. Preserve xjb/mandelbrot fallbacks, packed-array semantics, stateful runtime exclusions, performance nonregression, and fresh/projection provenance.
