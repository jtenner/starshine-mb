...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [SL]004 - Slot Validation and Artifact Replay - Lock the completed pass into the late slot and prove parity against Binaryen on both focused and large-artifact lanes.
    ...
    - Current blocker: the late-slot regression work is in-tree and the focused Binaryen lanes are green. `src/passes/registry_test.mbt` locks `reorder-locals` as boundary-only, `src/passes/optimize_test.mbt` locks the modeled late `simplify-locals` slot plus `simplify-locals -> vacuum` cleanup, `src/passes/simplify_locals_test.mbt` now covers the captured-source-local ordering guard, the large-local equivalent-cleanup bailout, the scalar-result tee guard for multivalue local-set producers, the nested consumed-result one-armed `if` lift, the live-intervening-set block-return lift, the pure-write and read-only-trap tee parity cases, the trapping-write barrier guard, and the redundant self-tee-set cleanup case, `src/passes/simplify_locals_wbtest.mbt` now locks sparse pending local-effect footprints for sinkables plus the dense sinkable bulk-clear gate, `src/passes/pass_manager_wbtest.mbt` now locks the direct source threshold gate for the branchy structured call-heavy family plus the native artifact raw reasons for `Func 1058`, `1274`, `1409`, `1410`, `1411`, `1884`, `1949`, `1950`, `2053`, `262`, `4238`, `4239`, `473`, `570`, and `571`, and `src/passes/perf_test.mbt` covers the raw ladder / pure-call-tail escapes plus the widened structured call-heavy, validator-shaped, parser-shaped, local-churn, transformer-shaped, transformer-catch, small-loop, stringview-trim-loop, straight-line builder, straight-line lane-builder, straight-line adjacent-tee, no-local-write, decode-shaped, branchy-decode, and sparse-medium-equivalent-cleanup skips. Oracle evidence stays green at `.tmp/pass-fuzz-sl-genvalid-10000-sl004` (`10000/10000`, `0` mismatches), the refreshed post-perf smoke lane `.tmp/simplify-locals-perf-smoke` (`100/100`, `0` mismatches), the post-nested-child parity lane `.tmp/pass-fuzz-sl-parity-after-nested-if` (`1000/1000`, `0` mismatches), the post-trap/self-tee parity lane `.tmp/pass-fuzz-sl-parity-after-self-tee-fix` (`10000/10000`, `0` mismatches), and the new post-readonly-trap parity lane `.tmp/pass-fuzz-sl-parity-after-readonly-trap-tee-fix` (`10000/10000`, `0` mismatches). Runtime is materially better: raw simplify-locals now bypasses hot lift for the giant multivalue ladder / pure-call-tail families, the validator and decoder structured call-heavy walkers (`Func 298`, `640`, `871`, `901`, `951`, `1103`, `1795`, `1906`, `1921`, `2129`, `3529`), the unchanged branchy structured call-heavy walkers (`Func 1058`, `1274`), the small structured binary decode/encode helpers (`Func 1950`, `2053`), the branchy structured decode fanout (`Func 1949`), the compact string-view trim loop pair (`Func 4238`, `4239`), the large `ModuleTransformer.walk_opt_catch...` family (`Func 1409`, `1410`, `1411`), the loop-heavy validator family (`Func 1106`, `1114`), the parser-shaped local-churn family (`Func 3802`, `3810`), the transformer `ModuleTransformer.apply_action_opt...` family (`Func 1582`, `1588`, `1647`-`1678`, `1685` and siblings), the small-loop call-heavy trio (`Func 3794`, `3885`, `3895`), the giant low-call local-churn body (`Func 3722`), the multivalue parser/result ladders (`Func 353`, `370`, `381`, `409`, `414`, `3764`), the giant straight-line adjacent-tee constructor (`Func 473`), the straight-line builder no-op `Func 262`, the straight-line lane-builder rewrite family now led by `Func 1884`, and broad no-local-write no-op bodies including `Func 570`, `571`, and `1678` traced functions in the current artifact replay; large local-count functions still skip the late equivalent-copy cleanup entirely, and medium `56+` local helper bodies now skip that late cleanup too when copy-equivalence candidates stay sparse; detached-node cleanup no longer pays `hot_delete_node`'s whole-function unreferenced scan once simplify-locals has already proven the node detached; pending sinkable local effects are now tracked as sparse local-id lists with reusable scratch stamps instead of full local-count bit vectors, with dense barrier clears bulk-zeroing the aggregate counters instead of walking every active sinkable effect; synthesized `local.tee` rewrites are now restricted to scalar-result producers so multivalue local-set chains like `Func 1898` stay unchanged under the pass; nested consumed-result trees now also lift one-armed defaultable `if` writes instead of leaving them stranded as `if (Void)` plus later local traffic; pure pending local writes can now sink across trapping loads into the first consuming get, matching Binaryen's `moonbit.bytes_equal`-style tee fold; read-only trapping pending values can now also commute past later read-only trap sites into the first consuming get as a tee, while memory-write barriers still stop that sink; redundant `local.set L (local.tee L value)` pairs now collapse back to a plain `local.set`, matching Binaryen's `moonbit.unsafe_make_string` cleanup; live intervening roots no longer force block-return lifting to choose only the tail local write, so `moonbit.gc.free` can now lift the same block result local Binaryen does; and the raw simplify-locals dispatcher now computes gate stats once per function instead of rescanning the same body for every heuristic. The old `Func 409` traced replay wall is gone, `Func 1884` now lands on `skip-raw reason=straight-line-lane-builder`, and the compact-loop gate bug is understood: raw gate `locals_count` excludes params, so the real `StringView.trim_* .inner` artifact functions only matched after retuning that bound to the actual `34` / `35` local counts. Fresh traced native replays now show `Func 1058` and `1274` on `skip-raw reason=branchy-structured-call-heavy`, `Func 4238` and `4239` on `skip-raw reason=stringview-trim-loop-churn`, `Func 1409` / `1410` / `1411` on `skip-raw reason=transformer-catch-structured-call-heavy`, and `Func 1884` on `skip-raw reason=straight-line-lane-builder`; warmed 30-second runs now land at `1484849us` and `1391794us`, and the remaining runtime residue is led by `SourceLocRepr.parse` (`Func 4499`), `json.ParseContext.lex_zero` (`Func 3769`), and the smaller helper band around `Func 1867`, `1897`, and the now-preserved `Func 1898`. The native pass suite still SIGSEGVs before the `#cfg(target="native")` artifact whitebox tests can act as a reliable gate, so the direct native CLI replay remains the source of truth for this blocker. Parity also improved on the artifact lane: the earliest `--simplify-locals` self-opt mismatch moved from normalized-WAT line `379` to `1855`, then to `2204`, then to `2453`, and now to `4029`, retiring the earlier `tlsf/searchBlock`, `moonbit.bytes_equal`, `moonbit.unsafe_make_string`, and `moonbit.gc.free` block-result plus trap-tee gaps. The next parity work is absolute `Func 216` (`_M0FP37jtenner9starshine3cmd35run__cmd__exit__code__with__adapter`), where Binaryen lifts an `if (result i32)` directly into a call argument while Starshine still materializes it through `local.set 3`, followed by the sibling compare/parity lane.
