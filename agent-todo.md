...
- Starshine wasm artifact implemented-pass comparison:
  ...
  - fresh rebuilt release-artifact status:
    ...
    - post-merge simple type compaction is now landed too: Starshine drops to `2469198` bytes, `2875` defined functions, and `109` printed types on the fresh artifact.
    - the remaining DFE parity gap on that fresh artifact is now almost entirely in call/index layout rather than type duplication: Binaryen lands at `2376579` bytes with `2892` defined functions and `110` printed types, while Starshine is still `92619` bytes larger, made up of a `92077`-byte code-section delta and a `561`-byte element-section delta, partially offset by slightly smaller type/function metadata.
