...
0. Shared blocker on canonical artifact parity.
  - [HOT]001 - Post-SSA Hot-Pipeline Replay Hardening - The large-artifact `DFE -> RUME -> MP -> OR -> GR -> GSI -> SSA` prefix now completes, validates, and canonicalizes under Binaryen: `ssa-nomerge` no longer aborts on synthetic predecessor copies, duplicate dead-local cleanup, or loop fallthrough result lowering, and the first detached-shared-subtree DCE abort is also fixed. The remaining ordered full-chain blocker has moved downstream: after post-SSA cleanup passes mutate the module, a later hot pass aborts while re-lifting the emitted function body in `hot_lift_func`, so compare signoff is now blocked on hot-lift / later-pass replay rather than SSA or Binaryen canonicalization of the post-SSA artifact.
     ...
     - Current blocker: direct post-prefix `--ssa-nomerge` replay on `.tmp/self-compare-prefix-6/starshine.raw.wasm` improved from `474.513s` to `428.777s` after caching `HotModuleContext` / resolved function types, skipping lower on unchanged hot passes, and removing the worst use-def / phi-placement quadratic scans, but the run is still orders of magnitude over budget. The remaining sink is hot-lift inner-loop work and allocation churn, not the SSA rewrite body, and grouped hot-pass batching is not yet safe because later cleanup passes still expose re-lift verification failures.
