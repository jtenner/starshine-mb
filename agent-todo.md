...
3. Test against binaryen.
   ...
   - For future coverage-only RUME sweeps, use `bun scripts/pass-fuzz-compare.ts --pass remove-unused-module-elements --keep-going-after-command-failures ...` when the goal is to keep collecting comparable cases after known Binaryen parser/canonicalization failures; command failures remain recorded and replayable but no longer spend the mismatch/failure cutoff in that explicit mode.
