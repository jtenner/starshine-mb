...
- **Latest bounded iteration (2026-07-15):** nonthrowing synthetic catch-all lowering and recursive suffix ownership remain the current narrow measured output win (`212` Starshine cleanup bytes versus `236` Binaryen). Profiler-backed follow-ups admit direct `i32.or`/`i32.xor`, cache immutable EH prerequisites once per run, consume the admission-time effective-terminal suffix proof, and cache each exact scalar legacy-try support decision through mutation. The EH prerequisite now drives distinct `DeferredCatchPayloadRepair` and `DeferredExceptionalTransferRepair` admission outcomes before any mutation; a red-first test exposed and closed prior partial operand/local rewriting in the presence of rooted catch or rethrow markers. The post-gate representative pass-only median is `1,347.5 us` (`5.06x` Binaryen v130's `266.05 us`), still outside the `<=2x` target. Typed catches/payloads, nested-pop repair, `rethrow`, `delegate`, structured suffix control deletion, broader behavior, a GenValid profile, four-lane signoff, performance, and public wiring remain open. Source: `docs/wiki/raw/binaryen/2026-07-15-flatten-version-130-nonthrowing-bridge-suffix-cache-impact.md`.
- **Deliverables:**
