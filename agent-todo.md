...
- `RemoveUnusedModuleElements`: closed-world typed `struct.new` / `struct.new_desc`, ref-bearing `array.new` / `array.new_fixed` / `array.new_elem`, mutable `array.set` / `array.init_elem`, and ref-bearing `array.fill` payloads now defer unread funcref/global/elem references until a live `struct.get*` or `array.get*` read occurs; remaining GC precision is broader source/content readers such as `array.copy` and any additional hardening around reference-only non-function retention.
