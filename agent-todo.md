...
- [DAE]006 - Both-Canonical Frontier Advancement
  ...
  - Current classification:
    ...
    - `0583-2026-05-26-dae-func505-overflow-temp-reduction.md` reduced the overflow/error-return subshape and classifies it as representation-only unsigned comparison polarity plus temporary-local drift: Starshine's `acc > limit` and Binaryen's `limit < acc` guard the same error-construction path around the same `((-1 - digit) / 10)` limit expression. No pass behavior or compare normalizer changed; the overall Func505 frontier remains unknown/risky because loop induction/exit-carrier drift is still live.
  - Next action:
    - Reduce the remaining Func505 loop induction/exit carrier before changing pass logic or compare tooling.
    - If a focused fixture proves semantic equivalence for a subshape, add the smallest TDD pass cleanup or a narrow diagnostic-only normalizer; otherwise keep the frontier classified unknown/risky with exact artifact evidence.
  - Deliverables:
