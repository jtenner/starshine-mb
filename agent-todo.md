...
- [FUZ]1021 - Invalid Binary Malformed-Byte Matrix
  - Status: IN PROGRESS (cron 23, 2026-05-21). Latest SIMD operand-context slice adds decode-rejected, non-smoke `malformed-simd-load-memarg-align-uleb`, `malformed-simd-store-memarg-align-uleb`, `malformed-simd-load-memarg-offset-uleb`, `malformed-simd-store-memarg-offset-uleb`, `overwide-simd-load-memarg-align-uleb`, `overwide-simd-store-memarg-align-uleb`, `overwide-simd-load-memarg-offset-uleb`, and `overwide-simd-store-memarg-offset-uleb`, which emit valid `0xFD 0x00` `v128.load` / `0xFD 0x0B` `v128.store` subopcodes before corrupting the following memarg alignment or offset ULEB; docs now name SIMD malformed and overwide alignment/offset operand-context coverage. Previous atomics malformed/overwide load/store align/offset memarg, atomics reserved-subopcode byte, section-payload underflow, atomic-prefixed subopcode malformed/overwide, overwide section vector-count, malformed vector-count, and smoke-scheduler slices are landed and documented; continue FUZ1021 by selecting another malformed-byte class such as remaining invalid UTF-8, opcode/prefix, lane, additional SIMD operand-context corruptions, or additional atomics operand-context corruptions. Current TDD evidence: focused invalid-binary wbtest first failed on missing SIMD memarg strategies, then `moon test src/fuzz` passed after implementation.
