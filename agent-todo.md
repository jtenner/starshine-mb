...
- [SGO]003C - Full Read-Only-To-Write FlowScanner Parity
  ...
  - Subtasks:
    ...
    - `[SGO]003C5` loop-specific scanner: direct loop self-guards and simple pure adjacent consumers are covered, and the narrow independent `const; global.set <other>` value-loop prefix from 0638/0677, the Binaryen-positive independent `const; local.set` prefix from 0678, and the constant-operand `i32.store` prefix from 0679 are now implemented without reusing the full block scanner; future loop broadening still needs exact Binaryen-positive evidence plus branch/trap/effect guardrails;
