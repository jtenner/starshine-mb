...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [RUB]002 - Multi-Slot Cleanup and Artifact Compare - Replay the pass in each Binaryen slot and confirm later `merge-blocks` opportunities match the reference output.
     ...
     - Current blocker: the top-level scheduler gap is now fixed: `optimize` and `shrink` both replay `remove-unused-brs` in three modeled slots, focused preset tests cover those trace counts, and the old `Func 24`, `Func 299`, `Func 1329`, and `Func 7756` serial verifier failures are fixed in-source. `Func 7756` was a multi-value branch-payload rewrite bug: RUB now refuses to collapse a branch-payload `if` when the surviving payload side would be forwarded as fewer branch slots than the original `br` carried. The old single-pass artifact gate at `Func 807` is no longer red either: a native `cmd` regression now replays `--remove-unused-brs` on `tests/node/dist/starshine-debug-wasi.wasm` through `run_cmd_with_adapter` and validates the output module in-memory. The first saved oracle family is now fixed too: Binaryen-style explicit `nop` bodies in trivial `if` arms are preserved in-source and covered by focused pass tests. But the oracle gate is still red overall: `.tmp/pass-fuzz-rub-genvalid-1000` hit the `20`-mismatch cutoff after `31` compared `gen-valid` cases, with `0` command failures and `0` validation failures, and the fresh spot-check `.tmp/pass-fuzz-rub-genvalid-100-after-if-arm-nop-fix` improved to `97/100` normalized matches but still left `3` mismatches. The remaining work is now the next saved mismatch families plus ordered-prefix proof/runtime follow-up, not a known direct-pass validate break.
     ...
     - Reproduce the fresh post-fix spot-check: `bun scripts/pass-fuzz-compare.ts --pass remove-unused-brs --generator gen-valid --count 100 --max-failures 5 --out-dir .tmp/pass-fuzz-rub-genvalid-100-after-if-arm-nop-fix`
     - Inspect the next saved mismatches: `.tmp/pass-fuzz-rub-genvalid-100-after-if-arm-nop-fix/failures/case-000021-gen-valid`, `.tmp/pass-fuzz-rub-genvalid-100-after-if-arm-nop-fix/failures/case-000050-gen-valid`, and `.tmp/pass-fuzz-rub-genvalid-100-after-if-arm-nop-fix/failures/case-000066-gen-valid`
     - Doc: [0066#L188](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L188)
