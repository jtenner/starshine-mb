...
- [O4Z-AUDIT-MP] - Widen `memory-packing` to Binaryen behavior parity
  - Status: reopened v0.1.0 release-gating audit. Direct compare and the saved active-segment artifact slot are green, but the pass dossier still says Starshine lacks Binaryen's passive-segment and segment-user half.
  - Scope: passive segment splitting/remapping, `memory.init` rewrites, `memory.fill` zero-slice insertion, `data.drop` expansion for split passive segments, lazy drop-state globals, imported-memory `zeroFilledMemory`, GC `array.new_data` / `array.init_data` conservatism, and `MaxDataSegments` limiting.
  - Deliverables: apply the common checklist; add source-backed passive/user fixtures before behavior changes; classify each remaining active-only or segment-user difference as implemented behavior, representation-only drift, tool-blocked, or explicit non-goal.
