...
| `redundant-set-elimination` / `rse` | Direct behavior, 1x timing, and the canonical late O4z scheduler slot are closed. The public optimize/shrink rosters run `heap-store-optimization -> redundant-set-elimination -> vacuum -> dae-optimizing`. | None for v0.1.0. |
| `dae-optimizing` | Active-partial; all numeric selected production admissions are removed, the Binaryen-v131 direct four-lane matrices are complete for both variants, and the optimizing suffix now reuses the shared Binaryen-v131 O4z default-function roster after its required `precompute-propagate` prefix. The retained artifact is valid/idempotent at `2,991,168` bytes; the shared-roster replay measured `47.956s` productive and `44.490s` byte-identical. Exact one-based slot `48` and public O4z execution are covered. Remaining blockers are phase-sensitive `1024/4096` and touched/module-size policies, artifact gross-positive/remap renewal, accepted medians, full preset-artifact replay, and release integration. | **Open implementation-policy, performance, artifact-preset, and release closeout.** |
...
- **Status:** production no longer admits work by numeric `FuncNNN`; high/low selected lists, caller bands, result cardinality order, adjacent windows, nullable prefixes/gates, and major productive caps are removed or replaced by exact dependency/shape frontiers. Current deterministic totals are `252/252`, `346/346`, `6215/6215`, and `9693/9693`. Native SHA-256 `95daa8811dceffee74da3082cfb765e17b2d7497db27aa54090ccb94fce42e8c` completes explicit Binaryen-v131 regular `100000`, dedicated `10000`, wasm-smith `10000`, and random-all `10000` lanes for both plain DAE and DAEO. Both regular lanes are exact `100000/100000`; wasm-smith has zero mismatches and only the known `44` Binaryen failures; all dedicated/random residuals are classified smaller or equal-size fewer-local Starshine wins with zero validation/semantic failures. The optimizing nested suffix now prepends public `precompute-propagate` and reuses `inlining_nested_function_pipeline_passes(4, 4)`, with touched adapters for merge-locals/local-subtyping and validated final tuple/terminal-return repair. Native SHA-256 `931e6ea8b0160316aff4200cfcc4fdb46d672b9145a36873103e489231757fc2` preserves the `93 + 1 + 6` v131 random-all smoke and the valid byte-idempotent `2,991,168`-byte artifact in `47.956s` / `44.490s`. Public optimize/shrink registry and runtime rosters place DAEO exactly once at one-based slot `48`, after `heap-store-optimization -> redundant-set-elimination -> vacuum` and before `inlining-optimizing`. Open work is phase-sensitive `1024/4096` and touched/module-size policy, refreshed gross-positive/remap and performance medians, full preset-artifact replay, full repository gate, and release metadata.
...
- **Why:** O4z slot 48 and public late-tail execution are now proven, but gross direct artifact parity, nested scheduler runtime, preset-artifact replay, and release integration remain open.
- **Deliverables:**
  ...
  - [x] Implement the touched-function nested `precompute-propagate -> default O4z function pipeline` using the shared inlining scheduler roster; preserve touched-only merge/local-subtyping adapters and validated tuple/terminal-return closure.
  - [ ] Remove or narrowly justify large-module/touched-set guards with runtime and performance evidence.
  - [x] Keep direct DAE and `dae-optimizing` profiles/results separate; renew both variants against explicit Binaryen v131 at release scale.
  ...
  - [x] Complete current four-lane direct and optimizing closeout.
  - [ ] Complete the ordered slot-48 full-artifact replay; exact registry placement and public O4z execution are covered by focused tests.
...
- **Status:** all ten `DaeWorkFamily` labels now have concrete dispatcher handlers; reason-coded decisions/counters, the coherent immutable original snapshot, and the mutable current boundary graph with module/call/signature/body epochs are implemented. The dependency graph records direct and direct-tail edges, exact typed and epoch-stamped caller-parameter-to-callee-parameter operand-slice edges and their deterministic family-component closure, exact uniform-value caller invalidations, conservative result/type/body/cleanup domains, and normalized Tarjan SCCs. A monotone per-slot solver merges forwarded-only, uniform, conflicting, and blocked states and attaches solved values to epoch-bearing uniform work items. `dae_process_work_item(...)` owns common stale refresh, no-change/deferred/commit outcomes, diagnostics, computed/localized, solved uniform-literal, GC parameter, GC result, dropped-result, and post-cleanup boundary-revisit production dispatch, plus atomic forwarding. The low exact-parameter GC chain and terminal result-caller closure now consume those GC families; broad GC batches and remaining exact result/type/body/cleanup extraction still need common plans.
...
- **Status:** the ordinary core full-module forward rescans are replaced by a deterministic queue over current caller/callee invalidations. Binaryen's source-backed one-wave single-caller profitability stop remains between queue frontiers. The shared work-item model carries target/component/exact members/family, solved per-slot values, and expected epochs; the common dispatcher owns computed/localized and solved uniform-literal candidate execution, production forwarding transactions over exact collector-provided members, stale refresh, explicit deferred termination, diagnostics, and post-commit invalidation requeue. Broad original-shape identity and scalar constructor seeds now solve current values after the relevant forwarding frontier at every module size; the former `defined <= 1024` disablement is removed. This generically removes the former fixed definitions `4593, 4592, 4591, 4589, 4588, 4587, 4586, 4584` without preempting the computed-cycle proof. Small-module immutable globals now carry policy on uniform work items and run dispatcher-backed waves to exhaustion, removing their selected scan and `64` cap. Broad immutable/suffix work, broad GC batches, control/type repair, broader cleanup invalidations, and other selected artifact lanes still use separate phases, arrays, bands, or caps. Dropped-result candidate waves now run at every module size; only their `4096` ordering split remains cardinality-based. Fact-selected dropped results now run dispatcher-backed waves to exhaustion, removing the `32`/`14`/`8` caps and selected definitions `3566`, `3732`, and `3814`. High-payoff wrapper/callee result cleanup is also module-size independent and uncapped: both `8192` gates and both two-item limits are removed, with a 513-definition regression. Ordinary dropped-result ordering no longer switches at `4096`; exact current caller dependencies order wrappers before candidate callees. Adjacent high-payoff local cleanup no longer uses the `4097..8199` pair band or preceding-eight root window; exact call-sink uniform commits provide its cross-phase root frontier. The terminal GC-result closure no longer has its depth-`16` cutoff and runs dispatcher-backed dependency postorder to exhaustion. Small reverse/low/broad exact-literal and large immutable/global production caps `8/64/64/512` are removed. Broad nullable exact-literal discovery runs after ordinary reverse work at every module size and covers the full definition range instead of the former `>4096` / first-`4096` policy. The low-callee caller prefix `64`, shifted range `64..128`, their former `14/21` caps, and fixed shifted-wrapper list `128, 131, 133, ... 196` are all gone; exact wrapper-body/type discovery plus ordinary dependencies own the behavior. The obsolete exact-parameter definitions `37/38/41` and touched-definition `164` trigger are deleted because ordinary direct-GC discovery already owns the behavior byte-identically. Selected unread definitions `505/538/3799`, the mid/high exact-literal lists, and selected exact-literal/default definitions `326/327/328/1795` are removed. Generic nontrapping pure-expression seeds plus phase-local changed-body direct-callee invalidation own their behavior without unrestricted module admission. The artifact-only definitions `3737/3765`, caller `281`, and constant-`1024` bridge are deleted as redundant with generic forwarding/uniform and unread ownership. The definition-`505` rewritten-control switch is removed; branch-safe simplification is generic after validated parameter rewrites. The selected zero-tee list `215/260/261/1137/1978` is deleted after explicit Binaryen-v131 direct-DAEO probes showed the tee shape should remain and the current artifact proved the lane unproductive. All definition-`237` production scheduling is removed as byte-identical dead policy; its helper corpus is regression-only pending shared-pass absorption. Selected production schedules `246/287/867` are removed as byte-identical dead policy. Definitions `233/256` are removed after explicit v131 direct-DAEO probes showed Starshine's selected constant-if/exported-loop cleanup was non-oracle. Definition `236` is no longer selected: generic changed, straight-line, call-free local compaction enters isolated nested cleanup and reaches exact v131 output. Definitions `267/268` now use monotone generic constant-equality/if folding after callee rewrite instead of a selected eight-step loop. Func408 single-use i32 constant-local cleanup is original-shape seeded rather than identity-gated and reduces the retained artifact by 60 bytes; its selected nop schedule is removed to match direct v131 preservation when no boundary changes. Exported definitions `288/298/299` preserve their v131 switch-carrier/return-wrapper/loop-carrier/inverted-if shapes rather than running non-boundary selected cleanup; the fixed Func288 local map is no longer production-reachable. Func408 pair-if reconstruction now uses its exact recognizer on touched definitions and passes at arbitrary definition `709`. Former Func311/Func313 terminal-call cleanup is seeded by original shape plus exact current-body change, batched to fixpoint, and passes at arbitrary definitions `711/713`; its internal `4/4/9` body caps are removed in favor of pattern exhaustion. Unrestricted whole-module admission was rejected after a `~54s` to `~139s` productive-runtime regression. Former Func327/Func1794 const-if, terminal-local, and default-wrapper cleanup now uses changed-callee or exact foldable-const-if evidence, passes at arbitrary definition `727`, and has no `2`/`8` local caps. The final Func323 production identity is replaced by exact single-caller plus immutable-original body-change evidence and passes at arbitrary caller/callee definitions `813/823`; no numeric `selected_scratch_defs[...]` production admission remains. Forwarding-component discovery itself is uncapped: the former 8-component, 64-node, 8-round, 129-instruction, and bounded sink/cycle retry limits are removed in favor of monotonic exhaustion.
...
- **Reason:** the baseline inventory still finds hard-coded cleanup chains, exact-parameter definitions, local maps, and module/prefix admission windows even though the major productive-attempt caps are now removed.
...
- **Reason:** direct v131 four-lane evidence is now complete, but DAEO still has a DAE-local 22-step roster, broad touched/size skips, unaccepted artifact/runtime medians, open preset/public execution, and release metadata gates.
