...
3. Test against binaryen.
   ...
   - The accumulated command-failure corpus now stays separately replayable while fuzzing RUME: Starshine decoder/validator coverage can be filtered via `starshine-invalid-limits` and `starshine-invalid-range-for-limits`, while the Binaryen parser-compatibility families remain split across `binaryen-invalid-type-index`, `binaryen-rec-group-zero`, and `binaryen-invalid-wasm-type-neg64` for follow-up outside semantic mismatch reduction.
