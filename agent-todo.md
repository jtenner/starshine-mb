...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [CP]003 - Scheduler, Registry, And Early-Slot Replay - Wire the pass into Starshine's active optimization surfaces.
     - Status: landed. `code-pushing` is now an active hot pass in `src/passes/optimize.mbt`, no longer lives in the removed-pass set, replays once in the early `optimize` / `shrink` slot after `precompute`, and is covered by registry, preset-trace, and direct CLI tests.
   ...
   - [CP]004 - Binaryen Parity Proof And Follow-On Gaps - Close the compare loop and keep deferred oracle gaps explicit.
     - Status: in progress. The named compare-pass lane is now available in `scripts/pass-fuzz-compare.ts` / `bun fuzz compare-pass` via `--pass code-pushing`, and the first local smoke run `bun scripts/pass-fuzz-compare.ts --pass code-pushing --count 1 --generator gen-valid --min-compared 1 --max-failures 1 --out-dir .tmp/pass-fuzz-code-pushing-smoke` finished with `1/1` compared cases, `1` normalized match, `0` mismatches, and `0` command failures. The regenerated debug artifact still re-enables direct compare too. After fixing the artifact-invalid rewrite by refusing same-region root reordering inside non-void regions and refusing sinks into result-producing `if` arms, native `--code-pushing --out /tmp/code-pushing-validate.wasm tests/node/dist/starshine-debug-wasi.wasm` validates again, `src/cmd/cmd_test.mbt` now has a native debug-artifact regression for `--code-pushing`, and the latest direct compare now writes `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-174855`, where canonical wasm and normalized WAT still differ and Starshine remains far over Binaryen's runtime (`4869.181 ms` pass vs `52.840 ms`; `7268.019 ms` total vs `376.812 ms`).
    ...
    - Current blocker: exact TNH / implicit-trap parity needs `HotPipelineOptions` or equivalent hot-pass policy plumbing; the GC `ref-into-if` family is follow-up work until local-type repair is representable in the current pipeline. The latest direct compare is [`.tmp/self-opt-code-pushing-call-fed-carrier-20260408`](./.tmp/self-opt-code-pushing-call-fed-carrier-20260408), where the old `48978` dropped-carrier family is gone and the first normalized WAT deltas are now `72005`, `105621`, and `126757`, with the pass still slower than Binaryen (`1190.423 ms` vs `55.888 ms` pass; `4033.438 ms` vs `431.717 ms` total). The remaining `72005` blocker is still the explicit-exit `LocalSet(Block(...), LocalGet)` terminal-owner family: one exact terminal-inner-owner reducer is landed and safe, but the closer call-prefixed live-shaped variant is still blocked because widening it reintroduced the earlier drift around `71748`. The larger `105621` and `126757` structural diffs still look like Binaryen alias-local synthesis rather than another missed safe same-region reorder. The combined cleanup oracle is unblocked again too: `simplify-locals` no longer aborts or emits an invalid module on the debug artifact, native `cmd` regressions now cover `--simplify-locals` directly, and both native `--simplify-locals` and `--code-pushing --simplify-locals` validate on `tests/node/dist/starshine-debug-wasi.wasm`. But the combined replay [`.tmp/self-opt-code-pushing-plus-simplify-locals-20260408`](./.tmp/self-opt-code-pushing-plus-simplify-locals-20260408) is still canonically and normalized red, with Starshine much larger (`1956918` bytes vs Binaryen `1791630`) and still slower (`1145.246 ms` vs `399.478 ms` pass). So the next correctness work should stay on the remaining `72005` family or the structural alias-local synthesis lane, not on reviving more broad guard churn inside `code-pushing`.
      - The new dead-gap `gen-valid` regression family is now retired. After narrowing the dead-gap conflict rule so pure crossed `global.get` reads no longer block a dead `local.set`, `src/passes/code_pushing_test.mbt` now pins the exact saved shape from `case-000113-gen-valid`, and the refreshed named lanes are green at [`.tmp/pass-fuzz-code-pushing-genvalid-1000-20260408c`](./.tmp/pass-fuzz-code-pushing-genvalid-1000-20260408c) (`1000/1000`) plus [`.tmp/pass-fuzz-code-pushing-genvalid-10000-20260408b`](./.tmp/pass-fuzz-code-pushing-genvalid-10000-20260408b) (`10000/10000`), both with `0` mismatches, `0` validation failures, and `0` command failures.
      - The mixed and smith-only fuzz lanes are now semantically green on the current pass state too. [`.tmp/pass-fuzz-code-pushing-both-1000-20260408b`](./.tmp/pass-fuzz-code-pushing-both-1000-20260408b) finishes `289/1000` compared with `289` normalized matches, `0` mismatches, and `20` Binaryen-side `wasm-smith` command failures; [`.tmp/pass-fuzz-code-pushing-smith-1000-20260408b`](./.tmp/pass-fuzz-code-pushing-smith-1000-20260408b) finishes `165/1000` compared with `165` normalized matches, `0` mismatches, and the same `20` Binaryen parser/canonicalization failures (`invalid type index`, `rec-group-zero`, `invalid wasm type -64`). So the remaining parity blocker is back to the debug artifact, not the fuzz lanes.
      - The latest direct compare is now [`.tmp/self-opt-code-pushing-call-fed-carrier-20260408`](./.tmp/self-opt-code-pushing-call-fed-carrier-20260408). Canonical wasm and normalized WAT still differ, but the old `48978` / printed `func $127` / pipeline `Func 148` dropped-carrier family is now gone after the new narrow extraction rewrite. `code-pushing` now handles two deliberate dropped-carrier extraction subfamilies in-tree: direct alias `LocalSet(LocalGet ...)` extraction, and a narrow `i32` call-fed `LocalSet(Call ...)` extraction that splits to `LocalTee(temp, Call ...)` inside the carrier plus a later alias set after the sibling `if`, while still refusing carrier prefixes with explicit exits. The first remaining normalized WAT deltas are now `72005`, `105621`, and `126757`, and Starshine remains over Binaryen on both whole-pass and whole-command time (`1190.423 ms` vs `55.888 ms` pass; `4033.438 ms` vs `431.717 ms` total). The larger structural families are clearer too: the `105621` and `126757` diffs in printed `func $314` and `func $366` are not just missed same-root reorders. Binaryen is materializing extra alias locals before the shared `if`, increasing local count and then forwarding those aliases after the `if`. If that is the real intended frontier, exact parity there may need local synthesis or a neighboring pass, not another guard-only widening inside current `code-pushing`.
      - One narrower terminal-owner subfamily is now landed too. `src/passes/code_pushing_test.mbt` and `src/passes/code_pushing.mbt` now admit the exact earlier explicit-exit `LocalSet(Block(...), LocalGet)` carrier where the payload `if` has one terminal arm and one direct inner-owner exit arm, but only when the trailing `local.get` is the same payload-written local. The kept replay [`.tmp/self-opt-code-pushing-terminal-inner-owner-20260408`](./.tmp/self-opt-code-pushing-terminal-inner-owner-20260408) stays valid and keeps the same live frontier (`72005`, `105621`, `126757`), so that exact reduced family is safe but still not the real artifact blocker.
      - A closer live-shaped retry is now explicitly rejected. The current `Func 238` blocker still has a call-prefixed inner block before the final payload `br`, and widening that exact call-prefixed carrier wrote [`.tmp/self-opt-code-pushing-call-prefixed-carrier-20260408`](./.tmp/self-opt-code-pushing-call-prefixed-carrier-20260408) and reintroduced an earlier structural drift around printed line `71748`. So the next `72005` step is no longer “admit call-prefixed terminal inner-owner carriers”; that family is now narrowed and blocked too.
      - The downstream cleanup oracle is unblocked again. `simplify-locals` no longer aborts or emits invalid wasm on `tests/node/dist/starshine-debug-wasi.wasm`: dead `local.tee` rewrites now require a sole `Drop` stack use, dead `local.set` rewrites now require that the written local is unread anywhere in the function, and `src/cmd/cmd_test.mbt` now has a native debug-artifact regression for direct `--simplify-locals`. Fresh native `cmd` runs validate both `--simplify-locals` and `--code-pushing --simplify-locals`, and the combined replay [`.tmp/self-opt-code-pushing-plus-simplify-locals-20260408`](./.tmp/self-opt-code-pushing-plus-simplify-locals-20260408) now completes instead of failing early. That combined compare is still red though: canonical wasm and normalized WAT both differ, Starshine is still larger (`1956918` bytes vs `1791630`), and pass time is still behind (`1145.246 ms` vs `399.478 ms`). So this fix reopens the cleanup oracle lane, but it does not yet turn the remaining structural families into a code-pushing signoff.
      - `simplify-locals` has one more safe cleanup slice now: it can fold a top-level pure `local.set` into its lone later top-level `local.get`, and the reduced pass repro is pinned in `src/passes/simplify_locals_test.mbt`. The broader structured `LocalSetDef` forwarding version is explicitly not kept because native `--simplify-locals` replay on `tests/node/dist/starshine-debug-wasi.wasm` reintroduced stack-underflow (`Func 1910` / earlier `Func 104`). The kept root-only version validates on the debug artifact and retires the saved fuzz alias case, but the named `gen-valid` lane [`.tmp/pass-fuzz-simplify-locals-genvalid-500-20260408f`](./.tmp/pass-fuzz-simplify-locals-genvalid-500-20260408f) still sits at `27/47` normalized matches because the remaining mismatches are mostly non-parity cleanup differences where Starshine emits `nop` and Binaryen keeps `drop (const ...)`. Treat those as TODO canonicalization evidence, not current safety blockers.
      - The next direct `simplify-locals` artifact frontier is now pinned too. `src/passes/simplify_locals_test.mbt` keeps the same-local result-`if` storeback shape unchanged: `local.set L (if (result T) cond then V else (local.get L))`, followed by a later `local.get L`. That matches the early direct compare families around [`.tmp/self-opt-simplify-locals-20260408c`](./.tmp/self-opt-simplify-locals-20260408c) where Binaryen turns a storeback `if (result)` into a narrower conditional update or use-site forwarding. The next meaningful simplify-locals work should reduce that family, not reopen broader dead-local-set relaxations.
      - That same-local storeback family is now partially landed. `src/passes/simplify_locals.mbt` rewrites the root-level shape `local.set L (if (result T) cond then V else (local.get L))` into a void `if` whose `then` arm writes `L`, and `src/passes/simplify_locals_test.mbt` now keeps that reducer green. Native `--simplify-locals` replay on the debug artifact still validates, and the direct compare [`.tmp/self-opt-simplify-locals-20260408d`](./.tmp/self-opt-simplify-locals-20260408d) now moves the old `529` storeback family toward Binaryen by removing the outer result-`if` storeback wrapper. The remaining simplify-locals frontier is therefore narrower again: the first diff family at `214` is still intact, and the later `641` family still needs deeper use-site forwarding than this rewrite provides.
      - The next simplify-locals reducer is now pinned too. `src/passes/simplify_locals_test.mbt` keeps the simpler immediate-use producer shape unchanged for now: a root-level `local.set` whose child is a result-`if`, followed immediately by a root that consumes that local via `local.get`. A broader structured forwarding attempt was not kept: native `--simplify-locals` replay on `tests/node/dist/starshine-debug-wasi.wasm` aborted during lowering because cloned control producers still carried live label structure the local clone helper did not remap. The kept in-tree boundary now includes both the simple immediate-use case and the nested-explicit-exit variant, so the next meaningful simplify-locals work should reduce the real live `641` family more directly instead of reopening broad structured-forwarding probes.
      - One narrow immediate-use forwarding slice is now landed safely. `src/passes/simplify_locals.mbt` now moves a root-level `local.set` of a result-`if` into the lone immediate later use slot when the use path is unique, the crossed prefix is pure, and the producer `if` has no nested explicit exits. This uses the original HOT value node instead of cloning it, so the debug artifact stays valid under native `--simplify-locals`. The fresh replay [`.tmp/self-opt-simplify-locals-20260408h`](./.tmp/self-opt-simplify-locals-20260408h) shrinks Starshine from `1958511` to `1958408` bytes, but it still does not move the old `641` family, so that live frontier remains the next real simplify-locals target.
      - One tee-preserving immediate-use slice is now landed too. `src/passes/simplify_locals.mbt` can now move a root-level `local.set` of a result-`if` into the immediate later use as a `local.tee` when the crossed path is the single child-chain into the next root and every remaining read of that SSA value stays in a later root of the same holder. `src/passes/simplify_locals_test.mbt` now keeps that reducer green, native `--simplify-locals` replay on `tests/node/dist/starshine-debug-wasi.wasm` still validates, and the fresh compare [`.tmp/self-opt-simplify-locals-20260408i`](./.tmp/self-opt-simplify-locals-20260408i) trims Starshine again from `1958408` to `1958404` bytes. But the first direct hunk at `214` is still unchanged, so the next meaningful simplify-locals work is no longer the plain tee-preserving lane; it is the still-open multi-root storeback/extraction family that also feeds the later `$2` set before the final consumer.
      - That tee-preserving mover is now broader in one safe direction too. `src/passes/simplify_locals.mbt` now admits read-only structured producers, not just result-`if` producers: a root-level `local.set` whose value subtree uses only local reads plus structured control with no explicit exits and no nested local writes can also move into the immediate later use as a `local.tee` under the same single-child-chain / later-same-holder-read constraints. `src/passes/simplify_locals_test.mbt` now keeps the pure producer reducer green, native `--simplify-locals` replay still validates on `tests/node/dist/starshine-debug-wasi.wasm`, and the fresh compare [`.tmp/self-opt-simplify-locals-20260408j`](./.tmp/self-opt-simplify-locals-20260408j) cuts Starshine much further from `1958404` to `1950336` bytes. The first direct hunk at `214` is still unchanged though, so this broader read-only structured mover is useful cleanup, not the final fix for the remaining multi-root storeback/extraction family.
      - That remaining multi-root family is now pinned more directly too. `src/passes/simplify_locals_test.mbt` now keeps a closer `214`-style chain in-tree: same-local result-`if` storeback, then the `$2`-style pure local-set fed by the updated local, then the final consumer local-set. Current behavior is explicit there now: `simplify-locals` narrows the first root into a void `if`, but still leaves the later two `local.set` roots in place. So the next meaningful step is to decide whether that live family wants a deliberate multi-root extraction rewrite or a bounded second simplify-locals round, instead of reopening more one-root forwarding variants blindly.
      - That decision point is now explicit too. `src/passes/simplify_locals_test.mbt` also reruns that closer multi-root fixture for two consecutive hot rounds, and it still leaves the later two `local.set` roots in place. The direct artifact experiment says the same thing: [`.tmp/self-opt-simplify-locals-twice-20260408`](./.tmp/self-opt-simplify-locals-twice-20260408) shrinks further from `1950336` to `1950197`, but the first direct hunk at `214` is still unchanged. So the remaining frontier is not just “run simplify-locals again”; it needs a deliberate multi-root extraction rewrite if we want to move that family.
      - That call-prefixed blocker is now pinned in-tree directly. `src/passes/code_pushing_test.mbt` keeps the pass negative for the call-prefixed terminal-inner-owner carrier, while `src/ir/hot_lower_live_repro_test.mbt` proves the same manual reorder still lowers and validates. So the `72005` frontier is now split cleanly: one exact terminal-inner-owner carrier is landed, and the closer live-shaped call-prefixed version is explicitly blocked.
      - A tighter non-repro is now in-tree too: `src/passes/code_pushing_test.mbt` proves the pass already handles the simpler tee-fed sibling case where the movable `local.set` is followed by a kept condition `local.set`, then the later `if`, then later reads. So the surviving `48981` artifact family is narrower again and still points at the dropped-carrier / extraction frontier rather than plain same-region sibling motion.
      - The same is now true for the next `72005` lane too. A new reducer in `src/passes/code_pushing_test.mbt` proves the pass already handles the simpler alias-set shape from that diff: alias `local.set`, then a kept loaded condition `local.set`, then the later `if`, then later reads. So the surviving `72005` artifact family also still depends on broader artifact context than ordinary same-region sibling motion.
      - The blocked `44251` terminal-owner family is now pinned one step closer too. A smaller explicit-exit `LocalSet(Block(...), LocalGet)` carrier with a terminal arm and an inner-owner `br` arm is now covered in both `src/passes/code_pushing_test.mbt` and `src/ir/hot_lower_live_repro_test.mbt`: manual reorder still lowers and validates, but the current pass intentionally leaves it unchanged. A fresh retry that widened this exact family wrote [`.tmp/self-opt-code-pushing-20260408d`](./.tmp/self-opt-code-pushing-20260408d) and reintroduced the same earlier `func $127` structural drift at `44251`, so this smaller block-carrier shape now clearly belongs to the same not-kept terminal-owner frontier as the earlier condition-set reducer.
      - That means the `72005` frontier is no longer a separate immediate target either. The surviving printed `func $238` mismatch still has the same earlier explicit-exit `LocalSet(Block(...), LocalGet)` blocker before the alias `local.set $11` / later `if`, so another direct widening there would just reopen the same blocked terminal-owner family. The next useful correctness work should stay on the distinct `48981` dropped-carrier / root-splitting frontier in `func $127`, not on another `72005` same-region sink attempt.
      - That first explicit `48981` extraction boundary is now landed rather than only pinned. `src/passes/code_pushing_test.mbt` and `src/ir/hot_lower_live_repro_test.mbt` now cover both the direct alias extraction and the closer call-fed extraction with a synthesized temp local, and the fresh replay confirms that this is enough to remove the old `func $127` family entirely. So the next real CP004 step is no longer another `48981` reducer. It is either the still-blocked `72005` terminal-owner family or the larger alias-local synthesis families at `105621` / `126757`.
     ...
     - Doc: [0073](/home/jtenner/Projects/starshine-mb-code-pushing/docs/0073-2026-04-02-code-pushing-binaryen-plan.md#L1)
3. Do work.
   - `[CP]001`, `[CP]002`, and `[CP]003` are landed. Continue with `[CP]004`, and keep the remaining parity gaps split between default-semantics bugs vs trap-policy or local-typing follow-up work.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
  - [SL]001 - Full Local Simplification Audit - Harden the existing simplify-locals pass so the late local cleanup exactly matches Binaryen's post-coalescing behavior.
    - Status: the current correctness blocker on the checked-in debug artifact is fixed. `simplify-locals` no longer aborts or emits invalid output on `tests/node/dist/starshine-debug-wasi.wasm`: dead `local.tee` rewrites are now limited to sole `Drop` stack consumers, dead `local.set` rewrites are now limited to locals that are unread anywhere in the function, `src/cmd/cmd_test.mbt` has a native debug-artifact regression for direct `--simplify-locals`, and both `--simplify-locals` plus `--code-pushing --simplify-locals` validate again. The remaining work is precision/parity, not artifact safety.
   ...
   - [SL]002 - Slot Validation and Artifact Replay - Lock the corrected pass into scheduler tests and replay the MoonBit debug-artifact compare harness.
     - Current blocker: the direct artifact safety lane is green now, but parity is not. The combined cleanup replay [`.tmp/self-opt-code-pushing-plus-simplify-locals-20260408`](./.tmp/self-opt-code-pushing-plus-simplify-locals-20260408) completes again and stays normalized red, with Starshine larger (`1956918` bytes vs `1791630`) and slower (`1145.246 ms` vs `399.478 ms` pass), so the next SL work is to recover safe dead-local precision without reopening the invalid-module bug. The first post-fix `gen-valid` simplify-locals fuzz lane [`.tmp/pass-fuzz-simplify-locals-genvalid-500-20260408`](./.tmp/pass-fuzz-simplify-locals-genvalid-500-20260408) was only `24/500` compared with `4` matches before the `20`-mismatch cap. After landing the dead pure-immediate cleanup slice, [`.tmp/pass-fuzz-simplify-locals-genvalid-500-20260408b`](./.tmp/pass-fuzz-simplify-locals-genvalid-500-20260408b) improves to `44/500` compared with `24` matches. The remaining earliest mismatches are no longer dead pure local-def cleanup; they are local-forwarding/canonicalization families such as `drop(local.tee const)` plus later `local.get`, which Binaryen rewrites to direct stack values.
