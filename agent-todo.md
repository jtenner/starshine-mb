...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [CP]004 - Binaryen Parity Proof And Follow-On Gaps - Close the compare loop and keep deferred oracle gaps explicit.
    ...
    - Current blocker: exact TNH / implicit-trap parity needs `HotPipelineOptions` or equivalent hot-pass policy plumbing; the GC `ref-into-if` family is follow-up work until local-type repair is representable in the current pipeline; the current default-semantics diff still shows same-region `if` placement gaps on the debug artifact, but the stable non-void fence is now narrower than before. The latest direct compare is `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-92230`. It stayed valid on the real artifact after a new pass relaxation that ignores earlier explicit exits under `Drop` roots in non-void regions, and that change removed the old `43657` `local.set $167` mismatch family. But the first `19348` `local.set $20` family is still present, the next early gaps remain around `48981`, `61077`, and `72008`, and the larger structural mismatch family around `105624` is still there too. Runtime also did not improve yet (`10745.351 ms` Starshine pass vs `54.208 ms` Binaryen; `13345.644 ms` total vs `379.276 ms`). The current last-known-valid non-void rules are now twofold: allow reordering when the prefix up through the pushpoint has no explicit exits, even if a later root in the same region branches; and also ignore earlier explicit exits under `Drop` roots when deciding whether the non-void sink is blocked. `src/passes/code_pushing_test.mbt` now covers both with branch-free nested/root non-void regressions, a "later branch root" regression, and a dropped-result-prefix regression. The broader zero-result-prefix relaxation still does not hold on the real artifact: ignoring earlier zero-result branch roots reintroduced the same `Func 1977` stack underflow on `tests/node/dist/starshine-debug-wasi.wasm`, and `src/ir/hot_lower_live_repro_test.mbt` now records why that remains a blocker instead of a fix across several closer reducers. The reduced earlier-zero-result-branch prefix, the same prefix inside an outer result carrier, the simplified extracted parent-escape carrier, the closer reordered real parent-escape carrier, and even the closer real carrier after parent-err extraction plus carried-`if` demotion all still lower and validate. That last closer shape still hits the same `InvalidBranchArity(_, _, 0, 1)` cliff immediately once the parent `err` block becomes result-producing, so the real artifact still needs a richer outer rewrite step than the local carrier rewrite alone. A separate predecessor-root reduction also stayed valid: an earlier dropped result-carrying block root before two later `local.set`s and a sinkable `if` still lowers after the unused `local.set` moves past the `if`, which is why the new `Drop`-root relaxation is now safe enough to keep. The next closer HOT reducer is also green now: an earlier self-contained `Block` root whose internal payload branches stay entirely inside nested labels still lowers after the same sink, but a first owner-aware pass relaxation for that family was rolled back because it also weakened the known-unsafe zero-result and split-carry blockers. So the remaining pass work is narrower again: identify a rule that captures the first `19348` family without broadening to all non-escaping earlier roots. The older correctness story still stands: earlier attempts such as `trailing-payload-split` and `nested-only` also reintroduced `Func 1977`, traced root-order dumps still show one concrete bad move under the root-only relaxation (`LocalSet(45)#525` shifting from before `LocalSet(50)#528` / `If#534` to after `If#534`), and the remaining unsafe family still needs extra real-function scaffolding beyond the carrier extraction plus inner carried-`if` rewrite alone. The validator offset is still pinned at `br 3` to label `@8` in `Func 1977`, `src/ir/hot_lower_live_repro_test.mbt` still proves the underlying `InvalidBranchArity(_, _, 0, 1)` mechanism, and `src/passes/code_pushing_test.mbt` still guards the parent-exit payload-carry and parent-escape carrier boundaries directly at the pass layer.
