...
- The active simplify-locals debt is no longer the old `Func 71` canonical mismatch bucket. The remaining live work is:
  ...
  - broader warning cleanup: native builds still emit repo-wide legacy dependency, deprecation, and unused-code warnings outside this landed simplify-locals slice
- Do not revive the broad lowered-`nop` stripping experiment: Binaryen still preserves sentinel `nop`s in several reduced simplify-locals families, and the kept policy is the narrower pass-specific lower preservation plus dead copied-temp cleanup.
...
- Canonical ordered path and nested-shape notes live in the living wiki page [binaryen/no-dwarf-default-optimize-path.md](/home/jtenner/Projects/starshine-mb/docs/wiki/binaryen/no-dwarf-default-optimize-path.md#canonical-top-level-shape) and the archived source note [0066](/home/jtenner/Projects/starshine-mb/docs/wiki/raw/research/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#pass-reference-index).
...
1. Canonical correctness landed.
   - [RUME]003 - Oracle-stable fast-path pass follow-up - Direct pass execution, CLI flag and registry wiring, focused section/index rewrite coverage, explicit memarg-index rewrites, and canonical compare-harness artifact parity are in place; `remove-unused-module-elements` is now canonically correct against the oracle on the debug artifact.
     ...
     - Command-failure classification from `.tmp/pass-fuzz-rume-rerun`: Starshine decode/validation failures accounted for `5/14` cases, split into four `DecodeAt(InvalidLimits, ...)` cases (`000007`, `000055`, `000091`, `000099`) and one final-module validation failure `Invalid range for limits` (`000073`). `pass-fuzz-compare` now keeps those Starshine families replayable as `starshine-invalid-limits` and `starshine-invalid-range-for-limits` instead of one undifferentiated Starshine bucket. Binaryen parser failures accounted for `9/14` cases, split into six `invalid type index` cases (`000009`, `000021`, `000031`, `000047`, `000079`, `000095`), one `Recursion groups of size zero not supported` case (`000029`), and two `invalid wasm type: -64` cases (`000081`, `000083`). These are fuzz-coverage blockers rather than RUME semantic mismatches, but they now persist cleanly for follow-up instead of aborting the run.
     ...
     - Doc: [RUME parity](/home/jtenner/Projects/starshine-mb/docs/wiki/binaryen/passes/remove-unused-module-elements/parity.md) records the current semantic-signoff interpretation; the archived optimize-path source note is [0066#pass-reference-index](/home/jtenner/Projects/starshine-mb/docs/wiki/raw/research/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#pass-reference-index).
2. Do work.
...
3. Test against binaryen.
   ...
   - The accumulated command-failure corpus now stays separately replayable while fuzzing RUME: Starshine decoder/validator coverage can be filtered via `starshine-invalid-limits` and `starshine-invalid-range-for-limits`, while the Binaryen parser/validator compatibility families now stay split across `binaryen-invalid-type-index`, `binaryen-rec-group-zero`, `binaryen-invalid-wasm-type-neg64`, and the shared-memory validator family `binaryen-initializer-expression-not-constant` for follow-up outside semantic mismatch reduction.
   - For future coverage-only RUME sweeps, use `bun scripts/pass-fuzz-compare.ts --pass remove-unused-module-elements --keep-going-after-command-failures ...` when the goal is to keep collecting comparable cases after known Binaryen parser/canonicalization failures; command failures remain recorded and replayable but no longer spend the mismatch/failure cutoff in that explicit mode. That mode exposed and closed one additional semantic mismatch at `.tmp/pass-fuzz-rume-keep-going-2026-04-24/failures/case-000186-wasm-smith`: Binaryen keeps imported tables and nonempty active expression elem segments even when all initializers are `ref.null`. Starshine now matches, and `.tmp/pass-fuzz-rume-keep-going-2026-04-24-fix` reached `298/298` comparable `wasm-smith` cases with `0` mismatches, `0` validation failures, `0` generator failures, and `2` command failures (`binaryen-rec-group-zero`, `starshine-command-failed`).
...
0. Shared blocker on canonical artifact parity.
  - [HOT]001 - Post-SSA Hot-Pipeline Replay Hardening - The large-artifact `DFE -> RUME -> MP -> OR -> GR -> GSI -> SSA` prefix now completes, validates, and canonicalizes under Binaryen, and DCE itself no longer emits the invalid lowered output that previously blocked the chain on final dead-result control. The remaining ordered full-chain work is now downstream of that fix: rerun the exact post-SSA cleanup replay on a valid baseline artifact, refresh any remaining later-pass re-lift failures on the rebased branch, and keep the serial hot batch workable until the worker-queue slice is safe.
     ...
     - Current blocker: the stale notes about a raw `wasm-tools validate`-clean debug artifact and DCE `Func 1730` native final-validation failure are retired on current head. Fresh `2026-05-02` `wasm-tools validate tests/node/dist/starshine-debug-wasi.wasm` still reports the known raw-validator `too many locals` limit, but the compare harness can proceed through the Binaryen-accepted fallback. Direct DCE self-opt compare is now green at normalized-WAT/canonical-function level, so the active hot-pipeline blocker has moved back to the modeled cleanup prefix: `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --out-dir .tmp/validate-stale/self-post-ssa-cleanup --dead-code-elimination --vacuum --optimize-instructions --simplify-locals` currently fails with `skip-invalid-lower` on suspicious escape carriers (`Func 225`, `Func 235`). Keep reducing those prefix-specific carrier failures and the remaining runtime budget before claiming ordered-prefix proof.
  ...
  - [IR2]001 - Hot Lower Payload-Forwarder Repair - Restore the canonical hot-lower behavior for typed payload-forwarder and returned-if shapes before landing more cleanup rewrites on top.
     ...
     - Current blocker: the full repo test suite is green again (`moon test` => `1929/1929`), so this is no longer an active red-test blocker. The remaining work is to refresh exact artifact-backed evidence for the still-risky payload-forwarder families and only reopen this slice if new ordered-prefix replay or focused `hot_lower` regressions show those shapes drifting from the intended typed-block and `unreachable` contracts. The latest stack-fix slices are now in-tree too: hot lowering no longer under-emits repeated shared operand nodes, with focused live repros for both the minimal shared-operand underflow and a loop-carried nested value-`if` / later-`drop` shape, and the binary-const result-spill rewrite now refuses to hoist mixed-typed call operands ahead of the call result-spill local, with a focused live repro covering the reduced `local.get i64 ; i32.const ; call ; local.set i32 ; local.get i32` family. The remaining work is replay proof on the large artifact, not another known reduced hot-lower red test.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [DCE]003 - Runtime Budget and Oracle Refresh - Close the remaining DCE speed gap and refresh parity evidence on a valid baseline/oracle path.
     ...
     - Current status: the stale native-release `Func 1730` stack-underflow blocker is retired on current head. Fresh `2026-05-02` replay of `_build/native/release/build/cmd/cmd.exe --dead-code-elimination --out .tmp/validate-stale/dce-out.wasm tests/node/dist/starshine-debug-wasi.wasm` exits `0`, and `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --out-dir .tmp/validate-stale/self-dce --dead-code-elimination` exits `0` with normalized WAT equality through fallback and canonical function equality. The `2026-05-03` pre-skip slices now avoid running DCE on original suspicious escape-carrier functions that are discarded after lower anyway and on the large-local artifact no-op family, keeping canonical parity green at `.tmp/dce-large-local-skip-20260503` while improving the direct lane from the fresh baseline Starshine `5823.923ms` wall / `1959.419ms` pass to Starshine `1350.014ms` wall / `107.080ms` pass versus Binaryen `487.904ms` wall / `104.521ms` pass. Remaining DCE work is now mostly whole-command wall time and representation drift, not direct pass time, a native final-validation crash, or canonical-function mismatch.
     - Reproduce current direct native replay: `_build/native/release/build/cmd/cmd.exe --dead-code-elimination --out .tmp/validate-stale/dce-out.wasm tests/node/dist/starshine-debug-wasi.wasm`
     - Reproduce current compare lane: `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --out-dir .tmp/validate-stale/self-dce --dead-code-elimination`
     - Doc: [0066#L178](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L178)
   - [BIN]001 - Iterative Expr Decoder - Replace the current recursive binary expression decoder with an explicit worklist / stack machine so deeply nested valid wasm does not depend on native call-stack budget.
     ...
     - Reproduce the old failure shape: `gdb -batch -ex run -ex 'thread apply all bt' --args _build/native/release/test/cmd/cmd.whitebox_test.exe 'cmd_wbtest.mbt:87-88'`
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [RUB]002 - Multi-Slot Cleanup and Artifact Compare - Replay the pass in each Binaryen slot and confirm later `merge-blocks` opportunities match the reference output.
     ...
     - Current blocker: the scheduler gap, old serial verifier failures, and the earlier artifact-backed correctness blockers are fixed; the remaining work is now concentrated in explicit-pass type-order noise plus later unchanged runtime families whose traces still need pass-vs-lift separation.
       - `optimize` and `shrink` both replay `remove-unused-brs` in three modeled slots, focused preset tests cover those trace counts, and native `cmd` replay now keeps the checked-in debug artifact lane wasm-tools-valid, including the extracted `Func 2244` tail-return value-if regression that previously left stack values stranded after RUB voidified the outer returned `if`.
      ...
      - The later parity fix for `.tmp/self-opt-rub-20260410-compare-next/binaryen.nop20.wasm` now validates the old `Func 3771` failure, and the `2026-04-13` follow-up work now includes both the earlier scan/liveness audit and four later candidate-filter slices: hybrid large-free-list HOT liveness (`deleted_nodes`), one-shot lifted skip summaries, one per-cycle scan for label refs / branch-payload children / `has_br_table`, threaded root-site and single-arm-`nop` context instead of whole-function rediscovery scans, bounded detached cleanup, less concat churn, plus new raw filters for very large void-`if` / return families and medium-size unchanged control ladders, followed by the lifted `medium-branchy-block-ladder-noop` retirement for extracted artifact funcs `144`, `301`, `353`, `1512`, `1547`, `1859`, and `1867`, the lifted `call-heavy-mixed-if-mesh-noop` retirement for extracted artifact funcs `408`, `413`, `739`, `832`, `902`, `1022`, `1448`, and `1815`, and now the lifted `localset-heavy-value-if-mesh-noop` retirement for extracted artifact funcs `837`, `3021`, `3120`, `3130`, and `3134`.
      - Fresh `2026-05-02` direct self-opt replay at `.tmp/validate-stale/self-remove-unused-brs` retires the stale red-parity note: `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --out-dir .tmp/validate-stale/self-remove-unused-brs --remove-unused-brs` exits `0` with normalized WAT equality through fallback and canonical function equality. Overall wall time remains over budget (`2274.681ms` Starshine vs `669.977ms` Binaryen), but direct pass time is no longer the leading RUB blocker in that run (`222.239ms` Starshine vs `237.503ms` Binaryen).
     ...
     - The newer tail-return guard is now fixed too: RUB no longer voidifies a returned value-`if` when a nonfallthrough arm still relies on typed roots to satisfy the surrounding return context, so extracted debug-artifact `Func 2244` and the full `.tmp/o4z-cron-remove-unused-brs-20260425-2` self-opt compare lane stay valid again (`normalizedWatEqual=true`, `canonicalFuncPrettyEqual=true`).
     ...
     - The larger `gen-valid` reruns remain green too: `bun scripts/pass-fuzz-compare.ts --pass remove-unused-brs --generator gen-valid --count 1000 --max-failures 20 --out-dir .tmp/pass-fuzz-rub-genvalid-1000-rerun` => `1000/1000` normalized matches, `0` mismatches, `0` command failures, and `bun scripts/pass-fuzz-compare.ts --pass remove-unused-brs --generator gen-valid --count 10000 --min-compared 10000 --max-failures 20 --out-dir .tmp/pass-fuzz-rub-genvalid-10000` => `10000/10000` compared cases, `10000/10000` normalized matches, `0` mismatches, `0` validation failures, `0` generator failures, `0` command failures.
      - Remaining work:
       - Keep the direct artifact lane canonically green while later ordered-prefix work lands.
       - Re-run a larger mixed-generator compare-pass lane before final preset proof.
       - Treat remaining RUB work as ordered-neighborhood and end-to-end runtime work unless a fresh mutation-backed mismatch is reproduced.
     - Risks:
      ...
      - Overall self-opt wall time remains above Binaryen on current head even though the direct RUB pass timer itself is no longer slower in the latest replay, so any broader carrier-family rewrite needs a narrow guard and a measured payoff.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [PC]001 - Constant Folding Surface - Audit the exact constant, tuple, and local-evaluable fragments Binaryen folds in the top-level `precompute` slots for `-O` / `-Os`.
     ...
     - Current status: the stale direct artifact parity gap is retired on current head. Fresh `2026-05-02` replay of `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --out-dir .tmp/validate-stale/self-precompute --precompute` exits `0` with normalized WAT equality through fallback and canonical function equality. Remaining work is runtime and representation drift: the same run still reports canonical wasm/text-form drift plus Starshine `8744.582ms` wall / `274.940ms` pass versus Binaryen `574.248ms` wall / `162.315ms` pass.
   ...
   - [PC]002 - Early/Late Slot Regression and Artifact Parity - Harden the pass for both top-level slots and compare the resulting folds against Binaryen on the debug artifact.
     ...
     - Current focus: keep the direct debug-artifact parity lane green, reduce the current runtime gap, and keep the fuzz parity gate for `precompute` itself green.
     - Doc: [0066#L208](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L208)
3. Do work.
   - Land the slices above in dependency order in the implementing file(s) and any required scheduler, preset, or dispatcher surfaces.
   - Wire the pass into the exact top-level slot(s) and nested rerun sites documented in the research doc before calling the work done.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [CP]001 - Motion Safety Rules - Port Binaryen's code-motion rules for pushing work deeper into control flow without duplicating invalid side effects.
     ...
     - Status: initial explicit hot pass landed for the narrow safe local-set sinking surface: a const-like `local.set` immediately before a void `if` is replaced by a `nop` while a cloned `local.set` moves into the single consuming `if` arm, and the pass refuses locals with later reads, multiple writes, else/condition reads, non-void `if`s, or non-const/trapping values. This keeps the active surface conservative until broader Binaryen code-motion rules are researched and fuzzed.
   ...
   - [CP]002 - Rewrite Coverage and Artifact Validation - Implement the move and test it against branchy, trap-sensitive fixtures plus the MoonBit debug artifact.
     ...
     - Current blocker: focused pass/registry/CLI coverage is in-tree for direct `--code-pushing`; `moon test src/passes`, `moon test src/cmd`, `moon info`, full `moon test`, `.tmp/pass-fuzz-code-pushing-genvalid-10000-after-typed-block-v2` (`10000/10000`, `0` mismatches, `0` validation/generator/command failures), and `.tmp/pass-fuzz-code-pushing-wasmsmith-10000-after-typed-block-v2` (`9952/10000` comparable, `9952` matches, `0` mismatches, `0` validation failures, `48` classified Binaryen/canonicalization command failures) are green. The earlier `CannotEncodeNonStandardTypeIndex` generator/invalid-binary failure is fixed by making descriptor metadata use absolute type indexes, and `--emit-gen-valid-batch` now uses a portable coverage-forced profile that external `wasm-tools`/Binaryen compare lanes can validate. The prior `case-009332-wasm-smith` typed-unreachable block mismatch is fixed by flattening the narrow Binaryen-owned dead typed-block-before-unreachable shape in `code-pushing`; manual replay and the fresh wasm-smith 10000 lane are clean. The first pass-internal performance audit is now landed: the typed-block flattening path checks parent dead context before scanning block bodies, branch detection no longer double-walks structured regions, and local-set sinking checks arm uses before a bounded single whole-function get/write count. A synthetic no-arm-use candidate ladder moved direct `pass:code-pushing` time from `503152 us` to `2609 us`, the focused rerun `.tmp/pass-fuzz-code-pushing-perf-guard-genvalid-10000` is `10000/10000` with `0` mismatches, and `.tmp/pass-fuzz-code-pushing-perf-guard-wasmsmith-200` is `199/200` comparable with `0` mismatches and one Binaryen-side command failure. The checked debug WASI artifact is still rejected by `wasm-tools validate` for too many locals. The optimized WASI artifact compare is semantically green (`Normalized WAT equal: yes`, `Canonical function compare equal: yes`) but still much slower than Binaryen (`1569.014 ms` Starshine wall / `596.997 ms` pass vs `125.980 ms` Binaryen wall / `28.829 ms` pass in the latest run), so the pass is not parity-complete or ready for public optimize preset scheduling.
     - Next tasks:
       1. Continue profiling the optimized-artifact lane after the landed pass-internal guard cleanup; current direct `code-pushing` optimized-WASI compare is semantically green but much slower than Binaryen, and the micro-ladder win does not yet prove artifact-budget parity.
       2. Treat the debug artifact's plain `wasm-tools validate` local-count failure as a raw-validator limitation, not a compare-harness blocker, because the Binaryen-accepted fallback path can still support pass comparisons.
       3. Decide whether to expand the implemented Binaryen surface or keep the pass direct-only after performance is better understood.
     - Doc: [0066#L213](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L213)
3. Do work.
   - Land the slices above in dependency order in the implementing file(s) and any required scheduler, preset, or dispatcher surfaces.
   - Wire the pass into the exact top-level slot(s) and nested rerun sites documented in the research doc before calling the work done.
...
3. Do work.
   - Land the slices above in dependency order in the implementing file(s) and any required scheduler, preset, or dispatcher surfaces.
   - Wire the pass into the exact top-level slot(s) and nested rerun sites documented in the research doc before calling the work done.
   - Current status: the repo now has a standalone active hot `merge-blocks` pass that flattens branch-free block roots in any region, preserves lowered `nop` roots and dead `local.tee` + `drop` pairs on changed functions to match Binaryen's direct `--merge-blocks` behavior, rewrites flattened dead-value-before-`unreachable` suffix roots into explicit dropped roots before the exit, and is wired into the representable late `optimize` / `shrink` cleanup slots as `merge-blocks -> remove-unused-brs -> remove-unused-names -> merge-blocks -> precompute -> optimize-instructions -> heap-store-optimization`. Hot lowering now also canonicalizes leading/trailing dead-value `unreachable` forms into Binaryen-stable order, which fixes the reduced `case-002970` mismatch family and keeps the reduced single-result typed param root, typed single-result `if`-arm param block, reduced single-param multi-result block root, reduced single-param multi-result `if` arm, reduced typed single-param void-result block root and `if` arm, reduced multi-result typed param probes, the typed multi-param single-result root plus typed multi-param multi-result root and typed multi-param single-result and multi-result `if`-arm probes, the typed single-param single-result loop root and `if`-arm loop carriers, the reduced typed single-param multi-result loop root and `if` arm, the reduced typed single-param void-result loop root and `if` arm, the reduced typed multi-param void-result loop root and `if` arm, the reduced typed single-param externref/funcref single-result and multi-result loop-root follow-ups, the reduced typed single-param externref multi-result loop `if` arm, and the newer funcref single-result plus multi-result reduced block/`if` shapes canonically equal to Binaryen. A fresh full wasm-smith sign-off rerun at `.tmp/pass-fuzz-merge-blocks-10000-wasm-smith-seed51a-signoff-rerun3` reached `8030/8030` comparable normalized matches with `0` mismatches, `0` Starshine command failures, and only the recurring Binaryen parser blockers (`binaryen-rec-group-zero`, `bad section size`, and invalid type-index families). The two previously noted Starshine hot-pipeline abort repros from `.tmp/pass-fuzz-merge-blocks-10000-wasm-smith-seed51a-signoff/failures/case-004009-wasm-smith` and `case-007531-wasm-smith` no longer reproduce under repeated native-release `--merge-blocks` runs in this workspace. The debug artifact was also rebuilt locally from MoonBit in this workspace by copying `_build/wasm/debug/build/cmd/cmd.wasm` to `tests/node/dist/starshine-debug-wasi.wasm` and `_build/wasm/release/build/cmd/cmd.wasm` to `tests/node/dist/starshine-optimized-wasi.wasm`; both validate with `wasm-tools validate`. Ordered artifact replay is now fresh too: `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --out-dir .tmp/self-opt-merge-blocks-20260421-rerun --merge-blocks` reports `Normalized WAT equal: yes` and `Canonical function compare equal: yes` (with only canonical-byte/runtime drift remaining), while `moon run --target native --release src/cmd -- --debug-serial-passes --tracing pass --merge-blocks --out .tmp/merge-blocks-debug-serial-replay.wasm tests/node/dist/starshine-debug-wasi.wasm` validates and shows `5196` direct `merge-blocks` function replays with no skips, no writeback failures, and no changed functions. There is no remaining actionable Starshine-side `merge-blocks` parity/sign-off bug in this workspace.
4. Test against binaryen.
   - Add edge-case and regression tests beside the implementing file and any scheduler or dispatcher coverage needed for the pass.
   - Compare Starshine vs Binaryen with `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --<pass>` and any required ordered-prefix replay.
   - Reduced direct oracle smokes are now green for both known writeback families: `bun scripts/self-optimize-compare.ts .tmp/merge-blocks-nop-divergence/in.wasm --out-dir .tmp/merge-blocks-nop-divergence/out --merge-blocks` reports canonical wasm and normalized WAT equality after preserving lowered `nop`s, and `bun scripts/self-optimize-compare.ts .tmp/merge-blocks-tee-probe/tee-drop.wasm --out-dir .tmp/merge-blocks-tee-probe/out-fixed --merge-blocks` now does the same for the dead `local.tee` + `drop` family. The reduced typed param-block probes `bun scripts/self-optimize-compare.ts .tmp/mb-probe/param1.wasm --out-dir .tmp/mb-probe/param1.final --merge-blocks`, `bun scripts/self-optimize-compare.ts .tmp/mb-probe/caseB.wasm --out-dir .tmp/mb-probe/caseB.final --merge-blocks`, and `bun scripts/self-optimize-compare.ts .tmp/mb-probe/caseC.wasm --out-dir .tmp/mb-probe/caseC.final --merge-blocks` are now canonically green too, the reduced single-param multi-result follow-ups `bun scripts/self-optimize-compare.ts .tmp/mb-next-probes/single_param_multi_result_root.wasm --out-dir .tmp/mb-next-probes/single_param_multi_result_root.final --merge-blocks` plus `bun scripts/self-optimize-compare.ts .tmp/mb-next-probes/single_param_multi_result_if_arm.wasm --out-dir .tmp/mb-next-probes/single_param_multi_result_if_arm.final --merge-blocks` now match Binaryen as well, the reduced single-param void-result block probes `bun scripts/self-optimize-compare.ts .tmp/codex-void-param-probes/block_root.wasm --out-dir .tmp/codex-void-param-probes/out-block_root-fixed --merge-blocks` and `bun scripts/self-optimize-compare.ts .tmp/codex-void-param-probes/block_if_arm.wasm --out-dir .tmp/codex-void-param-probes/out-block_if_arm-fixed --merge-blocks` now match Binaryen too, the reduced single-param void-result loop probes `bun scripts/self-optimize-compare.ts .tmp/codex-void-param-probes/loop_root.wasm --out-dir .tmp/codex-void-param-probes/out-loop_root-fixed --merge-blocks` and `bun scripts/self-optimize-compare.ts .tmp/codex-void-param-probes/loop_if_arm.wasm --out-dir .tmp/codex-void-param-probes/out-loop_if_arm-fixed --merge-blocks` now match Binaryen too, the newer typed multi-param follow-ups `bun scripts/self-optimize-compare.ts .tmp/codex-tmp/mb-candidates/multi_param_single_result_root.wasm --out-dir .tmp/codex-tmp/mb-candidates/out-multi_param_single_result_root --merge-blocks`, `bun scripts/self-optimize-compare.ts .tmp/codex-tmp/mb-next-block/multi_param_multi_result_root.wasm --out-dir .tmp/codex-tmp/mb-next-block/out-multi_param_multi_result_root --merge-blocks`, `bun scripts/self-optimize-compare.ts .tmp/codex-tmp/mb-candidates/multi_param_single_result_if_arm.wasm --out-dir .tmp/codex-tmp/mb-candidates/out-multi_param_single_result_if_arm --merge-blocks`, and `bun scripts/self-optimize-compare.ts .tmp/codex-tmp/mb-candidates/multi_param_multi_result_if_arm.wasm --out-dir .tmp/codex-tmp/mb-candidates/out-multi_param_multi_result_if_arm --merge-blocks` are canonically green too, the typed multi-param void-result loop follow-ups `bun scripts/self-optimize-compare.ts .tmp/codex-multi-param-void/loop_root.wasm --out-dir .tmp/codex-multi-param-void/final-loop_root --merge-blocks` plus `bun scripts/self-optimize-compare.ts .tmp/codex-multi-param-void/loop_if_arm.wasm --out-dir .tmp/codex-multi-param-void/final-loop_if_arm --merge-blocks` now match Binaryen as well while the neighboring `block_root` and `block_if_arm` probes in the same directory stay canonically green, the typed single-param single-result loop follow-ups `bun scripts/self-optimize-compare.ts .tmp/codex-tmp/mb-candidates/loop_single_param_single_result_root.wasm --out-dir .tmp/codex-tmp/mb-candidates/out-loop_single_param_single_result_root --merge-blocks` plus `bun scripts/self-optimize-compare.ts .tmp/codex-tmp/mb-candidates/loop_single_param_single_result_if_arm.wasm --out-dir .tmp/codex-tmp/mb-candidates/out-loop_single_param_single_result_if_arm --merge-blocks` now match Binaryen as well, and the typed single-param multi-result loop follow-ups `bun scripts/self-optimize-compare.ts .tmp/codex-tmp/mb-next-loop/loop_single_param_multi_result_root.wasm --out-dir .tmp/codex-tmp/mb-next-loop/out-loop_single_param_multi_result_root-fixed --merge-blocks` plus `bun scripts/self-optimize-compare.ts .tmp/mb-next-probes/single_param_multi_result_if_arm.wasm --out-dir .tmp/mb-next-probes/single_param_multi_result_if_arm.final --merge-blocks` are canonically green too. The reduced mismatch repro [`.tmp/signoff/merge-blocks-mismatch-repro.wat`](./.tmp/signoff/merge-blocks-mismatch-repro.wat) and original fuzz artifact [`.tmp/pass-fuzz-merge-blocks-5000-wasm-smith-seed51a/failures/case-002970-wasm-smith`](./.tmp/pass-fuzz-merge-blocks-5000-wasm-smith-seed51a/failures/case-002970-wasm-smith) are now fixed in-tree by the dead-value-before-`unreachable` rewrite. Small wasm-smith-only follow-up sweeps at `.tmp/pass-fuzz-compare-merge-blocks-slice-smith`, `.tmp/pass-fuzz-compare-merge-blocks-next-slice-smith`, `.tmp/pass-fuzz-compare-merge-blocks-tee-fix-smith`, `.tmp/pass-fuzz-compare-merge-blocks-single-param-slice`, and `.tmp/pass-fuzz-compare-merge-blocks-caseb-fix` stayed mismatch-free on comparable cases and only stopped on Binaryen-side parser or command-failure buckets, the stronger rerun `.tmp/pass-fuzz-compare-merge-blocks-single-param-multi-result-20260420` reached `99/99` normalized matches on comparable cases with no MergeBlocks mismatches, the loop-root follow-up `.tmp/pass-fuzz-compare-merge-blocks-loop-multi-result-20260420` reached `28/28` comparable normalized matches before the usual Binaryen-only `binaryen-rec-group-zero` parser stop, the void-loop follow-up `.tmp/pass-fuzz-compare-merge-blocks-void-loop-20260420b` reached `28/28` comparable normalized matches before one Binaryen-side command failure, and the newer sign-off sweep `.tmp/pass-fuzz-compare-merge-blocks-signoff-20260420a` reached `199/199` comparable normalized matches with `0` mismatches before one Binaryen-side command failure. The newer reduced ref-typed oracle probes `.tmp/codex-mergeblocks-direct-20260421/externref_multi_result_if_arm`, `.tmp/codex-mergeblocks-direct-20260421/externref_multi_result_loop_root`, `.tmp/codex-mergeblocks-direct-20260421/externref_multi_result_loop_if_arm`, and `.tmp/codex-mergeblocks-probes-20260421c2/out-funcref_{single_result_block_root,single_result_if_arm,multi_result_block_root,multi_result_if_arm}` are also canonically green, while the stronger wasm-smith sweeps `.tmp/pass-fuzz-compare-merge-blocks-20260421b-smith`, `.tmp/pass-fuzz-compare-merge-blocks-20260421c-smith`, `.tmp/pass-fuzz-merge-blocks-5000-wasm-smith-seed51a-rerun2`, `.tmp/pass-fuzz-merge-blocks-10000-wasm-smith-seed51a-signoff`, and the fresh rerun `.tmp/pass-fuzz-merge-blocks-10000-wasm-smith-seed51a-signoff-rerun3` reached `399/399`, `797/797`, `3686/3686`, `7491/7491`, and `8030/8030` normalized comparable matches with `0` mismatches. The representative recurring `binaryen-rec-group-zero`, `binaryen-bad-section-size`, and invalid-type-index failures reproduce under plain `wasm-opt --all-features` before any pass runs, so they remain external Binaryen parser blockers. Ordered debug-artifact replay is not runnable in this workspace today because `tests/node/dist/starshine-debug-wasi.wasm` is absent, which leaves no remaining actionable Starshine-side `merge-blocks` parity blocker here.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [RSE]001 - Registry And Same-Block Same-Value Elimination - Landed direct active pass slice for Binaryen's late `redundant-set-elimination`: active registry/dispatcher wiring, raw fast path, HOT fallback, same-value `local.set` shell removal, same-value `local.tee` shell removal, and RHS evaluation preservation.
     - Status: implemented in `src/passes/rse.mbt`, wired through `src/passes/optimize.mbt`, `src/passes/pass_manager.mbt`, `src/cmd/cmd_wbtest.mbt`, and `scripts/lib/pass-fuzz-compare-task.ts`. Focused tests cover repeated same-value sets/tees and different overwritten writes. Fresh evidence: `.tmp/pass-fuzz-rse-genvalid-10000-raw` is `10000/10000` normalized matches with `0` mismatches; `.tmp/pass-fuzz-rse-10000-raw` reaches `6759` comparable matches with `0` mismatches before `20` Binaryen-side command failures; `.tmp/self-opt-rse-native-20260426b` is normalized-WAT equal via fallback and canonical-function equal on `tests/node/dist/starshine-debug-wasi.wasm`, with Starshine faster than Binaryen on this expensive artifact lane.
   ...
   - [RSE]002 - CFG Value Flow And Refined Local Gets - Complete the remaining source-backed Binaryen `CFGWalker` contract beyond the landed direct slice: full block start/end local value identities, predecessor agreement/disagreement merge handling, loop convergence or documented conservative skips, and strict-subtype equivalent-local `local.get` retargeting.
     - Deliverables: branch-join positive and negative tests beyond the simple structured cases, loop convergence/skip tests, `rse-gc.wast`-style type-refinement tests, direct compare-pass parity after refined-get support, final `rse -> vacuum` cleanup-slot replay, explicit non-goals for globals, memory stores, GC field writes, `LocalGraph`, liveness, and arbitrary overwritten-write deletion.
...
1. Bootstrap the proof toolchain and package boundary.
   - [PRV]001 - Why3/Z3 bootstrap and proof-enabled package wiring.
     ...
     - Status: the validator-adjacent `src/validate_proof` boundary exists and fresh `2026-05-02` `moon prove src/validate_proof` succeeds with `9 goals proved`, so the proof-enabled package itself is no longer a bootstrap blocker. Remaining PRV001 work is documentation/CI policy cleanup and any direct-`src/validate` blocker capture not already reflected in the wiki.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [RG]001 - Global Cost Model and Reindexing - Direct public `reorder-globals` module pass is now active.
     ...
     - Remaining blocker: no `reorder-globals-always` sibling yet and no late-tail preset slot yet because `string-gathering` remains unimplemented; `directize` now has direct explicit-pass oracle signoff, but the ordered `string-gathering -> reorder-globals -> directize` tail still needs replay before preset scheduling.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [DIR]001 - Indirect-to-Direct Eligibility - Direct explicit-pass port is in-tree and oracle-signed.
     - Status: `directize` is registered as an active module pass, dispatched through the module-pass path, and listed by the pass-fuzz compare harness. The implementation builds module-wide immutable-table facts, rejects imported/exported/runtime-mutated tables, records active constant-offset function/ref.func element targets, rewrites compatible constant-index `call_indirect` / `return_call_indirect` sites to direct calls, turns known holes/out-of-range/wrong-type targets into `unreachable`, and lowers the narrow known-target `select` shape to direct-call `if` arms with fresh locals.
     - Focused coverage: `src/passes/directize_test.mbt` covers constant table target rewrite, table-mutation bailout, select lowering, and known-hole trap rewrite.
     - Validation evidence: `moon info`, `moon fmt`, `moon test src/passes`, `moon test src/cmd`, and full `moon test` are green; `.tmp/pass-fuzz-directize-genvalid-10000-final2` is `10000/10000` normalized matches with `0` mismatches/failures; `.tmp/pass-fuzz-directize-mixed-10000-final2` is `9975` comparable normalized matches with `0` mismatches and `25` Binaryen-side command failures; `.tmp/self-opt-directize-debug-final2` reports canonical wasm and normalized WAT equality on `tests/node/dist/starshine-debug-wasi.wasm`.
     - Remaining follow-up: keep late-tail preset scheduling deferred until the neighboring `string-gathering -> reorder-globals -> directize` slot can be replayed together, and add `directize-initial-contents-immutable` pass-arg support only if/when Starshine grows a pass-arg surface for it.
...
2. Expand core scalar and control instructions in the valid generator.
   - [FZG]002 - Full Scalar Numeric Opcode Generation - Add type-directed valid generation for the missing scalar numeric opcodes: integer div/rem/shift/rotate, full signed/unsigned comparisons, float ceil/floor/trunc/nearest/sqrt/min/max/copysign, unsigned conversions, sign-extension variants, and saturating truncations.
     - Status: complete. Coverage-forced `gen-valid` modules now emit a deterministic expanded scalar numeric prelude covering integer div/rem/shift/rotate, integer and float comparisons, float ceil/floor/trunc/nearest/sqrt/min/max/copysign, unsigned conversions, sign-extension, and saturating truncation families. `GenValidFeatureStats` now records `numeric_full_ops`, and `NumericFullOps` floors can pass once the scanner sees the widened surface.
     - Validation: `moon test --package jtenner/starshine/validate`, `moon info`, full `moon test`, and `bun scripts/pass-fuzz-compare.ts --pass remove-unused-module-elements --generator gen-valid --count 1000 --max-failures 20 --out-dir .tmp/pass-fuzz-genvalid-wide-smoke-rume` are green; focused coverage lives in `gen_valid coverage-forced emits expanded scalar numeric surface`.
   - [FZG]003 - Core Control Surface Expansion - Generate `br_table`, standalone `unreachable`, `local.tee`, and typed `select` in `src/validate/gen_valid.mbt`.
     - Status: complete. Coverage-forced `gen-valid` modules now emit a deterministic core-control prelude covering `br_table`, standalone `unreachable`, `local.tee`, and typed `select`; `GenValidFeatureStats` records exact `br_table`, `standalone_unreachable`, `local_tee`, and `typed_select` counters; the validator floor test proves those rows can satisfy explicit requirements.
...
3. Expand memory, table, global, and bulk-op variation.
   ...
   - [FZG]006 - Memory Limit and Proposal Variants - Add generator support for zero-min, unbounded, shared, and memory64 memory shapes behind explicit config toggles.
     - Status: complete. Coverage-forced `gen-valid` modules now emit zero-min, unbounded, shared, and memory64 memory shapes while preserving valid shared-memory maxima and keeping memory64 behind explicit feature toggles. Natural generation can also vary memory limits/proposals without making memory index `0` memory64, so existing active data offsets and default memory instructions remain type-valid. `GenValidFeatureToggles` now exposes `allow_memory_limit_variants`, `allow_shared_memories`, and `allow_memory64`; `GenValidFeatureStats` records `memory_limit_variants`; and the `MemoryLimitVariants` ledger row can satisfy explicit floors.
     - Validation: `moon test --package jtenner/starshine/validate`, `moon info`, `moon fmt`, full `moon test`, and `bun scripts/pass-fuzz-compare.ts --pass remove-unused-module-elements --generator gen-valid --count 1000 --max-failures 20 --out-dir .tmp/pass-fuzz-genvalid-fzg006-rume` are green; focused coverage lives in `gen_valid coverage-forced emits memory limit and proposal variants` and `gen_valid memory limit variants obey proposal toggles`.
   ...
   - [FZG]007 - Table Limit and Initialization Variants - Generate zero-min/unbounded tables, typed GC/externref tables, and table init expressions where the AST supports them.
     - Status: complete. Coverage-forced `gen-valid` modules now emit zero-min and unbounded table limits, externref and GC/reference-typed table shapes, and table initializer expressions whose const expression type matches the target table ref type. Natural generation can also vary table limits, non-funcref table ref types, and initializer expressions behind the new `allow_table_limit_variants` toggle. `GenValidFeatureStats` now records `table_limit_variants`, and the `TableLimitVariants` ledger row can satisfy explicit floors.
     - Validation: `moon test --package jtenner/starshine/validate`, `moon info`, `moon fmt`, full `moon test`, and `bun scripts/pass-fuzz-compare.ts --pass remove-unused-module-elements --generator gen-valid --count 1000 --max-failures 20 --out-dir .tmp/pass-fuzz-genvalid-fzg007-rume` are green; focused coverage lives in `gen_valid coverage-forced emits table limit and initializer variants` and `gen_valid table limit variants obey toggle`.
   ...
   - [FZG]008 - Const-Expression Expansion - Generate valid `global.get` of prior immutable globals, `ref.func`, and safe GC/ref const-expression forms in globals, table initializers, element offsets, and data offsets.
     - Status: complete. Coverage-forced `gen-valid` modules now emit immutable imported and previously-defined globals for `global.get` const expressions, `ref.func` global initializers, safe struct default-constructor global initializers, table initializer `global.get`, and active element/data offsets that read immutable `i32` globals. `GenValidFeatureToggles` now includes `allow_const_expr_variants`, `GenValidFeatureStats` records `const_expr_variants`, and the `ConstExprVariants` ledger row can satisfy explicit floors.
     - Validation: `moon test --package jtenner/starshine/validate`, `moon info`, `moon fmt`, full `moon test`, and `bun scripts/pass-fuzz-compare.ts --pass remove-unused-module-elements --generator gen-valid --count 1000 --max-failures 20 --out-dir .tmp/pass-fuzz-genvalid-fzg008-rume` are green; focused coverage lives in `gen_valid coverage-forced emits const-expression variants`.
