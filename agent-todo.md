...
- `RemoveUnusedModuleElements`: closed-world typed `struct.new` / `struct.new_desc` payloads now defer unread funcref/global references until a live `struct.get*` read occurs; remaining GC precision is array payload consumers, broader GC content readers, and any additional hardening around reference-only non-function retention.
