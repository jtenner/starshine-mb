...
- [FUZ]1021 - Invalid Binary Malformed-Byte Matrix
  - Status: IN PROGRESS (cron 23, 2026-05-21). Latest prefix-opcode slice adds decode-rejected, non-smoke `invalid-atomic-prefix-subopcode-byte`, which emits a reserved `0xFE` atomics subopcode byte and keeps it distinct from malformed/overwide atomics subopcode ULEB carriers; docs now name the atomics reserved-subopcode byte coverage. Previous section-payload underflow, atomic-prefixed subopcode malformed/overwide, overwide section vector-count, malformed vector-count, and smoke-scheduler slices are landed and documented; continue FUZ1021 by selecting another malformed-byte class such as remaining invalid UTF-8, opcode/prefix, lane, memarg, or atomics operand-context corruptions. Current TDD evidence: focused invalid-binary wbtest first failed on missing `invalid-atomic-prefix-subopcode-byte`, then passed after implementation.
