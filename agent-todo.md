...
- **Status (2026-07-22 partial):** the arbitrary `touched_count > 8` full-roster skip is removed. The ordinary nested replay now derives a Binaryen-shaped worth set from changed function signatures, result-change callers in both original and current call graphs, and exact caller local-count growth for transitional scratch cleanup; plain callsite-only edits and touched callers with unchanged preexisting locals are no longer replayed. Module `>100`, local `>128`, instruction `>1000`, material-caller admission, and exact convergence feedback remain open. Focused guard-removal trials now show two owner-order blockers: module-guard removal preempts selected literal/global producers, while function-size guard removal lets the nested roster consume a 129-local changed boundary before the authoritative broad changed-boundary transaction records and owns that cleanup. A red-first module-guard removal trial exposed phase-order regressions in the selected literal/global fixtures, so the module guard remains until those producers move ahead of or into the common lifecycle.
- **Deliverables:**
