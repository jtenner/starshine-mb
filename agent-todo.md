...
- [O4Z-AUDIT-LCSE] - Deep audit `local-cse`
  ...
  - Deliverables: apply the common checklist; keep the covered before-`if`/then positive, after-`if` and else-arm negatives, before-loop/loop-body, return-boundary, `br_table` boundary, and unreachable-boundary negatives, tiny-root repeated-`global.get` no-op, and repeated `struct.new` generative-root negative green; add remaining GC/generative-root negatives and idempotent-call positives where local syntax and annotation plumbing can model Binaryen safely; coordinate shape tests with `[AUDIT004-J]`/`[AUDIT004-K]`; classify missed CSE opportunities by barrier type.
