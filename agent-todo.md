...
- [O4Z-AUDIT-CF] - Deep audit and widen `code-folding` to Binaryen behavior parity
  ...
  - Deliverables:
    ...
    - [ ] `[O4Z-AUDIT-CF-K]` Refresh oracle signoff after every behavior-widening batch: direct `--pass code-folding --count 1000` first, then `10000` before closing; classify each mismatch as semantic-safe/size-winning, representation-only, size-losing, unknown/risky, validation failure, tool/Binaryen failure, or true semantic mismatch with rationale. Progress: the invalid-output fix lane ran a 100000-case direct smoke at `.tmp/pass-fuzz-code-folding-100000-after-td-fixes` with `0` Starshine command/validation failures; remaining mismatches are classified in the research note as semantic-safe size-winning cleanup drift for pure dead value debris before `unreachable` (`012741`/`043481`, `023083`, `082547`) and semantic-safe but size-losing typed-dead-wrapper representation drift (`046375`).
