...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [RUB]002 - Multi-Slot Cleanup and Artifact Compare - Replay the pass in each Binaryen slot and confirm later `merge-blocks` opportunities match the reference output.
     ...
     - Next oracle step: separate the remaining explicit-pass type-order noise from real RUB body diffs first, then reduce the later loop/block-order family, and rerun self-opt compare plus a larger mixed-generator compare-pass lane before spending more budget on ordered-prefix proof.
     - Remaining work:
       - Reduce a later mutation-backed artifact mismatch to a focused standalone or native `cmd` regression instead of reopening the fixed `br_table` continuation-wrapper family.
     ...
     - Risks:
       ...
       - The new continuation-wrapper slice already showed that broad carrier-family work can regress runtime if no-op guard discovery widens; future work must preserve the new fast structural guards on nested block ladders.
       - Performance is still materially over budget on self-opt compare (`706.823 ms` Starshine pass time vs `104.074 ms` Binaryen on the latest run), so any broader carrier-family rewrite needs a narrow guard and a measured payoff.
     - Doc: [0066#L188](/home/jtenner/Projects/starshine-mb/docs/wiki/raw/research/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L188)
