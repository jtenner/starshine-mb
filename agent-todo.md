...
- **Status (2026-07-22 partial):** canonical direct-call operand slices, canonical parameter-slot actions, and ordinary unused-parameter analysis migration landed. The ordinary scalar transaction now carries a preconstructed complete candidate, exact path/kind call identities, an independent type epoch, and caller-owned operand epochs through revalidation and rollback. Ordinary nested-call/structured-operand localization and solved uniform-value materialization now build typed caller/callee edits inside that same transaction. Remaining exact-literal/immutable/suffix constant paths, broader family migration, GC/results/control/type reconstruction, metadata finalization, and complete touched/invalidation ownership remain open.
- **Deliverables:**
  ...
  - [ ] Move GC refinement, result removal, control reconstruction, and remaining older paths through immutable validated plans. *(ordinary scalar localization and solved `DaeUniformActualWork` materialization are plan-owned; exact-literal/immutable/suffix constants plus forwarding/broad localization remain)*
