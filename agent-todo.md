...
## Post-v0.2.0 Binaryen 133 Intake

### [IR2-V133] U01 — Implement the v133 optimizer-shape corpus

- **Goal / why:** close the source-confirmed optimizer opportunities introduced between Binaryen 132 and 133 while retaining v132 as the current comparison baseline.
- **Deliverables / tasks:** [exact release inventory and shape map](docs/wiki/binaryen/version-133-upgrade.md); implement the red cases for constraint spans, tees, loops and preset slots; tail-call and empty-export passes; publish and resume OI; shared-object conversion; GTO prototype placeholder; saturating i64 lowering; intrinsic tail calls; atomic dealign. Expand the explicitly open guard and subtype variants before signoff.
- **Required APIs / invariants:** valid tail-call result and EH handling; shared/stack-switching opcode and feature support; separate suspension effects; natural atomic alignment; JS-visible descriptor field order; imported/exported wrapper type correctness. Preserve operand evaluation, trap order, and all required effects.
- **Dependencies:** land source/IR support before publish and resume rewrites; whole-module type remapping before shared-object lowering; complete effects before continuation and tail-call motion. Verify any new evidence against the official v133 binary without replacing the ordinary v132 oracle.
- **Fuzz progress:** the initial 61 focused tests and 12,334 default wasm-gc tests passed in the green-phase commit `00ba1836a`; four additional regressions cover shared-object table elements, the intentionally unsupported intrinsic boundary, and direct/dynamic non-tail intrinsic calls. The refreshed default suite passes 12,338/12,338. Post-commit 10,000-case v133 lanes now cover CA, OI, tail-call, empty exports, intrinsic lowering, global effects, GTO, dealign, and shared objects; CA also has a verified v132 lane. The non-tail intrinsic fix removed all 55 intrinsic-specific comparison differences; the shared-object intrinsic guard removed all 55 Starshine validation failures in a 10,133-comparison rerun. The four new i64 conversions passed 40,000 deterministic original-vs-lowered runtime calls. A later red-first GTO slice removes the unread field of one isolated private struct and retains constructor effects; renewed 10,000-case v132/v133 random-all and v132 GC-subtype lanes are recorded in the v133 wiki page.
- **Exit criteria / suggested tests:** retain open parity work until the renewed 1,201 GTO random-all mismatches, 10,000/10,000 GTO GC-subtype mismatches, 2,216 shared-object, 2,632 CA, and 1,080 OI v133 output differences are classified with semantic and size evidence; smaller generic control/representation differences in the other lanes also remain open. GTO's random-all 864 canonical size losses and GC-subtype 6,250 losses especially need reduction; build a private-struct aggregate and implement hierarchy-aware removal. All 86 shared-object size losses arose in older `remove-unused-brs-*` generator profiles; shared-object-specific output differences still need proof or alignment. The 167 v133 shared-object oracle command failures comprise released `try_table` and intrinsic validation bugs; avoid treating them as Starshine comparisons. Complete the remaining released exception, descriptor, subtype, atomic, `struct.wait`, and i64 wasm2js variants with red fixtures before full release signoff. The i64 implementation currently covers only the four new saturating conversions. Keep the ordinary baseline at verified v132.
