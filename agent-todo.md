...
- [DAE]006 - Both-Canonical Frontier Advancement
  ...
  - Next action:
    - Add a focused Moon regression for DAE-touched `return` followed by same-sequence allocation/store/value debris, then extend the smallest safe DAE dead-suffix/raw-cleanup path to drop only instructions syntactically after a root `return` while preserving all operands/effects before the return.
