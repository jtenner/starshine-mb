...
- The definitive fresh original-input WAGO checkpoint is now `.tmp/wago-o4z-pass8-final13-drop-shells-urem-fresh-v2-20260821/`, covering all 1,330 tracked fixtures at or below 2 MiB and the unchanged 766-file stable cohort with native SHA-256 `6c0e1d6a48a3ef28f8123aae78a621182b12055d538b0a2e491ba979f5f41b45`. Starshine emits 413,343 bytes versus pinned Binaryen v131 at 415,485, a fresh 2,142-byte / 0.516% win with 110/599/57 smaller/equal/larger, 3,068 positive residual bytes, 46,787 bytes removed from the 460,130-byte baseline, and 104.80% original-gap closure. Exactly three stable-cohort artifacts change without regressions: issue1809 897 → 882, urem 1,149 → 1,136, and fasta 23,724 → 23,721. Final-only cleanup now removes constant-RHS nontrapping integer operators whose result is dropped while preserving the left operand's effects/traps and converts dropped `local.tee` shells to `local.set`; an exact dropped i32 result-block select shell retains its potentially trapping `i32.load8_u`; division/remainder remains untouched. The 37-function stateful Emscripten candidate family fails closed because the intermediate rewrite changed local allocation and made fannkuch seven bytes larger; guarded fannkuch remains byte-identical at 23,332 while 41-function fasta keeps its three-byte win. The exact one-function/38-local/17-page urem family receives canonicalization-only post-selection competition; 80 paired runs versus the same-source disabled binary are 104.982 versus 104.744 ms (+0.23%, measurement-neutral), while a rejected extra precompute continuation cost +1.23% and a rejected full second schedule cost +18.77%. Issue1809 preserves two memory-growth traps, pages, immutable exported globals, and full-memory hashes; urem preserves 2,000 deterministic call results/trap classes, stack-pointer state, and import-event counts; fasta preserves allocator/event ordering and full-memory hash `c5f619ac…b464`. The runtime-safe retained projection remains distinct and now projects 412,207 versus 415,485, a 3,278-byte / 0.789% win with 112/598/56 smaller/equal/larger, 2,029 positive residual bytes, 47,923 bytes removed, and 107.34% gap closure. Full validation is 10,600/10,600; all passes 6,951/6,951; command 267/267. Remaining fresh leaders are ifs +1,434, JSON-SIMD +397, primes +378, fannkuch +170, fasta +129, scalar JSON +86, startup JSON +60, urem +38, issue1809 +35, fuzzcase 1825 +33, deeply nested struct +25, bump-pointer GC +24, and return-call-indirect +21 each. Preserve xjb/mandelbrot fallbacks, packed-array semantics, stateful runtime exclusions, performance nonregression, and fresh/projection provenance.
