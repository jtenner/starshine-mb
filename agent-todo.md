...
| `dae-optimizing` | Represented Binaryen-v131 behavior, common lifecycle ownership, four-lane residual classification, validity, and exact-once nested convergence are closed through `676b90f68`; current generated residuals are all smaller or equal-size fewer-local wins. | **Open performance only:** `[O4Z-DAE-PERF]001`. Cached lifecycle facts and single-owned commit handling reduced the dense-call artifact from `>1500s` to `12.093s` with byte-identical valid output, but Binaryen remains `0.356s`; no semantic, validation, size-losing, or lifecycle blocker remains. |
...
### [O4Z-DAE]001 - Close plain DAE and DAEO for v0.1.1

- **Status (2026-07-24):** represented behavior and lifecycle closure complete through `676b90f68c2993899c1f5432a2d4253dead50774`. Current bounded suites are DAE whitebox `395/395`, DAEO public `342/342`, pass-manager `306/306`, and full Moon `9908/9908`. Native SHA-256 `e6d703492f7008bd3265569e8620cfb6514cd1604e533e4bf497ed03bf8c066e` completed regular `100000`, dedicated `10000`, random-all `10000`, and wasm-smith `10000` lanes for both variants with zero Starshine failures and zero positive canonical-size residuals. Exact-once case-515 tracing records one core commit, one shared nested roster, one empty frontier wave, and zero rollbacks. Release remains blocked only by `[O4Z-DAE-PERF]001`. The detailed source-backed record is `docs/wiki/binaryen/passes/dead-argument-elimination/reopened-parity-plan.md`.
- **Release target:** `v0.1.1`; do not change versions, tag, or publish as part of this task.
- **Goal:** represented Binaryen-v131 DAE behavior and lifecycle ownership are complete; close the remaining artifact-scale performance blocker.
- **Why:** generated semantics, validity, residual classification, exact-once convergence, and repository gates are green, but the dense-call stress result is not an acceptable unreviewed release state.
- **Non-goals:** byte-identical output; optimizing indirect/call_ref callers that upstream DAE treats as unseen; speculative opcode/proposal breadth; duplicating shared local-layout algorithms inside DAE.
- **Dependencies:** `[O4Z-DAE-PERF]001` only for DAE release closure.
- **Exit criteria:** `[O4Z-DAE-PERF]001` is fixed or explicitly accepted by release review, and the final docs/commit tree is clean.
- **Release-closure category-6 checklist (closed 2026-07-24):** all represented phase-local applications, result/control/type products, stale retries, invalidation, diagnostics, cleanup, and convergence now route through producer-owned outcomes/effects; unsupported exceptional products remain narrowly fail closed. DAEO owns one shared exact-once nested roster and one effect frontier. Cleanup-deleted dependencies seed from pre/post graphs, and optional large-module batches remain acceleration-only.
...
- **Deliverables:**
  ...
  - [ ] Move GC refinement, result removal, control reconstruction, and remaining older paths through immutable validated plans. *(dispatcher families, optimizing unread/dropped/GC batches, ordinary-core GC parameter refinement and dropped-result removal, reverse/low exact revisits, low exact cleanup, broad immutable-global and adjacent-root revisits, dead-suffix parameter restoration, boundary cleanup, verified immediate suffixes, selected ref-null compatibility producers, and the late semantic global-suffix specialization now crosses the common signature plan; specialized dropped-result products now fail closed unless they project into the common signature plan; signature-changing control/type candidates now fail closed unless they project into `DaeRewritePlan`; body-only control/type mutation helpers remain)*
...
- **Deliverables:**
  - [ ] Strict-improvement reference-parameter LUB across every owned direct operand, including old-type local repair for incompatible writes. *(direct-GC batch discovery is definition-count-independent; direct actual evidence now covers allocation, exact `ref.func`, direct/tail call, typed indirect/tail-indirect, and typed call-ref/tail-call-ref producers across parsed and decoded abstract reference params; ordinary-core parameter refinement dispatches `DaeGcParameterRefinementWork`, derives exact `DaeRefineParameterType` plus any simultaneous `DaeRefineResult` actions, and commits through `DaeRewritePlan`; the optimizing direct-GC batch commits refreshed per-definition combined candidates through the same action-bearing plan with exact merged effects; written refined parameters from both ordinary-core and optimizing batch producers now receive entry-initialized old-type locals with all nested gets/sets/tees remapped; nonnullable old types use nullable storage plus recursive `ref.as_non_null` repair on gets/tees, matching Binaryen's nondefaultable-local policy. ordinary-core collection plus optimizing-batch initial and stale-refresh evidence now share one exact-caller, fail-closed actual-value collector, and both parameter LUB application paths share one exact-length, exact-call-count strict-refinement combiner; concrete subtype and ancestor walks now use flattened type-section slots and resolve group-local `RecIdx` supertypes against the owning recursive-group base, so members after the first entry in recursive groups participate in LUB legality; the obsolete multi-target mutable collector and private batch plan have been deleted)*
  - [ ] One returned-value API for fallthrough, explicit returns, block/if/loop/select, typed/typeidx controls, static locals, declared globals, and typed GC struct-field/array-element/table reads, tuples, flat multivalue lanes, and represented exceptional exits. *(GC result closure derives nested direct result dependencies from stable call identities; single-result analysis joins multiple nested explicit returns with fallthrough, recovers terminal and nested explicit-return plus fallthrough values through canonical multi-instruction `DaeValueSlice` evidence, treats `ref.null` arms as nullable bottom and rewrites control-pair nulls to the joined result type; direct `ref.null` result evidence participates in the generic instruction candidate path, so null-bottom explicit returns join with sibling/fallthrough leaves instead of aborting the multi-return collector; represented legacy `try` protected and catch bodies participate in return discovery, preventing an unsound narrower fallthrough result when exceptional regions return a wider type; terminal all-return block/loop/if/try-table/legacy-try controls now contribute their explicit-return LUB without requiring a fallthrough value, typed `try_table` normal fallthrough shares the block/loop single-body reconstruction while preserving its catch table; typed legacy-`try` normal fallthrough now refines when every catch is proven nonfallthrough and fails closed when a catch can supply a wider value; typed `local.get` and effect-preserving `local.tee` terminal slices derive the declared local reference type through the same candidate path; it accepts parsed/decoded abstract heap LUBs, derives exact `ref.func` result types, follows direct and typed call/tail-call indirect/reference signatures across concrete and abstract heap results, including call-ref slices whose function reference comes from any branch-free single-value producer rather than only `ref.func`; effectful argument/target producers are preserved because returned-value evidence does not rewrite the slice, and canonical `DaeValueSlice` recovery handles multi-instruction argument and target expressions, and ordinary plus descriptor-bearing struct/array constructors, exact `cont.new` / `cont.bind` constructors, `string.const`, all represented string array constructors, `string.concat`, `string.as_wtf16`, `stringview_wtf16.slice`, `ref.i31`, typed `ref.cast`, `ref.cast_desc_eq`, `ref.as_non_null`, `ref.get_desc`, plus typed `struct.atomic.get` / `array.atomic.get` reference reads and `extern.convert_any` / `any.convert_extern` result expressions enter the same reference-refinement evidence with exact heap/nullability, conversion-preserved nullability, and trap preservation, and refines self-called functions when recursive results do not flow out, while decoded flat multivalue fallthrough lanes are covered and tuple/control producers still need consolidation)*
  - [ ] Lane-wise result LUB plus call, tee, parent-control, and caller refinalization. *(productive ordinary-core GC parameter/result and dropped-result commits, plus GC result dependency closure, construct and drain epoch-bearing `DaeControlReconstructionWork` and `DaeTypeRepairRetryWork` followups through one executor using producer effects and stale-item refresh. Result-only GC refinement now represents exact `DaeRefineResult` actions and commits its signature/body/type candidate through the common `DaeRewritePlan`; combined parameter/result and shrinking dropped-result products use the same action plan, while broader lane/caller repair remains open)*
  - [ ] One deterministic type edit plan covering reuse/append, typeidx controls, rec groups, exactness/nullability, abstract heaps, tags, call_ref/call_indirect, tables/elements, globals, and metadata policy. *(flattened function-type lookup/reuse/append and concrete subtype/ancestor traversal now share flattened recursive-group indexing; the ordinary matcher, batch lookup, and selected typeidx-control helper all reuse simple function signatures from grouped members while excluding supertypes/descriptors; pruning, mixed-group edits, and complete metadata/user remapping remain open)*
...
- **Status (2026-07-23 partial):** committed work-item effects drive dependency requeue by explicit invalidation domain across before/after graphs. Result invalidation uses exact undropped observer/direct-tail rows, metadata-only changes remain local, and common work families publish narrow parameter/materialization, result, and control-only domain sets. Dispatcher unread/localization reuses the supplied immutable original/current graph, and stack-carried void-effect recovery is plan-owned. The canonical core worklist now aggregates `DaeDiagnostics` in production and both plain DAE and DAEO emit one deterministic trace record containing every counter plus every stable reason code; cleanup reentry emits its own scoped record. Optimizing unread/direct-GC/dropped-result signature batches now carry additive produced/applied plan counts in `DaeCommitEffects`, so merged transactions retain exact diagnostic multiplicity. Ordinary DAEO nested replay shares that diagnostics object, delays the primary record until cleanup completes, and reports exact admitted touched-function and scheduled shared-roster pass counts. Producers without domains retain the conservative caller/callee fallback; legacy result/type/body/cleanup loops and selected, specialized-mini-roster, and convergence-wave diagnostic counters still need migration.
- **Deliverables:**
  ...
  - [ ] Plumb production counters for selected-only admission, guards, stale retries, rollbacks, localization, type/result edits, and nested replay. *(plain DAE and DAEO now emit deterministic canonical-core records for analyzed/produced/applied/requeued work, semantic edit counters, and all stable reason codes; ordinary optimizing signature batches report exact per-plan produced/applied counts and DAEO shared-roster replay reports exact touched/pass counts; selected-only, guard, scratch-local reuse, effect replay, specialized-loop, producer-specific mini-roster, and convergence-wave counters remain)*
...
- **Status (closed 2026-07-24):** DAEO derives worth only from producer-owned effects, runs the shared `precompute-propagate`-first roster exactly once per admitted wave, and converges through one epoch-bearing effect frontier. The former private mini-rosters, folded-callee recursion, whole-pass retry, artifact-size selectors, local-growth inference, and duplicate convergence owners are deleted. The frontier seeds callees from both pre-cleanup and post-cleanup dependency graphs, so cleanup cannot strand a newly uncalled target. Case `515` proves one core commit, one shared `41`-pass nested execution, one empty frontier wave, and zero rollbacks.
- **Deliverables:**
  - [x] Define `worthOptimizing` from productive param/refinement/localization/result changes and affected callers, not artifact size or definition identity.
  ...
  - [x] Reclassify the former canonical-positive random-all family after current closure: all `688` DAEO random-all residuals are smaller, with aggregate delta `-633,486`; retained historical artifact local-layout evidence remains a shared-owner checkpoint rather than a current generated blocker.
...
- **Deliverables:**
  ...
  - [x] Add arbitrary-definition red-first tests for generic recognizers and keep artifact functions only as regressions.
  - [x] Keep plain and optimizing traces, normalizers, matrices, artifacts, and medians separate.
  - [x] After behavior widening, run focused/white-box/public tests, serial full tests, wasm-gc CI validation, external validation, and explicit-v131 regular/dedicated/wasm-smith/random-all lanes.
  - [x] Classify every residual per source family/function; no missing or larger family is offset by unrelated aggregate wins.
  - [x] Reconcile completion-matrix lifecycle rows and exact slot-48 execution; retain the repository's current `v0.1.1` label without changing versions.
- **Exit criteria:** behavior/evidence criteria are complete; accepted artifact-scale performance and no active DAE-owned release blocker remain under `[O4Z-DAE-PERF]001`.

### [O4Z-DAE-PERF]001 - Resolve dense-call artifact-scale pass-local performance

- **Owner:** original-boundary snapshot collection and dependency/worklist setup in `src/passes/dead_argument_elimination.mbt`.
- **Goal:** make plain DAE complete a dense-call artifact-scale module within an explicitly reviewed release bound without changing semantics, dropping evidence, or adding representation gates.
- **Why:** on 2026-07-24, Starshine plain DAE initially exceeded the `1500`-second command bound on `.tmp/starshine-predecessor-current.wasm` (`3,597,499` bytes, `5,374` definitions, roughly `251,000` rendered direct calls); Binaryen v131 `--dae` completed in `0.356s`. The current implementation completes in `12.093s` and emits the same valid SHA-256 `fa73c6aac3c93ac9c97bd261ce24403c4b7cfb8fbdb417f4ed74e1488caf11a9` output as the intermediate clean runs. The timeout is resolved, but the pass-local gap remains release-review work.
- **Deliverables:**
  ...
  - [x] Reuse the fixed-loop epoch/dependency graph in the plain post-revisit exact-literal worklist when no intervening producer committed; a same-session native A/B improved `65.833s -> 62.265s`.
  - [ ] Reduce the remaining `12.093s` fixed-loop/selected-lane cost to an explicitly accepted release bound, then renew the focused lifecycle trace, generated four-lane matrix, and repository gates.
