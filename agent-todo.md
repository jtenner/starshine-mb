...
- **Current progress:** analysis-only Flat IR classification, scalar function-result materialization, reachable/unreachable `local.tee` lowering at function roots, structured-region roots, and ordinary operand positions, ordered scalar ordinary-operand preludes, branch-free defaultable scalar `block`/`if` result routing, and plain carried scalar `br` routing into one temp per defaultable scalar block target are implemented with focused verifier-backed tests. Direct terminal `br` now unblocks its target block's result erasure; `br_if` flowing values, `br_table`, branch-targeted `if`, loop/try values, generic unreachable placeholders, EH, profiles, and public wiring remain open.
- **Deliverables:**
