...
- [ ] Add an end-to-end `re_reloop` fixture that exercises non-direct `RRBrTable` targets via module reconstruction (not only helper-level CFG tests) and keeps scratch-local insertion covered.
- [ ] Consolidate duplicated `RRBrTable` dispatch-building logic in `re_reloop` (dup-safe and scratch-local branches) to reduce divergence/regression risk.
- [ ] Decide and document parser behavior for `(func (export "...") (import ...))` shorthand; add explicit parse/lowering diagnostics or support tests.
