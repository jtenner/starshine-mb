...
- **Deliverables:**
  ...
  - [ ] Remove parity-affecting `1024`/`4096`, prefix, nullable-count, and phase-only admissions; retain only explicitly optional cleanup budgets. *(the common exact-literal worklist no longer accepts an external productive limit or inert call-facts argument, and both early uniform-constant collection lanes now use one module-size-independent semantic admission policy. The computed/unread dependency worklist now also runs at every optimizing module size. The separate broad-cycle preservation gate, nullable-count, and other phase-order admissions remain tracked.)*
