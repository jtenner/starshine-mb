...
- [O4Z-AUDIT-SSA] - Deep audit `ssa-nomerge`
  - Status: direct explicit-pass corruption, final-lane unreachable-debris drift, first debug-artifact nested-param/body-local shape gaps, `Func 4302` false-positive suspicious skip, the high-threshold raw-structured artifact size regression, and most large-structured raw no-op coverage were fixed/reduced on 2026-06-09; audit remains active for exact artifact parity, the remaining huge structured functions, and O4z slot policy/neighborhood replay.
  ...
  - Remaining deliverables: classify or reduce the remaining exact artifact first diff after `.tmp/self-ssa-nomerge-debug-wasi-canonical-local` (`defined=0 abs=27`, currently representation/local-numbering drift and not proven corruption); keep the remaining `17` huge `large-structured-local-writes` functions in backlog until measured raw-structured rewrite cost and validation evidence exists (2026-06-10 sampling found the smallest skip is `Func 3668`, `4509` instrs / `620` locals, and the next named target is `Func 1265`, `4563` / `614`, both instruction-threshold-only); replay early `SSA` slot / artifact neighborhoods; decide whether the current O4z `ssa-nomerge` no-op remains intentional or can be narrowed; resolve or route the current `-O4z` debug-artifact abort in a later `remove-unused-names` `Func 2560` neighborhood before using preset evidence to re-enable SSA; file pass-local runtime fixes here and whole-command residuals under `[WALL]001`.
