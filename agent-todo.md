...
- **Current differences:**
  ...
  - [ ] Close the reopened `flatten` signoff exposed by the expanded RUB corpus. Historical evidence was `837` valid `-O4z` outputs and `163` Starshine command failures; effective-terminal/no-fallthrough routing now makes the exact repro and the original first-1,000 corpus green at `1,000/1,000`. First-class HOT lifting now admits single-arm tagged catches (including payload lanes), `catch_all`, direct delegates, result-typed legacy tries, and legacy `rethrow`; lowering preserves those forms for unrelated HOT pipelines. Fresh explicit-v131 lanes on native SHA-256 `4b60d4b09f573a82419cef9d5ef5a6d88018a4a7edf6c18f7e44b97e017bb217` are: default `10,000/10,000` matched and idempotent; `flatten-legacy-eh` `10,000/10,000` compare-normalized, runtime-equal, and idempotent; `merge-locals-legacy-eh` `10,000/10,000` compare-normalized, runtime-equal, and idempotent; and `flatten-all` `10,000/10,000` valid/idempotent with `1,632` direct plus `6,701` compare-normalized matches and only the `842` scalar-`br_if` plus `825` multivalue measured-win candidates. Random-all remains `9,629` comparable with `371` Binaryen-only failures, but residuals fall from `819` to `781`; all `38` `merge-locals-legacy-eh` bridge gaps are gone, leaving the same eight non-EH families led by `546` coverage-forced canonical-encoding cases. A representative legacy result try is `67` versus Binaryen `71` bytes and both converge to `37` bytes under v131 `-Oz`. Multi-arm legacy tries remain an explicit upstream HOT representation boundary. An August 1 review additionally found that a later lowered tee could mutate a local before an earlier sibling `local.get` consumed its value; dependency-aware prelude staging now snapshots matching direct and wrapped reads, preserves multiple reads, and avoids nonmatching snapshots. A Node runtime fixture preserves results `42`, `0`, `41`, and `42`. No-fallthrough queries now consume one summary per region rather than repeatedly scanning complete root lists; counters prove one scan for unchanged cached regions, cached positive facts recompute when their terminator root changes, and unseen post-mutation regions fail closed, while the new nested/dead-suffix benchmark is performance-neutral (`196.667 ms` versus `197.103 ms`) and therefore does not requalify the pass. Rebuilt native SHA-256 `65c8298709b14006bcf2c07da970aaf15e0a5125f96eb71def75305740a10ee6` completes a restarted `flatten-all` `10,000/10,000` lane with the unchanged `1,632` direct, `6,701` cleanup-normalized, `842` scalar-`br_if`, and `825` multivalue populations; idempotence is `10,000/10,000`, all `13,270` runtime exports agree, and failures are zero. The attempted fresh default `10,000` lane timed out during input generation before producing cases and is not evidence. Close only after final classification of the coverage-forced and smaller non-EH families, the deeper ownership/failure-atomicity audit, and exact historical performance remeasurement. The new 120-function/960-branch stress lane is linear and valid but measures `8,987 us` Starshine versus `926.059 us` Binaryen v131 (`9.70x`), so it is not a replacement for the exact historical 120-function candidate-dense qualification. Isolated direct RUB at O4z levels `4/4` remains valid and no-larger for all `10000` cases.
