...
2. Land the first proof pilot in `env`.
   - [PRV]002 - `Env` / `LabelStack` proof kernel.
     - Deliverables: extend the `src/validate_proof` helper kernel beyond the already-landed `label_stack_storage_index`, `latest_stack_index`, `group_relative_absolute_index`, `group_member_relative_index`, `defined_func_body_index`, `defined_func_absolute_index`, and `bounded_index` helpers, prove the remaining stack-from-top label lookup and rectype-resolution invariants identified in the strategy doc, and keep the existing `env_tests` oracle green.
     - Required APIs: `label_stack_storage_index`, `latest_stack_index`, `group_relative_absolute_index`, `group_member_relative_index`, `defined_func_body_index`, `defined_func_absolute_index`, `bounded_index`, `LabelStack::new`, `LabelStack::push`, `LabelStack::get`, `Env::with_rectype`, `Env::resolve_subtype`, and `Env::resolve_heaptype_subtype`.
