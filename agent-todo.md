...
- **Latest bounded iteration (2026-07-15):** commits `18a868a5d`, `9c6d614c9`, and `40d4fad8c` admit one exact owned resultless call argument with one multiply child, two multiply children, or a bounded deeper two-multiply-plus-constant tree after unconditional legacy-try table transfer. A fixed detached-baseline matrix moved from `0/129` to `126/129` transformed functions across five of seven modules; alternate-unary and structured suffix functions remain gated. Starshine encoded-size and generated-runtime comparison is blocked because synthetic legacy `Try` cannot lower while lifted WAT activates the catch-payload repair gate. Native pass-only timing on 120 dense functions is `9,592.5 us` current versus `551 us` baseline and `334 us` Binaryen v130, so EH representation, actual output measurement, and performance remain public-exposure blockers. Source: `docs/wiki/raw/binaryen/2026-07-15-flatten-version-130-nested-call-argument-impact.md`.
- **Deliverables:**
