...
- Both-canonical diagnostic helper: `scripts/self-optimize-compare.ts --canonicalize-binaryen-output --dae-optimizing` currently first-diffs at `defined=505 abs=522` in `.tmp/dae-func505-bool-carrier-artifact`. Research notes `0576` through `0584` reduce the saved Func505 body from unknown/risky output-shape drift to representation-only control/local-carrier, branch-polarity, temp-local, and dropped-debris drift, but no diagnostic normalizer or pass-shape cleanup has moved the artifact frontier yet.
...
- [DAE]006 - Both-Canonical Frontier Advancement
  ...
  - Current classification:
    ...
    - `0584-2026-05-26-dae-func505-loop-carrier-reduction.md` reduced the remaining loop induction/exit-carrier subshape and classifies it as representation-only control/local-carrier drift: Starshine's high-local `index < length` carrier and extra exit block align with Binaryen's low-local `length > index` carrier and direct exit block under the same parse-loop effects and continuation. The saved Func505 first-diff body is now classified representation-only after inspected subshape reductions, but canonical compare still first-diffs at Func505 until a diagnostic normalizer or pass-shape cleanup changes the text.
  - Next action:
    - Choose whether to add narrow diagnostic-only normalizers for the now-classified Func505 representation families and replay the both-canonical artifact to discover the next frontier, or leave Func505 as a documented representation boundary and work another DAE backlog slice.
