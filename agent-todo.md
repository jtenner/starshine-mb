...
- [DAE]001 - Call-Graph Pruning and Touched-Function Tracking
  - Status: partial active implementation landed on 2026-05-11 and was extended the same day. `dae-optimizing` and `dead-argument-elimination-optimizing` are active module-pass names; the current core handles private direct-call dead scalar parameter removal, self-recursive forwarded-parameter pruning for adjacent `local.get` operands, dropped/uncalled no-param result removal, dead-suffix local-use suppression after root `unreachable`, removed-actual side-effect preservation including value-producing `if` operands, export / `ref.func` escape bailouts, and unused simple function type pruning.
  - Remaining deliverables: broader Binaryen DAE parity for constant actual materialization, non-adjacent/localized forwarding and recursive cycles, GC param/result refinement, full call operand localization, complete result-removal scheduling with dead internal call filtering, and any newly discovered semantic direct compare mismatches beyond the saved 1000-case local-declaration frontier. Current evidence: `.tmp/pass-fuzz-dae-690-final2-1000` at `--count 1000 --seed 0x5eed --pass dae-optimizing --max-failures 20 --jobs auto` reported `998/1000` compared, `985` normalized matches, `13` mismatches, `0` validation failures, and `2` Binaryen/tool command failures after the case-000690 escaped-result self-call operand fix. Follow-up classification in `docs/wiki/raw/research/0558-2026-05-12-dae-local-declaration-frontier.md` found all 13 remaining `gen-valid` mismatches are local-declaration-only diffs, while the two command failures are unchanged Binaryen/tool `binaryen-rec-group-zero` parser failures. Artifact replay `.tmp/dae-optimizing-artifact-replay-finish2` stays semantically red at first differing function `defined=11 abs=28` but Starshine pass-local time was faster (`665.946ms` vs Binaryen `935.447ms`).
