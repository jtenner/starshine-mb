...
- **Latest bounded iteration (2026-07-15):** nonthrowing synthetic catch-all lowering and recursive suffix ownership remain the current narrow measured output win (`212` Starshine cleanup bytes versus `236` Binaryen). Two red-first code commits added an exact terminal legacy-try table proof cache that fails closed when only suffix ownership was cached, admitted direct `i32.shr_u` resultless-call roots under the recursive distinct one-use proof, avoided duplicate generic routing after payload-bearing `br_if`/`br_table` rewrites, and moved the private outside-roster boundary to `i32.rotl`. The exact phase profile attributes median 120-function batch time to `55.5 us` classify, `152.5 us` state construction, `251.5 us` admission, `684.5 us` rewrite, and `5 us` result handling. The final representative pass-only rerun is `1,182.5 us` (`4.44x` Binaryen v130's `266.05 us`), improved from iteration-start `1,259 us` but still outside the `<=2x` target. Typed catches/payloads, nested-pop repair, `rethrow`, `delegate`, structured suffix control deletion, broader behavior, a GenValid profile, four-lane signoff, performance, and public wiring remain open. Source: `docs/wiki/raw/binaryen/2026-07-15-flatten-version-130-nonthrowing-bridge-suffix-cache-impact.md`.
- **Deliverables:**
