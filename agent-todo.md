...
- Latest diagnostic timing is over the 2x target for the current Func509 artifact: timing-only helper replay `.tmp/dae011-timing-only-20260526` completed without normalized-WAT comparison and reported `2785.602ms` Starshine pass versus `836.627ms` Binaryen pass. `[DAE]011` remains open for selected-lane profiling/optimization; top-level detail timers from `.tmp/dae011-detail-trace.stderr` attribute the release traced artifact run to `2748777us` in `detail:dae:core`, `4us` in skipped `detail:dae:nested-total`, and `47855us` in `detail:dae:prune-unused-simple-func-types`. Core-family detail timers from `.tmp/dae011-core-detail-trace.stderr` attributed core to `85798us` setup, `486741us` fixed primary loop, `1917924us` selected artifact-specific lanes, and `96963us` module raw cleanup. Selected-lane detail timers from `.tmp/dae011-selected-detail-trace/stderr.txt` attributed that selected-lane aggregate to `1654447us` in selected dropped-result removal, `171910us` in post-raw selected lanes, and sub-`70000us` buckets for reverse exact-literal, low-wrapper callee, early-shape, literal/unread, and late-shape lanes; total `detail:dae:core` was `2753535us`. Dropped-result detail timers from `.tmp/dae011-dropped-result-detail-trace/stderr.txt` attribute the dropped-results bucket to `14279us` in shared call facts, `1662875us` in the selected-def candidate loop, and `25us` in adjacent Func299 cleanup. Research note `0599` fixed the selected dropped-result helper to respect shared `dropped_calls` facts before entering the expensive helper, but rebuilt timing `.tmp/dae011-dropped-facts-timing-rebuilt-20260526` still reports `2807.905ms` Starshine pass versus `850.135ms` Binaryen pass. Research note `0600` then split selected-def helper internals: `.tmp/dae011-helper-detail-trace/stderr.txt` attributes `1629234us` to repeated helper calls, dominated by whole-module `rewrite-calls` (`567297us`), whole-module `remove-call-drops` (`509930us`), and `undropped-scan` (`233609us`); type-index and section-copy costs are negligible. The refreshed timing-only replay `.tmp/dae011-helper-timers-20260526` still reports `2773.402ms` Starshine pass versus `856.877ms` Binaryen pass, so the next DAE011 slice should be caller-filtered/batched dropped-result rewriting using precomputed direct-caller facts.
...
- [DAE]011 - Performance Stabilization
  ...
  - Deliverables:
    ...
    - [ ] Optimize the selected-def candidate loop with a caller-filtered or batched dropped-result path, especially using `DaeCurrentCallFacts.direct_callers[callee]` to avoid repeated whole-module scans while preserving self-call, dead-suffix, and fact-consistency guards.
