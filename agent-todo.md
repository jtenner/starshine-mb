...
- [FUZ]1006 - GenValid Call, Reference Call, Indirect Call, Recursion, And Tail-Call Widening
  - Status: IN PROGRESS (cron 23, 2026-05-21) - callable-reference local/block/global/table, direct-call graph, nonzero `call_indirect`, and nested direct/indirect/ref-tail slices landed; resume remaining richer nonzero-table `call_indirect` diversity and broader/deeper valid tail-call terminal placement widening.
  ...
  - Current evidence: the 2026-05-21 callable-reference source-variant slices make coverage-forced modules reserve a typed function-reference local for the declared callable, add a typed function-reference global initialized by a valid null const expression, and emit validated ordinary `call_ref` forms fed by `ref.func -> local.set -> local.get`, a typed function-reference `block` result, and `global.get`. The follow-up 2026-05-21 direct-call graph slice makes coverage-forced modules emit validated direct `call` traffic to an imported function, a recursive self-call, and a pair of mutually recursive defined functions. The next 2026-05-21 indirect/tail slice makes coverage-forced modules emit ordinary `call_indirect` through table `0` with dynamic element indices `0` and `1`, and a `return_call` nested inside a terminal control block. The follow-up table/ref-tail slice makes coverage-forced modules emit a typed function-reference table value feeding `call_ref` and adds nested terminal `return_call_indirect` at table element index `1` plus nested terminal `return_call_ref` fed by a typed function-reference block result. Focused TDD: `moon test src/validate` first failed on missing `coverage-forced GenValid routes call_ref through locals and block results`, then passed after implementation; then failed on missing `coverage-forced GenValid emits imported self and mutual direct calls`, then passed after implementation; then failed when the call-ref source test was tightened to require a `global.get -> call_ref` source, and passed after the typed global source implementation; then failed on missing `coverage-forced GenValid emits nonzero call_indirect and nested tail calls`, and passed after the nonzero indirect/nested-tail implementation; then failed when the call-ref source and nested-tail tests were tightened to require table-fed `call_ref`, nested `return_call_indirect`, and nested `return_call_ref`, and passed after the table/ref-tail implementation. Direct-call/global-callref slice validation: `moon info`, `moon fmt`, `moon test`, and both `bun scripts/pass-fuzz-compare.ts --pass directize --generator gen-valid --count 1000 --out-dir .tmp/pass-fuzz-directize-fuz1006-direct-calls-1000` and `.tmp/pass-fuzz-directize-fuz1006-global-callref-1000` passed; the latest indirect/tail slice validation also passed `moon info` (only pre-existing DAE warnings), `moon fmt`, `moon test` (`3456/3456`), and `bun scripts/pass-fuzz-compare.ts --pass directize --generator gen-valid --count 1000 --out-dir .tmp/pass-fuzz-directize-fuz1006-indirect-tail-1000` (`1000/1000` compared, `1000` normalized matches, `0` validation/generator/command failures or mismatches). The latest table/ref-tail slice validation passed `moon info` (only pre-existing DAE warnings), `moon fmt`, `moon test` (`3456/3456`), and `bun scripts/pass-fuzz-compare.ts --pass directize --generator gen-valid --count 1000 --out-dir .tmp/pass-fuzz-directize-fuz1006-table-ref-tail-1000` (`1000/1000` compared, `1000` normalized matches, `0` validation/generator/command failures or mismatches). Remaining work still includes richer nonzero-table `call_indirect` diversity and deeper valid tail-call terminal placement beyond the currently covered terminal control-block variants.
