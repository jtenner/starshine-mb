...
- The current unreduced artifact frontier is still anchored at the preserved-`nop` difference in `Func 29`, and the later `Func 41` family is narrower now: the old tee-backed copied-local drop is fixed in-tree, leaving a smaller alias-only `$2` vs `$4` local-use drift inside the `then` arm.
