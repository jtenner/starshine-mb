...
- **Status (2026-07-22 partial):** canonical direct-call operand slices, canonical parameter-slot actions, and ordinary unused-parameter analysis migration landed. The scalar and forwarding plans carry complete candidates, exact call identities, type/member/caller epochs, original ownership, validation, and rollback. `DaeFamilyRewritePlan` now wraps every dispatcher result plus the optimizing unread/dropped-result/GC batches and ordinary core candidates before commit. Legacy selected cleanup/application entry points, metadata finalization, and complete touched/invalidation ownership remain open.
- **Deliverables:**
  ...
  - [ ] Move GC refinement, result removal, control reconstruction, and remaining older paths through immutable validated plans. *(dispatcher families, optimizing unread/dropped/GC batches, and ordinary core candidates now cross scalar/component/family validation; legacy selected cleanup/application paths remain)*
