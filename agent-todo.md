...
- [FUZ]1010 - GenValid SIMD Exact Opcode And Relaxed-SIMD Widening
  - Status: IN PROGRESS (cron 23, 2026-05-21): representative exact standard-SIMD opcode counters are landed for phase-1/phase-2/phase-3 coverage, and the phase-1 counter set has been widened to all currently emitted splats, lane extract/replace families, and core bitwise ops while preserving coarse SIMD phase facts for exact-counted instructions. Next run should widen phase-2/phase-3 exact counters toward the full currently emitted standard-SIMD matrix, then add relaxed-SIMD emission behind an explicit non-Binaryen-oracle toggle/profile.
