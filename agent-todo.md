...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [PC]001 - Constant Folding Surface - Audit the exact constant, tuple, and local-evaluable fragments Binaryen folds in the top-level `precompute` slots for `-O` / `-Os`.
     ...
     - Current blocker: the compare harness `gen-valid` lane is repaired, and root-level dead-drop rewrite plus root-`nop` cleanup moved the latest smoke rerun to `.tmp/pass-fuzz-pc-genvalid-100-rerun6` (`97` compared, `77` matches, `20` mismatches), but the lane is still red. The remaining dominant family is now mixed root-`nop` count parity: some side-effecting root sequences still need extra preserved `nop`s to match Binaryen, while some all-`nop` void bodies still over-retain `nop`s. The debug artifact replay also remains canonically unequal and much slower (`~119595 ms` vs Binaryen `~1745 ms`).
