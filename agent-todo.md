...
- **Deliverables:**
  ...
  - [ ] Move GC refinement, result removal, control reconstruction, and remaining older paths through immutable validated plans. *(dispatcher families, optimizing unread/dropped/GC batches, ordinary core candidates, reverse/low exact revisits, low exact cleanup, broad immutable-global and adjacent-root revisits, dead-suffix parameter restoration, and boundary cleanup now cross scalar/component/family validation; older producer-specific mutation helpers remain)*
...
- **Status (2026-07-22 partial):** the arbitrary `touched_count > 8` and module `>100` full-roster skips are removed. Ordinary nested replay derives a Binaryen-shaped worth set from changed function signatures, result-change callers in both original and current call graphs, and exact caller local-count growth for transitional scratch cleanup; plain callsite-only edits and touched callers with unchanged preexisting locals are not replayed. Producer-owned body-only worth functions now enter the broad filtered roster regardless of encoded body size; a focused frontier test covers multiple small admissions and exact exclusion. Local `>128` and instruction `>1000` do not suppress semantic nested replay: those checks remain isolated to the earlier broad changed-boundary fast-skip policy, preserving its authoritative 129-local transaction before the unconditional per-function roster. The selected literal/global owner-order blocker was resolved by common epoch-bearing uniform work items. A fresh 101-definition red-first replay is green, and the two formerly blocked literal/global fixtures now match explicit Binaryen v131 output after nested cleanup. The exact-boundary heavy-replay policy is now removed: every validated changed boundary enters the filtered roster. DAEO carries the consumed one-wave single-caller state across post-cleanup reentry and iterates final strictly-smaller validated waves to an unchanged/non-retainable fixed point. The earlier broad-cleanup discriminator and exact invalidation-frontier convergence outside that broad-cleanup trigger remain open.
- **Deliverables:**
  ...
  - [ ] Replace `touched_count > 8`, module `>100`, local `>128`, instruction `>1000`, and material-caller thresholds with source-matched per-function profitability or explicitly non-parity budgets. *(`touched_count`, module size, semantic nested local/instruction skips, material body-only admission, and exact changed-boundary heavy-replay thresholds are removed; only the earlier broad-cleanup local/instruction ownership discriminator remains in this lifecycle slice.)*
  - [ ] Feed nested body changes back into DAE until boundary and nested cleanup both converge. *(broad-cleanup-triggered final waves now iterate under strict encoded-size decrease, validation, and carried one-wave state until unchanged/non-retainable; replace the remaining trigger and whole-pass retry with exact effect invalidations)*
