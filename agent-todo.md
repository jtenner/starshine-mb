...
- [FUZ]1006 - GenValid Call, Reference Call, Indirect Call, Recursion, And Tail-Call Widening
  ...
  - Current evidence: the 2026-05-21 callable-reference source-variant slice makes coverage-forced modules reserve a typed function-reference local for the declared callable and emit validated ordinary `call_ref` forms fed by both `ref.func -> local.set -> local.get` and a typed function-reference `block` result. The follow-up 2026-05-21 direct-call graph slice makes coverage-forced modules emit validated direct `call` traffic to an imported function, a recursive self-call, and a pair of mutually recursive defined functions. Focused TDD: `moon test src/validate` first failed on missing `coverage-forced GenValid routes call_ref through locals and block results`, then passed after implementation; then failed on missing `coverage-forced GenValid emits imported self and mutual direct calls`, then passed after implementation. Direct-call slice validation: `moon info`, `moon fmt`, `moon test`, and `bun scripts/pass-fuzz-compare.ts --pass directize --generator gen-valid --count 1000 --out-dir .tmp/pass-fuzz-directize-fuz1006-direct-calls-1000` all passed, with the directize smoke reporting `1000/1000` compared, `1000` normalized matches, and `0` validation/generator/command failures or mismatches. Remaining work still includes richer `call_indirect` table diversity, `call_ref` from globals/table/control values, and deeper valid tail-call terminal placement.
