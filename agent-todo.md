...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [DCE]002 - Prefix Regression and Artifact Replay - Lock the corrected DCE behavior into focused tests and rerun the MoonBit debug-artifact compare harness.
     ...
     - Status: focused DCE regressions plus follow-up hot-lift/effects fixes landed; replay now gets past the earlier `func[80]` / `func[106]` hot-lift aborts, but canonical parity is still blocked by two downstream validation gaps: `--vacuum` lowers loop parameter seeds inside typed loops and fails at `func[81]`, and `--dead-code-elimination` still lowers invalid structured output at `func[67]` after dead-result cleanup.
     - Doc: [0066#L178](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L178)
3. Do work.
