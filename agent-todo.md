...
- [FUZ]1005 - GenValid Control Flow, Multi-Value, And Branch Payloads
  - Status: IN PROGRESS (cron 23, 2026-05-21). First branch-payload slice landed: `control-heavy` now forces a reusable empty-param multi-result function type, emits a multi-value `TypeIdxBlockType` block, and adds typed i32 value-block payload coverage for `br_if` and `br_table` alongside the existing `br` payload. The latest slice adds an explicit typed-body value-producing `i32` loop and a nested-control `return` in return-typed generated functions, with the body environment now carrying the active function result vector so nested returns can be stack-planned instead of guessed. Focused TDD evidence: `moon test src/validate` first failed because the new value-loop and nested-return assertions were missing the value-loop surface, then passed after implementation; `moon info`, `moon fmt`, and `moon test` pass with pre-existing warnings in DAE/pass-manager tests; `bun fuzz compare-pass --generator gen-valid --count 200 --min-compared 200 --pass code-folding --out-dir .tmp/pass-fuzz-fuz1005-result-loop-return-code-folding-200` and `bun fuzz compare-pass --generator gen-valid --count 200 --min-compared 200 --pass remove-unused-brs --out-dir .tmp/pass-fuzz-fuz1005-result-loop-return-rub-200` both reported `200/200` compared, `200` normalized matches, and `0` validation/generator/command failures or mismatches. Still to resume: deeper randomized/nested label-depth variation, feature-fact distinctions for payload-bearing control, and broader non-i32/ref/v128 payload combinations.
