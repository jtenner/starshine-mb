...
- **Status (2026-07-22 partial):** canonical direct-call operand slices, canonical parameter-slot actions, and ordinary unused-parameter analysis migration landed. The ordinary scalar transaction now carries a preconstructed complete candidate, exact path/kind call identities, an independent type epoch, and caller-owned operand epochs through revalidation and rollback. Ordinary nested-call/structured-operand localization, solved uniform values, exact-literal/immutable-global fallback, and verified immediate-suffix materialization now build typed caller/callee edits inside that same transaction. Legacy direct constant entry points, broader family migration, GC/results/control/type reconstruction, metadata finalization, and complete touched/invalidation ownership remain open.
- **Deliverables:**
  ...
  - [ ] Move GC refinement, result removal, control reconstruction, and remaining older paths through immutable validated plans. *(ordinary scalar localization, solved/fallback/suffix constants, forwarding component signature edits, and component-triggered ref-null materialization are plan-owned; legacy direct constants and broad localization remain)*
