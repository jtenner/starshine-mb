...
- [DAE]006 - Both-Canonical Frontier Advancement
  ...
  - Current classification:
    ...
    - `0580-2026-05-25-dae-func505-empty-count-branch-inversion.md` closed the final empty-input count branch subshape with a selected-Func505-only cleanup for `(x == 0 ? A : B)` to `(x ? B : A)`. The current `.tmp/dae-func505-inverted-count-artifact` replay still first-diffs at `defined=505 abs=522`, both canonical outputs validate, and pass-local timing is still over target (`3068.534ms` Starshine versus `955.834ms` Binaryen). The remaining loop induction, digit/underscore guard, overflow/error-construction, and temp-local reuse drift remains unknown/risky.
  - Next action:
    - Reduce one remaining Func505 live region before changing pass logic or compare tooling: loop induction/exit carrier, lower-bound digit/underscore guard, or overflow/error-return construction and temp-local reuse.
