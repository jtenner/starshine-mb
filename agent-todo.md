...
0. Shared blocker on canonical artifact parity.
  ...
  - [IR2]001 - Hot Lower Payload-Forwarder Repair - Restore the canonical hot-lower behavior for typed payload-forwarder and returned-if shapes before landing more cleanup rewrites on top.
     ...
     - Current blocker: `moon test` is currently red with 18 failures, 12 of them in [`src/ir/hot_lower_test.mbt`](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt). The active broken families are the payload-forwarder and returned-if cases covered by [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L358), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L437), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L461), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L522), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1381), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1427), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1475), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1527), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1586), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1614), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1648), and [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1678). The concrete failure modes are now tight: one fixture mis-retargets an else-arm branch depth in a flattened parent-exit payload wrapper, several returned-if and split-carry fixtures lower to typed/value shapes without the required `unreachable`, three WAT fixtures in the compare-shape family fail to parse at all, and the remaining parent-exit tests still wrap `br $outer` payloads in extra result blocks. Several pass regressions below are the same shape after rewrite rather than independent bugs.
     ...
     - Exit Criteria: the 12 current `src/ir/hot_lower_test.mbt` failures are green, no extra `moon test` regressions are introduced in `src/passes`, and the lowered forms for the listed fixtures match the intended typed-block / `unreachable` contracts.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [DCE]002 - Prefix Regression and Artifact Replay - Lock the corrected DCE behavior into focused tests and rerun the MoonBit debug-artifact compare harness.
     ...
     - Status: focused DCE regressions plus the earlier hot-lift/effects fixes landed, but the suite is not currently green. `moon test` still fails on four DCE fixtures in the same lowered payload-forwarder family: [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L435), [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L582), [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L761), and [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L840). What those reds mean: DCE is still preserving a dead `unreachable` tail and its split payload wrapper in the store/store/return ladder, failing to voidify a nonfinal nonfallthrough value block inside a lifted void region, losing the required explicit `unreachable` when stacked payload blocks retarget to an inner result block, and over-collapsing an outer payload-forwarder wrapper that the oracle keeps live. Those regressions overlap directly with [IR2]001, so the next DCE correctness slice must rebase on repaired lowering before taking new oracle diffs as truth.
     - Doc: [0066#L178](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L178)
