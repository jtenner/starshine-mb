...
- **Deliverables:**
  ...
  - [ ] One returned-value API for fallthrough, explicit returns, block/if/loop/select, typed/typeidx controls, static locals, tuples, flat multivalue lanes, and represented exceptional exits. *(GC result closure derives nested direct result dependencies from stable call identities; single-result analysis now joins multiple nested explicit returns with fallthrough, accepts abstract heap LUBs, and refines self-called functions when recursive results do not flow out, while tuple/control producers still need consolidation)*
