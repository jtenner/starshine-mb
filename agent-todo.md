...
- The first refreshed artifact-backed correctness bug is fixed in-tree: `simplify-locals` no longer sinks pre-loop pending `local.set` initializers into loop headers, which had been turning one-time induction-variable setup into repeated `local.tee` reinitialization inside the loop body.
- The current unreduced artifact frontier is no longer the stale pre-rebase `UInt64.to_byte` family. The refreshed normalized-WAT compare now diverges much earlier, starting in the line-`226` cluster with nested `if (result i32)` / `nop` handling and later larger control-value materialization differences.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [SL]004 - Slot Validation and Artifact Replay - Lock the completed pass into the late slot and prove parity against Binaryen on both focused and large-artifact lanes.
     ...
     - Current blocker: the late-slot regression work is still in-tree and the focused Binaryen lanes remain green, but the refreshed artifact frontier changed again once `tests/node/dist/starshine-debug-wasi.wasm` was rebuilt and the compare lane resumed. `src/passes/registry_test.mbt` still locks `reorder-locals` as a `module_pass`, `src/passes/optimize_test.mbt` still locks the modeled `heap2local -> simplify-locals -> precompute` cleanup slot, and `src/passes/simplify_locals_test.mbt` now also covers the new loop-carried-initializer guard: pre-loop `local.set` values may not sink into loop headers, because the loop body executes repeatedly and cannot inherit the outer pending-sink set safely. That fixes the first refreshed artifact-backed wrong-code bug from `StringView.make_init_no_rc`, where Starshine had been reinitializing the induction local every iteration with a synthesized `local.tee`. The reduced `Func 216` self-opt repro remains canonically equal to Binaryen, but the full artifact compare still diverges earlier in normalized WAT than the stale pre-rebase `UInt64.to_byte` note suggested. The current frontier starts in the early line-`226` cluster with nested `if (result i32)` / `nop` preservation and later larger control-value materialization differences, so the next parity step should reduce that refreshed early control/value family rather than continuing to optimize the old helper cluster. The two newer cleanup follow-ons are also still in-tree: `src/ir/hot_lower_test.mbt` covers dead nested `local.tee` pruning during lower, `src/passes/perf_test.mbt` covers dropping dead raw adjacent `local.set` / `local.get` pairs without hot lift, and `src/passes/optimize_instructions_test.mbt` matches the lowered shape after that dead-tee cleanup. Those follow-ons remain secondary until the refreshed early artifact frontier is reduced.
