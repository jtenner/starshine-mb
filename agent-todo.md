...
- **Exit criteria / suggested tests:** make every named red file green for the intended rewrite, add eligibility and invalid-input guards, run focused direct and active dispatcher tests, then use a verified native Starshine binary for pass-targeted 10,000-case comparisons at the adopted oracle version with classified residuals. The current red tests intentionally fail; they are corpus intake, not implementation signoff.
