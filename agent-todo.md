...
- [FUZ]1020 (IN PROGRESS) - Invalid AST Strategy Expansion Across Validator Families
  - Run 23 continuation update: added function-body AST variants for direct-call payload mismatch and `br_if` / `br_table` payload mismatch, then added exception-payload variants for `throw` operand-stack mismatch and `try_table` catch-target payload mismatch, then added the `invalid-function-body-memory-address-type` stack-typing variant for memory32 load addresses, and now added the `invalid-function-body-memory64-address-type` variant for memory64 load addresses, plus focused strategy and repair checks proving matching valid call/branch/exception/memory-address payload forms validate. Earlier Run 23 work added body-stack variants for invalid local/global/table/memory indexes. Continue with another focused AST-invalid family slice rather than reopening these call/branch/exception/memory-address variants.
