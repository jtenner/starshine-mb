...
- [FUZ]1020 (IN PROGRESS) - Invalid AST Strategy Expansion Across Validator Families
  ...
  - Current run evidence: the return-value slice first failed on missing `InvalidFunctionBodyReturnValueType` / missing mutator, then passed after implementation with `moon test src/validate`, `moon info`, `moon fmt`, full `moon test`, and `moon run src/fuzz -- validate-invalid-ast smoke --seed 0x10ce3`.
