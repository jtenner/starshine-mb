...
- **Deliverables:**
  ...
  - [ ] Move GC refinement, result removal, control reconstruction, and remaining older paths through immutable validated plans. *(dispatcher families, optimizing unread/dropped/GC batches, ordinary-core GC parameter refinement and dropped-result removal, reverse/low exact revisits, low exact cleanup, broad immutable-global and adjacent-root revisits, dead-suffix parameter restoration, boundary cleanup, verified immediate suffixes, and selected ref-null compatibility producers now cross scalar/component/family validation; GC/result and control/type mutation helpers remain)*
