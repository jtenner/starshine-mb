...
- [O4Z-AUDIT-SSA] - Deep audit `ssa-nomerge`
  ...
  - Finding: `docs/wiki/raw/research/0722-2026-06-09-ssa-nomerge-exceptional-edge-audit.md` fixed a true try_table/throw corruption caused by applying normal-flow-only local SSA across exceptional edges. `ssa-nomerge` now skips exceptional-flow HOT mutation, and the raw no-write path now recursively materializes default body-local reads across branchy control and cleans dropped-unreachable debris before hard `unreachable` tails. Follow-ups keep nested structured param/body-local writes on canonical slots when the next observable use reads before overwrite, keep body-local `local.tee` canonical for branch continuations/result branches that read before overwrite, avoid dead/unneeded branch and label merge copies, preserve no-copy `br_if` and no-copy `br_table` terminators, insert needed enclosing-label copies after divergent aliases before later branch exits, preserve void and result-typed non-loop `br_table` exits with canonical merge copies, narrow the `Func 4302` suspicious-carrier false positive, and raise the structured raw budget enough to reduce direct artifact `large-structured-local-writes` skips from `2117` to `9`. Latest validation for the result-typed `br_table` slice: focused `ssa_nomerge_test` `123/123`, `moon test src/passes` `2142/2142`, full `moon test` `5402/5402`, direct compare `.tmp/pass-fuzz-ssa-nomerge-result-brtable-10000` compared `9977/10000` with `9977` normalized matches and `0` mismatches, and GenValid parity `.tmp/pass-fuzz-ssa-nomerge-parity-result-brtable-10000` compared `10000/10000` with `10000` compare-normalized matches and `0` mismatches. Latest direct artifact replay `.tmp/self-ssa-nomerge-debug-wasi-result-brtable` is still not exact-normalized/canonical-function equal (`defined=0 abs=27`) with pass-local time `16.233 ms` vs Binaryen `349.014 ms`; earlier post-cleanup final `.tmp/pass-fuzz-ssa-nomerge-final-100000-after-debris` remains the current 100000-case final baseline (`99751/100000`, `0` mismatches, `249` Binaryen/tool command failures).
