...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [TO]005 - Exact Slot Gate And Oracle Proof - Keep `tuple-optimization` active on the explicit hot-pass surface, but finish the Binaryen-matching preset slot, feature gate, and parity proof in a follow-up slice.
     ...
     - Current blocker: the explicit pass surface is ready, but the surrounding Binaryen neighbors are still not representable in the preserved public preset order, so the next keepable work is proof and exact-slot preparation rather than approximate scheduling. The tuple pass itself now recognizes one-hop scalar-forward copy bridges from direct non-`Block` multivalue spill producers, the compare-shaped host-`local.tee` seed bridge, the raw decoded one-hop carrier shape, the narrowed no-host root-carrier family, the overlap-aware exact-copy family where a copy group writes back into one of its source lane locals, and the newer non-canonical synthetic root-carrier copy groups whose destination lane locals are not strictly increasing. New white-box regressions in `src/passes/tuple_optimization_wbtest.mbt` now lock the extra synthetic copy-group analysis, the no-host source/copy root-carrier path, the single-use / overlap-aware exact-copy rewrites, and the new synthetic `tuple.make` carrier shape for scrambled and overlapping exact-copy groups. Hot lowering also now keeps typed terminal `if` carriers that feed `return`. The latest kept-tree debug-artifact compare is `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-161962`, and it is still red: canonical wasm and normalized WAT differ, Starshine is still over budget (`5334.668 ms` wall, `895.764 ms` pass) versus Binaryen (`339.989 ms` wall, `2.997 ms` pass). The earlier tuple-pass artifact hunk around the source/copy rematerialization family is gone; the first visible normalized-WAT drift is now mostly Binaryen-only `nop` residue, and the first later non-`nop` drift is a hot-lower policy gap where Binaryen keeps tuple scratch locals plus `tuple.extract` around the synthetic copy carrier while Starshine lowers the same kept HOT shape through direct scalar scratch locals and a tighter local layout. The next correctness step is therefore in `src/ir/hot_lower.mbt`, not more tuple analysis/rewrite broadening.
