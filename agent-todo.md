...
- **Why:** on 2026-07-24, Starshine plain DAE exceeded the `1500`-second command bound on `.tmp/starshine-predecessor-current.wasm` (`3,597,499` bytes, `5,374` definitions, roughly `251,000` rendered direct calls); Binaryen v131 `--dae` completed in `0.356s`. Focused case-515 medians remain `20.351ms` plain and `31.380ms` DAEO, so the gap is artifact-scale setup/worklist complexity.
- **Deliverables:**
  - [ ] Add bounded phase timing around original-boundary snapshot subcollectors and current dependency construction without committing debug output.
  - [ ] Identify repeated full-module/call-graph scans whose complexity dominates dense-call inputs.
  - [ ] Consolidate or cache those facts without changing producer ownership or invalidation semantics.
  - [ ] Rebuild the native binary and rerun the dense-call Starshine/Binaryen comparison, focused lifecycle trace, generated four-lane matrix, and repository gates if output-affecting code changes.
