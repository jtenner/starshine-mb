...
- Census note: `[FUZ]1020D1` audited the checked-in invalid-AST registry and wiki history. The registry currently has 261 deterministic AST-invalid strategy specs across all 15 `ValidationIssueFamily` buckets after `[FUZ]1020E4` added `try_table` `catch_ref` payload-plus-exnref compatibility coverage. Already-covered breadth includes the closed `[FUZ]1020A`/`B`/`C`/`D3`/`D4`/`D5`/`E1`/`E2`/`E3`/`E4` families: start out-of-range/wrong-kind plus imported-start signature variants, cross-kind duplicate export names, datacount absent/too-small/too-large plus mixed active/passive cardinality mismatch, element function-index/typed-expression/table-index basics plus active element/table ref-type mismatch, table/data/memory bulk operand/index families, atomics non-shared and stack/index families, SIMD lane/operand/index families, GC aggregate/input/variance/descriptor basics, ref/branch payloads including `br_on_null`, `br_on_non_null`, `br_on_cast`, multi-value `br_table` arity, `call_indirect` table element compatibility, `return_call_indirect` table element compatibility, ordinary `try_table` catch payload mismatch, and `try_table` `catch_ref` payload-plus-exnref mismatch, name-map out-of-range indices, and code-section cardinality/imported-body families. The remaining open `[FUZ]1020F*`/`G*` slices below are the de-duplicated next concrete gaps; do not reopen older A/B/C/D/E1/E2/E3/E4 variants unless new evidence shows their tests no longer cover the stated rule.
...
- [FUZ]1021C1 (p2) - Relaxed-SIMD laneselect mask local-index malformed-byte validation closeout
  - Unit: rerun validation for the already-implemented decode-rejected `malformed-relaxed-simd-laneselect-mask-local-index-uleb` and `overwide-relaxed-simd-laneselect-mask-local-index-uleb` strategies now that the local Moon core install is repaired.
  - Acceptance: `moon test src/fuzz` reaches and passes the focused invalid-binary tests; if green, remove this closeout slice from the active backlog and commit the validation evidence. If it fails, update this slice with the precise compile/test blocker.
  - Suggested tests: `moon test src/fuzz`, then `moon info`, `moon fmt`, and full `moon test` if source/docs are changed.
