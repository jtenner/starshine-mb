...
- Future SGO work should require new evidence: a semantic mismatch, wasm validation failure, targeted artifact/code-size need, string/GC/refinalization breadth requirement, or measured SGO-specific wall-time owner. The 2026-05-21 `[SGO]003` breadth continuation has now covered fact-sensitive i31, independent `call_indirect`, clean scalar-load / `table.get`, independent ordinary `call_ref`, clean `local.set`, clean exported `global.set`, clean `table.set`, and clean `i32.store` FlowScanner slices with focused tests and direct 10k pass fuzz. Broader Binaryen `SimplifyGlobals.cpp` families, exact default-function scheduler parity, nested `vacuum` wrapper overhead, and compare-harness normalization for explicit default local initialization are non-blocking follow-ups, not active v0.1.0 release blockers.
