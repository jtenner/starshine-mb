...
- [PC]001 - Runtime And Representation Drift
  - Deliverables: keep direct debug-artifact semantic parity green after the dead-root `nop` normalization fix; classify and reduce canonical wasm/text-form drift where a small safe change is justified. Whole-command runtime gap diagnosis is explicitly deferred outside this slice.
  - Current status: saved `.tmp/recheck-precompute/` repros replay green (`100 / 100`, seed `0xa11d`, out dir `.tmp/recheck-precompute-after`); the raw stack-level shortcut covers no-candidate functions, nested nop-only control, adjacent scalar folds, branch-free constant-`if` arm picks, immutable module-constant `global.get` folds, mutable/global no-candidate reads, dropped flat nontrapping scalar/global/select expressions, safely voidable dropped single-result blocks, and preserved effectful/trapping dropped tails after safe raw folds are exhausted while skipping HOT lift/pass timers. The standard mixed 10k lane stays at `0` semantic mismatches before known Binaryen/tool command failures stop the run (`6759` compared, `.tmp/pass-fuzz-precompute-dropped-block-raw`). A 2026-05-08 compare-tooling fix showed the earlier `.tmp/pc-artifact-branch-blockdrop-raw` `canonical function compare equal` result was masked by WAT data-string parentheses; the current direct debug-artifact classification at `.tmp/pc-artifact-drift-classified` reports canonical wasm/text unequal, normalized WAT unequal, first differing function `defined=4 abs=21`, Starshine pass time about `123ms` versus Binaryen about `155ms`, `1913 / 4671` function type-index differences from Binaryen type reordering, and `757 / 4671` code-body byte differences dominated by Binaryen temporary-local/block scaffolding plus dropped intermediate constants around `memory.size` / `local.tee` scalar folds. This remains open as representation drift, but not as a small raw shortcut candidate.
