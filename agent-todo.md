...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [TO]004 - Scheduler, Registry, And Oracle Proof - Promote `tuple-optimization` from removed placeholder to a real hot pass at Binaryen's multivalue-gated slot and prove parity on fuzz plus the MoonBit artifact.
     ...
     - Why: the HOT-native rewrite is now landed, but the implementation is still incomplete until the registry, preset order, feature gate, and artifact compare path all match the documented Binaryen slot.
     - Deliverables: register the pass as active; gate it on multivalue; place it after `code-pushing` and before `simplify-locals-nostructure`; add feature-off and dispatcher coverage; prove the landed HOT-native rewrite against Binaryen with pass-fuzz compare and debug-artifact compare.
...
3. Do work.
   - Continue in dependency order from the landed local-growth helper, candidate analysis, and good-component rewrite: scheduler/parity proof is next.
