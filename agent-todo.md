...
- Both-canonical diagnostic helper: `scripts/self-optimize-compare.ts --canonicalize-binaryen-output --dae-optimizing` last completed at `defined=505 abs=522` in `.tmp/dae-func505-bool-carrier-artifact`. Research notes `0576` through `0584` reduce the saved Func505 body from unknown/risky output-shape drift to representation-only control/local-carrier, branch-polarity, temp-local, and dropped-debris drift. Note `0585` adds a narrow diagnostic-only canonical-function normalizer for that inspected Func505 parser-loop family, but the recovery-run full artifact replay timed out after 300s before discovering the next frontier.
- Latest diagnostic timing is over the 2x target for the current Func505 artifact: `2721.553ms` Starshine pass versus `873.679ms` Binaryen pass for `.tmp/dae-func505-bool-carrier-artifact`. Per user direction, keep prioritizing correctness/frontier classification over pass time for now.
...
- `[DAE]014` latest source/index sync is closed as of 2026-05-26: `docs/wiki/binaryen/passes/dae-optimizing/index.md` and `starshine-strategy.md` now link research notes `0576` through `0584` and record the current Func505 diagnostic frontier as a representation-only saved body that still needs a normalizer or pass-shape cleanup to advance beyond `defined=505 abs=522`.
...
- [DAE]006 - Both-Canonical Frontier Advancement
  ...
  - Current classification:
    ...
    - `0585-2026-05-26-dae-func505-diagnostic-normalizer.md` adds a marker-rich diagnostic-only canonical-function normalizer for the inspected Func505 parser-loop family plus a focused fake-tool script fixture. This changes only compare fallback text; it does not affect raw WAT/wasm equality or pass behavior. A full artifact replay in this recovery run timed out after 300s before writing `result.json`, so the next frontier is not yet known.
  - Next action:
    - Re-run the both-canonical artifact compare with the Func505 diagnostic normalizer on a fast enough local binary/environment to discover and classify the next frontier; record the artifact dir, validation command, pass-local timing, and first differing function.
    - If a future focused fixture proves a remaining subshape is more than representation drift, add the smallest TDD pass cleanup; otherwise avoid raw wasm comparison changes.
  - Deliverables:
