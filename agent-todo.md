...
| `memory-packing` | Closed on 2026-07-26 after post-repair v131 renewal, Binaryen-accurate passive profitability thresholds, decoded legacy-`try` user traversal/rewrite support, five-family operand-extraction regressions, and a dedicated three-family GenValid aggregate. Regular `100000`, dedicated `10000`, and random-all `10000` are exact; wasm-smith retains only the known pass-independent case-9332 unreachable-drop shell across `9956` comparable cases plus 44 Binaryen-only failures. | None. |
...
| `remove-unused-brs` | Post-repair v131 matrix completed on 2026-07-26. The first repair retired all `181` open random-all cases. The next red-first slice now admits multivalue-return functions that were incorrectly rejected by the root stack-hazard guard, while preserving the nested multivalue-carrier and call/memory safety boundaries. Eight focused GenValid leaves plus an expanded aggregate now isolate constant carried `br_if`, same-target `br_table`, multi-function/multivalue-return, result refinalization, dropped multivalue values, wrappers, selectification, and measured-win boundaries. | **Open fresh four-lane renewal and scheduler reconciliation:** `[AUDIT-CORRECTNESS]001`; fuzz/compare lanes are intentionally deferred until implementation and deterministic coverage are complete. |
...
- **Status:** **Active; `merge-blocks`, `once-reduction`, `memory-packing`, and `local-subtyping` renewed; `remove-unused-brs` repair is progressing but parity remains blocked.** Implementation and red-first deterministic regression work are complete for `merge-blocks`, `once-reduction`, `memory-packing`, `local-subtyping`, `remove-unused-brs`, `precompute` / `precompute-propagate`, full `ssa` plan application, and the raw merge-locals cleanup. The 2026-07-26 RUB renewal confirms regular and wasm-smith safety but reopens direct parity. The first repair slice now makes all `181` previously open random-all cases match in a replay of the original `870` residuals while preserving the `689` measured smaller Starshine cleanups. Dedicated replay remained `10000/10000` mismatches before the latest root-hazard repair. That repair now allows the previously skipped multivalue-return functions to run the existing typed carried-branch, switch, selectification, and cleanup machinery. Eight focused GenValid leaves and random-all aggregate membership are in place. Per user request, no fuzz or compare lane has been run after this implementation; fresh dedicated and four-lane evidence plus the precompute matrices stay open below.
...
- **Deliverables:**
  ...
  - [x] Run the required four-lane v131 matrix for `memory-packing` (2026-07-26: regular `100000/100000`, dedicated `memory-packing-all` `10000/10000`, and random-all `10000/10000` exact; wasm-smith `9955/9956` exact plus the known pass-independent case-9332 unreachable-drop shell, with 44 Binaryen-only failures and zero Starshine validation/generator/property/semantic failures).
  ...
  - [x] Run the required four-lane v131 matrix for `remove-unused-brs` (2026-07-26 baseline: regular `100000/100000` cleanup-normalized green; wasm-smith `9956` comparable with `9954` direct plus `2` cleanup-normalized and `44` Binaryen-only failures; dedicated `10000/10000` mismatches; random-all `870` residuals). Follow-up repair matches the `181` prior random parity gaps, preserves the `689` measured wins, admits multivalue-return functions previously skipped by the root hazard guard, and adds focused dedicated leaves. Keep this item open until the intentionally deferred fresh four-lane renewal is run.
