...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [TO]005 - Exact Slot Gate And Oracle Proof - Keep `tuple-optimization` active on the explicit hot-pass surface, but finish the Binaryen-matching preset slot, feature gate, and parity proof in a follow-up slice.
    ...
    - Current blocker: the explicit pass surface is ready, but the surrounding Binaryen neighbors are still not representable in the preserved public preset order, so the next keepable work is proof and exact-slot preparation rather than approximate scheduling. The tuple pass itself now recognizes one-hop scalar-forward copy bridges from direct non-`Block` multivalue spill producers, reduced chained mixed scalar-forward bridges from synthetic groups that still retain a source lane, the compare-shaped host-`local.tee` seed bridge, the raw decoded one-hop carrier shape, the narrowed no-host root-carrier family, the overlap-aware exact-copy family where a copy group writes back into one of its source lane locals, the newer non-canonical synthetic root-carrier copy groups whose destination lane locals are not strictly increasing, the reduced nested branch-exit source-root family that now stages through dedicated split locals plus a typed multivalue carrier instead of the older direct root block shortcut, the reduced root-local.set exact-copy family where canonical destination lanes now still route through a typed carrier when the payload source lanes are scrambled, the reduced source-host-copy passthrough family where staged init roots now rematerialize the old non-host lanes and host-only tail-live0 exact-copy groups no longer add a redundant second carrier, the newer terminal host-lane exact-copy family where drop-only tail lanes no longer keep the copy group rewrite alive once only one forwarded non-drop lane survives, and the newer upstream no-host exact-copy family where a copy group now stays scalarized when its only downstream consumer is a terminal drop-only child copy. A newer function-root anchor-host exact-copy rewrite slice now also stages terminal non-host drop tails through an earlier host-result spill instead of the older direct final `local.tee` path, so the reduced terminal drop-only host fixture now matches Binaryen and the checked-in debug-artifact parity regression for `func $3598` is retired. The newer native parity-helper slice now decodes normalized `wasm-opt --strip-debug` `.wasm` output by function index instead of extracting WAT with `awk`, which exposed imported-module false positives in the old native tuple checks. The pass also now forwards `copy_payload_locals` through rewritten scalar-forward copy groups, keeps the anchor-host fast path off when a rewritten copy group feeds another rewritten copy group, and preserves original body locals while pruning only appended dead locals after rewrite, so direct native Binaryen parity is green again for the mixed direct-producer scalar-forward bridge, nested no-host source-copy chain, and nested exact-copy scalar-result families. The latest kept-tree debug-artifact compare is `/tmp/self-opt-tuple-current`, and it is still red: canonical wasm and normalized WAT differ, Starshine is still over budget (`7313.353 ms` wall, `1283.629 ms` pass) versus Binaryen (`325.650 ms` wall, `3.012 ms` pass). The native debug-artifact regressions still retire the old missing-lane `func $1930` bug and the later `func $3598` staged-copy bug, but full Binaryen parity is still not proven; the current head hunk now starts in `func $3639`, and the remaining reduced host-drop family is now clearly later than the tuple rewrite root itself.
     - Current proof status: the latest current-head reruns are clean on the targeted pass harnesses: `bun scripts/pass-fuzz-compare.ts --pass tuple-optimization --generator gen-valid --count 1000 --max-failures 20 --starshine-bin /tmp/moon-build-cmd-current/native/debug/build/cmd/cmd.exe --out-dir .tmp/pass-fuzz-tuple-optimization-genvalid-1000-current` is `1000/1000` normalized matches with `0` mismatches, `0` validation failures, `0` generator failures, and `0` command failures, and `bun scripts/pass-fuzz-compare.ts --pass tuple-optimization --generator wasm-smith --count 200 --max-failures 20 --starshine-bin /tmp/moon-build-cmd-current/native/debug/build/cmd/cmd.exe --out-dir .tmp/pass-fuzz-tuple-optimization-smith-200-current` reached `165/165` normalized matches with `0` mismatches before stopping on `20` Binaryen-side parser failures. Earlier larger clean runs remain as historical evidence: `.tmp/pass-fuzz-tuple-optimization-genvalid-10000` was `10000/10000`, `.tmp/pass-fuzz-tuple-optimization-genvalid-2000` was `2000/2000`, `.tmp/pass-fuzz-tuple-optimization-genvalid-500` was `500/500`, and `.tmp/pass-fuzz-tuple-optimization-smith-200` previously saved the same `20` Binaryen parser failures (`17` `invalid type index`, `1` `Recursion groups of size zero not supported`, and `2` `invalid wasm type: -64`). Focused reduced pipeline regressions are now also in-tree for direct scalar-forward seed bridges, nested scalar-result carriers, the imported-call host-`local.tee` bridge, the reduced no-host direct-spill / exact-copy root-carrier repros, the new overlap-aware exact-copy copyback repro, the reduced function-root drop-tail host carrier, and the debug-artifact `func $3598` replay. No new pass-fuzz rerun has been taken on this exact checkpoint yet because the corrected native parity helper immediately exposes the still-red deterministic `terminal drop-only host copy groups` and `chained host-copy tail-live0` cases; the remaining proof gap is still the red debug-artifact compare above plus those two reduced host-drop families, not missing coverage on the earlier reduced lanes.
     - Remaining tuple TODOs:
       - Fix direct Binaryen parity for `terminal drop-only host copy groups`; the reduced HOT rewrite roots are already correct before lowering, but the final emitted/local-order shape still differs from Binaryen.
       - Fix direct Binaryen parity for `chained host-copy tail-live0`; Binaryen still keeps a tighter nested drop/result-block shape while Starshine leaves separate root carrier locals and drops.
       - Trace the exact post-rewrite stage that changes the already-correct terminal host-drop HOT shape; current evidence points at later lowering or local-order emission rather than the tuple rewrite itself.
       - Close the full debug-artifact mismatch that now starts in `func $3639` in `/tmp/self-opt-tuple-current`.
       - After the deterministic host-drop cases are green again, rerun the direct native tuple regressions, `bun scripts/pass-fuzz-compare.ts --pass tuple-optimization ...`, and then the full `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --tuple-optimization` replay.
       - Once correctness is signed off, reduce tuple-only replay cost; Starshine is still far slower than Binaryen on the artifact compare.
