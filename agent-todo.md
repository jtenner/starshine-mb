...
- The definitive fresh original-input WAGO checkpoint is now `.tmp/wago-o4z-pass8-final16-dfe-transitive-type-remap-fresh-v2-20260821/`, covering all 1,330 tracked fixtures at or below 2 MiB and the unchanged 766-file stable cohort with native SHA-256 `7fb646d4cd84c7e9eeddef364498f9ed3ad3fe17b5f14700c18a328aff098823`. Starshine emits 413,300 bytes versus pinned Binaryen v131 at 415,485, a fresh 2,185-byte / 0.526% win with 110/602/54 smaller/equal/larger, 3,025 positive residual bytes, 46,830 bytes removed from the 460,130-byte baseline, and 104.89% original-gap closure. Final16 repairs Starshine-local DuplicateFunctionElimination type compaction: retained simple function type definitions are now recursively rewritten through the old-to-new type map when they mention a remapped index. Previously duplicate leaf signatures could merge while transitive function-reference signatures retained stale indices; direct DFE then failed validation on `type-equivalence.5`, and the later O4z pipeline converted its valid returning `run` body into `unreachable`. The exact direct pass now validates, 100 fresh source/Binaryen/Starshine instances all return, and final O4z output is byte-identical to Binaryen at 33 bytes. `type-equivalence.8` also closes 66 → 59 bytes and becomes byte-identical to Binaryen. These are the only fresh changes, net -8 bytes with zero regressions. Red-first DFE coverage asserts the nested kept-type remap; command coverage rejects the prior O4z `unreachable` result. The explicit pinned-v131 DFE development lane compares 10,000/10,000 with 9,942 normalized, 58 current raw output residuals, and zero command/validation/property/generator failures; all 58 residuals reproduce with the exact final14-era baseline binary and are not introduced by this repair. DFE pass-local 500-pair median improves 134.5 → 133.0 µs (-1.12%). Over 1,000 alternating whole-command pairs, `type-equivalence.5` is +0.43% paired-median noise while `type-equivalence.8` is 1.11% faster. Final15’s abstract-bottom `ref.test` repair and final14’s candidate-local global pruning remain unchanged. The runtime-safe retained projection remains distinct and now projects 412,164 versus 415,485, a 3,321-byte / 0.799% win with 112/601/53 smaller/equal/larger, 1,986 positive residual bytes, 47,966 bytes removed, and 107.44% gap closure. Full validation is 10,604/10,604; all passes 6,953/6,953; command 269/269. Remaining fresh leaders are ifs +1,434, JSON-SIMD +397, primes +378, fannkuch +170, fasta +129, scalar JSON +86, startup JSON +60, urem +38, issue1809 +35, deeply nested struct +25, bump-pointer GC +24, return-call-indirect +21 each, array-copy-inline +19, and fuzzcase 1820/1825 +8/+7. Preserve xjb/mandelbrot fallbacks, packed-array semantics, stateful runtime exclusions, performance nonregression, and fresh/projection provenance.
