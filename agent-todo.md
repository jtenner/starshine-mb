...
- **Status (2026-07-22 partial):** canonical direct-call operand slices, canonical parameter-slot actions, and ordinary unused-parameter analysis migration landed. The scalar, forwarding-component, and family plans now carry validated `DaeCommitEffects` alongside the candidate: structural touched ownership stays separate from semantic worth, per-definition `worth_decided` records whether a producer owns the worth decision, and effects record parameter/result/body/local/control/type/metadata consequences, invalidation domains, and decision reasons. Successful scalar/component/family applications, `DaeWorkCommitted`, exact-literal worklist batches, core aggregation, and the DAEO pass-manager adapter preserve those effects. Scalar removal marks the callee worth without marking callsite-only callers; localization marks exact scratch-owning callers; materialization marks the callee; forwarding marks component members and any triggered ref-null follow-up. The pass manager trusts producer-owned worth for covered definitions and retains structural inference only for legacy producers; exact caller local-count growth is an explicitly temporary compatibility bridge until those scalar producers return their plans directly. The selected exact-literal scan/mutate helper is removed: ordinary literals, immediate suffixes, immutable globals, dynamically exposed global callees, and broad-cleanup folded callees now enter epoch-bearing `DaeUniformActualWork` with original ownership and producer effects. `DaeFamilyRewritePlan` still wraps every other dispatcher result and older producer family before commit. Older producer-specific mutation helpers, metadata finalization, and complete invalidation ownership remain open.
- **Deliverables:**
  ...
  - [ ] Move GC refinement, result removal, control reconstruction, and remaining older paths through immutable validated plans. *(dispatcher families, optimizing unread/dropped/GC batches, ordinary-core GC parameter refinement and dropped-result removal, reverse/low exact revisits, low exact cleanup, broad immutable-global and adjacent-root revisits, dead-suffix parameter restoration, and boundary cleanup now cross scalar/component/family validation; older producer-specific mutation helpers remain)*
...
- **Deliverables:**
  ...
  - [x] Preserve Binaryen's one-wave single-caller profitability throttle. *(the core reports consumed state; DAEO carries it across post-cleanup work and every final convergence retry, and both written-forwarding and changed-terminal/default-argument fixtures prevent a second same-pass pruning wave)*
...
- **Status (2026-07-22 partial):** all ordinary-core dropped-result admissions—including the three zero-parameter predicates and argument-side repair—preserve their source order but now dispatch `DaeDroppedResultWork`, commit through the family transaction, and report producer-owned callee/result plus exact caller/drop effects. Per-definition worth ownership prevents those effects from suppressing legacy changed boundaries merged into the same core wave. The obsolete selected-definition wrapper, descending attempt/cap collectors, empty fallback-neighborhood carrier, and rejected bucketed-attempt scaffold are deleted; their useful fact, timing, and exact-touched tests now target the live definition helper. The current dependency graph derives active non-tail undropped result observers, active direct-tail dependencies, their combined result-forwarding frontier, and the inverse per-caller result dependency rows from stable call identities while excluding dropped calls from observer evidence. Result-domain requeue consumes those exact rows, and GC result closure now performs dependency postorder plus forwarder requeue through the same graph without a productive cap or terminal-body shape scan. Ordinary dropped-result admission now consumes the authoritative current boundary guard: original/current ownership, every direct observer dropped, no direct-tail callee relation, and no outgoing direct/indirect/reference tail from the target. Unrelated tail calls no longer block removal; callers without an exact target definition retain the historical fail-closed aggregate tail barrier. Specialized lane breadth and unified return/control repair remain open.
- **Deliverables:**
  - [x] Remove a result only when every owned direct call drops it and unseen/tail dependencies permit it. *(ordinary `DaeDroppedResultWork` and the live definition helper require the current authoritative boundary guard; specialized result-removal helpers without an exact target retain a conservative aggregate fallback)*
...
- **Status (2026-07-22 partial):** the arbitrary `touched_count > 8`, module `>100`, local `>128`, and instruction `>1000` lifecycle selectors are removed. Ordinary nested replay derives a Binaryen-shaped worth set from changed function signatures, result-change callers in both original and current call graphs, and exact caller local-count growth for transitional scratch cleanup; plain callsite-only edits and touched callers with unchanged preexisting locals are not replayed. Producer-owned body-only worth functions enter the same frontier regardless of encoded body size. Worth ownership is explicit per definition, so one migrated producer can authoritatively exclude a callsite-only caller without suppressing compatibility inference for unrelated legacy changes merged into the same wave. Changed-boundary local compaction and immutable-field folding now run transactionally at every function/module cardinality, then the shared `precompute-propagate`-first roster executes exactly once; the former private broad-cleanup roster and folded-callee recursion are deleted. The selected literal/global owner-order blocker was resolved by common epoch-bearing uniform work items. A 101-definition replay and the formerly blocked literal/global fixtures match explicit Binaryen v131 output. Every validated changed boundary enters the filtered roster. DAEO carries the consumed one-wave single-caller state across post-cleanup reentry and iterates final strictly-smaller validated waves to an unchanged/non-retainable fixed point. Exact effect-frontier convergence and retirement of remaining legacy worth inference remain open.
- **Deliverables:**
  ...
  - [ ] Feed nested body changes back into DAE until boundary and nested cleanup both converge. *(the shared roster now executes once and cleanup-exposed boundary fixtures are idempotent in one invocation; strict-size final waves still use whole-pass retry rather than exact effect invalidations)*
