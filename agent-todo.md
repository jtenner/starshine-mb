...
- [FUZ]1004 - GenValid Type, Subtyping, And GC Topology Widening
  - Status: IN PROGRESS (cron 23, 2026-05-21); active; exact-reference local/block flow, mutually recursive struct/array rec-group, and descriptor final/open subtype-matrix slices landed on 2026-05-21, but broader recursive topology diversity and organic non-forced coverage remain.
  ...
  - Current evidence: `coverage-forced GenValid routes exact refs through locals and block results` now proves coverage-forced modules declare exact-reference locals and emit an exact-result block routing an exact reference through `local.set` / `local.get`; `coverage-forced GenValid emits mutually recursive struct and array topology` now proves GC-heavy modules validate with a rec group where a struct field references an array sibling and that array's element type references the struct sibling; `GC-heavy GenValid emits descriptor final-open subtype matrix` now proves GC-heavy modules validate with an open descriptor/describes super-pair plus final descriptor/describes subtype-pair; focused `moon test --package jtenner/starshine/validate --file gen_valid_tests.mbt`, `moon test src/validate`, full `moon test`, and a 100-case `remove-unused-module-elements` gen-valid compare smoke at `.tmp/pass-fuzz-fuz1004-descriptor-rume` pass. Remaining work should target organic non-forced topology diversity and broader recursive shape variation rather than redoing these exact-ref, mutual struct/array, or descriptor final/open slices.
