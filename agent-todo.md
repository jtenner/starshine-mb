...
- [FUZ]1012 - Randomized Coverage-Forced Templates
  - Status: IN PROGRESS (cron 23, 2026-05-21). First slice landed deterministic branch/loop template variation for coverage-forced preludes plus `gen_valid_coverage_template_signature(...)` so tests can prove same-seed stability and cross-seed template drift without consuming extra PRNG before existing forced-family helpers. The next slice extracted memory/table/global prelude helpers and varies memory-vs-table coverage order deterministically from the current function shape, so it widens forced template layouts without perturbing the PRNG stream used by later forced-family value helpers. The SIMD slice adds deterministic SIMD phase-order variation from the current function shape and splits the template signature's coarse `simd` label into `simd1` / `simd2` / `simd3` so tests can observe both old and inverted SIMD prelude orders without consuming additional generator PRNG. The latest slice groups direct-call, `call_indirect`, and `call_ref` forced templates behind a function-shape order key and splits the signature's coarse `call` label into `direct_call`, `call_indirect`, `call_ref`, and `tail_call`, so tests can observe direct-before-indirect, indirect-before-direct, and ref-before-direct call prelude layouts.
  ...
  - Current evidence: `moon test src/validate` passes with regressions proving coverage-forced template signatures are deterministic for the same seed, differ across selected seeds, preserve numeric/control floors, include both memory-before-table and table-before-memory forced prelude layouts across a bounded seed window, include both `simd1|simd2` and `simd2|simd1` SIMD phase orders while preserving `uses_v128`, and include direct-before-indirect, indirect-before-direct, and `call_ref`-before-direct call-template orders while preserving direct/indirect call facts. Docs note the seed-deterministic branch/loop, memory/table, SIMD phase-order, and call-template variation.
  - Remaining: randomize additional forced families beyond the branch/loop, memory/table, SIMD phase, and call templates (for example more granular ref/GC exception template variants), add explicit template-family diagnostics or manifest-visible facts if the public ledger needs them, then rerun `moon test src/fuzz` / relevant fuzz smoke before closing.
