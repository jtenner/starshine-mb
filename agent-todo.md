...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [DCE]003 - Runtime Budget and Oracle Refresh - Close the remaining DCE speed gap and refresh parity evidence on a valid baseline/oracle path.
     ...
     - Reproduce the native-default-I/O blocker directly in-test: `moon test --target native --package jtenner/starshine/cmd --file cmd_wbtest.mbt --filter 'run_cmd reports dead-code-elimination native default-io blocker on debug artifact'`
     - Reproduce broader native-release instability: `moon test --target native --release --package jtenner/starshine/cmd --file cmd_wbtest.mbt`
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [RUB]002 - Multi-Slot Cleanup and Artifact Compare - Replay the pass in each Binaryen slot and confirm later `merge-blocks` opportunities match the reference output.
     ...
     - Current blocker: the scheduler gap, old serial verifier failures, and the earlier artifact-backed correctness blockers are fixed; the remaining work is now concentrated in explicit-pass type-order noise plus later unchanged runtime families whose traces still need pass-vs-lift separation.
      ...
      - Fresh interleaved self-opt replay on the same lane now averages `402.0653 ms` for the current tree versus `431.7387 ms` for the immediate pre-slice baseline (~`6.87%` faster on average and ~`5.81%` faster by median for the latest lifted skip slice alone, after the earlier audit and raw filters already cut the lane further), but canonical and normalized parity are still red, the first checked remaining hunk `func $384` still traces as `changed=false`, and Binaryen remains far faster on the same lane.
     ...
     - The larger `gen-valid` reruns remain green too: `bun scripts/pass-fuzz-compare.ts --pass remove-unused-brs --generator gen-valid --count 1000 --max-failures 20 --out-dir .tmp/pass-fuzz-rub-genvalid-1000-rerun` => `1000/1000` normalized matches, `0` mismatches, `0` command failures, and `bun scripts/pass-fuzz-compare.ts --pass remove-unused-brs --generator gen-valid --count 10000 --min-compared 10000 --max-failures 20 --out-dir .tmp/pass-fuzz-rub-genvalid-10000` => `10000/10000` compared cases, `10000/10000` normalized matches, `0` mismatches, `0` validation failures, `0` generator failures, `0` command failures.
      - Next oracle step: separate the remaining explicit-pass type-order noise from real RUB body diffs first, keep treating early `changed=false` compare hunks as non-RUB until mutation is proven, then split the new runtime work between the still-leading unchanged pass-heavy self-opt functions (`Func 497`, `Func 1168`, `Func 229`, `Func 990`, `Func 883`, `Func 1213`, `Func 1061`, `Func 105`, `Func 1063`) and the older lift-heavy `Func 1382` trace before the later loop/block-order family, and rerun self-opt compare plus a larger mixed-generator compare-pass lane before spending more budget on ordered-prefix proof.
     - Remaining work:
     ...
     - Risks:
      ...
      - Performance is still materially over budget on self-opt compare even after the latest scan/liveness audit, Binaryen-shaped raw filtering, the later medium-branchy lifted hot skip, the later call-heavy mixed-if mesh hot skip, and the later localset-heavy value-if mesh hot skip (`402.0653 ms` current interleaved average vs `431.7387 ms` latest pre-slice baseline, while Binaryen remains in the ~`92-96 ms` band on the same lane), so any broader carrier-family rewrite needs a narrow guard and a measured payoff.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [CP]002 - Rewrite Coverage and Artifact Validation - Implement the move and test it against branchy, trap-sensitive fixtures plus the MoonBit debug artifact.
     ...
     - Next tasks:
       1. Repair or replace the debug WASI artifact compare input because `tests/node/dist/starshine-debug-wasi.wasm` currently fails `wasm-tools validate` with `too many locals`.
       2. Continue profiling the optimized-artifact lane after the landed pass-internal guard cleanup; current direct `code-pushing` optimized-WASI compare is semantically green but much slower than Binaryen, and the micro-ladder win does not yet prove artifact-budget parity.
       3. Decide whether to expand the implemented Binaryen surface or keep the pass direct-only after the debug artifact lane is usable and performance is better understood.
     - Doc: [0066#L213](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L213)
3. Do work.
   - Land the slices above in dependency order in the implementing file(s) and any required scheduler, preset, or dispatcher surfaces.
   - Wire the pass into the exact top-level slot(s) and nested rerun sites documented in the research doc before calling the work done.
