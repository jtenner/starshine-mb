...
- The last fresh original-input WAGO sweep remains final7 over all 1,330 tracked fixtures at or below 2 MiB and the unchanged 766-file stable cohort. The runtime-safe final9 retained-artifact projection is `.tmp/wago-o4z-pass8-final9-safe-projected-20260821/`; replacing primes, fannkuch, fasta, and retained ifs with the latest exact-transform outputs projects 412,261 Starshine bytes versus pinned Binaryen v131 at 415,485, a 3,224-byte / 0.776% win with projected 112/597/57 smaller/equal/larger, 2,080 positive residual bytes, 47,869 bytes removed from the 460,130-byte baseline, and 107.22% original-gap closure. Native SHA-256 is `84c39740f7056c4bef913badd5c946484bd375b7f4f117d2165360d4239003d8`. Fresh outputs are primes 22,068, fannkuch 23,335, fasta 23,727, and runtime-safe ifs fallback 23,632; applying only the exact shift fold to retained ifs yields a separately validated/runtime-equal 22,527-byte projection artifact. All fresh artifacts pass independent validation and exact state, and primes passes the expanded stack/temp-ret/main/allocator oracle. Final-only canonicalization now combines consecutive constant i32 left shifts only when both counts and their sum remain below 32, removing 36 bytes from each projected stateful artifact while preserving the masked-count boundary. Vacuum now computes its existing instruction/stack-effect facts first and runs the same raw preclean immediately for guarded-hazard functions, avoiding irrelevant specialized scans. Controlled primes A/B improves raw preclean 39.701 → 36.876 ms (-7.12%), pipeline 192.925 → 188.934 ms (-2.07%), whole command 359.286 → 354.869 ms (-1.23%), and direct level-correct Vacuum 12.176 → 11.779 ms (-3.27%) with byte-identical direct output. Explicit-v131 Vacuum smoke is regular 1,000 normalized; the dedicated lane has 699 normalized plus 301 pre-existing smaller outputs that are byte-identical under isolated baseline/final binaries, with zero failures. The broader projection remains retained-artifact evidence, not a fresh corpus sweep. Full validation is 10,582/10,582; all passes 6,948/6,948; command 252/252; pass manager 335/335. Remaining leaders are ifs +414, JSON-SIMD +397, primes +378, fannkuch +173, fasta +135, scalar JSON +86, urem +64, startup +60, issue1809 +50, fuzzcase 1825 +33, deeply nested struct +25, bump-pointer GC +24, and return-call-indirect +21 each. Preserve xjb/mandelbrot fallbacks, packed-array semantics, stateful runtime exclusions, and fresh-sweep provenance.
