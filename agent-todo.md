...
- **Current progress:** analysis-only Flat IR classification, scalar function-result materialization, reachable/unreachable `local.tee` lowering at function roots, structured-region roots, and ordinary operand positions, ordered scalar ordinary-operand preludes, branch-free defaultable scalar `block`/`if` routing, branch-free zero-input/no-backedge scalar `loop` result routing, plain carried scalar `br`, and simple same-type carried scalar `br_if` routing into one temp per defaultable scalar block target are implemented with focused verifier-backed tests. Direct terminal branches now unblock target-block result erasure, and mixed supported `br`/`br_if` exits share one target temp. Rich or target/flow-type-mismatched `br_if`, inputful/backedge loops, `br_table`, branch-targeted `if`, legacy-try values, generic unreachable placeholders, EH, profiles, and public wiring remain open.
- **Deliverables:**
