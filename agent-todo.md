...
| `ssa-nomerge` | Direct behavior is closed, including the 2026-07-21 unsupported-HOT admission repair: loop-backedge, one-arm, multisource, and sibling LocalGraph routes fail closed instead of aborting, and invalid combined batch writeback restores the original module. Fresh current-native/v131 evidence is regular `100000/100000` normalized, aggregate `10000/10000` normalized, wasm-smith `9956/9956` normalized plus 44 Binaryen failures, and all 15 formerly aborting saved inputs exit 0 and validate. | None; full `ssa` remains separate future work. |
