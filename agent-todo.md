...
- [FUZ]1021 - Invalid Binary Malformed-Byte Matrix
  - Status: IN PROGRESS (cron 23, 2026-05-21). Latest atomics operand-context slice adds decode-rejected, non-smoke `overwide-atomic-load-memarg-align-uleb`, `overwide-atomic-store-memarg-align-uleb`, `overwide-atomic-load-memarg-offset-uleb`, and `overwide-atomic-store-memarg-offset-uleb`, which emit valid `0xFE 0x10` `i32.atomic.load` / `0xFE 0x17` `i32.atomic.store` subopcodes before corrupting the following memarg alignment or offset ULEB; docs now name atomics malformed and overwide alignment/offset operand-context coverage. Previous atomics malformed load/store align/offset memarg, atomics reserved-subopcode byte, section-payload underflow, atomic-prefixed subopcode malformed/overwide, overwide section vector-count, malformed vector-count, and smoke-scheduler slices are landed and documented; continue FUZ1021 by selecting another malformed-byte class such as remaining invalid UTF-8, opcode/prefix, lane, overwide SIMD operands, or additional atomics operand-context corruptions. Current TDD evidence: focused invalid-binary wbtest first failed on missing overwide atomics memarg strategies, then `moon fmt && moon test src/fuzz` passed after implementation.
