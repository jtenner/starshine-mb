...
- **Deliverables:**
  ...
  - [ ] One returned-value API for fallthrough, explicit returns, block/if/loop/select, typed/typeidx controls, static locals, tuples, flat multivalue lanes, and represented exceptional exits. *(GC result closure now derives nested direct result dependencies from stable call identities rather than inspecting only a function's final top-level call; the returned-value analyzer itself still has specialized lanes to consolidate)*
