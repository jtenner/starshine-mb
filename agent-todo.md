...
- [FUZ]1017 - Command Harness Generator Profiles, Pass Profiles, And Idempotence Checks
  - Status: IN PROGRESS (cron 23, 2026-05-22). Current slice landed profile modifiers for `cmd-harness`: `gen=<gen-valid-profile>` / `generator=<...>`, `passes=none|cleanup|optimize-core|each-pass|default-pipeline`, `idempotence`, and `codec-idempotence`; `run_cmd_fuzz_harness(...)` exposes matching direct parameters and reports generator/pass labels plus idempotence counters in stats and persisted failure metadata. The optimizer-failure pass minimizer now removes reproducing chunks before final single-pass cleanup and `optimize`-stage failure reports compute a minimized pass list from the generated failing module. Persisted generated-case failure metadata now also includes the resolved GenValid config label, the generated module attempt count, and generated feature facts. `moon test src/cmd` and `moon test src/fuzz` pass for the profile/minimizer slice; `moon test src/cmd` passes for the metadata slice.
  ...
  - Deliverables remaining: stronger each-pass/common-cluster/default-pipeline coverage where full pass expansion is affordable, and native differential-availability tests where supported.
