...
- Latest diagnostic timing is over the 2x target for the current Func509 artifact: timing-only helper replay `.tmp/dae011-timing-only-20260526` completed without normalized-WAT comparison and reported `2785.602ms` Starshine pass versus `836.627ms` Binaryen pass. `[DAE]011` remains open for deeper `dae_run_core` profiling; detail timers from `.tmp/dae011-detail-trace.stderr` attribute the current release traced artifact run to `2748777us` in `detail:dae:core`, `4us` in skipped `detail:dae:nested-total`, and `47855us` in `detail:dae:prune-unused-simple-func-types`.
...
- [DAE]011 - Performance Stabilization
  ...
  - Deliverables:
    ...
    - [ ] Add deeper `detail:dae:core:*` timers around expensive setup/scanner families inside `dae_run_core` before choosing an optimization.
