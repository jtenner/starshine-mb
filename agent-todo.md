...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [DCE]002 - Prefix Regression and Artifact Replay - Lock the corrected DCE behavior into focused tests and rerun the MoonBit debug-artifact compare harness.
     ...
     - Status: focused DCE regressions plus the earlier hot-lift/effects fixes are landed, and the suite is green again (`moon test` => `1929/1929`). A HOT-based live repro still locks the reduced typed-loop carrier hazard in DCE, and the later lowering-side stack bug behind the stale `Func 1730` checkpoint is now fixed in `hot_lower`: shared HOT operands are re-emitted correctly instead of being skipped by `node_id` reuse. The exact artifact path is now covered too by a native `cmd` regression that runs `--dead-code-elimination` on `tests/node/dist/starshine-debug-wasi.wasm` through `run_cmd_with_adapter` and validates the output module in-memory. But the large-oracle gate is still red: `.tmp/pass-fuzz-dce-genvalid-1000` hit the `20`-mismatch cutoff after only `28` compared `gen-valid` cases, with `0` command failures and `0` validation failures. A sampled family is `case-000002-gen-valid`, where Binaryen keeps `drop (f64.const 45)` and Starshine removes it entirely. The remaining work is now explicit fuzz mismatch reduction plus ordered-prefix proof/runtime follow-up, not a known single-pass artifact validate crash.
     - Saved mismatch cases: `000001`, `000002`, `000003`, `000005`, `000006`, `000007`, `000010`, `000011`, `000012`, `000013`, `000014`, `000016`, `000020`, `000021`, `000022`, `000023`, `000024`, `000026`, `000027`, `000028` under `.tmp/pass-fuzz-dce-genvalid-1000/failures/case-000XXX-gen-valid`.
     - Reproduce the corpus: `bun scripts/pass-fuzz-compare.ts --pass dead-code-elimination --generator gen-valid --count 1000 --max-failures 20 --out-dir .tmp/pass-fuzz-dce-genvalid-1000`
     - Reproduce one saved case exactly: `bun scripts/pass-fuzz-compare.ts --pass dead-code-elimination --replay-failures-from .tmp/pass-fuzz-dce-genvalid-1000 --case-index 2 --out-dir .tmp/pass-fuzz-dce-case-000002-replay`
     - Inspect the sampled mismatch: compare `.tmp/pass-fuzz-dce-genvalid-1000/failures/case-000002-gen-valid/starshine.wat` vs `.tmp/pass-fuzz-dce-genvalid-1000/failures/case-000002-gen-valid/binaryen.wat`
     - Doc: [0066#L178](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L178)
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [RUN]001 - Triple-Slot Replay and Ordered-Prefix Parity - The core function pass is landed with exact single-pass oracle parity for same-typed block peeling and loop demotion, and the public `optimize` / `shrink` presets now replay `remove-unused-names` in all three currently-modeled RUN slots. Focus has shifted from missing scheduler wiring to proof and throughput.
     ...
     - Current blocker: standalone `--remove-unused-names` parity is exact and the repeated-slot scheduler wiring is now landed. The stricter oracle evidence is also clean: `.tmp/self-opt-run-remove-unused-names` is canonical-wasm-equal and normalized-WAT-equal on `tests/node/dist/starshine-debug-wasi.wasm`, `.tmp/pass-fuzz-run-genvalid-5000` reached `5000/5000` normalized matches with `0` mismatches and `0` command failures, and `.tmp/pass-fuzz-run-smith-2000` reached `1767/1767` normalized matches with `0` mismatches before the Binaryen command-failure cutoff. The remaining repo-side work is ordered-prefix artifact proof plus more serial throughput / GC-churn reduction, not missing direct-pass semantics.
   ...
   - [RUN]002 - Pass-Fuzz Command-Failure Classification And Replay - Keep the `remove-unused-names` differential-fuzz command-failure corpus visible as a distinct follow-up item so Binaryen parser and canonicalization gaps are tracked separately from Starshine semantic regressions.
     - Deliverables: preserve the saved `wasm-smith` corpus under `.tmp/pass-fuzz-run-smith`; replay the Binaryen-only failures independently of Starshine pass work with `--replay-failures-from`, `--failure-class`, and `--case-index`; persist `input.print.wat` beside saved command-failure inputs so parser gaps can be inspected without a second manual print step; keep the harness-level `failureClass` buckets current as new command-failure families appear.
     - Current blocker: larger `remove-unused-names` pass fuzzing is still semantically clean on Starshine (`.tmp/pass-fuzz-run-genvalid-5000` reached `5000/5000` normalized matches with `0` mismatches and `0` command failures; `.tmp/pass-fuzz-run-smith-2000` reached `1767/1767` normalized matches with `0` mismatches), but the larger `wasm-smith` sweep still stops on `200` Binaryen-side canonicalization failures. The current classified families in that larger run are `187` `invalid type index` cases, `8` `Recursion groups of size zero not supported` cases, `4` `invalid wasm type: -64` cases, and `1` `invalid tag index` case (`case 662`). The saved `invalid tag index` repro is tracked in [0072](/home/jtenner/Projects/starshine-mb/docs/0072-2026-03-28-run-invalid-tag-index-binaryen-parser-gap.md).
     - Suggested Tests: `bun scripts/pass-fuzz-compare.ts --pass remove-unused-names --generator gen-valid --count 5000 --max-failures 20 --out-dir .tmp/pass-fuzz-run-genvalid-5000`, `bun scripts/pass-fuzz-compare.ts --pass remove-unused-names --generator wasm-smith --count 2000 --max-failures 200 --out-dir .tmp/pass-fuzz-run-smith-2000`, `bun scripts/pass-fuzz-compare.ts --pass remove-unused-names --replay-failures-from .tmp/pass-fuzz-run-smith-2000 --case-index 662 --out-dir .tmp/pass-fuzz-run-smith-case-662-replay`, `bun scripts/pass-fuzz-compare.ts --pass remove-unused-names --replay-failures-from .tmp/pass-fuzz-run-smith-2000 --failure-class binaryen-invalid-tag-index --out-dir .tmp/pass-fuzz-run-smith-invalid-tag-index-replay`, and targeted `wasm-opt --all-features --remove-unused-names` replay on the saved failing inputs.
3. Do work.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [RUB]002 - Multi-Slot Cleanup and Artifact Compare - Replay the pass in each Binaryen slot and confirm later `merge-blocks` opportunities match the reference output.
     ...
     - Reproduce the corpus: `bun scripts/pass-fuzz-compare.ts --pass remove-unused-brs --generator gen-valid --count 1000 --max-failures 20 --out-dir .tmp/pass-fuzz-rub-genvalid-1000`
     - Reproduce one saved case exactly: `bun scripts/pass-fuzz-compare.ts --pass remove-unused-brs --replay-failures-from .tmp/pass-fuzz-rub-genvalid-1000 --case-index 2 --out-dir .tmp/pass-fuzz-rub-case-000002-replay`
     - Inspect the sampled mismatch: compare `.tmp/pass-fuzz-rub-genvalid-1000/failures/case-000002-gen-valid/starshine.wat` vs `.tmp/pass-fuzz-rub-genvalid-1000/failures/case-000002-gen-valid/binaryen.wat`
     - Doc: [0066#L188](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L188)
   - [IR]003 - Lifted Label Ownership Repro for RUB Artifact Gate - Reduce `Func 24` from the MoonBit debug artifact to a standalone `hot_lift`/verify repro so RUB artifact compare can resume.
     ...
     - Status: the in-tree repros now cover both the malloc-shaped `Func 24` case and the loop string-reader `Func 299` case in `src/passes/remove_unused_brs_test.mbt`. The label-owner corruption is fixed for value-`if` select rewrites, one-armed `if br` rewrites, and exit-only value-`if` voidification by preserving the original label owner instead of replacing labeled `If` nodes in place. The later `Func 7756` branch-arity failure is also fixed with focused regressions for multi-slot payload ladders and multi-value branch-payload `if` arms, and the stale `Func 807` single-pass stack-underflow gate is now covered green by a native `cmd` artifact replay test. The remaining work is later ordered-prefix evidence, not replaying the already-fixed label-owner, branch-arity, or single-pass validate cases.
