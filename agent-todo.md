...
- Large-artifact `simplify-locals` parity is currently blocked earlier on this rebased tree because `tests/node/dist/starshine-debug-wasi.wasm` is absent, so the canonical self-opt compare frontier cannot be refreshed locally until that debug artifact is rebuilt or restored.
- The last unreduced artifact frontier from the pre-rebase compare moved past the retired `Func 216` gap and into the later `UInt64.to_byte` helper family, led by `wast.wt_push_le_bytes` (`Func 520`) and sibling `binary.encode_*` / buffer-write helpers.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [SL]004 - Slot Validation and Artifact Replay - Lock the completed pass into the late slot and prove parity against Binaryen on both focused and large-artifact lanes.
     ...
     - Current blocker: the late-slot regression work is still in-tree and the focused Binaryen lanes remain green, but the artifact parity frontier has changed. `src/passes/registry_test.mbt` now locks `reorder-locals` as a `module_pass`, `src/passes/optimize_test.mbt` now locks the modeled `heap2local -> simplify-locals -> precompute` cleanup slot, and `src/passes/simplify_locals_test.mbt` now covers both the reduced one-use `if (result i32)` call-argument sink that retired the old absolute `Func 216` parity gap and the stacked multivalue-fed guard that keeps the invalid control move out of the pass. The reduced `Func 216` self-opt repro is canonically equal to Binaryen again, but the rebased branch no longer has `tests/node/dist/starshine-debug-wasi.wasm`, so the full `--simplify-locals` artifact frontier cannot be refreshed locally until that debug artifact is rebuilt or restored. The last unreduced artifact mismatch from the pre-rebase compare had already moved beyond `Func 216` into the later `UInt64.to_byte` helper family, led by `wast.wt_push_le_bytes` (`Func 520`) and sibling `binary.encode_*` / buffer-write helpers. The two newer cleanup follow-ons are also in-tree: `src/ir/hot_lower_test.mbt` now covers dead nested `local.tee` pruning during lower, `src/passes/perf_test.mbt` now covers dropping dead raw adjacent `local.set` / `local.get` pairs without hot lift, and `src/passes/optimize_instructions_test.mbt` now matches the lowered shape after that dead-tee cleanup. Those follow-ons did not show a measured frontier move on the earlier artifact, so the next parity step should reduce the `UInt64.to_byte` helper family once the debug artifact is back rather than widening more cleanup heuristics blindly.
