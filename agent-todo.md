...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [DCE]003 - Runtime Budget and Oracle Refresh - Close the remaining DCE speed gap and refresh parity evidence on a valid baseline/oracle path.
     ...
     - Current blocker: direct native `--dead-code-elimination` replay on `tests/node/dist/starshine-debug-wasi.wasm` is now valid end-to-end but still takes roughly `4.4s-4.7s`, far above the last usable Binaryen DCE timing budget on the same artifact family. The trace-only large-function debug-dump guard is landed, but the longer debug-artifact cleanup replay (`DCE -> RUN -> VQ -> OI -> SL`) is still runtime-bound before it yields a fresh parity checkpoint. The normal `bun scripts/self-optimize-compare.ts ... --dead-code-elimination` entrypoint currently stops before Binaryen runs because the baseline input fails validation, so this slice needs either a regenerated valid baseline or continued use of the last successful oracle artifact until the compare path is repaired.
     - Doc: [0066#L178](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L178)
3. Do work.
