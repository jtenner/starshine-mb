...
- [DAE]002 - Nested Post-Inlining Cleanup and Artifact Compare
  - Status: active. A first narrow scheduler slice now runs `dead-code-elimination -> simplify-locals -> vacuum` only on DAE-touched functions for small modules and keeps untouched functions unchanged; it still only traces the upstream `precompute-propagate` prefix and is not the full Binaryen `optimizeAfterInlining` lane. Broad/large modules remain guarded because the earlier whole-module cleanup rewrote untouched functions and the current artifact lane is not yet performant enough for full nested replay. Artifact replay `.tmp/dae-optimizing-artifact-replay-finish2` stayed semantically red at first differing function `defined=11 abs=28` while Starshine pass-local time was faster (`665.946ms` vs Binaryen `935.447ms`); a later local replay in `.tmp/dae002-nested-artifact-mincleanup2` still differed at `defined=11 abs=28` but showed an unresolved current-runtime discrepancy that timed out before the nested marker in a traced run.
  ...
  - Current DAE002 evidence: `moon test src/passes` passes with the touched-only scheduler regression; `.tmp/dae002-mincleanup-200` reported `199/200` compared, `198` normalized matches, `1` local-declaration mismatch, `0` validation failures, and `1` Binaryen/tool command failure; `.tmp/dae002-mincleanup-1000` reported `998/1000` compared, `985` normalized matches, `13` mismatches, `0` validation failures, and `2` Binaryen/tool command failures.
