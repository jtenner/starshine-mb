...
- **Status:** reason-coded decisions/counters, the coherent immutable original snapshot, and the mutable current boundary graph with module/call/signature/body epochs are implemented. The dependency graph now records direct, direct-tail, conservative forwarding/uniform/result/type/body/cleanup domains plus deterministic normalized Tarjan SCCs. Epoch-bearing family work items cover all planned candidate families; the computed/localized retry is the first specialized production consumer with stale-item refresh. Family-specific dependency extraction, common plans across the remaining specialized phases, and de-artifacted later scheduling remain open.
...
- **Status:** the ordinary core full-module forward rescans are replaced by a deterministic queue over current caller/callee invalidations. Binaryen's source-backed one-wave single-caller profitability stop remains between queue frontiers. The shared work-item model now carries target/component/family and expected epochs, and the small-module computed/localized retry uses it with stale requeue and SCC refresh. Broad single-parameter identity constants are definition-independent. Broad cross-family execution, multi-parameter constants, GC/results, and selected artifact lanes still use separate phases, arrays, bands, or caps. Forwarding-component discovery itself is now uncapped: the former 8-component, 64-node, 8-round, 129-instruction, and bounded sink/cycle retry limits are removed in favor of monotonic exhaustion.
