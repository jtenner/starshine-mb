...
- [DAE]002 - Nested Post-Inlining Cleanup and Artifact Compare
  ...
  - Current DAE002 evidence: `moon test src/passes` passes with the touched-only scheduler regressions; `.tmp/dae002-optimize-casts-200` reported `199/200` compared, `198` normalized matches, `1` mismatch, `0` validation failures, and `1` Binaryen/tool command failure; `.tmp/dae002-optimize-casts-1000` reported `998/1000` compared, `985` normalized matches, `13` mismatches, `0` validation failures, and `2` Binaryen/tool command failures, preserving the prior local-declaration frontier shape; a post-optimize-casts debug-artifact trace still skips nested cleanup at `touched=12`.
