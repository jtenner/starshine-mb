...
- `RemoveUnusedModuleElements`: closed-world typed `struct.new` / `struct.new_desc`, mutable `struct.set`, ref-bearing `array.new` / `array.new_fixed` / `array.new_elem`, mutable `array.set` / `array.init_elem`, ref-bearing `array.fill`, and ref-bearing `array.copy` payloads now defer unread funcref/global/elem references until a live `struct.get*` or `array.get*` read occurs; remaining GC precision is future GC consumer expansion plus any later hardening around reference-only carriers.
