...
- **Latest bounded iteration (2026-07-15):** nonthrowing synthetic catch-all lowering and recursive suffix ownership remain the current narrow measured output win (`212` Starshine cleanup bytes versus `236` Binaryen). Two red-first code commits cached immutable label-use facts across support and routing, admitted direct `i32.shl` and `i32.shr_s` resultless-call roots under the recursive distinct one-use proof, threaded rewrite-state caches through rich branch admission, and moved the private outside-roster boundary to `i32.shr_u`. The final representative pass-only median is `1,259 us` (`4.73x` Binaryen v130's `266.05 us`), improved from the iteration-start `1,347.5 us` but still outside the `<=2x` target. Typed catches/payloads, nested-pop repair, `rethrow`, `delegate`, structured suffix control deletion, broader behavior, a GenValid profile, four-lane signoff, performance, and public wiring remain open. Source: `docs/wiki/raw/binaryen/2026-07-15-flatten-version-130-nonthrowing-bridge-suffix-cache-impact.md`.
- **Deliverables:**
