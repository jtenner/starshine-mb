...
- Both-canonical diagnostic helper: `scripts/self-optimize-compare.ts --canonicalize-binaryen-output --dae-optimizing` still first-diffs at `defined=509 abs=526`; the latest replay `.tmp/dae006-outer-block-suffix2-20260526` kept that frontier. Research notes `0576` through `0584` reduce the saved Func505 body from unknown/risky output-shape drift to representation-only control/local-carrier, branch-polarity, temp-local, and dropped-debris drift; `0585` adds a diagnostic-only canonical-function normalizer for that inspected Func505 parser-loop family; `0586` records the successor Func509 frontier as a semantic-safe, size-losing dead-return-suffix cleanup gap; `0588` adds a focused outer-block reduction/helper but does not close the artifact because the final cleanup hook still does not hit Func509; `0589` proves the printed `--print-func 526` post-return wrapper shape is already covered by the existing helper, narrowing the unresolved cause to exact in-memory final-cleanup input or later lowering/encoding reintroduction.
...
- `[DAE]014` latest source/index sync is closed as of 2026-05-26: `docs/wiki/binaryen/passes/dae-optimizing/index.md` and `starshine-strategy.md` now link research notes `0576` through `0586` and record the current Func509 diagnostic frontier as a semantic-safe dead-return-suffix cleanup gap that needs a small TDD pass cleanup to advance beyond `defined=509 abs=526`.
...
- [DAE]005 - Debug Artifact Raw Frontier and Type-Section Drift
  - Goal: decide how far raw-byte parity should be pursued after the current default `defined=336 abs=353` type-section/type-index drift.
  - Why: the default helper intentionally keeps Binaryen's direct raw output as the byte reference, while Starshine is compared after strip-debug canonicalization; both-canonical mode can move past this, proving many body frontiers are compare-layer representation families.
  - Deliverables:
    - Keep default raw behavior unchanged unless the team explicitly changes the byte-reference contract.
    - If raw parity is desired, investigate type-section/type-index construction rather than Func408 or Func336 body rewrites.
    - Add a safe type-id stripping/normalization only in diagnostic paths where signatures are proven equivalent.
    - Keep `binaryen.raw.wasm` and `starshine.raw.wasm` in artifacts for future raw inspections.
...
- [DAE]006 - Both-Canonical Frontier Advancement
  - Goal: continue the diagnostic both-canonical frontier from current `defined=509 abs=526`.
  - Current classification:
    ...
    - `0578-2026-05-25-dae-func505-default-zero-guard-cleanup.md` verified the suspicious `i32.gt_s (local.get $5) (i32.const 57)` reads an unwritten non-param local that defaults to zero after param removal, added a focused Moon regression, and landed a guarded pass-side cleanup for that semantic-safe subshape.
    - `0579-2026-05-25-dae-func505-successor-frontier.md` completed the successor both-canonical replay in `.tmp/dae-func505-recovery-artifact`: the first diff remains `defined=505 abs=522`, Starshine pass time is `3264.383ms` versus Binaryen `1033.990ms`, both canonical wasm outputs validate, and the remaining loop induction, digit guard, error construction, and final empty-input branch drift is classified unknown/risky rather than representation-only.
    - `0580-2026-05-25-dae-func505-empty-count-branch-inversion.md` closed the final empty-input count branch subshape with a selected-Func505-only cleanup for `(x == 0 ? A : B)` to `(x ? B : A)`. The `.tmp/dae-func505-inverted-count-artifact` replay still first-diffed at `defined=505 abs=522`, both canonical outputs validated, and pass-local timing stayed over target (`3068.534ms` Starshine versus `955.834ms` Binaryen). The remaining loop induction, digit/underscore guard, overflow/error-construction, and temp-local reuse drift remained unknown/risky.
    - `0581-2026-05-25-dae-func505-bool-carrier-cleanup.md` closed the lower-bound bool-carrier subshape with a selected-Func505-only cleanup for `predicate; if(result i32) 1 else 0; if` to `predicate; if`. The current `.tmp/dae-func505-bool-carrier-artifact` replay still first-diffs at `defined=505 abs=522`, both canonical outputs validate, Starshine canonical output is 8 bytes smaller than the prior successor artifact, and pass-local timing is still over target (`2721.553ms` Starshine versus `873.679ms` Binaryen). The remaining loop induction/exit carrier, underscore guard polarity, overflow/error-construction, and temp-local reuse drift remains unknown/risky.
    - `0582-2026-05-25-dae-func505-underscore-guard-reduction.md` reduced the underscore guard polarity around `Func4495` and classifies that local subshape as representation-only control inversion: Starshine's `if (predicate) br K else BODY` and Binaryen's `if (eqz predicate) BODY` call the predicate once, preserve the parse body effects, and join at the same continuation. No pass behavior or compare normalizer changed.
    - `0583-2026-05-26-dae-func505-overflow-temp-reduction.md` reduced the overflow/error-return subshape and classifies it as representation-only unsigned comparison polarity plus temporary-local drift: Starshine's `acc > limit` and Binaryen's `limit < acc` guard the same error-construction path around the same `((-1 - digit) / 10)` limit expression. No pass behavior or compare normalizer changed.
    - `0584-2026-05-26-dae-func505-loop-carrier-reduction.md` reduced the remaining loop induction/exit-carrier subshape and classifies it as representation-only control/local-carrier drift: Starshine's high-local `index < length` carrier and extra exit block align with Binaryen's low-local `length > index` carrier and direct exit block under the same parse-loop effects and continuation. The saved Func505 first-diff body is now classified representation-only after inspected subshape reductions.
    - `0585-2026-05-26-dae-func505-diagnostic-normalizer.md` adds a marker-rich diagnostic-only canonical-function normalizer for the inspected Func505 parser-loop family plus a focused fake-tool script fixture. This changes only compare fallback text; it does not affect raw WAT/wasm equality or pass behavior.
    - `0586-2026-05-26-dae-func509-dead-return-suffix-frontier.md` completed the successor both-canonical replay in `.tmp/dae006-next-frontier-20260526`: the first diff moved to `defined=509 abs=526`, both canonical outputs validate with only the existing large-local-count VM warning, Starshine pass time is `2923.619ms` versus Binaryen `891.787ms`, and the frontier is classified semantic-safe, size-losing dead-return-suffix cleanup gap because Starshine leaves unreachable allocation/store/value debris after a function `return` while Binaryen stops at the same returned object.
    - `0587-2026-05-26-dae-func509-return-suffix-cleanup-attempt.md` added focused root-return and terminal-wrapper cleanup tests plus narrow DAE cleanup helpers. A recovery follow-up fixed the pending lowered-return helper tuple bug and added a block-wrapped terminal-wrapper regression, but the replay `.tmp/dae006-return-suffix-recovery-20260526` still first-diffs at `defined=509 abs=526` (`2870.440ms` Starshine pass versus `867.327ms` Binaryen pass). Trace spot-checking shows no final-return-suffix cleanup hit on the debug artifact, so the current helpers still miss the actual pre-encode Func509 shape. A later recovery probe rebuilt the native CLI and printed `--dae-optimizing --print-func 526` to `.tmp/dae-print-func526.err`, confirming the pass-side shape is an outer `block I64` containing an inner `block (Void)` with `br Label 1` exits, an `unreachable`, a fallthrough `local.get $20; return`, then post-inner `local.set $2` plus the 8-byte wrapper suffix.
    - `0588-2026-05-26-dae-func509-outer-block-reduction.md` added a focused white-box reduction and helper for the outer-block/no-fallthrough wrapper family. The latest replay `.tmp/dae006-outer-block-suffix2-20260526` still first-diffs at `defined=509 abs=526` (`2791.086ms` Starshine pass versus `843.419ms` Binaryen pass), and tracing still shows no final-return-suffix cleanup hit, so the exact final-cleanup input or later lowering/encoding boundary remains unresolved.
    - `0589-2026-05-26-dae-func509-final-cleanup-input-reduction.md` added a focused white-box fixture for the printed `--print-func 526` outer-block/fallthrough-return/post-return-wrapper shape and proved the existing helper already strips it. The printed shape is therefore not the missing matcher; the remaining issue is exact in-memory input at the final hook or suffix introduction/reintroduction after that hook.
  - Next action:
    - Instrument or dump the exact instruction list received by `dae_strip_module_root_return_suffix_once` for `defined=509 abs=526`; the `--print-func` and post-encode canonical shapes are not sufficient because the reduced printed shape is covered but the artifact still does not trace a final-return-suffix cleanup hit.
    - Determine whether the wrapper suffix is introduced or reintroduced after the DAE final cleanup by lowering/encoding, or whether the in-memory cleanup input differs from both textual shapes.
    - If the final-cleanup input is safe and matchable, extend `dae_strip_root_return_suffix_instrs` with a regression for that exact shape and replay the both-canonical artifact. If the suffix is introduced after DAE, document the boundary and choose whether a later cleanup hook or diagnostic classification is appropriate.
    - Preserve the landed root-return, simple wrapper-suffix, outer-block, and printed-shape tests/helpers if they remain valid, but do not claim Func509 closure until the both-canonical artifact compare moves beyond `defined=509 abs=526`.
  - Deliverables:
    ...
    - Record each moved frontier with artifact dir, validation command, pass-local timing, and classification.
  - Known closed subfrontiers:
    - Func208, Func218, Func239, Func259, Func333: canonical wrapper families.
    - Func336: equivalent type-id/type-section representation drift under both-canonical mode after type-id stripping.
    - Func372, Func396, Func400, Func403, Func412, Func427, Func444, Func445, Func446, Func447: inspected wrapper/inert-drop/unreachable-debris families, plus Func445 selected result removal.
