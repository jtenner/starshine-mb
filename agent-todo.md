...
- [O4Z-AUDIT-SSA] - Deep audit `ssa-nomerge`
  ...
  - Finding: `docs/wiki/raw/research/0722-2026-06-09-ssa-nomerge-exceptional-edge-audit.md` fixed a true try_table/throw corruption caused by applying normal-flow-only local SSA across exceptional edges. `ssa-nomerge` now skips exceptional-flow HOT mutation, and the raw no-write path now recursively materializes default body-local reads across branchy control and cleans dropped-unreachable debris before hard `unreachable` tails. Follow-ups keep nested structured param/body-local writes on canonical slots when the next observable use reads before overwrite, keep body-local `local.tee` canonical for branch continuations/result branches that read before overwrite, avoid dead/unneeded branch and label merge copies, preserve no-copy `br_if` and no-copy `br_table` terminators, narrow the `Func 4302` suspicious-carrier false positive, and raise the structured raw budget enough to reduce direct artifact `large-structured-local-writes` skips from `2117` to `9`. Latest validation for the high-threshold/no-copy-`br_table` slice: focused `ssa_nomerge_test` `120/120`, `moon test src/passes` `2139/2139`, full `moon test` `5398/5398`, direct compare `.tmp/pass-fuzz-ssa-nomerge-brtable-8192-10000` compared `9977/10000` with `9977` normalized matches and `0` mismatches, and GenValid parity `.tmp/pass-fuzz-ssa-nomerge-parity-brtable-8192-genvalid-normalized-1000` compared `1000/1000` with `1000` compare-normalized matches and `0` mismatches. Latest direct artifact replay `.tmp/self-ssa-nomerge-debug-wasi-brtable-8192` is still not exact-normalized/canonical-function equal (`defined=0 abs=27`) with pass-local time `15.972 ms` vs Binaryen `343.315 ms`; earlier post-cleanup final `.tmp/pass-fuzz-ssa-nomerge-final-100000-after-debris` remains the current 100000-case final baseline (`99751/100000`, `0` mismatches, `249` Binaryen/tool command failures).
