...
- [O4Z-AUDIT-SSA] - Deep audit `ssa-nomerge`
  ...
  - Active `SSANM` work slices, grouped by identifier:
    ...
    - [ ] [SSANM-003b] - Implement LocalGraph `local.tee` no-merge writes
      - Goal: make `local.tee` use the same per-write no-merge eligibility as `local.set` while preserving stack-result semantics.
      - Deliverables: tee freshening, tee get-retargeting, dead tee overwrite, tee inside child expressions where HOT/raw lowering supports it, and fail-closed tests where mutation would duplicate effects or traps.
