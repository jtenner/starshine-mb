...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [DCE]002 - Prefix Regression and Artifact Replay - Lock the corrected DCE behavior into focused tests and rerun the MoonBit debug-artifact compare harness.
     ...
     - Status: focused DCE regressions plus the earlier hot-lift/effects fixes landed; replay gets past the old `func[80]` / `func[106]` hot-lift aborts, the typed-loop lowering bug that broke canonical `--vacuum` replay at `func[81]` is fixed, the dead-result wrapper failures at `func[67]` and `func[190]` are closed, detached shared-subtree cleanup no longer aborts on parent-first deletion order, final non-fallthrough structured control now materializes explicit `unreachable` tails when Starshine really voidifies the last live control, and the unchanged raw-function fast path no longer drops lift/lower canonicalization for dead `drop` tails after non-fallthrough roots. The brief branchy no-drop raw-skip widening was backed out because those wrappers still need the hot typed view for safe result trimming. The minimized `moonbit.string_equal` replay shape plus rebuilt release-artifact direct and five-pass DCE outputs validate and survive `wasm-opt --strip-debug`, and the carried-wrapper `unreachable` omission in `run_cmd_with_adapter.inner` is now fixed and locked down with exact IR and DCE live repro tests. Some remaining Binaryen diffs are now known-safe stack-polymorphic wrapper deltas rather than correctness bugs; after stripping type/local churn and label-only decoration, the next real body mismatch is later, around structural line `6606`, where Binaryen keeps an explicit `unreachable` before a later `if` and Starshine still closes the wrapper earlier.
     - Doc: [0066#L178](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L178)
   - [DCE]003 - Runtime Budget and Oracle Refresh - Close the remaining DCE speed gap and refresh parity evidence on a valid baseline/oracle path.
     ...
     - Current blocker: direct native `--dead-code-elimination` replay on `tests/node/dist/starshine-debug-wasi.wasm` is now valid end-to-end but still takes about `43.4s` on the current serial native path, versus about `1.336s` for Binaryen `--dce` and another `1.782s` for Binaryen normalization on the same artifact. The trace-only large-function debug-dump guard is landed, but the remaining runtime is still dominated by serial hot-lift / DCE work on the large artifact and by the current front parity hunk at structural line `6606`. The longer debug-artifact cleanup replay (`DCE -> RUN -> VQ -> OI -> SL`) is still runtime-bound before it yields a fresh parity checkpoint, and the normal `bun scripts/self-optimize-compare.ts ... --dead-code-elimination` entrypoint still stops before Binaryen runs because the baseline input fails validation, so this slice needs either a regenerated valid baseline or continued use of direct artifact diffing until the compare path is repaired.
     - Doc: [0066#L178](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L178)
3. Do work.
