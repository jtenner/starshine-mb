...
- The definitive fresh original-input WAGO checkpoint is now `.tmp/wago-o4z-pass8-final19-const-expr-validation-fresh-v2-20260822/`, covering all 1,330 tracked fixtures at or below 2 MiB and the unchanged 766-file stable cohort with native SHA-256 `a56af9ecee4d5f3842196c4d59f0e557decb99ae3fbdfe6db3cb46cedbe54938`. Final19 fixes the remaining successful-but-externally-invalid output: active data and element offsets now reject `i32.clz/ctz/popcnt` and `i64.clz/ctz/popcnt`, while global and payload extended-constant semantics remain unchanged. `elem.82` changes from a successful 23-byte invalid output to an explicit `initializer expression is not constant` command error, leaving 1,275 command successes and 1,275 independently valid successful outputs. This is the only fresh status/output change from final18; Starshine emits 413,297 bytes versus pinned Binaryen v131 at 415,485, a fresh 2,188-byte / 0.527% win with 110/603/53 smaller/equal/larger, 3,022 positive residual bytes, 46,833 bytes removed from the 460,130-byte baseline, and 104.90% original-gap closure. Final18 removes duplicate O4z orchestration work: when final canonicalization leaves the fixed-point input structurally unchanged, the already-validated first finish result is reused for the precanonical candidate and stateful suffix instead of rerunning the same finish roster. Exact module equality is the admission proof; changed canonical inputs still run independently. Thirty paired runs improve or remain neutral across ifs (-0.05%), primes (-0.25%), fannkuch (-0.17%), and fasta (-0.48%) by paired median, with selected bytes unchanged. The retained projection remains 412,161 versus 415,485, a 3,324-byte / 0.800% win with 112/602/52 smaller/equal/larger and 1,983 positive residual bytes. Full validation is 10,607/10,607; validator 1,750/1,750; all passes 6,953/6,953; command 271/271. Final17’s empty-else reductions remain signed off: many-table 102, fannkuch 23,331, fasta 23,720, and broad-only `try_table.1` 1,176; the stateful full-memory hashes remain `b46c752a789beced575e808e38bc59fed14b584a257f686338849381b77c0ea0` and `c5f619ac224a8890782b031dfa981bfcbf7957c9a1726dc4fee33f2750c2b464`. The retained projection removes 47,969 bytes and closes 107.45% of the original gap. Remaining fresh leaders are ifs +1,434, JSON-SIMD +397, primes +378, fannkuch +169, fasta +128, scalar JSON +86, startup JSON +60, urem +38, issue1809 +35, deeply nested struct +25, bump-pointer GC +24, return-call-indirect +21 each, array-copy-inline +19, and fuzzcase 1820/1825 +8/+7. Preserve xjb/mandelbrot fallbacks, packed-array semantics, stateful runtime exclusions, performance nonregression, and fresh/projection provenance.
