...
- `simplify-locals` reduced the old absolute `Func 216` parity slice in-tree: the pass now sinks a single-use self-contained `if (result i32)` into later call arguments, but still refuses to move stack-coupled or multivalue-fed control producers.
- `tests/node/dist/starshine-debug-wasi.wasm` has been regenerated, and the canonical `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --simplify-locals` lane is live again on this rebased tree.
- The refreshed artifact-backed correctness bugs found so far are fixed in-tree: `simplify-locals` no longer sinks pre-loop pending `local.set` initializers into loop headers, and one-armed `if` lifting now preserves both Binaryen's `then`-arm `nop` sentinel and any live prefix roots such as `call` / `store` before the tail `local.set`.
- The refreshed `moonbit.malloc` artifact-backed safety bug is now fixed in-tree: pending sinkables now collect local read/write effects through `if` / `try` / `try_table` region bodies, and one-use structured-result inlining only fires on the leading evaluation path when the replacement is effectful. The fresh compare lane at `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-588924` shows `Func 26` (`moonbit.malloc`) printing identically in Starshine and Binaryen again.
- The reduced `Func 41` tee-backed alias family is now fixed in-tree too: late equivalent-copy cleanup only protects tee-defined locals when the current `local.get` is a direct call child, so same-arm load/store aliases can canonicalize back to the source local while later call arguments still keep the tee-backed local alive.
- A lowering-side side-fix is now in-tree too: `hot_lower` dead nested `local.tee` pruning no longer counts `local.get`s that appear only in dead tails after unconditional exits such as `unreachable`. The focused regression is green, but the fresh compare lane at `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-1670794` shows that this cleanup does not move the full `simplify-locals` artifact frontier by itself.
- A first exact-writeback cleanup attempt that stripped lowered `nop` roots was rejected on correctness/parity grounds: `.tmp/pass-fuzz-sl-parity-after-lowered-cleanup` diverged almost immediately because Binaryen preserves many lowered `nop`s. Do not revive the broad `nop` stripping without a reduced oracle-backed proof.
- The kept exact-writeback cleanup is narrower and signed off: `simplify-locals` now only prunes dead copied `local.tee` roots after lower, and `.tmp/pass-fuzz-sl-parity-after-lowered-tee-cleanup` is green at `10000/10000` normalized matches with `0` mismatches.
- Exact writeback now also drops dead adjacent `local.set` / `local.get` pairs after lower without synthesizing new tees, and `.tmp/pass-fuzz-sl-parity-after-lowered-dead-setget-drop` is green at `10000/10000` normalized matches with `0` mismatches.
- The validator-structured raw skip now also runs the cheap exact temp cleanup before returning, and `.tmp/pass-fuzz-sl-parity-after-raw-validator-cleanup-rerun` is green at `10000/10000` normalized matches with `0` mismatches.
- The raw `structured-pure-copy-call-tail` shortcut now also sinks single-use effectful temps when their later `local.get` appears after a pure stack prefix, and the `validator-structured-call-heavy` skip path now applies that same raw rewrite before its cheap cleanup. The new synthetic regression in `src/passes/simplify_locals_test.mbt` stays on the raw shortcut lane via trace and proves the effectful `call -> local.set -> pure-prefix -> local.get -> i32.lt_s` temp disappears without falling back to the full exact pass.
- The validator-structured raw skip now also sinks copied pure locals across later flat zero-stack statement groups when the target and its source locals stay untouched, and the new heavy traced regression in `src/passes/simplify_locals_test.mbt` proves the `local.get -> local.set -> nested-call local.set -> local.get` copy temp disappears while staying on `skip-raw reason=validator-structured-call-heavy`. The current focused parity check is green at `2000/2000` normalized matches with `0` mismatches in `/tmp/starshine-pass-fuzz-compare-2`, but this narrower copy sink does not retire the large-artifact frontier by itself.
- The validator-structured raw skip now also sinks exact copied locals across one immediate zero-result cleanup `if` statement group when that barrier is the only top-level structured op and never touches the source or target locals. The new heavy traced regression in `src/passes/simplify_locals_test.mbt` stays on `skip-raw reason=validator-structured-call-heavy` and proves the `local.get -> local.set -> if -> local.get` copy temp disappears without reviving the broader structured-barrier rewrite that previously hurt parity.
- The validator-structured raw skip now also sinks single-use effectful temps across an intervening pure local-copy barrier when the barrier local is disjoint from both the temp local and the temp producer's source locals. The new heavy traced regression in `src/passes/simplify_locals_test.mbt` stays on `skip-raw reason=validator-structured-call-heavy` and proves the loop-shaped `call -> local.set -> local.get/local.set barrier -> local.get -> i32.lt_s` temp disappears while the later pure increment temp still sinks into the following `i32.store`. The focused pass fuzz lane is green again at `2000/2000` normalized matches with `0` mismatches in `/tmp/starshine-pass-fuzz-compare-2`.
- Huge straight-line tee-heavy builder initializers now skip raw too, and the traced native lane shows debug-artifact `Func 1800` on `skip-raw reason=huge-straight-line-call-builder`.
- Dense low-loop structured call-heavy helpers now skip raw too, and the traced native lane shows debug-artifact `Func 395`, `430`, `818`, `2083`, and `2098` on `skip-raw reason=dense-structured-call-heavy`. The 10k parity lane `.tmp/pass-fuzz-sl-parity-after-dense-structured-call-heavy` is green at `10000/10000` normalized matches with `0` mismatches or failures.
- The current unreduced artifact frontier still starts at absolute `Func 71`, `_M0FP37jtenner9starshine3cmd31run__cmd__with__adapter_2einner` (WAT `$50`). The fresh traced artifact replay still shows `Func 71` taking `skip-raw reason=validator-structured-call-heavy`, but the old loop-temp `$273 / $276` drift is now gone too: `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-2` now matches Binaryen's inlined `call $4504` and `i32.add` shapes there, and the first remaining diff is the Binaryen-only `nop` sentinels before that same `if` and `i32.store`, followed by the deeper `$6 / $5 / $7` call-indirect block chain later in the function. This checkpoint is correctness-clean, `moon test src/passes` is green at `485/485`, and the live compare timings are now `5159.475ms` / `2456.558ms` for Starshine vs `551.574ms` / `279.031ms` for Binaryen.
- The traced artifact replay corrected the apparent first mismatch after the cleanup-`if` fix: the earliest meaningful simplify-locals drift is no longer the old `Func 71` local-copy family. The first real body mismatches now show up later in unchanged exact-path functions such as `Func 386` and `Func 399`, where Starshine reports `pass[simplify-locals]:done changed=false` but Binaryen still prints tighter control/value forms (for example the loop/block reshuffle in `Func 386` and a narrower `if`-to-expression canonicalization inside `Func 399`). This means the next frontier is in exact-path no-op parity, not in the validator raw skip.
- A broad lowered `if (result i32) -> select` cleanup was investigated and rejected. Direct reduced Binaryen probes on `/tmp/sl-select-test1.wat` through `/tmp/sl-select-test5.wat` showed that Binaryen does **not** selectify the simple direct-result, call-argument, or local-set variants that Starshine can trivially rewrite, so do not revive the broad select cleanup without a reduced artifact-backed proof that matches Binaryen on the specific unchanged exact-path shape first.
- The next parity and performance work should stay focused on two remaining buckets instead of broader cleanup passes: the unreduced non-adjacent exact-expression `Func 71` drift, and the remaining internal branchy no-op cluster now led by `Func 473`, `308`, and `1488` after the `Func 1800`, `395`, `430`, `818`, `2083`, and `2098` families were removed from the hotspot list. The traced native `pass:simplify-locals total_us` checkpoint is now `2278863`, down from `2416268`.
