...
- [O4Z-AUDIT-MP] - Widen `memory-packing` to Binaryen behavior parity
  - Status: reopened v0.1.0 release-gating audit, partially widened on 2026-06-07. Focused tests now cover active segment-op cleanup, split-passive `memory.init`/`memory.fill`, split-passive `data.drop`, lazy drop-state globals, `__llvm*` no-split behavior, data-name repair and split suffixes, GC data-user no-split behavior, `MaxDataSegments` guards, `trapsNeverHappen` active trap elision, imported-memory `zeroFilledMemory`, constant out-of-range split-passive source traps, high-address i32 active saturating offset repair, and operand side-effect/trap preservation for lowered active/split-passive `memory.init` paths. `moon test` passed after implementation; direct `memory-packing` compare at `.tmp/pass-fuzz-memory-packing-20260607-side-effects-10000` compared `7602/10000` with `0` mismatches before the Binaryen/tool command-failure cap (`19` Binaryen empty-recursion-group, `1` Binaryen bad-section-size). Saved-slot replay remains before closing the audit.
  - Remaining scope: Binaryen option-surface details not covered by the current hot-pipeline options; the previously documented operand side-effect gap is implemented for the current constant-source/size lowering surface.
  - Deliverables: apply the common checklist; use gap audit `docs/wiki/raw/research/0715-2026-06-07-memory-packing-parity-gap-audit.md`; add source-backed option/limit/GC fixtures before further behavior changes; classify each remaining difference as implemented behavior, representation-only drift, tool-blocked, or explicit non-goal, then refresh direct compare evidence before closing.
...
- [O4Z-AUDIT-GSI] - Widen `global-struct-inference` beyond subset signoff
  - Status: reopened v0.1.0 release-gating audit. Current direct compare is green for the documented Starshine subset, but the parity page still lists broad official Binaryen behavior outside that subset.
  - Scope: open-world direct immutable-global reads, closed-world candidate-global reasoning over locals and params, subtype propagation, one/two unique-value selection, non-constant un-nesting, packed fields, atomic gets, `ref.get_desc`, sibling `global-struct-inference-desc-cast`, carrier breadth, larger decision trees, and Binaryen-style refinalization triggers.
  - Deliverables: apply the common checklist; keep deferrals narrow and evidence-backed; implement Binaryen-positive families where local representation supports them; keep sibling `gsi-desc-cast` rejected until its own real pass surface is safe.
