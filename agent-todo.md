...
1. Canonical correctness landed.
   - [RUME]003 - Oracle-stable fast-path pass completed - Direct pass execution, CLI flag and registry wiring, focused section/index rewrite coverage, explicit memarg-index rewrites, and canonical compare-harness artifact parity are in place; `remove-unused-module-elements` is now canonically correct against the oracle on the debug artifact.
     ...
     - Remaining blocker after semantic cleanup: the post-fix `wasm-smith` command-failure corpus is still coverage-only work, and the imported-table no-op `nullref` active-elem mismatch is now closed. After landing explicit shared-memory support, the memory64 validator fix, extended-const arithmetic validation, and the imported-table `nullref` elem cleanup, `.tmp/pass-fuzz-rume-nullref-rerun` reached `147/148` normalized matches with `19` Binaryen command failures and `1` later semantic mismatch before the `20`-failure cutoff. Starshine still contributes no command failures in that slice. Binaryen still accounts for the `19/20` failure slots: fourteen `invalid type index` cases (`000009`, `000021`, `000024`, `000031`, `000047`, `000050`, `000052`, `000054`, `000056`, `000079`, `000095`, `000124`, `000141`, `000143`), one `Recursion groups of size zero not supported` case (`000029`), two `invalid wasm type: -64` cases (`000081`, `000083`), and two later parser failures at `000162` and `000167`. The new semantic frontier is `.tmp/pass-fuzz-rume-nullref-rerun/failures/case-000104-wasm-smith`, where Binaryen keeps an active `nullfuncref` segment on an exported defined table while Starshine drops that default-value initializer as a no-op.
...
2. Do work.
   ...
   - The remaining post-fix work is no longer validator cleanup; it is one later semantic RUME mismatch on defined-table default-value active elem retention plus Binaryen parser-compatibility blockers.
