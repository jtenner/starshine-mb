...
- **Deliverables:**
  ...
  - [x] Run the required four-lane v131 matrix for `remove-unused-brs` (2026-07-26 baseline: regular `100000/100000` cleanup-normalized green; wasm-smith `9956` comparable with `9954` direct plus `2` cleanup-normalized and `44` Binaryen-only failures; dedicated `10000/10000` mismatches; random-all `870` residuals). Follow-up repairs match the `181` prior random parity gaps, preserve the `689` measured wins, admit safe multivalue-return roots, and make wrapper cleanup exact. The development aggregate is now `5655/10000` exact with zero failures and a `-81681`-byte residual delta; focused multivalue-drop is a uniform measured `-12` win. The dedicated typing, residual classification, old-random stability, and final four-lane renewal are complete; direct RUB behavior is closed.
