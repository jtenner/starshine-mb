...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [CP]004 - Binaryen Parity Proof And Follow-On Gaps - Close the compare loop and keep deferred oracle gaps explicit.
    ...
    - Current blocker: exact TNH / implicit-trap parity needs `HotPipelineOptions` or equivalent hot-pass policy plumbing; the GC `ref-into-if` family is follow-up work until local-type repair is representable in the current pipeline. The latest direct compare is [`.tmp/self-opt-code-pushing-inner-owner-v2b`](./.tmp/self-opt-code-pushing-inner-owner-v2b), where the older `61077` family is gone and the current first normalized WAT deltas are back at `48978`, `72005`, `105621`, and `126757`, with the pass still far behind Binaryen (`960.390 ms` vs `52.076 ms`). The current first live family is still the traced printed `func $127` / pipeline `Func 148` nested dropped-carrier shape: top-level root `Drop#1987` owns `Block#1429`, whose body contains `Drop#1782`, whose child `Block#1659` contains `Block#1753`, whose body roots include `LocalSet(310)#1776`. The later `LocalSet(335)#2738` and `If#2747` are sibling roots outside that dropped carrier. In lifted HOT, `LocalSet(310)#1776` is not the pure `local.get`-fed root seen in the lowered WAT; it wraps `Call#1775`, so `cp_root_pushable_local` rejects it by design. That means the first remaining parity gap is still a root-splitting / carrier-extraction problem rather than another traversal-gate miss. The pass now admits three exact owner-carrier subfamilies: the simple owner-only carried-`if` case with no nested explicit exits, the direct inner-owner payload-exit case, and the live-like direct inner-owner case with an unrelated trailing `local.get`. `src/passes/code_pushing_test.mbt` and `src/ir/hot_lower_live_repro_test.mbt` both cover those new positive reducers. A fresh live probe against `If#2747` was still useful, though: the guard stopped at `offending_root=304`, and once the payload `If#219` was inspected more closely it turned out to carry two features at once, both a branch back to the inner block owner `213` and a terminal exit. The direct inner-owner widening is keepable and is now in-tree; the terminal-plus-inner-owner variant is also reduced and HOT-valid, but it is intentionally still fenced off in `src/passes/code_pushing.mbt` because widening that too removed the `48978` family only by introducing a worse earlier structural mismatch around printed line `44251`. A fresh retry confirmed that even narrowing that terminal-owner experiment to `i32` carriers still reintroduced the same earlier drift at `44254`, so the pass stays on the conservative fence and `src/passes/code_pushing_test.mbt` now pins the direct inner-owner-plus-terminal carrier as an explicit negative boundary. A follow-up live trace narrowed that further: the earlier `44254` drift is still downstream fallout from the same `Func 148` move, not a second earlier rewrite. Under the widened pass, roots `LocalSet(335)#2738`, `LocalSet(365)#2741`, `If#2747` become `LocalSet(365)#2741`, `If#2747`, `LocalSet(335)#2738`. `src/passes/code_pushing_test.mbt` and `src/ir/hot_lower_live_repro_test.mbt` now pin that closer terminal-owner shape directly too, with the intervening condition `LocalSet` preserved before the later `If`: manual reorder still lowers and validates, but the pass intentionally keeps it fenced off. Since the earlier replay, the pass also descends through `Drop`-wrapped control roots, and `src/passes/code_pushing_test.mbt` still has both the direct dropped-owner traversal regression and a closer live-like dropped-owner predecessor-chain regression that preserves the owner-self branch paths, the later root-level `br`, and the intervening call barrier from the first `19348` family. `src/ir/hot_lower_live_repro_test.mbt` covers the same reordered live-like shape and proves it still lowers and validates, and native `--code-pushing --out .tmp/code-pushing-validate.wasm tests/node/dist/starshine-debug-wasi.wasm` is still valid after the corresponding pass changes. The named small compare lane is green again too: after narrowing dead-set motion to the actual crossed gap and allowing local-only gap roots while still blocking nonlocal effects, both saved dead-set parity repros are covered in `src/passes/code_pushing_test.mbt`, and `bun scripts/pass-fuzz-compare.ts --pass code-pushing --count 100 --generator gen-valid --min-compared 50 --max-failures 5 --out-dir .tmp/pass-fuzz-code-pushing-post-arity` now finishes `100/100` compared with `100` normalized matches, `0` validation failures, `0` command failures, and `0` mismatches. The newest exact positive reducers now include the earlier explicit-exit `LocalSet(If ...)` carrier with nested local writes, the earlier explicit-exit `If` root before a later `if`, and the direct inner-owner owner-carrier variants. The newest HOT-only positive reducers now include both terminal-owner variants: the direct inner-owner plus terminal-exit carrier, and the closer live-like form where an unrelated condition `LocalSet` still stays before the later `If`. The newest exact negative reducer is still an earlier branch-root predecessor with one intervening used `LocalSet` and a branch-free crossed gap before the later `if`: `src/ir/hot_lower_live_repro_test.mbt` proves that manual reorder still lowers and validates, and the matching pass rewrite had to be rolled back immediately because the real debug artifact reintroduced the same `Func 1977` stack underflow. That same crossed-gap non-repro now survives the next closer wrappers too: the parent-escape carrier, extracted parent-escape carrier, closer real carrier, and extracted real carrier variants all still lower and validate. But the failing boundary moved earlier again: a reordered real crossed-gap carrier already hits `InvalidBranchArity(_, _, 0, 1)` once the parent `err` block becomes result-producing, and the smaller parent-escape crossed-gap carrier does too. So the crossed-gap tail itself is no longer the distinguishing unsafe ingredient; the remaining blocker sits farther upstream in the rewrite step that makes the parent `err` block result-producing at all. On the performance side, the newer whole-second cuts are now coming from late-scan caching rather than broad pass setup. Reusing cached `HotEffectsSummary.node_masks`, allocating `CpSummaryCache` summaries lazily instead of eagerly, precomputing dropped-owner/global scans once per function, and carrying the reachable-prefix start index directly moved the direct native debug-artifact path from the `7.100s` band down into the `4.7s` to `5.1s` band, with traced local runs around `analysis:effects ~= 0.25s`, `pass:code-pushing ~= 2.44s-2.62s`, and `lift ~= 1.32s-1.49s`. The next whole-second cut then came from a stronger traversal disqualifier: `code-pushing` now rejects functions and nested regions unless they contain an SFA `LocalSet` root with a cheap movable value before a later `If`/pushpoint root, memoizes that by region holder, and reuses the same region cache during the actual rewrite walk. That pushed the same direct native debug-artifact path down again from `5.072s` to `4.205s` and then `3.604s`, while traced `pass:code-pushing` dropped from `2469578 us` to `1731189 us` and then `1290492 us`. The old unchanged parser outliers mostly disappeared: `Func 3617` fell from `337161 us` to `574 us`, `Func 1558` from `254285 us` to `410 us`, and `Func 1530` from `197051 us` to `242 us`. The next perf tranche stayed in the late rewrite scans themselves: terminal unrelated pushpoints are now ignored during traversal gating, `code-pushing` caches the last root index that reads each local per region, dead-set motion carries an incremental crossed-gap summary instead of rescanning the suffix for every candidate, and both trivial dead-gap roots plus pushable `LocalSet` roots are memoized within an iteration. That pushed the same direct native debug-artifact path down again from `3.604s` to `3.160s-3.162s`, while the latest traced run `/tmp/code-pushing-pass-trace10.log` has `pass:code-pushing = 1095736 us`, `analysis:effects = 242494 us`, `lift = 1269233 us`, and unchanged pass time down to `809435 us`. The next perf slice then stayed in unchanged-region root traffic: `code-pushing` now reads valid region roots through a pass-local precomputed view instead of repeated `hot_region_root_count(...)` / `hot_region_get(...)` calls, rebuilds moved roots sparsely instead of allocating root-sized `movable` / `move_after_target` arrays for every pushpoint scan, and reuses that same view across same-region sink and push rewrites. The latest traced native replay `/tmp/code-pushing-trace-pass3.log` now has `pass:code-pushing = 978556 us`, `analysis:effects = 249694 us`, `lift = 1274934 us`, and `lower = 60600 us`, while direct native `--code-pushing` runs on `tests/node/dist/starshine-debug-wasi.wasm` are now in the `2.996s-3.176s` band. The hotspot mix narrowed again: mutating `Func 1816` dropped to `149339 us`, while the largest unchanged outliers are now `Func 2609` (`30802 us`), `Func 1910` (`27009 us`), `Func 1678` (`25576 us`), and `Func 2010` (`19791 us`). So the next performance decision is now even tighter: either attack the 19 rewrite iterations inside `Func 1816`, or stop shaving local pass cost and spend the next budget on the permission-gated full compare replay. The broader zero-result-prefix relaxation still does not hold on the real artifact: ignoring earlier zero-result branch roots reintroduced the same `Func 1977` stack underflow on `tests/node/dist/starshine-debug-wasi.wasm`, and `src/ir/hot_lower_live_repro_test.mbt` still records why that remains a blocker instead of a fix across several closer reducers. The reduced earlier-zero-result-branch prefix, the same prefix inside an outer result carrier, the simplified extracted parent-escape carrier, the closer reordered real parent-escape carrier, and even the closer real carrier after parent-err extraction plus carried-`if` demotion all still lower and validate. That last closer shape still hits the same `InvalidBranchArity(_, _, 0, 1)` cliff immediately once the parent `err` block becomes result-producing, so the real artifact still needs a richer outer rewrite step than the local carrier rewrite alone. The older correctness story still stands too: earlier attempts such as `trailing-payload-split` and `nested-only` also reintroduced `Func 1977`, traced root-order dumps still show one concrete bad move under the root-only relaxation (`LocalSet(45)#525` shifting from before `LocalSet(50)#528` / `If#534` to after `If#534`), and the remaining unsafe family still needs extra real-function scaffolding beyond the carrier extraction plus inner carried-`if` rewrite alone. The validator offset is still pinned at `br 3` to label `@8` in `Func 1977`, `src/ir/hot_lower_live_repro_test.mbt` still proves the underlying `InvalidBranchArity(_, _, 0, 1)` mechanism, and `src/passes/code_pushing_test.mbt` still guards the parent-exit payload-carry and parent-escape carrier boundaries directly at the pass layer.
      ...
      - The latest direct compare is now [`.tmp/self-opt-code-pushing-20260408c`](./.tmp/self-opt-code-pushing-20260408c). Canonical wasm and normalized WAT still differ, the first normalized WAT deltas are still `48981`, `72005`, `105621`, and `126757`, and Starshine remains over Binaryen on both whole-pass and whole-command time (`1129.958 ms` vs `56.281 ms` pass; `3979.064 ms` vs `430.897 ms` total). The first remaining artifact family is still the printed `func $127` / pipeline `Func 148` nested dropped-carrier shape around `LocalSet $310`. The larger structural families are now clearer too: the `105621` and `126757` diffs in printed `func $314` and `func $366` are not just missed same-root reorders. Binaryen is materializing extra alias locals before the shared `if`, increasing local count and then forwarding those aliases after the `if`. If that is the real intended frontier, exact parity there may need local synthesis or a neighboring pass, not another guard-only widening inside current `code-pushing`.
      ...
      - The first explicit `48981` extraction boundary is now in-tree too. `src/passes/code_pushing_test.mbt` keeps a smaller dropped-carrier alias case unchanged, where `LocalSet(2)` still lives inside the dropped carrier and the later `if` sits outside it; `src/ir/hot_lower_live_repro_test.mbt` proves that manually extracting that alias `local.set` across the carrier boundary still lowers and validates. So the next real CP004 step is no longer “find another same-region sink.” It is “design a deliberate dropped-carrier extraction rewrite,” because that cross-region move is the first concrete mechanism now pinned for the remaining `48981` family.
