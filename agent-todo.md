...
- [FUZ]1021 - Invalid Binary Malformed-Byte Matrix
  - Status: IN PROGRESS (cron 23, 2026-05-21). Latest `array.new_elem` GC instruction-index ULEB slice landed `malformed-array-new-elem-type-index-uleb`, `malformed-array-new-elem-elem-index-uleb`, `overwide-array-new-elem-type-index-uleb`, and `overwide-array-new-elem-elem-index-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep `array.new_elem` instruction frames well-formed while corrupting type/element index ULEB payloads with malformed or over-budget encodings, with registry entries, focused replay coverage, LEB/instruction/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-array-new-elem-type-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous overwide exception/GC instruction-index ULEB slice landed `overwide-throw-tag-index-uleb`, `overwide-try-table-catch-count-uleb`, `overwide-try-table-catch-tag-index-uleb`, `overwide-try-table-catch-label-index-uleb`, `overwide-struct-new-type-index-uleb`, `overwide-struct-get-field-index-uleb`, `overwide-array-new-data-type-index-uleb`, and `overwide-array-new-data-index-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep exception and GC instruction frames well-formed while corrupting tag/count/label/type/field/data index ULEB payloads beyond the `u32` byte budget, with registry entries, focused replay coverage, LEB/instruction/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-throw-tag-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous overwide bulk-index ULEB slice landed `overwide-data-index-uleb`, `overwide-elem-index-uleb`, `overwide-memory-init-data-index-uleb`, `overwide-memory-init-memory-index-uleb`, `overwide-table-init-elem-index-uleb`, `overwide-table-init-table-index-uleb`, `overwide-memory-copy-dst-index-uleb`, `overwide-memory-copy-src-index-uleb`, `overwide-table-copy-dst-index-uleb`, and `overwide-table-copy-src-index-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep bulk `0xfc` instruction frames well-formed while corrupting data/element/memory/table index ULEB payloads beyond the `u32` byte budget, with registry entries, focused replay coverage, LEB/instruction/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-data-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon test`, and `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`. Previous typed-select overwide-ULEB slice landed `overwide-select-type-count-uleb` as a decode-rejected, non-smoke binary-invalid strategy that keeps the typed `select` instruction frame well-formed while corrupting the value-type vector count beyond the `u32` byte budget, with registry entry, focused replay coverage, LEB/instruction/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-select-type-count-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous branch overwide-ULEB slice landed `overwide-br-label-index-uleb`, `overwide-br-if-label-index-uleb`, `overwide-br-table-target-count-uleb`, `overwide-br-table-target-label-index-uleb`, and `overwide-br-table-default-label-index-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep branch instruction frames well-formed while corrupting label/count ULEB payloads beyond the `u32` byte budget, with registry entries, focused replay coverage, LEB/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-br-label-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous const signed-LEB slice landed `overwide-i32-const-sleb` and `overwide-i64-const-sleb` as decode-rejected, non-smoke binary-invalid strategies that keep minimal const instruction frames well-formed while corrupting signed LEB payloads beyond the allowed byte budgets, with registry entries, focused replay coverage, LEB/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-i32-const-sleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous ref-heaptype overwide-LEB slice landed `overwide-ref-null-heaptype-immediate`, `overwide-ref-cast-heaptype-immediate`, and `overwide-ref-test-heaptype-immediate` as decode-rejected, non-smoke binary-invalid strategies that keep reference instruction frames well-formed while corrupting heaptype immediates with over-budget signed-33-bit LEB payloads, with registry entries, focused replay coverage, LEB/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-ref-null-heaptype-immediate`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous blocktype overwide-ULEB slice landed `overwide-blocktype-type-index-uleb` as a decode-rejected, non-smoke binary-invalid strategy that keeps a block instruction frame well-formed while corrupting the signed-33-bit blocktype type-index payload with an over-budget LEB, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-blocktype-type-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous memarg overwide-ULEB slice landed `overwide-load-memarg-align-uleb`, `overwide-store-memarg-align-uleb`, `overwide-load-memarg-offset-uleb`, and `overwide-store-memarg-offset-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep load/store instruction frames well-formed while corrupting memarg align/offset payload ULEBs with an over-budget unsigned LEB, with registry entries, focused replay coverage, LEB/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-load-memarg-align-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous memory/table limit overwide-ULEB slice landed `overwide-memory-min-uleb`, `overwide-table-min-uleb`, `overwide-memory-max-uleb`, `overwide-table-max-uleb`, `overwide-import-memory-min-uleb`, `overwide-import-table-min-uleb`, `overwide-import-memory-max-uleb`, and `overwide-import-table-max-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep memory/table section or import descriptor frames well-formed while corrupting limit payload ULEBs with an over-budget unsigned LEB, with registry entries, focused replay coverage, LEB/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-memory-min-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous singleton section payload overwide-ULEB slice landed `overwide-start-func-index-uleb` and `overwide-datacount-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep start/data-count section frames well-formed while corrupting the singleton payload ULEB with a six-byte unsigned LEB that exceeds the `u32` byte budget, with registry entries, focused replay coverage, LEB/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-start-func-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous local/global/call-family instruction index overwide-ULEB slice landed `overwide-local-index-uleb`, `overwide-local-set-index-uleb`, `overwide-local-tee-index-uleb`, `overwide-global-index-uleb`, `overwide-global-set-index-uleb`, `overwide-call-func-index-uleb`, `overwide-return-call-func-index-uleb`, `overwide-ref-func-index-uleb`, `overwide-call-indirect-type-index-uleb`, `overwide-call-indirect-table-index-uleb`, `overwide-return-call-indirect-type-index-uleb`, `overwide-return-call-indirect-table-index-uleb`, `overwide-call-ref-type-index-uleb`, and `overwide-return-call-ref-type-index-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep local/global/call-family instruction opcodes and preceding immediates well-formed while corrupting local/global/function/type/table-index payloads with a six-byte unsigned LEB that exceeds the `u32` byte budget, with registry entries, focused replay coverage, LEB/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-call-func-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Latest section/import/tag type-index overwide-ULEB slice landed `overwide-function-section-type-index-uleb`, `overwide-import-func-type-index-uleb`, `overwide-import-tag-type-index-uleb`, and `overwide-tag-type-index-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep function/import/tag section frames and descriptor kind/attribute bytes well-formed while corrupting type-index payloads with a six-byte unsigned LEB that exceeds the `u32` byte budget, with registry entries, focused replay coverage, LEB/function-import-export/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-function-section-type-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous export descriptor index overwide-ULEB slice landed `overwide-export-func-index-uleb`, `overwide-export-table-index-uleb`, `overwide-export-memory-index-uleb`, `overwide-export-global-index-uleb`, and `overwide-export-tag-index-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep the export section frame and descriptor kind byte well-formed while corrupting the descriptor index payload with a six-byte unsigned LEB that exceeds the `u32` byte budget, with registry entries, focused replay coverage, LEB/function-import-export/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-export-func-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous import/export string-length overwide-ULEB slice landed `overwide-export-name-length-uleb`, `overwide-import-module-name-length-uleb`, and `overwide-import-field-name-length-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep the export/import section frames well-formed while corrupting the nested export/module/field string-length payloads with six-byte unsigned LEBs that exceed the `u32` byte budget, with registry entries, focused replay coverage, LEB/fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-export-name-length-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous name-section string-length overwide-ULEB slice landed `overwide-name-section-func-name-length-uleb`, `overwide-name-section-local-name-length-uleb`, `overwide-name-section-module-name-length-uleb`, `overwide-name-section-type-name-length-uleb`, `overwide-name-section-table-name-length-uleb`, `overwide-name-section-memory-name-length-uleb`, `overwide-name-section-global-name-length-uleb`, `overwide-name-section-elem-name-length-uleb`, `overwide-name-section-data-name-length-uleb`, `overwide-name-section-field-name-length-uleb`, `overwide-name-section-label-name-length-uleb`, and `overwide-name-section-tag-name-length-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep the custom-section and name subsection frames well-formed while corrupting nested name string-length payloads with six-byte unsigned LEBs that exceed the `u32` byte budget, with registry entries, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-name-section-func-name-length-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous code local count overwide-ULEB slice landed `overwide-code-local-decl-count-uleb` and `overwide-code-local-run-count-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep the code section, function body frame, and local-declaration framing well-formed while corrupting the body-local declaration vector count or local-run count payload with a six-byte unsigned LEB that exceeds the `u32` byte budget, with registry entries, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-code-local-decl-count-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous type-signature count overwide-ULEB slice landed `overwide-functype-param-count-uleb` and `overwide-functype-result-count-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep the type-section and function-type frames well-formed while corrupting the signature parameter/result vector-count payload with a six-byte unsigned LEB that exceeds the `u32` byte budget, with registry entries, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `overwide-functype-param-count-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous type-signature count malformed-ULEB slice landed `malformed-functype-param-count-uleb` and `malformed-functype-result-count-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep the type-section and function-type frames well-formed while corrupting the signature parameter/result vector-count payload with an unterminated unsigned LEB, with registry entries, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-functype-param-count-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous type-signature value-type byte slice landed `invalid-functype-param-valtype-byte` and `invalid-functype-result-valtype-byte` as decode-rejected, non-smoke binary-invalid strategies that keep the type-section and function-type frames well-formed while corrupting one parameter or result value-type byte, with registry entries, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `invalid-functype-param-valtype-byte`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous export/import name-length malformed-ULEB slice landed `malformed-export-name-length-uleb`, `malformed-import-module-name-length-uleb`, and `malformed-import-field-name-length-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep export/import section frames well-formed while corrupting the nested string-length payload with an unterminated unsigned LEB, with registry entries, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-export-name-length-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous label-name malformed-ULEB slice landed `malformed-name-section-label-name-length-uleb` as a decode-rejected, non-smoke binary-invalid strategy that keeps the custom-section and label-name subsection frames well-formed while corrupting the nested label-name string-length payload with an unterminated unsigned LEB, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-name-section-label-name-length-uleb`, then passed after implementation. Final validation for that slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous name-section name-map length malformed-ULEB slice landed `malformed-name-section-table-name-length-uleb`, `malformed-name-section-memory-name-length-uleb`, `malformed-name-section-global-name-length-uleb`, `malformed-name-section-elem-name-length-uleb`, `malformed-name-section-data-name-length-uleb`, `malformed-name-section-field-name-length-uleb`, and `malformed-name-section-tag-name-length-uleb` as decode-rejected, non-smoke binary-invalid strategies that keep the custom-section and respective name-map subsection frames well-formed while corrupting the stored name string-length payload with an unterminated unsigned LEB, with registry entries, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-name-section-table-name-length-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous name-section type-name length malformed-ULEB slice landed `malformed-name-section-type-name-length-uleb` as a decode-rejected, non-smoke binary-invalid strategy that keeps the custom-section and type-name map subsection frames well-formed while corrupting the type-name string-length payload with an unterminated unsigned LEB, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-name-section-type-name-length-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous name-section module-name length malformed-ULEB slice landed `malformed-name-section-module-name-length-uleb` as a decode-rejected, non-smoke binary-invalid strategy that keeps the custom-section and module-name-subsection frames well-formed while corrupting the module-name string-length payload with an unterminated unsigned LEB, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-name-section-module-name-length-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous name-section local-name length malformed-ULEB slice landed `malformed-name-section-local-name-length-uleb` as a decode-rejected, non-smoke binary-invalid strategy that keeps the custom-section and local-name-subsection frames well-formed while corrupting the local-name string-length payload with an unterminated unsigned LEB, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-name-section-local-name-length-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous name-section function-name length malformed-ULEB slice landed `malformed-name-section-func-name-length-uleb` as a decode-rejected, non-smoke binary-invalid strategy that keeps the custom-section and function-name-subsection frames well-formed while corrupting the function-name string-length payload with an unterminated unsigned LEB, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-name-section-func-name-length-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous element segment function-index malformed-ULEB slice landed `malformed-element-segment-func-index-uleb` as a decode-rejected, non-smoke binary-invalid strategy that keeps the element-section frame, active segment flags, offset expression, and function-index vector count well-formed while corrupting the segment function-index payload with an unterminated unsigned LEB, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-element-segment-func-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous data segment byte-count malformed-ULEB slice landed `malformed-data-segment-byte-count-uleb` as a decode-rejected, non-smoke binary-invalid strategy that keeps the data-section frame and passive segment flags well-formed while corrupting the data byte-vector count with an unterminated unsigned LEB, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-data-segment-byte-count-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous code local-run-count malformed-ULEB slice landed `malformed-code-local-run-count-uleb` as a decode-rejected, non-smoke binary-invalid strategy that keeps the code section, function body frame, and local-declaration vector count well-formed while corrupting the local run-count with an unterminated unsigned LEB, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-code-local-run-count-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous code local-decl-count malformed-ULEB slice landed `malformed-code-local-decl-count-uleb` as a decode-rejected, non-smoke binary-invalid strategy that keeps the code section and function body frames well-formed while corrupting the body-local declaration vector count with an unterminated unsigned LEB, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-code-local-decl-count-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous code body-size malformed-ULEB slice landed `malformed-code-body-size-uleb` as a decode-rejected, non-smoke binary-invalid strategy that keeps the code-section payload frame well-formed while corrupting the per-function body-size field with an unterminated unsigned LEB, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-code-body-size-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous code-section payload-size slice landed `code-section-size-overflow` as a decode-rejected, non-smoke binary-invalid strategy that declares a code-section payload length one byte larger than the available section bytes while keeping the enclosed function body complete, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `code-section-size-overflow`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous function-section type-index byte slice landed `malformed-function-section-type-index-uleb` as a decode-rejected, non-smoke binary-invalid strategy that corrupts a function-section type-index payload with an unterminated unsigned LEB while keeping the existing import descriptor and tag-section type-index carriers distinct, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-function-section-type-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous import-tag descriptor byte slice landed `invalid-import-tag-attribute-byte` as a decode-rejected, non-smoke binary-invalid strategy that corrupts an imported tag descriptor's attribute byte while keeping the existing tag-section `invalid-tag-attribute-byte` carrier distinct, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `invalid-import-tag-attribute-byte`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous GC instruction-index slice landed `malformed-struct-new-type-index-uleb`, `malformed-struct-get-field-index-uleb`, `malformed-array-new-data-type-index-uleb`, and `malformed-array-new-data-index-uleb` as decode-rejected, non-smoke binary-invalid strategies that corrupt `struct.new`, `struct.get`, and `array.new_data` type/field/data-index immediates with unterminated unsigned LEB payloads, with registry entries, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-struct-new-type-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous exception-immediate slice landed `malformed-throw-tag-index-uleb`, `malformed-try-table-catch-count-uleb`, `malformed-try-table-catch-tag-index-uleb`, and `malformed-try-table-catch-label-index-uleb` as decode-rejected, non-smoke binary-invalid strategies that corrupt `throw` tag-index and `try_table` catch count/tag-index/label-index immediates with unterminated unsigned LEB payloads, with registry entries, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-throw-tag-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous tail/ref call index slice landed `malformed-return-call-indirect-type-index-uleb`, `malformed-return-call-indirect-table-index-uleb`, `malformed-call-ref-type-index-uleb`, and `malformed-return-call-ref-type-index-uleb` as decode-rejected, non-smoke binary-invalid strategies that corrupt the `return_call_indirect`, `call_ref`, and `return_call_ref` type/table-index immediates with unterminated unsigned LEB payloads, with registry entries, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-return-call-indirect-type-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous `memory.init` first-immediate slice landed `malformed-memory-init-data-index-uleb` as a decode-rejected, non-smoke binary-invalid strategy that corrupts the data-segment index immediate after the `0xfc 0x08` subopcode with an unterminated unsigned LEB payload, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-memory-init-data-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. Previous `table.init` first-immediate slice landed `malformed-table-init-elem-index-uleb` as a decode-rejected, non-smoke binary-invalid strategy that corrupts the element-segment index immediate after the `0xfc 0x0c` subopcode with an unterminated unsigned LEB payload, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-table-init-elem-index-uleb`, then passed after implementation. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. The previous prefixed table-index instruction slice landed `malformed-table-size-index-uleb`, `malformed-table-grow-index-uleb`, and `malformed-table-fill-index-uleb` as decode-rejected, non-smoke binary-invalid strategies that corrupt the `table.size`, `table.grow`, and `table.fill` table-index immediates after their `0xfc` subopcodes with unterminated unsigned LEB payloads. The previous table-index instruction slice landed `malformed-table-set-index-uleb` as a decode-rejected, non-smoke binary-invalid strategy that corrupts the `table.set` table-index immediate with an unterminated unsigned LEB, with registry entry, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. Earlier TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-table-set-index-uleb`, then passed after implementation. Earlier final validation for that slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`, and `moon test`. The previous section/import descriptor maximum-limit slice landed `malformed-memory-max-uleb`, `malformed-table-max-uleb`, `malformed-import-memory-max-uleb`, and `malformed-import-table-max-uleb` as decode-rejected, non-smoke binary-invalid strategies that corrupt malformed maximum-limit ULEB payload bytes, with registry entries, focused replay coverage, fuzz docs, wiki log update, and public enum refresh. Earlier TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on missing `malformed-import-memory-max-uleb` / `malformed-memory-max-uleb`, then passed after implementation. The previous import descriptor limits slice landed `malformed-import-memory-min-uleb` and `malformed-import-table-min-uleb` as decode-rejected, non-smoke binary-invalid strategies that corrupt memory/table import minimum-limit ULEB payload bytes. The previous import descriptor byte slice landed `invalid-import-global-valtype-byte` and `invalid-import-global-mutability-byte` as decode-rejected, non-smoke binary-invalid strategies that corrupt global-import value-type and mutability payload bytes. An earlier import descriptor slice landed `invalid-import-table-reftype-byte` and `invalid-import-memory-limits-flag-byte` for table-import reftype and memory-import limits-flag payload bytes. Earlier slices also landed `invalid-typed-select-valtype-byte`, `malformed-memory-size-memory-index-uleb` / `malformed-memory-grow-memory-index-uleb`, active-segment `malformed-data-segment-memory-index-uleb` / `malformed-element-segment-table-index-uleb`, `malformed-br-table-target-label-index-uleb`, `malformed-ref-test-heaptype-immediate`, `malformed-ref-cast-heaptype-immediate`, section-limit LEB coverage through `malformed-memory-min-uleb`, `malformed-table-min-uleb`, `malformed-memory-max-uleb`, and `malformed-table-max-uleb`, SIMD lane-overflow coverage through `simd-extract-lane-index-overflow`, the bulk-init secondary-index LEB strategies, import descriptor type-index LEB strategies, export descriptor index-LEB strategies, bulk-copy index-LEB strategies, `invalid-element-kind-byte` for passive/declarative function-index elemkind corruption, and duplicate known-section decode rejection for import/export/table/memory/global/function/code/element/data/tag sections. Earlier final validation for the maximum-limit slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon test`, and `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`. Remaining blocker: the task is intentionally broader than this slice; next run should continue with another malformed-byte family such as malformed/overwide LEB carriers in more contexts, more UTF-8/name payload corruption, additional invalid heap/ref encodings, shape-specific binary-origin SIMD lane validation beyond the current coarse decode-overflow fixtures, or additional section descriptor bytes without weakening stage-aware accounting.
