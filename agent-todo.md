...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [DCE]003 - Runtime Budget and Oracle Refresh - Close the remaining DCE speed gap and refresh parity evidence on a valid baseline/oracle path.
     ...
     - Current blocker: the old baseline-validation note is stale. `moon run src/cmd -- --dead-code-elimination --out /tmp/dce-self-opt-out.wasm tests/node/dist/starshine-debug-wasi.wasm` now exits `0`, so the checked-in artifact itself is a valid baseline for DCE replay. The current blocker is native-release divergence: `_build/native/release/build/cmd/cmd.exe --dead-code-elimination --out /tmp/dce-self-opt-release-out.wasm tests/node/dist/starshine-debug-wasi.wasm` fails final validation with `stack underflow` in `(Func 1730)`, which is the same failure now tripping `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --dead-code-elimination` before Binaryen runs. A broader native-release check is also red: `moon test --target native --release --package jtenner/starshine/cmd --file cmd_test.mbt` currently exits with `SIGSEGV` from `cmd.blackbox_test.exe`. The CLI now dumps native final-validate failures to `/tmp/starshine-invalid-final.wasm` and `/tmp/starshine-invalid-final-func.txt`, and a focused native child-process test locks that dump path in for smaller invalid modules. The repo-side correctness gate is green in source-mode and fuzz-mode; the remaining blocker is reducing and fixing the native DCE / lowering divergence before meaningful runtime or ordered-prefix parity numbers can be trusted.
