...
- [FUZ]1012 - Randomized Coverage-Forced Templates
  ...
  - Current evidence: `moon test src/validate` passes with regressions proving coverage-forced template signatures are deterministic for the same seed, differ across selected seeds, preserve numeric/control floors, and now include both memory-before-table and table-before-memory forced prelude layouts across a bounded seed window. Docs note the seed-deterministic branch/loop and memory/table ordering variation.
  - Remaining: randomize additional forced families beyond the branch/loop and memory/table templates (for example ref/call/SIMD template variants), add explicit template-family diagnostics or manifest-visible facts if the public ledger needs them, then rerun `moon test src/fuzz` / relevant fuzz smoke before closing.
