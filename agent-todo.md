...
- [FUZ]1021 - Invalid Binary Malformed-Byte Matrix
  - Status: IN PROGRESS (cron 23, 2026-05-21). Latest section-frame slice adds decode-rejected, non-smoke `section-payload-length-underflow`, which shrinks a real section payload-size field while leaving the trailing bytes in place; docs now name both generic section payload length overflow and underflow. Previous atomic-prefixed subopcode, overwide section vector-count, malformed vector-count, and smoke-scheduler slices are landed and documented; continue FUZ1021 by selecting another malformed-byte class such as remaining invalid UTF-8, opcode/prefix, lane, memarg, or atomics operand-context corruptions. Current TDD evidence: focused invalid-binary wbtest first failed on missing `section-payload-length-underflow`, then passed after implementation.
