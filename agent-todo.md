...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [RUB]002 - Multi-Slot Cleanup and Artifact Compare - Replay the pass in each Binaryen slot and confirm later `merge-blocks` opportunities match the reference output.
     ...
     - Current blocker: the top-level scheduler gap is now fixed: `optimize` and `shrink` both replay `remove-unused-brs` in three modeled slots, focused preset tests cover those trace counts, and the old `Func 24`, `Func 299`, `Func 1329`, and `Func 7756` serial verifier failures are fixed in-source. `Func 7756` was a multi-value branch-payload rewrite bug: RUB now refuses to collapse a branch-payload `if` when the surviving payload side would be forwarded as fewer branch slots than the original `br` carried. The old single-pass artifact gate at `Func 807` is no longer red either: a native `cmd` regression now replays `--remove-unused-brs` on `tests/node/dist/starshine-debug-wasi.wasm` through `run_cmd_with_adapter` and validates the output module in-memory. The first two saved oracle families are fixed too: Binaryen-style explicit `nop` bodies in trivial `if` arms are preserved in-source, and lone explicit root `nop`s are preserved only when they were the whole body instead of leaving a stray `nop` behind after multi-root trimming. The newer then-arm dropped-wrapper family is fixed too: RUB now removes dropped single-result wrappers after lowering carried guards, and the focused repro is locked in `src/passes/remove_unused_brs_test.mbt`. The newer prefixed one-arm payload suffix family is fixed too: when a result block has a side-effectful prefix root before a one-arm payload branch, RUB now lowers that narrow suffix shape to `drop(br_if ...)`, matching Binaryen on the focused standalone repro instead of leaving the `if` behind. The carried-suffix wrapper family compiled through Binaryen’s parser is fixed too: RUB now treats an enclosing suffix-region branch exit as a valid carried tail, and a native `cmd` regression compiles that fixture through `wasm-opt` before normalizing Starshine’s output with `wasm-opt -S --strip-debug`. The smaller self-target if-arm family is fixed too: a new block-root rewrite now sinks side-effectful self-target block branches into arm-local wrappers instead of leaving `(then ... (br $done))` behind, matching Binaryen on the reduced standalone repro and locking that behavior in with a new focused pass regression. The raw structured-return-ladder false-positive is fixed too: the pass manager no longer skips ladders that still contain prefix-guard payload/result-block carriers needing the hot rewrite, and fresh helper trace now shows `Func 252` lifting and rewriting that artifact family instead of bailing out as `structured-return-ladder-noop`. Fresh `gen-valid` oracle evidence is still green at `.tmp/pass-fuzz-rub-genvalid-100-after-root-nop-count-fix` (`100/100`), `.tmp/pass-fuzz-rub-genvalid-1000-rerun` (`1000/1000`), and `.tmp/pass-fuzz-rub-genvalid-10000` (`10000/10000` compared, `10000/10000` normalized matches, `0` mismatches, `0` validation failures, `0` generator failures, `0` command failures) under the `--min-compared 10000` guard, and the newer mixed-generator samples `.tmp/pass-fuzz-rub-both-progress-20260409`, `.tmp/pass-fuzz-rub-both-progress-20260409-b`, `.tmp/pass-fuzz-rub-both-progress-20260409-c`, and `/tmp/pass-fuzz-rub-progress-20260409c` all stayed clean on compared cases (`1793/1793`, `0` mismatches, `0` validation failures) before hitting `100` Binaryen-side command failures. The full self-opt blocker is now clearer even though the artifact still does not move: `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --remove-unused-brs` still first diverges on the early `Func 50` carried-wrapper family where Binaryen keeps nested branch-carry result blocks and Starshine still lowers to the older unreachable-wrapped shape, and a later loop/block-order mismatch remains around `Func 381`.
     ...
     - Next oracle step: reduce the remaining early `Func 50` carried-wrapper mismatch first, then the later `Func 381` loop/block-order mismatch, and rerun self-opt compare plus a larger mixed-generator compare-pass lane before spending more budget on ordered-prefix proof.
     - Remaining work:
       - Reduce the early `Func 50` artifact mismatch to a focused standalone or native `cmd` regression that preserves the exact nested branch-carry result-block shape Binaryen keeps.
       - Audit the later `Func 381` loop/block-order mismatch separately so RUB does not paper over a lift/lower canonicalization difference with an unrelated branch rewrite.
     ...
     - Risks:
       - The debug-artifact diff still contains functions where RUB reports `changed=false`, so some remaining normalized-WAT differences may be lift/lower round-trip noise rather than pass logic; reductions must confirm pass mutation before widening any rewrite.
       - The new self-target arm rewrite already showed that broad block-root rewrites can regress older block-local chain flattening cases; future work on `Func 50` must preserve the empty-arm and simple-chain families that Binaryen still flattens through `br_if`.
       - Performance is still materially over budget on self-opt compare (`742.463 ms` Starshine pass time vs `98.176 ms` Binaryen on the latest run), so any broader carrier-family rewrite needs a narrow guard and a measured payoff.
     - Doc: [0066#L188](/home/jtenner/Projects/starshine-mb/docs/wiki/raw/research/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L188)
