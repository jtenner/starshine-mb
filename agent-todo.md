...
- [O4Z-AUDIT-SL] - Deep audit `simplify-locals`
  ...
  - Current finding: direct 2026-06-04 quick compare was semantically green with the built native binary (`998` normalized matches, `2` Binaryen/tool `binaryen-rec-group-zero` command failures, `0` mismatches); the first checklist-path attempt failed with `ENOENT` because this workspace built the binary under `_build/native/release/build/cmd/cmd.exe`, not `target/native/release/build/cmd/cmd.exe`. Added focused `try_table` EH boundary coverage for nonthrowing value sinking and may-throw producer preservation; no implementation change was needed. `moon fmt`, `moon test src/passes` (`1581/1581`), and the final quick gate `moon info`; `moon fmt`; `moon test` (`4766/4766`) were green after the coverage addition.
  - Deliverables: apply the common checklist; coordinate raw gate coverage with `[AUDIT002-F]`/`[AUDIT002-G]`; scale direct compare to `10000` and refresh `SL` slot evidence before closing; record shrink candidates separately from semantic fixes.
