...
- [O4Z-AUDIT-DAE] - Deep audit `dae-optimizing`
  ...
  - Current finding: research notes `1562-2026-07-12-dae-large-module-nested-cleanup-guard-relaxation.md`, `1563-2026-07-12-dae-func3737-merge-blocks-drop-carrier.md`, `1564-2026-07-12-dae-generic-folded-multivalue-suffix-defaults.md`, and `1565-2026-07-13-dae-generic-constant-condition-inter-argument-defaults.md` remove the coarse `>100` defined-function nested-cleanup skip, fix the folded-multivalue Func3737 shared-drop-carrier `merge-blocks` abort, replace the bespoke Func3737/Func3765 forwarded-default helper pair, and add a guarded generic literal-condition/dead-zero-if inter-argument default recognizer. `moon test src/passes/dae_optimizing_test.mbt`, `moon test src/passes`, and a fresh normalized 1000-case direct compare are green; the audit remains open for broader missing behavior families rather than these repaired operand-localization blockers.
  - Deliverables: apply the common checklist; coordinate with `[AUDIT003-*]`; refresh DAE-normalized direct compare and late `DAE` slot evidence; keep accepted raw-cleanup drift separate from semantic mismatches; continue shrinking the remaining broad gaps (real `precompute-propagate`, fuller default-function replay, broader operand localization beyond direct literal conditions and guarded suffixes, immutable-global-conditioned Func323 genericization, GC param refinement, result refinement, and the rest of selected/artifact genericization) before claiming broader large-module signoff.
...
- [AUDIT]003 - DAE Selected-Shape De-Artifacting And Genericization
  ...
  - Current finding: note `1564-2026-07-12-dae-generic-folded-multivalue-suffix-defaults.md` replaces the bespoke Func3737/Func3765 forwarded-default helpers with a guarded generic folded-multivalue suffix recognizer, and note `1565-2026-07-13-dae-generic-constant-condition-inter-argument-defaults.md` adds a low-definition generic literal-condition result-if plus dead-zero-if bridge. The older Func323 family is now narrower rather than closed: its immutable-global condition and artifact cleanup sequence remain selected, while the direct-literal inter-argument family is generic.
