...
- **Current progress:** analysis-only Flat IR classification, scalar function-result materialization, root `local.tee` lowering, and ordered scalar ordinary-operand preludes scoped to their owning structured region are implemented with focused verifier-backed tests; value-carrying control, branch payloads, nested tees, unreachable placeholders, EH, profiles, and public wiring remain open.
- **Deliverables:**
