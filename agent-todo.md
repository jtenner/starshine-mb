...
- Latest diagnostic timing is over the 2x target for the current Func509 artifact: timing-only helper replay `.tmp/dae011-timing-only-20260526` completed without normalized-WAT comparison and reported `2785.602ms` Starshine pass versus `836.627ms` Binaryen pass. `[DAE]011` remains open for selected-lane profiling/optimization; top-level detail timers from `.tmp/dae011-detail-trace.stderr` attribute the release traced artifact run to `2748777us` in `detail:dae:core`, `4us` in skipped `detail:dae:nested-total`, and `47855us` in `detail:dae:prune-unused-simple-func-types`. Core-family detail timers from `.tmp/dae011-core-detail-trace.stderr` now attribute the latest core run to `85798us` setup, `486741us` fixed primary loop, `1917924us` selected artifact-specific lanes, and `96963us` module raw cleanup, with total `detail:dae:core` at `2762312us`.
...
- [DAE]011 - Performance Stabilization
  ...
  - Deliverables:
    ...
    - [ ] Split `detail:dae:core:selected-lanes` into smaller rewrite-family timers before choosing the optimization target.
