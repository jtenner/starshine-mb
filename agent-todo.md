...
- `RemoveUnusedModuleElements`: table metadata ops, pure writes, and non-observable live tables no longer pin dead active contents or table initializer helpers; the remaining indirect-call/table gap is Binaryen-style callable-signature fallback for mutated indirect tables.
