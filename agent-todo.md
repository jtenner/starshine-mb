...
- The definitive fresh original-input WAGO checkpoint is now `.tmp/wago-o4z-pass8-final15-bottom-ref-test-fresh-20260821/`, covering all 1,330 tracked fixtures at or below 2 MiB and the unchanged 766-file stable cohort with native SHA-256 `33ee0c5763bf247aa57d99ad773c3fd33ca3852d3b78ab2464fa16a9733d6143`. Starshine emits 413,308 bytes versus pinned Binaryen v131 at 415,485, a fresh 2,177-byte / 0.524% win with 110/600/56 smaller/equal/larger, 3,033 positive residual bytes, 46,822 bytes removed from the 460,130-byte baseline, and 104.88% original-gap closure. Final15 adds an explicit OptimizeInstructions heap-relation miss when a source heap differs from a non-null abstract bottom target (`none`, `noextern`, `nofunc`, `noexn`, or `nocont`, including shared spellings); same-bottom identity and nullable-target planning remain unchanged. The only fresh artifact change is `ref-test/commands.22` 73 → 70, closing it to Binaryen with zero regressions. Fresh-instance calls with null, objects, arrays, functions, and a WebAssembly.Global all return 0 for input, Binaryen, and Starshine. Exact final14-vs-final15 200-pair median is 3.885 versus 4.184 ms, 7.13% faster. Red-first coverage spans all five bottom families. A focused `local-subtyping-null-bottom` explicit-v131 lane is 1,000/1,000 normalized with zero failures or mismatches. The full random-all seed-`0x5555` development lane compared 9,989/10,000 with 6,660 normalized and 3,329 current cross-profile residuals, zero validation/property/generator failures, and 11 legacy-EH Starshine command failures that reproduce unchanged under the exact final14 binary; this is current broad evidence, not a new complete OI closeout claim. Final14’s candidate-local global pruning remains unchanged: fuzzcase 1820 is 814, fuzzcase 1825 is 594 and externally valid, and the slower 2031 reduction remains rejected. The runtime-safe retained projection remains distinct and now projects 412,172 versus 415,485, a 3,313-byte / 0.797% win with 112/599/55 smaller/equal/larger, 1,994 positive residual bytes, 47,958 bytes removed, and 107.42% gap closure. Full validation is 10,602/10,602; all passes 6,952/6,952; command 268/268. Remaining fresh leaders are ifs +1,434, JSON-SIMD +397, primes +378, fannkuch +170, fasta +129, scalar JSON +86, startup JSON +60, urem +38, issue1809 +35, deeply nested struct +25, bump-pointer GC +24, return-call-indirect +21 each, array-copy-inline +19, and fuzzcase 1820/1825 +8/+7. Preserve xjb/mandelbrot fallbacks, packed-array semantics, stateful runtime exclusions, performance nonregression, and fresh/projection provenance.
