...
- **Deliverables:**
  - [ ] Strict-improvement reference-parameter LUB across every owned direct operand, including old-type local repair for incompatible writes. *(direct-GC batch discovery is definition-count-independent; direct actual evidence now covers allocation, exact `ref.func`, direct/tail call, typed indirect/tail-indirect, and typed call-ref/tail-call-ref producers across parsed and decoded abstract reference params; both the batch and ordinary core parameter-refinement producer dispatch `DaeGcParameterRefinementWork` and preserve producer effects through the family transaction; written refined parameters from both ordinary-core and optimizing batch producers now receive entry-initialized old-type locals with all nested gets/sets/tees remapped; nonnullable old types use nullable storage plus recursive `ref.as_non_null` repair on gets/tees, matching Binaryen's nondefaultable-local policy. ordinary-core collection and optimizing-batch stale refresh now share one exact-caller, fail-closed actual-value collector, and both parameter LUB application paths share one exact-length, exact-call-count strict-refinement combiner; the batch's initial multi-target collection remains specialized)*
