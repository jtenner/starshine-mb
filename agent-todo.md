...
- [FUZ]1021 - Invalid Binary Malformed-Byte Matrix
  - Status: IN PROGRESS (cron 23, 2026-05-20). Follow-up bulk-init secondary-index LEB slice landed: `malformed-memory-init-memory-index-uleb` and `malformed-table-init-table-index-uleb` are decode-rejected, non-smoke binary-invalid strategies that corrupt the second resource-index immediates on `memory.init` and `table.init` with unterminated ULEBs, with registry entries, focused replay coverage, fuzz-hardening/LEB docs, wiki log entry, and `.mbti` public enum refresh. TDD evidence: `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt` failed before implementation on the missing bulk-init strategy registry/replay coverage, then passed after implementation. Earlier slices also landed the import descriptor type-index LEB strategies, export descriptor index-LEB strategies, bulk-copy index-LEB strategies, `invalid-element-kind-byte` for passive/declarative function-index elemkind corruption, and `duplicate-function-section`, `duplicate-code-section`, `duplicate-element-section`, `duplicate-data-section`, and `duplicate-tag-section` for duplicate known-section decode rejection. Final validation for this slice ran `moon fmt`, `moon info` (pre-existing DAE unused warnings only), `moon test --package jtenner/starshine/fuzz --file invalid_binary_wbtest.mbt`, `moon test src/fuzz`, `moon test`, and `moon run src/fuzz -- validate-invalid-binary smoke --seed 0x5eed`. Remaining blocker: the task is intentionally broader than this slice; next run should continue with another malformed-byte family such as malformed/overwide LEB carriers in more contexts, invalid lane immediates beyond the current SIMD lane cases, more UTF-8/name payload corruption, invalid heap/ref encodings, or additional section descriptor bytes without weakening stage-aware accounting.
