...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [RUB]002 - Multi-Slot Cleanup and Artifact Compare - Replay the pass in each Binaryen slot and confirm later `merge-blocks` opportunities match the reference output.
     ...
     - Current blocker: the top-level scheduler gap is now fixed: `optimize` and `shrink` both replay `remove-unused-brs` in three modeled slots, focused preset tests cover those trace counts, the old `Func 24` `InvalidLabelOwner(4, 16)` failure is fixed, and the reduced `Func 299` loop-string-reader verifier case is now fixed with an in-tree regression. The next serial-verify blocker on `tests/node/dist/starshine-debug-wasi.wasm` is `func[(Func 1329)]: verify before remove-unused-brs: InvalidLabelOwner(9, 26)` after replaying a freshly built native `cmd` binary.
     - Doc: [0066#L188](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L188)
   - [IR]003 - Lifted Label Ownership Repro for RUB Artifact Gate - Reduce `Func 24` from the MoonBit debug artifact to a standalone `hot_lift`/verify repro so RUB artifact compare can resume.
     ...
     - Status: the in-tree repros now cover both the malloc-shaped `Func 24` case and the loop string-reader `Func 299` case in `src/passes/remove_unused_brs_test.mbt`. The label-owner corruption is fixed for value-`if` select rewrites and one-armed `if br` rewrites by preserving the original label owner and only inlining the synthetic single-`br_if` wrapper when its block label is unused. The remaining work is reducing `Func 1329`, not another replay of the already-fixed cases.
