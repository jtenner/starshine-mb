...
0. Shared blocker on canonical artifact parity.
  ...
  - [IR2]001 - Hot Lower Payload-Forwarder Repair - Restore the canonical hot-lower behavior for typed payload-forwarder and returned-if shapes before landing more cleanup rewrites on top.
     ...
     - Current blocker: the full repo test suite is green again (`moon test` => `1929/1929`), so this is no longer an active red-test blocker. The remaining work is to refresh exact artifact-backed evidence for the still-risky payload-forwarder families and only reopen this slice if new ordered-prefix replay or focused `hot_lower` regressions show those shapes drifting from the intended typed-block and `unreachable` contracts. A focused typed loop-carrier live repro is now locked in-tree, but the later RUB debug-artifact `Func 807` stack-underflow path still lacks a smaller standalone `hot_lower` reduction.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [DCE]002 - Prefix Regression and Artifact Replay - Lock the corrected DCE behavior into focused tests and rerun the MoonBit debug-artifact compare harness.
     ...
     - Status: focused DCE regressions plus the earlier hot-lift/effects fixes are landed, and the suite is green again (`moon test` => `1929/1929`). A new HOT-based live repro now locks one reduced typed-loop carrier hazard: DCE preserves loop-body `drop` roots that consume shared loop input children instead of deleting them and breaking lowering. The remaining work is still artifact replay and ordered-prefix proof; direct native `--dead-code-elimination` replay on `tests/node/dist/starshine-debug-wasi.wasm` is still blocked later at `Func 1730` with final-module `stack underflow`, so this slice remains open on the larger artifact.
     - Doc: [0066#L178](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L178)
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [RUB]002 - Multi-Slot Cleanup and Artifact Compare - Replay the pass in each Binaryen slot and confirm later `merge-blocks` opportunities match the reference output.
     ...
     - Current blocker: the top-level scheduler gap is now fixed: `optimize` and `shrink` both replay `remove-unused-brs` in three modeled slots, focused preset tests cover those trace counts, and the old `Func 24`, `Func 299`, `Func 1329`, and `Func 7756` serial verifier failures are fixed in-source. `Func 7756` was a multi-value branch-payload rewrite bug: RUB now refuses to collapse a branch-payload `if` when the surviving payload side would be forwarded as fewer branch slots than the original `br` carried. The next debug-artifact blocker on `tests/node/dist/starshine-debug-wasi.wasm` is now `final module validate: stack underflow` in `func[(Func 807)]` after replaying a freshly built native `cmd` binary.
     - Doc: [0066#L188](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L188)
