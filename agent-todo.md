...
- **Exit criteria / suggested tests:** all 61 named v133 tests and all 12,334 default wasm-gc tests now pass after the initial implementation. Finish pass-targeted 10,000-case comparisons with a fresh native Starshine binary and the verified v132 baseline, classify and repair residuals, and expand the explicitly open eligibility and invalid-input guards. The four saturating i64 conversions are implemented, while the rest of wasm2js i64 lowering remains open. Shared-object and GTO type/index surfaces need fuzz scrutiny before signoff.
