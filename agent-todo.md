...
- DeadCodeElimination research baseline:
  ...
  - the latest reduced coverage now proves the generic loop-wrapped branch-payload bypass family validates and roundtrips locally in raw, generated, and pre-lifted binary paths; the remaining `Func 716` failure still needs a more artifact-specific reduction.
  ...
  - next step: reduce the later fresh-artifact `Func 716` direct-DCE blocker to a focused nested result-block regression, then rerun normalized Starshine-vs-Binaryen comparison at the direct and five-pass checkpoints.
