...
- [FUZ]1012 - Randomized Coverage-Forced Templates
  ...
  - Current evidence: `moon test src/validate` passes with a new regression proving coverage-forced template signatures are deterministic for the same seed, differ across selected seeds, and preserve numeric/control floors. Docs note the seed-deterministic branch/loop ordering variation.
  - Remaining: randomize additional forced families beyond the final branch/loop template (for example memory/table/ref/call/SIMD template variants), add explicit template-family diagnostics or manifest-visible facts if the public ledger needs them, then rerun `moon test src/fuzz` / relevant fuzz smoke before closing.
