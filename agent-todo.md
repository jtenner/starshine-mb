...
1. Canonical correctness landed.
   - [RUME]003 - Oracle-stable fast-path pass completed - Direct pass execution, CLI flag and registry wiring, focused section/index rewrite coverage, explicit memarg-index rewrites, and canonical compare-harness artifact parity are in place; `remove-unused-module-elements` is now canonically correct against the oracle on the debug artifact.
     ...
     - Remaining blocker after semantic cleanup: the post-fix `wasm-smith` command-failure corpus is still coverage-only work. Starshine accounted for `7/20` cases: six `DecodeAt(InvalidLimits, ...)` inputs (`000007`, `000014`, `000036`, `000040`, `000055`, `000091`) plus one final-module validation failure `Invalid range for limits` (`000073`). Binaryen accounted for `13/20` parser failures: eight `invalid type index` cases (`000009`, `000021`, `000024`, `000031`, `000047`, `000050`, `000052`, `000056`), one `Recursion groups of size zero not supported` case (`000029`), and four later parser-compatibility failures (`000054`, `000079`, `000081`, `000083`). The first Starshine family is broader than RUME itself: current core limit types do not model shared-memory flags, so valid shared-memory imports still fail in the decoder before pass replay begins.
...
2. Do work.
   ...
   - The remaining post-fix work is no longer semantic RUME cleanup; it is upstream fuzz-coverage work on shared-memory limit decoding and Binaryen parser-compatibility blockers.
