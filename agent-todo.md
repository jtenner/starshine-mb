...
- `[FUZ]003A` is complete. `remove-unused-module-elements` now drops unused imported functions, compacts dead simple function types after that import removal, and keeps surviving export/start/elem/name references coherent through the remap. The focused rerun `bun scripts/pass-fuzz-compare.ts --pass remove-unused-module-elements --generator gen-valid --count 20 --max-failures 5 --out-dir .tmp/pass-fuzz-fuz003a-genvalid-smoke` improved from the old `5/5` imported-function mismatches to `20/20` compared cases with `18` normalized matches; the remaining `case-000002-gen-valid` and `case-000020-gen-valid` mismatches are a distinct no-op start-section pruning family, not imported-function retention, and are now tracked under `[FUZ]003B`.
- The next unfinished validator fuzz slice is `[FUZ]003B`.
...
3. Widen valid generation topology first.
   ...
   - [FUZ]003B - Gen-valid `RUME` no-op start-section parity follow-up.
     - Goal: determine and match the exact Binaryen rule behind the two saved `gen-valid` cases where `remove-unused-module-elements` drops a `start` section that Starshine currently preserves.
     - Why: the widened `coverage-forced` batch still carries a deterministic `2/20` compare-pass mismatch family on the small `RUME` smoke lane even after the imported-function fix landed.
     - Deliverables: add focused `start`-section regressions in `src/passes/remove_unused_module_elements_test.mbt`, reduce the positive boundary plus at least one nearby negative boundary, and only drop `start` when the proved preconditions are narrow enough to avoid deleting observable start behavior.
     - Required APIs: `src/passes/remove_unused_module_elements.mbt`, `src/passes/remove_unused_module_elements_test.mbt`, and the existing `gen-valid` compare harness smoke lane.
     - Invariants: keep the imported-function fix from `[FUZ]003A` intact; do not remove arbitrary surviving start functions; preserve function/export/elem remaps while matching the exact Binaryen start-pruning contract for this family.
     - Dependencies: `[FUZ]003A`; saved repros `.tmp/pass-fuzz-fuz003a-genvalid-smoke/failures/case-000002-gen-valid/` and `case-000020-gen-valid/`; research note `0091`.
     - Exit criteria: the saved `case-000002-gen-valid` and `case-000020-gen-valid` repros match Binaryen, and the follow-up smoke `bun scripts/pass-fuzz-compare.ts --pass remove-unused-module-elements --generator gen-valid --count 20 --max-failures 5 --out-dir .tmp/pass-fuzz-fuz003b-genvalid-smoke` no longer reports this `start`-section-only family.
     - Suggested tests: `moon test --package jtenner/starshine/passes --file remove_unused_module_elements_test.mbt`; `bun scripts/pass-fuzz-compare.ts --pass remove-unused-module-elements --generator gen-valid --count 20 --max-failures 5 --out-dir .tmp/pass-fuzz-fuz003b-genvalid-smoke`.
