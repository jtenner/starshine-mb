...
- **Deliverables:**
  ...
  - [ ] Replace `touched_count > 8`, module `>100`, local `>128`, instruction `>1000`, and material-caller thresholds with source-matched per-function profitability or explicitly non-parity budgets. *(`touched_count`, module size, semantic nested local/instruction skips, and the encoded-size admission for producer-owned body-only worth functions are removed; the earlier broad-cleanup local/instruction discriminator and exact-boundary heavy-replay policy remain.)*
