...
- [FUZ]1021 - Invalid Binary Malformed-Byte Matrix
  - Status: IN PROGRESS (cron 23, 2026-05-20). Narrow opcode/subopcode byte slice landed: `invalid-opcode-byte` and `invalid-prefix-subopcode-byte` are decode-rejected, non-smoke binary-invalid strategies with registry, replay, docs, and `.mbti` coverage. Validation evidence: `moon test src/fuzz`, `moon fmt`, `moon info` (pre-existing DAE unused warnings only), and `moon test`. Remaining blocker: the task is intentionally broader than this slice; next run should continue with another malformed-byte family such as invalid heap/ref/blocktype encodings, memarg/alignment corruption, or deeper section/vector count mismatches without weakening the current stage-aware accounting.
