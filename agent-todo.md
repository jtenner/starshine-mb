...
0. Shared blocker on canonical artifact parity.
  - [HOT]001 - Post-SSA Hot-Pipeline Replay Hardening - The large-artifact `DFE -> RUME -> MP -> OR -> GR -> GSI -> SSA` prefix now completes, validates, and canonicalizes under Binaryen, and DCE itself no longer emits the invalid lowered output that previously blocked the chain on final dead-result control. The remaining ordered full-chain work is now downstream of that fix: rerun the exact post-SSA cleanup replay on a valid baseline artifact, refresh any remaining later-pass re-lift failures on the rebased branch, and keep the serial hot batch workable until the worker-queue slice is safe.
     ...
     - Current blocker: post-prefix `--ssa-nomerge` replay still benefits from the raw no-write / local-write bypasses, linked label stacks, owned typecheck state, and final-module-only lowering path, but large-artifact replay remains over budget because a much smaller set of very large functions now dominates wall time, concentrated in a `br`-heavy nested `block` / `if` cluster plus one loop / `br_table` outlier. The normal self-opt compare entrypoint is also currently blocked earlier because `tests/node/dist/starshine-debug-wasi.wasm` fails baseline validation before Binaryen runs, so ordered replay signoff must either use a regenerated valid baseline or continue diffing against the last successful oracle artifact until that entry path is refreshed.
  ...
  - [IR2]001 - Hot Lower Payload-Forwarder Repair - Restore the canonical hot-lower behavior for typed payload-forwarder and returned-if shapes before landing more cleanup rewrites on top.
     ...
     - Current blocker: `moon test` is currently red with 13 failures, 6 of them in [`src/ir/hot_lower_test.mbt`](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt). The malformed compare-shape fixtures at [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1381), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1427), and [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1475) now parse, and the earlier flattened parent-exit branch-depth mismatch at [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L358) plus the direct returned-if and split-carry fixtures at [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L437), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L461), and [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L522) are no longer red. The remaining active broken families are the packed local-set carry and parent-exit payload cases at [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1381), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1527), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1586), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1614), [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1648), and [hot_lower_test.mbt](/home/jtenner/Projects/starshine-mb/src/ir/hot_lower_test.mbt#L1678). Concretely, lowering still leaves a typed `if` plus outer `drop` where the compare-shape local-set carriers should already be voidified, and it still keeps one extra packed result wrapper around parent-exit payload branches where the tests expect the direct `br $outer` shape.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [DCE]002 - Prefix Regression and Artifact Replay - Lock the corrected DCE behavior into focused tests and rerun the MoonBit debug-artifact compare harness.
     ...
     - Status: focused DCE regressions plus the earlier hot-lift/effects fixes landed, but the suite is not currently green. `moon test` now fails on seven DCE fixtures in the same lowered payload-forwarder family: [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L65), [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L435), [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L582), [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L761), [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L840), [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L1481), and [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L1536). The new breakage is important: the carried parent-exit family at [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L65) and [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L435) now fails final validation with stack underflow after lowering, and the typed loop-body wrapper cases at [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L1481) and [dead_code_elimination_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/dead_code_elimination_test.mbt#L1536) are red again because the current hot-lower shape still leaves the wrong explicit `unreachable` / wrapper contract for DCE cleanup. Those regressions still overlap directly with [IR2]001, so the next DCE correctness slice must rebase on repaired lowering before taking new oracle diffs as truth.
     - Doc: [0066#L178](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L178)
   - [DCE]003 - Runtime Budget and Oracle Refresh - Close the remaining DCE speed gap and refresh parity evidence on a valid baseline/oracle path.
     ...
     - Current blocker: direct native `--dead-code-elimination` replay on `tests/node/dist/starshine-debug-wasi.wasm` is still runtime-bound and the normal `bun scripts/self-optimize-compare.ts ... --dead-code-elimination` entrypoint still stops before Binaryen runs because the baseline input fails validation, but the immediate gating problem is smaller: until [IR2]001 and the four red DCE fixtures are green, fresh parity/perf numbers are not trustworthy enough for signoff.
     - Doc: [0066#L178](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L178)
3. Do work.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [RUN]001 - Triple-Slot Replay and Ordered-Prefix Parity - The core function pass is landed with exact single-pass oracle parity for same-typed block peeling and loop demotion, and the public `optimize` / `shrink` presets now replay `remove-unused-names` in all three currently-modeled RUN slots. Focus has shifted from missing scheduler wiring to proof and throughput.
     - Deliverables: keep the preset replay stable; retain repeated-slot regressions around stale labels exposed by `remove-unused-brs`; keep canonical compare parity green on the debug artifact while replaying the exact ordered prefix that is currently modeled in the active preset.
     - Current blocker: standalone `--remove-unused-names` parity is exact and the repeated-slot scheduler wiring is now landed, but Starshine pass time is still roughly `82.722-87.984ms` versus Binaryen at roughly `14.014-14.700ms` on `tests/node/dist/starshine-debug-wasi.wasm`, so the remaining work is ordered-prefix artifact proof plus more serial throughput / GC-churn reduction rather than missing core semantics.
     - Doc: [0066#L183](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L183)
   - [RUN]002 - Pass-Fuzz Command-Failure Classification And Replay - Keep the `remove-unused-names` differential-fuzz command-failure corpus visible as a distinct follow-up item so Binaryen parser and canonicalization gaps are tracked separately from Starshine semantic regressions.
     - Deliverables: preserve the saved `wasm-smith` corpus under `.tmp/pass-fuzz-run-smith`; replay the Binaryen-only failures independently of Starshine pass work with `--replay-failures-from` and `--failure-class`; keep the harness-level `failureClass` buckets current as new command-failure families appear.
     - Current blocker: fresh `remove-unused-names` pass fuzzing is semantically clean on Starshine (`.tmp/pass-fuzz-run-genvalid` reached `500/500` normalized matches with `0` mismatches and `0` command failures; `.tmp/pass-fuzz-run-smith` reached `165/165` normalized matches with `0` mismatches), but the `wasm-smith` sweep still stops on `20` Binaryen-side canonicalization failures. The current classified families are seventeen `invalid type index` cases (`000009`, `000021`, `000024`, `000031`, `000047`, `000050`, `000052`, `000054`, `000056`, `000079`, `000095`, `000124`, `000141`, `000143`, `000162`, `000167`, `000185`), one `Recursion groups of size zero not supported` case (`000029`), and two `invalid wasm type: -64` cases (`000081`, `000083`).
     - Suggested Tests: `bun scripts/pass-fuzz-compare.ts --pass remove-unused-names --generator gen-valid --count 500 --max-failures 5 --out-dir .tmp/pass-fuzz-run-genvalid`, `bun scripts/pass-fuzz-compare.ts --pass remove-unused-names --generator wasm-smith --count 300 --max-failures 20 --out-dir .tmp/pass-fuzz-run-smith`, `bun scripts/pass-fuzz-compare.ts --pass remove-unused-names --replay-failures-from .tmp/pass-fuzz-run-smith-classified --failure-class binaryen-invalid-type-index --out-dir .tmp/pass-fuzz-run-smith-invalid-type-index-replay`, and targeted `wasm-opt --all-features --remove-unused-names` replay on the saved failing inputs.
3. Do work.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [RUB]002 - Multi-Slot Cleanup and Artifact Compare - Replay the pass in each Binaryen slot and confirm later `merge-blocks` opportunities match the reference output.
     ...
     - Current blocker: `moon test` is currently red on the dropped returned-if family in [remove_unused_brs_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/remove_unused_brs_test.mbt#L1073) and [remove_unused_brs_test.mbt](/home/jtenner/Projects/starshine-mb/src/passes/remove_unused_brs_test.mbt#L1093). Both fixtures mean the same thing: `remove-unused-brs` is recognizing that each arm exits via `return`, but it is not finishing the value-to-void rewrite when the surrounding result is dead, so the lowered code still lacks the expected `unreachable` after the now-void `if`, both in the plain `drop (if ...)` shape and through the `local.tee -> drop` wrapper. Recent lift inspection still shows the broader remaining parity families arrive in HOT as `Block(result) -> Return -> If` with zero-result holder blocks around simple arms, so the next correctness slice should fix the red dropped-return fixtures first and then target those lifted entry points directly instead of widening existing return-context tail helpers wholesale.
     - Doc: [0066#L188](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L188)
3. Do work.
   - Land the slices above in dependency order in the implementing file(s) and any required scheduler, preset, or dispatcher surfaces.
   - Wire the pass into the exact top-level slot(s) and nested rerun sites documented in the research doc before calling the work done.
...
1. Keep the remaining follow-up visible.
   ...
   - [OI]002 - Dual-Slot Replay and Regression Lock - Verify both OI slots remain correct after surrounding pipeline work and lock the behavior in focused tests.
     ...
     - Current blocker: `optimize-instructions` is not part of the current red `moon test` set, so this is no longer an immediate correctness blocker. The remaining work is repeated-slot and ordered-prefix evidence refresh after the current `hot_lower` / DCE / RUB repair work settles, especially for the still-blocked tee/control parity families. Current repo state: the pass is implemented, registered, and preset-wired, and none of the 18 active failures come from [`src/passes/optimize_instructions_test.mbt`](/home/jtenner/Projects/starshine-mb/src/passes/optimize_instructions_test.mbt).
     - Doc: [0066#L193](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L193)
2. Do work.
