...
- [DAE]006 - Both-Canonical Frontier Advancement
  ...
  - Next action:
    - Instrument or reduce the exact instruction list received by `dae_strip_module_root_return_suffix_once` for `defined=509 abs=526`; the `--print-func` and post-encode canonical shapes are not sufficient because the new outer-block helper is covered but does not hit the artifact.
    ...
    - Preserve the landed root-return, simple wrapper-suffix, and outer-block tests/helpers if they remain valid, but do not claim Func509 closure until the both-canonical artifact compare moves beyond `defined=509 abs=526`.
  - Deliverables:
