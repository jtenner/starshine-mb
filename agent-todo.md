...
- [O4Z-AUDIT-DAE] - Deep audit `dae-optimizing`
  ...
  - Current finding: research notes `1562-2026-07-12-dae-large-module-nested-cleanup-guard-relaxation.md` through `1566-2026-07-13-dae-generic-immutable-global-inter-argument-defaults.md` remove the coarse `>100` defined-function nested-cleanup skip, fix the folded-multivalue Func3737 shared-drop-carrier `merge-blocks` abort, and genericize folded-multivalue, direct-literal-condition, and immutable-defined-global-condition default families. The former selected Func323 8/9-parameter rewrite is removed; only its already-seven-parameter dropped folded-block artifact cleanup remains selected with explicit reopening evidence. `moon test src/passes/dae_optimizing_test.mbt`, `moon test src/passes`, and a fresh normalized 1000-case direct compare are green; the audit remains open for broader missing behavior families rather than these repaired operand-localization blockers.
  - Deliverables: apply the common checklist; coordinate with `[AUDIT003-*]`; refresh DAE-normalized direct compare and late `DAE` slot evidence; keep accepted raw-cleanup drift separate from semantic mismatches; continue shrinking the remaining broad gaps (real `precompute-propagate`, fuller default-function replay, broader operand localization beyond guarded direct-literal and immutable-defined-global conditions, GC param refinement, result refinement, and the rest of selected/artifact genericization) before claiming broader large-module signoff.
