...
- [O4Z-AUDIT-DAE] - Deep audit `dae-optimizing`
  ...
  - Current finding: research notes `1562-2026-07-12-dae-large-module-nested-cleanup-guard-relaxation.md` through `1567-2026-07-13-dae-block-fallthrough-propagation-and-imported-tag-type-gap.md` remove the coarse `>100` defined-function nested-cleanup skip, fix the folded-multivalue Func3737 shared-drop-carrier `merge-blocks` abort, genericize folded-multivalue/direct-literal/immutable-global default families, and add Binaryen-shaped effect-preserving block-fallthrough constant propagation to the private touched prefix. The former selected Func323 8/9-parameter rewrite is removed; only its already-seven-parameter dropped folded-block artifact cleanup remains selected with explicit reopening evidence. DAE/pass tests are green and a 1000-case wasm-smith smoke has zero mismatches among 996 comparisons, but it also exposes a real validation failure: imported tag payload types are omitted from simple type-pruning liveness, so tag-only `case-000140` loses its referenced `TypeIdx`. The audit remains active.
  - Deliverables: fix imported-tag type liveness first with red public-pipeline coverage and saved-case replay; apply the common checklist; coordinate with `[AUDIT003-*]`; refresh DAE-normalized direct compare and late `DAE` slot evidence; keep accepted raw-cleanup drift separate from semantic mismatches; continue shrinking the remaining broad gaps (fuller `precompute-propagate` fallthrough/worklist behavior, fuller default-function replay, broader operand localization beyond guarded direct-literal and immutable-defined-global conditions, GC param refinement, result refinement, and the rest of selected/artifact genericization) before claiming broader large-module signoff.
