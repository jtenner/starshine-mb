...
- [FUZ]1007 - GenValid Memory, Table, Data, And Element Segment Widening
  - Status: IN PROGRESS (cron 23, 2026-05-21). Current slice landed nonzero bulk-resource body coverage: the shared GenValid bulk-memory prelude now emits `memory.copy` / `memory.fill` over the first nonzero i32 memory index and uses a second passive data index for `memory.init` / `data.drop` when available; the bulk-table prelude now emits `table.fill` / `table.copy` over the first nonzero table index without consuming RNG. Focused anchors: `memory-heavy profile emits nonzero memory bulk and segment operators` and `topology-heavy profile emits nonzero table bulk and element operators`; `moon test src/validate` passed. Resume here for the remaining matrix: nonzero scalar memory memargs, explicit table get/set/grow/size over nonzero tables, broader `memory.init` / `table.init` compatibility matching, and final compare-pass/profile signoff before removing this task.
