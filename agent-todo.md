...
The full 56-slot roster and historical closeout evidence live in `docs/wiki/binaryen/passes/late-pipeline-dispatch.md` and the pass dossiers. Direct `remove-unused-brs`, `merge-blocks`, and `flatten` ownership is closed with focused runtime, full Moon, explicit-v131 four-lane, residual classification, and native-release benchmark evidence; scheduler placement remains unchanged. Remaining v0.1.1 work is exact scheduler integration and nested-rerun ownership.
...
- **Current differences:**
  ...
  - [x] Close the reopened `flatten` signoff exposed by the expanded RUB corpus (2026-08-02). Native SHA-256 `96d0ad4aff0e0a759faa887f5879fc7eac731adbb91fdfb3ac014549ee52e903` against explicit official Binaryen v131 completes regular `100,000/100,000`, wasm-smith `9,955/10,000` comparable with `45` Binaryen/tool failures, `flatten-all` `10,000/10,000`, and random-all `9,641/10,000` comparable with `359` Binaryen failures; Starshine has zero command, generator, validation, or property failures. All `2,468` dedicated/random residual pairs converge byte-for-byte under v131 `-Oz --strip-debug`, and every canonical Starshine residual is smaller, so the documented scalar-`br_if`, multivalue, coverage-forced, and seven smaller non-EH families are classified as direct cleanup wins with explicit reopening criteria. The ownership/failure-atomicity audit confirms frozen pre-mutation proof, admission-before-repair, and atomic target/deletion preflight; a combined regression proves a later deferred branch leaves an earlier repairable catch untouched. The reconstructed historical 120-function candidate-dense lane measures `1,392 us` Starshine versus `126.709 us` Binaryen v131 (`10.99x`), retaining the bounded exception because absolute pass-local cost remains about `1.4 ms`, far below the `<1 s` target. Multi-arm legacy tries remain an explicit upstream HOT representation boundary. Isolated direct RUB at O4z levels `4/4` remains valid and no-larger for all `10,000` cases.
