...
| `remove-unused-brs` | Post-repair v131 work now retires all `181` old random gaps, admits safe multivalue-return roots, and repairs nested result-wrapper cleanup with CFG-backed continuation proof. Focused wrapper evidence is `1000/1000` exact. The fresh development aggregate is `5655/10000` exact with zero failures and a `-41684`-byte residual delta, so the old uniformly size-losing blocker is eliminated. Wrapper, result-refinalize, selectify, and single-target-table leaves are exact-green. | **Open targeted parity repair, fresh four-lane renewal, and scheduler reconciliation:** `[AUDIT-CORRECTNESS]001`; remaining owners are the `625` uniform `+1` multivalue-drop gaps, `1220` equal-size reference-typing/shape gaps, and classification of `2500` smaller compound residuals. |
