...
| `remove-unused-brs` | Post-repair v131 work retires the old random gaps, admits safe multivalue-return roots, repairs nested wrappers with CFG proof, and flattens exact dropped literal multivalue blocks. Focused wrapper is `1000/1000` exact; focused multivalue-drop is `1000/1000` measured wins, uniformly `-12` bytes. The development aggregate is `5655/10000` exact with zero failures and a `-81681`-byte residual delta; all size-different residuals are smaller Starshine outputs. | **Open targeted parity repair, fresh four-lane renewal, and scheduler reconciliation:** `[AUDIT-CORRECTNESS]001`; remaining owners are `1220` equal-size reference-typing/shape gaps and classification of `3125` smaller residuals. |
...
- **Deliverables:**
  ...
  - [x] Run the required four-lane v131 matrix for `remove-unused-brs` (2026-07-26 baseline: regular `100000/100000` cleanup-normalized green; wasm-smith `9956` comparable with `9954` direct plus `2` cleanup-normalized and `44` Binaryen-only failures; dedicated `10000/10000` mismatches; random-all `870` residuals). Follow-up repairs match the `181` prior random parity gaps, preserve the `689` measured wins, admit safe multivalue-return roots, and make wrapper cleanup exact. The development aggregate is now `5655/10000` exact with zero failures and a `-81681`-byte residual delta; focused multivalue-drop is a uniform measured `-12` win. Keep this item open for equal-size typing repair, smaller-residual classification, and a final fresh four-lane renewal.
