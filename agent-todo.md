...
- **Status (2026-07-22 partial):** the arbitrary `touched_count > 8` full-roster skip is removed. The ordinary nested replay now derives a Binaryen-shaped worth set from changed function signatures, result-change callers in both original and current call graphs, and exact caller local-count growth for transitional scratch cleanup; plain callsite-only edits and touched callers with unchanged preexisting locals are no longer replayed. Module `>100`, local `>128`, instruction `>1000`, material-caller admission, and exact convergence feedback remain open. A red-first module-guard removal trial exposed phase-order regressions in the selected literal/global fixtures, so the module guard remains until those producers move ahead of or into the common lifecycle.
- **Deliverables:**
