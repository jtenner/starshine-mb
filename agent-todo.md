...
- **Status (2026-07-22 partial):** canonical direct-call operand slices, canonical parameter-slot actions, and ordinary unused-parameter analysis migration landed. The scalar, forwarding-component, and family plans now carry validated `DaeCommitEffects` alongside the candidate: structural touched ownership stays separate from semantic worth, and effects record parameter/result/body/local/control/type/metadata consequences, invalidation domains, and decision reasons. Successful scalar/component/family applications, `DaeWorkCommitted`, core aggregation, and the DAEO pass-manager adapter preserve those effects. Scalar removal marks the callee worth without marking callsite-only callers; localization marks exact scratch-owning callers; materialization marks the callee; forwarding marks component members and any triggered ref-null follow-up. The pass manager trusts producer-owned worth for covered definitions and retains structural inference only for legacy producers; exact caller local-count growth is an explicitly temporary compatibility bridge until those scalar producers return their plans directly. `DaeFamilyRewritePlan` still wraps every dispatcher result, optimizing unread/dropped-result/GC batches, ordinary core candidates, reverse/low exact revisits, low exact cleanup, broad immutable-global and adjacent-root revisits, dead-suffix parameter restoration, and boundary cleanup before commit. Older producer-specific mutation helpers, metadata finalization, and complete invalidation ownership remain open.
- **Deliverables:**
...
- **Deliverables:**
  - [ ] Match represented Binaryen per-operand constants: numeric/SIMD, `ref.null`, `ref.func`, `string.const`, supported ref conversions, and immutable `global.get`. *(uniform `v128.const`, `ref.func`, `string.const`, `ref.i31`, and externalized/re-internalized i31 conversion expressions use the generic value-slice/materialization path with exact structural equality; broader reference literals remain. Binaryen v131 DAE visits individual call operands through a single-`Literal` `PossibleConstantValues`, so generic `TupleMake` constant-expression support is not a DAE parameter requirement.)*
...
- **Status (2026-07-22 partial):** the arbitrary `touched_count > 8` full-roster skip is removed. The ordinary nested replay now derives a Binaryen-shaped worth set from changed function signatures, result-change callers in both original and current call graphs, and exact caller local-count growth for transitional scratch cleanup; plain callsite-only edits and touched callers with unchanged preexisting locals are no longer replayed. Module `>100`, material-caller admission, and exact convergence feedback remain open. Local `>128` and instruction `>1000` no longer suppress semantic nested replay: the size checks are isolated to the earlier broad changed-boundary fast-skip policy, preserving its authoritative 129-local transaction before the now-unconditional per-function nested roster. The module-guard removal trial still preempts selected literal/global producers. A red-first module-guard removal trial exposed phase-order regressions in the selected literal/global fixtures, so the module guard remains until those producers move ahead of or into the common lifecycle.
- **Deliverables:**
