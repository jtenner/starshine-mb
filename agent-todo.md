...
- [JSON-AS]001 - `remove-unused-brs` corrupts AssemblyScript incremental-GC loop exits
  ...
  - Deliverables:
    ...
    - [ ] Refresh direct `--remove-unused-brs` compare evidence and classify any remaining mismatches separately from this artifact trap. 2026-06-05 refresh command `bun scripts/pass-fuzz-compare.ts --count 1000 --seed 0x5eed --pass remove-unused-brs --out-dir .tmp/pass-fuzz-remove-unused-brs-refresh-1000 --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe` stopped at the harness failure cap after `52` compared cases: `26` normalized matches, `26` mismatches, `1` Binaryen rec-group-zero command failure, and `0` validation/property/generator failures. Sampled mismatches are agent-classified as semantic-safe but size/representation-losing constant-control cleanup gaps (`br_if (i32.const 1)` vs `br`, constant-false blocks, and unreachable tails after infinite loops), not the json-as loop-exit corruption family; still needs a focused normalization or optimization follow-up before closing.
