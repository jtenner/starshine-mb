...
- [FUZ]1021 - Invalid Binary Malformed-Byte Matrix
  - Status: IN PROGRESS (cron 23, 2026-05-21). Latest atomics operand-context slice adds decode-rejected, non-smoke `malformed-atomic-load-memarg-align-uleb`, which emits a valid `0xFE 0x10` `i32.atomic.load` subopcode then corrupts the following memarg alignment ULEB; docs now name the atomics memarg malformed-byte coverage. Previous atomics reserved-subopcode byte, section-payload underflow, atomic-prefixed subopcode malformed/overwide, overwide section vector-count, malformed vector-count, and smoke-scheduler slices are landed and documented; continue FUZ1021 by selecting another malformed-byte class such as remaining invalid UTF-8, opcode/prefix, lane, memarg, or additional atomics operand-context corruptions. Current TDD evidence: focused invalid-binary wbtest first failed on missing `malformed-atomic-load-memarg-align-uleb`, then passed after implementation.
