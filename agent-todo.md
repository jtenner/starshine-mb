...
- **Status:** **Active; `merge-blocks`, `once-reduction`, `memory-packing`, and `local-subtyping` renewed.** Implementation and red-first deterministic regression work are complete for `merge-blocks`, `once-reduction`, `memory-packing`, `local-subtyping`, `remove-unused-brs`, `precompute` / `precompute-propagate`, full `ssa` plan application, and the raw merge-locals cleanup. On 2026-07-26, `memory-packing` completed a fresh v131 owner audit and matrix after adding decoded legacy-`try` traversal, Binaryen's passive metadata/referrer profitability formula, end-to-end operand extraction regressions, and `memory-packing-all`: regular `100000/100000`, dedicated `10000/10000`, and random-all `10000/10000` exact; wasm-smith has `9955` exact across `9956` comparable cases plus the known pass-independent case-9332 unreachable-drop shell and 44 Binaryen-only failures. Remaining pass matrices stay open below.
