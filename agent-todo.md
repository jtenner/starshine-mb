...
- [FUZ]1010 - GenValid SIMD Exact Opcode And Relaxed-SIMD Widening
  - Status: IN PROGRESS (cron 23, 2026-05-21): representative exact standard-SIMD opcode counters are landed for phase-1/phase-2/phase-3 coverage, the phase-1 counter set has been widened to all currently emitted splats, lane extract/replace families, and core bitwise ops, and the phase-2/phase-3 counter sets now cover the currently emitted standard-SIMD matrix while preserving coarse SIMD phase facts for exact-counted instructions. The first relaxed-SIMD widening slice is landed behind the explicit `allow_relaxed_simd` toggle and `relaxed-simd` profile, outside Binaryen-oracle portable profiles; it emits validated drop-wrapped relaxed swizzle, lane-select, truncation, madd/nmadd, min/max, q15/dot, and dot-add forms and exact-counts each represented relaxed opcode. The SIMD memory-index slice uses explicit nonzero i32 memory indices for the forced phase-3 SIMD memory prelude when available, preserving varied alignment/offset memargs and default-memory fallback when no nonzero i32 memory exists. The latest lane-index slice extends the forced phase-1 SIMD lane prelude with min/middle/max legal single-lane immediates for 16-, 8-, and 4-lane families plus both legal lanes for 2-lane families, and documents the coverage in the generator ledger / WAST SIMD page. The latest v128 typed-body slice makes `SIMD-heavy` enable the typed body generator, guarantees typed-body function signatures with v128 params/results when type budget permits, guarantees a v128 local in coverage-forced typed bodies, emits a dedicated v128 `local.set`, and locks fixed-seed validation evidence for v128 signatures, locals/local.set, and value-producing v128 control results. The latest binary roundtrip slice fixture-locks every current relaxed-SIMD instruction to exact `0xFD` subopcodes `256..275` and decodes those bytes back to the matching `Instruction` variants, with the binary and WAST SIMD docs updated to point at this byte-level contract. Latest TDD evidence: `moon test src/binary` passed after adding `relaxed SIMD instructions use exact 0xFD subopcode roundtrips`; no implementation change was required because encode/decode already carried the correct relaxed opcode mapping. Previous TDD evidence: `moon test src/validate` first failed on `SIMD-heavy profile threads v128 through signatures locals and control results`; after implementation `moon test src/validate` passed. Next run should continue FUZ1010 with targeted `remove-relaxed-simd` compare once the explicit relaxed profile is wired into compare-pass profiles, or choose the next unproven standard-SIMD exact coverage surface from the deliverables.
