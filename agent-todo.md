...
- [FUZ]1009 - GenValid Numeric Constants, Arithmetic, Conversion, And Trap-Safe Cleanup Surfaces
  - Status: IN PROGRESS (cron 23, 2026-05-21) - arithmetic/reinterpret and boundary-literal slices landed; resume with deeper exact-opcode coverage.
  - Latest evidence: coverage-forced GenValid now emits and exact-counts `i32.add`, `i64.add`, `f32.add`, `f64.add`, `i32.reinterpret_f32`, `i64.reinterpret_f64`, `f32.reinterpret_i32`, and `f64.reinterpret_i64`; it also emits tested boundary literals for integer `0`/`1`/`-1`/min/max and float `-0`, infinities, and explicit NaN payloads in non-Binaryen-oracle coverage-forced profiles. The boundary TDD failure was `moon test src/validate` failing on missing `i32.const -1`, then `moon test src/validate` passed after implementation. Earlier arithmetic TDD failure was `moon test src/validate` failing on missing `i32.add`, then `moon test src/validate` passed. Full validation on 2026-05-21 after the boundary slice: `moon info` passed with pre-existing DAE unused warnings, `moon fmt` passed, `moon test` passed (`3459` tests).
  - Next steps: consider exact counters for additional numeric sub/mul/div/rem/bitwise/shift/rotate/comparison/conversion opcodes and add powers-of-two / nontrivial expression-tree evidence before declaring the broad task complete.
