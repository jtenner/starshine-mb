...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [SL]004 - Slot Validation and Artifact Replay - Lock the completed pass into the late slot and prove parity against Binaryen on both focused and large-artifact lanes.
     ...
     - Current blocker: the late-slot regression work is in-tree and the focused Binaryen lanes are green. [`src/passes/registry_test.mbt`](./src/passes/registry_test.mbt) locks `reorder-locals` as boundary-only, [`src/passes/optimize_test.mbt`](./src/passes/optimize_test.mbt) locks the modeled late `simplify-locals` slot plus `simplify-locals -> vacuum` cleanup, and [`src/passes/simplify_locals_test.mbt`](./src/passes/simplify_locals_test.mbt) now also covers the captured-source-local ordering guard that protects the new sinker invalidation fast path. Oracle evidence stays green at `.tmp/pass-fuzz-sl-genvalid-10000-sl004` (`10000/10000`, `0` mismatches) and the refreshed post-perf smoke lane `.tmp/simplify-locals-perf-smoke` (`100/100`, `0` mismatches). Runtime is materially better but still not signed off: the equivalent-copy phase now uses explicit equivalence classes instead of an `O(local_count^2)` matrix, sinkable tracking is sparse and dense-active instead of repeatedly clearing full local arrays, and detached-node cleanup now dedupes candidates with a precomputed label-owner map. On the local debug artifact, the 30-second traced replay now reaches `Func 409` with cumulative `pass:simplify-locals` time `7837550us` at the end of `Func 408` (down from `8131899us` before the dense-active sinkable rewrite), but the native single-pass command still exceeds `90s`, so the remaining blocker is large-function runtime rather than correctness. The next work is to cut `scan-root-region` or pending-effect cost on the `Func 409` `block` or `if` cluster and, if needed, the remaining detached-node cleanup overhead after that.
     ...
     - Reproduce 30s traced runtime checkpoint: `timeout 30s env STARSHINE_TRACING=pass _build/native/release/build/cmd/cmd.exe --simplify-locals --out /tmp/sl-artifact-trace-30s-after-dense.raw.wasm tests/node/dist/starshine-debug-wasi.wasm > /tmp/sl-artifact-trace-30s-after-dense.log 2>&1`
