...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [CP]004 - Binaryen Parity Proof And Follow-On Gaps - Close the compare loop and keep deferred oracle gaps explicit.
    ...
    - Current blocker: exact TNH / implicit-trap parity needs `HotPipelineOptions` or equivalent hot-pass policy plumbing; the GC `ref-into-if` family is follow-up work until local-type repair is representable in the current pipeline; the current default-semantics diff still shows same-region `if` placement gaps on the debug artifact, but the stable non-void fence is now narrower than before. The latest direct compare is `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-109759`. It stayed valid on the real artifact after a new pass relaxation that ignores earlier `Block` roots in non-void regions when their explicit exits stay entirely inside nested labels owned by that earlier block, while still blocking sinks past `Block` roots with escaping exits. That tighter `Block`-root rule sits on top of the already-landed `Drop`-root relaxation, improved Starshine runtime (`9754.809 ms` pass vs `10745.351 ms`; `12203.915 ms` total vs `13345.644 ms`), and keeps the old `43657` `local.set $167` mismatch family gone. But it did not move the earliest parity gap: the first `19348` `local.set $20` family is still present, the next early gaps remain around `48981`, `61077`, and `72008`, and the larger structural mismatch family around `105624` is still there too. WAT inspection of that first `19348` family now suggests the non-void fence is probably no longer the immediate blocker there: the corresponding `func $67` slice still shows multiple `local.set`, `local.get`, and later `local.tee` sites for `local $20`, so the next useful reduction likely needs to explain how Binaryen sinks that non-SFA-looking local-set or prove a narrower Starshine local-analysis condition. The current last-known-valid non-void rules are now threefold: allow reordering when the prefix up through the pushpoint has no explicit exits, even if a later root in the same region branches; ignore earlier explicit exits under `Drop` roots when deciding whether the non-void sink is blocked; and ignore earlier self-contained `Block` roots whose explicit exits stay inside nested labels owned by that block. `src/passes/code_pushing_test.mbt` now covers all three with branch-free nested/root non-void regressions, a "later branch root" regression, a dropped-result-prefix regression, and the new self-contained earlier-`Block` regression. The broader zero-result-prefix relaxation still does not hold on the real artifact: ignoring earlier zero-result branch roots reintroduced the same `Func 1977` stack underflow on `tests/node/dist/starshine-debug-wasi.wasm`, and `src/ir/hot_lower_live_repro_test.mbt` now records why that remains a blocker instead of a fix across several closer reducers. The reduced earlier-zero-result-branch prefix, the same prefix inside an outer result carrier, the simplified extracted parent-escape carrier, the closer reordered real parent-escape carrier, and even the closer real carrier after parent-err extraction plus carried-`if` demotion all still lower and validate. That last closer shape still hits the same `InvalidBranchArity(_, _, 0, 1)` cliff immediately once the parent `err` block becomes result-producing, so the real artifact still needs a richer outer rewrite step than the local carrier rewrite alone. The older correctness story still stands too: earlier attempts such as `trailing-payload-split` and `nested-only` also reintroduced `Func 1977`, traced root-order dumps still show one concrete bad move under the root-only relaxation (`LocalSet(45)#525` shifting from before `LocalSet(50)#528` / `If#534` to after `If#534`), and the remaining unsafe family still needs extra real-function scaffolding beyond the carrier extraction plus inner carried-`if` rewrite alone. The validator offset is still pinned at `br 3` to label `@8` in `Func 1977`, `src/ir/hot_lower_live_repro_test.mbt` still proves the underlying `InvalidBranchArity(_, _, 0, 1)` mechanism, and `src/passes/code_pushing_test.mbt` still guards the parent-exit payload-carry and parent-escape carrier boundaries directly at the pass layer.
