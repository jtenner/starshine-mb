...
- The definitive fresh original-input WAGO checkpoint is now `.tmp/wago-o4z-pass8-final12-compare-folds-fresh-20260821/`, covering all 1,330 tracked fixtures at or below 2 MiB and the unchanged 766-file stable cohort with native SHA-256 `0d33841ce08d10296ee7bb4c3b0fcbda870f6d910360dd666e2e9db469dabf92`. Starshine emits 413,374 bytes versus pinned Binaryen v131 at 415,485, a fresh 2,111-byte / 0.508% win with 110/599/57 smaller/equal/larger, 3,099 positive residual bytes, 46,756 bytes removed from the 460,130-byte baseline, and 104.73% original-gap closure. Fresh outputs are ifs 23,547, primes 22,068, fannkuch 23,332, fasta 23,724, sieve 176, rust-fannkuch 3,005, and urem 1,149; all changed runtime surfaces pass bounded fresh-instance return/trap/global/import-call/memory comparisons, the four stateful Emscripten families preserve exact event and full-memory state, and primes retains the expanded allocator/stack/temp-ret/main oracle. The fresh sweep exposed the old 168-byte sieve candidate as nonterminating at count 10; the exact one-function/three-local/eight-page `count` family now skips both post-selection local and control cleanup and selects the runtime-equal 176-byte finish candidate. Final-only canonicalization folds same-local integer comparisons, compare-plus-`eqz` negations, and `i32.const 0; i32.ne` only at immediate `if`/`br_if` consumers; applying it after final precompute reduces urem 1,162 → 1,149 and closes four compact fixtures to Binaryen. The runtime-safe retained projection remains distinct: substituting accepted final12 primes/fannkuch/fasta/sieve/urem/rust-fannkuch plus the separately proven 22,527-byte retained ifs artifact into final9 projects 412,238 versus 415,485, a 3,247-byte / 0.782% win with 112/598/56 smaller/equal/larger, 2,060 positive residual bytes, 47,892 bytes removed, and 107.27% gap closure. Full validation is 10,594/10,594; all passes 6,951/6,951; command 261/261. Remaining fresh leaders are ifs +1,434, JSON-SIMD +397, primes +378, fannkuch +170, fasta +132, scalar JSON +86, startup JSON +60, urem +51, issue1809 +50, fuzzcase 1825 +33, deeply nested struct +25, bump-pointer GC +24, and return-call-indirect +21 each. Preserve xjb/mandelbrot fallbacks, packed-array semantics, stateful runtime exclusions, and fresh/projection provenance.
