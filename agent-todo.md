...
- The last fresh original-input WAGO sweep remains final7 over all 1,330 tracked fixtures at or below 2 MiB and the unchanged 766-file stable cohort. The current runtime-safe final9 retained-artifact projection is `.tmp/wago-o4z-pass8-final9-safe-projected-20260821/`: 413,483 Starshine bytes versus pinned Binaryen v131 at 415,485, a 2,002-byte / 0.482% win with projected 112/597/57 smaller/equal/larger, 3,302 positive residual bytes, 46,647 bytes removed from the 460,130-byte baseline, and 104.48% original-gap closure. Native SHA-256 is `de2470509283618793ca6103b0e64ce33a5c14c3e67c4fae1436fb3a56b19a67`. The projection replaces 20 independently validated/runtime-classified artifacts and is not a fresh original-input sweep. Largest wins are ifs 23,632 → 22,563 with exact event/trap/full-memory state, `isa_call` 210 → 132, Winch globals 870 → 850, defined/imported memory-copy 243 → 224 and 231 → 212, and table-grow 127 → 116. Heap2Local now correctly applies i8/i16 signed and unsigned storage conversion when mutable packed arrays are scalarized into raw i32 locals; final8's 128/255 and 32768/65535 wrong returns are repaired to -128/-1 and -32768/-1, with the two fixtures byte-identical to Binaryen at 90 and 95 bytes. Broad replay discoveries are not admitted without exact behavior: primes changed the main trap and memory, fannkuch/fasta changed `stackAlloc`, sieve began hanging at count 10, and bump-pointer GC changed a snapshot comparison to a self-comparison. Fuzzcase 1825's known external validation failure and the linked-list reoptimization LocalSubtyping/HotLift abort remain open. Full validation is 10,575/10,575; all passes 6,945/6,945; command 183/183; pass manager 335/335; Precompute 107/107; Heap2Local 22/22. Explicit-v131 Heap2Local smoke is regular 1,000 normalized, dedicated 244 normalized plus 756 established smaller debris residuals with no H2L-operation drift, wasm-smith 997 normalized plus three Binaryen failures, and bounded random-all 85 normalized plus 15 smaller cross-profile residuals with no packed get drift. Remaining leaders are primes +1,472, ifs +450, JSON-SIMD +397, fannkuch +219, fasta +181, scalar JSON +86, urem +64, startup +60, issue1809 +50, fuzzcase 1825 +33, deeply nested struct +25, bump-pointer GC +24, and return-call-indirect +21 each. Preserve xjb/mandelbrot fallbacks, packed-array semantics, stateful runtime exclusions, and fresh-sweep provenance.
