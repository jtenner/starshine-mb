...
- **Latest bounded iteration (2026-07-15):** commits `b789c2ff7`, `a2ce97352`, and `7af372b56` add a resultless synthetic catch-all `Try` lowering bridge, replace six bounded call-argument recognizers with one recursive distinct one-use collector, admit direct `i32.sub`, and reuse recognized suffix ownership results. A detached three-probe matrix now lowers, encodes, validates, and runs current Starshine output. Aggregate bytes are `204` input, `263` Starshine direct, `255` Starshine cleanup, `275` Binaryen direct, and `236` Binaryen cleanup; Starshine's matched cleanup remains 19 bytes larger and is a size-losing parity gap. Refreshed pass-only timing improved from `16,880 us` baseline to `3,682.5 us`, but remains `13.84x` Binaryen v130's `266.05 us`. Typed catches/payloads, nested-pop repair, `rethrow`, `delegate`, structured suffix control deletion, broader behavior, a GenValid profile, four-lane signoff, cleanup parity, performance, and public wiring remain open. Source: `docs/wiki/raw/binaryen/2026-07-15-flatten-version-130-internal-output-recursive-ownership-impact.md`.
- **Deliverables:**
