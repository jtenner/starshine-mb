...
- [DAE]006 - Both-Canonical Frontier Advancement
  ...
  - Current classification:
    ...
    - `0582-2026-05-25-dae-func505-underscore-guard-reduction.md` reduced the underscore guard polarity around `Func4495` and classifies that local subshape as representation-only control inversion: Starshine's `if (predicate) br K else BODY` and Binaryen's `if (eqz predicate) BODY` call the predicate once, preserve the parse body effects, and join at the same continuation. No pass behavior or compare normalizer changed; the overall Func505 frontier remains unknown/risky because loop induction/exit-carrier and overflow/error-return/temp-local drift are still live.
  - Next action:
    - Reduce one remaining Func505 live region before changing pass logic or compare tooling: loop induction/exit carrier or overflow/error-return construction and temp-local reuse.
