...
- [SGO]003 - Binaryen `SimplifyGlobals.cpp` Breadth Coordination
  ...
  - Completed evidence: landed behavior and refactor slices are recorded in `docs/wiki/raw/research/0574` through `0632` where applicable, `docs/wiki/log.md`, and the SGO parity/readiness pages. Do not duplicate the full completed slice history here; use those docs as the source of truth.
...
- [SGO]003A - Next-Slice Probe Inventory Refresh
  - Status: active; start here before behavior-bearing implementation if no exact target is already chosen.
  - Goal: refresh the post-0632 gap list from Binaryen `SimplifyGlobals.cpp`, official lit files, and current Starshine tests so the next implementation batch is source-backed rather than inferred from analogy.
  - Tasks:
    - Compare current `src/passes/simplify_globals_optimizing.mbt` matchers against `docs/wiki/binaryen/passes/simplify-globals-optimizing/parity-matrix.md`.
    - Identify exact positive/negative fixture pairs for the next 2-3 implementation slices.
    - Separate behavior gaps from refactor-only cleanup and parser/tool blockers.
  - Exit criteria: a short research note with ranked candidate slices, each with exact fixture shapes, expected Binaryen behavior, local current behavior, and risk classification.
...
- [SGO]003C - Read-Only-To-Write Nested If Arm Breadth
  - Status: active candidate; behavior-bearing.
  - Goal: broaden nested `if (result ...)` arm-flow positives only where both arms are independently safe and the candidate-derived value is not consumed by effects/traps.
  - Candidate shapes: additional arm-local clean effects already supported by FlowScanner, transparent arm-local result blocks, supported pure/ref/numeric post-consumers after the nested-if result, and exact no-else recursive nested patterns with source support.
  - Required negatives: candidate steers the inner if, candidate feeds trapping loads/table.get, candidate feeds call operands, unsupported post-consumers, arm branches/returns, nested `else` forms not source-probed, and global-derived `local.set` / `local.tee`.
  - Exit criteria: batch tests covering condition/first-arm/second-arm placements, paired caught/trapping negatives where applicable, direct SGO fuzz, matrix update.
...
- [SGO]003D - Read-Only-To-Write Safe Side-Effect Independence
  ...
  - Candidate shapes: remaining clean store/table/bulk forms if exact Binaryen positives exist, additional scalar-load/table.get independence forms, and recursive combinations with already-supported pure prefixes.
...
- [SGO]003E - Read-Only-To-Write Call And Effect Boundary Study
  ...
  - Tasks:
    - Probe ordinary direct calls with no candidate operands, wrong-global mutations, candidate-global reads, and generated-effect-like callees.
    - Confirm whether calls can ever count as local syntactic read-only-to-write events or should remain runtime-trace-only barriers/fact invalidators.
    - If positives exist, design an effect summary that tracks reads as well as writes before implementation.
  - Required negatives: imported calls, indirect calls, `call_ref`, `return_call`, recursive cycles, callee reads candidate global, callee mutates candidate global, and candidate-derived call operands.
  - Exit criteria: research note and either explicit deferred status or a scoped implementation plan with new analysis APIs.
...
- [SGO]003I - Runtime Function-Effect Summary Reads/Writes Upgrade
  - Status: active research candidate; dependency for broader call behavior.
  - Goal: decide whether SGO needs callee global-read summaries in addition to the current global-write summaries.
  - Tasks:
    - Audit current `direct calls clear only mutated globals` implementation.
    - Probe Binaryen call-negative/generated-effects cases for read-only-to-write and runtime trace separately.
    - Design fixed-point summaries for reads/writes/imported/escaping calls if needed.
  - Exit criteria: research note and, if accepted, tests for cycles/recursion, wrong-global read/write, imported-call barriers, and generated-effect boundaries.
...
- [SGO]003N - Nested Cleanup Wrapper Guard Frontier
  - Status: active blocker for some SGO nested cleanup breadth; overlaps `[SGO]004` but belongs here when it blocks behavior slices.
  - Goal: remove or narrow the current SGO wrapper guard that filters value-block/control touched functions out of nested cleanup, without reintroducing HOT cleanup verifier failures.
  - Tasks:
    - Reproduce the current verifier frontier with minimal touched functions.
    - Identify whether the fix belongs in SGO scheduling, HOT lift/lower, or a nested cleanup pass.
    - Add focused regression before lifting any guard.
  - Exit criteria: guard narrowed with validation evidence, or documented as `[SGO]004` runtime/scheduler-only follow-up.
