...
- [RSE]002 - CFG Value Flow And Refined Local Gets
  - Current status: 2026-05-10 added branch-disagreement merge identities for self-set folding, body-local default identities for redundant default writes, raw strict-subtype equivalent-local `local.get` retargeting, raw preservation of fallthrough facts after one-armed terminating `if`s, paired vacuum cleanup for nested pure `drop` / `nop` debris, vacuum empty-then/live-else inversion, raw structured block/if label-exit merge tracking with unreachable-tail skipping so `br_if` block exits no longer misfold final const sets, GC conditional branch-exit recording for `br_on_null` / `br_on_non_null` / `br_on_cast` / `br_on_cast_fail`, HOT block/if label-exit merge tracking with reachable fallthrough sources so agreeing exits still fold while bypassing exits keep final sets alive, raw/HOT branch-free loop fallthrough facts so post-loop same-value sets can fold when no loop backedge is seen, identity-preserving refinement wrappers for raw `ref.as_non_null` / `ref.cast` / `ref.cast_desc_eq` so concrete-heap refined gets now retarget through straight-line and branch-merge shapes, programmatic lowered-instruction coverage for `ref.cast` / `ref.cast_desc_eq`, `rse-gc.wast` `needs-refinalize`-style raw fixtures where redundant tee removal retargets following `struct.get` / `array.get` accessors to the refined subtype, raw string.const value identities for repeated string local writes, and explicit conservative loop-backedge / outer-exit regressions where post-loop const sets survive. Direct compare-pass stayed semantic-green at `.tmp/pass-fuzz-rse-rse002-string-const` (`6759/10000`, `0` mismatches, `20` Binaryen/tool command failures).
  - Remaining deliverables: replace the documented conservative loop-backedge skips with fixed-point convergence where parity needs it, extend the same structured label start/end identities through the remaining HOT/control families beyond block/if and the raw GC conditional-branch lanes, and broaden remaining `rse-gc.wast`-style type-refinement coverage beyond the reduced abs-heap plus concrete-heap wrapper and aggregate-accessor refinalization fixtures.
