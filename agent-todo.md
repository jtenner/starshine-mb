...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   - [PC]001 - Constant Folding Surface - Audit the exact constant, tuple, and local-evaluable fragments Binaryen folds in the top-level `precompute` slots for `-O` / `-Os`.
     ...
     - Current blocker: the repaired `gen-valid` smoke lane is now green after root-level dead-drop rewrite, root-`nop` cleanup, and root empty-block collapse, with `.tmp/pass-fuzz-pc-genvalid-100-rerun8` at `100/100` compared and `0` mismatches. The next gate is the gold-standard `10k` compare run plus the debug artifact replay, which still remains canonically unequal and much slower (`~119595 ms` vs Binaryen `~1745 ms`).
