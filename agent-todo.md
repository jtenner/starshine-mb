...
- [DAE]002 - Nested Post-Inlining Cleanup and Artifact Compare
  - Status: active next slice after `[DAE]001` completion. The pass emits a trace marker for the required `precompute-propagate` prefix, but intentionally does not run the whole-module cleanup lane because that rewrote untouched functions and worsened direct parity. Artifact replay `.tmp/dae-optimizing-artifact-replay-finish2` stays semantically red at first differing function `defined=11 abs=28` while Starshine pass-local time was faster (`665.946ms` vs Binaryen `935.447ms`).
  - Deliverables: implement a touched-function scheduler that prepends `precompute-propagate` before rerunning the default function pipeline only on Binaryen's touched set; add nested-run scheduler tests; compare `--dae-optimizing` output on the debug artifact; keep direct-pass semantic regressions visible if a broader fuzz run discovers a new non-local-declaration mismatch.
