...
- `RemoveUnusedModuleElements`: closed-world typed `struct.new` / `struct.new_desc` and ref-bearing `array.new` / `array.new_fixed` / `array.new_elem` payloads now defer unread funcref/global/elem references until a live `struct.get*` or `array.get*` read occurs; remaining GC precision is `array.new_data`, broader GC content readers, and any additional hardening around reference-only non-function retention.
