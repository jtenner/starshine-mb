...
- [FUZ]1021 - Invalid Binary Malformed-Byte Matrix
  - Status: IN PROGRESS (cron 23, 2026-05-21). Latest atomics operand-context slice adds decode-rejected, non-smoke `malformed-atomic-load-memarg-offset-uleb` and `malformed-atomic-store-memarg-offset-uleb`, which emit valid `0xFE 0x10` `i32.atomic.load` / `0xFE 0x17` `i32.atomic.store` subopcodes plus a valid alignment byte before corrupting the following memarg offset ULEB; docs now name atomics alignment and offset malformed-byte coverage. Previous atomics load/store align memarg, atomics reserved-subopcode byte, section-payload underflow, atomic-prefixed subopcode malformed/overwide, overwide section vector-count, malformed vector-count, and smoke-scheduler slices are landed and documented; continue FUZ1021 by selecting another malformed-byte class such as remaining invalid UTF-8, opcode/prefix, lane, overwide atomics operands, or additional atomics operand-context corruptions. Current TDD evidence: focused invalid-binary wbtest first failed on missing `malformed-atomic-load-memarg-offset-uleb` / `malformed-atomic-store-memarg-offset-uleb`, then `moon test src/fuzz` passed after implementation.
