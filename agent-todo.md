...
- **Deliverables:**
  - [ ] Match represented Binaryen constants: numeric/SIMD, `ref.null`, `ref.func`, `string.const`, supported ref conversions, tuple constants, and immutable `global.get`. *(uniform `v128.const`, `ref.func`, `ref.i31`, and externalized/re-internalized i31 conversion expressions now use the generic value-slice/materialization path with exact structural equality; broader reference literals and tuple constants remain)*
