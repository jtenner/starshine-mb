...
- [FUZ]1021 - Invalid Binary Malformed-Byte Matrix
  - Status: IN PROGRESS (cron 23, 2026-05-21). Latest atomic-prefixed subopcode slice adds decode-rejected, non-smoke `malformed-atomic-prefix-subopcode-uleb` and `overwide-atomic-prefix-subopcode-uleb` strategies over the `0xFE` atomics prefix, updating FZG041 replay coverage plus binary/validate docs. Previous overwide section vector-count, malformed vector-count, and smoke-scheduler slices are landed and documented; continue FUZ1021 by selecting another malformed-byte class such as remaining section boundary/payload-length, invalid UTF-8, opcode/prefix, lane, memarg, or atomics operand-context corruptions. Current TDD evidence: focused invalid-binary wbtest first failed on missing `malformed-atomic-prefix-subopcode-uleb`, then passed after implementation.
