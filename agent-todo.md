...
- [O4Z-AUDIT-LCSE] - Deep audit `local-cse`
  ...
  - Deliverables: apply the common checklist; keep the covered before-`if`/then positive, after-`if` and else-arm negatives, before-loop/loop-body negative, and tiny-root repeated-`global.get` no-op green; add remaining hard control-boundary negatives, GC/generative-root negatives, and idempotent-call positives where local annotation plumbing can model Binaryen safely; coordinate shape tests with `[AUDIT004-J]`/`[AUDIT004-K]`; classify missed CSE opportunities by barrier type.
