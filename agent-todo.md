...
0. Shared blocker on canonical artifact parity.
  - [HOT]001 - Post-SSA Hot-Pipeline Replay Hardening - The large-artifact `DFE -> RUME -> MP -> OR -> GR -> GSI -> SSA` prefix now completes, validates, and canonicalizes under Binaryen: `ssa-nomerge` no longer aborts on synthetic predecessor copies, duplicate dead-local cleanup, or loop fallthrough result lowering, and the first detached-shared-subtree DCE abort is also fixed. The remaining ordered full-chain blocker has moved downstream: after post-SSA cleanup passes mutate the module, a later hot pass aborts while re-lifting the emitted function body in `hot_lift_func`, so compare signoff is now blocked on hot-lift / later-pass replay rather than SSA or Binaryen canonicalization of the post-SSA artifact.
     ...
     - Current blocker: post-prefix `--ssa-nomerge` replay now additionally skips raw no-write, straight-line local-write, and structured local-write functions, reuses linked label stacks and owned typecheck state during hot lift, trims SSA destruction/local placement churn, and lowers without extra hot verification under final-module-only policy. The suite is green, and direct native replay on `.tmp/self-compare-prefix-6/starshine.raw.wasm` is down to about `18.8 s`, but large-artifact replay is still over budget because a much smaller set of very large functions now dominates wall time, concentrated in a `br`-heavy nested `block` / `if` cluster plus one loop / `br_table` outlier; grouped hot-pass batching is still not safe because later cleanup passes still expose re-lift verification failures.
...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [DCE]002 - Prefix Regression and Artifact Replay - Lock the corrected DCE behavior into focused tests and rerun the MoonBit debug-artifact compare harness.
     ...
     - Status: focused DCE regressions plus follow-up hot-lift/effects fixes landed; replay now gets past the earlier `func[80]` / `func[106]` hot-lift aborts, the typed-loop lowering bug that broke canonical `--vacuum` replay at `func[81]` is fixed, the dead-result wrapper failures at `func[67]` and `func[190]` are closed by keeping typed wrappers concrete when DCE would otherwise retag dead `block` results, Binaryen-facing `ref.null` encoding now roundtrips with heap-type immediates, and detached shared-subtree cleanup no longer aborts on parent-first deletion order. The remaining ordered-chain blocker is no longer that DCE cleanup crash; it has moved to a later hot-lift abort while re-lifting the post-cleanup artifact.
     - Doc: [0066#L178](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L178)
3. Do work.
