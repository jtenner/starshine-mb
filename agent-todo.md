...
- The current unreduced artifact frontier now starts at absolute `Func 71`, `_M0FP37jtenner9starshine3cmd31run__cmd__with__adapter_2einner` (WAT `$50`) in `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-1593005`. Binaryen is still folding more tee-backed same-arm loads/stores and surrounding call-result temporaries there, while Starshine keeps extra `local.tee` / `local.set` scaffolding.
- The follow-on `hot_lower` dead nested `local.tee` pruning and raw adjacent dead `local.set` / `local.get` cleanup are now covered in-tree, but they were not shown to move the full artifact frontier and should stay secondary to the next real parity reduction.
