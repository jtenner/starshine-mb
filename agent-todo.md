...
- [DAE]006 - Both-Canonical Frontier Advancement
  ...
  - Current classification:
    ...
    - `0587-2026-05-26-dae-func509-return-suffix-cleanup-attempt.md` added focused root-return and terminal-wrapper cleanup tests plus narrow DAE cleanup helpers, but the successor replay `.tmp/dae006-wrapper-suffix2-20260526` still first-diffs at `defined=509 abs=526` (`2829.276ms` Starshine pass versus `841.003ms` Binaryen pass). Trace spot-checking shows no final-return-suffix cleanup hit on the debug artifact, so the helper misses the actual pre-encode flat Func509 shape.
  - Next action:
    - Instrument or reduce the actual pre-encode Starshine `@lib.Instruction` sequence for Func509, then replace or extend the focused Moon regression to match that exact flat shape.
    ...
    - Rebuild the native CLI and rerun the both-canonical artifact compare to discover and classify the successor frontier.
  - Deliverables:
