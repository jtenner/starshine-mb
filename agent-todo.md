...
- **Deliverables:**
  ...
  - [ ] Lane-wise result LUB plus call, tee, parent-control, and caller refinalization. *(productive GC result commits now construct and schedule epoch-bearing `DaeControlReconstructionWork` and `DaeTypeRepairRetryWork` followups from producer effects; broader lane and caller repair remain open)*
