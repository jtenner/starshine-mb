...
- **Status:** **Active; `merge-blocks`, `once-reduction`, and `local-subtyping` renewed.** Implementation and red-first deterministic regression work are complete for `merge-blocks`, `once-reduction`, `memory-packing`, `local-subtyping`, `remove-unused-brs`, `precompute` / `precompute-propagate`, full `ssa` plan application, and the raw merge-locals cleanup. On 2026-07-26, `local-subtyping` completed a fresh v131 source/lit audit and required matrix: regular `100000/100000`, dedicated `10000/10000`, random-all `10000/10000`, and wasm-smith `9955` exact plus one cleanup-normalized match across `9956` comparable cases, with only 44 classified Binaryen-v131 tool failures. Remaining pass matrices stay open below.
