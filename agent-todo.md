...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [RUB]002 - Multi-Slot Cleanup and Artifact Compare - Replay the pass in each Binaryen slot and confirm later `merge-blocks` opportunities match the reference output.
     ...
     - Current blocker: the top-level scheduler gap is now fixed: `optimize` and `shrink` both replay `remove-unused-brs` in three modeled slots, focused preset tests cover those trace counts, and the old `InvalidLabelOwner(4, 16)` serial-verify failure is fixed in-source by preserving label ownership when value-`if` rewrites demote to blocks. The remaining gate is replaying `tests/node/dist/starshine-debug-wasi.wasm` through a freshly built `cmd` binary and resuming the real artifact compare.
     - Doc: [0066#L188](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L188)
   - [IR]003 - Lifted Label Ownership Repro for RUB Artifact Gate - Reduce `Func 24` from the MoonBit debug artifact to a standalone `hot_lift`/verify repro so RUB artifact compare can resume.
     ...
     - Status: the in-tree repro now lives in `src/passes/remove_unused_brs_test.mbt` as the malloc-shaped after-each-pass verifier case, and the label-owner corruption is fixed by demoting rewritten value-`if` nodes into labeled blocks instead of deleting their holder regions. The remaining work is fresh-binary artifact replay, not another standalone reducer.
