...
- Starshine wasm artifact implemented-pass comparison:
  ...
  - fresh rebuilt release-artifact status:
    ...
    - adding `DeadCodeElimination` after that four-pass prefix is the first fresh-artifact later-pass divergence: Starshine remains byte-identical to the `... -> OnceReduction` output, while Binaryen trims `4` bytes from the `code` section and removes dead `drop`s after unconditional `br` tails in at least two branch-heavy functions.
