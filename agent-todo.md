...
- `[DAE]004` mid-size fact-driven dropped-result batching advanced on 2026-05-26 by research note `0604`: the bounded fact-driven candidate queue now runs for modules up to `2048` defined functions before the handpicked selected-def fallback, still using current direct-call facts and the caller-filtered result-removal helper. Focused regression `dae-optimizing removes high fact-discovered dropped callee result outside selected list` failed before the guard widening (`1 != 0`) and now passes. The guarded debug-artifact timing replay `.tmp/dae004-mid-fact-timing-20260526` stays inside the `Starshine <= 2x Binaryen` target (`1439.590ms` vs `914.804ms`), while an unguarded experiment did not complete cleanly; the huge debug artifact therefore remains on the closed DAE011 selected-list path, and `[DAE]004` remains open until artifact/fuzz evidence proves the selected fallback can be removed or no remaining dropped-result scheduling gap exists.
