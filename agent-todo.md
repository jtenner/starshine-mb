...
- [O4Z-AUDIT-DAE] - Deep audit `dae-optimizing`
  ...
  - Current finding: research notes `1562-2026-07-12-dae-large-module-nested-cleanup-guard-relaxation.md` and `1563-2026-07-12-dae-func3737-merge-blocks-drop-carrier.md` remove the coarse `>100` defined-function nested-cleanup skip, fix the folded-multivalue Func3737 shared-drop-carrier `merge-blocks` abort, and refresh the high-defined-function DAE fixtures to assert durable post-cleanup semantics. `moon test src/passes/dae_optimizing_test.mbt` and `moon test src/passes` are green again; the audit remains open for broader missing behavior families rather than this pass-suite blocker.
  - Deliverables: apply the common checklist; coordinate with `[AUDIT003-*]`; refresh DAE-normalized direct compare and late `DAE` slot evidence; keep accepted raw-cleanup drift separate from semantic mismatches; continue shrinking the remaining broad gaps (real `precompute-propagate`, fuller default-function replay, broader operand localization, GC param refinement, result refinement, and selected/artifact genericization) before claiming broader large-module signoff.
