...
- [FUZ]1012 - Randomized Coverage-Forced Templates
  ...
  - Current evidence: `moon test src/validate` passes with regressions proving coverage-forced template signatures are deterministic for the same seed, differ across selected seeds, preserve numeric/control floors, include both memory-before-table and table-before-memory forced prelude layouts across a bounded seed window, and now include both `simd1|simd2` and `simd2|simd1` SIMD phase orders while preserving `uses_v128`. Docs note the seed-deterministic branch/loop, memory/table, and SIMD phase-order variation.
  - Remaining: randomize additional forced families beyond the branch/loop, memory/table, and SIMD phase templates (for example ref/call template variants), add explicit template-family diagnostics or manifest-visible facts if the public ledger needs them, then rerun `moon test src/fuzz` / relevant fuzz smoke before closing.
