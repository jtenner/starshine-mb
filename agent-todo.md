...
- **Deliverables:**
  ...
  - [ ] Move GC refinement, result removal, control reconstruction, and remaining older paths through immutable validated plans. *(dispatcher families, optimizing unread/dropped/GC batches, ordinary-core GC parameter refinement and dropped-result removal, reverse/low exact revisits, low exact cleanup, broad immutable-global and adjacent-root revisits, dead-suffix parameter restoration, and boundary cleanup now cross scalar/component/family validation; the general remove/constant entry is scalar-plan-backed; producer-specific suffix/ref-null, GC/result, and control/type mutation helpers remain)*
...
- **Status (2026-07-22 partial):** committed work-item effects drive dependency requeue by explicit invalidation domain across before/after graphs. Result invalidation uses exact undropped observer/direct-tail rows, metadata-only changes remain local, and common work families publish narrow parameter/materialization, result, and control-only domain sets. Dispatcher unread/localization now reuses the supplied immutable original/current graph for its authoritative guard instead of reconstructing boundary facts. Stack-carried void-effect recovery is a normal scalar-plan option with exact producer effects; the former manual retry candidate and synthesized effect record are deleted. Producers without domains retain the conservative caller/callee fallback; legacy result/type/body/cleanup producers and diagnostics still need migration.
- **Deliverables:**
