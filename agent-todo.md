...
- [CF]002 - Late-Slot Regression and Artifact Compare
  - Current status: further narrowed on 2026-05-10. Direct `code-folding` now covers typed/value `if` suffix hoists, branch-to-outer-label full-tail hoists, branch-free structured suffix hoists, unprofitable full-void `if` skips, conservative block-exit/fallthrough tail hoists, and branch-payload embedded block-exit hoists, with safety guards for live block labels / result-region trailing `unreachable`. Direct fuzz remains semantic-green at `.tmp/pass-fuzz-code-folding-cf002-defined220-wip` (`6759` compared, `0` mismatches, `20` Binaryen command failures). The debug artifact `--code-folding` command still differs first at `defined=220 abs=237`, but the visible repeated `call $28(local.get $6)` branch-exit tails have been shared; the remaining diff is helper-block / value-carrying branch-payload wrapper shape. Latest pass-local timing is not yet acceptable (`644.393ms` Starshine vs `181.997ms` Binaryen; > 2x), so `[CF]002` stays open for both exact helper-block parity and pass-local runtime recovery.
  - Remaining deliverables: close or classify the remaining `defined=220 abs=237` helper-block / branch-payload wrapper diff; recover pass-local speed to the repo floor (`starshine_time <= 2 * binaryen_time`) or document an accepted exception; implement function-ending helper-label sharing as needed; keep branch-scope and EH movement guards conservative; rerun late-pipeline slot and `--code-folding` artifact parity.
