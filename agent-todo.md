...
- **Status (2026-07-22 partial):** canonical direct-call operand slices, canonical parameter-slot actions, and ordinary unused-parameter analysis migration landed. The scalar and forwarding plans carry complete candidates, exact call identities, type/member/caller epochs, original ownership, validation, and rollback. The family dispatcher now also wraps unread/localization, uniform, GC parameter/result, dropped-result, control-reconstruction, and type-repair outputs in one immutable `DaeFamilyRewritePlan` envelope before commit. Legacy direct/selected entry points, broader producer migration, metadata finalization, and complete touched/invalidation ownership remain open.
- **Deliverables:**
  ...
  - [ ] Move GC refinement, result removal, control reconstruction, and remaining older paths through immutable validated plans. *(all dispatcher families now cross scalar/component/family plan validation; legacy direct/selected constants, broad GC/results, and broad localization remain)*
