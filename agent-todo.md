...
- The current unreduced artifact frontier still starts at absolute `Func 71`, `_M0FP37jtenner9starshine3cmd31run__cmd__with__adapter_2einner` (WAT `$50`) in `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-672403`. Binaryen is still folding nested tee-backed same-arm loads/stores and surrounding call-result temporaries there, while Starshine keeps extra `local.tee` / `local.set` scaffolding.
- The next parity work should stay focused on a real `Func 71` reduction instead of more generic cleanup passes: the root-level dead-tee writeback cleanup and the dead adjacent set/get cleanup both landed, but the opening `$269` / `$273` / `$276` drift in that function is still unreduced because it depends on non-adjacent nested exact-expression cleanup, not top-level lowered debris.
