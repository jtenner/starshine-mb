...
- DeadCodeElimination research baseline:
  ...
  - current DCE blocker after that fix: the fresh release artifact still fails later during direct `--dead-code-elimination` / five-pass replay at `Func 716` / `Func 695`, with the remaining diff still centered on a nested `block bt=I32` carrying `br (Label 1)` values through result-typed `if` control.
  - next step: reduce that remaining artifact-local `Func 716` / `Func 695` shape below the current generated-path nested branch-payload repro, then decide whether the next fix belongs in DCE block/if truncation or in typed-to-raw lowering for surviving branch-payload control.
...
## v0.1.0 Active Slice Focus
  ...
  - blockers:
    ...
    - direct native replay now gets past the earlier `Func 27` raw nested-`if` merge bug, the `Func 227` DCE pre-effect truncation bug, and the later `Func 313` / `Func 315` dead block-exit propagation bug, but it is still blocked before a trustworthy parity check by the next post-encode validation failure in `Func 531` for direct `--dead-code-elimination` replay and `Func 527` for the five-pass shared prefix on `_build/wasm/release/build/cmd/cmd.wasm`.
  - risks:
  ...
  - implementation features:
    ...
    - direct native replay still first fails `--dead-code-elimination` on `_build/wasm/release/build/cmd/cmd.wasm` at `func 448` (`expected i32 but nothing on stack`), with the five-pass shared prefix failing in the same family at `func 444`.
    - the next step is still to reduce that remaining valid payload-loss shape to a generated-path regression, then rerun the fresh artifact parity checkpoint once the new first blocker is fixed.
