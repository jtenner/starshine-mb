...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [RUB]002 - Multi-Slot Cleanup and Artifact Compare - Replay the pass in each Binaryen slot and confirm later `merge-blocks` opportunities match the reference output.
     ...
     - The larger `gen-valid` rerun is now green too: `bun scripts/pass-fuzz-compare.ts --pass remove-unused-brs --generator gen-valid --count 1000 --max-failures 20 --out-dir .tmp/pass-fuzz-rub-genvalid-1000-rerun` => `1000/1000` normalized matches, `0` mismatches, `0` command failures.
     - Next oracle step: spend the next fuzz budget on broader generator coverage or a larger `gen-valid` budget instead of the now-retired saved mismatch list.
     - Doc: [0066#L188](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L188)
