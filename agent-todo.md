...
- [O4Z-AUDIT-SSA] - Deep audit `ssa-nomerge`
  ...
  - Finding: `docs/wiki/raw/research/0722-2026-06-09-ssa-nomerge-exceptional-edge-audit.md` fixed a true try_table/throw corruption caused by applying normal-flow-only local SSA across exceptional edges. `ssa-nomerge` now skips exceptional-flow HOT mutation, and the raw no-write path now recursively materializes default body-local reads across branchy control and cleans dropped-unreachable debris before hard `unreachable` tails. Follow-ups keep nested structured param/body-local writes on canonical slots when the next observable use reads before overwrite, avoid dead/unneeded branch and label merge copies, narrow the `Func 4302` suspicious-carrier false positive, and raise the structured raw budget enough to reduce direct artifact `large-structured-local-writes` skips from `2117` to `17`. Validation: focused `ssa_nomerge_test` `33/33`, `moon test src/ir` `251/251` earlier in the slice, latest `moon test src/passes` `2052/2052`, full `moon test` `5247/5247` earlier in the slice, native `src/cmd` build passed with pre-existing warnings, latest direct 10000-case compare `.tmp/pass-fuzz-ssa-nomerge-canonical-local-10000` compared `9977/10000` with `9977` normalized matches and `0` mismatches, initial final `.tmp/pass-fuzz-ssa-nomerge-final-100000` exposed `3` semantic-safe unreachable-debris mismatches, and post-cleanup final `.tmp/pass-fuzz-ssa-nomerge-final-100000-after-debris` compared `99751/100000` with `99751` normalized matches, `0` mismatches, and `249` Binaryen/tool command failures. Latest direct artifact replay `.tmp/self-ssa-nomerge-debug-wasi-canonical-local` is still not exact-normalized/canonical-function equal (`defined=0 abs=27`) but is size-winning for Starshine (`3150432` bytes vs Binaryen `3155990`) with pass-local time `26.731 ms` vs Binaryen `392.500 ms`.
  - Remaining deliverables: either accept with explicit closeout criteria or further reduce the remaining exact artifact first diff after `.tmp/self-ssa-nomerge-debug-wasi-structured-5376` (`defined=0 abs=27`, agent-classified as representation/local-allocation drift: Starshine uses fewer body-local temporaries than Binaryen in the same allocator-style function, with both artifacts validating); keep the remaining `11` huge `large-structured-local-writes` functions in backlog until measured raw-structured rewrite cost and validation evidence exists (2026-06-11 trace now leaves `Func 265`, `Func 502`, `Func 3518`, `Func 3536`, `Func 3781`, `Func 3885`, `Func 4119`, `Func 4522`, `Func 5417`, `Func 5418`, and `Func 5419`; `Func 3668` and `Func 1265` were admitted by the `4608` branch budget, and `Func 2114`, `Func 3139`, `Func 95`, and `Func 3554` were admitted by the `5376` branch/no-branch budgets after extracted validation and full-artifact trace validation); replay early `SSA` slot / artifact neighborhoods; decide whether the current O4z `ssa-nomerge` no-op remains intentional or can be narrowed; resolve or route the current `-O4z` debug-artifact abort in a later `remove-unused-names` `Func 2560` neighborhood before using preset evidence to re-enable SSA; file pass-local runtime fixes here and whole-command residuals under `[WALL]001`.
  - GenValid coverage note: 2026-06-10 widened the `ssa-nomerge-coverage` profile and feature ledgers for structured early-return local traffic, block-target `br`/`br_if` merge-copy candidates, structured param merges, and branch-local writes that are not read after the join. 2026-06-11 added `ssa-nomerge-smoke` as a quick middle profile that emits those structured branch shapes without dense loop/br_table/parallel-copy/fail-closed stress slices. A 2026-06-11 pass-behavior follow-up narrowed the unconditional block-target `br` merge-copy family by writing immediate `local.set; br` exits directly to the canonical merge slot when the continuation needs that local; the smoke lane remains dirty (`100/100` representation mismatches) because broader structured local allocation and `br_if` lowering drift remains. The latest GenValid coverage slice made dense `ssa-nomerge-coverage` / `ssa-nomerge-stress` emit positive typed loop-param/backedge and `try_table`/`throw_ref` exceptional fail-closed specimens, and feature-guided batch tests can now require every dense SSA fact (loop, br_table, parallel-copy/cycle, unreachable carrier, dropped increment guard/reversed, typed-loop, exceptional, nested-CFG, and nested result-if variants). `ssa-nomerge-parity` remains limited to A–E, and `ssa-nomerge-smoke` remains quick structured-branch signal only. This improves future direct compare input variety but does not close the remaining artifact/preset deliverables above.
