...
- [O4Z-AUDIT-LCSE] - Deep audit `local-cse`
  ...
  - Current finding: direct 2026-06-04 post-fix 10000-case compare was semantically green (`6768` normalized matches, `20` Binaryen/tool command failures, `0` mismatches); later return-boundary and `br_table` boundary coverage runs also stayed green (`6771` normalized matches each, `20` Binaryen/tool command failures each, `0` mismatches). Debug-WASI pass-local timing still cleared the 2x Binaryen budget after the before-`if` into `then` reuse fix.
  - Deliverables: apply the common checklist; keep the covered before-`if`/then positive, after-`if` and else-arm negatives, before-loop/loop-body, return-boundary, and `br_table` boundary negatives, and tiny-root repeated-`global.get` no-op green; add remaining hard control-boundary negatives, GC/generative-root negatives, and idempotent-call positives where local annotation plumbing can model Binaryen safely; coordinate shape tests with `[AUDIT004-J]`/`[AUDIT004-K]`; classify missed CSE opportunities by barrier type.
