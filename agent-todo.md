...
- [AUD]001 - Fresh Direct-Pass Mismatch Triage
  ...
  - Passes with active problems: `global-refining`, `memory-packing`, `precompute`, `remove-unused-brs`, `ssa-nomerge`.
  ...
  - Current mismatch families from the audit: `global-refining` nullability/type-tightening drift, `memory-packing` data-segment normalization drift, `precompute` dead `block` / `br_table` cleanup drift, `remove-unused-brs` shared dead branch-wrapper cleanup drift, and `ssa-nomerge` temp-local shaping drift.
