...
- [O4Z-AUDIT-CF] - Deep audit and widen `code-folding` to Binaryen behavior parity
  ...
  - Deliverables:
    ...
    - [ ] `[O4Z-AUDIT-CF-K]` Refresh oracle signoff after every behavior-widening batch: direct `--pass code-folding --count 1000` first, then `10000` before closing; classify each mismatch as semantic-safe/size-winning, representation-only, size-losing, unknown/risky, validation failure, tool/Binaryen failure, or true semantic mismatch with rationale. Progress: the invalid-output fix lane ran a 100000-case direct smoke at `.tmp/pass-fuzz-code-folding-100000-after-td-fixes` with `0` Starshine command/validation failures; remaining mismatches are classified in the research note as semantic-safe size-winning cleanup/representation drift for pure dead value debris before `unreachable` (`012741`/`043481`, `023083`, `082547`) and the now-size-winning typed-dead-wrapper/nop-elision family (`046375`).
    - [ ] `[O4Z-AUDIT-CF-L]` Measure pass-local performance after the broader implementation: Starshine must stay within the repo floor (`starshine_time <= 2 * binaryen_time`) on the available direct artifact/timing lane, or the slowdown must be attributed to a specific pass-local owner with a follow-up slice rather than hidden under `[WALL]001`. Current blocker: requested debug-WASI timing after the `046375` cleanup is outside the floor at `.tmp/code-folding-046375-self-compare-after-large-gate` (`5051.616ms` Starshine pass vs `185.490ms` Binaryen pass); earlier reruns in `.tmp/code-folding-046375-self-compare*` were similarly around 5s, so investigate pass-local code-folding/fixpoint/lowering timing before audit closeout.
    - [ ] `[O4Z-AUDIT-CF-M]` Close the audit only after docs/backlog reflect the final parity status: update the code-folding wiki pages, research note, and `docs/wiki/log.md` with exact commands, counts, timings, mismatch classes, late-slot evidence, and any explicitly deferred Binaryen differences. Progress: docs now record the invalid-output fixes, 100000-case counts, tool/Binaryen command-failure breakdown, the `046375` size-winning follow-up, the requested timing failure, and five residual mismatch classifications; final closeout still needs generated late-slot evidence, `[O4Z-AUDIT-CF-L]` timing recovery/attribution, and a final accept/fix decision for those mismatches.
