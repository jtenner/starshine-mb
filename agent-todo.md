...
- [O4Z-AUDIT-DAE] - Deep audit `dae-optimizing`
  ...
  - Current finding: research notes `1562-2026-07-12-dae-large-module-nested-cleanup-guard-relaxation.md`, `1563-2026-07-12-dae-func3737-merge-blocks-drop-carrier.md`, and `1564-2026-07-12-dae-generic-folded-multivalue-suffix-defaults.md` remove the coarse `>100` defined-function nested-cleanup skip, fix the folded-multivalue Func3737 shared-drop-carrier `merge-blocks` abort, and replace the bespoke Func3737/Func3765 forwarded-default helper pair with a guarded generic folded-multivalue suffix recognizer. `moon test src/passes/dae_optimizing_test.mbt`, `moon test src/passes`, and a fresh normalized 1000-case direct compare are green again; the audit remains open for broader missing behavior families rather than these repaired folded-multivalue blockers.
  - Deliverables: apply the common checklist; coordinate with `[AUDIT003-*]`; refresh DAE-normalized direct compare and late `DAE` slot evidence; keep accepted raw-cleanup drift separate from semantic mismatches; continue shrinking the remaining broad gaps (real `precompute-propagate`, fuller default-function replay, broader operand localization beyond the guarded suffix case, GC param refinement, result refinement, and the rest of selected/artifact genericization) before claiming broader large-module signoff.
...
- [AUDIT]003 - DAE Selected-Shape De-Artifacting And Genericization
  ...
  - Current finding: note `1564-2026-07-12-dae-generic-folded-multivalue-suffix-defaults.md` closes one concrete family by replacing the bespoke Func3737/Func3765 forwarded-default helpers with a guarded generic folded-multivalue suffix exact-literal recognizer. The remaining genericization backlog is therefore narrower than the last inventory and should start from what is still selected after that slice.
