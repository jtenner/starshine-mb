...
- [O4Z-AUDIT-SSA] - Deep audit `ssa-nomerge`
  - Status: direct explicit-pass corruption, final-lane unreachable-debris drift, first debug-artifact nested-param/body-local shape gaps, `Func 4302` false-positive suspicious skip, the high-threshold raw-structured artifact size regression, and most large-structured raw no-op coverage were fixed/reduced on 2026-06-09; the 2026-06-11 near-threshold branch budget follow-up reduced the artifact `large-structured-local-writes` census from `17` to `15`; audit remains active for exact artifact parity, the remaining huge structured functions, and O4z slot policy/neighborhood replay.
  ...
  - Remaining deliverables: classify or reduce the remaining exact artifact first diff after `.tmp/self-ssa-nomerge-debug-wasi-structured-4608` (`defined=0 abs=27`, currently representation/local-numbering drift and not proven corruption); keep the remaining `15` huge `large-structured-local-writes` functions in backlog until measured raw-structured rewrite cost and validation evidence exists (2026-06-11 trace now leaves `Func 95`, `Func 265`, `Func 502`, `Func 2114`, `Func 3139`, `Func 3518`, `Func 3536`, `Func 3554`, `Func 3781`, `Func 3885`, `Func 4119`, `Func 4522`, `Func 5417`, `Func 5418`, and `Func 5419`; `Func 3668` and `Func 1265` were admitted by the `4608` branch budget after extracted validation); replay early `SSA` slot / artifact neighborhoods; decide whether the current O4z `ssa-nomerge` no-op remains intentional or can be narrowed; resolve or route the current `-O4z` debug-artifact abort in a later `remove-unused-names` `Func 2560` neighborhood before using preset evidence to re-enable SSA; file pass-local runtime fixes here and whole-command residuals under `[WALL]001`.
