...
- Both-canonical diagnostic helper: `scripts/self-optimize-compare.ts --canonicalize-binaryen-output --dae-optimizing` still first-diffs at `defined=509 abs=526`; the latest successful replay `.tmp/dae006-outer-block-suffix2-20260526` kept that frontier. Research notes `0576` through `0584` reduce the saved Func505 body from unknown/risky output-shape drift to representation-only control/local-carrier, branch-polarity, temp-local, and dropped-debris drift; `0585` adds a diagnostic-only canonical-function normalizer for that inspected Func505 parser-loop family; `0586` records the successor Func509 frontier as a semantic-safe, size-losing dead-return-suffix cleanup gap; `0588` adds a focused outer-block reduction/helper but does not close the artifact because the final cleanup hook still does not hit Func509; `0589` proves the printed `--print-func 526` post-return wrapper shape is already covered by the existing helper; `0590` proves the exact function-level value-block suffix cannot be stripped at the DAE final hook without invalid output, narrowing the unresolved cause to a later lowering/encoding cleanup boundary.
...
- [DAE]006 - Both-Canonical Frontier Advancement
  ...
  - Current classification:
    ...
    - `0589-2026-05-26-dae-func509-final-cleanup-input-reduction.md` added a focused white-box fixture for the printed `--print-func 526` outer-block/fallthrough-return/post-return-wrapper shape and proved the existing helper already strips it. The printed shape is therefore not the missing matcher.
    - `0590-2026-05-26-dae-func509-inmemory-block-suffix-boundary.md` added a focused safety regression for the exact function-level value-block suffix implied by `body_raw`. A broad DAE final-hook strip made `.tmp/dae006-inmemory-suffix-20260526` invalid (`block (result i64)` left in statement position in functions 509 and 514), so that shape must be preserved before lowering/encoding.
  - Next action:
    - Do not broaden `dae_try_strip_outer_block_no_fallthrough_wrapper_suffix` to non-void first blocks; the focused safety regression covers the invalid Func509 in-memory value-block suffix.
    - Decide whether a later post-lowering cleanup hook should remove the canonical dead-return wrapper debris, or whether the current Func509 both-canonical diff should be documented as a lowerer/diagnostic boundary rather than a DAE final-hook matcher miss.
    - Preserve the landed root-return, simple wrapper-suffix, outer-block, printed-shape, and in-memory-boundary tests/helpers if they remain valid, but do not claim Func509 closure until the both-canonical artifact compare moves beyond `defined=509 abs=526` or the boundary is explicitly accepted.
  - Deliverables:
