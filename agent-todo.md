...
- [O4Z-AUDIT-CF] - Deep audit and widen `code-folding` to Binaryen behavior parity
  ...
  - Current finding: source/test review found active owner, registry, dispatcher, and CLI coverage in place for a narrowed HOT pass. Focused coverage has started for full-`if` `unreachable` terminal suffix sharing, unsupported `br_on_null` label poisoning, live-label structured suffix bailouts, and a widened single-result typed block-exit branch-payload slice that shares matching plain-`br` payloads with a fallthrough value or with other plain-`br` payloads. This is still not Binaryen behavior parity. Baseline slice `[O4Z-AUDIT-CF-A]` is green: `moon info`, `moon fmt`, `moon test src/passes` (`1590/1590`), native `src/cmd` build at `_build/native/release/build/cmd/cmd.exe`, direct 1000-case compare (`998` normalized matches, `0` mismatches, `2` `binaryen-rec-group-zero` command failures), timing-only debug-WASI replay (`172.276ms` Starshine pass vs `169.576ms` Binaryen pass, within <=2x), and full `moon test` (`4775/4775`). Late-slot replay and broader Binaryen behavior parity remain open.
  - Deliverables:
    ...
    - [ ] `[O4Z-AUDIT-CF-B]` Build a source-backed Binaryen shape matrix from `CodeFolding.cpp` and `test/lit/passes/code-folding.wast`: rows for named-block exits, `if` arms, terminating returns, terminating `return_call*`, terminating `unreachable`, unsupported-branch poison, outside-target movement bailouts, EH-sensitive bailouts/repair, helper/profitability cases, and fixpoint exposure. Link every row to local tests or a new test TODO.
    - [ ] `[O4Z-AUDIT-CF-C]` Finish expression-exit candidate modeling for named block exits: record branch-backed tails and fallthrough tails with explicit provenance, target label, branch payload roots, result arity, movement proof, and profitability score. Preserve the existing `br_on_*` / `br_if` / `br_table` / `delegate` poison behavior until the corresponding Binaryen-supported handling is intentionally implemented and tested.
    - [ ] `[O4Z-AUDIT-CF-D]` Widen named-block expression-exit folding beyond the current single-result payload-root slice: add tests first for multi-root shared suffixes, multi-value branch payloads where HOT/lower can represent them, branch-plus-fallthrough payload suffixes, duplicate branch-only tail groups without fallthrough, and helper-block insertion/profitability cases. Implement only the tested safe subset and keep validation after each rewrite.
    - [ ] `[O4Z-AUDIT-CF-E]` Widen `if` expression-exit folding to Binaryen's arm rules: unnamed block/unnamed block suffixes, one-block/one-non-block synthetic wrapping where safe, partial suffix folds, full-arm folds, named-arm negatives, unreachable-condition bailouts, refined-result preservation, and profitability thresholds. Keep live-label and branch-target movement negatives green.
