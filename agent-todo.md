...
- The last fresh original-input WAGO sweep remains final7 over all 1,330 tracked fixtures at or below 2 MiB and the unchanged 766-file stable cohort. The runtime-safe final9 retained-artifact projection is `.tmp/wago-o4z-pass8-final9-safe-projected-20260821/`; replacing only its primes artifact with the newly integrated fresh original-input output projects 412,801 Starshine bytes versus pinned Binaryen v131 at 415,485, a 2,684-byte / 0.646% win with projected 112/597/57 smaller/equal/larger, 2,620 positive residual bytes, 47,329 bytes removed from the 460,130-byte baseline, and 106.01% original-gap closure. Native SHA-256 is `32c4cc048afcfe5457aad04c3308b01a0e436754c2f92895a0b7663c1facf901`. Primes now optimizes directly from 23,162 to 22,480 versus Binaryen's 21,690, preserving 14 ordinary state events, a 24-event expanded stack/temp-ret sequence, fresh main and allocator traps, 131,072-byte memory, and all full-memory hashes. The safe continuation performs final canonicalization, one bounded local fixed-point wave, and the existing finish roster after the accepted stateful plain-inlining candidate. It protects only the exact internal variadic packer across five observed local layouts in a 36-function stateful Emscripten module; broad 21,955-byte replay still fails with an out-of-bounds main trap and changed memory. The broader projection remains retained-artifact evidence, not a fresh corpus sweep. Heap2Local's packed signed-read repair and the final9 ifs/ISA/memory/GC winners remain unchanged. Full validation is 10,577/10,577; all passes 6,947/6,947; command 183/183; pass manager 335/335; CoalesceLocals 87/87. Reducer-enabled CoalesceLocals smoke was artifact-bound; the capped explicit-v131 no-reducer lane compared 36 cases with 3 normalized plus 33 smaller residuals, zero failures, and no stateful-export trigger, so it is development evidence only. Remaining leaders are primes +790, ifs +450, JSON-SIMD +397, fannkuch +219, fasta +181, scalar JSON +86, urem +64, startup +60, issue1809 +50, fuzzcase 1825 +33, deeply nested struct +25, bump-pointer GC +24, and return-call-indirect +21 each. Preserve xjb/mandelbrot fallbacks, packed-array semantics, stateful runtime exclusions, and fresh-sweep provenance.
