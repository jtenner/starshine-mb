...
2. Slice gameplan in `agent-todo.md` and determine deliverables.
   ...
   - [DCE]002 - Prefix Regression and Artifact Replay - Lock the corrected DCE behavior into focused tests and rerun the MoonBit debug-artifact compare harness.
     ...
     - Status: focused DCE regressions plus the earlier hot-lift/effects fixes landed; replay gets past the old `func[80]` / `func[106]` hot-lift aborts, the typed-loop lowering bug that broke canonical `--vacuum` replay at `func[81]` is fixed, the dead-result wrapper failures at `func[67]` and `func[190]` are closed, detached shared-subtree cleanup no longer aborts on parent-first deletion order, final non-fallthrough structured control now materializes Binaryen-style explicit `unreachable` tails, and the unchanged raw-function fast path no longer drops lift/lower canonicalization for dead `drop` tails after non-fallthrough roots. The minimized `moonbit.string_equal` replay shape and the full debug-artifact `--dead-code-elimination` output now both validate and survive `wasm-opt --strip-debug`; relative to the last usable Binaryen oracle artifact, `func 27` and `func 46` now match Binaryen's unreachable-tail counts and overall unreachable-count agreement improved on 50 functions. The remaining work is structural parity outside naming/order plus a fresh ordered replay once the compare entry path is usable again.
     - Doc: [0066#L178](/home/jtenner/Projects/starshine-mb/docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md#L178)
