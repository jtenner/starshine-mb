...
3. Do work.
   - Land the slices above in dependency order in the implementing file(s) and any required scheduler, preset, or dispatcher surfaces.
   - Wire the pass into the exact top-level slot(s) and nested rerun sites documented in the research doc before calling the work done.
   - Current status: the repo now has a standalone active hot `merge-blocks` pass that flattens branch-free block roots in any region, preserves lowered `nop` roots and dead `local.tee` + `drop` pairs on changed functions to match Binaryen's direct `--merge-blocks` behavior, is wired into the representable late `optimize` / `shrink` cleanup slots as `merge-blocks -> remove-unused-brs -> remove-unused-names -> merge-blocks -> precompute -> optimize-instructions -> heap-store-optimization`, and now keeps reduced single-result typed param block probes canonically equal to Binaryen while leaving reduced multi-result typed param block probes unchanged and Binaryen-stable. Remaining work is broader typed/result-shape parity beyond that single-result param-block slice, late-cleanup interaction coverage beyond the current slot replay tests, stronger ordered-artifact/fuzz sign-off, and reducing the still-open typed single-result `if`-arm param-block direct mismatch from `.tmp/mb-probe/caseB.final`.
4. Test against binaryen.
   - Add edge-case and regression tests beside the implementing file and any scheduler or dispatcher coverage needed for the pass.
   - Compare Starshine vs Binaryen with `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --<pass>` and any required ordered-prefix replay.
   - Reduced direct oracle smokes are now green for both known writeback families: `bun scripts/self-optimize-compare.ts .tmp/merge-blocks-nop-divergence/in.wasm --out-dir .tmp/merge-blocks-nop-divergence/out --merge-blocks` reports canonical wasm and normalized WAT equality after preserving lowered `nop`s, and `bun scripts/self-optimize-compare.ts .tmp/merge-blocks-tee-probe/tee-drop.wasm --out-dir .tmp/merge-blocks-tee-probe/out-fixed --merge-blocks` now does the same for the dead `local.tee` + `drop` family. The reduced typed param-block probes `bun scripts/self-optimize-compare.ts .tmp/mb-probe/param1.wasm --out-dir .tmp/mb-probe/param1.final --merge-blocks` and `bun scripts/self-optimize-compare.ts .tmp/mb-probe/caseC.wasm --out-dir .tmp/mb-probe/caseC.final --merge-blocks` are now canonically green too, while `.tmp/mb-probe/caseB.final` remains the next reduced direct divergence to fix. Small wasm-smith-only follow-up sweeps at `.tmp/pass-fuzz-compare-merge-blocks-slice-smith`, `.tmp/pass-fuzz-compare-merge-blocks-next-slice-smith`, `.tmp/pass-fuzz-compare-merge-blocks-tee-fix-smith`, and `.tmp/pass-fuzz-compare-merge-blocks-single-param-slice` stayed mismatch-free on comparable cases and only stopped on the Binaryen-only `binaryen-rec-group-zero` parser family.
