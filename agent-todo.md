...
- Both-canonical diagnostic helper: `scripts/self-optimize-compare.ts --canonicalize-binaryen-output --dae-optimizing` currently advances through the inspected wrapper/type-id families to `.tmp/dae-func447-normalized-artifact`, first diff `defined=448 abs=465`.
- Latest committed diagnostic timing is close to the 2x target: `1972.045ms` Starshine pass versus `994.406ms` Binaryen pass. Repeat timing before signoff.
...
- [DAE]003 - Constant-Actual and Unread-Parameter Generalization
  ...
  - Why: current coverage includes private direct callees where every caller passes the same exact literal, selected scalar memory-load siblings, typed single-result block wrappers, immutable `global.get` for selected Func330/def313, and many selected debug-artifact defs; broader candidate discovery remains incomplete.
  - Deliverables:
...
- [DAE]004 - Selected Result-Removal Broadening
  ...
  - Why: selected defs now include artifact families such as `298`, `299`, `427`, `445`, `3566`, `3814`, `3834`, and `4229`; Func445 proved the both-canonical frontier can still expose real result/signature gaps.
  - Deliverables:
...
- [DAE]006 - Both-Canonical Frontier Advancement
  - Goal: continue the diagnostic both-canonical frontier from current `defined=448 abs=465`.
  - Next action:
    - Inspect `.tmp/dae-func447-normalized-artifact/func-defined448-abs465.binaryen.pretty.txt` and `.starshine.pretty.txt` plus the `.wat` diff.
...
- [DAE]011 - Performance Stabilization
  ...
  - Deliverables:
    - Repeat timing for `.tmp/dae-func447-normalized-artifact` or the latest artifact before claiming signoff.
