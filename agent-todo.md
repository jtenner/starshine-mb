...
- [RSE]002 - CFG Value Flow And Refined Local Gets
  - Current status: 2026-05-10 added branch-disagreement merge identities for self-set folding, body-local default identities for redundant default writes, raw strict-subtype equivalent-local `local.get` retargeting, raw preservation of fallthrough facts after one-armed terminating `if`s, paired vacuum cleanup for nested pure `drop` / `nop` debris, vacuum empty-then/live-else inversion, and raw structured block/if label-exit merge tracking with unreachable-tail skipping so `br_if` block exits no longer misfold final const sets. Direct compare-pass stayed semantic-green at `.tmp/pass-fuzz-rse-rse002-next-followup` (`6759/10000`, `0` mismatches, `20` Binaryen/tool command failures).
  - Remaining deliverables: finish loop convergence or documented conservative loop skips, extend the same structured label start/end identities through the remaining HOT/control families, and broaden `rse-gc.wast`-style type-refinement coverage beyond the reduced abs-heap fixture.
  ...
  - Tests: branch-join positives/negatives, raw block-exit negatives, loop convergence/skips, broader `rse-gc.wast`-style type-refinement tests, direct compare-pass parity, and eventual late-tail preset replay.
