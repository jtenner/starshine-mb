...
- `[DAE]013` preset / ordered-neighborhood integration is closed as of 2026-05-26 by research note `0603`: `dae-optimizing` remains direct-pass-only for v0.1.0 under `dae-optimizing` and `dead-argument-elimination-optimizing`, and public `optimize` / `shrink` presets intentionally skip it until new ordered-prefix or artifact evidence proves default-preset safety and runtime. Remaining DAE active breadth is `[DAE]003` and `[DAE]004`.
- `[DAE]004` wider mid-size fact-driven dropped-result batching advanced on 2026-05-26 by research notes `0604` and `0605`: the bounded fact-driven candidate queue now runs for modules up to `4096` defined functions before the handpicked selected-def fallback, still using current direct-call facts and the caller-filtered result-removal helper. Focused regression `dae-optimizing removes high fact-discovered dropped callee result outside selected list` now targets defined function `3000`; it failed before the `4096` guard widening (`1 != 0`) and now passes. Direct compare `.tmp/pass-fuzz-dae004-4096-20260526-full2` reproduced the accepted DAE010/DAE011 counts with `0` validation failures. The earlier guarded debug-artifact timing replay `.tmp/dae004-mid-fact-timing-20260526` stayed inside the `Starshine <= 2x Binaryen` target (`1439.590ms` vs `914.804ms`), while an unguarded experiment did not complete cleanly; the huge debug artifact therefore remains on the closed DAE011 selected-list path, and `[DAE]004` remains open until artifact/fuzz evidence proves the selected fallback can be removed or no remaining dropped-result scheduling gap exists.
...
- [DAE]004 - Selected Result-Removal Broadening
  ...
  - Deliverables:
    ...
    - [ ] Remove the large-artifact handpicked selected-def fallback when artifact/fuzz evidence proves the fact-driven queue covers the remaining result-removal frontier without reopening the DAE011 pass-local runtime cliff.
