...
- [O4Z-AUDIT-SSA] - Deep audit `ssa-nomerge`
  - Status: direct explicit-pass corruption and final-lane unreachable-debris drift fixed on 2026-06-09; audit remains active for O4z slot policy and artifact/neighborhood replay.
  ...
  - Finding: `docs/wiki/raw/research/0722-2026-06-09-ssa-nomerge-exceptional-edge-audit.md` fixed a true try_table/throw corruption caused by applying normal-flow-only local SSA across exceptional edges. `ssa-nomerge` now skips exceptional-flow HOT mutation, and the raw no-write path now recursively materializes default body-local reads across branchy control and cleans dropped-unreachable debris before hard `unreachable` tails. Validation: focused `ssa_nomerge_test` `28/28`, `moon test src/ir` `251/251`, `moon test src/passes` `2047/2047`, full `moon test` `5245/5245`, native `src/cmd` build passed with pre-existing warnings, direct 10000-case compare `.tmp/pass-fuzz-ssa-nomerge-audit-after-10000` compared `9977/10000` with `0` mismatches, initial final `.tmp/pass-fuzz-ssa-nomerge-final-100000` exposed `3` semantic-safe unreachable-debris mismatches, and post-cleanup final `.tmp/pass-fuzz-ssa-nomerge-final-100000-after-debris` compared `99751/100000` with `99751` normalized matches, `0` mismatches, and `249` Binaryen/tool command failures.
  - Remaining deliverables: replay early `SSA` slot / artifact neighborhoods; decide whether the current O4z `ssa-nomerge` no-op remains intentional or can be narrowed; investigate the current `-O4z` debug-artifact abort in a later `remove-unused-names` slot before using preset evidence to re-enable SSA; file pass-local runtime fixes here and whole-command residuals under `[WALL]001`.
