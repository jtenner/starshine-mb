...
- [SGO]003O - Refactor-Only Matcher Maintainability Queue
  - Status: opportunistic; lower priority than behavior-bearing slices after commits `0f8b8902` through `39fa0a22`. The 0641 clean-pop helper slice centralized repeated FlowScanner pop/taint checks without behavior broadening, and the 0643 call-result helper slice centralized repeated call-boundary stack handling while preserving the adjacent-`global.get` arm-result exception.
