...
- [O4Z-AUDIT-SSA] - Deep audit `ssa-nomerge`
  ...
  - GenValid coverage note: 2026-06-10 widened the `ssa-nomerge-coverage` profile and feature ledgers for structured early-return local traffic, block-target `br`/`br_if` merge-copy candidates, structured param merges, and branch-local writes that are not read after the join. This improves future direct compare input variety but does not close the remaining artifact/preset deliverables above.
