...
- [FUZ]1010 - GenValid SIMD Exact Opcode And Relaxed-SIMD Widening
  - Status: IN PROGRESS (cron 23, 2026-05-22): representative exact standard-SIMD opcode counters are landed for phase-1/phase-2/phase-3 coverage, the phase-1 counter set has been widened to all currently emitted splats, lane extract/replace families, and core bitwise ops, and the phase-2/phase-3 counter sets now cover the currently emitted standard-SIMD matrix while preserving coarse SIMD phase facts for exact-counted instructions. Relaxed-SIMD generation is landed behind the explicit `allow_relaxed_simd` toggle and `relaxed-simd` profile, outside Binaryen-oracle portable profiles; binary encode/decode fixtures now lock exact `0xFD` subopcodes `256..275`. This cron slice wired `--gen-valid-profile` through `pass-fuzz-compare`, records `genValidProfile` in `result.json`, and validates compare inputs/outputs with `wasm-tools validate --features all`. TDD evidence: `bun scripts/test/pass-fuzz-compare-command.ts` first failed because compare-pass treated `--gen-valid-profile` as a pass flag, then failed because validation omitted `--features all`, and now passes. Smoke evidence: `bun scripts/pass-fuzz-compare.ts --pass remove-unused-brs --generator gen-valid --gen-valid-profile binaryen-oracle-portable --count 5 --max-failures 5 --out-dir .tmp/pass-fuzz-fuz1010-portable-profile-smoke` passed with `5/5` compared and `5` normalized matches. A direct `relaxed-simd` profile smoke at `.tmp/pass-fuzz-fuz1010-relaxed-profile-smoke2` was blocked before comparison by the local external `wasm-tools` rejecting section id `14` even under `--features all`; cron 23 added the `binaryen-oracle-relaxed-simd` profile to keep imports/tables/mems/globals/tags/elems/datas/ref-types/atomics/memory64 and similar nonportable oracle surfaces disabled while enabling `v128`, relaxed SIMD, and typed body generation. TDD evidence: `moon test src/validate` first failed because `BinaryenOracleRelaxedSimdProfile` was missing, then passed after wiring the profile. Smoke evidence: `bun scripts/pass-fuzz-compare.ts --pass remove-unused-brs --generator gen-valid --gen-valid-profile binaryen-oracle-relaxed-simd --count 1 --max-failures 1 --out-dir .tmp/pass-fuzz-fuz1010-relaxed-oracle-profile-smoke-count1` reached comparison with `1/1` compared, `0` validation failures, and `0` command failures; it produced one normalized mismatch under `remove-unused-brs`, so do not claim relaxed-SIMD pass semantic signoff yet. A count-5 attempt at `.tmp/pass-fuzz-fuz1010-relaxed-oracle-profile-smoke` timed out after emitting inputs and before result.json. This cron slice widened phase-2 standard-SIMD exact coverage to emit and count all integer lane shifts (`i8x16/i16x8/i32x4/i64x2.{shl,shr_s,shr_u}`), preserving `has_simd_phase2` for those opcodes. TDD evidence: `moon test src/validate` first failed because the new shift exact-counter fields were missing, then passed after generation/counting/fact wiring. Smoke evidence: `bun scripts/pass-fuzz-compare.ts --pass vacuum --generator gen-valid --gen-valid-profile binaryen-oracle-portable --count 1 --max-failures 1 --out-dir .tmp/pass-fuzz-fuz1010-simd-shifts-portable-vacuum-count1` passed with `1/1` compared, `1` normalized match, and no validation/generator/command failures; a larger `remove-unused-brs` count-5 smoke at `.tmp/pass-fuzz-fuz1010-simd-shifts-portable-smoke` timed out after input emission and before `result.json`. This cron slice widened the phase-2 exact standard-SIMD coverage again to emit and count integer unary/reduction forms (`i8x16.abs`, `i8x16.neg`, `i8x16.popcnt`, `i8x16.bitmask`, `i16x8.abs`, `i16x8.neg`, `i16x8.q15mulr_sat_s`, `i16x8.all_true`, `i32x4.abs`, `i32x4.neg`, `i32x4.bitmask`, `i64x2.abs`, `i64x2.neg`, and `i64x2.all_true`) while preserving `has_simd_phase2` for those exact-counted opcodes. TDD evidence: `moon test src/validate` first failed because the new exact-counter fields were missing, then passed after generation/counting/fact wiring. Signoff evidence: `moon info` passed with pre-existing DAE warnings; `moon fmt` passed; `moon test` passed with `3476/3476`; `bun scripts/pass-fuzz-compare.ts --pass vacuum --generator gen-valid --gen-valid-profile binaryen-oracle-portable --count 1 --max-failures 1 --out-dir .tmp/pass-fuzz-fuz1010-simd-unary-portable-vacuum-count1` timed out after input emission before `result.json`, matching the recent compare-smoke timeout shape rather than producing a validation/generator/command failure. This cron slice widened phase-2 standard-SIMD exact comparison coverage with additional emitted/counted representatives across integer and float lanes (`i8x16.ne`, `i8x16.ge_u`, `i16x8.ge_u`, `i32x4.le_s`, `i64x2.eq`, `f32x4.ge`, and `f64x2.eq`) while preserving `has_simd_phase2` for those exact-counted instructions. TDD evidence: `moon test src/validate` first failed because the new exact-counter fields were missing, then passed after generation/counting/fact wiring. Signoff evidence: `moon info` passed with pre-existing DAE warnings; `moon fmt` passed; `moon test` passed with `3476/3476`. A count-1 `vacuum` portable compare smoke at `.tmp/pass-fuzz-fuz1010-simd-comparison-portable-vacuum-count1` timed out after input emission and before `result.json`, matching the recent compare-smoke timeout shape rather than producing a validation/generator/command failure. This cron slice widened phase-3 standard-SIMD exact conversion coverage to emit/count `f64x2.promote_low_f32x4`, `i32x4.trunc_sat_f32x4_{s,u}`, `f32x4.convert_i32x4_{s,u}`, `i32x4.trunc_sat_f64x2_{s,u}_zero`, and `f64x2.convert_low_i32x4_{s,u}` in addition to the already counted demote form. TDD evidence: `moon test src/validate` first failed because the new exact-counter fields were missing, then passed after generation/counting/fact wiring. Signoff evidence: `moon info` passed with pre-existing DAE warnings; `moon fmt` passed; `moon test` passed with `3476/3476`. A count-1 `vacuum` portable compare smoke at `.tmp/pass-fuzz-fuz1010-simd-conversion-portable-vacuum-count1` timed out after input emission and before `result.json`, matching the recent compare-smoke timeout shape rather than producing a validation/generator/command failure. This cron slice widened phase-2 standard-SIMD exact comparison coverage to emit and count the full generated `i8x16` comparison family (`eq`, `ne`, signed/unsigned `lt`/`gt`/`le`/`ge`) while preserving `has_simd_phase2` for those exact-counted instructions and updating the generator coverage ledger. TDD evidence: `moon test src/validate` first failed because the new `i8x16` comparison exact-counter fields were missing, then passed after generation/counting/fact wiring. Signoff evidence: `moon info` passed with pre-existing DAE warnings; `moon fmt` passed; `moon test` passed with `3476/3476`. A count-1 `vacuum` portable compare smoke at `.tmp/pass-fuzz-fuz1010-i8x16-comparison-portable-vacuum-count1` timed out after input emission and before `result.json`, matching the recent compare-smoke timeout shape rather than producing a validation/generator/command failure. This cron slice widened phase-2 standard-SIMD exact comparison coverage to emit and count the full generated `i16x8` comparison family (`eq`, `ne`, signed/unsigned `lt`/`gt`/`le`/`ge`) while preserving `has_simd_phase2` for those exact-counted instructions and updating the generator coverage ledger. TDD evidence: `moon test src/validate` first failed because the new `i16x8` comparison exact-counter fields were missing, then passed after generation/counting/fact wiring. Signoff evidence: `moon info` passed with pre-existing DAE warnings; `moon fmt` passed; `moon test` passed with `3476/3476`. A count-1 `vacuum` portable compare smoke at `.tmp/pass-fuzz-fuz1010-i16x8-comparison-portable-vacuum-count1` timed out after input emission and before `result.json`, matching the recent compare-smoke timeout shape rather than producing a validation/generator/command failure. This cron slice widened phase-2 standard-SIMD exact comparison coverage to emit and count the full generated `i32x4` comparison family (`eq`, `ne`, signed/unsigned `lt`/`gt`/`le`/`ge`) while preserving `has_simd_phase2` for those exact-counted instructions and updating the generator coverage ledger. TDD evidence: `moon test src/validate` first failed because the new `i32x4` comparison exact-counter fields were missing, then passed after generation/counting/fact wiring. Signoff evidence: `moon info` passed with pre-existing DAE warnings; `moon fmt` passed; `moon test` passed with `3476/3476`. A count-1 `vacuum` portable compare smoke at `.tmp/pass-fuzz-fuz1010-i32x4-comparison-portable-vacuum-count1` timed out after input emission and before `result.json`, matching the recent compare-smoke timeout shape rather than producing a validation/generator/command failure. This cron slice widened phase-2 standard-SIMD exact comparison coverage to emit and count the rest of the generated wider comparison families: `i64x2.{ne,lt_s,gt_s,le_s}` alongside the existing `eq/ge_s`, full `f32x4.{eq,ne,lt,gt,le,ge}`, and `f64x2.{ne,lt,gt,le}` alongside the existing `eq/ge`. TDD evidence: `moon test src/validate` first failed because the new exact-counter fields were missing, then passed after generation/counting/fact wiring. Signoff evidence: `moon info` passed with pre-existing DAE warnings; `moon fmt` passed; `moon test` passed with `3476/3476`. A count-1 `vacuum` portable compare smoke at `.tmp/pass-fuzz-fuz1010-wide-comparison-portable-vacuum-count1` timed out after input emission and before `result.json`, matching the recent compare-smoke timeout shape rather than producing a validation/generator/command failure. This cron slice widened the phase-2 exact standard-SIMD arithmetic surface to emit and count the missing `i8x16` and `i16x8` arithmetic/saturating/min/max representatives: `i8x16.add_sat_u`, `i8x16.sub`, `i8x16.sub_sat_s`, `i8x16.min_u`, `i8x16.max_s`, `i16x8.add`, `i16x8.add_sat_s`, `i16x8.sub`, and `i16x8.sub_sat_u`, while keeping the already-counted sibling forms in the same coverage assertion. TDD evidence: `moon test src/validate` first failed because the new exact-counter fields were missing, then passed after generation/counting/fact wiring. Signoff evidence: `moon info` passed with the pre-existing DAE warnings; `moon fmt` passed; `moon test` passed with `3476/3476`; a count-1 `vacuum` portable compare smoke at `.tmp/pass-fuzz-fuz1010-i8-i16-arith-portable-vacuum-count1` timed out after 180s before producing `result.json`, matching the recent compare-smoke timeout shape rather than producing a validation/generator/command failure. This cron slice widened phase-2 standard-SIMD exact arithmetic coverage to emit and count the remaining generated i16/i32/i64/float arithmetic gaps: `i16x8.min_s`, `i16x8.max_u`, `i32x4.sub`, `i32x4.min_u`, `i32x4.max_s`, `i64x2.sub`, `f32x4.sub`, `f32x4.mul`, `f32x4.pmin`, `f32x4.pmax`, `f64x2.sub`, `f64x2.mul`, `f64x2.pmin`, and `f64x2.pmax`, while preserving `has_simd_phase2` through exact-counted instruction facts. TDD evidence: `moon test src/validate` first failed because the new exact-counter fields were missing, then passed after generation/counting/fact wiring. Signoff evidence: `moon info` passed with the pre-existing DAE warnings; `moon fmt` passed; `moon test` passed with `3476/3476`; a count-1 `vacuum` portable compare smoke at `.tmp/pass-fuzz-fuz1010-arith-gaps-portable-vacuum-count1` timed out after 180s after emitting inputs and before `result.json`, matching the recent compare-smoke timeout shape rather than producing a validation/generator/command failure. This cron slice widened phase-3 standard-SIMD exact extension/extmul coverage to emit and count all represented integer lane extension variants (`i16x8.extend_{low,high}_i8x16_{s,u}`, `i32x4.extend_{low,high}_i16x8_{s,u}`, and `i64x2.extend_{low,high}_i32x4_{s,u}`) and all represented extmul variants (`i16x8.extmul_{low,high}_i8x16_{s,u}`, `i32x4.extmul_{low,high}_i16x8_{s,u}`, and `i64x2.extmul_{low,high}_i32x4_{s,u}`), while preserving coarse SIMD phase facts for exact-counted instructions. TDD evidence: `moon test src/validate` first failed because the new exact-counter fields were missing, then passed after generation/counting/fact wiring; focused scan/stat coverage now includes `i32x4.extend_low_i16x8_s` and `i64x2.extmul_high_i32x4_u`. Signoff evidence: `moon info` passed with the pre-existing DAE warnings; `moon fmt` passed; `moon test` passed with `3476/3476`; a count-1 `vacuum` portable compare smoke at `.tmp/pass-fuzz-fuz1010-ext-extmul-portable-vacuum-count1` timed out after 240s after emitting `cases.jsonl` and `inputs/` but before producing `result.json`, matching the recent compare-smoke timeout shape rather than producing a validation/generator/command failure. This cron slice widened phase-2 standard-SIMD float unary coverage to emit and count `f32x4.abs`, `f32x4.neg`, `f32x4.sqrt`, `f64x2.abs`, `f64x2.neg`, and `f64x2.sqrt`, while preserving coarse SIMD phase facts through exact-counted instruction facts. TDD evidence: `moon test src/validate` first failed because the new float-unary exact-counter fields were missing, then passed after generation/counting/fact wiring. Signoff evidence: `moon info` passed with the pre-existing DAE warnings; `moon fmt` passed; `moon test` passed with `3476/3476`; a count-1 `vacuum` portable compare smoke at `.tmp/pass-fuzz-fuz1010-float-unary-portable-vacuum-count1` timed out after 240s after input emission and before producing `result.json`, matching the recent compare-smoke timeout shape rather than producing a validation/generator/command failure. This cron slice strengthened the phase-3 standard-SIMD memory exact-counter regression so coverage-forced GenValid now asserts every currently emitted SIMD memory counter individually: base `v128.load`/`v128.store`, load-extend, load-splat, lane load/store, and zero-load families. TDD note: the new assertions passed immediately because generation/counting for these memory forms had already landed; the change closes the unproven ledger/test gap rather than adding new opcodes. Focused evidence: `moon test src/validate` passed with `960/960`. Signoff evidence: `moon info` passed with the pre-existing DAE warnings; `moon fmt` passed; `moon test` passed with `3476/3476`; a count-1 `vacuum` portable compare smoke at `.tmp/pass-fuzz-fuz1010-simd-memory-assertions-portable-vacuum-count1` timed out after 240s after emitting `cases.jsonl` and `inputs/` but before producing `result.json`, matching the recent compare-smoke timeout shape rather than producing a validation/generator/command failure. This cron slice then strengthened the phase-2/phase-3 standard-SIMD exact-counter regression so coverage-forced GenValid now asserts the remaining already emitted/counted phase-2 shift, reduction, integer arithmetic/min/max, float arithmetic, and float rounding forms individually, including `i8x16.shr_s`, `i16x8.shl`, `i32x4.all_true`, `i64x2.bitmask`, `i32x4.mul`, `i64x2.add`, `f32x4.div`, `f64x2.max`, and all `f32x4`/`f64x2` rounding counters, plus the remaining already emitted/counted phase-3 narrow/extadd forms `i8x16.narrow_i16x8_u`, `i16x8.narrow_i32x4_{s,u}`, `i16x8.extadd_pairwise_i8x16_{s,u}`, and `i32x4.extadd_pairwise_i16x8_{s,u}`. TDD note: these assertions also passed immediately because the generator and scanner already emitted/counted the opcodes; the change closes the remaining standard-SIMD test-ledger gaps rather than adding new opcodes. Focused evidence: `moon test src/validate` passed with `960/960`. Next run should either classify/fix the relaxed-oracle mismatch with a pass whose semantics are expected to handle relaxed SIMD, or continue broadening beyond exact counters into the remaining FUZ1010 deliverables such as deeper lane/index variation and memory/profile coverage.
  - Goal: extend SIMD generation from phase-level coverage to exact opcode and relaxed-SIMD coverage.
  - Why: relaxed SIMD is now represented by an explicit non-Binaryen-oracle GenValid profile, but remaining SIMD surfaces still need exact coverage and profile-gated generation. Phase counters are too coarse to detect missing individual SIMD op families.
