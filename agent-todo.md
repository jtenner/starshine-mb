...
- [SGO]003 - Binaryen `SimplifyGlobals.cpp` Breadth
  ...
  - 2026-05-23 dominance-lit regression: official `simplify-globals-dominance.wast` is now mapped to local runtime tests for the dominated-then/recursive-call flavor. Starshine already rewrites the pre-call dominated read and keeps recursive-call, else-arm, and post-if join boundaries conservative; this was pinned as a test/docs slice in `docs/wiki/raw/research/0575-2026-05-23-sgo-dominance-lit-regression.md`.
  - 2026-05-23 same-init/runtime guardrails: `docs/wiki/raw/research/0576-2026-05-23-sgo-same-init-runtime-guardrails.md` pins alias-init/direct-literal one-shot behavior and block-wrapped runtime-set operands as conservative. Do not broaden same-init expression equivalence or runtime facts from cleanup-obvious result blocks without a new exact Binaryen-positive fixture.
  - 2026-05-23 official lit source-alignment guardrails: offsets (`0580`), prefer-earlier copy chains (`0581`), and startup-only nested GC initializer propagation (`0582`) are now pinned as tests/docs-only slices. The nested GC test keeps the imported middle `global.get` and confirms no nested cleanup is triggered by startup-only global-initializer rewrites.
