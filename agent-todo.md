...
| `remove-unused-brs` | Post-repair v131 work retires the old random gaps, admits safe multivalue-return roots, repairs nested wrappers with CFG proof, flattens dropped literal multivalues, and matches null-only `br_if` result typing while preserving Binaryen's broader selector-prefixed `br_table` result. The development aggregate is `6875/10000` exact; all `3125` residuals are source-backed smaller Starshine cleanups totaling `-81681` bytes, with zero failures. | **Open final evidence and scheduler reconciliation:** `[AUDIT-CORRECTNESS]001`; replay the old random residuals, run the fresh four-lane matrix, then reconcile the extra slot. |
...
- **Deliverables:**
  ...
  - [x] Run the required four-lane v131 matrix for `remove-unused-brs` (2026-07-26 baseline: regular `100000/100000` cleanup-normalized green; wasm-smith `9956` comparable with `9954` direct plus `2` cleanup-normalized and `44` Binaryen-only failures; dedicated `10000/10000` mismatches; random-all `870` residuals). Follow-up repairs match the `181` prior random parity gaps, preserve the `689` measured wins, admit safe multivalue-return roots, and make wrapper cleanup exact. The development aggregate is now `5655/10000` exact with zero failures and a `-81681`-byte residual delta; focused multivalue-drop is a uniform measured `-12` win. The dedicated typing and smaller-residual classification is now complete; keep this item open for old-random replay and a final fresh four-lane renewal.
