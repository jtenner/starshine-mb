...
- [FUZ]1005 - GenValid Control Flow, Multi-Value, And Branch Payloads
  - Status: IN PROGRESS (cron 23, 2026-05-21). First branch-payload slice landed: `control-heavy` now forces a reusable empty-param multi-result function type, emits a multi-value `TypeIdxBlockType` block, and adds typed i32 value-block payload coverage for `br_if` and `br_table` alongside the existing `br` payload. Focused TDD evidence: `moon test src/validate` first failed because the new `br_if` payload and multi-value block assertions were missing, then passed after implementation; `bun fuzz compare-pass --generator gen-valid --count 200 --min-compared 200 --pass code-folding --out-dir .tmp/pass-fuzz-fuz1005-code-folding-200` reported `200/200` compared, `200` normalized matches, and `0` validation/generator/command failures or mismatches. Still to resume: value-producing loops, deeper randomized/nested label-depth variation, returns from nested control, and broader non-i32/ref/v128 payload combinations.
