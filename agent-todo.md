...
- [FUZ]1021 - Invalid Binary Malformed-Byte Matrix
  - Status: IN PROGRESS (cron 23, 2026-05-21). Latest overwide section vector-count ULEB slice adds decode-rejected, non-smoke `overwide-*-section-count-uleb` strategies for type, code, import, export, table, memory, global, function, element, data, and tag sections, using the shared well-formed section-frame helper and focused FZG040 replay coverage. Previous malformed vector-count and smoke-scheduler slices are landed and documented; continue FUZ1021 by selecting another malformed-byte class such as remaining section boundary/payload-length, invalid UTF-8, opcode/prefix, lane, or memarg contexts. Current TDD evidence: focused invalid-binary wbtest first failed on missing `overwide-type-section-count-uleb`, then passed after implementation.
