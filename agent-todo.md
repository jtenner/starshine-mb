...
- [INL]002 - Nested Useful-Passes Replay and Artifact Parity
  - Status: partial approximation only. The pass emits the required nested-cleanup trace marker and runs an unfiltered Starshine cleanup lane with untouched-function body restoration, touched-function unused-local compaction, and a conservative unreachable-root collapse for touched functions, but this is not yet Binaryen's exact `precompute-propagate` + filtered default-function scheduler.
  - Deliverables: prepend the real `precompute-propagate` equivalent, rerun the default function pipeline on the exact Binaryen touched set, add nested-run scheduler tests that distinguish touched callers/callees/untouched functions, and compare `--inlining-optimizing` output against Binaryen and the debug artifact.

- [INL]003 - Heuristic Classes, Action Filtering, and Size/Iteration Policy
  - Status: active follow-up after `[INL]001` direct-surface signoff. Current Starshine still uses a smaller tiny/one-use subset plus selective exact-`unreachable` cleanup heuristics instead of Binaryen's full `Shrinks` / `MayNotShrink` / flexible policy ladder, optimize-level-sensitive loop/call gating, deterministic action filtering, repeated-work caps, and combined-size growth guard.
  - Deliverables: model the remaining Binaryen heuristic classes/options that are still in-surface for direct calls, add reduced tests for trivial-wrapper/flexible/action-filtering families, and rerun direct compare lanes to show heuristic-only mismatches move under this slice rather than `[INL]001`.

- [INL]004 - No-Inline Flags and Clone Survival
  - Status: active unsupported direct-inliner breadth. Starshine does not yet model Binaryen's `no-inline`, `no-full-inline`, or `no-partial-inline` suppression flags, and it has no local proof that any future function-cloning utility preserves those flags the way Binaryen's `copyFunction` does.
  - Deliverables: represent the three suppression controls locally, preserve them across future clone/copy helpers, add focused fixtures for `no-inline.wast` and `no-inline-monomorphize-inlining.wast` families, and keep inline-hint metadata explicitly distinct from no-inline policy.
...
- [INL]007 - Plain `inlining` Direct Signoff
  - Status: active follow-up. The shared direct-call core now has green optimizing evidence, but plain `--pass inlining` still lacks its own direct 10k signoff and should not inherit optimizing acceptance automatically.
  - Deliverables: add any plain-only stop-point regressions needed to protect the non-optimizing contract, run direct `--pass inlining` compare lanes on at least the standard and one broadened seed, and classify any residual plain-only mismatches separately from `[INL]002` optimizing-scheduler work.
