...
- **Current progress:** analysis-only Flat IR classification, scalar function-result materialization, reachable/unreachable `local.tee` lowering at function roots, structured-region roots, and ordinary operand positions, ordered scalar ordinary-operand preludes, branch-free defaultable scalar `block`/`if` routing, branch-free zero-input/no-backedge scalar `loop` result routing, plain carried scalar `br`, simple same-type carried scalar `br_if`, and the source-backed target-type versus false-path-flow-type `br_if` mismatch are implemented with focused verifier-backed tests. Direct terminal branches now unblock target-block result erasure, mixed supported `br`/`br_if` exits share one target temp, and mismatched `br_if` uses a second typed flow temp while evaluating the payload once. Simple scalar `br_table` fanout now evaluates one payload into a staging temp and copies it to every unique defaultable scalar block-target temp, including duplicate-label deduplication and terminal payload-root cleanup. Rich/multivalue `br_if` and `br_table` payloads now trigger an intentional whole-function fail-closed gate before operand spilling so shared flowing values cannot be reevaluated; implementing their shared-origin route remains open. Inputful/backedge loops, branch-targeted `if`, legacy-try values, generic unreachable placeholders, EH, profiles, and public wiring remain open.
- **Deliverables:**
