...
- DeadCodeElimination research baseline:
  ...
  - next step: rerun normalized Starshine-vs-Binaryen comparison at the now-valid direct DCE and five-pass checkpoints, then investigate any remaining size/output-shape gap as parity work instead of correctness repair.
...
## v0.1.0 Active Slice Focus
  ...
  - implementation features:
    ...
    - direct native replay now validates again for both fresh-artifact `--dead-code-elimination` and `DFE -> RUME -> MemoryPacking -> OnceReduction -> DCE`; the remaining work is parity comparison, not another DCE validity blocker.
