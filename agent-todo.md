...
- **Latest bounded iteration (2026-07-15):** nonthrowing synthetic catch-all lowering and recursive suffix ownership remain the current narrow measured output win (`212` Starshine cleanup bytes versus `236` Binaryen). The latest profiler-backed follow-up admits direct `i32.or`, caches immutable EH prerequisites once per run, and makes try-target terminal checks consume the admission-time suffix proof instead of rebuilding use-def. Representative pass-only timing improves from `2,345.5 us` to `1,641 us` (`6.17x` Binaryen v130's `266.05 us`), still outside the `<=2x` target. Typed catches/payloads, nested-pop repair, `rethrow`, `delegate`, structured suffix control deletion, broader behavior, a GenValid profile, four-lane signoff, performance, and public wiring remain open. Source: `docs/wiki/raw/binaryen/2026-07-15-flatten-version-130-nonthrowing-bridge-suffix-cache-impact.md`.
- **Deliverables:**
