...
## 2026-03-27 RemoveUnusedBrs Parity: Lowered scalar returned value ladders with reorder-safe side-effect prefixes to `select` instead of leaving Binaryen-mismatched result `if` blocks behind

This checkpoint broadens the result-`if` to `select` rewrite in [`src/passes/remove_unused_brs.mbt`](/home/jtenner/Projects/starshine-mb-remove-unused-brs/src/passes/remove_unused_brs.mbt) from single-root arms to whole value-producing arm regions. The matcher now builds reorder-safe region expressions for each arm and treats `drop` of a reorder-safe value as reorder-safe itself, which allows returned scalar ladders like “direct return value” versus “side-effect prefix plus return value” to lower all the way to `select` the same way Binaryen does. That specifically covers the old `$2919`-style scalar family where Starshine had already stripped inner `return`s but was still leaving a result-typed `if` inside a wrapper block.

Regression coverage in [`src/passes/remove_unused_brs_test.mbt`](/home/jtenner/Projects/starshine-mb-remove-unused-brs/src/passes/remove_unused_brs_test.mbt) now includes the side-effect-prefix scalar ladder and a nested returned-wrapper sanity case so this path stays exercised after future parity work. Verification for this checkpoint was completed with `moon info`, `moon fmt`, `moon test src/passes/remove_unused_brs_test.mbt`, `moon test src/passes/perf_test.mbt`, and full `moon test`.

## 2026-03-27 RemoveUnusedBrs Parity: Stopped scalar return-fed control rewrites from voidifying value `if` ladders that Binaryen keeps as result producers

This checkpoint tightens [`RemoveUnusedBrs`](/home/jtenner/Projects/starshine-mb-remove-unused-brs/src/passes/remove_unused_brs.mbt) around single-value return-fed control. Tail `return (if (result ...))` rewrites now only take the old voidification path when both original arms already exit, which lets simple payload arms fall through to the later return-strip path that Binaryen uses. Return-context propagation through embedded control was also widened so result-bearing `block` and `loop` nodes under a `return` stay in return-context even before they contain explicit nested `return`s, preventing the branch-payload arm rewrite from firing inside returned loop bodies. In practice, that fixes the scalar returned-loop and returned-block families that were still collapsing to `if (Void) ... br ... unreachable` instead of staying as result-producing `if`/`select` forms.

Regression coverage was updated in [`src/passes/remove_unused_brs_test.mbt`](/home/jtenner/Projects/starshine-mb-remove-unused-brs/src/passes/remove_unused_brs_test.mbt) to reflect the actual Binaryen-oracle scalar behavior: simple tail-return payload arms and returned loop bodies now assert that `return` and `if (Void)` disappear, and new scalar returned-block ladder cases assert the expected `select`-based lowering. Verification for this checkpoint was completed with `moon info`, `moon fmt`, `moon test src/passes/remove_unused_brs_test.mbt`, `moon test src/passes/perf_test.mbt`, and full `moon test`.
