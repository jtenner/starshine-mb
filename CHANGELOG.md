...
## 2026-03-26 Optimize: split DCE branch-presence checks from branch rewrite lists

- **Dead-code-elimination branch-presence cache checkpoint** by **@jtenner**. Updated [CHANGELOG.md](/home/jtenner/Projects/starshine-mb-dce/CHANGELOG.md) and [src/passes/dead_code_elimination.mbt](/home/jtenner/Projects/starshine-mb-dce/src/passes/dead_code_elimination.mbt) so unchanged branch-heavy DCE functions now use a cheap per-label incoming-branch bitmap for fallthrough and voidify eligibility queries, and only build full branch-user lists when a rewrite path actually needs the concrete users. On the traced native `--dead-code-elimination` replay for `tests/node/dist/starshine-debug-wasi.wasm`, total DCE pass time dropped from about `629958us` to `621402us` while preserving byte-identical wasm output; large unchanged dispatcher functions like `Func 1897`, `Func 1035`, `Func 2751`, and `Func 1550` all moved in the right direction, though parity is still incomplete and the Binaryen 50% speed target is still not met.
...
## 2026-03-26 Optimize: skip branchless typed DCE bodies with impure dropped values

- **Dead-code-elimination impure-drop raw-skip checkpoint** by **@jtenner**. Updated [CHANGELOG.md](/home/jtenner/Projects/starshine-mb-dce/CHANGELOG.md), [src/passes/pass_manager.mbt](/home/jtenner/Projects/starshine-mb-dce/src/passes/pass_manager.mbt), and [src/passes/perf_test.mbt](/home/jtenner/Projects/starshine-mb-dce/src/passes/perf_test.mbt) so the branchless typed raw-skip path now accepts dropped values only when each `drop` immediately follows a definitely impure value producer, currently imported/direct/indirect calls and non-void structured controls. The new focused regression keeps pure dropped values on the hot path; in traced replay on `tests/node/dist/starshine-debug-wasi.wasm`, `skip-raw reason=no-dce-candidates` rose again from `1879` to `1909`, and hot unchanged functions `Func 2893` and `Func 1239` now skip raw lift entirely. Recent direct native `--dead-code-elimination` runs on the large artifact came in at about `2.69s` and `2.96s`, down from the prior post-`6fc0d60` runs around `2.95-2.97s` with a `3.42s` outlier. Parity is still incomplete and the Binaryen 50% speed target is still not met.

## 2026-03-26 Optimize: skip branchless typed DCE value bodies before hot lift

- **Dead-code-elimination branchless typed raw-skip checkpoint** by **@jtenner**. Updated [CHANGELOG.md](/home/jtenner/Projects/starshine-mb-dce/CHANGELOG.md), [src/passes/pass_manager.mbt](/home/jtenner/Projects/starshine-mb-dce/src/passes/pass_manager.mbt), and [src/passes/perf_test.mbt](/home/jtenner/Projects/starshine-mb-dce/src/passes/perf_test.mbt) so the raw DCE precheck now reuses the existing recursive instruction scan to track nested dead tails and drops, then skips branchless value-returning functions whose final top-level instruction is a fallthrough typed structured control. The new focused perf regressions keep nonfallthrough typed wrappers on the hot path; on `tests/node/dist/starshine-debug-wasi.wasm`, `skip-raw reason=no-dce-candidates` rose from `1578` to `1879` in traced replay, hot parser/wast functions like `Func 871` and `Func 899` now skip raw lift entirely, and repeated direct native `--dead-code-elimination` runs improved from the current ~`3.49s` baseline to about `2.95-2.97s` with one slower outlier at `3.42s`. Parity is still incomplete and the Binaryen 50% speed target is still not met.

## 2026-03-26 Optimize: skip DCE label-user indexing when a function has no branches

- **Dead-code-elimination branchless label-user checkpoint** by **@jtenner**. Updated [CHANGELOG.md](/home/jtenner/Projects/starshine-mb-dce/CHANGELOG.md) and [src/passes/dead_code_elimination.mbt](/home/jtenner/Projects/starshine-mb-dce/src/passes/dead_code_elimination.mbt) so DCE now proves whether a function contains any branch nodes before building per-label branch-user lists, and falls back to a pre-sized empty label map for fully branchless functions instead of paying the full target-indexing scan. After rebuilding the native CLI in the DCE worktree, the direct `--dead-code-elimination` replay on `tests/node/dist/starshine-debug-wasi.wasm` improved from the current `4.56s` baseline to about `3.24s`; parity is still incomplete and the Binaryen 50% speed target is still not met.
