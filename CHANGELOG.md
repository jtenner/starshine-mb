...
- **`heap2local` red-suite checkpoint** by **@jtenner**. Updated [`CHANGELOG.md`](./CHANGELOG.md), [`agent-todo.md`](./agent-todo.md), [`docs/0075-2026-04-03-heap2local-binaryen-comparison.md`](./docs/0075-2026-04-03-heap2local-binaryen-comparison.md), [`src/passes/heap2local.mbt`](./src/passes/heap2local.mbt), [`src/passes/heap2local_primary_test.mbt`](./src/passes/heap2local_primary_test.mbt), [`src/passes/heap2local_test.mbt`](./src/passes/heap2local_test.mbt), [`src/passes/optimize.mbt`](./src/passes/optimize.mbt), [`src/passes/optimize_test.mbt`](./src/passes/optimize_test.mbt), [`src/passes/pass_manager.mbt`](./src/passes/pass_manager.mbt), and [`src/passes/registry_test.mbt`](./src/passes/registry_test.mbt) so the worktree now ships a first `heap2local` pass slice for exclusive direct struct field traffic, wires that pass into the modeled optimize/registry surfaces, records Binaryen `version_125` behavior as the canonical shape matrix, and adds a primary parity suite that is intentionally red only on the currently missing Binaryen features. The active parity gaps are exclusive local-copy flow, tee flow, block result flow, `ref.as_non_null`, `ref.eq`, `ref.cast`, descriptor-bearing `struct.new_default_desc` plus `ref.get_desc`, `array.new_default`, `array.new`, `array.new_fixed` constant-index traffic, array `ref.test`, and the loop-carried mixed-provenance bailout.

## 2026-04-03 Signoff: land `reorder-locals` as an explicit module pass

- **`reorder-locals` explicit-pass signoff** by **@jtenner**. Updated [`CHANGELOG.md`](./CHANGELOG.md), [`agent-todo.md`](./agent-todo.md), [`docs/0063-2026-03-24-pass-port-batches-and-registry-map.md`](./docs/0063-2026-03-24-pass-port-batches-and-registry-map.md), [`docs/0073-2026-04-02-reorder-locals-binaryen-comparison.md`](./docs/0073-2026-04-02-reorder-locals-binaryen-comparison.md), [`docs/0074-2026-04-02-binaryen-multivalue-call-local-disparity.md`](./docs/0074-2026-04-02-binaryen-multivalue-call-local-disparity.md), [`scripts/lib/pass-fuzz-compare-task.ts`](./scripts/lib/pass-fuzz-compare-task.ts), [`scripts/lib/self-optimize-compare-task.ts`](./scripts/lib/self-optimize-compare-task.ts), [`scripts/test/pass-fuzz-compare-command.ts`](./scripts/test/pass-fuzz-compare-command.ts), [`scripts/test/self-optimize-compare-binaryen-require-converged-command.ts`](./scripts/test/self-optimize-compare-binaryen-require-converged-command.ts), [`scripts/test/self-optimize-compare-binaryen-roundtrip-command.ts`](./scripts/test/self-optimize-compare-binaryen-roundtrip-command.ts), [`scripts/test/self-optimize-compare-binaryen-stable-roundtrip-command.ts`](./scripts/test/self-optimize-compare-binaryen-stable-roundtrip-command.ts), [`scripts/test/self-optimize-compare-binaryen-unstable-roundtrip-command.ts`](./scripts/test/self-optimize-compare-binaryen-unstable-roundtrip-command.ts), [`src/cmd/cmd_test.mbt`](./src/cmd/cmd_test.mbt), [`src/passes/optimize.mbt`](./src/passes/optimize.mbt), [`src/passes/optimize_test.mbt`](./src/passes/optimize_test.mbt), [`src/passes/pass_manager.mbt`](./src/passes/pass_manager.mbt), [`src/passes/pkg.generated.mbti`](./src/passes/pkg.generated.mbti), [`src/passes/registry_test.mbt`](./src/passes/registry_test.mbt), [`src/passes/reorder_locals.mbt`](./src/passes/reorder_locals.mbt), and [`src/passes/reorder_locals_test.mbt`](./src/passes/reorder_locals_test.mbt) so `reorder-locals` now ships as an explicit module pass with focused CLI and registry coverage, documented stable-boundary compare tooling, and a measured hot-path cleanup that cut the debug-artifact runtime from `176.364 ms` into a retained `121.286-133.330 ms` band. Remaining raw emitted-wasm drift is documented as Binaryen multivalue-call writeback/materialization outside the sorter itself, so preset wiring stays deferred until the neighboring local-shape passes land.

## 2026-04-01 Checkpoint: validate `precompute` writebacks against full module context

- **`precompute` full-module validation checkpoint** by **@jtenner**. Updated [`CHANGELOG.md`](./CHANGELOG.md), [`agent-todo.md`](./agent-todo.md), [`docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md`](./docs/0066-2026-03-24-binaryen-no-dwarf-default-optimize-path.md), [`src/cmd/cmd_test.mbt`](./src/cmd/cmd_test.mbt), [`src/ir/hot_module_context.mbt`](./src/ir/hot_module_context.mbt), [`src/ir/hot_module_context_test.mbt`](./src/ir/hot_module_context_test.mbt), [`src/ir/pkg.generated.mbti`](./src/ir/pkg.generated.mbti), [`src/passes/pass_manager.mbt`](./src/passes/pass_manager.mbt), [`src/passes/precompute.mbt`](./src/passes/precompute.mbt), [`src/passes/precompute_test.mbt`](./src/passes/precompute_test.mbt), [`src/validate/pkg.generated.mbti`](./src/validate/pkg.generated.mbti), and [`src/validate/validate.mbt`](./src/validate/validate.mbt) so `precompute` now folds immutable `global.get` inputs through the hot module context, validates rewritten functions against the full module environment instead of in isolation, and locks the artifact replay plus reduced repro that were previously failing closed as false `invalid function index` skips. Focused `passes` and `validate` suites are green, and the native debug-artifact `--precompute` replay now drops from `1189` traced skips to `14`, leaving only the real `stack underflow` and `invalid-escape-carrier` families as the remaining parity blockers.
...
- **`pick-load-signs` signoff** by **@jtenner**. Updated [`CHANGELOG.md`](./CHANGELOG.md), [`agent-todo.md`](./agent-todo.md), [`docs/0069-2026-03-26-pick-load-signs.md`](./docs/0069-2026-03-26-pick-load-signs.md), [`scripts/lib/self-optimize-compare-task.ts`](./scripts/lib/self-optimize-compare-task.ts), [`scripts/test/self-optimize-compare-command.ts`](./scripts/test/self-optimize-compare-command.ts), [`src/cmd/cmd_test.mbt`](./src/cmd/cmd_test.mbt), [`src/passes/pass_manager.mbt`](./src/passes/pass_manager.mbt), and [`src/passes/perf_test.mbt`](./src/passes/perf_test.mbt) so `self-optimize-compare` now times the latest native command via `moon run --target native --release src/cmd --` after an untimed release build warmup, and `pick-load-signs` now raw-skips functions that cannot possibly rewrite before hot lift/use-def. Native artifact replay is locked in `cmd_test`, raw-skip coverage is locked in `perf_test`, and the debug artifact compare is now green at `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --pick-load-signs` with exact canonical/WAT parity plus Starshine runtime `2067.184 ms` versus Binaryen `1408.509 ms`. Differential fuzz parity is also green at `.tmp/pass-fuzz-pls-genvalid-10000`, which completed `10000/10000` compared with `10000` normalized matches and `0` mismatches or failures.
