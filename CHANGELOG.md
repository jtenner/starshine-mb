...
- Kept and clarified the non-pass planning edges that still matter during the transition: the MoonBit `name` subsection decode blocker remains in publishing blockers, `docs/pass-audit.md` is still called out for refresh against the `src/optimization` package, and the bottom-of-file section is now clearly framed as longer-horizon non-pass work rather than mixed in with immediate optimizer implementation items.
