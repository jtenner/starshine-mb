...
`,
  ...
  const fuzzOutDir = path.join(tmpdir, "fuzz-out");
  runBun(
    repoRoot,
    [
      "fuzz",
  ...
  assert(
    actualFuzzSweep === `run --target wasm src/fuzz -- cmd-harness smoke --seed 0x10 --seed-count 5 --shard-index 1 --shard-count 2 --report-json ${fuzzReportPath} --out-dir ${fuzzOutDir}`,
