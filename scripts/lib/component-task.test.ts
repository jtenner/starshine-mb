...
describe("component task", () => {
  ...
  test("makes shuffled MoonBit FFI declarations reproducible without changing bodies or annotations", () => {
    const header = "// Generated by wit-bindgen\n\n";
    ...
    const first = '#doc(hidden)\npub fn wasmExportFirst() -> Int {\n  @api.first()\n}';
    const second = '#doc(hidden)\npub fn wasmExportSecond(p0 : Int) -> Int {\n  @api.second(p0)\n}';
    const shuffled = header + [owned, free, second, first].join("\n\n") + "\n";
    const ordered = header + [free, owned, first, second].join("\n\n") + "\n";

    expect(normalizeGeneratedText(shuffled, true)).toBe(ordered);
    expect(normalizeGeneratedText(ordered, true)).toBe(ordered);
    expect(normalizeGeneratedText(shuffled)).toBe(shuffled);
  });
