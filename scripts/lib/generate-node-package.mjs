...
function renderReadme(packageMetas) {
  ...
  lines.push('');
  lines.push('## Core Locals Model');
  lines.push('');
  lines.push('Starshine now uses one canonical representation for function body locals across the Node package too.');
  lines.push('');
  lines.push('- Function body locals use `lib.Locals`, not `Array<ValType>`.');
  lines.push('- `lib.Locals` stores ordered `lib.LocalRun` entries with `count` + value type instead of a flattened local-type array.');
  lines.push('- Internal local-type lookup is run-based and uses a cached derived run-start index array; the cache is not the source of truth.');
  lines.push('- Typed `TFunc` values and plain `Func` values both use `Locals` for declared non-parameter body locals.');
  lines.push('- Parameters remain plain `Array<ValType>`; only body locals use the compact run representation.');
  lines.push('');
  lines.push('For JavaScript callers this mostly matters when constructing or inspecting locals directly:');
  lines.push('');
  lines.push('```js');
  lines.push("import { lib } from '@jtenner/starshine';");
  lines.push('');
  lines.push('const bodyLocals = lib.Locals.new([');
  lines.push('  lib.LocalRun.new(2, lib.ValType.i32()),');
  lines.push('  lib.LocalRun.new(1, lib.ValType.funcref()),');
  lines.push(']);');
  lines.push('```');
  lines.push('');
