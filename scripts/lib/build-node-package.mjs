...
export function buildNodePackage({
  ...
  run(moonBin, ['build', '--target', 'wasm-gc', '--release', '--package', 'jtenner/starshine/node_api'], root);

  console.log('Building optimized WASI CLI...');
  run(moonBin, ['build', '--target', 'wasm', '--release', '--package', 'jtenner/starshine/cmd'], root);
