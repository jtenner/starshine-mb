...
- For DAE / `dae-optimizing` mixed-generator compare-pass lanes, include `--normalize drop-consts` so known generated dropped-constant debris counts as `cleanupNormalizedMatchCount` instead of blocking the run as raw mismatches; still classify any remaining mismatches normally.
