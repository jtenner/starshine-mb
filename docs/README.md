...
- Run pass microbenchmarks through MoonBit's benchmark runner with `moon bench --release --target native src/passes_perf_long`; narrow to one source with `--package jtenner/starshine/passes_perf_long --file <file>`. Benchmark fixtures must be built outside `it.bench(...)`, validated once in setup, and proven reusable; use `validate_final_module=false` only for explicitly pass-local lanes. The twenty-two-case calibrated suite currently covers pass dispatch plus HOT lift/lower attribution for Directize, HeapStoreOptimization, MergeBlocks, RemoveUnusedBrs, ReorderGlobals, SimplifyLocals raw/breadth paths, and TupleOptimization. HeapStoreOptimization additionally has lift-plus-direct-pass lanes for both ordered fail-closed stores and allocation-only constructor sinking, separating pass work from lift/lower and proving when an exact raw path bypasses that envelope. Keep artifact/Binaryen comparisons in the paired wall-attribution tooling rather than treating synthetic Moon benchmarks as command-level parity evidence.
