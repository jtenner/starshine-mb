...
- Run pass microbenchmarks through MoonBit's benchmark runner with `moon bench --release --target native src/passes_perf_long`; narrow to one source with `--package jtenner/starshine/passes_perf_long --file <file>`. Benchmark fixtures must be built outside `it.bench(...)`, validated once in setup, and proven reusable; use `validate_final_module=false` only for explicitly pass-local lanes. The twenty-nine-case calibrated suite currently covers pass dispatch plus HOT lift/lower attribution for Directize, HeapStoreOptimization, MergeBlocks, RemoveUnusedBrs, ReorderGlobals, SimplifyLocals raw/breadth paths, TupleOptimization, and Vacuum. HeapStoreOptimization additionally has lift-plus-direct-pass lanes for both ordered fail-closed stores and allocation-only constructor sinking. Vacuum adds flat guarded-hazard lift/pass/lower/registry lanes plus depth-512 and depth-1024 singleton-wrapper registry cases, separating generic HOT cost from exact raw structural cleanup. Keep artifact/Binaryen comparisons in the paired wall-attribution tooling rather than treating synthetic Moon benchmarks as command-level parity evidence.
