...
- Status:
  - partial.
  - landed subset: closed-world typed `struct.new` / `struct.new_desc` payloads now defer unread field payload uses, unread payloads keep only reference identity alive until a live `struct.get` / `struct.get_s` / `struct.get_u` read occurs, `struct.new_desc` / `struct.new_default_desc` now also treat deferrable descriptor operands as reference-only carriers instead of eagerly promoting descriptor globals, mutable `struct.set` now feeds those same deferred struct-field buckets instead of eagerly promoting unread writes, referenced globals are preserved through that deferred path so unread field payload graphs are not compacted away unsafely, ref-bearing `array.new` / `array.new_fixed` / `array.new_elem` payloads now defer unread payload uses until a live `array.get` / `array.get_s` / `array.get_u` read occurs, mutable `array.set` / `array.init_elem` plus ref-bearing `array.fill` now feed the same deferred array-content buckets instead of eagerly promoting unread payloads, ref-bearing `array.copy` now propagates deferred payload identity across array types so unread copies remain reference-only until a live destination read occurs, and reference-identity plus conversion-style consumers (`ref.is_null`, `ref.as_non_null`, `ref.test`, `ref.cast`, `ref.eq`, `ref.get_desc`, `ref.test_desc`, `ref.cast_desc_eq`, `br_on_null`, `br_on_non_null`, `br_on_cast`, `br_on_cast_fail`, `extern.convert_any`, nested `any.convert_extern`) plus dropped carrier globals (`drop(global.get(...))`) now preserve only reference identity instead of forcing global carrier contents live.
  - remaining work: any future GC consumer expansion beyond the current struct/array, identity, and conversion instruction surface, plus later hardening if new reference-only carrier kinds appear beyond the descriptor/global cases already handled and the now-landed dropped-global case. `array.new_data` is not a remaining GC payload blocker under current Starshine validation because it is limited to numeric/vector element arrays.
