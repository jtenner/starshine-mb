...
- Status:
  ...
  - remaining work: any future GC consumer expansion beyond the current struct/array instruction surface, plus later hardening if new reference-only carrier kinds appear beyond the descriptor/global cases already handled. `array.new_data` is not a remaining GC payload blocker under current Starshine validation because it is limited to numeric/vector element arrays.
