...
- The current guard is narrower again. `src/passes/code_pushing.mbt` no longer
  ...
  prior/later same-source guards. One sharper standalone caveat is tracked
  separately now too: the autogenerated standalone `Func 1977` reconstruction is
  ...
  remove the standalone skip. The recursive suspicious walk is sharper again as
  well: it bottoms out at two mirrored leaf wrappers in the earlier tag-`76` /
  tag-`77` decode ladders rather than near the final reopened local-`45` tail.
