...
- The current guard is narrower again. `src/passes/code_pushing.mbt` no longer
  ...
  one-off-tail slice, the reduced crossed-gap positive slice, the reduced
  branch-root / nested-branch / branch-free parent-escape non-void-prefix cases,
  and the reduced nested result-`if` condition sink while still keeping the real
  same-source crossed-condition-set case fenced. The non-void explicit-exit
  prefix rule is candidate-aware now too: earlier unsafe `Block` prefixes are
  no longer blanket blockers for later safe candidates, but the pass still keeps
  the true same-source alias-condition-set carrier family fenced with dedicated
  prior/later same-source guards. The sharper standalone caveat is narrower now
  too: the old autogenerated standalone `Func 1977` invalid-lower fallback is
  closed, current-source standalone replay once again admits the reopened
  `Func 1977` local-`38` / `33` / `45` motion family and the repeated
  standalone `Func 1948` local-`135`-through-`145` / `153`-through-`163`
  family at the pass layer, and the remaining exact standalone WAT drift is now
  split more honestly. In `Func 1948`, most of that residual drift already
  exists in the no-pass reconstruction baseline. In `Func 1977`, the remaining
  residual drift is mostly Starshine writeback canonicalization after the now-
  correct admitted move, not another missed pushpoint-admission blocker.
  The refreshed named lanes
