...
- `[FZG]016` attaches the SIMD phase-3 counter: `SimdPhase3` reports nonzero coverage when the instruction scan sees SIMD memory, shuffle/swizzle, narrow, pairwise-add, dot-product, or float demote/promote forms. Coverage-forced modules now emit valid drop-wrapped phase-3 SIMD forms behind `allow_v128` when a memory is available; keep shuffle, lane, and memory-argument fixtures aligned with [`../wast/simd-authoring.md`](../wast/simd-authoring.md).
...
- `gen_valid coverage-forced emits simd phase 3 surface` proves the `[FZG]016` module validates, emits SIMD memory, shuffle/swizzle, narrow, pairwise-add, dot-product, and float demote/promote forms, and satisfies an explicit `SimdPhase3` floor. WAST text fixtures for these shapes should follow [`../wast/simd-authoring.md`](../wast/simd-authoring.md).
