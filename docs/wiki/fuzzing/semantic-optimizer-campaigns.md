...
The executor constructs typed deterministic imports, including `WebAssembly.Tag`, isolates execution in a killable worker, distinguishes independent and stateful invocation modes, records start/import events, snapshots imported and exported globals/memories/tables, hashes every observed memory byte, and blocks over-cap resources instead of sampling them as equivalent. Immutable active element segments provide exact `funcidx:<n>` identity across tables; modules with runtime table mutation still block cross-table identity. Every three-way report records runtime-interface, invocation-plan, original observation, Starshine observation, Binaryen observation, comparison, and total milliseconds. A parent deadline that kills the Starshine worker before it reports a compilation, instantiation, or invocation phase is `blocked-starshine-runtime` with pattern `starshine-tool-resource-uncertainty`; the report retains the exact `timeout:<n>ms` diagnostic, and compare-pass counts it as blocked rather than as a semantic failure. A completed worker that reports a known candidate compilation or interface failure remains a Starshine correctness failure. The semantic execution contract is versioned so cached reports and resumed journals created under the earlier timeout classification cannot be reused. Focused tests cover wrong scalar results, scalar and trap mismatches beside unrelated blocked resources, signed zero, strict versus canonical NaN, exact event prefixes at traps, full-memory changes beyond 64 KiB, over-cap blocking, static cross-table aliases, imported exceptions, v128 imports/exports, non-null exported and imported-function `i31ref` observations, nullable references, retained non-null struct and array crossings, explicit exception and continuation boundaries, trap normalization, deterministic plans, stage timings, subprocess timeout attribution, and three-way classifications.
...
- **Convergence** records every generation's canonical hash, exact raw-byte hash, and encoded size, detects fixed points, full structural cycles, late validation failure, late semantic divergence, persistent growth, and bounded nonconvergence. Consecutive exact raw hashes establish a fixed point for a deterministic optimizer; nonadjacent exact raw repetition establishes a cycle. Canonical hashes remain in diagnostics but never prove either condition because distinct raw inputs can share one projection and optimize differently. A generation with unavailable raw identity cannot establish a fixed point or cycle, so the search continues to the configured bound unless later known raw states do. Compare-pass accepts `--property convergence --convergence-max <n>` and persists every generated artifact and `starshine.optimizer-property-result.v1` record.
