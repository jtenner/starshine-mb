...
1. Mirror already-valid generated instruction families where the parser supports the syntax: GC refs/types, exception forms, memory/table bulk ops, descriptor reference test/cast forms, call-ref and tail-call forms, SIMD text forms, and richer import/export declarations. Ordinary `ref.test`, ordinary `ref.cast`, and `br_on_*` remain blocked on WAST parser/printer widening; see [`../wast/reference-instruction-authoring.md`](../wast/reference-instruction-authoring.md).
