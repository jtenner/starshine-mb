...
This file captures the official online sources rechecked after the 2026-04-23 `dfo` dossier was created. The goal was not to rewrite the dossier from scratch, but to verify whether Binaryen current `main` had drifted in a way that changes the teaching contract for Starshine developers.
...
## Durable current-main observations

- `DataFlowOpts.cpp` on current `main` still presents the pass as `Optimize using the DataFlow SSA IR`, still says `flatten` must run before it, still suggests `--flatten --dfo -Os`, and still implements a function-parallel pass.
...
- The current rewrite surface remains the same small pair as the `version_129` dossier: identical-input phi collapse when the replacement is constant, and all-constant `Expr` folding through nested `precompute`.
- `graph.h` on current `main` still defines DataFlow IR as an SSA representation designed to be convertible to Souper IR, still uses the integer-only `isRelevantType(type) { return type.isInteger(); }` filter, still models EH as unsupported, and still deliberately avoids ordinary loop phis by installing unknown loop-entry `Var`s when real loop variation is observed.
- `pass.cpp` on current `main` still registers the public upstream pass spelling `dfo` with the description `optimizes using the DataFlow SSA IR`.
...
- `src/passes/optimize.mbt:144-152` still keeps the local compatibility spelling `dataflow-optimization` in the removed-pass registry.
- `src/passes/optimize.mbt:468-472` still rejects explicit requests for removed passes with the removed-registry error path.
- `src/passes/optimize.mbt:156-279` still contains no active `dataflow-optimization` descriptor entry.
...
The only local hygiene issue found during this recheck was stale line anchors in the Starshine status page for the removed-pass request guard; the living page was updated to the current `src/passes/optimize.mbt:468-472` range.
