...
1. use the recovered high-resolution native-release `passes_perf_long` reconstruction under `.tmp` to profile recursive region traversal and target-copy/local construction now that every sparse conditional-flow lookup is binary; retain sparse proof storage unless an exact benchmark and failure-boundary proof justify another representation, commit a benchmark only if it is a legitimate maintained long-performance surface rather than telemetry, and do not replace the `970.5 us` gate unless the original measurement contract is recovered or explicitly requalified;
