...
For a payload-left binary, the right sibling may instead be one exact one-use supported scalar origin. Generic postorder operand flattening materializes it after the payload-free branch and before the binary. Pinned-v130 legacy-try and inputful-loop probes place rich right calls in that same order, so effects and traps remain not-taken-only. Type changes, multiple or non-lane-zero rich-left operands, rich payload origins combined with pre-branch left work, inputful-loop rich-left routing, sharing, nesting, delayed consumers, mixed tuple/scalar ownership, and repair-sensitive EH remain rejection.
...
The reversed-binary, rich-right, and rich-left iterations intentionally made no timing claim. They follow the updated feature-first priority: close semantic/control families before spending further effort on millisecond-scale micro-optimization. The durable `970.5 us` / `3.65x` checkpoint remains unrequalified rather than being replaced by a new small-owner benchmark.
...
- **Still gated:** typed catches and payloads, nested-pop repair, `rethrow`, `delegate`, multiple or non-lane-zero rich-left/payload-right flow, rich payload origins combined with pre-branch left work, inputful-loop rich-left routing, structured suffix control-plus-label deletion, broader branch/control families, result calls, indirect/reference calls, and public execution.
