...
- **Still gated:** typed catches and payloads, nested-pop repair, `rethrow`, `delegate`, multiple or non-lane-zero rich-left/payload-right flow, nonconstant/effectful or partial/richer if suffixes, inputful/value loops or loop backedges, try-like structured roots, mixed or multiple structured suffixes, shared descendants or external label users, broader branch/control families, result calls, indirect/reference calls, and public execution.
...
1. continue behavior-first implementation: prefer a complete typed catch-payload/nested-pop capability slice, `rethrow`/`delegate` repair, mixed/multiple or richer structured suffix controls beyond the exact block/if/loop families, or another bounded source-backed mixed/shared/nested control family; the exact independently scalar and tuple-made legacy-try and inputful-loop rich-left subsets now accept every individually supported rich payload origin;
