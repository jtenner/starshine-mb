...
- **Durable representation/repair boundary:** ordered childless scalar `Catch` entry values carry exact tag/type. Lowering accepts one scalar lane or any positive same-tag lane vector through scalar/result-only multivalue handler types. Internal flatten repairs scalar first-descendant paths through direct blocks and an `if` condition, plus arbitrary ordered direct-block-chain lane vectors, through entry locals. It also preserves any positive depth-zero catch-all rethrow population under one active catch through one `catch_all_ref` capture and repeated captured-exnref/`throw_ref` uses; each path may include an arbitrary strict direct chain of resultless untargeted if arms and blocks. One resultless outer-target delegate may be direct or end a strict direct resultless single-root unused-label catch-block chain before transparent inner-shell removal. Every unsupported population still selects the matching deferred admission before ordinary mutation.
- **Still gated:** partial or mixed-tag payload vectors, non-first-descendant or repeated payload uses outside the admitted condition path, broader independent lane paths, then/else payload ownership, nested catches, loops/multiple execution, catch-all payload extraction, sharing/outside ownership, nonzero/typed-composed/nested-catch rethrow, loop or nested-try rethrow ancestry, value-carrying or targeted rethrow ifs, if/loop/mixed catch delegate ancestry, non-direct-target/value-carrying/mixed/used-label/nested delegate populations outside the admitted block chain, multiple or non-lane-zero rich-left/payload-right flow, nonconstant/effectful or partial/richer if suffixes, inputful/value loops or loop backedges, try-like structured roots, unsupported mixed-root combinations, shared descendants or external label users, broader mixed/shared/nested branch/control families, result calls, indirect/reference calls, and public execution.
...
1. continue behavior-first implementation: extend catch repair beyond the exact scalar direct-block/if-condition path and arbitrary ordered direct-block vectors into other first-descendant lane paths plus a source-backed non-first-descendant or repeated-use policy; widen rethrow beyond positive strict direct resultless block/if ancestry into nonzero, typed-composed, loop, or nested exceptional ownership only with exact proof; widen delegate beyond direct or strict catch-block-chain representation only with matching admission/lowering proof; then close broader nested/loop-safe EH handling, richer structured roots, or another bounded mixed/shared/nested control family; retain whole-function failure atomicity for every unsupported population;
