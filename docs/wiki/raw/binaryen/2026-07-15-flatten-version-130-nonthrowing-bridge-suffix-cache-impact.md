...
1. use the recovered high-resolution native-release `passes_perf_long` reconstruction under `.tmp` to profile the remaining sequenced-root holder/node pair lookup or another demonstrated exact run-wide owner; table-target extraction, ancestry, type-result creation, target-local preflight, per-root prelude allocation, and the audited ownership/visitation scans are no longer the first suspects. Retain sparse proof storage unless an exact benchmark and failure-boundary proof justify another representation, commit a benchmark only if it is a legitimate maintained long-performance surface rather than telemetry, and do not replace the `970.5 us` gate unless the original measurement contract is recovered or explicitly requalified;
