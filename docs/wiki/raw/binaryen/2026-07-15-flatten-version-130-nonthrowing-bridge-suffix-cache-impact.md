...
1. use the recovered high-resolution native-release `passes_perf_long` reconstruction under `.tmp` to profile one remaining exact run-wide owner, preferably sequenced-root pair lookup or target-local vector validation/copy construction now that table-target extraction and terminal-payload membership are no longer linear; retain sparse proof storage unless an exact benchmark and failure-boundary proof justify another representation, commit a benchmark only if it is a legitimate maintained long-performance surface rather than telemetry, and do not replace the `970.5 us` gate unless the original measurement contract is recovered or explicitly requalified;
