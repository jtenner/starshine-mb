...
1. use the skipped maintained long-performance fixture from commit `02f402a19` at `src/passes_perf_long/flatten_sequenced_root_perf_test.mbt` to profile the remaining sequenced-root holder/node pair lookup with `moon test --release --target native --include-skipped src/passes_perf_long/flatten_sequenced_root_perf_test.mbt`, or select another demonstrated exact run-wide owner; table-target extraction, ancestry, type-result creation, target-local preflight, per-root prelude allocation, and the audited ownership/visitation scans are no longer the first suspects. The fixture preserves 15 samples at 256 and 512 pairs, checks that all 1,048,576 lookups per sample resolve, and stays skipped in ordinary validation because it is a manual measurement lane rather than a correctness-suite workload. Retain sparse proof storage unless an exact benchmark and failure-boundary proof justify another representation, and do not replace the `970.5 us` gate unless the original measurement contract is recovered or explicitly requalified;
