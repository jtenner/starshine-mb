...
- Current scalar catch-repair iteration: `55e508e83` (`feat: capture scalar catch payloads atomically`) and `864d32c06` (`feat: repair scalar typed catches in flatten`).
...
## Scalar catch-payload repair

Commits `55e508e83` and `864d32c06` implement the first Binaryen v130 `handleBlockNestedPops(...)` subset on top of the typed catch-entry representation. The generic HOT planner uses exact node-use sites and the catch body's first-descendant line. Every live typed payload in the function must be one childless scalar leading marker with exactly the entry root plus one nested use; each first-descendant ancestor must be exclusive, and any loop, nested try/catch path, multiple lane, catch-all marker, sharing, outside use, or alternate position rejects the complete transaction before mutation.

After complete preflight, HOT allocates the payload's exact local type, inserts `local.set(local, Catch(...))` immediately after the payload prefix, and replaces the old nested use with `local.get(local)`. The red-first IR invariant first failed on unbound APIs, then passed with nested-block capture and a loop-negative proving unchanged revision, nodes, roots, locals, and use edge.

Internal flatten now treats that exact plan as admissible only after every other whole-function gate passes. It applies the repair before ordinary operand/control rewriting, discards the stale pre-repair state, rebuilds all immutable branch/use/type/flatness proof snapshots, and then continues. The red-first whitebox fixture mirrors pinned v130 `try_catch_pop_fixup`: one `i32` payload beneath two nested blocks followed by a branch to the enclosing block. It locks entry capture, old-position replacement, branch target, HOT verification, lowering, and module validation.

Validation is HOT mutation `14/14`, IR `324/324`, focused flatten `263/263`, whitebox flatten `187/187`, passes `5,780/5,780`, full `9,247/9,247`, `moon fmt`, and `moon info` with 11 existing warnings. The reviewed IR `.mbti` adds only `hot_scalar_catch_payloads_repairable(...)` and `hot_repair_scalar_catch_payloads(...)`; no public flatten API is exposed. No performance lane was rerun; the durable representative remains `970.5 us`, or `3.65x` Binaryen v130. Registry, dispatcher, CLI execution, compare/API, generator/profile, presets, and public pass wiring remain absent.
...
- **Durable representation/repair boundary:** ordered childless scalar `Catch` entry values carry exact tag/type and one scalar lane lowers through a typed handler stack. Internal flatten now repairs one exact first-descendant scalar nested-pop family through an entry local. Unsupported catch populations still select `DeferredCatchPayloadRepair`, while `Rethrow`/`Delegate` select `DeferredExceptionalTransferRepair`, before ordinary mutation.
- **Still gated:** ordered multivalue typed catch entry, a second payload lane, non-first-descendant or repeated payload uses, nested catches, loops/multiple execution, catch-all payloads, sharing/outside ownership, `rethrow`, `delegate`, multiple or non-lane-zero rich-left/payload-right flow, nonconstant/effectful or partial/richer if suffixes, inputful/value loops or loop backedges, try-like structured roots, unsupported mixed-root combinations, shared descendants or external label users, broader mixed/shared/nested branch/control families, result calls, indirect/reference calls, and public execution.
...
1. continue behavior-first implementation: extend scalar catch repair beyond one first-descendant use or add ordered multivalue payload capture, then implement `rethrow`/`delegate` repair, richer structured roots beyond the exact block/if/loop roster, or another bounded source-backed mixed/shared/nested control family; retain whole-function failure atomicity for every unsupported catch population;
