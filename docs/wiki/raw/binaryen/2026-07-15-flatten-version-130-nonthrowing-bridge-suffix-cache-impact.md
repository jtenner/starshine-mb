...
## Typed catch-entry representation prerequisite

Commits `71971e924` and `161064a0f` choose the first executable Starshine representation for Binaryen legacy-EH payload `Pop` without adding a fake public stack instruction. `hot_build_catch_payload(...)` creates a childless scalar `HotOp::Catch` pseudo-value with exact tag/type; verification requires all payload markers to be the ordered leading roots of one catch region, with one tag and one entry owner. The generated IR snapshot adds that builder, exact tag query, and `InvalidCatchPayload`; no flatten API is exposed.

HOT lowering now treats those prefix roots as values already present on the exception-handler stack. One scalar lane lowers to a typed handler block targeted by `Catch(tag, depth 0)`; normal completion uses `br depth 1` to skip the handler. The same label analysis corrected the synthetic catch-all bridge from `CatchAll(depth 1)` to `CatchAll(depth 0)`, ensuring exceptional flow enters the catch body. The red-first lower test first aborted on unhandled `Catch`; an intermediate depth-1 implementation then failed validation with a payload/label type mismatch before the depth-0 handler route passed.

This resolves the representation direction only. Flatten still classifies every `Catch`/`CatchAll` function as `DeferredCatchPayloadRepair` before mutation. No payload local is allocated, no nested payload use is replaced, nested catches are not excluded by a repair walk, loops/multiple execution are not analyzed, and ordered multivalue payloads are not lowered. `Rethrow` and `Delegate` remain under `DeferredExceptionalTransferRepair`.

Validation is verify `4/4`, lower `89/89`, IR `323/323`, focused flatten `263/263`, private flatten `186/186`, passes `5,779/5,779`, full `9,245/9,245`, `moon fmt`, and `moon info` with 11 existing warnings. No performance lane was rerun; the durable representative remains `970.5 us`, or `3.65x` Binaryen v130. Registry, dispatcher, CLI execution, compare/API, generator/profile, presets, and public pass wiring remain absent.
...
- **Durable representation gate:** ordered childless scalar `Catch` entry values now carry exact tag/type and one scalar lane lowers through a typed handler stack. `flatten` nevertheless still selects `DeferredCatchPayloadRepair`, while `Rethrow`/`Delegate` select `DeferredExceptionalTransferRepair`, before any mutation. The representation direction is chosen; Binaryen-equivalent payload-to-local and nested-pop repair are not implemented.
- **Still gated:** multivalue typed catch entry, payload-to-local extraction, old-position replacement, nested-catch exclusion, loop/multiple-execution rejection, `rethrow`, `delegate`, multiple or non-lane-zero rich-left/payload-right flow, nonconstant/effectful or partial/richer if suffixes, inputful/value loops or loop backedges, try-like structured roots, unsupported mixed-root combinations, shared descendants or external label users, broader mixed/shared/nested branch/control families, result calls, indirect/reference calls, and public execution.
...
1. continue behavior-first implementation: prefer a complete typed catch-payload/nested-pop capability slice, `rethrow`/`delegate` repair, richer structured roots beyond the exact block/if/loop roster, or another bounded source-backed mixed/shared/nested control family; the exact independently scalar and tuple-made legacy-try and inputful-loop rich-left subsets now accept every individually supported rich payload origin, and exact ordinary/control dead suffix roots share one atomic mutation contract;
