...
One source-representable rich-left/payload-right subset is now admitted for legacy tries. Exactly one one-use supported rich left may precede the complete payload vector, so it must pair with lane zero after all higher lanes are consumed in reverse order. The independently scalar payload vector, or the component vector of one exclusively owned repeated `TupleMake`, may contain at most one supported rich origin. Rewrite stages the rich left before payload locals and the branch, replaces only the binary's left child with its read, and leaves the binary itself on the not-taken path. Pinned-v130 probes place the rich left call before payload calls and `br_if`, then retain the binary after the branch.

For a payload-left binary, the right sibling may instead be one exact one-use supported scalar origin. Generic postorder operand flattening materializes it after the payload-free branch and before the binary. Pinned-v130 legacy-try and inputful-loop probes place rich right calls in that same order, so effects and traps remain not-taken-only. Inputful loops now also admit the exact payload-right counterpart for independently scalar or exclusively tuple-made payload vectors containing at most one supported rich origin: admission freezes the branch root and rich-left/binary identities, and rewrite stages the rich left before payload locals and `br_if`; tuple flow inserts that left store before child-generated payload preludes. Commits `61055698d` and `d9aa4cd94` extend the same at-most-one-rich-payload vector proof to legacy tries for independently scalar lanes and one exclusively owned repeated `TupleMake`. The shared rewrite inserts the rich-left store before child-generated payload preludes, then preserves payload-vector order, the condition, payload-free branch, and false-path binary. Type changes, multiple or non-lane-zero rich-left operands, more than one rich payload, sharing, nesting, delayed consumers, mixed tuple/scalar ownership, and repair-sensitive EH remain rejection.
...
- **Still gated:** typed catches and payloads, nested-pop repair, `rethrow`, `delegate`, multiple or non-lane-zero rich-left/payload-right flow, more than one rich payload, structured suffix control-plus-label deletion, broader branch/control families, result calls, indirect/reference calls, and public execution.
