...
The reversed-binary, rich-right, legacy-try rich-left, inputful-loop rich-left, and inputful-loop rich-payload iterations intentionally made no timing claim. They follow the updated feature-first priority: close semantic/control families before spending further effort on millisecond-scale micro-optimization. The durable `970.5 us` / `3.65x` checkpoint remains unrequalified rather than being replaced by a new small-owner benchmark.
...
- **Still gated:** typed catches and payloads, nested-pop repair, `rethrow`, `delegate`, multiple or non-lane-zero rich-left/payload-right flow, more than one rich payload, the legacy-try rich-payload counterpart, structured suffix control-plus-label deletion, broader branch/control families, result calls, indirect/reference calls, and public execution.
...
1. continue behavior-first implementation: prefer a complete typed catch-payload/nested-pop capability slice, `rethrow`/`delegate` repair, verified structured control-plus-owned-label deletion, the exact legacy-try rich-payload counterpart, or another bounded source-backed mixed/shared/nested control family; the exact independently scalar and tuple-made inputful-loop rich-left subset with at most one rich payload is now complete;
