...
59. admits any positive control-only suffix vector whose roots independently satisfy the exact block, if, or loop proof, while keeping scalar/control-mixed vectors fail-closed.

The public registry, dispatcher, CLI execution path, preset scheduler, compare allowlist, and flatten API snapshots remain unchanged. Across the iterations recorded here, the generic IR `.mbti` additions are `hot_region_truncate_suffix(...)`, `hot_delete_detached_control_forest(...)`, `hot_delete_detached_control_subtree(...)`, and `hot_label_is_live(...)`; the forest API is generic IR surface, not public flatten wiring. `flatten` is still public-removed.
...
- **Still gated:** typed catches and payloads, nested-pop repair, `rethrow`, `delegate`, multiple or non-lane-zero rich-left/payload-right flow, nonconstant/effectful or partial/richer if suffixes, inputful/value loops or loop backedges, try-like structured roots, scalar/control-mixed suffixes, shared descendants or external label users, broader branch/control families, result calls, indirect/reference calls, and public execution.
...
1. continue behavior-first implementation: prefer a complete typed catch-payload/nested-pop capability slice, `rethrow`/`delegate` repair, scalar/control-mixed or richer structured suffix controls beyond the exact block/if/loop forest, or another bounded source-backed mixed/shared/nested control family; the exact independently scalar and tuple-made legacy-try and inputful-loop rich-left subsets now accept every individually supported rich payload origin;
