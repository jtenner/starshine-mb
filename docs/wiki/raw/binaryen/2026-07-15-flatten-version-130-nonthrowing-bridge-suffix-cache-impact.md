...
For a payload-left binary, the right sibling may instead be one exact one-use supported scalar origin. Generic postorder operand flattening materializes it after the payload-free branch and before the binary. Pinned-v130 legacy-try and inputful-loop probes place rich right calls in that same order, so effects and traps remain not-taken-only. Inputful loops now also admit the exact payload-right counterpart for independently scalar or exclusively tuple-made simple payload vectors: admission freezes the branch root and rich-left/binary identities, and rewrite stages the rich left before payload locals and `br_if`. Type changes, multiple or non-lane-zero rich-left operands, rich payload origins combined with pre-branch left work, sharing, nesting, delayed consumers, mixed tuple/scalar ownership, and repair-sensitive EH remain rejection.
...
- **Still gated:** typed catches and payloads, nested-pop repair, `rethrow`, `delegate`, multiple or non-lane-zero rich-left/payload-right flow, rich payload origins combined with pre-branch left work, structured suffix control-plus-label deletion, broader branch/control families, result calls, indirect/reference calls, and public execution.
...
1. continue behavior-first implementation: prefer a complete typed catch-payload/nested-pop capability slice, `rethrow`/`delegate` repair, verified structured control-plus-owned-label deletion, or another bounded source-backed mixed/shared/nested control family; the exact independently scalar and tuple-made inputful-loop rich-left subset is now complete;
