...
- **Measured Starshine wins:** nonthrowing synthetic catch-all bridge/control/local output is 24 aggregate bytes smaller than Binaryen after matched cleanup, with deterministic runtime agreement. The exact structured-block suffix removes three unreachable nodes plus one owned label immediately; Binaryen's retained direct probe shrinks `76 -> 63` bytes under matched `--vacuum --dce`, confirming the deleted family is cleanup debris under unconditional transfer.
...
- **Still gated:** typed catches and payloads, nested-pop repair, `rethrow`, `delegate`, multiple or non-lane-zero rich-left/payload-right flow, structured suffix loops/ifs/try-like roots, mixed or multiple structured suffixes, shared descendants or external label users, broader branch/control families, result calls, indirect/reference calls, and public execution.
...
1. continue behavior-first implementation: prefer a complete typed catch-payload/nested-pop capability slice, `rethrow`/`delegate` repair, broader structured suffix controls beyond the exact block/drop/const family, or another bounded source-backed mixed/shared/nested control family; the exact independently scalar and tuple-made legacy-try and inputful-loop rich-left subsets now accept every individually supported rich payload origin;
