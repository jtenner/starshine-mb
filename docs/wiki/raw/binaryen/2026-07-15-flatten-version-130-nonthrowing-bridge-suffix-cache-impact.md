...
- **Durable representation/repair boundary:** ordered childless scalar `Catch` entry values carry exact tag/type. Lowering accepts one scalar lane or exactly two same-tag lanes through scalar/result-only multivalue handler types. Internal flatten repairs the exact first-descendant scalar family and one ordered two-lane direct-block-chain family through entry locals. Unsupported catch populations still select `DeferredCatchPayloadRepair`, while `Rethrow`/`Delegate` select `DeferredExceptionalTransferRepair`, before ordinary mutation.
- **Still gated:** partial/mixed-tag/three-or-more payload lanes, non-first-descendant or repeated payload uses, broader independent lane paths, nested catches, loops/multiple execution, catch-all payloads, sharing/outside ownership, `rethrow`, `delegate`, multiple or non-lane-zero rich-left/payload-right flow, nonconstant/effectful or partial/richer if suffixes, inputful/value loops or loop backedges, try-like structured roots, unsupported mixed-root combinations, shared descendants or external label users, broader mixed/shared/nested branch/control families, result calls, indirect/reference calls, and public execution.
...
1. continue behavior-first implementation: extend catch repair beyond the exact scalar/two-lane direct paths into non-first-descendant or repeated-use policy/repair, then implement `rethrow`/`delegate`, broader nested/loop-safe EH handling, richer structured roots, or another bounded source-backed mixed/shared/nested control family; retain whole-function failure atomicity for every unsupported catch population;
