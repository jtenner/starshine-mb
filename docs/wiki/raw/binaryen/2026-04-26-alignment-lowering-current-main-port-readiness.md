# Binaryen `alignment-lowering` Current-Main And Port-Readiness Recheck

- **Captured:** 2026-04-26
- **Pass:** `alignment-lowering`
- **Scope:** focused current-main freshness plus Starshine port-readiness source map for the living `docs/wiki/binaryen/passes/alignment-lowering/` dossier.
- **Status:** immutable raw-source manifest and port-readiness note. Keep explanatory conclusions in the living dossier pages.
...
- Pass implementation on `main`: <https://github.com/WebAssembly/binaryen/blob/main/src/passes/AlignmentLowering.cpp>
...
- Pass base / local-fixup context on `main`: <https://github.com/WebAssembly/binaryen/blob/main/src/pass.h>
- Bit helper dependency on `main`: <https://github.com/WebAssembly/binaryen/blob/main/src/ir/bits.h>
- Dedicated lit test on `main`: <https://github.com/WebAssembly/binaryen/blob/main/test/lit/passes/alignment-lowering.wast>

The 2026-04-26 current-main recheck found no teaching-relevant drift from the already captured `version_129` contract. The pass remains a narrow scalar `Load` / `Store` chunk-lowering pass with fresh-local single-evaluation guards, `Bits::makeSignExt(...)` signed-load repair, operand-preserving unreachable handling, and no SIMD/atomic/bulk-memory rewrite surface in the reviewed owner file.
...
The previous living dossier already taught the transformed shapes and upstream strategy well. The missing reader aid was a direct first-slice / validation ladder that answers: “if Starshine implements this later, where should I start?”
