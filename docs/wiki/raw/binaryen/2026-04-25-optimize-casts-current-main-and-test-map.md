# Binaryen `optimize-casts` current-main implementation/test-map bridge
...
This file captures the primary online sources rechecked for the 2026-04-25 `optimize-casts` wiki refresh. It complements the earlier tagged-source manifest:
...
- `OptimizeCasts` remains the single owner file for the pass.
- `EarlyCastFinder` remains the stricter earlier-motion walker over `ref.cast` / `ref.as_non_null` candidates.
- `BestCastFinder` remains the later-reuse walker that records best casted values and less-refined gets.
- `FindingApplier` remains the materialization step that creates fresh refined locals and rewrites consumer gets.
- `ReFinalize` still runs after each rewrite half.
...
The lit file still covers the key teaching families used by the living dossier:
...
- The 2026-04-25 current-main recheck did **not** surface a teaching-relevant drift from the `version_129` contract recorded on 2026-04-22.
...
- The implementation/test split is now worth documenting directly because the folder previously had strategy and examples but no dedicated owner/helper/lit-test map page.
...
- Current Starshine still has no `src/passes/optimize_casts.mbt` owner file; the relevant local status is the removed-name registry entry, `OC` backlog slice, GC instruction/modeling primitives, and neighboring `heap2local -> local-subtyping -> coalesce-locals -> local-cse` dossiers.
...
Cite this file for current-main source provenance and implementation/test-map freshness. Cite [`2026-04-22-optimize-casts-primary-sources.md`](./2026-04-22-optimize-casts-primary-sources.md) for the original tagged-source manifest, and cite the living dossier pages for the beginner-to-advanced explanation.
