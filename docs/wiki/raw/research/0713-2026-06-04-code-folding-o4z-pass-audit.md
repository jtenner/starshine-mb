...
| Helper block / profitability / fixpoint | Uses a measured profitability heuristic, can add helper blocks, and repeats until no new fold is exposed. | Local expression-exit folds use a simplified suffix measure and direct after-block insertion; function-ending helper labels now use a node-count profitability threshold and the pass reruns its local visit/terminating-tail sequence to fixpoint. The late cleanup neighborhood has focused fixture coverage and generated wasm-smith replay evidence. | Current tests cover unprofitable tiny full-void `if` bailout; named-block and terminating-tail plans carry suffix length and profitability score; `code-folding reaches a root-anchored terminating-tail fixpoint`; `code-folding folds deeper non-root return subset then shallow fixpoint`; `code-folding prefers deeper return-call-indirect subset over shallow all-tail suffix`; `code-folding late branch-cleanup neighborhood keeps fixpoint-exposed return tails valid`; `code-folding late branch-cleanup neighborhood keeps shared return tail valid`; `code-folding late branch-cleanup neighborhood keeps non-root return subset valid`; `code-folding late branch-cleanup neighborhood keeps arbitrary non-root return subset valid`; `code-folding late branch-cleanup neighborhood keeps shared return-call tail valid`. | Exact Binaryen helper cost modeling remains a documented profitability-parity gap, not a semantic/neighborhood blocker; broad movement and EH parity remain separately owned. | `[O4Z-AUDIT-CF-J]` closed for fixpoint/late-neighborhood proof. |
...
- Broader helper-block creation and exact Binaryen profitability modeling remain conditional future work rather than a v0.1.0 direct blocker unless a semantic, validity, or proven downstream code-size issue appears. The local fixpoint and late-neighborhood cleanup proof is now covered by focused fixtures plus generated wasm-smith replay evidence.
