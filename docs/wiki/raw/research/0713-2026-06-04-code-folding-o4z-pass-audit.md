...
| General terminating `return` tails | Searches subsets of function-ending `return` tails, prefers deeper suffixes, rewrites old tails to a fresh helper label, and appends one shared suffix at function end. | Covered for adjacent no-else `if`/fallthrough tails, root-anchored non-adjacent `return` suffixes, arbitrary non-root `return` subsets with distractor tails, and subset groups that exclude a different root `return`; when the original root body can fall through, Starshine wraps it with a skip label so ordinary fallthrough does not execute the shared terminal suffix. The terminating-tail mover now accepts label-unused structured block/if/loop suffix roots through the same hoistability check used by expression-exit folding, plus narrow alpha-equivalent self-branching block cases whose internal branch targets remain inside the moved suffix; the equality helper carries a multi-label alpha map for the nested internal-label subset, guarded by crossed nested-label and outside-target tests that keep unsafe target scopes distinct. | `code-folding shares if-then and fallthrough return tails`; `code-folding shares non-adjacent return tails`; `code-folding shares non-root return tail subsets`; `code-folding shares non-root return subsets excluding a different root return`; `code-folding shares arbitrary non-root return subset among distractor tails`; `code-folding shares safe return subset past outside-target distractor`; `code-folding folds deeper non-root return subset then shallow fixpoint`; `code-folding shares non-root structured block return suffixes`; `code-folding shares non-root structured if return suffixes`; `code-folding shares non-root structured loop return suffixes`; `code-folding shares non-root self-branch block return suffixes`; `code-folding shares internal-target conditional branch return suffixes`; `code-folding shares non-root nested self-branch block return suffixes`; `code-folding keeps conditional return suffixes that branch to an ancestor target`; `code-folding shares non-root return tail subsets with root fallthrough`. | Exact helper-block cost modeling remains open; broader branch-bearing movement beyond the proven internal-target subset is conservatively rejected as a documented CF-H parity gap. | `[O4Z-AUDIT-CF-H/J]`. |
| Terminating `return_call`, `return_call_indirect`, `return_call_ref` | Records tail-call terminators through the same terminating-tail helper path. | Covered for root-anchored direct, indirect, and ref tail-call suffixes, including typed-result direct `return_call`, logical call-signature comparison for indirect/ref nodes, non-root direct/indirect/ref tail-call subsets with root fallthrough, arbitrary non-root subset-with-distractor coverage for direct, indirect, and ref tail calls, indirect deeper-subset-vs-shallow-operand coverage, direct `return_call` coverage for the narrow and nested self-branching block suffix cases plus a crossed nested-label negative, `return_call_indirect` and non-null `return_call_ref` coverage for the nested internal-label self-branching suffix case, and a crossed nested-label `return_call_indirect` negative. Tail-call-only operand suffixes are treated as Binaryen-unprofitable instead of being shared alone. | `code-folding shares direct return-call tails with typed results`; `code-folding shares non-root direct return-call tail subsets with root fallthrough`; `code-folding shares arbitrary non-root direct return-call subset among distractor tails`; `code-folding shares arbitrary non-root return-call-indirect subset among distractor tails`; `code-folding prefers deeper return-call-indirect subset over shallow all-tail suffix`; `code-folding shares arbitrary non-root return-call-ref subset among distractor tails`; `code-folding shares non-root self-branch block return-call suffixes`; `code-folding shares non-root nested self-branch return-call suffixes`; `code-folding keeps crossed nested self-branch return-call suffixes`; `code-folding shares return-call-indirect tails`; `code-folding shares non-root return-call-indirect tail subsets with root fallthrough`; `code-folding shares non-root nested self-branch return-call-indirect suffixes`; `code-folding keeps crossed nested self-branch return-call-indirect suffixes`; `code-folding shares return-call-ref tails`; `code-folding shares non-root return-call-ref tail subsets with root fallthrough`; `code-folding shares non-root nested self-branch return-call-ref suffixes`. | Needs more natural non-null `return_call_ref` text fixtures once local syntax/HOT support is cleaner; arbitrary safe subset sharing and current movement-safety negatives are covered for the tail-call family. | `[O4Z-AUDIT-CF-H/J]`. |
| Terminating `unreachable` tails | Runs terminating-tail folding for `unreachable` groups before return groups. | Covered for adjacent no-else `if`/fallthrough tails, root-anchored block-backed `unreachable` suffixes, arbitrary non-root `unreachable` subsets with root fallthrough and distractor tails, and the same narrow self-branching plus nested internal-label self-branching block suffix cases as return tails, with a crossed nested-label negative for the `unreachable` family. | `code-folding shares full if arms ending in unreachable`; `code-folding shares if-then and fallthrough unreachable tails`; `code-folding shares block-backed unreachable tails`; `code-folding shares non-root unreachable tail subsets with root fallthrough`; `code-folding shares arbitrary non-root unreachable subset among distractor tails`; `code-folding shares non-root self-branch block unreachable suffixes`; `code-folding shares non-root nested self-branch block unreachable suffixes`; `code-folding keeps crossed nested self-branch unreachable suffixes`; unreachable cleanup tests. | Exact helper cost modeling remains open; branch-bearing movement beyond the proven internal-target subset is conservatively rejected as a documented CF-H parity gap, and arbitrary safe subset sharing is covered for the unreachable family. | `[O4Z-AUDIT-CF-H/J]`. |
...
| Outside-target movement bailout | `canMove` rejects equal-looking suffixes if moving them would strand a branch target whose owner is inside the moved-out region; function-ending terminating-tail movement must use the stricter rule that moved items can only branch to labels owned inside the item. | Covered through label-use, expression-exit target-survival checks, and the terminating-tail internal-target proof. Local positives cover `return`, `unreachable`, `return_call`, `return_call_indirect`, and `return_call_ref` suffixes where nested branch targets remain inside the moved suffix; negatives cover live labels, outside-target switch-like return tails, crossed nested labels, and equal-looking conditional return suffixes whose `br_if` targets an ancestor label. | `code-folding keeps live-label structured if suffixes`; `code-folding keeps outside-target switch-like return tails`; `code-folding shares internal-target conditional branch return suffixes`; `code-folding keeps conditional return suffixes that branch to an ancestor target`; `code-folding shares non-root nested self-branch block return suffixes`; `code-folding shares non-root nested self-branch block unreachable suffixes`; `code-folding shares non-root nested self-branch return-call-indirect suffixes`; `code-folding shares non-root nested self-branch return-call-ref suffixes`; `code-folding keeps crossed nested self-branch return suffixes`; `code-folding keeps crossed nested self-branch return-call suffixes`; `code-folding keeps crossed nested self-branch unreachable suffixes`; `code-folding keeps crossed nested self-branch return-call-indirect suffixes`; branch-target survival helper, terminating-tail internal-target helper, and label-map equality in `code_folding.mbt`. | Broader branch/control-bearing terminating-tail movement remains conservatively rejected unless the target-scope proof keeps every moved target internal; this is accepted as CF-H's narrow parity gap. | `[O4Z-AUDIT-CF-H]`. |
...
- Agent closeout classification: CF-H is closed for the non-EH movement-safety surface currently representable in HOT. The remaining broad movement gap is intentionally narrow: Starshine still rejects branch/control-bearing shapes unless the target-scope proof says every moved target remains internal to the moved item, while Binaryen may have additional profitable branch-bearing cases. This is accepted as a conservative parity gap for v0.1.0 because the direct oracle lane is semantically green and the focused probes/tests cover the known unsafe target-scope families. EH-specific movement and repair remain owned by `[O4Z-AUDIT-CF-I]`, not CF-H.
