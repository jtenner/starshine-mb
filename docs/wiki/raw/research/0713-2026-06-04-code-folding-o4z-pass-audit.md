...
| `046375` | Binaryen reduces a typed-dead wrapper to `nop; unreachable`; Starshine now emits a valid typed-dead `block { unreachable }` plus the root `unreachable` and unused locals. | semantic-safe but size-losing representation drift | This was the formerly invalid-output family; the fix preserves validity by keeping bottom evidence in a typed-dead wrapper. Semantics are still immediate unreachable, but Binaryen's dead-wrapper cleanup is smaller. Future cleanup can target this only if it proves typed-dead wrapper flattening without reopening invalid multi-result/bottom-tail cases. |

Still required before closing the overall `[O4Z-AUDIT-CF]` parity track:

- rerun direct compare after the next behavior-widening batch or before closeout; the latest 100000-case lane has no Starshine validation/command failures but still has five documented semantic-safe representation/cleanup mismatches;
