...
# HSO generated mutable-descriptor `return_call` old-field wrapper floor

## Question

Can the dedicated HSO GenValid profile start covering the mutable-descriptor tail-call old-field family with a direct `return_call` result-wrapper sibling?

## Answer

Yes. The profile now emits a mutable-descriptor `struct.new_desc` root whose overwritten field is a true no-param `(result i32)` helper call. The descriptor operand is a mutable exact-descriptor `global.get`, the constructor is assigned to the exact described local, then a result-typed `try_table` exits through direct `return_call` before a later same-local same-field `struct.set` marker `i32.const I32(153)`.

This is the generated mutable-descriptor sibling of the pure-descriptor `return_call` profile floor from `1065` and of the focused source-backed mutable descriptor tail-call family from `1051`. It does not yet cover the mutable-descriptor `return_call_indirect` or `return_call_ref` generated siblings.

## Red/green profile floor

The focused profile floor first added an assertion for marker `153` in `src/fuzz/main_wbtest.mbt`. The red run failed because the profile had mutable-descriptor direct-call marker `144` and pure-descriptor tail-call markers `150`-`152`, but no mutable-descriptor direct tail-call sibling. After updating `src/validate/gen_valid.mbt`, the focused run passed.
...
Red result: failed at the new `i32.const I32(153)` floor.
...
bun scripts/pass-fuzz-compare.ts --count 20 --seed 0x5eed \
  ...
  --out-dir .tmp/pass-fuzz-heap-store-optimization-mutable-descriptor-return-call-oldfield-profile-20 \
  --jobs auto \
...
bun scripts/pass-fuzz-compare.ts --count 1000 --seed 0x5eed \
  --pass heap-store-optimization \
  --out-dir .tmp/pass-fuzz-heap-store-optimization-mutable-descriptor-return-call-oldfield-direct-1000 \
  --jobs auto \
  --starshine-bin target/native/release/build/cmd/cmd.exe \
  --max-failures 2000 \
  --keep-going-after-command-failures
...
Classification: generated-profile coverage expansion for a source-backed Binaryen behavior boundary. This adds the mutable-descriptor direct `return_call` result-wrapper old-field sibling to the dedicated profile; it is not an output-shape deferral and not a broad claim that HSO is closed.

Mutable-descriptor generated tail-call coverage now includes the direct `return_call` spelling. The mutable-descriptor `return_call_indirect` and `return_call_ref` generated siblings, exact descriptor `ref.cast`, broader HSO-D/E/F/G/I/J closeout work, performance refresh, O4z slot/neighborhood replay, and the final required compare matrix remain open.
