# OptimizeInstructions OI-J five-argument call descriptor miss

## Summary

Binaryen `version_130` folds the exact descriptor-cast miss where both `ref.cast_desc_eq` operands are direct calls with exactly five pure local arguments:
...
Local probe: `.tmp/oi-j-next-probes/desc-both-call-five-args-base-on-subtype.wat`.

- `wasm-tools validate --features all` accepted the input.
- `wasm-opt --version` reported `wasm-opt version 130 (version_130)`.
- `wasm-opt --all-features --optimize-instructions -S` rewrote `$cast` to ordered `local.get; local.get; local.get; local.get; local.get; call; drop; local.get; local.get; local.get; local.get; local.get; call; drop; unreachable` and removed `ref.cast_desc_eq`.
...
- `src/passes/pass_manager.mbt`
  ...
  - Adds a raw exact-descriptor miss arm for `LocalGet x5; Call; LocalGet x5; Call; RefCastDescEq` when the first call returns a non-null strict subtype of the cast target, the second call returns a non-null exact descriptor whose type metadata describes the target, and each call has exactly five parameters.
...
- `src/passes/optimize_instructions_test.mbt`
  - Adds red-first public-pipeline coverage for `optimize-instructions folds descriptor casts with five-arg call exact descriptor misses`, including an AST-level check for local/local/local/local/local/call/drop order.
...
- calls with more than five arguments or mixed arities;
...
```sh
moon test src/passes --target native --filter 'optimize-instructions folds descriptor casts with five-arg call exact descriptor misses'
```
...
- `wasm-tools validate --features all .tmp/oi-j-next-probes/desc-both-call-five-args-base-on-subtype.wat` passed.
- `wasm-opt --all-features --optimize-instructions -S .tmp/oi-j-next-probes/desc-both-call-five-args-base-on-subtype.wat` produced the Binaryen-shaped ordered local/local/local/local/local/call/drop/unreachable output with no residual `ref.cast_desc_eq`.
- Focused `moon test src/passes --target native --filter 'optimize-instructions folds descriptor casts with five-arg call exact descriptor misses'` passed `1/1` after implementation with pre-existing warnings.
- `moon fmt` passed.
- `moon info` passed with pre-existing warnings.
- `moon test` passed `7655/7655`.
- `moon build --target native --release src/cmd` passed with pre-existing warnings.
- Starshine probe replay with `_build/native/release/build/cmd/cmd.exe --optimize-instructions` produced validating wasm that prints `local.get $a; local.get $b; local.get $c; local.get $d; local.get $e; call $make_ref; drop; local.get $f; local.get $g; local.get $h; local.get $i; local.get $j; call $make_desc; drop; unreachable` and no residual `ref.cast_desc_eq`. The command also printed a pre-existing `wat2wasm` fallback warning while still producing the requested wasm.
- `bun fuzz compare-pass --count 10000 --seed 0x5eed --pass optimize-instructions --jobs auto --starshine-bin _build/native/release/build/cmd/cmd.exe --out-dir .tmp/oi-j-five-arg-call-descriptor-miss-genvalid-10000-20260705` compared `10000/10000`, normalized `10000`, and reported zero validation/property/generator/command failures and zero mismatches with Binaryen cache `10000/0`.
