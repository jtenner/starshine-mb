...
- direct selected tuple children with exactly two, three, four, five, or six scalar results are now localized by storing selected-child results to scratch locals in stack-pop order and reloading the requested lane;
- unsupported control/branch/EH/nested-region lanes, selected-child arities 7+, multi-result non-selected siblings, multi-use tuple producers, and generalized tuple-scratch reconstruction remain open OI-M work.
