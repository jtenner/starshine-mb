# OptimizeInstructions OI-J seven-argument call descriptor miss

## Summary

Binaryen `version_130` folds the exact descriptor-cast miss where both operands are direct calls with seven pure local arguments:
...
Starshine now covers this narrow equal seven-argument pure-local direct-call default-mode slice in the public raw dispatcher. This opens the currently source-probed arity-above-six surface only for `(7,7)` and does not prove arbitrary arity or non-local/effectful call arguments.

## Probe evidence

Local probe: `.tmp/oi-j-next-probes/desc-both-call-seven-args-base-on-subtype.wat`.

- `wasm-tools validate --features all` accepted the input.
- `wasm-opt --version` reported `wasm-opt version 130 (version_130)`.
- `wasm-opt --all-features --optimize-instructions -S` rewrote `$cast` to ordered `local.get` x7; `call $make_ref`; `drop`; `local.get` x7; `call $make_desc`; `drop`; `unreachable` and removed `ref.cast_desc_eq`.
...
- `src/passes/pass_manager.mbt`
  - Extends the explicit raw pure-local direct-call descriptor-miss arity predicate to the probed equal `(7,7)` shape only.
...
- `src/passes/optimize_instructions_test.mbt`
  - Adds red-first public-pipeline coverage for `optimize-instructions folds descriptor casts with seven-arg call exact descriptor misses`, using the shared direct-call descriptor-miss fixture and asserting the ordered `local.get` x7; `call(ref); drop`; `local.get` x7; `call(desc); drop`; `unreachable` body.
...
```sh
moon test src/passes --target native --filter 'optimize-instructions folds descriptor casts with seven-arg call exact descriptor misses'
```
...
- `wasm-tools validate --features all .tmp/oi-j-next-probes/desc-both-call-seven-args-base-on-subtype.wat` passed.
- `wasm-opt --all-features --optimize-instructions -S .tmp/oi-j-next-probes/desc-both-call-seven-args-base-on-subtype.wat` produced the Binaryen-shaped ordered seven-ref-arg call/drop plus seven-desc-arg call/drop/unreachable output with no residual `ref.cast_desc_eq`.
- Focused `moon test src/passes --target native --filter 'optimize-instructions folds descriptor casts with seven-arg call exact descriptor misses'` passed `1/1` after implementation with pre-existing warnings.
- `moon fmt` passed.
