...
# OptimizeInstructions OI-M tuple-optimization fifteen-effect public multivalue boundary

## Question

Does Binaryen `version_130` localize a public multivalue block with fifteen later non-selected effects when `--optimize-instructions` is followed by `--tuple-optimization`, and should Starshine count its public block/drop spelling as parity?

## Evidence

Probe: `.tmp/oi-m-tuple-optimization-fifteen-effects-probe.wat`
...
```sh
wasm-opt --all-features -S --optimize-instructions --tuple-optimization .tmp/oi-m-tuple-optimization-fifteen-effects-probe.wat -o -
```

Observed Binaryen output succeeds and reconstructs the block through tuple scratch plus scalar locals. It creates a tuple scratch local for the sixteen-result block, stores the selected first lane into a scalar temp, extracts/drops the fifteen later lanes through scalar temps, and returns the selected lane.

## Starshine coverage

Added boundary/status coverage in `src/passes/optimize_instructions_test.mbt`:

- `optimize-instructions intentionally keeps public multivalue block with fifteen later effects through tuple-optimization boundary`
...
- `wasm-opt --all-features -S --optimize-instructions --tuple-optimization .tmp/oi-m-tuple-optimization-fifteen-effects-probe.wat -o -` passed and localized through tuple scratch plus scalar locals.
- `moon test --target native src/passes/optimize_instructions_test.mbt --filter '*fifteen later effects through tuple-optimization*'` passed `1/1`.
