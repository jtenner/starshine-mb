...
- chose nullable fresh locals at this slice because the then-current Starshine validator/local model did not accept Binaryen's non-null body-local shape in this path; Slice 41 supersedes this workaround with initialized non-defaultable-local support.
...
This still is not OC closeout. At this slice, open transform/evidence gaps included exact non-null body locals, broader `move-cast-*` refinalization/chains, remaining mixed `ref.cast`/`ref.as_non_null` variants, broader best-cast/subtype coverage, adjacent-block reuse beyond branch-free root/source subsets, calls/effects/traps/control barriers, dedicated-profile compare/classification at closeout scale, larger direct compare refresh, wasm-smith/random-all lanes, O4z slot evidence, and pass-local timing.
...
- the existing repeated-cast source protection, same-local write barriers, strict early-motion self-tee exclusions, and nullable-fresh-local workaround remained unchanged at this slice; Slice 41 later fixes that blocker.
...
This still is not OC closeout. At this slice, open transform/evidence gaps included exact non-null body locals, broader local.tee/write variants beyond the current same-root alias and separate-local early-motion subsets, broader nested/control early motion beyond independent same-region scans, broader `move-cast-*` chains beyond the current best-selection/refinalization/materialized-reuse/source-feed/separate-local-tee/nested-region subsets, richer mixed `ref.cast`/`ref.as_non_null` chains, broader best-cast/subtype coverage, adjacent-block reuse beyond branch-free root/source subsets, calls/effects/traps/control barriers, dedicated-profile compare/classification at closeout scale, larger direct compare refresh, wasm-smith/random-all lanes, O4z slot evidence, and pass-local timing.
...
## Slice 40 separate-local tee early-motion result

The fortieth recursive slice closed a narrow source-backed separate-local `local.tee` motion gap, related to Binaryen lit `move-ref.as-for-separate-index` and `move-cast-side-effects`:

- a dropped read of source local `y`, followed by a dropped `local.tee x (local.get y)`, and then later `ref.as_non_null` roots for `y` and `x`, may move the `ref.as_non_null` to the earlier `y` read;
- the intervening tee writes a separate local, so it is nontrapping and should not clear pending facts for `y`;
- the same mechanism still clears pending facts for the tee destination, preserving same-local tee barriers and avoiding motion across calls, loads, trapping roots, or nonlinear control.

Before implementation, the new fixture failed red-first: the output still had only the two original `ref.as_non_null` roots and no fresh carrier `Local 2`. `src/passes/optimize_casts.mbt` now treats dropped `local.tee` roots whose value is a local.get or bounded nontrapping i32 pure tree as strict early-motion window items. The scanner flushes and clears only the tee-written local, preserving unrelated pending source-local facts across the tee.

Validation for this slice:

- `moon test --package jtenner/starshine/passes --file optimize_casts_test.mbt` failed red-first on `optimize-casts moves ref.as_non_null across separate-local tee` before implementation (`70/71` passed), then passed `71/71` after implementation.
- `moon fmt` passed.
- `moon test src/passes` passed `3886/3886`.
- `moon info` passed with pre-existing warnings.
- `moon build --target native --release src/cmd` passed with pre-existing warnings and produced `_build/native/release/build/cmd/cmd.exe`.
- Regular direct smoke `.tmp/pass-fuzz-optimize-casts-separate-tee-smoke-100`: compared/normalized `100/100`, zero validation/generator/property/command failures, zero mismatches, and Binaryen cache `100/0`.
- Tiny dedicated aggregate smoke `.tmp/pass-fuzz-optimize-casts-genvalid-all-after-separate-tee-smoke-20`: compared `20/20`, normalized `2`, left `18` raw mismatches, had zero validation/generator/property/command failures, and Binaryen cache `20/0`. Selected leaves were `best-cast=6`, `early-motion=5`, `barriers=3`, `later-reuse=3`, `static-folds=2`, and `neighborhood=1`. Agent classification remains expected open generated parity surface, not signoff.

This still is not OC closeout. At this slice, open transform/evidence gaps included exact non-null body locals, broader local.tee/write variants beyond the current source-tee, tee-alias, and dropped separate-local tee subsets, broader nested/control early motion beyond independent same-region scans, broader `move-cast-*` chains beyond the current best-selection/refinalization/materialized-reuse/source-feed/separate-local-tee/nested-region/repeated-equal/mixed-refinalized/broad-then-narrow/three-level best-cast subsets, richer mixed `ref.cast`/`ref.as_non_null` chains beyond adjacent direct/separate-tee/moved-root refinalization cases, broader best-cast/subtype coverage in generated aggregate residuals, adjacent-block reuse beyond branch-free root/source subsets, calls/effects/traps/control barriers, dedicated-profile compare/classification at closeout scale, larger direct compare refresh, wasm-smith/random-all lanes, O4z slot evidence, and pass-local timing.

## Slice 41 exact non-null fresh-local blocker fix

The forty-first recursive slice addressed the largest generated residual family from the dedicated `optimize-casts-all` aggregate: Binaryen declares fresh cast carrier locals with the exact non-null cast type, while Starshine had deliberately used nullable carrier locals because `validate_locals_decls` rejected non-defaultable body locals before checking definite initialization.

Red-first coverage was split across the blocker and the pass behavior:

- `validate_module accepts initialized non-defaultable body locals` required a non-null body local to validate after a dominating `local.set`;
- `validate_module rejects uninitialized non-defaultable body local reads` required the validator to keep rejecting reads before initialization, including unreachable-code reads;
- `optimize-casts materializes fresh exact non-null cast locals after initialization support` required OC's fresh local to use the exact non-null cast type instead of the previous nullable workaround.

`src/validate/typecheck.mbt` now threads an initialized-local bitset through expression typechecking, marks `local.set` and `local.tee` writes as initialized, rejects `local.get` for uninitialized locals, and conservatively merges initialization across `if`, `block`, `loop`, and `try_table` flow. `src/validate/validate.mbt` no longer rejects non-defaultable body-local declarations up front; params and defaultable body locals start initialized, while non-defaultable body locals start uninitialized. `src/passes/optimize_casts.mbt` now appends fresh cast carrier locals with the actual cast/refinement type, matching Binaryen's exact non-null shape when the cast target is non-null.

Validation for this slice:

- `moon test --package jtenner/starshine/validate --file validate.mbt` failed red-first on the initialized-local acceptance and uninitialized-read message before implementation, then passed `173/173`.
- `moon test --package jtenner/starshine/passes --file optimize_casts_test.mbt` failed red-first on the exact fresh-local assertion before implementation (`71/72` passed), then passed `72/72`.
- `moon fmt` passed.
- `moon test src/validate` passed `1657/1657`.
- `moon test src/passes` passed `3887/3887` with pre-existing warnings.
- `moon info` passed with pre-existing warnings.
- `moon build --target native --release src/cmd` passed with pre-existing warnings and produced `_build/native/release/build/cmd/cmd.exe`.
- Regular direct smoke `.tmp/pass-fuzz-optimize-casts-exact-local-smoke-100`: compared/normalized `100/100`, zero mismatches/failures, and Binaryen cache `100/0`.
- Dedicated aggregate smoke `.tmp/pass-fuzz-optimize-casts-genvalid-all-after-exact-local-smoke-100`: compared/normalized `100/100`, zero mismatches/failures, and Binaryen cache `98/2`. Selected leaves were `barriers=20`, `early-motion=18`, `best-cast=17`, `later-reuse=17`, `ref-as=12`, `static-folds=8`, and `neighborhood=8`.
