...
| Old field side-effect preservation | Source still preserves replaced old-field effects when they matter. Binaryen lit also includes `$side-effect-subsequent-ok`, where a later `struct.set(local.get)` replaces only the second field while preserving a side-effecting first field. | `hso_try_fold_into_struct_new(...)` replaces or wraps old field values; tests added in follow-up `0778` cover old-field call preservation and the moved-call fold into plain `struct.new`. Follow-up `0852` added coverage for the subsequent-chain old-field side-effect family; Starshine already matched Binaryen and focused HSO tests passed `204/204`. | Better covered after `0852`; descriptor/default combinations and broader old-field negative audit remain for HSO-D. |
