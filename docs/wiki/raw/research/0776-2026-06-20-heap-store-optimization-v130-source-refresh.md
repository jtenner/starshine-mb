...
| Target-local hazards | Source rejects moved values that read/write the target local before considering movement. | `hso_subtree_touches_local(...)`; negative tests cover direct and nested target-local reads, follow-up `0854` adds a moved-value target-local write negative, follow-up `0855` adds target-local chain variants where Binaryen either preserves both stores after an early target-local write or folds only an earlier harmless store before a later target-local-read hazard, and follow-up `0857` adds a descriptor-constructor target-local read negative. | Better covered after `0854`, `0855`, and `0857`; HSO-E should still add descriptor/later-field hazard combinations beyond this direct descriptor target-local read and any remaining moved-value hazard variants if missing. |
