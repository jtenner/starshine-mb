...
| Repeated stores / last value wins | Lit chain cases stay unchanged in `version_130`; source keeps scanning after a successful fold. | Consecutive-root, same-local, tee-plus-later-chain, repeated same-field last-value, wrong-target-local, many-fields independent tee roots, and pattern-breaker local-copy tests exist; `0789` added repeated-store / wrong-target coverage, `0848` fixed the tee-plus-later chain, and `0849` added coverage-only many-fields / pattern-breaker guards. | Better covered after `0789`, `0848`, and `0849`; HSO-C still needs broader `$many-news` review and debris/output-shape classification before closeout. |
