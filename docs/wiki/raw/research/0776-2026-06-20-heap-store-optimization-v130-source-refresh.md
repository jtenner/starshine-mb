...
| Narrow action surface: `StructSet` and `Block` only | `HeapStoreOptimization.cpp` records only `StructSet` and `Block` action roots. Dedicated lit remains unchanged from `version_129`. | `src/passes/heap_store_optimization.mbt` processes HOT regions and `StructSet`/constructor patterns, not array/memory/table store DSE. Follow-up `0790` added explicit array-store and ordinary-memory-store boundary tests. | Better covered after `0790`; table-store, generic DSE/load-forwarding wording, and future-Binaryen reopening criteria still need final HSO-H review. |
...
| Unreachable no-fold boundary | Source still rejects unreachable constructor/set pairs and leaves DCE to later passes. | Local pass has unreachable-tail repair; exact no-fold boundary still needs explicit no-fold coverage. | HSO-H/J should lock this boundary as an explicit non-goal with tests. |
