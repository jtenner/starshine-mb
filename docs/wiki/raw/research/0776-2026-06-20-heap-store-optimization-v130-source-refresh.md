...
| Target-local hazards | Source rejects moved values that read/write the target local before considering movement. | `hso_subtree_touches_local(...)`; negative tests cover direct and nested target-local reads, follow-up `0854` adds a moved-value target-local write negative, and follow-up `0855` adds target-local chain variants where Binaryen either preserves both stores after an early target-local write or folds only an earlier harmless store before a later target-local-read hazard. | Better covered after `0854` and `0855`; HSO-E should still add descriptor/later-field hazard combinations and any remaining moved-value hazard variants if missing. |
