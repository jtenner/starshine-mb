...
| Repeated stores / last value wins | Lit chain cases stay unchanged in `version_130`; source keeps scanning after a successful fold. | Consecutive-root and same-local folding tests exist. | Covered at a basic level; HSO-C should add/update narrow “last value wins” and wrong-local guards if missing. |
