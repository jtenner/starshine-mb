...
| Target-local hazards | Source rejects moved values that read/write the target local before considering movement. | `hso_subtree_touches_local(...)`; negative tests cover direct and nested target-local reads, and follow-up `0854` adds a moved-value target-local write negative. | Better covered after `0854`; HSO-E should still add target-local chain variants and descriptor/later-field hazard combinations if missing. |
