...
| Control-flow skip-local-set hazards | Source still uses `LazyLocalGraph::canMoveSet(...)`, with the one-bad-get exception when the bad get disappears in the optimized set. The dedicated lit also marks plain `return` in the moved set value safe because the taken return exits the function, and marks a call caught by an in-function `try_table` unsafe. | `hso_subtree_may_skip_local_set(...)`, `hso_subtree_may_escape_to_active_catch(...)`, and region owner/active-catch predicates. Branch-related tests exist at `:640`, `:675`, `:786`, `:828`, `:1893`; follow-up `0793` added the function-external return fold positive, and follow-up `0794` added the in-function caught-call negative. | Better covered after `0793` and `0794`; HSO-F still needs unsafe in-function branch negatives, safe external call/throw coverage, return-call classification, and the one-disappearing-bad-get exception review. |
