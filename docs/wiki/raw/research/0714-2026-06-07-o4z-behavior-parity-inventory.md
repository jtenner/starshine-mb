...
| --- | --- | --- | --- |
| `memory-packing` / `[O4Z-AUDIT-MP]` | Follow-up implementation covered the previously listed passive/segment-user families, `zeroFilledMemory`, `trapsNeverHappen`, GC boundaries, data names, high-address saturation, and max-segment limiting. Final 100000-case closeout had `99745` normalized matches, `6` agent-classified semantic-safe drift cases, `0` true semantic mismatches, and the saved slot replay remained canonical-wasm/normalized-WAT equal. | **No known direct behavior discrepancy for v0.1.0 direct pass.** Remaining drifts are representation-only unreachable debris or unobservable data-segment cleanup/remap differences. | Keep removed; reopen only for a new Binaryen-positive source option/family or true semantic mismatch. |
...
| Group | Passes |
| --- | --- |
| Reopened removed audits with documented behavior gaps | `once-reduction`, `remove-unused-names` in O4z mode, `duplicate-function-elimination`, `global-refining`, `global-struct-inference` |
