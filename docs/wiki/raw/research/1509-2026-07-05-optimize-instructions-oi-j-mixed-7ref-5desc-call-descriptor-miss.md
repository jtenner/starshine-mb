# OptimizeInstructions OI-J mixed 7-ref/5-desc call descriptor miss

## Summary

Binaryen `version_130` folds the exact descriptor-cast miss where the reference operand is a direct call with seven pure local arguments and the descriptor operand is a direct call with five pure local arguments:
...
Local probe: `.tmp/oi-j-next-probes/desc-mixed-7ref-5desc-call-base-on-subtype.wat`.

- `wasm-tools validate --features all` accepted the input.
- `wasm-opt --version` reported `wasm-opt version 130 (version_130)`.
- `wasm-opt --all-features --optimize-instructions -S` rewrote `$cast` to ordered `local.get` x7; `call $make_ref`; `drop`; `local.get` x5; `call $make_desc`; `drop`; `unreachable` and removed `ref.cast_desc_eq`.
...
- `src/passes/pass_manager.mbt`
  - Extends the explicit raw pure-local direct-call descriptor-miss arity predicate to the probed `(7,5)` shape only.
...
- `src/passes/optimize_instructions_test.mbt`
  - Adds red-first public-pipeline coverage for `optimize-instructions folds descriptor casts with mixed 7-ref 5-desc call exact descriptor misses`, using the shared direct-call descriptor-miss fixture and asserting the ordered `local.get` x7; `call(ref); drop`; `local.get` x5; `call(desc); drop`; `unreachable` body.
...
```sh
moon test src/passes --target native --filter 'optimize-instructions folds descriptor casts with mixed 7-ref 5-desc call exact descriptor misses'
```
...
- `wasm-tools validate --features all .tmp/oi-j-next-probes/desc-mixed-7ref-5desc-call-base-on-subtype.wat` passed.
- `wasm-opt --all-features --optimize-instructions -S .tmp/oi-j-next-probes/desc-mixed-7ref-5desc-call-base-on-subtype.wat` produced the Binaryen-shaped ordered 7-ref-arg call/drop plus 5-desc-arg call/drop/unreachable output with no residual `ref.cast_desc_eq`.
- Focused `moon test src/passes --target native --filter 'optimize-instructions folds descriptor casts with mixed 7-ref 5-desc call exact descriptor misses'` passed `1/1` after implementation with pre-existing warnings.
- `moon fmt` passed.
- `moon info` passed with pre-existing warnings.
- `moon test` passed `7699/7699`.
