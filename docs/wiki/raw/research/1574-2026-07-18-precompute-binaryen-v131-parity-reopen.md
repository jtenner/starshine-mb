...
Focused evidence at the latest checkpoint is green: binary roundtrips `106/106`; `precompute_wbtest.mbt` `9/9`; `precompute_test.mbt` `57/57`; `precompute_propagate_test.mbt` `15/15`; full `moon test` `9367/9367`; `moon check`; native release CLI builds; and exact normalized v131 matches for the shared `ref.func`, descriptor, shared-GC atomic/RMW, and GC multibyte fixtures. Fresh v131 comparison lanes complete `10000/10000` `precompute-all` (`7340` direct plus `2660` local-cleanup-normalized), `10000/10000` regular (`3723` direct plus `6277` cleanup-normalized), `10000/10000` stress with the same split, and `1000/1000` runtime/idempotence (`921` direct plus `79` cleanup-normalized, zero property or semantic failures). The first random-all attempt stopped at `50` cases on `28` structural differences and remains open for normalizer/classification work; it is not closeout evidence. The existing warnings are unrelated unused-field/reserved-name/pass-manager warnings.
