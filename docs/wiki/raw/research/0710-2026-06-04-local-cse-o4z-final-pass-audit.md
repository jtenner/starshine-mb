...
## Follow-up atomic store local-only boundary on 2026-06-05

A later focused LCSE hardening slice added core-built coverage for local-only scalar reuse across representative linear atomic stores: `i32.atomic.store` and `i64.atomic.store32`. Binaryen spot-checking the representative WAT at `.tmp/lcse-next-spots/atomic-store/input.wat` materialized the repeated scalar expression with `local.tee` / `local.get` across atomic store; Starshine intentionally leaves the expression unmaterialized and keeps atomic stores grouped with atomic/memory conservative boundaries rather than adding atomic or memory GVN. Agent classification: documented conservative deferral / missing-test-only coverage, not a semantic mismatch.

Validation evidence for this slice:

```sh
wasm-opt .tmp/lcse-next-spots/atomic-store/input.wat --all-features --local-cse -S -o .tmp/lcse-next-spots/atomic-store/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
moon info
moon fmt
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
...
moon test
moon build --target native --release src/cmd
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass local-cse --out-dir .tmp/pass-fuzz-local-cse-atomic-store-boundary-10000 --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe
```

Results: Binaryen materialized the representative scalar root across atomic store; the added conservative core-built coverage passed immediately (`134/134`), so this was missing-test-only coverage. `moon info` still hit the known Moon panic (`index out of bounds: the len is 36 but the index is 8329485`, exit `101`); `moon fmt` passed; focused LCSE tests passed (`134/134`). Broad `moon test src/passes` failed `1` unrelated dirty `simplify_locals_test.mbt` assertion (`1690/1691`) and full `moon test` likewise failed the same unrelated assertion (`4875/4876`); agent classification: unrelated/interleaved simplify-locals test-worktree failure, not an LCSE regression, because the slice changed only LCSE tests/docs and the focused LCSE lane remained green. Native build succeeded with warnings only and exit `0`; compare reached `6768` normalized matches, `0` mismatches, and `20` Binaryen/tool command failures. Agent classification: the compare command failures are oracle/tool failures, not Starshine semantic failures (`17` empty-recursion-group, `1` bad-section-size, `1` table-index-out-of-range, `1` invalid-tag-index).

## Follow-up wider SIMD lane-load root boundary on 2026-06-05

A later focused LCSE hardening slice added core-built coverage for repeated wider SIMD lane-load roots: `v128.load16_lane`, `v128.load32_lane`, and `v128.load64_lane`. Binaryen spot-checking the representative WAT at `.tmp/lcse-next-spots/simd-lane-load-widths/input.wat` materialized each repeated lane-load root with `local.tee` / `local.get`; Starshine intentionally leaves these vector lane-load roots unmaterialized rather than adding SIMD value numbering or SIMD-aware memory GVN. Agent classification: documented conservative deferral / missing-test-only coverage, not a semantic mismatch.

Validation evidence for this slice:

```sh
wasm-opt .tmp/lcse-next-spots/simd-lane-load-widths/input.wat --all-features --local-cse -S -o .tmp/lcse-next-spots/simd-lane-load-widths/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
moon info
moon fmt
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
...
moon test
moon build --target native --release src/cmd
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass local-cse --out-dir .tmp/pass-fuzz-local-cse-simd-wide-lane-load-boundary-10000 --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe
```
...
## Follow-up SIMD splat root boundary on 2026-06-05

A later focused LCSE hardening slice added core-built coverage for repeated SIMD splat roots across integer and floating lane widths: `i8x16.splat`, `i16x8.splat`, `i32x4.splat`, `i64x2.splat`, `f32x4.splat`, and `f64x2.splat`. Binaryen spot-checking the representative WAT at `.tmp/lcse-next-spots/simd-splat-roots/input.wat` materialized each repeated splat root with `local.tee` / `local.get`; Starshine intentionally leaves these vector roots unmaterialized rather than adding SIMD temp locals or SIMD value numbering. Agent classification: documented conservative deferral / missing-test-only coverage, not a semantic mismatch.

Validation evidence for this slice:

```sh
wasm-opt .tmp/lcse-next-spots/simd-splat-roots/input.wat --all-features --local-cse -S -o .tmp/lcse-next-spots/simd-splat-roots/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
moon info
moon fmt
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
...
moon test
moon build --target native --release src/cmd
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass local-cse --out-dir .tmp/pass-fuzz-local-cse-simd-splat-boundary-10000 --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe
```

Results: Binaryen materialized representative repeated splat roots; the added conservative core-built coverage passed immediately (`138/138`), so this was missing-test-only coverage. `moon info` still hit the known Moon panic (`index out of bounds: the len is 36 but the index is 8329485`, exit `101`); `moon fmt` passed; focused LCSE tests passed (`138/138`); `moon test src/passes` passed (`1696/1696`); full `moon test` passed (`4881/4881`); native build reported no work; compare reached `6773` normalized matches, `0` mismatches, and `20` Binaryen/tool command failures. Agent classification: the compare command failures are oracle/tool failures, not Starshine semantic failures (`17` empty-recursion-group, `1` bad-section-size, `1` table-index-out-of-range, `1` invalid-tag-index).

## Follow-up SIMD replace-lane root boundary on 2026-06-05

A later focused LCSE hardening slice added core-built coverage for repeated SIMD replace-lane roots across integer and floating vector lanes: `i8x16.replace_lane`, `i16x8.replace_lane`, `i32x4.replace_lane`, `i64x2.replace_lane`, `f32x4.replace_lane`, and `f64x2.replace_lane`. Binaryen spot-checking the representative WAT at `.tmp/lcse-next-spots/simd-replace-lane-roots/input.wat` materialized each repeated replace-lane root with `local.tee` / `local.get`; Starshine intentionally leaves these vector roots unmaterialized rather than adding SIMD temp locals or SIMD value numbering. Agent classification: documented conservative deferral / missing-test-only coverage, not a semantic mismatch.

Validation evidence for this slice:

```sh
wasm-opt .tmp/lcse-next-spots/simd-replace-lane-roots/input.wat --all-features --local-cse -S -o .tmp/lcse-next-spots/simd-replace-lane-roots/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
moon info
moon fmt
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
...
moon test
moon build --target native --release src/cmd
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass local-cse --out-dir .tmp/pass-fuzz-local-cse-simd-replace-lane-boundary-10000 --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe
```
...
## Follow-up SIMD any-true/all-true/bitmask root boundary on 2026-06-05

A later focused LCSE hardening slice added core-built coverage for repeated SIMD roots that return scalar `i32`: `v128.any_true`, lane-width `*.all_true`, and lane-width `*.bitmask`. Binaryen spot-checking the representative WAT at `.tmp/lcse-next-spots/simd-bitmask-truth-roots/input.wat` materialized each repeated root with `local.tee` / `local.get`; Starshine intentionally leaves these SIMD-derived roots unmaterialized rather than adding SIMD temp locals or value numbering. Agent classification: documented conservative deferral / missing-test-only coverage, not a semantic mismatch.

Validation evidence for this slice:

```sh
wasm-opt .tmp/lcse-next-spots/simd-bitmask-truth-roots/input.wat --all-features --local-cse -S -o .tmp/lcse-next-spots/simd-bitmask-truth-roots/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
moon info
moon fmt
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
...
moon test
moon build --target native --release src/cmd
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass local-cse --out-dir .tmp/pass-fuzz-local-cse-simd-bitmask-truth-boundary-10000 --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe
```

Results: Binaryen materialized representative repeated SIMD-derived scalar roots; the added conservative core-built coverage passed immediately (`141/141`), so this was missing-test-only coverage. `moon info` still hit the known Moon panic (`index out of bounds: the len is 36 but the index is 8329485`, exit `101`); `moon fmt` passed; focused LCSE tests passed (`141/141`); `moon test src/passes` passed (`1699/1699`); full `moon test` passed (`4884/4884`); native build reported no work; compare reached `6766` normalized matches, `0` mismatches, and `20` Binaryen/tool command failures. Agent classification: the compare command failures are oracle/tool failures, not Starshine semantic failures (`17` empty-recursion-group, `1` bad-section-size, `1` table-index-out-of-range, `1` invalid-tag-index).

## Follow-up wider SIMD arithmetic/logic root boundary on 2026-06-05

A later focused LCSE hardening slice added core-built coverage for a representative wider set of repeated pure SIMD arithmetic and logic roots beyond the earlier `v128.not` / `i8x16.eq` smoke: `v128.and`, `v128.xor`, `v128.bitselect`, `i8x16.abs`, `i16x8.neg`, `i32x4.add`, `i64x2.mul`, `f32x4.sqrt`, and `f64x2.add`. Binaryen spot-checking the representative WAT at `.tmp/lcse-next-spots/simd-vector-arith-logic-roots/input.wat` materialized each repeated root with `local.tee` / `local.get`; Starshine intentionally leaves these vector roots unmaterialized rather than adding SIMD temp locals or value numbering. Agent classification: documented conservative deferral / missing-test-only coverage, not a semantic mismatch.

Validation evidence for this slice:

```sh
wasm-opt .tmp/lcse-next-spots/simd-vector-arith-logic-roots/input.wat --all-features --local-cse -S -o .tmp/lcse-next-spots/simd-vector-arith-logic-roots/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
moon info
moon fmt
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
...
moon test
moon build --target native --release src/cmd
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass local-cse --out-dir .tmp/pass-fuzz-local-cse-simd-vector-arith-logic-boundary-10000 --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe
```

Results: Binaryen materialized representative repeated wider SIMD arithmetic/logic roots; the added conservative core-built coverage passed immediately (`142/142`), so this was missing-test-only coverage. `moon info` still hit the known Moon panic (`index out of bounds: the len is 36 but the index is 8329485`, exit `101`); `moon fmt` passed; focused LCSE tests passed (`142/142`); `moon test src/passes` passed (`1700/1700`); full `moon test` passed (`4885/4885`); native build reported no work; compare reached `6766` normalized matches, `0` mismatches, and `20` Binaryen/tool command failures. Agent classification: the compare command failures are oracle/tool failures, not Starshine semantic failures (`17` empty-recursion-group, `1` bad-section-size, `1` table-index-out-of-range, `1` invalid-tag-index).

## Follow-up SIMD float rounding root boundary on 2026-06-05

A later focused LCSE hardening slice added core-built coverage for repeated SIMD float rounding roots: `f32x4.ceil`, `f32x4.floor`, `f32x4.trunc`, `f32x4.nearest`, `f64x2.ceil`, `f64x2.floor`, `f64x2.trunc`, and `f64x2.nearest`. Binaryen spot-checking the representative WAT at `.tmp/lcse-next-spots/simd-float-rounding-roots/input.wat` materialized representative repeated vector rounding roots with `local.tee` / `local.get`; Starshine intentionally leaves these vector roots unmaterialized rather than adding SIMD temp locals or SIMD value numbering. Agent classification: documented conservative deferral / missing-test-only coverage, not a semantic mismatch.

Validation evidence for this slice:

```sh
wasm-opt .tmp/lcse-next-spots/simd-float-rounding-roots/input.wat --all-features --local-cse -S -o .tmp/lcse-next-spots/simd-float-rounding-roots/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
```

Results: Binaryen materialized representative repeated SIMD float rounding roots; the added conservative core-built coverage passed immediately (`143/143`), so this was missing-test-only coverage. `moon info` still hit the known Moon panic (`index out of bounds: the len is 36 but the index is 8329485`, exit `101`); `moon fmt` passed; focused LCSE tests passed (`143/143`); `moon test src/passes` passed (`1701/1701`); full `moon test` passed (`4886/4886`); native build reported no work; compare reached `6765` normalized matches, `0` mismatches, and `20` Binaryen/tool command failures. Agent classification: the compare command failures are oracle/tool failures, not Starshine semantic failures (`17` empty-recursion-group, `1` bad-section-size, `1` table-index-out-of-range, `1` invalid-tag-index).

## Follow-up SIMD widening/narrowing root boundary on 2026-06-05

A later focused LCSE hardening slice added core-built coverage for repeated SIMD widening and narrowing roots, including `i8x16.narrow_i16x8_{s,u}`, `i16x8.narrow_i32x4_{s,u}`, `i16x8.extadd_pairwise_i8x16_{s,u}`, `i32x4.extadd_pairwise_i16x8_{s,u}`, and lane-width `extend_low` / `extend_high` roots up to `i64x2`. Binaryen spot-checking the representative WAT at `.tmp/lcse-next-spots/simd-widen-narrow-roots/input.wat` materialized representative repeated roots with `local.tee` / `local.get`; Starshine intentionally leaves these vector roots unmaterialized rather than adding SIMD temp locals or SIMD value numbering. Agent classification: documented conservative deferral / missing-test-only coverage, not a semantic mismatch.

Validation evidence for this slice:

```sh
wasm-opt .tmp/lcse-next-spots/simd-widen-narrow-roots/input.wat --all-features --local-cse -S -o .tmp/lcse-next-spots/simd-widen-narrow-roots/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
```

Results: Binaryen materialized representative repeated SIMD widening/narrowing roots; the added conservative core-built coverage passed immediately (`144/144`), so this was missing-test-only coverage. `moon info` still hit the known Moon panic (`index out of bounds: the len is 36 but the index is 8329485`, exit `101`); `moon fmt` passed; focused LCSE tests passed (`144/144`); `moon test src/passes` passed (`1702/1702`); full `moon test` passed (`4887/4887`); native build reported no work; compare reached `6767` normalized matches, `0` mismatches, and `20` Binaryen/tool command failures. Agent classification: the compare command failures are oracle/tool failures, not Starshine semantic failures (`17` empty-recursion-group, `1` bad-section-size, `1` table-index-out-of-range, `1` invalid-tag-index).

## Follow-up SIMD conversion root boundary on 2026-06-05

A later focused LCSE hardening slice added core-built coverage for repeated SIMD conversion roots, including `f32x4.demote_f64x2_zero`, `f64x2.promote_low_f32x4`, `i32x4.trunc_sat_f32x4_{s,u}`, `f32x4.convert_i32x4_{s,u}`, `i32x4.trunc_sat_f64x2_{s,u}_zero`, and `f64x2.convert_low_i32x4_{s,u}`. Binaryen spot-checking the representative WAT at `.tmp/lcse-next-spots/simd-conversion-roots/input.wat` materialized representative repeated roots with `local.tee` / `local.get`; Starshine intentionally leaves these vector roots unmaterialized rather than adding SIMD temp locals or SIMD value numbering. Agent classification: documented conservative deferral / missing-test-only coverage, not a semantic mismatch.

Validation evidence for this slice:

```sh
wasm-opt .tmp/lcse-next-spots/simd-conversion-roots/input.wat --all-features --local-cse -S -o .tmp/lcse-next-spots/simd-conversion-roots/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
```

Results: Binaryen materialized representative repeated SIMD conversion roots; the added conservative core-built coverage passed immediately (`145/145`), so this was missing-test-only coverage. `moon info` still hit the known Moon panic (`index out of bounds: the len is 36 but the index is 8329485`, exit `101`); `moon fmt` passed; focused LCSE tests passed (`145/145`); `moon test src/passes` passed (`1703/1703`); full `moon test` passed (`4888/4888`); native build reported no work; compare reached `6770` normalized matches, `0` mismatches, and `20` Binaryen/tool command failures. Agent classification: the compare command failures are oracle/tool failures, not Starshine semantic failures (`17` empty-recursion-group, `1` bad-section-size, `1` table-index-out-of-range, `1` invalid-tag-index).
...
```sh
wasm-opt .tmp/lcse-next-spots/simd-relaxed-roots/input.wat --all-features --local-cse -S -o .tmp/lcse-next-spots/simd-relaxed-roots/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
```
...
A later focused LCSE hardening slice added core-built coverage for local-only scalar reuse across additional linear atomic cmpxchg width variants: `i64.atomic.rmw.cmpxchg`, `i32.atomic.rmw8.cmpxchg_u`, `i32.atomic.rmw16.cmpxchg_u`, `i64.atomic.rmw8.cmpxchg_u`, `i64.atomic.rmw16.cmpxchg_u`, and `i64.atomic.rmw32.cmpxchg_u`. Binaryen spot-checking the representative WAT at `.tmp/lcse-next-spots/atomic-cmpxchg-widths/input.wat` materialized the repeated local-only scalar with `local.tee` / `local.get` across representative cmpxchg variants; Starshine intentionally leaves the scalar unmaterialized and keeps the family grouped with atomic/memory conservative boundaries rather than adding atomic or memory GVN. Agent classification: documented conservative deferral / missing-test-only coverage, not a semantic mismatch.

Validation evidence for this slice:

```sh
wasm-opt .tmp/lcse-next-spots/atomic-cmpxchg-widths/input.wat --all-features --local-cse -S -o .tmp/lcse-next-spots/atomic-cmpxchg-widths/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
```

Results: Binaryen materialized the representative local-only scalar across cmpxchg width variants; the added conservative core-built coverage passed immediately (`147/147`), so this was missing-test-only coverage. `moon info` still hit the known Moon panic (`index out of bounds: the len is 36 but the index is 8329485`, exit `101`); `moon fmt` passed; focused LCSE tests passed (`147/147`); `moon test src/passes` passed (`1705/1705`); full `moon test` passed (`4890/4890`); native build reported no work; compare reached `6772` normalized matches, `0` mismatches, and `20` Binaryen/tool command failures. Agent classification: the compare command failures are oracle/tool failures, not Starshine semantic failures (`17` empty-recursion-group, `1` bad-section-size, `1` table-index-out-of-range, `1` invalid-tag-index).

## Follow-up atomic load width-variant boundary on 2026-06-05

A later focused LCSE hardening slice added core-built coverage for local-only scalar reuse across additional linear atomic load width variants: `i64.atomic.load`, `i32.atomic.load8_u`, `i32.atomic.load16_u`, `i64.atomic.load8_u`, `i64.atomic.load16_u`, and `i64.atomic.load32_u`. Binaryen spot-checking the representative WAT at `.tmp/lcse-next-spots/atomic-load-widths/input.wat` materialized the repeated local-only scalar with `local.tee` / `local.get` across representative load variants; Starshine intentionally leaves the scalar unmaterialized and keeps the family grouped with atomic/memory conservative boundaries rather than adding atomic or memory GVN. Agent classification: documented conservative deferral / missing-test-only coverage, not a semantic mismatch.

Validation evidence for this slice:

```sh
wasm-opt .tmp/lcse-next-spots/atomic-load-widths/input.wat --all-features --local-cse -S -o .tmp/lcse-next-spots/atomic-load-widths/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
```
...
## Follow-up atomic store width-variant boundary on 2026-06-05

A later focused LCSE hardening slice added core-built coverage for local-only scalar reuse across additional linear atomic store width variants: `i64.atomic.store`, `i32.atomic.store8`, `i32.atomic.store16`, `i64.atomic.store8`, and `i64.atomic.store16`. Binaryen spot-checking the representative WAT at `.tmp/lcse-next-spots/atomic-store-widths/input.wat` materialized the repeated local-only scalar with `local.tee` / `local.get` across representative store variants; Starshine intentionally leaves the scalar unmaterialized and keeps the family grouped with atomic/memory conservative boundaries rather than adding atomic or memory GVN. Agent classification: documented conservative deferral / missing-test-only coverage, not a semantic mismatch.

Validation evidence for this slice:

```sh
wasm-opt .tmp/lcse-next-spots/atomic-store-widths/input.wat --all-features --local-cse -S -o .tmp/lcse-next-spots/atomic-store-widths/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
```
...
## Follow-up SIMD float min/max root boundary on 2026-06-05

A later focused LCSE hardening slice added core-built coverage for repeated SIMD floating min/max roots: `f32x4.min`, `f32x4.max`, `f32x4.pmin`, `f32x4.pmax`, `f64x2.min`, `f64x2.max`, `f64x2.pmin`, and `f64x2.pmax`. Binaryen spot-checking the representative WAT at `.tmp/lcse-next-spots/simd-float-minmax-roots/input.wat` materialized representative repeated roots with `local.tee` / `local.get`; Starshine intentionally leaves these vector roots unmaterialized rather than adding SIMD temp-local/value-numbering or NaN/min-max-specific reasoning.

Validation evidence for this slice:

```sh
wasm-tools parse .tmp/lcse-next-spots/simd-float-minmax-roots/input.wat -o .tmp/lcse-next-spots/simd-float-minmax-roots/input.wasm
wasm-opt .tmp/lcse-next-spots/simd-float-minmax-roots/input.wasm --all-features --local-cse -S -o .tmp/lcse-next-spots/simd-float-minmax-roots/binaryen.wat
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
```
...
## `ref.as_non_null` root parity on 2026-06-07

Follow-up slice `[O4Z-AUDIT-LCSE-PARITY]005` removed the remaining `ref.as_non_null` blocker. The validator/typecheck blocker was narrower than LCSE itself: Starshine decoded short-form abstract nullable reference types such as `externref` as `AbsHeapTypeRefType(_)`, but `typecheck_ref_as_non_null` rejected that representation even though the local type model treats abstract ref types as nullable and defaultable. The fix accepts abstract nullable refs for `ref.as_non_null` and pushes an explicit non-null heap ref result.

LCSE implementation strategy: repeated `ref.as_non_null` now uses the same safe child-cache replay model as non-null `ref.cast`. The first occurrence caches the defaultable nullable operand with `local.tee`; replacement sites emit `local.get` plus the original `ref.as_non_null`. This preserves each checked operation, avoids appending non-default reference locals, and stays inside the existing local-window/effect invalidation model.

TDD and replay evidence:

- Focused validator TDD failed first (`532/533`) for `Typecheck RefAsNonNull accepts abstract nullable externref`, then passed after the typecheck fix.
- Focused LCSE TDD failed first (`165/166`) after flipping `local-cse defers repeated ref-as-non-null roots` into the positive `local-cse reuses repeated ref-as-non-null roots by caching nullable operand`, then passed after adding `RefAsNonNull` to the candidate and child-cache replay model.
- Native replay under `.tmp/lcse-parity007-ref-as-non-null/ref-as-non-null-spot.wat` now validates and prints the intended operand-cache shape for both `ref.as_non_null` and non-null `ref.cast`.

Validation evidence for this slice:

```sh
moon test --package jtenner/starshine/validate --file typecheck.mbt # first run failed before the typecheck fix
moon test --package jtenner/starshine/passes --file local_cse_test.mbt # first run failed before the LCSE fix
moon info
moon fmt
moon test --package jtenner/starshine/validate --file typecheck.mbt
moon test --package jtenner/starshine/passes --file local_cse_test.mbt
moon test src/validate
moon test src/passes
moon test
moon build --target native --release src/cmd
wasm-tools parse .tmp/lcse-parity007-ref-as-non-null/ref-as-non-null-spot.wat -o .tmp/lcse-parity007-ref-as-non-null/ref-as-non-null-spot.wasm
target/native/release/build/cmd/cmd.exe --local-cse -o .tmp/lcse-parity007-ref-as-non-null/ref-as-non-null-spot.starshine.wasm .tmp/lcse-parity007-ref-as-non-null/ref-as-non-null-spot.wasm
wasm-tools validate --features all .tmp/lcse-parity007-ref-as-non-null/ref-as-non-null-spot.starshine.wasm
bun validate readme-api-sync
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass local-cse --out-dir .tmp/pass-fuzz-local-cse-ref-as-non-null-10000 --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe --max-failures 2000 --keep-going-after-command-failures
```
