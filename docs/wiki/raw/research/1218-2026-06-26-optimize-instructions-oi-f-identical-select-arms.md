...
- Div/rem-payload follow-up red-first focused test: `moon test --target native src/passes/optimize_instructions_test.mbt --filter '*ref.i31 arms*'` failed `0/1` before implementation on the `i32.div_s` payload case, then passed `1/1` after adding same-order `i32.div_s` / `i32.div_u` / `i32.rem_s` / `i32.rem_u` local/constant payload-shell comparisons. Binaryen oracle `.tmp/oi-select-refi31-divrem-arms-probe.wat` passed and folded all four probed shapes.
