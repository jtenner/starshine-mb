...
## Superseded open-command notes

The earlier mid-slice open-command list below is superseded by the later 2026-07-05 evidence recorded in the executive finding and `inlining-optimizing` dossier:

- regular GenValid reached `100000/100000` cleanup-normalized with zero mismatches/failures in `.tmp/pass-fuzz-inlining-optimizing-ordinary-100000-allnorm-tailguard-20260705`;
- explicit wasm-smith reached `9956/10000` compared with `9953` normalized matches, three accepted cleanup-drift mismatches, and no Starshine failures in `.tmp/pass-fuzz-inlining-optimizing-wasm-smith-10000-tailguard-20260705`;
- random-all reached `10000/10000` with zero failures, and all `5260` raw residuals downstream-converged under shared `wasm-opt --all-features -Oz --strip-debug` in `.tmp/io-random-all-tailguard-downstream-oz-all-20260705`;
- the stricter 1x Binaryen timing fixture now passes for helper-chain sizes `1`, `5`, `10`, `20`, `50`, and `100`.

No active v0.1.0 INL closeout command remains. Future work should use the reopening criteria, not this superseded queue, unless a new regression appears.

## Historical recursive handoff queue
