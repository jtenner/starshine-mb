...
- The focused debug-artifact compare is still the remaining proof step, and it has now been rerun against the latest kept tree again. The newest saved evidence is `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-161962`: canonical wasm and normalized WAT still differ, Starshine remains over budget (`5334.668 ms` wall, `895.764 ms` pass) versus Binaryen (`339.989 ms` wall, `2.997 ms` pass), and full Binaryen parity is still not proven. The narrowed no-host root-carrier parity bugs are fixed, the overlap-aware exact-copy copyback family is fixed in reduced form, and the remaining non-canonical copy-root-carrier family is now fixed at the tuple-pass rewrite layer too: reduced scrambled and overlapping exact-copy repros now rewrite through a shared synthetic `TupleMake` producer in HOT instead of falling back to the older direct copyback root carrier. That moved the artifact drift forward. The earlier tuple-pass hunk around canonical `func $440` is gone. The first visible normalized-WAT drift is now mostly Binaryen-only `nop` residue, and the first later non-`nop` drift is lowerer policy: Binaryen keeps tuple scratch locals plus `tuple.extract` around that synthetic copy carrier, while Starshine's current `hot_lower` still lowers the same HOT shape through direct scalar scratch locals and a tighter local layout. Hot lowering still preserves typed terminal `if` carriers that feed `return`; that earlier control-family change remains green but is no longer the active blocker.
