...
- The focused debug-artifact compare is still the remaining proof step, and it has now been rerun against the latest kept tree again. The newest saved evidence is `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-326861`: canonical wasm and normalized WAT still differ, Starshine remains over budget (`5236.253 ms` wall, `885.032 ms` pass) versus Binaryen (`304.783 ms` wall, `3.264 ms` pass), and full Binaryen parity is still not proven. The narrowed no-host root-carrier parity bugs are fixed, the overlap-aware exact-copy copyback family is fixed in reduced form, the non-canonical copy-root-carrier family is fixed at the tuple-pass rewrite layer, and a new reduced nested branch-exit source-root family now stages through dedicated split locals plus a typed multivalue carrier instead of the older direct root block shortcut. A new focused `hot_lower` roundtrip regression also proved that explicit branch-exit temp copy chains already survive lift/lower unchanged, so that family is no longer blocked on lowerer policy. That reduced fix still did not retire the leading artifact family. After filtering Binaryen-only `nop`s, the first surviving drift is still in canonical `func $440`: Binaryen keeps an extra temp-copy local family before a later branch exit while Starshine still materializes those values directly. The next correctness step is therefore another reduced `func $440` tuple/lift repro, not more work in `hot_lower`.
