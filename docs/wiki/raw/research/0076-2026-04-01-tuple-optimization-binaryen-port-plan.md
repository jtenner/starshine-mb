...
- The focused debug-artifact compare is still the remaining proof step. The newest saved evidence is `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-364403`: canonical wasm and normalized WAT still differ, Starshine remains over budget (`5429.361 ms` wall, `934.878 ms` pass) versus Binaryen (`353.061 ms` wall, `3.456 ms` pass), and full Binaryen parity is still not proven. The narrowed no-host root-carrier parity bugs are fixed, the overlap-aware exact-copy copyback family is fixed in reduced form, the non-canonical copy-root-carrier family is fixed at the tuple-pass rewrite layer, the reduced nested branch-exit source-root family now stages through dedicated split locals plus a typed multivalue carrier instead of the older direct root block shortcut, a reduced root-local.set exact-copy family now also routes through a typed carrier whenever its payload source lanes are scrambled, the reduced source-host-copy passthrough family now keeps the old non-host lanes materialized while host-only tail-live0 exact-copy groups stop adding a redundant second carrier, terminal host-lane exact-copy groups with only drop-only tail lanes now stay scalar instead of being rewritten back through a redundant copy carrier, mixed direct/forwarded scalar bridge candidates from direct multivalue producers are now recognized and rewritten in reduced form, upstream no-host exact-copy groups now stay scalarized in reduced form when every downstream consumer is already a suppressed terminal drop-only child, and chained anchor-host copy groups now stage through intermediate host carriers instead of directly rematerializing the downstream host lane. A newer conservative guard now also suppresses all tuple rewrites in functions that contain a nested rootslot host-copy wrapper group; that retires the checked-in native debug-artifact failure where `func $1930` lost its old `local.set 68` / `local.set 67` lane materialization under `--tuple-optimization`, and the new native regression stays green in-tree. A newer scalar-result exact-copy collector now also recognizes reduced nested copy groups whose HOT carrier bodies are expressed as prefix `local.set`s plus a final scalar value root instead of the older trailing-payload layout, and the white-box/native regression suites are rebaselined to that scalarized contract. New focused `cmd`, native-cmd, and white-box regressions lock the rebaselined tail-live0 carrier shape, the nested rootslot artifact replay, the nested scalar-result exact-copy carrier family, and the narrower scalar-tail outputs in-source, while a focused `hot_lower` roundtrip regression still proves that explicit branch-exit temp copy chains already survive lift/lower unchanged and a focused `hot_lift` regression proves a reduced nested outer-block spill bridge still retains its scalar lane writes. The remaining compare is still red, but the retired missing-lane host-copy bug is no longer the head blocker; the current leading family is later in `func $3598`, where Binaryen keeps a second staged copy group after the tuple call and the current HOT-native rewrite still scalarizes that family differently.
...
- The reduced standalone repros have become more specific too. The earlier imported-call host-tee bug is fixed: the rewrite follows the actual `LocalTee` lane instead of assuming the host spill is always last, and the encoded output preserves `local.tee 0` into the later call instead of directly reading the fresh split local. The direct scalar-forward seed bridge family is now locked too at the pass-analysis layer, including the raw decoded one-hop shape that exposed the next drift boundary; mixed direct/forwarded scalar-forward bridges from direct producers now stay in the candidate graph instead of dropping out before rewrite; and chained mixed scalar-forward bridges from synthetic groups that still retain a source lane now also survive analysis and rewrite in reduced form. The newer source-host-copy passthrough fix now also rematerializes the old non-host lanes in the staged init root, a narrowed host-only tail-live0 guard suppresses the redundant second exact-copy carrier in the reduced chained repro, a newer terminal host-lane guard now also suppresses exact-copy rewrites once the only remaining non-drop traffic is a single host lane plus drop-only tail lanes, and a newer upstream no-host guard now keeps an exact-copy group scalarized when its only downstream consumer is a terminal drop-only child copy group. New native lowered-output checks prove the compare-shape, tail-live0, drop-only host-lane, and chained no-host dead-tail cases keep the old source lanes live or stay scalarized at the tail, while new cmd regressions now prove both the mixed direct-producer and chained mixed synthetic bridge families rewrite through staged carrier blocks. The large artifact rerun shows that these were still not sufficient by themselves: the surviving `func $1930` head hunk did not move, and the remaining drift still looks like a later mixed scalar-forward / local-allocation policy family rather than the repaired source-lane bug.
