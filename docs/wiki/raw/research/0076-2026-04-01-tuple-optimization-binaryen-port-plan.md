...
- The focused debug-artifact compare is still the remaining proof step, and it has now been rerun against the latest kept tree again. The newest saved evidence is `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-149131`: canonical wasm and normalized WAT still differ, Starshine remains over budget (`4939.432 ms` wall, `830.388 ms` pass) versus Binaryen (`318.315 ms` wall, `2.850 ms` pass), and full Binaryen parity is still not proven. The narrowed no-host root-carrier parity bugs are fixed, and the overlap-aware exact-copy copyback family is now fixed in reduced form too: reduced direct-spill, exact-copy, and overlapping exact-copy repros now match Binaryen after the tuple rewrite started carrying no-host source and copy groups through a synthetic lane-0 root `local.set` and after the copy matcher learned to follow one-hop alias payloads plus overlapping copyback lanes. Hot lowering still preserves typed terminal `if` carriers that feed `return`. That did not move the first surviving artifact hunk, though. The first surviving artifact-only drift is still the same earlier source/copy rematerialization family in canonical `func $440`, not the earlier no-host carrier, overlap-copy matcher, or terminal-return-if policy.
