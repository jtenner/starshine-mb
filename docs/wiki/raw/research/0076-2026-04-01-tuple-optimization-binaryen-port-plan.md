...
- The focused debug-artifact compare is still the remaining proof step, and it has now been rerun against the latest kept tree again. The newest saved evidence is `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-360457`: canonical wasm and normalized WAT still differ, Starshine remains over budget (`5139.118 ms` wall, `851.729 ms` pass) versus Binaryen (`335.280 ms` wall, `3.092 ms` pass), and full Binaryen parity is still not proven. The narrowed no-host root-carrier parity bugs are fixed, the overlap-aware exact-copy copyback family is fixed in reduced form, the non-canonical copy-root-carrier family is fixed at the tuple-pass rewrite layer, the reduced nested branch-exit source-root family now stages through dedicated split locals plus a typed multivalue carrier instead of the older direct root block shortcut, and a new reduced root-local.set exact-copy family now also routes through a typed carrier whenever its payload source lanes are scrambled. New focused `cmd` and white-box regressions lock that root-local.set case in-source, and the minimized oracle replay now matches Binaryen under the same `wasm-opt -S` normalizer aside from symbol naming. A focused `hot_lower` roundtrip regression also proved that explicit branch-exit temp copy chains already survive lift/lower unchanged, so that family is no longer blocked on lowerer policy. This reduced fix did retire the old leading artifact family: the earlier `func $440` hunk is gone. The new first surviving normalized-WAT drift is later in canonical `func $1930` (`binary.decode_section_payload`), where Starshine still keeps an extra chained tuple rematerialization carrier plus dead lane drops after a 3-lane host-copy chain while Binaryen materializes one fewer host-copy stage. The next correctness step is therefore reducing that chained host-copy redundancy family, not revisiting `hot_lower` or the retired `func $440` head hunk.
...
- The reduced standalone repros have become more specific too. The earlier imported-call host-tee bug is fixed: the rewrite follows the actual `LocalTee` lane instead of assuming the host spill is always last, and the encoded output preserves `local.tee 0` into the later call instead of directly reading the fresh split local. The direct scalar-forward seed bridge family is now locked too at the pass-analysis layer, including the raw decoded one-hop shape that exposed the next drift boundary. The large artifact rerun shows that this was still not sufficient by itself: the next surviving diff is still a later tuple-local rematerialization family, but the new raw-shape regression now shows that the tuple pass keeps the typed carrier alive and the remaining divergence happens later in lowering.
