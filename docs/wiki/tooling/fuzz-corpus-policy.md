...
| `accepted-divergence` | A replayable mismatch or shape drift accepted as a documented **Starshine win**, rather than merely a valid or neutral alternative representation. | pass signoff research, compare-pass mismatch triage, self-optimize artifact comparison, or docs/wiki decision. | Must cite semantic reasoning plus a measured benefit (for example canonical size, effective operations, validation strength, or pass-local performance). Reopen if the win disappears or new evidence finds a semantic mismatch, validation failure, runtime issue, or changed release requirement. |
...
- `classification`: human triage class for mismatches or divergences, when applicable. Use `starshine-win`, `parity-gap`, `size-losing`, `unknown-risky`, `tool-binaryen-failure`, `validation-failure`, or `true-semantic-mismatch`; the harness `status` is separate evidence, not this judgment.
