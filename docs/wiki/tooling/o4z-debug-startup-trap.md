...
- Function 8102 is covered by `simplify-locals-nostructure keeps initialization before stack-carried call read`; function 8889 is covered by the result-if unsafe-write invariant and prefix 35 now passes runtime.
- The full 69-pass main sequence is runtime-safe. The remaining self-hosted spec-output failure is introduced by the scalar roster's first `remove-unused-brs` pass and has been isolated to function 8788.
