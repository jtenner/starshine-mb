...
The historical host-visible symptom was `RuntimeError: unreachable`; the August 23, 2026 current-source self-optimization regression trapped as `RuntimeError: memory access out of bounds`. Those messages are trap-classification hints, not proof of structural invalidity: the failing self-optimized artifact passed `wasm-tools validate --features all`. The replay host is the Node-hosted WASI Preview 1 runner documented in [`wasi-runner-and-preview-boundary.md`](wasi-runner-and-preview-boundary.md). The debug-artifact regression and the current recursive self-optimization regression are repaired. Current source is checked through native convergence and multiple self-hosted generations, including the formerly failing stage-two-to-stage-three transition.
...
- Recursive self-optimization exposed a later plain-`precompute` recurrence at stage three. Stage one and stage two both passed `address.wast`; stage three validated structurally but its validator rejected command 1 with `invalid memory index in memarg`. Exact candidate capture showed the main output and similar-functions candidate remained capable, while every candidate containing final plain `precompute` failed. Hybridization across 20 Precompute-changed bodies isolated absolute function 9119. Its old `local.get 8` remained on the operand stack below two later values while `local.set 8` overwrote the local; lowering reloaded the new value instead of preserving the old carried value. The bounded guard now tracks one carried local marker through flat `local.get`, `local.set`, `local.tee`, `drop`, and `nop` traffic and rejects a rewrite when the same local is overwritten while its older value remains live.
- Final formatted bootstrap evidence is exact through self-hosted stages one through four. Stage one reproduces the native output byte-for-byte from the complete debug CLI; stage one self-optimizes to native-identical stage two; stage two self-optimizes to native-identical stage three; and stage three self-optimizes to native-identical stage four. Native iteration converges at generation 11, every generation passes `address.wast`, stage three and generation 11 pass the full checked-in suite (`284` total, `87` passed, `197` known skips, `0` failed), and generation 11 self-optimizes byte-for-byte to itself.
...
- The current final bootstrap uses debug input SHA-256 `caf6d3982a6254f570b8b478d8d202561b7823606e8f0d2de163f4a6e1587393`. Stage one is 5,236,654 bytes, SHA-256 `105f5a85f577035fe7dba281e8227dc1cf1a745ce24bad7c6ec007979dd140d8`; stage two is 5,223,999 bytes, SHA-256 `b44c95a3ccb6ed7f98d9c67e09b3e66f17585232a0c836baf851c97710191d02`; stage three is 5,210,785 bytes, SHA-256 `2cc27e8577177f4a0aabd338d04fb40abe20531f15dff7ff3d19b52bf2c2ad64`; and the generation-11 fixed point is 5,209,130 bytes, SHA-256 `e5f0d56e5b0403d518b01de6482b8da42d349d38fa0468985ccb7181980cc850`.
...
The first full native run covered all 842 valid inputs: 810 optimized and independently validated under the 30-second bound, with 2,275 semantic probes and 10,290 invocation vectors. It exposed validating wrong code in Flatten (`1793a`, fixture Fibonacci), feature-floor expansion in LocalSubtyping and GlobalRefining, and trap deletion in SimplifyGlobalsOptimizing (`issue-13034`). Those families now have direct regressions and targeted replay. The completed full rerun used native SHA-256 `6c6da9cc5334ba824e073e18013388670e9eca0ae6bd02cf70b5cc75a3bbe852`. Current native SHA-256 `797cb22884706dd376ed142eb7620813481e01e9cf5f85c464e5db2210b96e91` retains the `json-as` gate at 105/105 optimization, validation, and exact runtime; current self SHA-256 `1b624ca31e5dcc3f5b9f4ab48be6db89c94f5ae674ec3c63c87c333446c082f7` passes the exact 48-module/323-call WAGO manifest lane with 48/48 byte identity. The final full 842-input native rerun is complete on SHA-256 `797cb22884706dd376ed142eb7620813481e01e9cf5f85c464e5db2210b96e91`: 842/842 inputs validate, 810/842 optimize and independently validate under the 30-second bound, and the semantic lane records 2,275 probes, 2,006 completed probes, 205 unsupported JS signatures, 12 baseline timeouts, zero candidate-only/combined timeouts, zero probe errors, 10,290 vectors, and 10,035 exact vector matches. The 52 exact mismatching probes separate into 37 trap-detail/import-inventory-only cases, three ordinary-V8 feature blocks in `array-copy-inline/commands.7.wasm`, and 12 core-observable probes. Self-hosted optimization of fuzzcases `1793b` and `1793d` still exceeds a killable 120-second bound.

The next owner repair isolated `runtime/core/rust_fannkuch` to one-based O4z slot 19, `precompute-propagate`, and then to exported defined function 5. A loop-head read of default-initialized local 13 was replaced with zero even though a nested write reached a later outer-loop iteration. PrecomputePropagation now retains entry-default reads when the same local is both read and written under loops unless SSA provides a real loop phi. `run_fannkuch(7)` is restored from Starshine's wrong `528` to the source/Binaryen result `228`, including the exact exported-memory hash. The repaired 12-module residual replay optimizes and independently validates `12/12`; rust_fannkuch is no longer a mismatch. The lane still reports 12 mismatching probes because typed-reference `select.0.wasm` additionally surfaces `as-call_indirect-last` alongside `join-funcnull`; that extra probe remains unclassified rather than being treated as safe.
