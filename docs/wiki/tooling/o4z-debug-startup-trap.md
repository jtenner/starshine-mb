...
- With that fix, exact main plus `precompute` and `vacuum` passes runtime. The next failure was narrowed beyond the pass roster to final candidate canonicalization and remains separate from this lowering fix.
- Generated replay and bisection artifacts remain under `.tmp/self-opt-current-prefix-bisect/`, `.tmp/self-opt-address-prefix-bisect/`, `.tmp/self-opt-main-after-coalesce/`, and `.tmp/isolate-final-precompute.py`.
