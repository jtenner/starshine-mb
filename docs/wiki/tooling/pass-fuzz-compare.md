...
Optional compare normalizers are enabled with repeatable `--normalize <name>` flags. Supported normalizers include `--normalize drop-consts`, which removes only conservative dropped constant-expression debris, and `--normalize unreachable-control-debris`, which strips inspected DAE/gen-valid cleanup drift around local same-block `br_table` debris before `unreachable`, unused unreachable helper bodies, and printed function type ids. Exact WAT equality still increments `normalizedMatchCount`; equality reached only after these explicit compare normalizers increments `cleanupNormalizedMatchCount`, leaving remaining drift as `mismatch` cases. Use these for lanes like `dae-optimizing` where one side removes generated debris that the other side preserves; do not use them as a substitute for classifying new semantic or validation mismatches.
...
- `result.json` - aggregate counts, pass flags, Binaryen flags, generator mode, requested GenValid profile, feature filters, requested metamorphic transform ids, relative GenValid manifest path when present, generator counts, failure class counts, failure dirs, seed, requested count, and effective jobs.
