...
The bounded effect/trap scanner decodes instruction immediates before resuming
opcode classification, including the labels of `br` and `br_if` and the label
vector plus default label of `br_table`. Historical case records produced before
that decoding was complete can mistake zero-valued branch-label bytes for the
`unreachable` opcode. Rescan the saved input before using those historical
booleans as semantic-triage evidence.
...
point requires Node with native TypeScript stripping. Imported memory64 resource
construction and observation are tested with v26.10.0 using one 64 KiB page and
a two-page maximum; the bounded original/corrupt regression detects a replaced
`memory.size` result while retaining the complete imported-memory snapshot.
