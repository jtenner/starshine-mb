...
Use compare-pass for optimizer parity signoff or failure-replay workflows, not broad fuzz-suite coverage. Compare-pass runs now also write `summary.json` in the compact `starshine.fuzz-summary-report.v1` shape so `bun fuzz coverage-delta` can compare requested/compared cases, generator mix, GenValid transforms, input effect facts, statuses, failures, and failure artifacts across pass-fuzz runs. The detailed command, generator, normalization, failure-class, replay, `summary.json`, and `--jobs` contract now lives in [`pass-fuzz-compare.md`](pass-fuzz-compare.md); keep this page focused on the ordinary `src/fuzz` runner and wrapper split.
