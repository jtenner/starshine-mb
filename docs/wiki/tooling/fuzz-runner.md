...
| `validate-valid-metamorphic` | GenValid source modules plus semantics-preserving transforms, including custom-section stress variants with empty, binary, UTF-8-name, and larger opaque payloads that must survive binary roundtrip validation. Imported function/global/table/memory duplication transforms fail closed when adding an import would shift a defined item in the same index space; mixed imported/defined-memory coverage is locked by the exact `[FUZZ-META]001` seed. |
...
The `cmd-harness` suite additionally accepts `+`-separated modifiers after the base suite profile so optimizer-pipeline fuzzing can choose a generator profile and a stable or randomized pass profile without inventing new top-level suite names. Supported modifiers are `gen=<gen-valid-profile>` / `generator=<gen-valid-profile>`, `passes=none|cleanup|optimize-core|each-pass|common-clusters|fixed-point-cleanup|default-pipeline|random-short|random-medium|random-long|random-mixed`, `idempotence` / `opt-idempotence` / `check-idempotence` for an optimize-rerun check, `codec-idempotence` / `encode-decode-idempotence` for decode/encode stability, and `determinism` / `optimizer-determinism` for two fresh independent optimization runs from the same original bytes. Random profiles draw deterministically, with repeats allowed, from a registry-checked compatibility subset of executable hot passes. Arbitrary compatible compositions are checked for validity, determinism, and codec stability but are not assumed to be fixed points. `passes=fixed-point-cleanup` currently resolves to the directly verified `merge-blocks` idempotence configuration. Specialized SSA, local-allocation, alias, and module passes remain covered by named clusters, `each-pass`, and preset profiles. `passes=common-clusters` runs an affordable cleanup/control-flow cluster; `passes=default-pipeline` expands the current `optimize` preset into explicit pass names so failure metadata and minimization see the real sequence. For example:

```text
moon run src/fuzz -- cmd-harness smoke+gen=natural-small+passes=cleanup+codec-idempotence --seed 0x5eed
```

The lower-level [`run_cmd_fuzz_harness(...)`](../../../src/cmd/fuzz_harness.mbt) API exposes the same generator-profile, pass-profile, and idempotence controls directly and includes those labels in persisted failure metadata. Generated-case failures also persist the resolved GenValid config label, the generator attempt count for that module, and the generated module's feature facts so repro triage can see which profile/config surface produced the failing bytes. Both command-harness implementations now record original input bytes, first and second property outputs, the original ordered pass list, and an exact-class minimized pass list. The native fuzz CLI writes replay-compatible `failure-metadata.json`, `input.wasm`, `optimized-once.wasm`, and `optimized-twice.wasm` under `--out-dir/failures/` before returning nonzero.
...
bun fuzz optimizer-seeds --pass <name> [--self-semantic] [--debug-serial-passes]
```

Replay reconstructs the recorded property, pass list, semantic policy, runtime plan, and normal/serial mode. Promotion verifies the exact failure class, prefers a valid reduced artifact, writes deterministic `starshine.optimizer-case.v1` entries, and deduplicates by artifact plus predicate identity. Structural reduction calls optional Binaryen `wasm-reduce` through the stable replay predicate; missing tools are reported as unavailable without discarding the original failure. The curated seed command replays the small checked-in `tests/optimizer/seeds/` corpus directly.
...
`--gen-valid-profile` selects any named GenValid profile, including composite profiles that deterministically sample singleton leaves from the root seed and selected case index. The `engine-state-all` composite is stricter: it uses a seed-rotated exact 136-case weighted cycle over 45 `engine-state-*` leaves, so every full cycle realizes the declared weights and every singleton. The completeness threshold is derived from the aggregate's current positive weights; shorter partial cycles remain valid batches and are not required to contain every leaf. The cycle combines the original execution leaves with forced semantic, resource, instantiation, decoder, cross-instance, proposal, link-graph, initialization-graph, exception-unwind, type/call, LEB/index, table-reference, mixed-address-memory, trap-commit, metamorphic, compiler-boundary, NaN, invalid-binary, recursive-multivalue, and subtype-cast-graph cases. Engine-state profiles reject generic metamorphic transforms because those transforms may violate the start-driver, hidden-state, or resource-observation contract. The dedicated metamorphic-twins leaf instead creates its comparison binary through the FFI contract. The FFI also materializes ordered support graphs and post-encode invalid-binary mutations; the generic batch emitter records those requirements but emits its primary valid AST artifact only. `--require-feature` keeps only selected modules that advance the requested feature floor, `--exclude-feature` skips candidates that contain the named feature key, and `--max-attempts` bounds the candidate stream. `--metamorphic-transform <id>` may repeat; selected modules are transformed round-robin by the requested valid metamorphic transform ids before encoding, and unknown ids reject the batch instead of silently falling back. `--manifest <path>` is opt-in and writes schema `starshine.gen-valid.batch.v2` with the generator name, root seed, requested profile, acceptance-floor catalog, required/excluded feature filters, requested `metamorphic_transform_ids`, aggregate feature stats, and one record per emitted `.wasm` file including file name, root/case seeds, index, final FNV-1a wasm hash, actual static instruction count, config label, selected profile, attempt count, validation status, applied `transform_id`, and feature facts. Engine-state records additionally contain `starshine.gen-valid.engine-state.v3` metadata for the fixed ABI, observation origins, resource/export aliases, intended completion/runtime-trap/instantiation-failure/compile-failure class, support-module requirement, four-stage execution facts, hidden-state policy, and hard budgets. For composite profiles, `config_label` stays on the requested composite and `selected_profile` records the sampled leaf. Feature labels use the stable coverage-ledger strings accepted by [`validate_valid_feature_floor_by_label(...)`](../../../src/validate/validate.mbt), such as `v128` or `mems:2`.

Callers that need generation diagnostics can use [`gen_valid_module_result(...)`](../../../src/validate/gen_valid.mbt) instead of the compatibility `Module`-only wrapper. The result-bearing API reports the stable config label, retry attempt count, feature facts for successful modules, and the last validation message/candidate on failure. The fuzz package also exposes [`emit_gen_valid_batch_artifacts_with_manifest(...)`](../../../src/fuzz/main.mbt), which preserves the existing `.wasm` artifact names while returning an in-memory manifest with file name, seed, index, config label, selected profile, attempts, validation status, and feature facts for each generated input. The compare-pass `gen-valid` path now asks the batch emitter to write `inputs/gen-valid/manifest.json` next to the saved inputs so downstream triage can inspect generated-input facts without regenerating the batch.
