...
| WASI CLI package artifact | `node/internal/starshine.wasm-wasi.wasm` | Rebuilt by `npm run build` / `prepack` from `moon build --target wasm --release src/cmd`; ignored by Git but intentionally publishable through `node/internal/.npmignore`; validate released candidates with the self-opt artifact gates when optimized artifacts are in scope. |
