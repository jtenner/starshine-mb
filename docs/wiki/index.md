...
- [`binaryen/passes/ssa-nomerge/parity.md`](binaryen/passes/ssa-nomerge/parity.md) - Current `ssa-nomerge` signoff state after fixing the dead-param parity family: direct artifact replay and `10000 / 10000` `gen-valid` compare are green, while trace-level `writeback-validate:*` skips still keep raw-byte parity open.
