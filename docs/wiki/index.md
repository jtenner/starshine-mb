...
- [`binaryen/passes/late-pipeline-dispatch.md`](binaryen/passes/late-pipeline-dispatch.md) - Compact `-O4z` / `shrink` tail roster, ordered generated-artifact audit summary, and 2026-04-18 non-GitHub Binaryen terminology plus trunk-drift check, now also recording that the Debian manpage, docs.rs pass-enum page, and bundled README overview lag in different directions: docs.rs omits `HeapStoreOptimization` even though the Debian CLI manpage still lists `--heap-store-optimization`, and the README mirror misspells `RemoveUnusedBrs` as `RemoveUnsedBrs`. The page now anchors the non-GitHub release horizon with the Chromium refs listing, which still shows `version_129` as the newest directly reachable tag, notes that the current Chromium-mirror `main` changelog still does not advertise a newer optimization-pass addition even after its lead item shifted to API rename notes, and names the exact five remaining generated-artifact blockers after the slot-14 and slot-16 fixes.
...
- [`binaryen/passes/heap-store-optimization/index.md`](binaryen/passes/heap-store-optimization/index.md) - Active hot-pass landing page; now records the Debian manpage as the stronger current non-GitHub naming source and notes that the published `wasm_opt::Pass` enum page still omits `HeapStoreOptimization` even though that page explicitly says its exposed variants mirror the CLI pass names with Rust capitalization.
