...
- [`tooling/fuzz-runner.md`](tooling/fuzz-runner.md) - Repo rule that heavy fuzz belongs in `src/fuzz`, with reusable suite/profile/seed workflow plus the current active-vs-reserved suite inventory contract now including the live `validate-invalid-binary` lane.
...
- [`validate/fuzz-hardening.md`](validate/fuzz-hardening.md) - Current validator fuzz surface truth, including the live `validate-invalid-ast` plus `validate-invalid-binary` suites, the remaining text/spec-seed gaps, the staged hardening order, and the now-closed downstream `RUME` imported-function plus no-op `start`-section parity families exposed by widened `gen-valid` coverage.
...
- [`binaryen/passes/simplify-locals/index.md`](binaryen/passes/simplify-locals/index.md) - Hub page for the full simplify-locals dossier: shapes, upstream strategy, HOT-IR strategy, implementation map, barriers, raw lane, validation, performance frontier, and parity.
...
- [`binaryen/passes/remove-unused-brs/index.md`](binaryen/passes/remove-unused-brs/index.md) - Hub page for RUB overview, exhaustive pattern inventory, Binaryen strategy, HOT-IR strategy, shape notes, bailout notes, the current carried-wrapper plus large-dispatch artifact status, the mid-band unique tee-floor correction, the later lifted `br_table` / return hot skip, the later raw value-`if` / branch no-op skip, the later drop-heavy `Func 145` raw retirement, the lifted tagged result-prefix `Func 356` retirement, the typed `br_table` encoder raw retirement of `Func 1482`, the later medium-branchy lifted hot skip for `Func 144` / `301` / `353` / `1512` / `1547` / `1859` / `1867`, the later call-heavy mixed-if mesh hot skip for `Func 408` / `413` / `739` / `832` / `902` / `1022` / `1448` / `1815`, the later localset-heavy value-if mesh hot skip for `Func 837` / `3021` / `3120` / `3130` / `3134`, and the direct one-arm payload `br_table` parity guard from `Func 3771`.
...
- [`binaryen/passes/remove-unused-brs/parity.md`](binaryen/passes/remove-unused-brs/parity.md) - Current Binaryen phase model, in-tree coverage, the fixed carried-wrapper `br_table` slice, the direct one-arm payload `br_table` parity guard, the large-result dispatch raw skip, the false-prefix guard raw fix, the later unique tee-floor correction, the lifted large-`br_table` hot skip, the later raw value-`if` / branch skip, the later drop-heavy `Func 145` raw skip, the lifted tagged result-prefix hot skip, the later typed `br_table` encoder raw skip, the later medium-branchy lifted hot skip, the later call-heavy mixed-if mesh hot skip, the later localset-heavy value-if mesh hot skip, and the remaining final-shape plus split pass-vs-lift runtime work for `remove-unused-brs`.
