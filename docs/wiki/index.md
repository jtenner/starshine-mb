...
- [`binaryen/passes/late-pipeline-dispatch.md`](binaryen/passes/late-pipeline-dispatch.md) - Compact `-O4z` / `shrink` tail roster, ordered generated-artifact audit summary, and 2026-04-18 non-GitHub Binaryen terminology plus trunk-drift check, now also recording that the Debian manpage and docs.rs pass-enum surfaces lag in different directions. The page now anchors the non-GitHub release horizon with the Chromium refs listing, which still shows `version_129` as the newest directly reachable tag, and notes that the current Chromium-mirror `main` changelog still does not advertise a newer optimization-pass addition even after its lead item shifted to API rename notes.
...
- [`binaryen/passes/optimize-instructions/index.md`](binaryen/passes/optimize-instructions/index.md) - Active hot-pass landing page; now records the retired slot-16 `Func 652` carrier-wrapper bug and the still-live shared `Func 1818` blocker.
