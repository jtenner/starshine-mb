...
- [`validate/fuzz-hardening.md`](validate/fuzz-hardening.md) - Current validator fuzz surface truth, remaining hardening gaps, the staged order for config/generator/invalid-lane work, and the now-closed downstream `RUME` imported-function plus no-op `start`-section parity families exposed by widened `gen-valid` coverage.
