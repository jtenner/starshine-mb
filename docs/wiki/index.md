...
- [`binaryen/passes/late-pipeline-dispatch.md`](binaryen/passes/late-pipeline-dispatch.md) - Compact `-O4z` / `shrink` tail roster, ordered generated-artifact audit summary, and 2026-04-18 Binaryen terminology plus trunk-drift check, now also recording that Debian/docs.rs/README package surfaces lag in different directions: docs.rs omits `HeapStoreOptimization` even though the Debian CLI manpage still lists `--heap-store-optimization`, and the README mirror misspells `RemoveUnusedBrs` as `RemoveUnsedBrs`. The page now anchors the public release horizon with official GitHub release pages through `version_129` plus the Chromium refs listing, notes that the current `main` changelog on both Chromium and official GitHub still does not advertise a newer optimization-pass addition, and names the exact two remaining generated-artifact blockers after the later slot-19, slot-23, and slot-33 retirements.
...
- [`binaryen/passes/index.md`](binaryen/passes/index.md) - Namespace catalog for all active implemented Binaryen pass folders and their current documentation status, with an explicit boundary between Starshine's implemented subset and the upstream-only Binaryen additions visible in reachable release notes through `version_129`, now also cross-checking that official GitHub release pages plus the Chromium mirror still top out at that tag, that the current `main` changelog on both Chromium and official GitHub does not currently document a newer optimization-pass addition, and that the published docs.rs pass enum is only a partial naming source even though it explicitly says its exposed variants mirror the CLI names with Rust capitalization.
...
- [`binaryen/passes/dead-code-elimination/index.md`](binaryen/passes/dead-code-elimination/index.md) - Active hot-pass landing page; notes the current Binaryen `Dce` naming alignment plus the 2026-04-18 ordered-audit fact that DCE is expensive-but-successful rather than a current corruption blocker.
- [`binaryen/passes/global-refining/index.md`](binaryen/passes/global-refining/index.md) - Active module-pass landing page; now also records the current official-source `--global-refining` terminology sanity check.
...
- [`binaryen/passes/heap-store-optimization/index.md`](binaryen/passes/heap-store-optimization/index.md) - Active hot-pass landing page; now records the Debian manpage as the stronger package-surface naming source and notes that the published `wasm_opt::Pass` enum page still omits `HeapStoreOptimization` even though that page explicitly says its exposed variants mirror the CLI pass names with Rust capitalization.
- [`binaryen/passes/heap2local/index.md`](binaryen/passes/heap2local/index.md) - Folder now has a landing page plus the current parity note, current `Heap2Local` terminology sanity check, and the ordered-audit note that H2L is expensive-but-successful rather than currently corrupting the generated artifact.
- [`binaryen/passes/memory-packing/index.md`](binaryen/passes/memory-packing/index.md) - Active module-pass landing page; now also records the current official-source `--memory-packing` terminology sanity check.
- [`binaryen/passes/once-reduction/index.md`](binaryen/passes/once-reduction/index.md) - Active module-pass landing page; now also records the current official-source `--once-reduction` terminology sanity check.
- [`binaryen/passes/optimize-instructions/index.md`](binaryen/passes/optimize-instructions/index.md) - Active hot-pass landing page; now records the fully retired slot-16 `Func 652` / `Func 1818` HOT-lower bugs and the remaining later slot-44 `Func 1818` blocker.
...
- [`binaryen/passes/vacuum/index.md`](binaryen/passes/vacuum/index.md) - Active hot-pass landing page; now records the retired slot-23 `Func 652` replay, the retired slot-33 validator/writeback fix, and the newer upstream `unreachable`-preservation note.
