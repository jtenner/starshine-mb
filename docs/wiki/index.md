...
- [`binaryen/passes/late-pipeline-dispatch.md`](binaryen/passes/late-pipeline-dispatch.md) - Compact `-O4z` / `shrink` tail roster, ordered generated-artifact audit summary, and 2026-04-18 non-GitHub Binaryen terminology plus trunk-drift check, now also recording that the Debian manpage and docs.rs pass-enum surfaces lag in different directions. The page keeps the Chromium-hosted release-note horizon through `version_129` as the safer public baseline for upstream-only pass additions, while still preserving the direct Debian manpage confirmation for `--heap-store-optimization` / `--remove-unused-brs` and the March 2026 `Precompute` drift notes.
