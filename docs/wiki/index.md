...
- [`binaryen/passes/late-pipeline-dispatch.md`](binaryen/passes/late-pipeline-dispatch.md) - Compact `-O4z` / `shrink` tail roster, ordered generated-artifact audit summary, and 2026-04-18 Binaryen terminology plus trunk-drift check, now also recording that Debian/docs.rs/README package surfaces lag in different directions: docs.rs omits `HeapStoreOptimization` even though the Debian CLI manpage still lists `--heap-store-optimization`, and the README mirror misspells `RemoveUnusedBrs` as `RemoveUnsedBrs`. The page now anchors the reachable release horizon with the Chromium refs listing, which still shows `version_129` as the newest directly reachable tag, notes that the current `main` changelog on both Chromium and official GitHub still does not advertise a newer optimization-pass addition, and names the exact five remaining generated-artifact blockers after the slot-14 and slot-16 fixes.
