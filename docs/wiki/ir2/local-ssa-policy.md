...
- exceptional edges are skipped for now, matching the local SSA v1 normal-flow policy, so the write, influence, already-SSA, and defaultability queries are normal-flow facts rather than EH-complete proofs;
...
This graph is intentionally not a mutation engine yet. It is the staged bridge toward Binaryen-style `SSAify.cpp` decisions for future `ssa-nomerge` and full `ssa` work. The public full-`ssa` registry decision depends on that bridge explicitly: `ssa` is a known boundary-only pass name until merge-local mutation and dispatcher activation land. The first full-`ssa` consumer is now an analysis-only planner in [`../../../src/passes/ssa.mbt`](../../../src/passes/ssa.mbt): `ssa_full_build_merge_rewrite_plan(...)` consumes LocalGraph facts and records future Binaryen-style merge-local actions for explicit write inputs, parameter-entry prepends, and default-entry no-prepend inputs while skipping nondefaultable default-entry merges. It does not change HOT or raw rewrite behavior. Locked LocalGraph examples live in [`local_graph_test.mbt`](../../../src/ir/local_graph_test.mbt): simple set/get influence with explicit write facts, get-before-set entry reads, param-vs-default entry classification, legal-default classification for scalar numeric, SIMD `v128`, nullable GC refs, and nondefaultable non-null refs while rejecting params and explicit set/tee sources, single-source/merge get classification, already-SSA local classification for straight-line, branch, loop, param, body-local, tee, no-read, and dead-write fixtures, per-write no-merge eligibility for split overwrite regions, merge-feeding writes, and default/param entry merges, overwrite kills, child-expression tee facts, diamond merge sources, loop-carried sources, and Binaryen `canMoveSet` obstacle families. Full-`ssa` planner examples live in [`../../../src/passes/ssa_test.mbt`](../../../src/passes/ssa_test.mbt).
