...
This graph is intentionally not a general mutation engine yet. It is the staged bridge toward Binaryen-style `SSAify.cpp` decisions for future `ssa-nomerge` and full `ssa` work. The public full-`ssa` pass now uses that bridge in two ways: `ssa_full_build_merge_rewrite_plan(...)` consumes LocalGraph facts and records future Binaryen-style merge-local actions for explicit write inputs, parameter-entry prepends, and default-entry no-prepend inputs while skipping nondefaultable default-entry merges; `ssa_full_run(...)` uses LocalGraph merge-read detection as a fail-closed boundary so `[SSA-FULL]002B` can activate direct non-merge rewrites without pretending merge-local mutation is complete. Locked LocalGraph examples live in [`local_graph_test.mbt`](../../../src/ir/local_graph_test.mbt): simple set/get influence with explicit write facts, get-before-set entry reads, param-vs-default entry classification, legal-default classification for scalar numeric, SIMD `v128`, nullable GC refs, and nondefaultable non-null refs while rejecting params and explicit set/tee sources, single-source/merge get classification, already-SSA local classification for straight-line, branch, loop, param, body-local, tee, no-read, and dead-write fixtures, per-write no-merge eligibility for split overwrite regions, merge-feeding writes, and default/param entry merges, overwrite kills, child-expression tee facts, diamond merge sources, loop-carried sources, and Binaryen `canMoveSet` obstacle families. Full-`ssa` planner and active non-merge pass examples live in [`../../../src/passes/ssa_test.mbt`](../../../src/passes/ssa_test.mbt).
