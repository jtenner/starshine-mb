...
- The broader both-generator `precompute` lane remains blocked by GenValid branch-heavy constant-control debris, not by the restored medium-simd runtime corruption: `.tmp/pass-fuzz-precompute-simd-parser-guard-norm4-10000` stopped at 51 compared with 26 mismatches and one command failure. Do not widen `optimize`/`shrink` presets or narrow the large nested inlining cleanup guard until this remaining branch-heavy family is either fixed in pass behavior or covered by a deliberately scoped semantic normalizer with regression tests.
