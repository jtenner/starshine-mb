...
| `oi-block-exits` | optimize-instructions | Block ranges include exits in prefix operands | Repaired; focused regression green |
...
| `heap-variant-casts` | heap2local | Branch-selected scalarization retains failing casts | Repaired; focused regression green |
| `heap-variant-init` | heap2local | Variant reads retain default-null traps before initialization | Repaired; focused regression green |
...
Constraint lowering now receives the enclosing function identity. A sole
terminating loop whose declared results exactly match the function results
needs no additional outer `unreachable`: the function end consumes its static
result stack. Other result signatures retain the earlier stack-closure guard.
The [new fixture](../../../src/passes/constraint_lower_parity_followup_wbtest.mbt)
preserves generated byte encoding and checks the final instruction array.
All 38 constraint-focused tests pass after the failing-first repair.

DAE optimizing now finishes unread local storage cleanup on functions changed
by the committed boundary rewrite, before the shared nested pass roster. It
removes local reads whose results are dropped, replaces unread sets with drops,
and removes unread tees while retaining their value producers. Repeating that
shrinking cleanup exposes dead copy chains; final compaction preserves parameter
indices and remaps nested bodies, including legacy handlers. Effectful and
trapping producers retain their original evaluation positions. Both
[DAE regressions](../../../src/passes/dae_cleanup_parity_followup_test.mbt)
failed on retained locals before the repair and pass afterward. Untouched
functions and the nested roster's producer-owned eligibility stay unchanged.
All 710 DAE-focused tests pass after this repair.

The two source reviews account for residual shapes rather than requiring
larger Binaryen encodings:

- All 22 OptimizeInstructions tuple families use source-ordered lane
  materialization in `optimize_instructions.mbt` and the existing SB005
  representation contract. The 1,080 prior cases save 9,952 canonical bytes;
  retained pairs have the same effective local/effect/trap opcode counts.
  The generator's all-22 test and existing tuple/effect/neighbor tests cover
  that contract. Full-case runtime renewal is recorded with final verification.
- All three SimplifyGlobals templates are fixed by
  `gen_valid_sgo_profile_module` in `gen_valid.mbt`:
  immutable initializer reuse saves three bytes per case, while empty bodies
  replace nops after dropped reads or a provably false self-guard. The 4,945
  cases save 7,727 canonical bytes, with existing tests for each identity.
- RSE's three fixed templates remove nops or an unbranched void loop wrapper;
  the separate loop containing a backedge remains intact. Precompute's fixed
  templates remove nops or replace `drop(local.tee)` with `local.set`.
  Regenerated drop-cleanup case 44 confirms the formerly unsampled empty-body
  form; its source contains only dropped pure constants/select/block values.
- Regenerated DAE forwarded-suffix case 72 preserves the producer's global
  write and drops its result before calling the wrapper. Constant argument
  propagation removes the wrapper/leaf parameters and folds the leaf to one,
  saving 11 canonical bytes. Other DAE families remove empty-body nops or
  constant-null parameters; the new local cleanup improves the previously
  retained caller debris.

These source-backed shape classifications supersede the earlier **open**
...
This configuration path entered the execution contract as
`node-v2-runtime-config-v5`.
...
Runtime-v2 now retains an aligned scalar-result, return-versus-trap, or
normalized trap-class disagreement when an unrelated export, over-cap
resource, or cross-table identity surface is blocked. The comparison remains
`incomplete`, preserves every blocking diagnostic, and never promotes equal
partial observations to semantic matches. Reference-valued results, relaxed
SIMD without an allowed-result oracle, timeouts, unsupported outcomes, and
unknown incomplete surfaces still fail closed as blocked.

The execution contract is `node-v2-definite-partial-outcomes-v6`; semantic
cache entries and resume manifests from v5 cannot suppress fresh comparison.
Focused unit and executor tests cover the former blocked scalar mismatch,
three-way Starshine-failure propagation, equal-partial blocking, and the
relaxed-SIMD boundary.
