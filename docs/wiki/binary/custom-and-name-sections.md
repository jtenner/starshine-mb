...
The official WebAssembly 3.0 source refresh in [`../raw/wasm/2026-05-20-custom-name-section-subsection-refresh.md`](../raw/wasm/2026-05-20-custom-name-section-subsection-refresh.md) lists the current standardized subsection ids as `0` module, `1` function, `2` local, `3` label, `4` type, `10` field, and `11` tag. Starshine's `NameSec` also carries local subsection ids `5` table, `6` memory, `7` global, `8` element, and `9` data; treat those as Starshine-local compatibility/richer-metadata support until a newer primary source standardizes them.
...
| Decode name payload | [`Decode for NameSec`](../../../src/binary/decode.mbt) enforces strictly increasing subsection ids and rejects ids outside `0..11`. Ids `5..9` are Starshine-local name maps, not current official WebAssembly 3.0 name subsections. | `InvalidNameSubsectionOrder`, `InvalidNameSubsectionId` in `src/binary/decode.mbt`; source correction in `raw/wasm/2026-05-20-custom-name-section-subsection-refresh.md`. |
...
- function, type, tag, and the local Starshine table, memory, global, element, and data name maps must point at existing indices;
...
This is stricter than the core semantic rule that custom sections are ignored, and the table/memory/global/element/data families are broader than the current official name-section grammar. The stricter local rule is intentional because Starshine uses structured names for diagnostics, pretty output, fuzz coverage, and pass-maintained index maps. Do not cite these local failures as official WebAssembly semantic invalidity.
...
- Correcting primary-source refresh: [`../raw/wasm/2026-05-20-custom-name-section-subsection-refresh.md`](../raw/wasm/2026-05-20-custom-name-section-subsection-refresh.md)
