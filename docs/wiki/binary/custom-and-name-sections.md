...
- **Compiler facts are advisory and trust-gated.** `IgnoreFacts` is the arbitrary-input default, producer identity never creates trust, and `HintsOnly` cannot consume assertions. `TrustAssertions` runs the pre-pipeline `apply-compiler-facts` materializer; its first consumer preserves the reference operand and rewrites compatible exact-target `call_ref` / `return_call_ref` sites to ordinary direct calls. Unknown information stays distinct from negative information, and an absent closed set never means an empty complete set.
