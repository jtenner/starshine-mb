...
- **data-count** records the number of data segments early enough for code-section validation of every data-index user, including `memory.init`, `data.drop`, `array.new_data`, and `array.init_data`.

The current element-segment refresh in [`../wast/element-segment-authoring.md`](../wast/element-segment-authoring.md) confirms the official active/passive/declarative mode split, section-id/header matrix, typed declarative text breadth, and the historical WAST declarative-mode gap, superseded for function lists by the September 10 fix. The current data/data-count refresh in [`data-element-and-datacount-sections.md`](data-element-and-datacount-sections.md) rechecks the official data segment text/binary shapes; the focused data-count recheck [`../validate/data-count-and-code-data-indices.md`](../validate/data-count-and-code-data-indices.md) plus [`../validate/data-count-and-code-data-indices.md`](../validate/data-count-and-code-data-indices.md) own the broader any-code-data-index requirement. The focused validator page [`../validate/resource-sections-and-limits.md`](../validate/resource-sections-and-limits.md) owns the active-segment offset type checks, element/table type matching, data-count equality versus body-requirement split, and invalid-fuzzer family mapping. Starshine mirrors those concepts in [`ElemMode`](../../../src/lib/types.mbt), [`ElemKind`](../../../src/lib/types.mbt), [`DataMode`](../../../src/lib/types.mbt), `ElemSec`, `DataCntSec`, and `DataSec`.
...
Starshine enforces two separate local rules in [`src/validate/validate.mbt`](../../../src/validate/validate.mbt):
...
This split is useful for diagnostics: a mismatched count is a section-level `datacnt` problem, while missing data count for a covered instruction is reported against the function body that required it. The focused guide [`../validate/data-count-and-code-data-indices.md`](../validate/data-count-and-code-data-indices.md) owns the current local/spec gap: official binary modules require data-count when **any data index** occurs in code, while Starshine's pre-code requirement scanner currently names only `MemoryInit` and `DataDrop`. [`typecheck_array_new_data(...)`](../../../src/validate/typecheck.mbt) and [`typecheck_array_init_data(...)`](../../../src/validate/typecheck.mbt) still validate data-index existence once code typechecking runs, but direct core/binary `ArrayNewData` / `ArrayInitData` fixtures without `DataCntSec` are validator-gap evidence until the requirement scanner widens. The instruction/immediate side of `memory.init`, `data.drop`, `table.init`, and `elem.drop` lives in [`instruction-and-expression-encoding.md`](instruction-and-expression-encoding.md); the runtime memory stack/data-count side lives in [`../wast/memory-instruction-authoring.md`](../wast/memory-instruction-authoring.md), while WAST text defaults and `table.init` table/element ordering live in [`../wast/table-instruction-authoring.md`](../wast/table-instruction-authoring.md). Core GC array instructions (`array.new_data`, `array.init_data`, `array.new_elem`, `array.init_elem`) also carry data or element indices; their WAST/core split and constant-expression caveats live in [`../wast/gc-aggregate-instruction-authoring.md`](../wast/gc-aggregate-instruction-authoring.md).
...
- **Declarative mode in WAST lowering.** The September v132 upgrade adds `ElemSegment.declarative`, preserving `(elem declare func ...)` as `ElemMode::declarative()` instead of passive. Core/binary support for typed declarative segments is unchanged; general typed declarative WAST remains separate authoring work. See [the element guide](../wast/element-segment-authoring.md) and [the regression](../../../src/wast/binaryen132_call_ref_test.mbt).
- **Data-count presence and count equality are different checks.** Keep them distinct when adding diagnostics or invalid repros. Presence is officially tied to all code-section data-index users; Starshine currently prechecks only the bulk-memory pair before code typechecking.
...
- Validation and fuzzing: [`../../../src/validate/validate.mbt`](../../../src/validate/validate.mbt), [`../../../src/validate/gen_invalid_wbtest.mbt`](../../../src/validate/gen_invalid_wbtest.mbt), [`../validate/module-validation-phases.md`](../validate/module-validation-phases.md), [`../validate/ref-func-declarations.md`](../validate/ref-func-declarations.md), [`../validate/fuzz-hardening.md`](../validate/fuzz-hardening.md), [`../fuzzing/generator-coverage-ledger.md`](../fuzzing/generator-coverage-ledger.md)
