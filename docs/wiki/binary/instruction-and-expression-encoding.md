...
| `0xFE` | Atomics | ordered atomic load/store/RMW/cmpxchg, wait/notify, ordered fence, plus shared-GC struct/array atomic get/RMW/cmpxchg subcodes. Starshine preserves `SeqCst` / `AcqRel` in the current codec slice; `pause`, complete proposal gating, and high-level linear-atomic WAST remain gaps. See [`../wast/atomic-memory-instruction-authoring.md`](../wast/atomic-memory-instruction-authoring.md), [`../wasm-relaxed-atomics-boundary.md`](../wasm-relaxed-atomics-boundary.md), and [`../wasm-shared-everything-threads-boundary.md`](../wasm-shared-everything-threads-boundary.md). |
...
- **Prefixed spaces are part of instruction coverage.** A one-byte opcode audit misses scalar `0xFC` saturating truncations, bulk memory, table bulk operations, SIMD, atomics, and GC/custom-descriptor operations. It can also overclaim active-proposal coverage: Memory Control's prototype `memory.discard` and Wide Arithmetic's proposed `i64.add128` / `i64.mul_wide_*` are not current Starshine `0xFC` cases. Starshine now carries the current `SeqCst` / `AcqRel` / `Relaxed` ordered `0xFE` slice, but `pause` and full Relaxed Atomics coverage remain absent. For scalar numeric text fixtures, pair this guide with [`../wast/numeric-instruction-authoring.md`](../wast/numeric-instruction-authoring.md); for GC aggregates use [`../wast/gc-aggregate-instruction-authoring.md`](../wast/gc-aggregate-instruction-authoring.md); for ordinary and relaxed atomics use [`../wast/atomic-memory-instruction-authoring.md`](../wast/atomic-memory-instruction-authoring.md) and [`../wasm-relaxed-atomics-boundary.md`](../wasm-relaxed-atomics-boundary.md).
