...
| Indices | unsigned LEB | Section and instruction codecs | Includes `TypeIdx`, `FuncIdx`, `TableIdx`, `MemIdx`, `GlobalIdx`, `TagIdx`, `ElemIdx`, `DataIdx`, `LocalIdx`, and label depths. |
| `i32.const` / `i64.const` | signed LEB | [`Decode for I32/I64`](../../../src/binary/decode.mbt), [`Encode for I32/I64`](../../../src/binary/encode.mbt) | Numeric rewrite passes must preserve value semantics and account for signed-LEB size when size is the goal. |
