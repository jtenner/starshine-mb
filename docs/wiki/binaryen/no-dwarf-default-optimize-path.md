...
| Decision boundary | Removing, narrowing, or retaining the Starshine guard is still `[SSANM-010c]`, not decided by this source refresh or by `[SSANM-010b1]`. Per explicit user direction on 2026-06-14, defer this top-level scheduling decision until SSANM is otherwise fully implemented and keep the current guard in place while direct-pass evidence finishes. | [`../../../agent-todo.md`](../../../agent-todo.md) `[SSANM-010b]` / `[SSANM-010b1]` / `[SSANM-010c]`. |
