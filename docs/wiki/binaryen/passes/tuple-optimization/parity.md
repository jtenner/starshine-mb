...
- The old white-box exact-shape reds were stale expectation debt and are now rebaselined to the current Binaryen-backed scalarization contract; the current available exact-slot neighborhood replay is green under canonical function comparison, while the historical large debug artifact was unavailable in this workspace for rerun.
...
## Residual And Historical Red Surface

Direct explicit-pass parity has final 2026-06-30 closeout evidence. The TO-specific profile deliberately remains raw-red because it generates hotter tuple-local carrier shapes whose scalar spelling differs from Binaryen; the current dedicated residual is classified below as a narrow measured Starshine win rather than a true semantic mismatch:
...
- `moon build --target native --release src/cmd` followed by `bun scripts/pass-fuzz-compare.ts --pass tuple-optimization --count 10000 ... --jobs auto --starshine-bin _build/native/release/build/cmd/cmd.exe` is the current signoff shape; a merely present `target/native/...` artifact is not evidence of freshness without a timestamp/hash check (see [`../../../AGENTS.md`](../../../AGENTS.md) and [`../../../tooling/pass-fuzz-compare.md`](../../../tooling/pass-fuzz-compare.md))
- the full debug-artifact compare stays canonically green enough to remain out of the active parity-blocker backlog

## Sources

- Archived note: [research note 0076](./index.md)
