...
```sh
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass strip-target-features --out-dir .tmp/pass-fuzz-strip-target-features --jobs auto --starshine-bin _build/native/release/build/cmd/cmd.exe
```
