...
- Starshine's current cleanup suffix is an approximation: trace marker plus private touched-only `precompute-propagate-prefix`, followed by a touched-function filtered Starshine cleanup lane, not exact Binaryen filtered public `precompute-propagate` + option-specific default function pipeline.
...
- Latest `[INL]002` filtered-lane smoke evidence is `.tmp/pass-fuzz-inlining-optimizing-inl002-filtered-1000`: `998/1000` compared, `998` matches, `0` mismatches, `0` validation failures, and `2` ignored Binaryen/tool command failures.
