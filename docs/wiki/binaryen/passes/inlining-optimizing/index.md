...
- Starshine's current cleanup suffix is an approximation: trace marker plus private touched-only `precompute-propagate-prefix`, followed by a broad cleanup lane with untouched-function restoration and touched unused-local compaction, not exact Binaryen filtered public `precompute-propagate` + default function pipeline.
...
- Latest `[INL]002` prefix smoke evidence is `.tmp/pass-fuzz-inlining-optimizing-inl002-prefix-200`: `199/200` compared, `199` matches, `0` mismatches, `0` validation failures, and `1` ignored Binaryen/tool command failure.
