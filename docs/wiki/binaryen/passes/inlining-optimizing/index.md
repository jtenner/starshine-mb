...
Direct shared-engine parity is **reopened** with plain `inlining` for released `@binaryen.inline` Always/Never policy. `[V131-INL]001` owns that behavior; the optimizing touched-function rerun and shared nested scheduler remain separately owned by `[O4Z-NESTED]001`.
...
- Reviewed chosen inline actions are direct `call` / `return_call` based; `ref.func` and ref/indirect calls remain relevant to survival and repair. V131 releases a shared-engine full-inline policy input: after the `try_delegate` bailout, `@binaryen.inline "\00"` rejects and `@binaryen.inline "\7f"` accepts full inlining before ordinary profitability thresholds. Starshine does not yet consume this policy, and it remains separate from `@metadata.code.inline`; see [`../inlining/compilation-hints-vs-no-inline-flags-and-clone-survival.md`](../inlining/compilation-hints-vs-no-inline-flags-and-clone-survival.md).
- The optimizing suffix is part of the public contract, not optional polish.
- Starshine's current cleanup suffix remains an approximation of Binaryen's option-specific default function pipeline, but its prefix now uses the shared public touched-only `precompute-propagate` pass. The filtered cleanup lane drops the former extra pre-default `precompute`, gates option-controlled slots to Binaryen optimize/shrink thresholds, and includes the early second `remove-unused-names`, option-gated `merge-locals`, local `reorder-locals`, late local cleanup cluster, `code-folding`, option-gated `redundant-set-elimination`, and final `vacuum` slots. It is skipped for modules above the local large-module threshold (`80` defined functions), so the self-optimization wall-time cliff remains more suspicious in surrounding module-level graph, prediction, scan, rewrite, and iteration phases than in the nested suffix alone.
