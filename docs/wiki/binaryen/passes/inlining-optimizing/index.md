...
- The folder now also has an immutable raw primary-source manifest plus a dedicated Starshine status/port-strategy page, so readers no longer need to reconstruct either provenance or the local landing zone from older notes.
...
- New `inlining-optimizing` findings should update the strategy page, the planning/rerun page, and the Starshine page together so the upstream planner contract, the nested-helper story, and the local landing zone stay aligned.
