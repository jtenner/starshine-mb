...
- Tiny, one-use private, narrow shrinking-trivial two-parameter binary-wrapper, narrow shrinking-trivial three-parameter `select`-wrapper, and narrow shrinking-trivial two-parameter scalar-store-wrapper defined callee eligibility, now including the `i64.store8` width-store sibling.
...
- `[INL]003`: exact Binaryen heuristic classes/options plus action filtering, iteration caps, and size guard; narrow two-parameter binary, three-parameter `select`, and scalar-store `Shrinks` subsets through the latest `i64.store8` width-store slice landed on 2026-05-13 and the rest remains active;
