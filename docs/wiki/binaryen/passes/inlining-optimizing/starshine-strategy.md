...
- `[INL]006`: active; one narrow nested direct-`return_call` tail-call subset and typed multi-result wrapper subsets are implemented, including synthesized zero-param result block types for otherwise-inlineable parameterized multi-result helpers. `return_call`-inside-`try`, indirect/ref tail-call forms, and label/name/annotation repair remain open.
