...
- Tiny, one-use private, narrow shrinking-trivial two-parameter binary-wrapper, narrow shrinking-trivial three-parameter `select`-wrapper, and narrow shrinking-trivial parameter-passthrough memory/table/SIMD operation-wrapper defined callee eligibility, now including the supported SIMD two-/three-parameter breadth.
