...
The `[INL]002` closeout fuzz lane `.tmp/pass-fuzz-inlining-optimizing-inl002-closeout-10000-keep` is also green over compared cases (`9975/10000`, `9975` matches, `0` mismatches, `25` ignored Binaryen/tool command failures). The debug-artifact direct native abort is now cleared: `.tmp/inl002-direct-debug-artifact-current-20260515-184415` exits `0` and validates. `[INL]002` still remains open because manual debug-artifact compare `.tmp/self-opt-inlining-optimizing-inl002-manual-20260515-185400` shows Binaryen direct `--inlining-optimizing` succeeds in about `11s`, while Starshine direct replay takes about `522s` and still differs (`starshine.wasm` about `3.4M`, `binaryen.wasm` about `2.2M`; normalized WAT text about `175M` vs `134M`).
