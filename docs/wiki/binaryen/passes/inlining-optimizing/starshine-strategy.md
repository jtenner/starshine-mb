...
- Binaryen default function pipeline on only touched functions; Starshine now runs a touched-filtered cleanup approximation that includes the current local `reorder-locals`, late local cleanup, and `code-folding` slots, but the exact option-specific default pipeline expansion is not yet proven;
