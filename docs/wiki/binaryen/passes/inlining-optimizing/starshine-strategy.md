...
- Tiny, one-use private, narrow shrinking-trivial two-parameter binary-wrapper, narrow shrinking-trivial three-parameter `select`-wrapper, and narrow shrinking-trivial two-parameter memory-store-wrapper defined callee eligibility, now including the `v128.store32_lane` SIMD lane-store sibling.
...
- `[INL]003`: exact Binaryen heuristic classes/options plus action filtering, iteration caps, and size guard; narrow two-parameter binary, three-parameter `select`, and memory-store `Shrinks` subsets through the latest `v128.store32_lane` SIMD lane-store slice landed on 2026-05-14 and the rest remains active;
