...
- the `[INL]003` heuristic subsets for repeated two-parameter binary, three-parameter `select`, and two-parameter scalar-store passthrough wrappers, including the latest `i64.store32` width-store sibling;
...
- `[INL]003` heuristic classes, action filtering, and size/iteration policy; narrow two-parameter binary, three-parameter `select`, and scalar-store `Shrinks` subsets through `i64.store32` are implemented, while remaining trivial/flexible/action-filtering breadth stays active;
