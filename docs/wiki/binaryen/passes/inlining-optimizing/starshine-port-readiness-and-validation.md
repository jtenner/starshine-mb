...
- the `[INL]003` heuristic subsets for repeated two-parameter binary, three-parameter `select`, and parameter-passthrough memory/table/SIMD operation wrappers, including the latest supported SIMD two-/three-parameter breadth;
...
- `[INL]003` heuristic classes, action filtering, and size/iteration policy; narrow two-parameter binary, three-parameter `select`, and memory/table/SIMD operation `Shrinks` subsets through the supported SIMD two-/three-parameter breadth slice are implemented, while remaining trivial/flexible/action-filtering breadth stays active;
