...
- typed multi-result helper inlining through an existing type-indexed wrapper block, including no-param callees and parameterized callees when a reusable zero-param result type already exists;
- self-recursion skip;
...
- `[INL]006` remaining nested tail-call, general multi-result block-type synthesis, and name/annotation repair after the narrow direct-tail and existing-type multi-result subsets.
