...
- the `[INL]003` heuristic subsets for repeated two-parameter binary, three-parameter `select`, and two-parameter memory-store passthrough wrappers, including the latest `v128.store16_lane` SIMD lane-store sibling;
...
- `[INL]003` heuristic classes, action filtering, and size/iteration policy; narrow two-parameter binary, three-parameter `select`, and memory-store `Shrinks` subsets through `v128.store` are implemented, while remaining trivial/flexible/action-filtering breadth stays active;
