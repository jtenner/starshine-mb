...
- the `[INL]003` heuristic subsets for repeated two-parameter binary, three-parameter `select`, and parameter-passthrough memory/table operation wrappers, including the latest `table.fill` table-fill sibling;
...
- `[INL]003` heuristic classes, action filtering, and size/iteration policy; narrow two-parameter binary, three-parameter `select`, and memory/table operation `Shrinks` subsets through `table.fill` are implemented, while remaining trivial/flexible/action-filtering breadth stays active;
