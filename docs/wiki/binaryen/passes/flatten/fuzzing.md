...
The latest branch-index, in-place-tail, suffix-truncation, scalar/multivalue/legacy-try rewrite-proof, table-target-vector, lightweight ownership, tuple-branch count, conditional-site, scalar parent-population, sparse proof-cache, postorder-dispatch, shared-root, single-target staging, inputful-loop support-cache, constant-time branch append, and shared admission-roster commits remain internal work only. Commits `6a74918d6` and `1acb9bc14` preserve the exact pre-mutation branch population while eliminating growing prior-user scans and three later whole-live-node admission scans. They add no generator profile, harness allowlist entry, public pass descriptor, dispatcher, CLI execution, or compare/API exposure. Focused flatten is `245/245`, private flatten is `167/167`, passes are `5,742/5,742`, and the full suite is `9,203/9,203`. The branch-dense and root-heavy reconstructed fixtures improve `13.72%` and `6.45%`, but the original public gate harness remains unrecovered and the durable representative checkpoint remains `3.65x` Binaryen. Typed EH plus structured label-owner deletion are absent, the flatten aggregate does not exist, and no public compare lane is authorized yet.
