...
The latest branch-index, in-place-tail, suffix-truncation, scalar/multivalue/legacy-try rewrite-proof, table-target-vector, lightweight ownership, tuple-branch count, conditional-site, scalar parent-population, sparse proof-cache, postorder-dispatch, shared-root, single-target staging, inputful-loop support-cache, constant-time branch append, shared admission-roster, EH-prerequisite, and flatness-classification scan-sharing commits remain internal work only. Commits `7706110c1` and `2c5a54ac3` make the immutable node-index scan also record exact catch-payload/exceptional-transfer requirements and the complete Flat IR violation report consumed by `flatten_run`; the standalone classifier shares the same helpers. They add no GenValid profile, harness allowlist entry, public pass descriptor, dispatcher, CLI execution, or compare/API exposure. Focused flatten is `245/245`, private flatten is `169/169`, passes are `5,744/5,744`, and the full suite is `9,205/9,205`. Code 1 improved the reconstructed representative `1,131 -> 1,060 us` (`6.28%`); code 2 timing was overlapping/order-sensitive and is not a speed claim. The original public gate harness remains unrecovered and the durable representative checkpoint remains `3.65x` Binaryen. Typed EH plus structured label-owner deletion are absent, the flatten aggregate does not exist, and no public compare lane is authorized yet.
