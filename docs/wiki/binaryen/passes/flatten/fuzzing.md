...
The latest branch-index, in-place-tail, suffix-truncation, scalar/multivalue/legacy-try rewrite-proof, table-target-vector, lightweight ownership, tuple-branch count, and cached tuple conditional-site commits remain internal work only. They add no generator profile, harness allowlist entry, public pass descriptor, dispatcher, CLI execution, or compare/API exposure. Focused region editing remains `4/4`, focused flatten is `245/245`, private flatten is `157/157`, passes are `5,732/5,732`, and the full suite is `9,193/9,193`. The newest tuple branch and tuple block-`br_if` fixtures improved `13,697 -> 5,238 us` (`61.76%`) and `12,764.5 -> 5,578.5 us` (`56.30%`), and no full node-use/use-site builder remains in flatten. These targeted wins still do not requalify the durable representative checkpoint, which remains `3.65x` Binaryen. Typed EH plus structured label-owner deletion are absent, the flatten aggregate does not exist, and no public compare lane is authorized yet.
