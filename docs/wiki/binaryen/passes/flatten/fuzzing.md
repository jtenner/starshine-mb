...
The multiple-rich-payload iteration remains internal too. Commits `6e4229bae` and `f223f14d6` remove the temporary one-rich cap for independently scalar and exclusively tuple-made legacy-try and inputful-loop vectors while preserving exact ownership, lane-zero rich-left use, vector order, and tuple deletion proof. Focused flatten is `259/259`, private flatten `186/186`, passes `5,775/5,775`, and full suite `9,236/9,236`. Four pinned-v130 probes preserve left, every payload call in vector order, condition, payload-free `br_if`, and false-path binary order. No aggregate, generator test, allowlist, descriptor, dispatcher, CLI execution, compare/API, or preset surface was added, so no public compare lane is authorized.
