...
The latest branch-index, in-place-tail, suffix-truncation, scalar/multivalue/legacy-try rewrite-proof, table-target-vector, lightweight conditional-ownership, region-tail ownership, and loop-branch commits remain internal work only. They add no generator profile, harness allowlist entry, public pass descriptor, dispatcher, CLI execution, or compare/API exposure. Focused region editing remains `4/4`, focused flatten is `245/245`, private flatten is `153/153`, passes are `5,728/5,728`, and the full suite is `9,189/9,189`. The latest exact tuple-tail and inputful-loop `br_if` fixtures improved `6,569 -> 4,144.5 us` (`36.91%`) and `8,344 -> 6,870 us` (`17.67%`), but the reconstructed representative fixture was noisy and the stable representative checkpoint remains `3.65x` Binaryen. Typed EH plus structured label-owner deletion are still absent. Therefore no public compare lane is authorized yet.
