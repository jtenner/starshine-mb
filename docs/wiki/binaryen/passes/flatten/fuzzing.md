...
The latest branch-index, in-place-tail, suffix-truncation, scalar/multivalue/legacy-try rewrite-proof, table-target-vector, lightweight ownership, tuple-branch count, conditional-site, scalar parent-population, sparse proof-cache, postorder-dispatch, and shared-root commits remain internal work only. Commits `7801166ac` and `18101a947` remove generic late router attempts from ordinary recursive visits and retain sequence-root identity only for exact pre-mutation shared roots, with a frozen node limit for rewrite-created ids. They add no generator profile, harness allowlist entry, public pass descriptor, dispatcher, CLI execution, or compare/API exposure. Focused region editing remains `4/4`, focused flatten is `245/245`, private flatten is `163/163`, passes are `5,738/5,738`, and the full suite is `9,199/9,199`. A recovered native-release pass-only path measured the 1,200-function root-heavy fixture at `57,498 -> 52,402 -> 52,534.5 us`; the reconstructed 120-function representative was flat at `1,177 -> 1,176 -> 1,171.5 us`. These targeted results do not reproduce or requalify the durable representative checkpoint, which remains `3.65x` Binaryen. Typed EH plus structured label-owner deletion are absent, the flatten aggregate does not exist, and no public compare lane is authorized yet.
