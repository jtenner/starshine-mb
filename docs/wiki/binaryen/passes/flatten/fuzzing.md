...
The latest branch-index and in-place-tail commits remain internal performance work only. They add no generator profile, harness allowlist entry, public pass descriptor, dispatcher, CLI execution, or compare/API exposure. Focused/private/full validation is green, but exact candidate timing remains roughly `3.9x` Binaryen and typed EH plus structured label-owner deletion are still absent. Therefore no public compare lane is authorized yet.
