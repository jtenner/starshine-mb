...
## Scalar catch repair remains outside public fuzzing

Commits `55e508e83` and `864d32c06` add a whole-function generic scalar catch repair transaction and admit the pinned v130 `try_catch_pop_fixup` family internally. Exact preflight requires one scalar marker/lane, one first-descendant nested use, exclusive ancestry, and no loop, nested catch, sharing, outside use, catch-all marker, or second lane. Flatten applies the entry `local.set`/old-position `local.get` repair before ordinary rewrites and rebuilds its proof state. Final validation is HOT mutation `14/14`, IR `324/324`, focused flatten `263/263`, whitebox flatten `187/187`, passes `5,780/5,780`, and full `9,247/9,247`.

This is still not a public fuzz lane. No flatten aggregate, generator test, allowlist, descriptor, dispatcher, CLI execution, compare/API, preset, or public pass surface was added. A future EH profile must cover exact payload type/order, multiple lanes, nested catches, block nesting, loop/multiple-execution rejection, sharing/outside ownership, catch-all boundaries, `rethrow`, and `delegate`. No compare lane is authorized while those broad families remain deferred and public pass execution remains removed.
