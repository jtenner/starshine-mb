...
The latest branch-index, in-place-tail, suffix-truncation, scalar/multivalue/legacy-try rewrite-proof, table-target-vector, and lightweight conditional-ownership commits remain internal work only. They add no generator profile, harness allowlist entry, public pass descriptor, dispatcher, CLI execution, or compare/API exposure. Focused region editing remains `4/4`, focused flatten is `245/245`, private flatten is `151/151`, passes are `5,726/5,726`, and the full suite is `9,187/9,187`. The newest tuple-made legacy-try `br_if` fixture improved `16,524.5 -> 6,724.5 us` (`59.31%` across two commits), but the prior stable representative checkpoint remains `3.65x` Binaryen. Typed EH plus structured label-owner deletion are still absent. Therefore no public compare lane is authorized yet.
