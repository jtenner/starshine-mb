...
The latest branch-index, in-place-tail, suffix-truncation, scalar/multivalue/legacy-try rewrite-proof, table-target-vector, lightweight ownership, tuple-branch count, tuple conditional-site, distinct conditional-site, and scalar parent-population commits remain internal work only. They add no generator profile, harness allowlist entry, public pass descriptor, dispatcher, CLI execution, or compare/API exposure. Focused region editing remains `4/4`, focused flatten is `245/245`, private flatten is `159/159`, passes are `5,734/5,734`, and the full suite is `9,195/9,195`. The newest distinct non-tuple fixture moved `30 -> 28 ms`; the scalar fixture moved `22 -> 23 ms` and is correctness-only. These results still do not requalify the durable representative checkpoint, which remains `3.65x` Binaryen. Typed EH plus structured label-owner deletion are absent, the flatten aggregate does not exist, and no public compare lane is authorized yet.
