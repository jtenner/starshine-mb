...
The latest branch-index, in-place-tail, suffix-truncation, scalar/multivalue/legacy-try rewrite-proof, table-target-vector, lightweight ownership, tuple-branch count, conditional-site, scalar parent-population, and sparse proof-cache commits remain internal work only. Commits `e165fde1c` and `476848f9d` replace fifteen node-sized proof arrays with seven exact sparse entry vectors without changing the admitted transform surface or the post-mutation fail-closed contract. They add no generator profile, harness allowlist entry, public pass descriptor, dispatcher, CLI execution, or compare/API exposure. Focused region editing remains `4/4`, focused flatten is `245/245`, private flatten is `161/161`, passes are `5,736/5,736`, and the full suite is `9,197/9,197`. Reconstructed coarse timing moved `1,900 -> 1,800 us` per 120 after the conditional-cache slice, while terminal-cache and node-heavy samples were flat or slightly slower with overlapping ranges. These results do not requalify the durable representative checkpoint, which remains `3.65x` Binaryen. Typed EH plus structured label-owner deletion are absent, the flatten aggregate does not exist, and no public compare lane is authorized yet.
