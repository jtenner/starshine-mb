...
The latest branch-index, in-place-tail, suffix-truncation, scalar/multivalue rewrite-proof, and table-target-vector commits remain internal work only. They add no generator profile, harness allowlist entry, public pass descriptor, dispatcher, CLI execution, or compare/API exposure. Focused region editing remains `4/4`, focused flatten is `245/245`, private flatten is `149/149`, passes are `5,724/5,724`, and the full suite is `9,185/9,185`. The multivalue branch-index fixture measured one same-session `8.64%` reduction, while table-vector resolution was noisy and not a measured win; the prior best candidate checkpoint remains `3.65x` Binaryen. Typed EH plus structured label-owner deletion are still absent. Therefore no public compare lane is authorized yet.
