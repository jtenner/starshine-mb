...
- Depth-zero catch-all rethrow may appear under a strict direct block ancestry owned by the active catch. Lowering uses `catch_all_ref`, captures the exception reference once, and emits `throw_ref`; nonzero depth, typed-payload composition, nested catches, and non-direct ownership reject.
