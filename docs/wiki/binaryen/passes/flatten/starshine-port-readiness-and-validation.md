...
- placeholder `unreachable` now preserves nested terminal `br`/`br_table`/`return`/`throw`/`throw_ref` effects in their owner region without duplicating effects that HOT already exposes as an earlier root; payload and throw-argument work remains before the terminal in source order and later sibling preludes remain later, while `rethrow`/`delegate` stay gated with EH repair;
