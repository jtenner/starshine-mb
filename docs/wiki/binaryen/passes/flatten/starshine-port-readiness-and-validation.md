...
- branch-free scalar and independently produced multivalue legacy `try` now route exact do/catch results through shared typed locals; multivalue ownership requires one exclusive repeated consumer or region-tail span. One scalar first-descendant `Catch` use repairs through an entry local, while every unsupported catch population selects `DeferredCatchPayloadRepair` and `rethrow`/`delegate` select `DeferredExceptionalTransferRepair` before ordinary mutation;
- the catch repair transaction must remain whole-function fail-closed for ordered multivalue/second-lane payloads, nested catches, loops/multiple execution, sharing/outside ownership, catch-all payloads, and non-first-descendant uses; every admitted inserted-catch-block shape must validate after lowering;
...
## Latest scalar catch repair and unchanged public hold point

The first Binaryen v130 nested-pop family is now implemented internally. Generic HOT preflight accepts only one childless scalar `Catch` marker at entry whose exact second use lies on the catch body's first-descendant line. Every ancestor must be exclusive; loops, nested try/catch paths, multiple payload lanes, catch-all markers, sharing, outside uses, and non-first-descendant positions reject before any local, node, root, child, or revision mutation. The mutation inserts an exact typed entry `local.set` and replaces the old nested use with `local.get`.

Flatten admits that plan only after all other whole-function gates pass, applies it before ordinary rewriting, rebuilds every immutable analysis/proof snapshot, and then continues. The source-backed whitebox fixture matches pinned v130 `try_catch_pop_fixup` with an `i32` payload beneath two nested blocks and a branch to the enclosing block. HOT verification, lowering, and final module validation are green.

The red-first sequence was: unbound generic repair APIs; then scalar catch admission remained deferred. Final validation is HOT mutation `14/14`, IR `324/324`, focused flatten `263/263`, whitebox flatten `187/187`, passes `5,780/5,780`, full `9,247/9,247`, `moon fmt`, and `moon info` with 11 existing warnings. Ordered multivalue payloads, a second lane, nested catches, loops/multiple execution, catch-all payloads, `rethrow`, and `delegate` remain open. Therefore the flatten aggregate, four-lane matrix, ordered neighborhood, performance requalification, and public admission remain blocked; the durable representative remains `970.5 us` / `3.65x` Binaryen and was not remeasured.
