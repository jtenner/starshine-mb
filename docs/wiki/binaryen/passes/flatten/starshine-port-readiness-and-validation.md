...
- zero-input and independently scalar inputful/no-backedge scalar-result `loop` now route body results through a temp and leave a `local.get` outside; admitted inputful loops capture entries once in order and clear their parameter prefix, while backedge-bearing channels remain explicit negatives;
