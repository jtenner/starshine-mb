...
- branch-free scalar legacy `try` now routes exact do/catch results through a shared temp, while repair-sensitive `Catch`/`CatchAll`, `rethrow`, and `delegate` functions are explicitly pre-gated;
