...
## Latest typed catch-entry prerequisite and unchanged hold point

The first EH representation decision is now implemented below flatten. `HotOp::Catch` is a childless scalar pseudo-value carrying a tag and type; ordered markers must lead one catch region, and the verifier rejects outside, late, mixed-tag, non-scalar, child-bearing, or multiply owned markers. HOT lowering consumes one scalar marker from the implicit handler stack, emits a matching typed handler block, and targets that block at catch depth `0`. Normal flow branches over the handler at depth `1`. This also repairs the older synthetic catch-all bridge's target depth.

This is necessary but not sufficient for Binaryen v130 flatten readiness. `FlattenRunAdmission::DeferredCatchPayloadRepair` still wins before mutation whenever a catch marker exists. No flatten code yet allocates payload locals, moves payload extraction to catch entry, replaces nested positions, excludes inner catch payloads, rejects looped/multiple execution, or proves old-position ownership. Multivalue catch payloads, `rethrow`, and `delegate` remain open. Therefore the flatten aggregate, four-lane matrix, ordered neighborhood, performance requalification, and public admission remain blocked.

The red-first sequence was: unbound payload representation APIs; then unhandled lowerer `Catch`; then an intermediate validator failure for a payload/label type mismatch; finally exact valid output. Current validation is verify `4/4`, lower `89/89`, IR `323/323`, focused flatten `263/263`, private flatten `186/186`, passes `5,779/5,779`, full `9,245/9,245`, `moon fmt`, and `moon info` with 11 existing warnings. The durable representative remains `970.5 us` / `3.65x` Binaryen and was not remeasured.
