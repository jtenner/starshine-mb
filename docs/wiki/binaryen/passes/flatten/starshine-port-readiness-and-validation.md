...
The latest internal performance follow-up keeps those readiness conclusions unchanged. Exact branch users are now indexed once per label before mutation, and one-for-one control-result tail writes use exact region-root replacement rather than complete region splices. The two private invariants pass at `146/146`, with focused flatten `245/245`, passes `5,721/5,721`, and the full suite `9,181/9,181`. A same-session reconstructed benchmark improved by `13.99%` across the two commits, but exact candidate samples remain roughly `3.9x` Binaryen. No public surface changed, and performance remains a hard gate alongside typed EH, structured label-owner deletion, broader parity, and the four required signoff lanes.
