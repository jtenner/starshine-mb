...
- zero-input and independently scalar inputful scalar-result `loop` route body results through a temp and leave a `local.get` outside; admitted inputful loops capture entries once in order and clear their parameter prefix, payloadless and independently scalar one- or multi-parameter `br`/`br_if` backedges reuse entry locals, while multivalue single-producer and table channels remain explicit negatives;
