...
- branch-free scalar and independently produced multivalue legacy `try` now route exact do/catch results through shared typed locals; multivalue ownership requires one exclusive repeated consumer or region-tail span, while repair-sensitive `Catch`/`CatchAll`, `rethrow`, and `delegate` functions are explicitly pre-gated;
