...
Starshine's sibling-prelude staging is dependency-aware. A later prelude marks every local written by its `local.set`/`local.tee` subtree. Earlier direct reads of those locals are snapshotted before the prelude. A potentially trapping `ref.as_non_null` is different: the complete assertion is materialized before every newly inserted later-operand prelude, and its temporary uses the exact non-null result type. Reads of unrelated locals remain in place. Function-scoped generation marks and one reusable traversal stack answer dependency queries without allocating or clearing a full-local Boolean table per operand batch; tee and scalar-spill preludes record their exact write at construction, with subtree traversal retained as a fallback.

Repeated no-fallthrough queries use direct holder-indexed lazy summaries rather than a sorted sparse insertion array or eager all-region priming. The root has a separate slot; every stable pre-mutation holder ID maps to one optional slot. Each summary records the first direct unconditional root, an effective terminal branch when present, the dead-suffix start, normal-fallthrough possibility, and the HOT revision at construction. Current-revision hits are exact, stale normal-fallthrough facts remain conservative, and stale positive facts revalidate their recorded terminator before reuse or recomputation. Detached, replaced, post-snapshot, and out-of-range holders reject.
