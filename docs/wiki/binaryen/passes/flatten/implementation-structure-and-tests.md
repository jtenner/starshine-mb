...
`src/ir/hot_mutate.mbt` exposes `hot_delete_detached_control_subtree(...)` and `hot_label_is_live(...)`. The mutation collects one complete descendant tree, rejects roots or child references outside the set, shared/cyclic descendants, external branch/catch users, and incomplete control-label ownership, then tombstones nodes and labels with one revision bump. `src/ir/hot_mutate_test.mbt` locks success, external-target rejection, shared-descendant rejection, verification, and failure atomicity. The public IR snapshot adds these two generic APIs; no public `flatten` API changed.

`src/passes/flatten.mbt` now uses the mutation for three exact single-root suffixes after unconditional legacy-try `br_table`: a void `Block` with one direct `Drop(Const)`; a void `If` with one constant condition, complete then/else region holders, and one direct `Drop(Const)` per arm; and a zero-input/no-backedge void `Loop` with one direct `Drop(Const)` body. Every control, region holder, descendant, owned label, node owner, and outside label user is checked from immutable pre-mutation facts before detachment. `src/passes/flatten_test.mbt` proves complete node/label deletion and verifier-clean Flat IR for all three.

The latest red-first sequence is `260/261 -> 261/261` for the if and `261/262 -> 262/262` for the loop. Private flatten is `186/186`, passes `5,778/5,778`, full `9,240/9,240`, and `moon info` is green with 11 existing warnings. Pinned Binaryen v130 retains each new control under direct flatten; matched `--vacuum --dce` reduces each probe `76 -> 63` bytes. Nonconstant/effectful conditions, partial or richer arms, inputful/value loops, loop-target users/backedges, try-like roots, mixed/multiple structured roots, sharing, and external label users remain pre-mutation fail-closed.
