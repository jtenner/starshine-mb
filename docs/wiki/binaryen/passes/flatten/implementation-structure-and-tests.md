...
## Latest structured control deletion slice

`src/ir/hot_mutate.mbt` now exposes `hot_delete_detached_control_subtree(...)` and `hot_label_is_live(...)`. The mutation collects one complete descendant tree, rejects roots or child references outside the set, shared/cyclic descendants, external branch/catch users, and incomplete control-label ownership, then tombstones nodes and labels with one revision bump. `src/ir/hot_mutate_test.mbt` locks success, external-target rejection, shared-descendant rejection, verification, and failure atomicity. The public IR snapshot adds these two APIs; no public `flatten` API changed.

`src/passes/flatten.mbt` uses the mutation only for one exact preflighted suffix after unconditional legacy-try `br_table`: a void `Block` with one exclusively owned direct `Drop(Const)` and no label users. `src/passes/flatten_test.mbt` proves the block, drop, constant, and label disappear while the try/table channel verifies and classifies Flat. `src/passes/flatten_wbtest.mbt` keeps an externally targeted block as the fail-closed boundary. Focused flatten is `260/260`, private flatten `186/186`, IR `319/319`, passes `5,776/5,776`, and full `9,238/9,238`.

Broader structured suffix roots remain open: loops, ifs, try-like controls, mixed/multiple structured roots, nested sharing, and any external label or node owner still reject before flatten mutation.
