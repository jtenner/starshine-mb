...
## Latest scalar catch-payload repair slice

Commits `55e508e83` and `864d32c06` build on the typed `HotOp::Catch` entry and scalar handler lowering. `src/ir/hot_mutate.mbt` now exposes `hot_scalar_catch_payloads_repairable(...)` and `hot_repair_scalar_catch_payloads(...)`. The first computes a whole-function plan from exact use sites and Binaryen's first-descendant rule; the second mutates only after every catch passes. It inserts one exact typed local capture after the payload prefix and replaces the sole nested payload use with a local read. Loops, nested try/catch ancestry, multiple lanes, catch-all markers, sharing, outside uses, and non-first-descendant positions reject without changing revision, nodes, roots, locals, or edges.

`src/passes/flatten.mbt` now treats that exact scalar plan as admissible, keeps every other pass gate ahead of mutation, applies the repair, rebuilds `FlattenRewriteState`, and then performs ordinary flattening. `src/passes/flatten_wbtest.mbt` mirrors pinned v130 `try_catch_pop_fixup` with an `i32` payload under two nested blocks and a following outer branch; it locks the entry `local.set`, old-position `local.get`, preserved target, HOT verification, lowering, and module validation.

Validation is HOT mutation `14/14`, IR `324/324`, focused flatten `263/263`, whitebox flatten `187/187`, passes `5,780/5,780`, and full `9,247/9,247`. The generated IR snapshot adds only the generic repairability/repair APIs. Ordered multivalue payloads, a second lane, nested catches, loops/multiple execution, catch-all payloads, `rethrow`, and `delegate` remain unsupported; no public flatten pass API or wiring exists.
