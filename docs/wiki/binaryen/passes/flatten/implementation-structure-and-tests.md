...
Binaryen implements `flatten` in `src/passes/Flatten.cpp` as a function-parallel Flat-IR normalizer; Starshine now has an internal active-partial `src/passes/flatten.mbt` owner with ordered scalar operand preludes, reachable/unreachable tee lowering across region roots and operand positions, branch-free scalar block/if routing, and plain carried scalar `br` routing for defaultable scalar block targets, while keeping `flatten` publicly removed until broader control, payload, EH, and signoff work is complete.
