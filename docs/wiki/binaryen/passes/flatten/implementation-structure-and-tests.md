...
`src/ir/hot_mutate.mbt` exposes `hot_delete_detached_control_forest(...)`, its one-root wrapper `hot_delete_detached_control_subtree(...)`, and `hot_label_is_live(...)`. Forest deletion collects every complete descendant tree, rejects empty or duplicate roots, overlap, sharing/cycles, roots or child references outside the set, external branch/delegate/try-table-catch users, and incomplete control-label ownership, then tombstones the complete node/label union with one revision bump. `src/ir/hot_mutate_test.mbt` locks mixed block/if/loop success plus rejection without partial deletion when a later tree has an external label user. The public IR snapshot adds only the generic forest API in this iteration; no public `flatten` API changed.

`src/passes/flatten.mbt` uses that mutation for three exact structured root kinds after unconditional legacy-try `br_table`: a void `Block` with one direct `Drop(Const)`; a void `If` with one constant condition, complete then/else region holders, and one direct `Drop(Const)` per arm; and a zero-input/no-backedge void `Loop` with one direct `Drop(Const)` body. It now admits any positive suffix vector consisting entirely of individually supported roots from that roster. Every control, region holder, descendant, owned label, node owner, cross-tree ownership edge, and outside label user is checked from immutable pre-mutation facts before one suffix detachment and forest deletion. Scalar/control-mixed vectors stay gated because they do not share one atomic mutation contract.

The forest API red-first test failed on the unbound symbol, then passes HOT mutation `12/12` and IR `320/320`. The flatten forest fixture failed unchanged at `262/263`, then passes `263/263`; private flatten is `186/186`, passes `5,779/5,779`, full `9,242/9,242`, and `moon info` is green with 11 existing warnings. Fresh pinned Binaryen v130 retains an ordered block + if + loop suffix under direct flatten at `76` bytes; matched `--vacuum --dce` removes it at `63` bytes. Nonconstant/effectful conditions, partial or richer arms, inputful/value loops, loop-target users/backedges, try-like roots, scalar/control-mixed vectors, sharing, and external label users remain pre-mutation fail-closed.
