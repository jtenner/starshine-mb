...
## Latest typed catch-entry representation and lowering prerequisite

Commit `71971e924` makes `HotOp::Catch` a typed value and adds `hot_build_catch_payload(...)` plus `hot_catch_payload_tag(...)`. `hot_verify_control(...)` now requires every payload marker to be a childless scalar in the ordered prefix of one legacy catch region, with one tag and one owner. `src/ir/hot_verify_test.mbt` covers valid tag/type recovery and rejects outside or late payload roots. The generated IR API snapshot changes only for those generic HOT APIs and `InvalidCatchPayload`; it adds no flatten pass API.

Commit `161064a0f` teaches `src/ir/hot_lower.mbt` that the catch prefix is already on the handler stack. One scalar lane lowers through a typed handler block and `Catch(tag, depth 0)`; normal completion uses `br depth 1` to skip the handler. The same correction changes the synthetic catch-all bridge to `CatchAll(depth 0)`, so exceptions enter the catch body instead of targeting the outer continuation. `src/ir/hot_lower_test.mbt` locks exact output and validates a module with an `i32` tag payload.

The representation direction is now explicit, but `src/passes/flatten.mbt` still gates every `Catch`/`CatchAll` function before mutation. The next flatten-owned EH slice must extract typed entry values to locals, replace nested payload uses, exclude nested catches, and implement Binaryen's block-nested-pop rules; multivalue payloads and exceptional transfer remain unsupported. Validation reaches verify `4/4`, lower `89/89`, IR `323/323`, focused flatten `263/263`, private flatten `186/186`, passes `5,779/5,779`, and full `9,245/9,245`.
