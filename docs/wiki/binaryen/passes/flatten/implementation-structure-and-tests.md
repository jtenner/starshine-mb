...
| Internal owner | `src/passes/flatten.mbt` and `src/passes/flatten_test.mbt` | The first slices classify Flat IR violations, materialize scalar function results, use `hot_loop_entry_values_clear(...)` to remove admitted loop input prefixes after ordered typed-local capture, lower reachable/unreachable tees at function roots, structured-region roots, and ordinary operand positions, sequence ordered scalar ordinary-operand preludes, route branch-free defaultable scalar block/if and independently produced multivalue block/if/zero-input no-backedge loop results, branch-targeted independently scalar multivalue if arms with plain exits, branch-free scalar legacy try do/catch results behind an EH-repair gate, branch-targeted defaultable scalar if, and zero-input or independently scalar inputful loop results across payloadless or independently scalar one- and multi-parameter `br`/`br_if` backedges, and redirect plain carried scalar `br`, independently scalar multivalue block-targeting `br`, including mixed fallthrough plus nested plain exits, plus scalar `br_if` payloads through named target temp vectors for admitted block/if targets. Same-type `br_if` preserves its false-path flow through a shared `local.get`; rich ordinary payloads retain one producer, run before the condition, and feed chained conditionals through locals; the target/flow-type mismatch uses a second correctly typed flow temp and one target copy. All admitted block, if, and loop targets share one per-label typed local vector with centralized allocation and type matching. Branch-free multivalue blocks, ifs, and zero-input no-backedge loops use that vector when their defaultable scalar tails map to one exclusive repeated HOT consumer span, preserving result order while erasing the control result. Independently scalar `br_table` payloads retain one producer each, store in source order before selector preludes, copy to every unique scalar block/if target temp or one- or multi-parameter loop entry-local vector, clear their payloads, and remove dead terminal producer roots. Branch-free scalar legacy tries write exact do/catch tails into one shared typed local and become void. Branch-free multivalue legacy tries write exact independently scalar do/catch tails into one shared typed vector when one exclusive repeated consumer span owns the results. Functions containing legacy `Catch`/`CatchAll`, `rethrow`, or `delegate` nodes stay whole-function fail-closed until nested-pop repair exists. Nested terminal `br`/`br_table` operands leave an `unreachable` placeholder while owner-region tracking preserves exactly one real effect, and scalar body-result materialization refuses unsupported root controls. Multivalue conditional and table payloads plus branch-targeted multivalue loop controls remain open behind the whole-function gate. |
