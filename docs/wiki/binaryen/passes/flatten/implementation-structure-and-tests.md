...
| Internal owner | `src/passes/flatten.mbt` and `src/passes/flatten_test.mbt` | The first slices classify Flat IR violations, materialize scalar function results, lower reachable/unreachable tees at function roots, structured-region roots, and ordinary operand positions, sequence ordered scalar ordinary-operand preludes, route branch-free defaultable scalar block/if and zero-input/no-backedge loop results, and redirect plain carried scalar `br` plus simple scalar `br_if` payloads through named target temps for defaultable scalar block targets. Same-type `br_if` preserves its false-path flow through a shared `local.get`; the target/flow-type mismatch uses a second correctly typed flow temp and one target copy. Simple `br_table` stores once into a staging temp and copies to every unique target temp before clearing its payload. Rich/multivalue conditional and table payloads remain open and currently trigger a whole-function fail-closed gate before operand spilling, preventing shared HOT values from being reevaluated. |
