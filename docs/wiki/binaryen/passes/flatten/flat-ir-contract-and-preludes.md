...
The August 4 follow-up corrects the remaining trap-order hole in that rule. Prelude migration must preserve not only the value observed by an earlier local read but also the evaluation of a trapping wrapper around it. When a later operand lowers a tee or otherwise introduces a write to local `$x`, an earlier plain `local.get $x` is captured before the write. When the earlier operand is `ref.as_non_null`, flatten instead materializes the complete assertion before every newly inserted later-operand prelude and stores its exact non-null result type. This prevents a later call, global write, or memory store from running before the null trap. Writes to a different local still do not force a plain nontrapping read into a temporary, and multiple earlier operands retain source order.

The same follow-up makes dependency discovery function-scoped: generation-stamped local marks avoid full-table clearing, one pending-node array is reused for fallback traversal, and tee/scalar-spill preludes record exact writes when constructed. Region-flow summaries likewise use a separate root slot plus direct holder-indexed optional entries, are computed lazily, carry their construction revision, and revalidate stale positive terminators. Sorted insertion and eager all-region priming are no longer on the rewrite path.

So a good mental model is:

- flatten converts stack-style nesting into statement-style sequencing
- it does not change the intended order of observable work or local values
...
A partial lane vector, mixed tag, then/else path, other non-first-descendant or repeated use, direct-root composition outside the exact first ordered lane roots, nested catch, loop/multiple execution, catch-all payload extraction, and sharing/outside ownership remain separately deferred. At that checkpoint, direct exceptional transfer handled any positive depth-zero catch-all rethrow population under one active catch through arbitrary strict direct resultless untargeted block/if ancestry, plus one direct scalar value-if arm whose opposite arm is a matching simple value, and an outer-target resultless delegate whose sole catch representation was either direct or a strict direct resultless single-root unused-label block chain. The later nested-catch sections supersede the blanket nonzero/nested-catch boundary; typed composition, loops, nested try-body rethrows, targeted or broader value-carrying ifs, and non-block/mixed/value-carrying/used-label/nested delegate populations keep the complete function unchanged before unrelated flatten mutation.
...
At that checkpoint, internal flatten admitted this only when every depth step was a direct markerless resultless catch-all try owner. The next iteration supersedes the wrapper boundary without changing the Flat IR sequencing rule: ordinary rich work still becomes preludes, the rethrow immediate remains unchanged, and every exceptional owner/path is proven before mutation.

The lowering and flatten red-first transitions were `89/90 -> 90/90` and `200/201 -> 201/201`. Final IR was `327/327`, passes `5,794/5,794`, and full `9,264/9,264`.

## Strict nested-catch wrapper ancestry

Positive nonzero depth may now cross resultless unused-label controls between catch owners. Blocks must own the current node as their sole body root. An `if` must own it as the sole root of exactly one selected arm; the other arm cannot share that root. Arbitrary strict block/if mixes are accepted independently at every depth step before the next markerless resultless catch-all owner is matched.
