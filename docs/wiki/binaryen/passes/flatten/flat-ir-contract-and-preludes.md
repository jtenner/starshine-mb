...
Pinned Binaryen v130 probes show the same order: rich left call, payload values, condition, `br_if`, higher-lane consumption, then the lane-zero binary. Commits `843614438` and `35ac3740a` apply the same exact proof to inputful loops for independently scalar lanes and one exclusively owned repeated `TupleMake`. Commits `3cb5577ad` and `c5a0a738a` additionally allow at most one supported rich payload origin in that loop vector. Loop admission freezes the branch root and rich-left/binary identities; rewrite emits the rich-left local write before payload staging and the branch. For tuple flow, that write is inserted before any child-generated rich-payload prelude, preserving left-before-payload source order. Commits `61055698d` and `d9aa4cd94` apply the same bounded vector rule to legacy tries for independently scalar lanes and one exclusively owned repeated `TupleMake`. Both control families now permit at most one supported rich payload origin alongside the pre-branch rich left. The shared legacy-try rewrite inserts the left store before child-generated payload preludes, then preserves payload-vector order, condition, payload-free `br_if`, and false-path binary placement. More than one rich payload and multiple or non-lane-zero rich lefts remain gated. Exact branch-plus-consumer ownership, complete tuple coverage, result typing, direct-drop placement, current structure, and the pre-mutation proof boundary remain unchanged.
