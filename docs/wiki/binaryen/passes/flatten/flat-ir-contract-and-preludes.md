...
The later `3c819922c` / `41533e603` iteration supersedes only the earlier one-rethrow and direct-block-only ceilings. One active catch may now own any positive number of depth-zero rethrows while sharing one captured exception-reference local. Each rethrow must independently resolve to that catch through direct blocks and at most one resultless, untargeted `if` arm. Lowering visits arm regions but never treats the condition as rethrow ancestry and still refuses loops and nested try/catch discovery.
