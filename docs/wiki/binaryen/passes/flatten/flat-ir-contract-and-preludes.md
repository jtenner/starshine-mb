...
A partial lane vector, mixed tag, then/else path, other non-first-descendant or repeated use, broader independent lane path, nested catch, loop/multiple execution, catch-all payload extraction, and sharing/outside ownership remain separately deferred. Direct exceptional transfer is handled only by the exact catch-all depth-zero rethrow and direct outer-target delegate subsets above; every broader `Rethrow` or `Delegate` shape keeps the complete function unchanged before unrelated flatten mutation.
