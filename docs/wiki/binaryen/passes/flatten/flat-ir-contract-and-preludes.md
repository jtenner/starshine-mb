...
The August 1 local-ordering review tightened that rule further. Prelude migration must preserve not only side effects but also the value observed by an earlier local read. When a later operand lowers a tee or otherwise introduces a write to local `$x`, an earlier `local.get $x` is captured into a fresh temp before the write. The same dependency is preserved below an allowed `ref.as_non_null` wrapper by capturing the nullable source read; writes to a different local do not force staging. Multiple earlier reads are each kept on the pre-write side of the new prelude.
