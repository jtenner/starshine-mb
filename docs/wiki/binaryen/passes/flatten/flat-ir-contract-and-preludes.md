...
Starshine's internal legacy-try table route now uses one recursive resultless direct-call argument collector instead of bounded one-multiply, two-multiply, and deeper special cases. It records every immediate call argument first, then recursively records supported unary, conversion, and binary descendants in source operand order without a tree-depth cap. Every node must remain live, scalar, distinct, and one-use before complete suffix deletion; direct `i32.sub`, `i32.mul`, `i32.and`, `i32.or`, `i32.xor`, `i32.shl`, and `i32.shr_s` join the established top-level root roster, while `i32.shr_u` remains the tested boundary. One run-wide use-def snapshot owns the suffix proof, and one cached label-use bitset now serves support plus rewrite routing. Admission caches exact owned node vectors and owner regions; mutation may consume only those cached proofs. A cache miss after rewriting starts fails closed, so no stale analysis can widen ownership.

This is not generic purity or effect analysis. Binaryen v130 keeps the unreachable suffix in Flat IR order; Starshine deletes it only because the preceding `br_table` is unconditional and no deleted node can execute or be shared. Alternate unary/conversion opcodes, repeated descendants, result calls, indirect/reference calls, and structured suffix roots remain gated. On the refreshed three-probe matrix, nonthrowing synthetic-try elision reduces Starshine direct and cleanup output to `212` aggregate bytes versus Binaryen's `275` direct and `236` cleanup bytes; this narrow bridge/local/control family remains a measured Starshine size win. Current candidate-dense pass-local timing is `1,259 us`, or `4.73x` Binaryen, after scalar-try and label-use caching; performance remains outside target. See [`../../../raw/binaryen/2026-07-15-flatten-version-130-nonthrowing-bridge-suffix-cache-impact.md`](../../../raw/binaryen/2026-07-15-flatten-version-130-nonthrowing-bridge-suffix-cache-impact.md).
