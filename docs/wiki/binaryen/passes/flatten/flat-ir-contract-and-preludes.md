...
The internal legacy-try table route admits three exact structured root kinds after an unconditional `br_table`: void `block { drop(const) }`, void `if (const) { drop(const) } else { drop(const) }`, and zero-input/no-backedge void `loop { drop(const) }`. It now accepts any positive control-only suffix vector whose roots are individually one of those exact shapes. This is not ordinary prelude motion. The table proves the suffix cannot execute; immutable ownership and label-use facts prove every tree is exclusive; and `hot_delete_detached_control_forest(...)` removes the complete descendant/label union atomically after detachment. `hot_delete_detached_control_subtree(...)` remains the one-root wrapper.

The forest proof rejects duplicate or overlapping roots, cross-tree sharing, cycles, outside owners, incomplete labels, and outside branch/delegate/try-table-catch users before mutation. The if proof includes its condition, both HOT region-holder blocks, both arm drops/constants, the if label, and both region labels. The loop proof requires void type, zero branch arity, no loop-label users or backedges, one body root, and exclusive drop/constant ownership. Fresh pinned Binaryen v130 keeps an ordered block + if + loop suffix under direct flatten at `76` bytes, while matched `--vacuum --dce` removes it at `63` bytes. Richer, shared, targeted, or scalar/control-mixed shapes remain fail-closed before any flatten rewrite.
