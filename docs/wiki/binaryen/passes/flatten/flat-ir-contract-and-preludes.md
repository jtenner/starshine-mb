...
The internal legacy-try table route now admits three exact single-root controls after an unconditional `br_table`: void `block { drop(const) }`, void `if (const) { drop(const) } else { drop(const) }`, and zero-input/no-backedge void `loop { drop(const) }`. This is not ordinary prelude motion. The table proves the suffix cannot execute; immutable ownership and label-use facts prove the whole subtree is exclusive; and `hot_delete_detached_control_subtree(...)` removes every descendant and owned label atomically after detachment.

The if proof includes its condition, both HOT region-holder blocks, both arm drops/constants, the if label, and both region labels. The loop proof requires void type, zero branch arity, no loop-label users or backedges, one body root, and exclusive drop/constant ownership. Pinned Binaryen v130 keeps both roots under direct flatten, while matched `--vacuum --dce` reduces each probe from `76` to `63` bytes. Richer or shared shapes remain fail-closed before any flatten rewrite.
