...
- It now has an **internal active-partial** Starshine owner with a Flat IR classifier, scalar function-result materialization, reachable/unreachable tee lowering across region roots and operand positions, ordered scalar operand preludes, and branch-free defaultable scalar `block`/`if` routing; the public registry remains `Removed` while broader correctness work continues.
