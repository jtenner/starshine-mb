...
## 2026-07-16 scalar typed catch repair

Commits `55e508e83` and `864d32c06` implement the first source-backed `handleBlockNestedPops(...)` subset on top of the typed catch-entry representation. The generic HOT transaction preflights the complete function before mutation: every catch payload must be one childless scalar marker at catch entry, have exactly one nested use on the catch body's first-descendant line, and have exclusive ancestry with no loop, nested try/catch path, sharing, outside use, catch-all marker, or second payload lane. Only then does it allocate an exact typed local, insert `local.set(local, Catch(...))` immediately after the payload prefix, and replace the old nested use with `local.get`.

Internal flatten now admits that exact scalar family, performs the repair before ordinary flatten mutation, rebuilds every immutable rewrite proof from the repaired HOT, and validates the lowered module. The focused fixture matches pinned v130 `try_catch_pop_fixup`: an `i32` payload beneath two nested blocks followed by a branch to the enclosing block. The red-first sequence was unbound generic transaction APIs, then flatten admission remaining deferred. Final validation is HOT mutation `14/14`, IR `324/324`, focused flatten `263/263`, whitebox flatten `187/187`, passes `5,780/5,780`, full `9,247/9,247`, and green `moon info` with 11 existing warnings.

This is still a bounded scalar subset. Ordered multivalue payloads, a second lane, non-first-descendant uses, nested catches, loops or multiple execution, sharing/outside ownership, catch-all payloads, `rethrow`, and `delegate` remain whole-function fail-closed before ordinary flatten mutation. The reviewed `.mbti` adds only generic IR repairability/repair APIs; registry, dispatcher, CLI execution, compare/API, generator/profile, presets, and public flatten wiring remain absent. The durable `970.5 us` / `3.65x` performance checkpoint remains unrequalified.
