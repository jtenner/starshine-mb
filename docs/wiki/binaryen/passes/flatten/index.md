...
- The August 1 review found and repaired a separate prelude-ordering defect: when a later operand introduces a `local.set`, earlier reads of that same local are now snapshotted before the new prelude. Direct `local.get`, multiple earlier reads, and reads beneath `ref.as_non_null` are covered; nonmatching local writes do not allocate a dependency snapshot. A Node runtime fixture returns the original results (`42`, `0`, `41`, and `42`) for matching scalar, wrapped reference, multiple-read, and nonmatching-local cases. No-fallthrough facts now use one cached per-region summary containing the first unconditional root, effective terminal branch, dead-suffix start, and fallthrough possibility; rewrite rejects unseen post-mutation regions, while cached positive facts are recomputed if their recorded terminator root is replaced or shifted. The retained candidate-dense benchmark records a `196.667 ms` median versus `197.103 ms` before this change, so the scan bound is proven structurally but is not claimed as a pass-level speedup. Rebuilt native SHA-256 `65c8298709b14006bcf2c07da970aaf15e0a5125f96eb71def75305740a10ee6` completes a restarted `flatten-all` `10,000/10,000` lane with `1,632` direct plus `6,701` cleanup-normalized matches, the unchanged `842` `flatten-br-if` plus `825` multivalue residual populations, `10,000/10,000` idempotence, zero failures, and `13,270` runtime exports with no semantic mismatch.
