...
- Direct-pass closeout is restored after the July 31 carried payload-branch and single-arm legacy-EH repairs. Native SHA-256 `96d0ad4aff0e0a759faa887f5879fc7eac731adbb91fdfb3ac014549ee52e903` against official `wasm-opt version 131 (version_131)` completes regular GenValid at `100,000/100,000`, wasm-smith at `9,955` comparable with `45` Binaryen/tool failures, `flatten-all` at `10,000/10,000`, and random-all at `9,641` comparable with `359` Binaryen failures. Starshine has zero command, generator, validation, or property failures. The `1,667` dedicated residuals remain exactly `842` scalar-`br_if` plus `825` multivalue cases; the `801` random residuals remain the eight documented non-EH families. Every canonical Starshine residual is smaller, and all `2,468` pairs converge byte-for-byte under verified v131 `-Oz --strip-debug`, classifying them as direct cleanup wins with explicit reopening criteria. Multi-arm legacy tries remain an upstream HOT representation boundary, not an admitted direct-pass family.
- The August 4 repair closes the remaining sibling-prelude trap-order defect. An earlier `ref.as_non_null` is now materialized as one indivisible evaluation before any later operand prelude, using the exact non-null result type; a later tee's call, mutable-global write, and memory store therefore cannot precede the null trap. Plain `local.get` still snapshots only for a write to the same local. The executable Node fixture covers null/non-null, same/different target locals, nested assertions, one call, one global mutation, and one memory store; null cases trap with zero effects, while non-null cases return the original result and perform the effect once. Local-write discovery now reuses generation-stamped function scratch plus one traversal stack and records common prelude writes at construction. Region-flow summaries now occupy direct holder-indexed optional slots with a separate root entry, lazy construction, revision metadata, stale-positive revalidation, and explicit holder bounds/liveness checks. Native-release medians move `206.353 -> 202.429 ms` for repeated-small-set local writes, `206.507 -> 200.444 ms` for spread writes, `189.126 -> 188.819 ms` for the region-density fixture, and `1.349 -> 1.2675 ms` for the historical candidate-density fixture. These are modest measured wins plus a structural removal of per-batch full-local allocation, subtree scans for recorded writes, eager summary construction, binary search, and array insertion.
...
- The reconstructed historical 120-function candidate-dense native-release lane measures `1,392 us` Starshine versus `126.709 us` Binaryen v131 (`10.99x`). The relative ratio widened because v131 is faster than the historical v130 oracle, while Starshine remains below `1.5 ms` and far below the repository's `<1 s` absolute target; the bounded exception therefore remains qualified. The separate 120-function/960-branch stress lane measures `8,987 us` versus `926.059 us` and remains scaling telemetry.
...
Both red-first fixtures returned unchanged before implementation. Final validation is focused flatten `257/257`, private flatten `186/186`, passes `5,773/5,773`, full suite `9,234/9,234`, and green `moon info` with 11 existing warnings. Fresh pinned-v130 scalar and tuple-oriented probes order `call $left`, the payload vector, condition, and `br_if`, then retain `i32.sub`. This at-most-one boundary was the handoff point for the next iteration. Multiple/non-lane-zero rich lefts, typed EH, exceptional transfer, structured label-owner deletion, broader mixed/shared/nested flow, aggregate/signoff, ordered neighborhood, performance requalification, and public admission remained open. Public wiring was unchanged and `flatten` remained removed.

## 2026-07-16 multiple-rich-payload plus rich-left follow-up

Commits `6e4229bae` and `f223f14d6` remove the temporary one-rich-payload cap for the exact source-backed rich-left family. Independently scalar legacy-try and inputful-loop vectors now admit every payload lane whose origin is individually supported and exactly branch-plus-consumer owned. One exclusively owned repeated `TupleMake` admits the same rule after complete component, branch-slot, consumer, type, defaultability, and safe-deletion proof. The pre-branch rich left still pairs only with lane zero.

One red-first test per code commit covers both control families. Focused flatten moved `257/258 -> 258/258` and `258/259 -> 259/259`; private flatten is `186/186`, passes are `5,775/5,775`, the full suite is `9,236/9,236`, and `moon info` is green with 11 existing warnings. Four fresh pinned-v130 probes preserve rich left, every payload call in vector order, condition, payload-free `br_if`, and the false-path binary. No `.mbti` or public surface changed.

## 2026-07-16 structured subtree deletion iteration

Commit `62c330a12` adds an atomic HOT mutation for one detached control subtree plus every label it owns. It refuses rooted or externally owned nodes, shared/cyclic descendants, malformed control-label ownership, and outside branch or try-table catch users before mutation; successful deletion tombstones the complete node/label set with one revision advance. Commit `f7829507b` uses that capability for one exact legacy-try suffix family: an unconditional scalar `br_table` followed by one unshared, untargeted void `block` containing one direct `drop(const)`.

The focused behavior test failed unchanged at `259/260`, then passed at `260/260`; private flatten remains `186/186`, IR tests pass `319/319`, passes pass `5,776/5,776`, the full suite passes `9,238/9,238`, and `moon info` is green with 11 existing warnings. Pinned Binaryen v130 retains the structured block after direct `--flatten`; its matched `--vacuum --dce` output shrinks from `76` to `63` encoded bytes, confirming the subtree is cleanup debris. Starshine deletes the three exact nodes plus owned label immediately under the existing unconditional-transfer proof. Externally targeted blocks, shared descendants, other structured root kinds, mixed structured suffix vectors, multiple/non-lane-zero rich lefts, typed EH, exceptional transfer, broader mixed/shared/nested flow, the flatten aggregate, four-lane signoff, ordered neighborhood, performance requalification, and public admission remain open.

## 2026-07-16 structured `if` and `loop` suffix follow-up

Commits `51d080e09` and `cbb1a2395` extend the exact unconditional-transfer cleanup family from one void block to two additional structured roots. Starshine now deletes one void `if` with a constant condition and one direct `drop(const)` in each complete arm, including both region holders and all three owned labels, and one zero-input/no-backedge void `loop` with one direct `drop(const)` body and its owned label. Every root, descendant, region holder, and label is preflighted from immutable use and branch indexes before suffix detachment; the existing atomic HOT mutation performs complete deletion.

The red-first focused sequence moved `260/261 -> 261/261` and `261/262 -> 262/262`. Private flatten remains `186/186`, passes are `5,778/5,778`, the full suite is `9,240/9,240`, and `moon info` is green with 11 existing warnings. Pinned Binaryen v130 retains both dead controls under direct `--flatten`; matched `--vacuum --dce` reduces each probe from `76` to `63` encoded bytes. Nonconstant or effectful conditions, partial/richer arms, loop backedges or inputs, sharing, and external label users remain fail-closed. Public wiring, typed EH, exceptional transfer, aggregate/signoff, neighborhood, and performance gates remain unchanged.

## 2026-07-16 structured control-forest suffix follow-up

Commit `8b69a8e4c` generalizes the HOT mutation boundary from one detached control tree to an exact detached control forest. `hot_delete_detached_control_forest(...)` rejects empty or duplicate roots, overlap, cross-tree sharing, cycles, live roots or outside owners, incomplete label ownership, and outside branch/delegate/try-table-catch users before mutation; success tombstones the complete node/label union with one revision advance. The single-tree API remains a wrapper. Its red-first mixed block/if/loop invariant first failed on the unbound API and then passed in HOT mutation `12/12`; IR passes `320/320`. The reviewed `.mbti` adds only the generic forest API.

Commit `f524b8bcb` admits an ordered control-only suffix vector when every root is one of the existing exact supported block, if, or loop shapes. The red-first block + if + loop fixture failed unchanged at `262/263`, then passed at `263/263`; private flatten remains `186/186`, passes are `5,779/5,779`, the full suite is `9,242/9,242`, and `moon info` is green with 11 existing warnings. Fresh pinned Binaryen v130 direct flatten retains the three-root debris at `76` bytes, while matched `--vacuum --dce` removes it at `63` bytes.

## 2026-07-16 mixed ordinary/control forest follow-up

Commit `04cc07657` generalizes the mutation boundary again with `hot_delete_detached_forest(...)`. It accepts ordinary or control roots, traverses the complete descendant union, owns every traversed control label, and rejects empty/duplicate/overlapping roots, sharing, cycles, live roots, outside owners, incomplete labels, and outside branch/delegate/try-table-catch users before one node/label tombstone and revision advance. The control-forest and subtree APIs remain validating wrappers. Its red-first mixed drop/block/unreachable invariant moved from an unbound symbol to HOT mutation `13/13` and IR `321/321`; the reviewed `.mbti` adds only this generic IR API.

Commit `ca159e6d0` admits positive scalar/control-mixed suffix vectors when every root already passes an exact ordinary or structured recognizer and the vector contains both kinds. The red-first `drop(const) + block + if + loop + unreachable` fixture failed unchanged at `262/263`, then passed at `263/263`; private flatten is `186/186`, passes are `5,779/5,779`, the full suite is `9,243/9,243`, and `moon info` is green with 11 existing warnings. Fresh pinned Binaryen v130 direct flatten retains the complete mixed suffix at `76` bytes; matched `--vacuum --dce` removes it at `63` bytes. Richer controls, sharing, external users, typed EH, exceptional transfer, broader mixed/shared/nested flow, aggregate/signoff, ordered-neighborhood, performance, and public admission remain open.

## 2026-07-16 ordered two-lane typed catch repair

Commits `43ea95972` and `49079c0dc` extend the first source-backed `handleBlockNestedPops(...)` subset from one scalar marker to one exact ordered two-lane family. Generic HOT preflight accepts either the prior scalar first-descendant shape or two childless same-tag scalar markers followed by one exclusive direct block chain whose first two final-region roots use lane zero and lane one exactly once. Lane locals are allocated in source order; entry captures consume the handler stack in reverse lane order; old positions become typed local reads. A partial second lane, outside/shared use, mixed tag, third lane, loop, nested exceptional control, catch-all marker, or incomplete ancestry rejects the whole function before mutation.

Internal flatten now admits that exact plan, repairs before ordinary rewriting, rebuilds every immutable proof snapshot, and lowers two handler values through an existing result-only multivalue block type. The red-first sequence was unbound ordered repair APIs, then whitebox admission remaining `DeferredCatchPayloadRepair`. Final validation is HOT mutation `16/16`, IR `326/326`, focused flatten `263/263`, whitebox flatten `188/188`, passes `5,781/5,781`, full `9,250/9,250`, `moon fmt`, and green `moon info` with 11 existing warnings. The reviewed `.mbti` adds only `hot_ordered_catch_payloads_repairable(...)` and `hot_repair_ordered_catch_payloads(...)`; the lowering/pass commit adds no API snapshot.

This remains a bounded EH subset. Non-first-descendant or repeated payload uses, broader independent lane paths, nested catches, loops or multiple execution, sharing/outside ownership, and catch-all payload extraction remain whole-function fail-closed. Registry, dispatcher, CLI execution, compare/API, generator/profile, presets, and public flatten wiring remain absent. The durable `970.5 us` / `3.65x` performance checkpoint remains unrequalified.

## 2026-07-16 direct exceptional-transfer subsets

Commits `d76a91b3f` and `7bfb10372` resolve one exact Binaryen v130 family for each legacy exceptional transfer. Pinned `Flatten.cpp` does not rewrite either form; pinned `eh-utils.cpp` repairs only nested catch payload `Pop`, and pinned `wasm.h` stores delegate as `Try::delegateTarget`. Starshine therefore preserves transfer semantics rather than treating either operation as ordinary flatten work.

A depth-zero catch-all `rethrow` under a strict direct block ancestry is admitted only when it belongs to the active catch. HOT lowering captures the non-null exception through `catch_all_ref`, stores it in an `exnref` local, and emits `local.get` plus `throw_ref`; broader rethrow depths, typed-payload composition, nested-catch ownership, and non-direct ancestry remain deferred. A resultless inner try whose sole catch root delegates to its directly enclosing outer try is admitted only when its own label is unused and the target label remains exact and active. Lowering removes only that obsolete inner handler shell so exceptions propagate into the outer handler; non-direct, value-carrying, mixed catch/delegate, used-label, and broader nested populations remain deferred.
...
Commits `3c819922c` and `41533e603` superseded the earlier one-rethrow and direct-block-only boundaries. Any positive population of depth-zero rethrows under one active catch shared one captured exception-reference local; every admitted node independently resolved through strict direct ancestry to that same owning catch. At that checkpoint, the ancestry could contain direct blocks and at most one resultless, untargeted `if` arm. Lowering discovered rethrows through block bodies and then/else regions, never through an `if` condition, loop, or nested try/catch region, and emitted one `catch_all_ref` capture plus one `throw_ref` use per rethrow.

The red-first whitebox states were `192/193`, where lowering aborted above one rethrow, and `193/194`, where admission rejected the nested-if arm. Validation reached HOT lower `89/89`, IR `326/326`, focused flatten `263/263`, whitebox flatten `194/194`, passes `5,787/5,787`, full `9,256/9,256`, `moon fmt`, and green `moon info` with 11 existing warnings. No `.mbti` or public flatten surface changed. The one-if ceiling recorded here is superseded by the next section.

## 2026-07-16 strict exceptional ancestry breadth

Commit `9c237165d` removes the temporary single-if ceiling. A depth-zero catch-all rethrow may cross any strict direct chain of resultless untargeted if arms and direct blocks, while each rethrow still proves exact owning-catch ancestry. Conditions, loops, nested try/catch regions, value-carrying ifs, and used if labels remain excluded. The two-if/interleaved-block fixture failed at whitebox `194/195`, then passed `195/195` and validated after lowering.
...
## 2026-07-16 independent catch roots and scalar value-if rethrow breadth

Commit `fb9d071e8` supersedes the blanket independent-lane blocker for one exact source-backed layout. After the ordered leading same-tag payload markers, the first matching direct catch-region roots may now own their lanes independently instead of requiring one common block-chain carrier. Each lane still follows only the exact first-child path; one fixture combines a binary first child with a separate `if` condition and preserves a later catch root unchanged. Entry plus old-position ownership, lane order, reverse handler-stack capture order, and whole-function preflight remain mandatory.

Commit `52fc64b49` supersedes the blanket value-carrying-if rethrow blocker for one scalar subset. A depth-zero rethrow may be the terminal direct arm of an unused-label, defaultable scalar `if` when the opposite arm ends in one matching Flat-IR-simple scalar value and the `if` remains directly owned by the active catch. Flatten routes only the fallthrough value arm through a local, then erases the surrounding scalar legacy-try result through its existing local channel. Nonzero depth, typed composition, nested try/catch ownership, loops, targeted ifs, non-simple opposite arms, multivalue results, and broader value-control chains remain excluded.

Red-first whitebox counts moved `198/199 -> 199/199` and `199/200 -> 200/200`. Final validation is HOT mutation `16/16`, HOT lower `89/89`, IR `326/326`, focused flatten `263/263`, whitebox flatten `200/200`, passes `5,793/5,793`, full `9,262/9,262`, `moon fmt`, and green `moon info` with 11 existing warnings. No `.mbti` or public registry, dispatcher, CLI execution, compare/API, profile, preset, scheduler, or neighborhood-ready surface changed. Performance was not remeasured; `970.5 us` / `3.65x` Binaryen v130 remains the durable checkpoint.
...
This does not admit multiple label users, payload-carrying exits, branches outside the sole opposite arm, multi-root arms, value-carrying wrappers, loops, typed composition, nested try-body transfers, or broader targeted ancestry. The blanket rich-condition exclusion recorded here is superseded only by the exact one-use scalar `i32` condition slice below; shared, control, tee, non-`i32`, and otherwise unsupported conditions remain excluded. The flatten-specific aggregate, four-lane matrix, ordered neighborhood, performance requalification, docs closeout, and public admission remain open.

## 2026-07-16 rich targeted-if conditions and interleaved catch roots

Commits `61fb9919b` and `1fb8f64ec` complete the next exact two-code internal iteration. A targeted resultless catch-if may now keep a payloadless opposite-arm `br_if` whose scalar `i32` condition is one exact one-use supported non-control origin. Ordinary flattening materializes that condition into a local immediately before the branch in the same opposite arm; target identity, selected-arm ownership, rethrow depth, and target-catch slot remain exact. Separately, an ordered same-tag catch payload vector may map to independently owned direct catch roots with unrelated roots between the lane roots. Lane matching remains source ordered and retains exact entry-plus-old-position ownership; intervening and later roots remain unchanged.

The red-first whitebox states were `205/206`, returning `DeferredExceptionalTransferRepair`, and `206/207`, returning `DeferredCatchPayloadRepair`. Final validation is HOT mutation `16/16`, HOT lower `90/90`, IR `327/327`, focused flatten `263/263`, whitebox flatten `207/207`, passes `5,800/5,800`, full `9,270/9,270`, `moon fmt`, and green `moon info` with 11 existing warnings. No `.mbti` or public registry, dispatcher, CLI execution, compare/API, profile, preset, scheduler, or neighborhood-ready surface changed. Performance was not remeasured; `970.5 us` / `3.65x` Binaryen v130 remains the durable checkpoint.
