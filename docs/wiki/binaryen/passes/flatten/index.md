...
This remains a bounded EH subset. Non-first-descendant or repeated payload uses, broader independent lane paths, nested catches, loops or multiple execution, sharing/outside ownership, catch-all payloads, `rethrow`, and `delegate` remain whole-function fail-closed. Registry, dispatcher, CLI execution, compare/API, generator/profile, presets, and public flatten wiring remain absent. The durable `970.5 us` / `3.65x` performance checkpoint remains unrequalified.
