...
## 2026-07-16 typed catch-entry representation prerequisite

Commits `71971e924` and `161064a0f` choose and implement the first local representation direction for Binaryen legacy-EH payload `pop`: HOT now uses ordered typed `Catch` pseudo-values as the leading roots of one legacy catch region, carrying the exact tag and scalar value type. Verification rejects payload markers outside a catch region, after an ordinary catch-body root, with children, with non-scalar type, with mixed tags, or with multiple catch-region entry owners. The reviewed IR `.mbti` adds only `hot_build_catch_payload(...)`, `hot_catch_payload_tag(...)`, and `HotVerifyError::InvalidCatchPayload`; no public flatten API exists.

HOT lowering now consumes one scalar payload lane from the implicit exception stack by routing a typed `try_table` catch to a typed handler block, then lowering the catch body with the payload marker already on its value stack. The slice also corrected the synthetic catch-all bridge target from outer depth `1` to handler depth `0`; the normal path still branches over the handler at depth `1`. The red-first lower test initially aborted on unhandled `Catch`, then an intermediate attempt failed validation because payload and target-label types differed, and the final exact output validates.

This resolves the **representation direction**, not flatten EH readiness. `flatten_run` still selects `DeferredCatchPayloadRepair` for every `Catch`/`CatchAll` function before mutation. Multivalue payload entry, payload-to-local extraction, old-position replacement, nested-catch exclusion, block-nested-pop relocation, `rethrow`, and `delegate` remain open. Final validation is verify `4/4`, lower `89/89`, IR `323/323`, focused flatten `263/263`, private flatten `186/186`, passes `5,779/5,779`, full `9,245/9,245`, and green `moon info` with 11 existing warnings. The durable `970.5 us` / `3.65x` performance checkpoint is unchanged and unrequalified; all public wiring remains removed.
