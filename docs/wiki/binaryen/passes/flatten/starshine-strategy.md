...
Commits `51d080e09` and `cbb1a2395` reuse the verified atomic control-plus-label deletion path for one exact void `if` and one exact void `loop` after unconditional legacy-try table transfer. The if requires a constant condition, complete then/else region holders, one direct `drop(const)` root per arm, exclusive ownership, and no users of the if or region labels. The loop requires zero branch arity, no target users or backedges, one direct `drop(const)` body, and exclusive ownership. All facts are frozen before mutation.

The focused red-first sequence reaches `262/262`; private flatten is `186/186`, passes `5,778/5,778`, full `9,240/9,240`, and `moon info` is green. Pinned Binaryen v130 retains both roots under direct flatten; matched cleanup reduces each probe `76 -> 63` encoded bytes. Broader/multiple structured suffixes and all public admission gates remain open.
