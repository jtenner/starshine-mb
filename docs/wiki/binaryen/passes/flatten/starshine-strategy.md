...
`src/passes/flatten.mbt` owns a Flat IR classifier plus scalar body-result materialization, reachable/unreachable tee lowering across function roots, structured-region roots, and ordinary operand positions, ordered scalar operand preludes, and branch-free defaultable scalar `block`/`if` result routing, with focused coverage in `src/passes/flatten_test.mbt`. The registry and dispatcher intentionally remain public-removed until broader families are safe.
...
- complete branch-targeted control, loop/try, branch-payload-channel, generic unreachable-placeholder, EH, or unsupported-family behavior
