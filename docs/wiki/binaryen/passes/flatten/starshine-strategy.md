...
- rich operands become ordered preludes and typed local traffic;
- `local.tee` is eliminated;
- scalar and multivalue block/if/loop/legacy-try results route through typed locals;
- carried `br`, `br_if`, and `br_table` values route through target-local vectors;
