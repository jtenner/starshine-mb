...
Validation reaches focused flatten `259/259`, private flatten `186/186`, passes `5,775/5,775`, full `9,236/9,236`, and green `moon info`. Four fresh pinned-v130 probes cover heterogeneous independently scalar and homogeneous tuple-oriented legacy-try and inputful-loop shapes; every oracle orders the rich left, all payload calls in vector order, the condition, `br_if`, then `i32.sub`. No `.mbti`, registry, dispatcher, CLI execution, compare/API, generator, preset, or performance surface changed. Multiple/non-lane-zero rich lefts, typed EH, exceptional transfer, structured label-owner deletion, broader mixed/shared/nested control, aggregate/signoff, ordered neighborhood, performance requalification, and public admission remain open.
