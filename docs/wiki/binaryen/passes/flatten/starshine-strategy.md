...
`src/passes/flatten.mbt` owns a Flat IR classifier plus scalar body-result materialization, reachable/unreachable tee lowering across function roots, structured-region roots, and ordinary operand positions, ordered scalar operand preludes, branch-free defaultable scalar `block`/`if` result routing, defaultable scalar branch-targeted `if` routing, zero-input and independently scalar inputful/no-backedge scalar-result `loop` routing, and plain `br`, scalar `br_if` routing through typed temps, plus scalar `br_table` rich-origin and unique-target fanout for defaultable scalar block/if targets, with focused coverage in `src/passes/flatten_test.mbt`. Same-type `br_if` preserves the not-taken flow through one shared `local.get`; the target/flow-type mismatch uses a second flow temp and one target copy; rich ordinary scalar payloads are evaluated once before the condition and shared across chained false-path uses; `br_table` evaluates rich ordinary payloads once before its selector, copies to deduplicated target temps, and removes the dead terminal origin. Nested terminal `br`/`br_table` operands now become owner-region effects plus `unreachable` placeholders, with already-rooted effects deduplicated, and unsupported root value-controls are not scalar-materialized. Multivalue `br_if` and table payloads remain open behind a pre-rewrite whole-function gate. Branch-targeted ifs now share one temp across fallthrough arms and carried `br`/`br_if` flow after preflighting every label use; nondefaultable branch payloads remain whole-function fail-closed. Inputful no-backedge loops capture each independently scalar defaultable entry once in order, redirect body uses through typed locals, clear the parameter prefix, and route scalar results separately; nondefaultable and multivalue entry producers fail closed. The registry and dispatcher intentionally remain public-removed until broader families are safe.
...
- replace the remaining multivalue `br_if`/`br_table` gates, or implement backedge loop or legacy-try routing, broader terminal-placeholder families, EH, or unsupported-family behavior
