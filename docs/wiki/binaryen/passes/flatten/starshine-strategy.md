...
Commits `51d080e09` and `cbb1a2395` reuse the verified atomic control-plus-label deletion path for one exact void `if` and one exact void `loop` after unconditional legacy-try table transfer. Commit `8b69a8e4c` adds the forest mutation and keeps the one-root API as a wrapper; commit `f524b8bcb` admits a complete ordered block + if + loop suffix only after every tree passes the existing immutable shape/ownership/label proof and the vector is control-only.

The forest invariant moves from an unbound-API red to HOT mutation `12/12` and IR `320/320`. The flatten fixture moves `262/263 -> 263/263`; private flatten is `186/186`, passes `5,779/5,779`, full `9,242/9,242`, and `moon info` is green. Fresh pinned Binaryen v130 retains the three-root suffix under direct flatten at `76` bytes; matched cleanup removes it at `63` bytes. Richer controls, scalar/control-mixed vectors, typed EH, exceptional transfer, aggregate/signoff, ordered neighborhood, performance requalification, and all public admission gates remain open.
