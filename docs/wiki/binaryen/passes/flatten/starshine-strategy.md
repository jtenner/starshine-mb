...
This clears the old blanket Starshine output-measurement blocker only for the synthetic catch-all subset. Nonthrowing-body proof now removes dead handler scaffolding and keeps a block only for a live try-label channel. On the matched three-probe matrix, current Starshine is `212` bytes both direct and after cleanup, versus Binaryen's `275` direct and `236` cleanup bytes; the narrow bridge/control/local family is therefore a measured size win. Run-wide suffix ownership plus cached EH/effective-terminal proofs improve the representative pass-only median to `1,275 us`, but that remains `4.79x` Binaryen's `266.05 us`. Public removed status, absent profile/four-lane matrix, typed catch/pop repair, broader behavior, and performance remain explicit blockers. See [`../../../raw/binaryen/2026-07-15-flatten-version-130-nonthrowing-bridge-suffix-cache-impact.md`](../../../raw/binaryen/2026-07-15-flatten-version-130-nonthrowing-bridge-suffix-cache-impact.md).
