...
The latest exact two-code iteration continues behavior breadth. `5c4a664dd` admits exact one-use rich right operands for payload-left independently scalar lanes, and `a4055b7a9` admits the same rule for one exclusively owned repeated `TupleMake`. The shared consumer helper also serves exact inputful-loop flow. Rich operands are flattened after payload-free `br_if`, so calls, effects, and traps remain on the not-taken path; payload-right binaries still require simple left operands.

Final validation is focused flatten `249/249`, private flatten `186/186`, passes `5,765/5,765`, full `9,226/9,226`, and green `moon info`. Pinned-v130 legacy-try and loop probes support the placement rule. No public surface or performance gate changed; typed EH/pop, exceptional transfer, structured label-owner deletion, broader mixed/shared/nested control, generator/signoff, neighborhood proof, and public admission remain the higher-priority work.
