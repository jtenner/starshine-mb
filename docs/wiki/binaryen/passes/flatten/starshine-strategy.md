...
## Latest ordered catch-payload repair decision

Starshine now has both a local payload representation and two bounded flatten repairs. Typed legacy catch payloads remain ordered childless `HotOp::Catch` pseudo-values at catch entry. The scalar APIs retain the single first-descendant family. The new generic ordered transaction also accepts exactly two same-tag scalar markers when one exclusive direct block chain ends in ordered unary lane-use roots. It preflights the complete function, allocates locals in source order, inserts entry captures in reverse handler-stack order, and replaces both old positions with local reads. Partial/mixed-tag/third-lane, loop, nested exceptional-control, catch-all, shared, outside, or alternate-path shapes reject before mutation.

Internal flatten admits that exact plan, applies it before ordinary rewriting, and rebuilds every immutable proof from repaired HOT. Lowering accepts at most two lanes and resolves the pair through an existing result-only multivalue block type. The source-backed whitebox fixture uses `(i32, f32)` lanes under two nested blocks and validates the lowered module. HOT mutation is `16/16`, IR `326/326`, focused flatten `263/263`, whitebox flatten `188/188`, passes `5,781/5,781`, and full `9,250/9,250`.

This is not broad typed-EH readiness. Non-first-descendant/repeated uses, broader independent lane paths, nested catches, loops/multiple execution, and catch-all payload extraction remain fail-closed. The `.mbti` adds only two generic IR repair APIs; no registry, dispatcher, CLI execution, compare/API, GenValid, preset, or public pass surface changed, and the durable performance gate remains unrequalified.

## Latest arbitrary-lane and if-condition catch repair decision

Commits `82c4c260c` and `95f71db1e` supersede the earlier temporary two-lane ceiling while preserving the same conservative ownership model. Any positive same-tag leading marker vector may repair when one exclusive direct block chain ends in ordered unary lane-use roots. Local ids remain in source order, entry captures remain reverse stack order, and lowering requires an existing result-only handler type with the exact result vector. The scalar first-descendant path now follows an `if` condition but not either arm, matching pinned Binaryen v130 `EHUtils::getFirstPop(...)`.

Red-first whitebox counts moved `190/191 -> 191/191` and `191/192 -> 192/192`; final focused flatten is `263/263`, IR `326/326`, passes `5,785/5,785`, and full `9,254/9,254`. No `.mbti` or public execution surface changed. Partial/mixed-tag vectors, selected arms, other lane paths, repeated/sharing/outside use, nested catches, loops/multiple execution, and catch-all extraction remain fail-closed. The earlier two-lane section is historical and is superseded only on lane count and if-condition traversal.

## Latest direct rethrow and delegate decisions

Binaryen v130 source separates these forms from nested-pop repair: `Flatten.cpp` has no special rewrite for either; `eh-utils.cpp` only relocates legacy catch `Pop`; and `wasm.h` stores delegation on `Try::delegateTarget`. Starshine now admits two exact internal subsets behind complete pre-mutation ancestry and label proof.

Commit `d76a91b3f` admits one depth-zero catch-all rethrow under a direct block chain. Lowering converts the legacy active-catch identity into modern Core EH by using `catch_all_ref`, capturing the exception reference in a nullable `exnref` local, and emitting `throw_ref` from that exact local. Commit `7bfb10372` admits one resultless delegated try whose sole catch root targets the directly enclosing outer try and whose inner label has no users; lowering keeps the target active and makes the inner legacy shell transparent so the exception reaches the outer handler.
...
The direct ancestry could include blocks and one resultless `if` arm with an unused label at that checkpoint. Lowering searched then/else regions but not the condition and did not recurse into loops or nested tries. The two red-first whitebox states were `192/193` and `193/194`; validation reached whitebox `194/194`, HOT lower `89/89`, focused flatten `263/263`, IR `326/326`, passes `5,787/5,787`, and full `9,256/9,256`. No `.mbti`, registry, dispatcher, CLI, compare/API, profile, preset, scheduler, or public pass surface changed. The prior one-rethrow and block-only claims are retained above as superseded chronology.

## Latest strict exceptional ancestry decisions

Commit `9c237165d` removes the temporary single-if ceiling without weakening the ancestry proof. A depth-zero rethrow may now cross any strict direct sequence of resultless `if` arms and blocks, but every if label must have no users and every step must remain on the exact owning-catch path. Conditions, loops, nested try/catch regions, value-carrying ifs, and targeted ifs remain outside admission. The new two-if/interleaved-block behavior test was red at `194/195` and green at `195/195`.

Commit `88197c97e` widens only the delegated try's catch representation. The catch must have one root, which may be a strict direct chain of resultless single-root blocks ending in the delegate; each block label and the delegated try label must be unused. The target is still the exact active outer try through the existing direct outer ancestry. Lowering recognizes the same chain and removes only the empty catch wrappers and obsolete inner handler shell. The new delegate test was red at `195/196` and green at `196/196`.
