...
The bounded lowerer emits the standard stack/control equivalent for one lane and uses an existing result-only multivalue block type for exactly two same-tag lanes:
...
The payload is already on the stack after exceptional transfer to `$handler`, so catch-entry markers emit no instructions. Internal flatten can repair the scalar shape and one exact ordered two-lane shape into Binaryen-equivalent entry captures:
