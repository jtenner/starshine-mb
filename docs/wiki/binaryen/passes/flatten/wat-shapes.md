...
### Current Starshine nested dead-call argument slice

After an unconditional legacy-try `br_table`, Starshine can now delete one exact owned resultless direct call whose single rich argument is any of these audited forms, with any number of distinct scalar constants at other immediate argument positions:
...
The call and complete subtree are deleted only after immediate argument order, nested left-to-right order, distinctness, one-use ownership, and unconditional transfer are proven. A fixed matrix transformed `126/129` functions versus zero at the iteration baseline; alternate `i32.ctz` arguments and structured suffixes remained gated. Actual Starshine byte/runtime output is not yet representable through legacy-try lowering, and candidate-dense pass-local performance remains a blocker. See the [2026-07-15 impact evidence](../../../raw/binaryen/2026-07-15-flatten-version-130-nested-call-argument-impact.md).
