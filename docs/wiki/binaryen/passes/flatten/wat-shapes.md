...
The call and complete subtree are deleted only after immediate argument order, nested left-to-right order, distinctness, one-use ownership, and unconditional transfer are proven. Direct `i32.sub`, `i32.mul`, `i32.and`, `i32.or`, `i32.xor`, `i32.shl`, and `i32.shr_s` roots now use the same proof; `i32.shr_u` is the current outside-roster boundary. Admission caches the exact owned nodes and owner region from one run-wide use-def snapshot, label-use facts are also cached once per rewrite state, and mutation fails closed on a cache miss. Alternate unary/conversion arguments and structured suffixes remain gated. Nonthrowing synthetic catch-all lowering keeps the refreshed three-probe cleanup matrix a measured 24-byte Starshine win, while current candidate-dense pass-local performance remains `4.73x` Binaryen and blocks public exposure. See the [current 2026-07-15 impact evidence](../../../raw/binaryen/2026-07-15-flatten-version-130-nonthrowing-bridge-suffix-cache-impact.md).
