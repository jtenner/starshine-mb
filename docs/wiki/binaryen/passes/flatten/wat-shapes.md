...
This shape is admitted for independently scalar lanes and one exclusively owned repeated `TupleMake`; the shared exact consumer helper also serves admitted inputful-loop flow. Binaryen v130 legacy-try and loop probes place the rich right call after `br_if`, so Starshine's ordinary operand prelude preserves the not-taken-only effect/trap boundary. A rich left operand with the restored lane on the right remains gated because its source evaluation order is different.
