...
The payload is already on the stack after exceptional transfer to `$handler`, so the catch-entry marker emits no instruction. Internal flatten can now repair one exact nested scalar use into the Binaryen-shaped entry capture:

```text
catch entry: Catch(tag=$tag, type=i32)
             local.set $payload (Catch(...))
catch body:  nested blocks containing drop(local.get $payload)
```

The whole function is preflighted before mutation. The marker must have exactly its entry-root use plus one first-descendant nested use with exclusive ancestry; loops, nested try/catch paths, a second lane, catch-all markers, sharing, outside uses, or other positions reject. The source-backed positive matches v130 `try_catch_pop_fixup` and preserves the following branch target. Ordered multivalue payloads, `rethrow`, and `delegate` remain separately gated.
