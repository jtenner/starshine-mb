...
6. Fuzz signoff: after `moon build --target native --release src/cmd`, add a pass-targeted `bun scripts/pass-fuzz-compare.ts --pass monomorphize-always ... --jobs auto --starshine-bin _build/native/release/build/cmd/cmd.exe` lane only after ordinary `monomorphize` has a green shared engine.
