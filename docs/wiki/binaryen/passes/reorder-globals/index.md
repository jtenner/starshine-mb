...
- Keep it marked as an active direct Starshine module pass, while keeping `reorder-globals-always` boundary-only. The direct late-tail neighborhood has standard signoff, and public presets now append the accepted suffix; future changes should add fresh evidence rather than reopening the accepted SGO-fed RUME layout frontier.
