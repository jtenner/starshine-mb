...
- Keep it marked as an active direct Starshine module pass, while keeping `reorder-globals-always` boundary-only. The direct late-tail neighborhood now has standard signoff; public preset scheduling still needs a separate preset-order change and artifact evidence.
