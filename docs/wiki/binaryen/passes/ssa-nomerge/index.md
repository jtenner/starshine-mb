...
- The crucial no-merge decision is made **per set**, not per original local slot.
  ...
  - Starshine now consumes that LocalGraph decision table for straight-line raw `local.set` and `local.tee`, and for simple missing-else one-arm entry/default merge preservation; broader structured-control mutation is intentionally still a later SSANM slice
