...
- `agent-todo.md` has a dedicated `[O4Z-AUDIT-SSA]` / `SSANM-*` backlog for the reopened audit. The direct explicit pass has strong normalized compare evidence; `[SSANM-003a]` / `[SSANM-003b]` now make the raw straight-line `local.set` and `local.tee` subsets consume the LocalGraph rewrite plan; and `[SSANM-004a]` now uses planned default-entry get materialization for straight-line defaults plus a narrow normal structured-control subset. However broader structured control, merge preservation, huge-function classification, debug-WASI artifact classification, and the currently no-op O4z slot decision remain open.
...
- The crucial no-merge decision is made **per set**, not per original local slot.
  ...
  - Starshine now consumes that LocalGraph decision table for straight-line raw `local.set` and `local.tee`; broader structured-control mutation is intentionally still a later SSANM slice
- Reads with no explicit reaching set can be rewritten to explicit default values.
  ...
  - Starshine now consumes planned default-entry get decisions for straight-line functions and for a narrow normal structured-control subset with body-default reads and other writes
