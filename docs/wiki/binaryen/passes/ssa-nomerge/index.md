...
- The 2026-06-09 audit found and fixed a true semantic corruption family: HOT `ssa-nomerge` ignored exceptional edges, so a `try_table` body local write followed by `throw` to a catch target could be dropped while a later `local.get` read the default value. Starshine now skips exceptional-flow HOT mutation for this pass until local SSA grows exceptional-edge semantics. `[SSANM-007a1]` / `[SSANM-007a2a]` refreshed that boundary on 2026-06-14: real throwing `try_table` catch and catch-all exits preserve their local writes and `throw` but stay off ordinary structured LocalGraph trace reasons. `[SSANM-007a2b]` / `[SSANM-007a2c]` then locked direct HOT `throw_ref`, legacy catch/rethrow, and delegate terminators as no-mutation boundaries and closed the throwing EH fail-closed subtrack. No-throw `try_table` subsets remain a separate `[SSANM-007a3]` classification task.
