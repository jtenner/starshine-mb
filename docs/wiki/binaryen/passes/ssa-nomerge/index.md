...
- The crucial no-merge decision is made **per set**, not per original local slot.
  ...
  - Starshine now consumes that LocalGraph decision table for straight-line raw `local.set` and `local.tee`, for simple missing-else one-arm entry/default merge preservation, and for simple both-arm / block-`br_if` multi-source merge preservation; loop/backedge, branch-table, and broader structured-control mutation are intentionally still later SSANM slices
