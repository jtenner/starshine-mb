...
- Dropped-local-increment debris no longer triggers a whole-function HOT fail-closed skip. HOT SSA rewrite preserves reaching locals for the exact `drop(i32.add(local.get, i32.const 1))` shape and `local.tee` variants on reduced fixtures; reversed operands and non-`1` constants remain validated boundary shapes. Broader integer widths remain unclaimed until tested.
