...
- 2026-06-10 simple nested `drop(if (result i32) ...)` support: `moon test --package jtenner/starshine/passes --file ssa_nomerge_test.mbt` and `moon test --package jtenner/starshine/ir --file ssa_local_test.mbt` / `cfg_test.mbt` add positive fixtures A–D plus boundary fail-closed regressions E–H; targeted GenValid `ssa-nomerge-parity` lane stayed green at `1000/1000` (`0x5eed`, `.tmp/pass-fuzz-ssa-nomerge-parity-after-simple-drop-if-1000-v2`).
...
- A tiny nested-CFG subset is now supported on the HOT path: straight-line `drop(block (result t) ...)` value blocks with only local get/set/tee, constants, and simple numeric ops (`ssa_simple_drop_value_block_allowed(...)` plus inline rename in `ssa_local_visit_node`), and straight-line `drop(if (result i32) ...)` with both arms present, no nested control in the condition/arms, and no local writes in the condition (`ssa_simple_drop_result_if_allowed(...)` plus CFG-backed rename with per-root segment isolation and join phis on the post-`drop` continuation). `ssa_nomerge_has_nested_cfg_sensitive_control(...)` still fail-closes all other nested value-control shapes.
