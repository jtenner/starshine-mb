...
- A tiny nested-CFG subset is now supported on the HOT path: straight-line `drop(block (result t) ...)` value blocks with only local get/set/tee, constants, and simple numeric ops. `ssa_nomerge_has_nested_cfg_sensitive_control(...)` still fail-closes all other nested value-control shapes, and `ssa_simple_drop_value_block_allowed(...)` plus inline rename in `ssa_local_visit_node` model only that operand-nested block body.
- Nested CFG-sensitive behavior remains fail-closed for HOT SSA mutation beyond the supported subset and is regression-covered: nested result-typed `if` inside dropped value blocks, nested `br` terminators, nested `br_if`, nested `loop`, and broader nested control must not produce `pass[ssa-nomerge]:mutated` on the hot path and must leave the hot-path revision unchanged. Root-level value blocks and structured control bodies whose children are region-root slots remain outside this guard and may still rewrite through HOT SSA or the raw no-write path.
