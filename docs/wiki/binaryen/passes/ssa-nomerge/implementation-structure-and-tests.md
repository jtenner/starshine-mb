...
Agent classification: `defined=651 abs=678` was a size-losing / parity-risky typed result-loop carrier boundary around plain `br`, not a proven Starshine win. Broader typed-control helper policy remains unchanged; branchy/reference/table/cast typed-loop families stay owned by the existing `[SSANM-007b*]` typed-control boundary/helper tests. The new `defined=987 abs=1014` first diff is open under `[SSANM-009b7]` and still needs reduction/classification.
