...
| `src/passes/pass_manager.mbt:6826` | Straight-line raw alias rewrite path. `[SSANM-003a]` makes the no-`local.tee` `local.set` subset build and consume `SsaNoMergeRewritePlan`; `local.tee` still uses the legacy heuristic until `[SSANM-003b]`. |
...
| `src/passes/ssa_nomerge_test.mbt:48` | Straight-line local traffic can stay canonical, and `[SSANM-003a]` public-pipeline trace/output fixtures prove the raw `local.set` subset consumes the LocalGraph plan for live parameter overwrites and overwritten body-local writes. |
