...
`[SSANM-009b116]` continues the current-HEAD chain and fixes the first non-winning implementation gap. Targeted `abs=1495`, `1497`, and `1498` are masked-local equal Starshine local-count/body-size wins; `abs=1496`, `1499`, and `1500` match exactly. `defined=1474 abs=1501` reduced to an already-SSA nested compare-chain `local.tee`: Binaryen keeps the existing body local `1`, while Starshine freshened it to appended local `2`, adding an extra i32 local without a semantic or size win. The red-first test `ssa-nomerge leaves already SSA nested compare tee canonical` locks that shape. The raw dispatcher now recognizes only the narrow branch-free, no-body-`local.set`, single body-local `local.tee` chain whose body-local reads all target that tee and returns the original function with trace reason `structured-already-ssa-tee-noop`. Evidence: focused and full tests passed as recorded in the parity page; direct `ssa-nomerge` compare requested `10000` and found `0` mismatches; the debug-WASI replay artifacts validate, and targeted `abs=1501` is now exact with both bodies at `183` total bytes / `180` instruction bytes / one body i32 local. This is an ordinary raw-path canonical-preservation fix, not an O4z scheduling decision; `[SSANM-010c]` stays deferred and `o4z-ssa-nomerge-noop` remains in place.

`[SSANM-009b117]` continues through the next local-count-losing structured-load scratch candidate. `abs=1521..1524` are masked-local equal Starshine local-count wins, `abs=1525` is exact, and `abs=1526` is neutral same-count drift. Before the fix, `defined=1500 abs=1527` was byte-neutral but local-count-losing (`12` Starshine i32 locals versus Binaryen `11`). The reduced test `ssa-nomerge reuses unused body local for medium structured load scratch` widens the earlier structured-load scratch fixture just above the 8-10-local bound. The implementation keeps the widening narrow: 11-12-local compaction requires a plain load, excludes `br_table` bodies, and re-runs existing one-use scratch compaction after raw structured rewriting. In the debug-WASI replay artifacts, targeted `abs=1527` is now a Starshine local-count win with the same encoded body size (`142` bytes / `139` instruction bytes) and fewer body i32 locals (`8` versus Binaryen `11`).

`[SSANM-009b118]` reduces the two remaining post-`[SSANM-009b117]` local-count-losing debug-WASI candidates. The new tests `ssa-nomerge reuses unused body locals for branch dead tees` and `ssa-nomerge reuses unused body local for nested compare tee chain` cover the two owners. `pass_manager.mbt` now lets the scratch compactor handle no-`local.set`, small loaded nested-compare `local.tee` chains and loaded branch/call loops with dead branch-value tees, while continuing to exclude `br_table` cases. Raw `ssa-nomerge` code rewrites also drop local-name subsections so trimmed local declarations do not leave stale debug-name indices. Targeted replay `.tmp/self-ssa-nomerge-ssanm009b118-local-compaction4-20260615` validates both Starshine and Binaryen artifacts: `defined=1506 abs=1533` is fixed as a canonical Starshine local-count win (`3` body i32 locals versus Binaryen `4`, equal `339` body bytes / `336` instruction bytes). `defined=1501 abs=1528` is classified as a post-`wasm-opt --strip-debug` representation artifact: Starshine raw output is smaller and has fewer locals (`458` body bytes / `455` instruction bytes / `17` body i32 locals) than Binaryen raw (`467` / `464` / `19`), but Binaryen canonicalization of Starshine's raw output expands that function back to `467` / `464` / `20`. Evidence: focused tests passed `444/444`, `moon test src/passes` passed `2476/2476`, `moon info`, full `moon test` passed `5781/5781`, native build completed with pre-existing warnings, and direct compare `.tmp/pass-fuzz-ssa-nomerge-ssanm009b118-local-compaction-10000` compared `7600/10000` with `0` mismatches and `20` cached Binaryen command failures.
...
## `[SSANM-009b30a]` structured-load scratch compaction

`[SSANM-009b30a]` implements the reduced form of the `abs=598` / `abs=705` local-count-losing family. The red-first fixture `ssa-nomerge reuses unused body local for structured load scratch` reduces the `defined=678 abs=705` shape to a one-parameter structured function with an offset-8 memory load stored into a one-set/one-get scratch local between calls, plus dead one-use tee targets. Before the implementation the public pipeline appended scratch locals and failed the `<= 9` body-local budget; after the implementation it reuses unused same-typed body locals and trims unused trailing declarations.

The implementation is intentionally narrow in `src/passes/pass_manager.mbt`: for the small 8-10 body-local structured-load family, Starshine counts local refs after structured scratch freshening, remaps eligible one-set/one-get scratch locals and one-dead-tee targets to unused same-typed body locals, compacts remaining appended locals, and trims trailing unused declarations when no appended locals remain. The same helper is available to the call-heavy structured no-op boundary so a boundary shape can still remove a trailing one-use scratch declaration without admitting broader LocalGraph mutation. Existing allocator, branch/table, typed-control, and artifact local-budget guards remain green; broader exact Binaryen-local allocation remains out of scope unless a future slice proves a measured Starshine win.
...
## `[SSANM-009b94]` repeated one-shot temp-local classification at `abs=669`

`[SSANM-009b94]` follows the current self-artifact pair to `defined=642 abs=669`. Targeted `--print-func 669` extraction under `.tmp/ssanm009b93-next669` shows identical canonical `body_raw` token streams once local indexes are masked, with `81` local operations on both sides (`54` `local.get`, `16` `local.set`, and `11` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The only local-token differences are two one-use dead `local.tee` targets. Starshine reuses locals `11` and `26`, while Binaryen allocates fresh one-use locals `29` and `30`. Local-use analysis shows every differing local appears exactly once as a same-typed `local.tee` target on its side.

Measured deltas: defined function `642` encodes to the same `365` byte code body in both artifacts (`362` instruction bytes plus `3` local-declaration bytes). Starshine declares `27` body i32 locals versus Binaryen `29`, so this is local-count reducing and function-body byte-size neutral. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `669` reports `call-heavy-memory-structured-noop`, so this remains boundary/no-op representation classification rather than a new parity gap. Agent classification: semantic-safe Starshine local-count win / no-regression representation difference, not an implementation gap.

## `[SSANM-009b95]` repeated one-shot temp-local classification at `abs=671`

`[SSANM-009b95]` skips targeted exact `defined=643 abs=670`, then follows the current self-artifact pair to `defined=644 abs=671`. Targeted `--print-func 671` extraction under `.tmp/ssanm009b94-next671` shows identical canonical `body_raw` token streams once local indexes are masked, with `122` local operations on both sides (`73` `local.get`, `15` `local.set`, and `34` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The only local-token differences are 12 one-use dead `local.tee` targets. Starshine reuses locals `12`, `13`, `26`, `8`, `9`, `25`, `31`, `22`, `23`, `24`, `28`, and `27`, while Binaryen allocates fresh one-use locals `49..60`. Local-use analysis shows every differing local appears exactly once as a same-typed `local.tee` target on its side.

Measured deltas: defined function `644` encodes to the same `574` byte code body in both artifacts (`571` instruction bytes plus `3` local-declaration bytes). Starshine declares `47` body i32 locals versus Binaryen `59`, so this is local-count reducing and function-body byte-size neutral. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `671` reports `call-heavy-memory-structured-noop`, so this remains boundary/no-op representation classification rather than a new parity gap. Agent classification: semantic-safe Starshine local-count win / no-regression representation difference, not an implementation gap.

## `[SSANM-009b96]` repeated one-shot temp-local classification at `abs=672`

`[SSANM-009b96]` follows the current self-artifact pair to `defined=645 abs=672`. Targeted `--print-func 672` extraction under `.tmp/ssanm009b94-next672` shows identical canonical `body_raw` token streams once local indexes are masked, with `158` local operations on both sides (`103` `local.get`, `21` `local.set`, and `34` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The only local-token differences are 12 one-use dead `local.tee` targets. Starshine reuses locals `33`, `6`, `7`, `8`, `22`, `18`, `50`, `20`, `21`, `24`, `23`, and `26`, while Binaryen allocates fresh one-use locals `56..67`. Local-use analysis shows every differing local appears exactly once as a same-typed `local.tee` target on its side.

Measured deltas: defined function `645` encodes to the same `736` byte code body in both artifacts (`733` instruction bytes plus `3` local-declaration bytes). Starshine declares `54` body i32 locals versus Binaryen `66`, so this is local-count reducing and function-body byte-size neutral. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `672` reports `call-heavy-memory-structured-noop`, so this remains boundary/no-op representation classification rather than a new parity gap. Agent classification: semantic-safe Starshine local-count win / no-regression representation difference, not an implementation gap.

## `[SSANM-009b97]` local-slot permutation classification at `abs=673`

`[SSANM-009b97]` follows the current self-artifact pair to `defined=646 abs=673`. Targeted `--print-func 673` extraction under `.tmp/ssanm009b94-next673` shows identical canonical `body_raw` token streams once local indexes are masked, with `95` local operations on both sides (`58` `local.get`, `10` `local.set`, and `27` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The local-token differences are a same-typed permutation among nine one-use `local.tee` targets plus two paired set/get slots over body i32 locals `35..45`. Local-use analysis shows every differing local is same-typed and either one-use or paired on one local slot.

Measured deltas: defined function `646` encodes to the same `465` byte code body in both artifacts (`462` instruction bytes plus `3` local-declaration bytes), and both declare `45` body i32 locals. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `673` reports `call-heavy-memory-structured-noop`, so this remains boundary/no-op representation classification rather than a new parity gap. Agent classification: semantic-safe neutral local-slot drift, not an implementation gap.

## `[SSANM-009b98]` repeated one-shot temp-local classification at `abs=674`

`[SSANM-009b98]` follows the current self-artifact pair to `defined=647 abs=674`. Targeted `--print-func 674` extraction under `.tmp/ssanm009b94-next674` shows identical canonical `body_raw` token streams once local indexes are masked, with `134` local operations on both sides (`89` `local.get`, `21` `local.set`, and `24` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The only local-token differences are 13 one-use dead `local.tee` targets. Starshine reuses locals `12`, `32`, `14`, `15`, `35`, `17`, `18`, `39`, `38`, `20`, `21`, `42`, and `25`, while Binaryen allocates fresh one-use locals `47..59`. Local-use analysis shows every differing local appears exactly once as a same-typed `local.tee` target on its side.

Measured deltas: defined function `647` encodes to the same `557` byte code body in both artifacts (`554` instruction bytes plus `3` local-declaration bytes). Starshine declares `43` body i32 locals versus Binaryen `56`, so this is local-count reducing and function-body byte-size neutral. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `674` reports `structured-local-writes`, so this remains representation classification rather than a new parity gap. Agent classification: semantic-safe Starshine local-count win / no-regression representation difference, not an implementation gap.

## `[SSANM-009b99]` repeated one-shot temp-local classification at `abs=676`

`[SSANM-009b99]` skips targeted exact `defined=648 abs=675`, then follows the current self-artifact pair to `defined=649 abs=676`. Targeted `--print-func 676` extraction under `.tmp/ssanm009b98-next676` shows identical canonical `body_raw` token streams once local indexes are masked, with `179` local operations on both sides (`118` `local.get`, `24` `local.set`, and `37` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The only local-token differences are 13 one-use dead `local.tee` targets. Starshine reuses locals `11`, `13`, `14`, `16`, `17`, `26`, `27`, `28`, `33`, `35`, `36`, `40`, and `43`, while Binaryen allocates fresh one-use locals `62..74`. Local-use analysis shows every differing local appears exactly once as a same-typed `local.tee` target on its side.

Measured deltas: defined function `649` encodes to the same `805` byte code body in both artifacts (`802` instruction bytes plus `3` local-declaration bytes). Starshine declares `60` body i32 locals versus Binaryen `73`, so this is local-count reducing and function-body byte-size neutral. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `676` reports `structured-local-writes`, so this remains representation classification rather than a new parity gap. Agent classification: semantic-safe Starshine local-count win / no-regression representation difference, not an implementation gap.

## `[SSANM-009b100]` repeated one-shot temp-local classification at `abs=677`

`[SSANM-009b100]` follows the current self-artifact pair to `defined=650 abs=677`. Targeted `--print-func 677` extraction under `.tmp/ssanm009b99-next677` shows identical canonical `body_raw` token streams once local indexes are masked, with `312` local operations on both sides (`226` `local.get`, `47` `local.set`, and `39` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The only local-token differences are 23 one-use dead `local.tee` targets. Starshine reuses locals `23`, `25`, `27`, `29`, `30`, `32`, `33`, `35`, `36`, `40`, `44`, `54`, `58`, `61`, `63`, `64`, `65`, `68`, `74`, `77`, `80`, `81`, and `84`, while Binaryen allocates fresh one-use locals `93..115`. Local-use analysis shows every differing local appears exactly once as a same-typed `local.tee` target on its side.

Measured deltas: defined function `650` encodes to the same `1326` byte code body in both artifacts (`1323` instruction bytes plus `3` local-declaration bytes). Starshine declares `84` body i32 locals versus Binaryen `107`, so this is local-count reducing and function-body byte-size neutral. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `677` reports `call-heavy-memory-structured-noop`, so this remains boundary/no-op representation classification rather than a new parity gap. Agent classification: semantic-safe Starshine local-count win / no-regression representation difference, not an implementation gap.

## `[SSANM-009b101]` repeated one-shot temp-local classification at `abs=678`

`[SSANM-009b101]` follows the current self-artifact pair to `defined=651 abs=678`. Targeted `--print-func 678` extraction under `.tmp/ssanm009b99-next678` shows identical canonical `body_raw` token streams once local indexes are masked, with `33` local operations on both sides (`19` `local.get`, `6` `local.set`, and `8` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The only local-token differences are five one-use dead `local.tee` targets. Starshine reuses locals `4`, `5`, `6`, `7`, and `10`, while Binaryen allocates fresh one-use locals `14..18`. Local-use analysis shows every differing local appears exactly once as a same-typed `local.tee` target on its side.

Measured deltas: defined function `651` encodes to the same `173` byte code body in both artifacts (`170` instruction bytes plus `3` local-declaration bytes). Starshine declares `13` body i32 locals versus Binaryen `18`, so this is local-count reducing and function-body byte-size neutral. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `678` reports `call-heavy-memory-structured-noop`, so this remains boundary/no-op representation classification rather than a new parity gap. Agent classification: semantic-safe Starshine local-count win / no-regression representation difference, not an implementation gap.

## `[SSANM-009b102]` local-slot permutation classification at `abs=679`

`[SSANM-009b102]` follows the current self-artifact pair to `defined=652 abs=679`. Targeted `--print-func 679` extraction under `.tmp/ssanm009b99-next679` shows identical canonical `body_raw` token streams once local indexes are masked, with `98` local operations on both sides (`60` `local.get`, `20` `local.set`, and `18` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The local-token differences are a same-typed permutation among six one-use `local.tee` targets plus two set/get/set slots over body i32 locals `35..42`. Local-use analysis shows every differing local is same-typed and either one-use or paired on one local slot.

Measured deltas: defined function `652` encodes to the same `455` byte code body in both artifacts (`452` instruction bytes plus `3` local-declaration bytes), and both declare `43` body i32 locals. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `679` reports `call-heavy-memory-structured-noop`, so this remains boundary/no-op representation classification rather than a new parity gap. Agent classification: semantic-safe neutral local-slot drift, not an implementation gap.

## `[SSANM-009b103]` local-slot permutation classification at `abs=680`

`[SSANM-009b103]` follows the current self-artifact pair to `defined=653 abs=680`. Targeted `--print-func 680` extraction under `.tmp/ssanm009b99-next680` shows identical canonical `body_raw` token streams once local indexes are masked, with `84` local operations on both sides (`50` `local.get`, `15` `local.set`, and `19` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The local-token differences are a same-typed permutation among 11 one-use `local.tee` targets plus one paired set/get slot over body i32 locals `32..43`. Local-use analysis shows every differing local is same-typed and either one-use or paired on one local slot.

Measured deltas: defined function `653` encodes to the same `395` byte code body in both artifacts (`392` instruction bytes plus `3` local-declaration bytes), and both declare `42` body i32 locals. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `680` reports `structured-local-writes`, so this remains representation classification rather than a new parity gap. Agent classification: semantic-safe neutral local-slot drift, not an implementation gap.

## `[SSANM-009b104]` local-slot permutation classification at `abs=682`

`[SSANM-009b104]` skips targeted exact `defined=654 abs=681`, then follows the current self-artifact pair to `defined=655 abs=682`. Targeted `--print-func 682` extraction under `.tmp/ssanm009b99-next682` shows identical canonical `body_raw` token streams once local indexes are masked, with `64` local operations on both sides (`36` `local.get`, `13` `local.set`, and `15` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The local-token differences are a same-typed permutation among three one-use `local.tee` targets, one set/get/set slot, and one set/get slot over body i32 and i64 locals. Local-use analysis shows every differing local is same-typed and either one-use or paired on one local slot.

Measured deltas: defined function `655` encodes to the same `335` byte code body in both artifacts (`330` instruction bytes plus `5` local-declaration bytes), and both declare `(33 i32, 4 i64)` body locals. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `682` reports `call-heavy-memory-structured-noop`, so this remains boundary/no-op representation classification rather than a new parity gap. Agent classification: semantic-safe neutral local-slot drift, not an implementation gap.

## `[SSANM-009b105]` one-shot temp-local classification at `abs=684`

`[SSANM-009b105]` skips targeted exact `defined=656 abs=683`, then follows the current self-artifact pair to `defined=657 abs=684`. Targeted `--print-func 684` extraction under `.tmp/ssanm009b104-next684` shows identical canonical `body_raw` token streams once local indexes are masked, with `150` local operations on both sides (`98` `local.get`, `27` `local.set`, and `25` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The local-token differences are 16 one-use dead `local.tee` targets: Starshine reuses same-typed body i32 locals `21`, `22`, `24`, `25`, `26`, `28`, `29`, `31`, `32`, `33`, `34`, `35`, `36`, `39`, `41`, and `43`, while Binaryen allocates fresh one-use locals `53..68`. Local-use analysis shows every differing local appears exactly once as a same-typed `local.tee` target on its side.

Measured deltas: defined function `657` encodes to the same `742` byte code body in both artifacts (`739` instruction bytes plus `3` local-declaration bytes), but Starshine declares `50` body i32 locals versus Binaryen `66`. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `684` reports `structured-local-writes`, so this remains representation classification rather than a new parity gap. Agent classification: semantic-safe Starshine local-count win / no-regression representation difference, not an implementation gap.

## `[SSANM-009b106]` one-shot temp-local classification at `abs=689`

`[SSANM-009b106]` skips targeted exact `defined=658..661 abs=685..688`, then follows the current self-artifact pair to `defined=662 abs=689`. Targeted `--print-func 689` extraction under `.tmp/ssanm009b105-next689` shows identical canonical `body_raw` token streams once local indexes are masked, with `1233` local operations on both sides (`724` `local.get`, `287` `local.set`, and `222` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The local-token differences are 106 one-use dead `local.tee` targets. Starshine reuses lower same-typed i32 locals, while Binaryen allocates fresh one-use locals `490..595`. Local-use analysis shows every differing local appears exactly once as a same-typed `local.tee` target on its side.

Measured deltas: defined function `662` is smaller in Starshine (`6141` bytes: `6137` instruction bytes plus `4` local-declaration bytes) than Binaryen (`6163` bytes: `6159` instruction bytes plus `4` local-declaration bytes), and Starshine declares `489` body i32 locals versus Binaryen `595`. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `689` reports `structured-local-writes`, so this remains representation classification rather than a new parity gap. Agent classification: semantic-safe Starshine local-count and encoded-body-size win, not an implementation gap.

## `[SSANM-009b107]` one-shot temp-local classification at `abs=690`

`[SSANM-009b107]` follows the current self-artifact pair to `defined=663 abs=690`. Targeted `--print-func 690` extraction under `.tmp/ssanm009b105-next690` shows identical canonical `body_raw` token streams once local indexes are masked, with `52` local operations on both sides (`33` `local.get`, `7` `local.set`, and `12` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The local-token differences are seven one-use dead `local.tee` targets: Starshine reuses same-typed body i32 locals `8`, `10`, `11`, `12`, `13`, `14`, and `15`, while Binaryen allocates fresh one-use locals `19..25`. Local-use analysis shows every differing local appears exactly once as a same-typed `local.tee` target on its side.

Measured deltas: defined function `663` encodes to the same `295` byte code body in both artifacts (`292` instruction bytes plus `3` local-declaration bytes), but Starshine declares `18` body i32 locals versus Binaryen `25`. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `690` reports `call-heavy-memory-structured-noop`, so this remains boundary/no-op representation classification rather than a new parity gap. Agent classification: semantic-safe Starshine local-count win / no-regression representation difference, not an implementation gap.

## `[SSANM-009b108]` one-shot temp-local classification at `abs=691`

`[SSANM-009b108]` follows the current self-artifact pair to `defined=664 abs=691`. Targeted `--print-func 691` extraction under `.tmp/ssanm009b107-next691` shows identical canonical `body_raw` token streams once local indexes are masked, with `342` local operations on both sides (`218` `local.get`, `84` `local.set`, and `40` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The local-token differences are 17 one-use dead `local.tee` targets: Starshine reuses same-typed body i32 locals `18`, `19`, `29`, `33`, `38`, `51`, `53`, `55`, `64`, `65`, `66`, `67`, `68`, `72`, `75`, `78`, and `81`, while Binaryen allocates fresh one-use locals `126..142`. Local-use analysis shows every differing local appears exactly once as a same-typed `local.tee` target on its side.

Measured deltas: defined function `664` is smaller in Starshine (`1333` bytes: `1330` instruction bytes plus `3` local-declaration bytes) than Binaryen (`1349` bytes: `1345` instruction bytes plus `4` local-declaration bytes), and Starshine declares `124` body i32 locals versus Binaryen `141`. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `691` reports `call-heavy-memory-structured-noop`, so this remains boundary/no-op representation classification rather than a new parity gap. Agent classification: semantic-safe Starshine local-count and encoded-body-size win, not an implementation gap.

## `[SSANM-009b109]` / `[SSANM-009b110]` neutral local-slot classifications at `abs=696..697`

`[SSANM-009b109]` skips targeted exact `defined=665..668 abs=692..695`, then follows the current self-artifact pair to `defined=669 abs=696`. Targeted `--print-func 696` extraction under `.tmp/ssanm009b108-next696` shows identical canonical `body_raw` token streams once local indexes are masked, with `26` local operations on both sides (`17` `local.get`, `7` `local.set`, and `2` `local.tee`). The local-token differences are a same-typed permutation over body i32 locals `10` and `11`: one paired set/get slot and one one-use `local.tee` target swap sides. Defined function `669` encodes to the same `101` byte code body in both artifacts (`98` instruction bytes plus `3` local-declaration bytes), and both declare `10` body i32 locals. The Starshine trace for abs `696` reports `call-heavy-memory-structured-noop`.

`[SSANM-009b110]` follows the current self-artifact pair to `defined=670 abs=697`. Targeted `--print-func 697` extraction under `.tmp/ssanm009b108-next697` shows identical canonical `body_raw` token streams once local indexes are masked, with `25` local operations on both sides (`16` `local.get`, `7` `local.set`, and `2` `local.tee`). The local-token differences are a same-typed permutation over body i32 locals `9..11`: two paired set/get slots and one one-use `local.tee` target rotate across those slots. Defined function `670` encodes to the same `97` byte code body in both artifacts (`94` instruction bytes plus `3` local-declaration bytes), and both declare `10` body i32 locals. The Starshine trace for abs `697` reports `structured-local-writes`.

For both functions, no call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs once local indexes are masked. Local-use analysis shows every differing local is same-typed and either one-use or paired on one local slot. Both full artifacts validate with `wasm-tools validate --features all`. Agent classification: semantic-safe neutral local-slot drift, not an implementation gap. Targeted `defined=671..675 abs=698..702` outputs match exactly; these were followed by `[SSANM-009b111]` and `[SSANM-009b112]`.

## `[SSANM-009b111]` one-shot temp-local classification at `abs=703`

`[SSANM-009b111]` skips targeted exact `defined=671..675 abs=698..702`, then follows the current self-artifact pair to `defined=676 abs=703`. Targeted `--print-func 703` extraction under `.tmp/ssanm009b111-next703` shows identical canonical `body_raw` token streams once local indexes are masked, with `384` local operations on both sides (`212` `local.get`, `70` `local.set`, and `102` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The local-token differences are 50 one-use dead `local.tee` targets: Starshine reuses same-typed body i32 locals `14`, `15`, `20`, `21`, `27`, `29`, `31`, `35`, `37`, `39`, `43`, `45`, `47`, `51`, `57`, `59`, `61`, `63`, `65`, `67`, `69`, `71`, `73`, `75`, `77`, `79`, `81`, `83`, `85`, `87`, `89`, `91`, `93`, `95`, `97`, `99`, `101`, `103`, `105`, `107`, `109`, `111`, `113`, `115`, `123`, `124`, `126`, `127`, `128`, and `129`, while Binaryen allocates fresh one-use locals `139..188`. Local-use analysis shows every differing local appears exactly once as a same-typed `local.tee` target on its side.

Measured deltas: defined function `676` is smaller in Starshine (`1706` bytes: `1702` instruction bytes plus `4` local-declaration bytes) than Binaryen (`1754` bytes: `1750` instruction bytes plus `4` local-declaration bytes), and Starshine declares `137` body i32 locals versus Binaryen `187`. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `703` reports `call-heavy-memory-structured-noop`, so this remains boundary/no-op representation classification rather than a new parity gap. Agent classification: semantic-safe Starshine local-count and encoded-body-size win, not an implementation gap.

## `[SSANM-009b112]` local-count-losing classification at `abs=705`

`[SSANM-009b112]` skips targeted exact `defined=677 abs=704`, then follows the current self-artifact pair to `defined=678 abs=705`. Targeted `--print-func 705` extraction under `.tmp/ssanm009b111-next705` shows identical canonical `body_raw` token streams once local indexes are masked, with `12` local operations on both sides (`6` `local.get`, `2` `local.set`, and `4` `local.tee`). No call, load/store memarg, branch/table opcode, label target, constant, result type, or structured-control token differs.

The local-token differences are same-typed local-slot drift over three one-use `local.tee` targets plus one paired set/get slot. Starshine uses locals `6`, `7`, and `8` as one-use tee targets and appends paired set/get local `10`; Binaryen uses locals `7`, `8`, and `9` as the one-use tee targets and reuses paired set/get local `6`. Local-use analysis shows the one-use tee targets are dead one-shot carriers, and the paired set/get slot is self-contained on one same-typed local.

Measured deltas: defined function `678` encodes to the same `57` byte code body in both artifacts (`54` instruction bytes plus `3` local-declaration bytes), but Starshine declares `10` body i32 locals versus Binaryen `9`. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `705` reports `call-heavy-memory-structured-noop`. Agent classification at classification time: body-size-neutral but Starshine-local-count-losing parity gap; because there was no measured Starshine win, it was split to `[SSANM-009b30a]` rather than accepted as representation drift. `[SSANM-009b30a]` now adds the reduced structured-load scratch compaction guard and direct compare evidence described above.
