...
Measured deltas: defined function `678` encodes to the same `57` byte code body in both artifacts (`54` instruction bytes plus `3` local-declaration bytes), but Starshine declares `10` body i32 locals versus Binaryen `9`. Both full artifacts validate with `wasm-tools validate --features all`. The Starshine trace for abs `705` reports `call-heavy-memory-structured-noop`. Agent classification: body-size-neutral but Starshine-local-count-losing parity gap; because there is no measured Starshine win, this is kept open under `[SSANM-009b30a]` rather than accepted as representation drift.
