...
Agent classification: `defined=108 abs=135` was a true straight-line behavior-parity gap and is fixed. The new `defined=215 abs=242` first diff is open under `[SSANM-009b2]`; its current evidence points at a large structured branch/value-carrier artifact family rather than a huge-function guard.
