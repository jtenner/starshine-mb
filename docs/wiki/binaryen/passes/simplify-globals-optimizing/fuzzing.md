...
- 2026-08-12 final SimplifyLocals live-out repair refresh: native SHA-256 `443fa73acbe3789b0e1b330fdf28652fe5f567c4e6df53470e46217b65b92d47` ran `.tmp/pass-fuzz-sgo-final-simplifylocals-liveout-final-443fa73-dedicated-10000-v131-20260812` with the six-leaf aggregate, seed `0x5eed`, 16 workers, explicit verified Binaryen-v131 binary, and mismatch reduction disabled. It compared `10000/10000`: `5055` normalized matches and `4945` residuals, with zero validation, property, generator, or command failures and Binaryen cache `10000/0`. Agent inspection classifies every residual as a smaller Starshine shape: nested-cleanup `1427` and read-only-to-write `2127` are one byte smaller, while initializer-folding `1391` is three bytes smaller because Starshine retains the smaller initializer alias; aggregate `-7727` raw/canonical bytes. All six leaves were sampled: same-init/dead-set `1404`, runtime `2206`, startup `1445`, nested cleanup `1427`, initializer folding `1391`, and read-only-to-write `2127`. The lane refreshes SGO evidence after the structured-child live-out repair and retained late SimplifyLocals suffix; the harness itself reports differences, not semantic classifications.
- 2026-08-12 final merge-blocks suffix refresh: native SHA-256 `15804fd785eada79e95fcfc783cc026c5bab86f71fa80e24d1c176a923e7c86e` ran `.tmp/pass-fuzz-sgo-final-mergeblocks-fixed-dedicated-10000-20260812` with the six-leaf aggregate, seed `0x5eed`, 16 workers, explicit verified Binaryen-v131 binary, and mismatch reduction disabled. It compared `10000/10000`: `5055` normalized matches and `4945` residuals, with zero validation, property, generator, or command failures; Binaryen cache was `9984/16`. Agent inspection classifies every residual as a smaller Starshine shape: `3554` one-byte removals of Binaryen-retained `nop`, plus `1391` three-byte cases where Starshine retains the smaller `global.get` initializer alias while Binaryen duplicates the source initializer, for aggregate `-7727` raw bytes. All six leaves were sampled: same-init/dead-set `1404`, runtime `2206`, startup `1445`, nested cleanup `1427`, initializer folding `1391`, and read-only-to-write `2127`. The harness reports differences, not semantic classifications.
- 2026-08-12 final precompute/value-block repair refresh: native SHA-256 `1007a0ac0e944ee8406b9886c3bc54bfbff6ff54120887ba936ac6f34109debc` ran `.tmp/pass-fuzz-sgo-final-precompute-valueblock-fix-dedicated-10000-final-20260812` with the six-leaf aggregate, seed `0x5eed`, 16 workers, explicit native binary, and mismatch reduction disabled. It compared `10000/10000`: `5001` normalized matches and `4999` residuals, with zero validation, property, generator, or command failures and Binaryen cache `10000/0`. Agent inspection classifies every residual as the already-reviewed smaller Starshine cleanup family: `3554` one-byte removals of Binaryen-retained `nop`, plus `1445` three-byte startup-offset constant substitutions, for aggregate `-7889` raw bytes. All six leaves were sampled: same-init/dead-set `1404`, runtime `2206`, startup `1445`, nested cleanup `1427`, initializer folding `1391`, and read-only-to-write `2127`. This renews the SGO generated matrix after the structured operand-block SSA repair; the harness itself reports differences, not semantic classifications.
- 2026-08-12 structured-drop refresh: native SHA-256 `f12e5a80657760773d895d29e7b354c364840822d3aa656c5d6b2532874d8801` ran `.tmp/pass-fuzz-sgo-genvalid-all-10000-final-structured-drop-20260812` with the six-leaf `simplify-globals-optimizing-all` aggregate, seed `0x5eed`, 16 workers, and the explicit native binary. It compared `10000/10000`: `5001` exact normalized matches plus `4999` agent-classified cleanup wins, with zero validation, property, generator, or command failures and Binaryen cache `10000/0` hits/misses. The residuals divide exactly into `3554` one-byte wins where Starshine deletes a semantically empty `nop` retained by Binaryen and `1445` three-byte startup-propagation wins where Starshine replaces an immutable `global.get` data offset with its equal constant, for `-7889` aggregate bytes. Common verified-v131 `-Oz --strip-debug --all-features` makes all `4999/4999` pairs byte-identical. The same refresh keeps the prior `1391` initializer-folding size losses eliminated by retaining the smaller `global.get` alias instead of duplicating a multi-instruction source initializer. The harness reports the wins as raw mismatches; semantic/size classification is an agent judgment backed by the inspected uniform shapes, exact size deltas, and common-Oz convergence.
