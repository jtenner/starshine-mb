...
- preserve the neighboring negatives where `global.get $g` is the nested `if` condition and therefore can decide whether the call runs, where the nested-if result becomes an operand to a post-if call, where the global-derived value is stored through `local.tee`, or where a transparent arm block feeds it to a trapping load.
