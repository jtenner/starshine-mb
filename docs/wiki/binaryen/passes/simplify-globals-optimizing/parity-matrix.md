...
- reference-typed nested-if arm results through the nontrapping `ref.is_null` post-consumer, including a transparent result-block wrapper around the same flow,
- preserve the neighboring negatives where `global.get $g` is the nested `if` condition and therefore can decide whether the call runs, where the nested-if result becomes an operand to a post-if call, where a post-if `select` result feeds a trapping load, where a block-wrapped post-if load consumes the global-derived value, where the global-derived value is stored through `local.tee`, where a transparent arm block feeds it to a trapping load, where a clean `if` wrapper result feeds a trapping load, where the candidate read steers an inner if-wrapper branch, or where the `ref.is_null` result feeds a post-consumer call.
