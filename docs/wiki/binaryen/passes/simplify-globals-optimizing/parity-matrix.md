...
- preserve the neighboring negatives where `global.get $g` is the nested `if` condition and therefore can decide whether the call runs, where the nested-if result becomes an operand to a post-if call, where a post-if `select` result feeds a trapping load, where a block-wrapped post-if load consumes the global-derived value, where the global-derived value is stored through `local.tee`, where a transparent arm block feeds it to a trapping load, where a clean `if` wrapper result feeds a trapping load, where the candidate read steers an inner if-wrapper branch, where a nontrapping reference-test/result feeds a post-consumer call, where the reference op may trap such as nullable `i31.get_s`, or where the numeric op may trap such as regular float-to-int truncation.
