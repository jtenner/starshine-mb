...
- preserve the neighboring negatives where `global.get $g` is the nested `if` condition and therefore can decide whether the call runs, or where the nested-if result becomes an operand to a post-if call.
