...
- How much of Binaryen's `FlowScanner` should Starshine model before accepting side-effecting-but-safe condition positives? Starshine now has pure-condition positives, pure `select` positives, a conservative stack/value-flow bridge for independent `local.tee` / `i32.load` plus extra pure ops and nested-if arm-flow positives, including clean arm-local effects, transparent arm value blocks, post-if `select` value uses, supported pure post-if consumers, transparent block wrappers, clean value-producing if-wrappers, and reference `ref.is_null` / `ref.eq` / `ref.i31` / `extern.convert_any` / `any.convert_extern` and nontrapping numeric conversion/division post-consumers, the recursive no-else nested-pattern carveout through the three-layer lit shape, and negatives for values flowing into `local.tee`, post-if call operands, post-if select-to-load consumers, arm-block/if-wrapper trapping loads, nullable `i31.get_s`/`i31.get_u`, regular float-to-int truncations, multiple same-global reads, nested global-steered calls/if-wrappers, reference-test results feeding calls, unsupported select escapes, wrong-target writes, non-constant set operands, `else` arms, extra dropped same-global reads, and trapping loads; the remaining recommendation is to keep broader official safe-side-effect positives as separate value-flow slices.
