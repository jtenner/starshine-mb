...
6. **Optimizing wrapper hardening**
   ...
   - the current guard is now characterized: nested cleanup runs when `touched_count <= 8`, skips as `large-touched-set` when `touched_count > 8`, and skips as `large-module` when the module has more than `100` defined functions; remove or justify these limits only with artifact/perf evidence.
...
| Optimizing wrapper | touched functions receive ordinary cleanup; untouched functions do not; current guard behavior is explicit (`<= 8` touched functions run, `> 8` touched functions skip, `> 100` defined functions skip) |
...
- current scheduler guards: at-or-below threshold runs cleanup, above threshold skips cleanup, and large modules skip cleanup.
