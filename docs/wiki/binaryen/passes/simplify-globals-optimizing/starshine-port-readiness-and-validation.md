...
3. **Complete dead or redundant write repair**
   ...
   - extend the landed simple pure-condition bridge only with focused source-backed non-trapping operators; the current unary, bitwise, shift/rotate, and i64-compare bridge covers `i32.clz` / `i32.ctz` / `i32.popcnt`, `i32.and` / `i32.or` / `i32.xor`, `i32.shl` / `i32.shr_s` / `i32.shr_u` / `i32.rotl` / `i32.rotr`, plus `i64.eqz` and `i64` equality/relational compares;
