...
- `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --simplify-globals-optimizing --out-dir .tmp/sgo-direct-debug-artifact-core-skip-noglobal2 --starshine-bin _build/native/release/build/cmd/cmd.exe` completed direct debug-artifact replay after adding SGO core perf timers and a no-global function-body fast path. It stayed valid but red: canonical wasm/WAT/function compare were unequal and the first difference remained `defined=48 abs=69`, but Starshine size stayed below Binaryen (`2,860,295` bytes versus `2,861,435`) and Starshine output was byte-identical to the prior trace-stats artifact. Removing Binaryen's single explicit `local.set $0 (i32.const 0)` from the first-diff function makes the Starshine and Binaryen function text identical, so this frontier is classified as local/default-init representation drift rather than a known semantic mismatch. Starshine pass runtime remained red and noisy (`343.981ms` versus Binaryen `119.278ms`). A traced direct run in `.tmp/sgo-core-skip-noglobal2-trace.txt` reported `nested-cleanup-stats touched=79 cleaned=78 filtered-large=1`; the new `detail:sgo:rewrite-code` timer dropped to about `15.466ms` from the earlier instrumented `59.254ms`, while `detail:sgo:plan-flags` was about `29.526ms` and `detail:sgo:collect-global-infos` about `13.464ms`.
