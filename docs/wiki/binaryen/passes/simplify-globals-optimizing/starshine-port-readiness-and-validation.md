...
- `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --simplify-globals-optimizing --out-dir .tmp/sgo-direct-debug-artifact-threshold-192-1000 --starshine-bin _build/native/release/build/cmd/cmd.exe` completed direct debug-artifact replay after raising the SGO nested cleanup local threshold to `192` while keeping the `1000` instruction threshold. It stayed valid but red: canonical wasm/WAT/function compare were unequal and the first difference remained `defined=48 abs=69`, but Starshine size was `2,860,295` bytes versus Binaryen `2,861,435` after the previous positive size gap was attributed to local-heavy touched functions `5243`, `5360`, and `5353`. Starshine pass runtime was `368.521ms` versus Binaryen `116.712ms`, and the first diff remains local/default-init representation drift after Starshine removed redundant zero stores more aggressively than Binaryen.
