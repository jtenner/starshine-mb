...
6. **Optimizing wrapper hardening**
   ...
   - the current guard is now characterized: nested cleanup runs when `touched_count <= 8`, skips as `large-touched-set` when `touched_count > 8`, skips as `large-module` when the module has more than `100` defined functions, and skips as `large-touched-function` when a touched function has more than `128` locals or more than `1000` instructions; remove or justify these limits only with artifact/perf evidence.
...
- current scheduler order and guards: the current default cleanup pass list is exact, no `precompute-propagate` prefix appears, at-or-below threshold runs cleanup, above threshold skips cleanup, large modules skip cleanup, and large touched functions skip cleanup.
