...
- `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --simplify-globals-optimizing --out-dir .tmp/sgo-direct-debug-artifact-trace-stats --starshine-bin _build/native/release/build/cmd/cmd.exe` completed direct debug-artifact replay after adding SGO nested cleanup accounting. It stayed valid but red: canonical wasm/WAT/function compare were unequal and the first difference remained `defined=48 abs=69`, but Starshine size stayed below Binaryen (`2,860,295` bytes versus `2,861,435`) after the previous positive size gap was attributed to local-heavy touched functions `5243`, `5360`, and `5353`. Removing Binaryen's single explicit `local.set $0 (i32.const 0)` from the first-diff function makes the Starshine and Binaryen function text identical, so this frontier is classified as local/default-init representation drift rather than a known semantic mismatch. Starshine pass runtime remained red (`361.967ms` versus Binaryen `112.157ms`). A traced direct run in `.tmp/sgo-next-trace.txt` reported `nested-cleanup-stats touched=79 cleaned=78 filtered-large=1`; traced nested cleanup timers accounted for only tens of milliseconds (`lift` about `31.179ms`, `lower` about `13.042ms`, `simplify-locals` about `12.830ms`), leaving most of the `316.940ms` traced SGO pass time in the SGO core / uninstrumented module work rather than the measured nested hot-pass bodies.
