...
3. **Complete dead or redundant write repair**
   ...
   - keep the landed single-const same-as-init write detection, narrow read-only-to-write self-guard detection, and exact/eqz/bidirectional compare/block-wrapped-condition/nested-block-wrapped-condition/block-yielded-condition/block-yielded-condition+set/block-wrapped-set/block-wrapped-condition+set `if return; set` family, including the current `i32.eqz`, bidirectional compare-const, nested block-condition, block-wrapped condition reads including `i32.eqz`, no-op const/drop prefixes before block-wrapped condition reads, transparent `nop` / void-`block` body support, and no-op const/drop prefixes before the write;
...
The earlier `.tmp/pass-fuzz-sgo-10k` baseline found seven mismatches from missing multi-instruction global-initializer folding and over-eager reference-constant replacement in element item expressions; those two families are now covered by focused regressions, and the same-init, exact-copy-chain, adjacent/eqz/bidirectional compare-const/transparent/nested/block-wrapped-condition/no-op-condition-prefix/const-drop-body read-only-to-write, exact/eqz/bidirectional compare/block-wrapped-condition/nested-block-wrapped-condition/block-yielded-condition/block-yielded-condition+set/block-wrapped-set/block-wrapped-condition+set `if return; set`, and straight-line runtime propagation slices preserved the green mixed-generator lane.
