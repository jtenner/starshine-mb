...
- `bun fuzz compare-pass --count 1000 --seed 0x5eed --pass simplify-globals-optimizing --max-failures 20 --keep-going-after-command-failures --out-dir .tmp/pass-fuzz-sgo-nested-pruned-1k` compared `998/1000` mixed-generator modules after pruning the SGO nested cleanup list, matched all `998`, found `0` mismatches, `0` validation failures, and hit `2` Binaryen/tool command failures.
