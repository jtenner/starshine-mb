...
6. **Optimizing wrapper hardening**
   ...
   - the current guard is now artifact-informed: nested cleanup runs above the earlier `8` touched-function and `100` defined-function limits; individually large touched functions (`> 128` locals or `> 1000` instructions) are filtered out, and cleanup skips with `reason=large-touched-function` only when every touched function is filtered; startup-only data-offset and global-initializer rewrites leave the touched-function set empty and do not trigger nested cleanup. The direct debug-artifact compare now runs cleanup on the artifact's small touched functions, but remains red at a representation/local-cleanup frontier and is slower than Binaryen pass-local time, so `[SGO]002` remains active.
...
| Optimizing wrapper | touched functions receive ordinary cleanup; untouched functions do not; touched-function-count and module-function-count cleanup skips are removed; individually large touched functions (`> 128` locals or `> 1000` instructions) are filtered out, and an all-large touched set still skips with `reason=large-touched-function` |
...
- `bun scripts/self-optimize-compare.ts tests/node/dist/starshine-debug-wasi.wasm --simplify-globals-optimizing --out-dir .tmp/sgo-direct-debug-artifact-filtered-rse --starshine-bin _build/native/release/build/cmd/cmd.exe` completed direct debug-artifact replay after relaxing the touched-count/module-count cleanup guards and adding the final nested `redundant-set-elimination` slot. It stayed valid but red: canonical wasm/WAT/function compare were unequal, first difference was `defined=48 abs=69`, Starshine size was `2,863,031` bytes versus Binaryen `2,861,435`, Starshine pass runtime was `316.928ms` versus Binaryen `112.331ms`, and the first diff was local/default-init representation drift after Starshine removed redundant zero stores more aggressively than Binaryen.
