...
3. **Complete dead or redundant write repair**
   ...
   - keep broad side-effecting-but-safe condition positives as later value-flow slices because Binaryen's actual matcher is effect/value-flow based rather than a list of adjacent syntax forms; the landed subset covers pure `select` forms, a conservative local stack/value-flow bridge for independent `local.tee` / `i32.load` plus single-`global.get` pure/select chains and nested-if arm-flow families with independent conditions, clean call operands, arm-local calls, clean arm-local `local.tee` / `i32.load` effects, transparent arm value blocks, post-if `select` value uses, and supported pure post-if consumers, and the recursive no-else nested-pattern carveout through the three-layer lit shape with `else` / extra-dropped-read negatives.
