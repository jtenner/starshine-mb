...
3. **Complete dead or redundant write repair**
   ...
   - keep broad side-effecting-but-safe condition positives as later value-flow slices because Binaryen's actual matcher is effect/value-flow based rather than a list of adjacent syntax forms; the landed subset covers pure `select` forms, a conservative local stack/value-flow bridge for independent `local.tee` / `i32.load` plus single-`global.get` pure/select chains and nested-if arm-flow families with independent conditions, clean call operands, arm-local calls, clean arm-local `local.tee` / `i32.load` effects, transparent arm value blocks, post-if `select` value uses, supported pure post-if consumers, nested transparent block-result wrappers around the supported nested-if post-consumer forms, clean value-producing `if` wrappers around another supported nested-if arm result, and the recursive no-else nested-pattern carveout through the three-layer lit shape with `else` / extra-dropped-read negatives.
...
- How much of Binaryen's `FlowScanner` should Starshine model before accepting side-effecting-but-safe condition positives? Starshine now has pure-condition positives, pure `select` positives, a conservative stack/value-flow bridge for independent `local.tee` / `i32.load` plus extra pure ops and nested-if arm-flow positives, including clean arm-local effects, transparent arm value blocks, post-if `select` value uses, supported pure post-if consumers, transparent block wrappers, and clean value-producing if-wrappers, the recursive no-else nested-pattern carveout through the three-layer lit shape, and negatives for values flowing into `local.tee`, post-if call operands, post-if select-to-load consumers, arm-block/if-wrapper trapping loads, multiple same-global reads, nested global-steered calls/if-wrappers, unsupported select escapes, wrong-target writes, non-constant set operands, `else` arms, extra dropped same-global reads, and trapping loads; the remaining recommendation is to keep broader official safe-side-effect positives as separate value-flow slices.
