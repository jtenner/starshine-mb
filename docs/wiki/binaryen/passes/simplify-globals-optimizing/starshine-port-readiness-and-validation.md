...
6. **Optimizing wrapper hardening**
   ...
   - the current guard is now characterized: nested cleanup runs when `touched_count <= 8`, skips as `large-touched-set` when `touched_count > 8`, runs at exactly `100` defined functions, skips as `large-module` when the module has more than `100` defined functions, and skips as `large-touched-function` when a touched function has more than `128` locals or more than `1000` instructions; startup-only data-offset and global-initializer rewrites leave the touched-function set empty and do not trigger nested cleanup; remove or justify these limits only with artifact/perf evidence.
