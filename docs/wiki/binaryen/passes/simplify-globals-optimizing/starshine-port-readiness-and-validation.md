...
3. **Complete dead or redundant write repair**
   ...
   - extend the landed simple pure-condition bridge only with focused source-backed non-trapping operators; the current bitwise and shift/rotate bridge covers `i32.and` / `i32.or` / `i32.xor` plus `i32.shl` / `i32.shr_s` / `i32.shr_u` / `i32.rotl` / `i32.rotr`;
