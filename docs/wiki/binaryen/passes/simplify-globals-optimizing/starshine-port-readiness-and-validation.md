...
6. **Optimizing wrapper hardening**
   ...
   - the current guard is now characterized: nested cleanup runs when `touched_count <= 8`, skips as `large-touched-set` when `touched_count > 8`, runs at exactly `100` defined functions, reports `large-touched-set` before `large-module` when both apply, skips as `large-module` when the module has more than `100` defined functions, reports `large-module` before `large-touched-function` when both size skips apply, and skips as `large-touched-function` when a touched function has more than `128` locals or more than `1000` instructions; startup-only data-offset and global-initializer rewrites leave the touched-function set empty and do not trigger nested cleanup; remove or justify these limits only with artifact/perf evidence.
...
| Optimizing wrapper | touched functions receive ordinary cleanup; untouched functions do not; current guard behavior is explicit (`<= 8` touched functions run, `> 8` touched functions skip, `> 100` defined functions skip, touched functions with `> 128` locals or `> 1000` instructions skip) |
...
- current scheduler order and guards: the current default cleanup pass list is exact, no `precompute-propagate` prefix appears, function-body rewrites trigger cleanup while startup-only data-offset and global-initializer rewrites do not, at-or-below threshold runs cleanup, above threshold skips cleanup, large modules skip cleanup, and large touched functions skip cleanup.
