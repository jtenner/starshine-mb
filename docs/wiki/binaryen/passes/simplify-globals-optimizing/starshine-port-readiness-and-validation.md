...
3. **Complete dead or redundant write repair**
   ...
   - extend the landed simple pure-condition bridge only with focused source-backed non-trapping operators; the current unary, bitwise, shift/rotate, i64-compare, and i64-value bridge covers `i32.clz` / `i32.ctz` / `i32.popcnt`, `i32.and` / `i32.or` / `i32.xor`, `i32.shl` / `i32.shr_s` / `i32.shr_u` / `i32.rotl` / `i32.rotr`, `i64.eqz` and `i64` equality/relational compares, plus non-trapping i64 unary/arithmetic/bitwise/shift-rotate operators feeding those conditions;
