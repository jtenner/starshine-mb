...
2. Add further simple pure-condition operators only when they are non-trapping, source/probe-backed, and covered by focused tests; the 2026-05-18 bitwise and shift/rotate slices added `i32.and` / `i32.or` / `i32.xor` plus `i32.shl` / `i32.shr_s` / `i32.shr_u` / `i32.rotl` / `i32.rotr`.
