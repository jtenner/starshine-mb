...
- single-const private global write propagation along straight-line, adjacent/nested plain-block, and no-else if-body runtime traces with call/control/early-exit barriers plus guardrail coverage for imported globals, independent writes, latest-write wins, loops, `try_table` barriers, and else-arm conservatism,
...
- broader runtime linear-trace propagation beyond the landed straight-line, plain-block, and no-else if-body single-const write facts,
