...
- never-read, single-const same-as-init, adjacent/eqz/bidirectional compare-const/transparent/nested/block-wrapped-condition/no-op-condition-prefix/const-drop-body read-only-to-write self-guard, and exact/eqz/bidirectional compare/block-wrapped-condition/block-wrapped-set/block-wrapped-condition+set `if return; set` read-only-to-write `global.set` rewriting to `drop(value)` with the writer marked as touched,
...
- broader `read-only-to-write` structural matching and its whole-pass iteration beyond the landed adjacent/eqz/bidirectional compare-const/transparent/nested/block-wrapped-condition/no-op-condition-prefix/const-drop-body self-guard and exact/eqz/bidirectional compare/block-wrapped-condition/block-wrapped-set/block-wrapped-condition+set `if return; set` shapes,
