...
- single-const private global write propagation along straight-line, adjacent plain-block, and nested plain-block runtime traces with call/control/early-exit barriers plus guardrail coverage for imported globals, independent writes, latest-write wins, loops, and `try_table` barriers,
...
- broader runtime linear-trace propagation beyond the landed straight-line and plain-block single-const write facts,
