...
Binaryen promotes this family because the call decides which arm runs, while `$once` only flows out to the final branch condition. Starshine's current subset accepts one or more transparent block-result wrappers for supported pure post-consumers like `i32.eqz`, the matching non-branching value-loop wrapper for direct self-guards and the same pure post-consumer shape, and the supported clean-sibling `select` value form; it preserves the neighboring negative where the block- or loop-wrapped post-consumer is a trapping `i32.load` whose address comes from the global-derived value.
