...
The reference-typed post-consumer slices admit the same nested-if arm flow when a `funcref` candidate flows through nontrapping `ref.is_null`, an `eqref` candidate flows through nontrapping ordinary `ref.test` or `ref.eq`, an `i32` candidate flows through nontrapping `ref.i31` and then `ref.eq`, or an `anyref` candidate flows through nontrapping `extern.convert_any` / `any.convert_extern` / `ref.is_null`, with transparent result-block wrappers around the supported conversion chain when covered, before the outer same-global write guard. The paired negatives keep the global mutable when the ordinary `ref.test` / reference-test boolean result feeds a post-consumer call before the final branch, preserve `ref.as_non_null`-fed `ref.test` because the operand may trap first, and preserve nullable `i31.get_s` because it can trap. The `ref.eq` / conversion fixtures use `ref.i31` initializers or non-null-to-null writes where needed so the tests prove read-only-to-write FlowScanner behavior instead of same-as-init null cleanup.
