...
Starshine currently has an artifact-informed approximation of this nested replay in [`src/passes/pass_manager.mbt`](../../../../../src/passes/pass_manager.mbt). Focused characterization in [`src/passes/simplify_globals_optimizing_test.mbt`](../../../../../src/passes/simplify_globals_optimizing_test.mbt) proves the current guard behavior: cleanup now runs above the earlier `8` touched-function and `100` defined-function limits, while individually large touched functions (`> 192` locals or `> 1000` instructions) are filtered out and an all-large touched set still reports `large-touched-function`. The nested cleanup trace now also reports `nested-cleanup-stats touched=... cleaned=... filtered-large=...`; the debug artifact's traced direct run in `.tmp/sgo-nested-timers-trace.txt` reported `touched=79 cleaned=78 filtered-large=1`. The same trace includes SGO core timers and nested aggregate/per-slot timers: `detail:sgo:nested-total` was `213.664ms`, led by `simplify-locals`, `vacuum`, and `remove-unused-brs` wrapper slots. Upstream Binaryen has no equivalent touched-function-size guard in SGO; treat the remaining filter and the slower direct artifact replay as a Starshine performance/artifact blocker to remove, tune, or justify with evidence, not as source parity.
