...
As of the 2026-05-21 Starshine slices, the local SGO matcher covers adjacent `i32.eqz`, bidirectional compare-const, a simple pure-condition self-guard family including `i32.clz` / `i32.ctz` / `i32.popcnt`, `i32.and` / `i32.or` / `i32.xor`, `i32.shl` / `i32.shr_s` / `i32.shr_u` / `i32.rotl` / `i32.rotr`, `i64.eqz` / `i64` compare guards, non-trapping i64 value operators feeding those conditions, and `f32` / `f64` compare guards, nested block-condition, block-wrapped pure-condition and block-yielded condition-read forms including `i32.eqz` / compare-const, no-op const/drop prefixes before block-wrapped condition reads, this transparent `nop` / void-`block` body family, no-op const/drop prefixes before the single constant write for self guards, pure `select` self-guards, a conservative side-effecting stack/value-flow subset where independent `local.set` / `local.tee` / non-candidate `global.set`, scalar memory-load, `table.get`, or clean `table.set` / `i32.store` / `i64.store` / `i32.store8` / `i32.store16` / `i64.store8` / `i64.store16` operands are preserved while a single actual `global.get` flows through supported non-trapping pure ops, `select`, and the final branch, and the source-backed nested-if arm-flow positives where direct-call/`call_indirect`/pure/local-tee conditions, clean direct or indirect call operands, clean scalar memory-load, `table.get`, or clean `table.set` / `i32.store` / `i64.store` / `i32.store8` / `i32.store16` / `i64.store8` / `i64.store16` operands, arm-local calls, clean arm-local `local.tee`, scalar load, or `table.get` effects, transparent arm value blocks, post-if `select` value use, and supported pure post-if consumers are independent of the candidate global. It still does not cover Binaryen's full generic safe-condition rule, arbitrary side-effecting condition value-flow, or the full iterative `read-only-to-write` family.
