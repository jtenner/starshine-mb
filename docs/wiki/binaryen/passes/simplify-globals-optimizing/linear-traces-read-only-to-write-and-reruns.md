...
As of the 2026-05-23 Starshine slices, the local SGO matcher covers adjacent `i32.eqz`, bidirectional compare-const, a simple pure-condition self-guard family including `i32.clz` / `i32.ctz` / `i32.popcnt`, `i32.and` / `i32.or` / `i32.xor`, `i32.shl` / `i32.shr_s` / `i32.shr_u` / `i32.rotl` / `i32.rotr`, `i64.eqz` / `i64` compare guards, non-trapping i64 value operators feeding those conditions, and `f32` / `f64` compare guards, nested block-condition, block-wrapped pure-condition and block-yielded condition-read forms including `i32.eqz` / compare-const, no-op const/drop prefixes before block-wrapped condition reads, this transparent `nop` / void-`block` body family, no-op const/drop prefixes before the single constant write for self guards, pure `select` self-guards, a conservative side-effecting stack/value-flow subset where clean `nop` / clean void blocks / clean void loops / clean void `if`s / clean result blocks / clean result loops / clean result `if`s, independent `local.set` / `local.tee` / non-candidate `global.set`, scalar memory-load, `table.get`, or clean `table.set` / `i32.store` / `i64.store` / `i32.store8` / `i32.store16` / `i64.store8` / `i64.store16` / `i64.store32` / `f32.store` / `f64.store` / `memory.fill` / `memory.copy` / `memory.init` / `table.fill` / `table.init` / `table.copy` operands or operandless `elem.drop` / `data.drop` are preserved while a single actual `global.get` flows through supported non-trapping pure ops, `select`, and the final branch, and the source-backed nested-if arm-flow positives where direct-call/`call_indirect`/pure/local-tee conditions, clean direct or indirect call operands, clean scalar memory-load, `table.get`, or clean `table.set` / `i32.store` / `i64.store` / `i32.store8` / `i32.store16` / `i64.store8` / `i64.store16` / `i64.store32` / `f32.store` / `f64.store` / `memory.fill` / `memory.copy` / `memory.init` / `table.fill` / `table.init` / `table.copy` operands or operandless `elem.drop` / `data.drop`, arm-local calls, clean arm-local `local.tee`, scalar load, or `table.get` effects, transparent arm value blocks, post-if `select` value use, and supported pure post-if consumers are independent of the candidate global. It still does not cover Binaryen's full generic safe-condition rule, arbitrary side-effecting condition value-flow, or the full iterative `read-only-to-write` family.
...
That is why the source needs an upward value-flow walk instead of just a flat “side effects yes/no” check. Starshine now has pure-condition positives plus pure `select` self-guards where the global-derived value is the select condition input, first selected value input, or second selected value input. It also has a conservative local stack/value-flow scanner for official-style `local.set` / non-candidate `global.set` / `local.tee` / `i32.load` / `global.get + const` / `select` / `i32.eqz` positives plus clean `nop` / clean void blocks / clean void loops / clean void `if`s / clean result blocks / clean result loops / clean result `if`s, clean `local.set`, non-candidate `global.set`, clean `table.set`, clean `i32.store` / `i64.store` / `i32.store8` / `i32.store16` / `i64.store8` / `i64.store16` / `i64.store32` / `f32.store` / `f64.store` / `memory.fill` / `memory.copy` / `memory.init` / `table.fill` / `table.init` / `table.copy` / `elem.drop` / `data.drop`, scalar memory-load, and `table.get` probes and nearby extra pure-op chains, plus nested-if arm-flow cases where independent direct, `call_indirect`, or ordinary `call_ref` calls with clean operands, pure ops, local/global sets, local tees, scalar memory loads, `table.get`, or clean `table.set` / `i32.store` / `i64.store` / `i32.store8` / `i32.store16` / `i64.store8` / `i64.store16` decide which value arm runs and the candidate global only flows out through an arm; independent arm-local calls are also preserved when they do not consume the global-derived value. The 2026-05-20/21 follow-ups also permit clean arm-local `local.set` / non-candidate `global.set` / `local.tee`, scalar memory-load, `table.get`, or clean `table.set` / `i32.store` / `i64.store` / `i32.store8` / `i32.store16` / `i64.store8` / `i64.store16` / `i64.store32` / `f32.store` / `f64.store` / `memory.fill` / `memory.copy` / `memory.init` / `table.fill` / `table.init` / `table.copy` / `elem.drop` / `data.drop` effects, transparent arm value blocks around the candidate read, post-if `select` value uses with clean sibling operands, nested-if results that pass through supported pure post-if consumers such as `i32.eqz`, nontrapping `ref.is_null`, ordinary nontrapping `ref.test`, nontrapping `ref.eq`, nontrapping `ref.i31`, fact-sensitive `ref.i31`-fed `ref.as_non_null` / `i31.get_s` / `i31.get_u` including typed `select` results whose value operands are both proven `ref.i31`, nontrapping `extern.convert_any` / `any.convert_extern`, nontrapping numeric conversions/division, clean `memory.size` / `table.size` nullary size queries, and selected parser-backed nontrapping SIMD consumers including integer splat/arithmetic/compare/reduction/bitmask chains, f32/f64 splat/unary/arithmetic/compare chains, lane extracts, lane replacements, integer SIMD shifts, remaining parser-backed integer/float SIMD comparisons, basic nontrapping SIMD arithmetic/unary ops, and parser-backed SIMD extension/conversion/narrow/extmul/dot/extadd-pairwise chains before the outer branch, flat or nested transparent result blocks and matching non-branching value loops that wrap supported direct/pure FlowScanner condition chains, adjacent post-loop `i32.eqz`, compare-const, or reverse compare-const self-guards where the loop yielded a direct candidate read, block-yielded supported FlowScanner chains in exact `if return; set` bodies, clean value-producing `if` wrappers inside block conditions and exact `if return; set` block conditions when an independent wrapper condition selects an arm with a supported size-query/SIMD chain, one or more transparent result blocks or the matching non-branching value-loop wrapper that wrap the nested-if plus supported pure/select/post-conversion consumers before the outer branch, and clean value-producing `if` wrappers around another supported nested-if arm result. The `local.tee`, trapping load or `table.get`, and independent call are preserved when independent; the global read only contributes to the final branch decision through supported pure/select/nested-arm/block-result/if-wrapper flow. Guardrails preserve cases where `global.get` feeds a trapping memory load or `table.get`, escapes through `local.tee`, flows into `i32.store` / `i64.store` / `i32.store8` / `i32.store16` / `i64.store8` / `i64.store16`, appears multiple times in the same condition, steers the nested `if` condition, reaches a non-condition consumer, becomes an operand to a post-if call, becomes a `call_indirect` / `call_ref` operand, feeds a trapping load after a post-if `select`, feeds a trapping load inside a transparent arm block, feeds a block- or loop-wrapped post-if load, feeds a post-loop `i32.eqz`, compare, or reverse compare whose loop value comes from a trapping load, feeds a clean if-wrapper result into a trapping load, feeds a clean condition if-wrapper arm into a trapping load, steers an inner if-wrapper branch, feeds a nontrapping ordinary `ref.test` / reference-test / numeric-conversion result to a post-consumer call, flows through `ref.as_non_null` before ordinary `ref.test`, flows through nullable or non-`ref.i31`-proven `i31.get_s`/`i31.get_u` that may trap, flows through regular float-to-int truncation that may trap, feeds an effectful `memory.grow` / `table.grow` size change, feeds a SIMD lane/shift/replace-lane/compare/arithmetic/core-bitwise/extadd-pairwise/extension/conversion result or boolean into a post-consumer call, feeds a global-derived address into `v128.load`, or uses unsupported SIMD/parser forms not yet source-backed by Starshine WAT lowering. Other official side-effecting positives remain future slices.
...
One current Starshine-specific caveat: after recursive or arm-local control-shaped core rewrites, small value-block/control bodies can expose a HOT nested-cleanup verification frontier. The SGO wrapper therefore filters touched functions with value blocks or value-producing `if` bodies containing nested `if` control out of the nested cleanup lane and returns the valid core rewrite for that shape; this is a local cleanup-scheduler guard, not a Binaryen source rule.
...
As of the 2026-05-18 follow-up slices, Starshine covers the exact, `i32.eqz`, bidirectional compare-const, pure-condition, and block-wrapped-condition/block-wrapped-pure-condition/nested-block-wrapped-condition/block-yielded-condition/block-yielded-condition+set/block-wrapped-set/block-wrapped-condition+set `if return; set` family and keeps focused trailing-`nop` negatives so it does not widen into a broader CFG proof by accident. Later SIMD FlowScanner slices apply the same exact-body rule to supported block-yielded SIMD chains, including the 2026-05-21 lane/shift/replace subset.
...
Starshine currently has an artifact-informed approximation of this nested replay in [`src/passes/pass_manager.mbt`](../../../../../src/passes/pass_manager.mbt). Focused characterization in [`src/passes/simplify_globals_optimizing_test.mbt`](../../../../../src/passes/simplify_globals_optimizing_test.mbt) proves the current guard behavior: cleanup now runs above the earlier `8` touched-function and `100` defined-function limits, while individually large touched functions (`> 192` locals or `> 1000` instructions) and value-block/control touched functions are filtered out; an all-large touched set still reports `large-touched-function`, and an all-structural touched set reports `value-block-control`. The nested cleanup trace also reports `nested-cleanup-stats touched=... cleaned=... filtered-large=... filtered-structural=...`.
