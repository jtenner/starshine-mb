...
Starshine currently has a guarded approximation of this nested replay in [`src/passes/pass_manager.mbt`](../../../../../src/passes/pass_manager.mbt). Focused characterization in [`src/passes/simplify_globals_optimizing_test.mbt`](../../../../../src/passes/simplify_globals_optimizing_test.mbt) proves the current guard behavior: cleanup runs for changed-function sets at or below `8`, skips with `large-touched-set` above `8`, skips with `large-module` when the module has more than `100` defined functions, and skips with `large-touched-function` when a touched function has more than `128` locals or more than `1000` instructions. Upstream Binaryen has no equivalent guard in SGO; treat these limits as a Starshine performance/artifact blocker to remove or justify with evidence, not as source parity.
