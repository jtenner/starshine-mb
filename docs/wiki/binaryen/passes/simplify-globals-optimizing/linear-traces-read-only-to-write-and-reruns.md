...
That is why the source needs an upward value-flow walk instead of just a flat “side effects yes/no” check. Starshine now has pure-condition positives plus pure `select` self-guards where the global-derived value is the select condition input, first selected value input, or second selected value input. It also has a conservative local stack/value-flow scanner for official-style `local.tee` / `i32.load` / `global.get + const` / `select` / `i32.eqz` positives and nearby extra pure-op chains, plus nested-if arm-flow cases where independent calls with clean operands, pure ops, local tees, or loads decide which value arm runs and the candidate global only flows out through an arm; independent arm-local calls are also preserved when they do not consume the global-derived value. The 2026-05-20 follow-ups also permit clean arm-local `local.tee` / `i32.load` effects, transparent arm value blocks around the candidate read, post-if `select` value uses with clean sibling operands, nested-if results that pass through supported pure post-if consumers such as `i32.eqz` and nontrapping `ref.is_null` before the outer branch, one or more transparent result blocks that wrap the nested-if plus supported pure/select post-consumers before the outer branch, and clean value-producing `if` wrappers around another supported nested-if arm result. The `local.tee`, trapping load, and independent call are preserved when independent; the global read only contributes to the final branch decision through supported pure/select/nested-arm/block-result/if-wrapper flow. Guardrails preserve cases where `global.get` feeds a trapping `i32.load`, escapes through `local.tee`, appears multiple times in the same condition, steers the nested `if` condition, reaches a non-condition consumer, becomes an operand to a post-if call, feeds a trapping load after a post-if `select`, feeds a trapping load inside a transparent arm block, feeds a block-wrapped post-if load, feeds a clean if-wrapper result into a trapping load, steers an inner if-wrapper branch, or feeds a `ref.is_null` result to a post-consumer call. Other official side-effecting positives remain future slices.
