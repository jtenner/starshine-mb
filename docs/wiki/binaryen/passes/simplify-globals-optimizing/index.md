...
- The August 3 SGO nontermination was nested SimplifyLocalsNoStructure false progress on a byte-identical `f32.const nan`: structural float equality treats NaN as unequal to itself. Root carrier forwarding, nop hoisting, and root finalization now report explicit mutation facts. The reduced trapping-conversion fixture converges while preserving `f32.const nan -> i32.trunc_f32_s`; the complete SGO test file is `324/324` and completes in normal repository-test time. The August 4 source reconstruction restored the same behavior and focused results.
- A 2026-08-11 O4z size-recovery slice made SGO's bounded cheap cleanup delete adjacent pure `local.get; drop` pairs when the local has no prior write in the same instruction sequence. The prior-write guard preserves local-tee/local-set payoff carriers used by the official side-effecting-select cleanup families. A red-first 257-function regression owns the production path where another cheap cleanup suppresses the expensive nested roster. After nested cleanup finishes, a module-wide linear final sweep removes all `nop` roots, adjacent stack-neutral local/global reads and numeric, vector, or reference constants followed by `drop`, and immediate pure unary or binary numeric expression trees whose result is dropped; this ordering preserves earlier local-tee/local-set payoff matching while reclaiming final debris even in functions that did not need the expensive nested roster. The same slice stops duplicating single-use multi-instruction immutable initializers while their source global remains, matching Binaryen's smaller alias shape. The final sweep also removes dropped two-instruction result blocks containing only a pure value and `br 0`, pure result-`if` trees whose condition and both arms are independently nontrapping, and stack-neutral effectful result blocks ending in a pure value plus `br 0`. The broader block rewrite requires a bounded exact stack-effect proof for the retained prefix and rejects nested owner-targeting branches, type-indexed results, unknown instructions, and exception handlers. Native SHA-256 `f12e5a80657760773d895d29e7b354c364840822d3aa656c5d6b2532874d8801` reduces 96 of the 105 `json-as` artifacts with no growth, from `21,140,906` to `20,708,243` aggregate bytes (`-432,663`), while exact WIPC remains `105/105`, `1,284` suites, and `10,656` tests. The remaining verified-v131 O4z gap is `5,063,119` bytes / `32.36%`. SGO rollback deep-copies only touched function instruction forests rather than encoding and decoding the full module; the representative snapshot phase falls from `39.023 ms` to `1.773 ms`. An exact eleven-pair alternating A/B against the preceding trivial-block binary is neutral on `fast-path-deserialize` (`-0.01%`) and improves `map` by `2.08%`, while producing smaller deterministic outputs. Native/self-optimized artifact optimization remains byte-identical at `5,031,542` bytes, recursive spec remains `284/64/220/0`, full Moon is `10,350/10,350`, and both wasm-gc full validation plus native CI fuzz are green.
- That touched-function nested rerun is **not** the same helper used by `dae-optimizing` and `inlining-optimizing`:
  - it does **not** prepend `precompute-propagate`
  - it reruns per changed function through a nested `PassRunner`
  - separately, after SGO changes a module, a `<1000`-definition transactional final suffix runs `precompute-propagate -> merge-blocks -> simplify-locals -> remove-unused-brs -> bounded coalesce-locals -> sgo_apply_final_cleanup`; it validates and encodes both sides and commits only a strictly smaller candidate
- Imports, exports, actual calls, nonlinear control flow, and type mismatches are major bailout or conservatism families.
- A 2026-08-12 final-suffix signoff repaired structured local-state propagation needed by the suffix's `precompute-propagate` wave. The final transaction is owned by SGO rather than a new top-level O4z slot and is skipped for artifact-scale modules. A follow-up inserted `merge-blocks`, then repaired its initial `custom.spec.wasm` runtime failure by failing closed on a stack-carried local overwritten before a later call. The next late `simplify-locals` experiment initially validated and shrank but failed exact WIPC with allocator/TLSF aborts. Function isolation reduced that failure to defined function 43 / absolute function 47 in `bool.spec.wasm`: child-only adjacent-set/get cleanup erased a conditional call-result write still read after the enclosing `if`. The repaired cleanup propagates inherited and later-sibling local reads through blocks, `if` arms, loops, and `try_table`, with loop-body reads retained for backedges. Native SHA-256 `443fa73acbe3789b0e1b330fdf28652fe5f567c4e6df53470e46217b65b92d47` retains `precompute-propagate -> merge-blocks -> simplify-locals -> remove-unused-brs -> bounded coalesce-locals -> sgo_apply_final_cleanup` and produces `20,252,110` aggregate bytes across the 105 `json-as` artifacts: naive `6,601,920`, SIMD `6,841,190`, SWAR `6,809,000`; every artifact is smaller than the signed MergeBlocks checkpoint, aggregate `-24,387`, and optimize/external validation plus exact no-cache WIPC both pass `105/105`. Against verified Binaryen v131 `15,645,124`, the remaining gap is `4,606,986` bytes / `29.4468%`. Self-optimization is green at debug `13,770,560`, release `5,068,591`, and self-optimized `5,014,671` bytes; native and self-optimized-Wasm optimizers emit byte-identical output SHA-256 `4e17218bcb9bf42ac8f813333dd1dff395597363d39b3190bd8c7d0c9cc4bf00`, recursive spec is `284/64/220/0`, and full wasm-gc validation passes including `86,820` binary roundtrips. The `final-cleanup-suffix accepted=true` trace, the conditional call-result live-out regression, and the safe nested-block regression cover the retained path.
...
## 2026-08-21 stateful O4z suffix deferral

The broad final SGO cleanup suffix remains part of ordinary direct pass behavior, but O4z now defers it for the exact 44/50-function stateful Emscripten family with `_main`, `runPostSets`, `stackAlloc`, `stackSave`, and `stackRestore`. On primes the suffix recovered six intermediate bytes that the later locked O4z schedule recovered independently, while costing roughly 15 ms. The exact family now runs `sgo_apply_final_cleanup` and emits `final-cleanup-suffix skipped=true reason=o4z-stateful-emscripten-later-schedule`; all unrelated families retain the prior suffix.

Against a controlled otherwise-identical 22,108-byte primes baseline, 16 alternating pairs reduce median SGO time 22.168 → 6.820 ms (-69.24%), pipeline time 206.355 → 193.420 ms (-6.27%), and whole-command time 369.817 → 360.334 ms (-2.56%). The final artifact is also four bytes smaller through an independent restored-owner stack carry. IFS uses the same deferral with byte-identical/runtime-equal output. The exact final native binary is `dcfc1383f399312fa1dc3fce21a84ed6a0bf6ba607d47ec78464a7a3a633c093`.

## 2026-08-21 nested validation batching

The touched-function nested roster no longer performs a second whole-module validation after every generic hot pass. `run_hot_pipeline_apply_hot_pass_to_touched(...)` already validates every changed definition against the current module and repairs invalid writeback. SGO now retains its deep-copy rollback snapshot, runs the complete roster, performs one timed `detail:sgo:nested:validate-final` whole-module check, and returns the rollback transaction if that final check fails.

On fresh original-input primes, alternating traced old/new runs reduce median `pass:simplify-globals-optimizing` from 46.539 to 21.847 ms (-53.1%) and `detail:sgo:nested-total` from 28.505 to 3.700 ms (-87.0%). The main optimization pipeline falls from 228.172 to 204.346 ms (-10.44%). The exact final native binary is `3fce0ab9e1d4934084d1e4e0312e598b53633a950247cd6d83cc8688e78cd92b`. Regular explicit-v131 GenValid smoke is 1,000/1,000 normalized; the six-leaf dedicated lane has the established 471 normalized plus 529 smaller Starshine outputs and zero validation/property/generator/command failures. This is a performance refresh, not a replacement for the existing full closeout matrix.
...
## 2026-08-25 trapping GC allocation global writes

The broad WAGO semantic lane found `wasmtime-core3/core/gc/issue-13034`: the original exported `run` traps while allocating an array of length `1073741817`, but O4z returned normally after SimplifyGlobalsOptimizing removed the unread global write and its allocation. Prefix bisection with exact-reference support found slot 49 safe and slot 50 `simplify-globals-optimizing` unsafe; validation remained green throughout.

The SGO dispatcher now recognizes an immediate GC allocation result consumed by `global.set`—array and struct constructor families, recursively inside structured regions—and fails closed before the touched-function nested cleanup roster can erase the mandatory allocation/OOM trap. The direct regression preserves `array.new_default` and `global.set`. Full O4z now reproduces the original `requested new array is too large` trap. Focused SGO tests are `336/336` and pass-manager white-box tests `343/343`.
