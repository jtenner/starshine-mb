...
# `string-lifting` Fuzzing Profile

Recommended smoke lane: run the ordinary mixed-generator compare-pass lane for this pass:

```sh
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass string-lifting --out-dir .tmp/pass-fuzz-string-lifting --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe
```
