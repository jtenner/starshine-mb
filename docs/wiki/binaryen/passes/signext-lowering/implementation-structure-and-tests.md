...
| --- | --- | --- |
| `src/passes/SignExtLowering.cpp` | Transform owner | First returns if `getModule()->features.hasSignExt()` is false; on the enabled path it defines the function-parallel postwalk, matches five sign-extension unary opcodes, builds same-width `shl` + arithmetic `shr_s` replacements, reuses the original child once, replaces the current root, and disables `FeatureSet::SignExt`. |
...
The core owner file has four responsibilities:

1. **Gate by module feature state.** If `FeatureSet::SignExt` is absent, return without walking or rewriting functions.
2. **Walk enabled functions independently.** Each function body can be rewritten without looking at other functions, globals, tables, memories, or types.
3. **Rewrite matched unary roots.** The five matched operators are `ExtendS8Int32`, `ExtendS16Int32`, `ExtendS8Int64`, `ExtendS16Int64`, and `ExtendS32Int64`. Each replacement is the corresponding storage-width `Shl` followed by signed `ShrS` with shift counts `24`, `16`, `56`, `48`, and `32`.
4. **Clear the feature bit.** After function rewriting, Binaryen disables `FeatureSet::SignExt` on the module.
...
| --- | --- | --- |
| [`src/passes/optimize.mbt:126-153`](../../../../../src/passes/optimize.mbt) | Boundary-only and removed pass lists omit `signext-lowering`; a repo search also found no matching registry entry. | A future port needs a new registry entry only after behavior lands. |
| [`src/passes/pass_manager.mbt`](../../../../../src/passes/pass_manager.mbt) | Active pipeline/dispatcher source has no `signext-lowering` case. | A future instruction-only port could dispatch here; feature cleanup may require module ownership. |
| [`src/wast/types.mbt:471-475`](../../../../../src/wast/types.mbt) | WAT opcode enum includes the five sign-extension opcodes. | Parser-facing prerequisite. |
| [`src/wast/keywords.mbt:345-349`](../../../../../src/wast/keywords.mbt) | Text mnemonics map to those opcode cases. | WAT input prerequisite. |
| [`src/wast/parser.mbt:2149-2153`](../../../../../src/wast/parser.mbt) | Parser classifies all five mnemonics as unary operations. | Baseline input coverage for future pass tests. |
| [`src/wast/lower_to_lib.mbt:1306-1310`](../../../../../src/wast/lower_to_lib.mbt) | WAT opcodes lower to library instructions. | Bridge from parsed WAT to lib IR. |
| [`src/lib/types.mbt:771-775`](../../../../../src/lib/types.mbt) | Lib instruction cases exist. | Likely transform-building surface. |
| [`src/binary/encode.mbt:2563-2567`](../../../../../src/binary/encode.mbt), [`src/binary/decode.mbt:3019-3023`](../../../../../src/binary/decode.mbt) | Codec maps bytes `0xC0` through `0xC4` to the five constructors. | Confirms current binaries can contain direct sign-extension opcodes. |
| [`src/lib/show.mbt:1337-1341`](../../../../../src/lib/show.mbt) | Pretty-printer emits underscoreless spellings like `i32.extend8s`. | WAT-golden tests should normalize or intentionally assert the hygiene gap. |
| [`src/validate/typecheck.mbt:4033-4037`](../../../../../src/validate/typecheck.mbt) | Typechecker handles unary same-type stack behavior. | Validation baseline for before/after tests. |
| [`src/ir/hot_lift.mbt:900-904`](../../../../../src/ir/hot_lift.mbt) | HOT lifting classifies all five as unary HOT ops. | Makes a HOT rewrite feasible if feature cleanup is separate. |
