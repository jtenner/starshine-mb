...
This later invalidation happens in the checker, not in the scanner. The invalidation is effect-sensitive, not a blanket "any write kills everything" rule: 2026-06-04 Starshine slices confirmed Binaryen reuses a local-only arithmetic tree across intervening `struct.set`, `array.set`, and `array.fill` writes, because the repeated tree reads locals rather than the mutated GC field, array element, or filled range. Starshine now models `struct.set`, `array.set`, and `array.fill` as operand-taking no-result instructions for this narrow case without treating the write instructions themselves as reusable roots. The same audit stream confirmed `br_on_non_null` mirrors `br_on_null` as a narrow reference-control fallthrough continuation case for local-only repeated trees; Starshine models the operand without making the branch operation reusable.
