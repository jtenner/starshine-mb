...
Before, `array.set` variant:

```wat
(drop (i32.add (local.get $x) (local.get $y)))
(array.set $arr (local.get $arr) (i32.const 0) (i32.const 7))
(drop (i32.add (local.get $x) (local.get $y)))
```

Before, `array.fill` variant:

```wat
(drop (i32.add (local.get $x) (local.get $y)))
(array.fill $arr (local.get $arr) (i32.const 0) (i32.const 7) (i32.const 1))
(drop (i32.add (local.get $x) (local.get $y)))
```

Before, `array.copy` variant:

```wat
(drop (i32.add (local.get $x) (local.get $y)))
(array.copy $arr $arr (local.get $dst) (i32.const 0) (local.get $src) (i32.const 0) (i32.const 1))
(drop (i32.add (local.get $x) (local.get $y)))
```
...
Before, `array.init_elem` variant:

```wat
(drop (i32.add (local.get $x) (local.get $y)))
(array.init_elem $arr $elem (local.get $arr) (i32.const 0) (i32.const 0) (i32.const 1))
(drop (i32.add (local.get $x) (local.get $y)))
```

Before, `memory.copy` / `memory.fill` / `memory.init` / `memory.grow` / `memory.size` / `table.get` / `table.copy` / `table.fill` / `table.init` / `table.set` / `table.grow` / `table.size` / `global.set` / `data.drop` / `elem.drop` variants:
...
;; or: (drop (memory.size))
;; or: (drop (table.get 0 (i32.const 0)))
...
;; or: (table.set 0 (i32.const 0) (ref.null func))
;; or: (drop (table.grow 0 (ref.null func) (i32.const 1)))
;; or: (drop (table.size 0))
;; or: (global.set $g (i32.const 7))
;; or: (data.drop 0)
;; or: (elem.drop 0)
(drop (i32.add (local.get $x) (local.get $y)))
```
...
(struct.set $box 0 (local.get $box) (i32.const 7)) ;; or array.set/fill/copy/init_data/init_elem/memory.copy/fill/init/grow/size/table.get/copy/fill/init/set/grow/size/global.set/data.drop/elem.drop ...
(drop (local.get $tmp))
```
...
- the intervening `struct.set`, `array.set`, `array.fill`, `array.copy`, `array.init_data`, or `array.init_elem` mutates GC storage, or `memory.copy` / `memory.fill` / `memory.init` mutates linear memory, or `memory.grow` mutates memory size, or `memory.size` reads memory size, or `table.get` / `table.size` reads table state, or `table.copy` / `table.fill` / `table.init` / `table.set` / `table.grow` mutates table elements or table size, or `global.set` mutates a global, or `data.drop` / `elem.drop` mutate segment availability, not those locals
- this is an effect-invalidation decision, not arbitrary heap or memory CSE

Starshine status: these Binaryen-positive shapes are covered and implemented narrowly by modeling `struct.set`, `array.set`, `array.fill`, `array.copy`, `array.init_data`, `array.init_elem`, `memory.copy`, `memory.fill`, `memory.init`, `memory.grow`, `table.copy`, `table.fill`, `table.init`, `table.set`, and `global.set` operands, the side-effecting `memory.grow` / `table.grow` results, the `memory.size` / `table.get` / `table.size` stack results, plus the zero-operand `data.drop` / `elem.drop` side effects in the raw/module path. The intervening effect/state-read instructions themselves are still not arbitrary reusable roots; `memory.grow` roots, memory-size expressions across `memory.grow`, `table.grow` roots, table-state-dependent roots, and global-dependent expressions across `global.set` are explicitly guarded by no-reuse regressions, while repeated `memory.size` roots without intervening growth are covered as a narrow size-read case rather than arbitrary memory GVN.
...
Before, reference tail-call variant:

```wat
(drop (i32.add (local.get $x) (local.get $y)))
(return_call_ref $t (local.get $x) (ref.func $target))
(drop (i32.add (local.get $x) (local.get $y)))
```
...
(block (result (ref i31))
  (drop (i32.add (local.get $x) (local.get $y)))
  (br_on_cast $exit anyref (ref i31) (local.get $maybe_any))
  (drop)
  (drop (i32.add (local.get $x) (local.get $y)))
  (ref.i31 (i32.const 0)))
```
