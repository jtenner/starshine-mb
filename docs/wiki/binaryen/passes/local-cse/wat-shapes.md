...
Before, `memory.copy` / `memory.fill` / `memory.init` / `table.copy` / `table.fill` / `table.init` / `table.set` / `table.grow` / `global.set` / `data.drop` / `elem.drop` variants:
...
(struct.set $box 0 (local.get $box) (i32.const 7)) ;; or array.set/fill/copy/init_data/init_elem/memory.copy/fill/init/table.copy/fill/init/set/grow/global.set/data.drop/elem.drop ...
(drop (local.get $tmp))
```
...
Starshine status: these Binaryen-positive shapes are covered and implemented narrowly by modeling `struct.set`, `array.set`, `array.fill`, `array.copy`, `array.init_data`, `array.init_elem`, `memory.copy`, `memory.fill`, `memory.init`, `table.copy`, `table.fill`, `table.init`, `table.set`, and `global.set` operands, the side-effecting `table.grow` result, plus the zero-operand `data.drop` / `elem.drop` side effects in the raw/module path. The intervening side-effect instructions themselves are still not reusable roots; `table.grow` roots and global-dependent expressions across `global.set` are explicitly guarded by no-reuse regressions.
