...
The honest remaining state is preset-slot restraint until the missing ordered neighborhoods are representable, plus broader shape-hardening follow-up for hard control-boundary negatives, GC/generative-root negatives, and idempotent-call positives where local annotation plumbing can model Binaryen safely. Tiny-root repeated `global.get` no-op plus before-loop into loop-body and return-boundary coverage are now durable in the direct test surface.
...
| `src/passes/local_cse_test.mbt` | Direct repeated-tree, parent-cancellation, load/store barrier, local-write, before-`if` / then-arm positive, paired after-`if` / else-arm negative, before-loop into loop-body negative, return-boundary negative, and tiny-root `global.get` no-op tests. |
...
- additional hard control-boundary negatives beyond after-`if`, else-arm, before-loop into loop-body, and return
