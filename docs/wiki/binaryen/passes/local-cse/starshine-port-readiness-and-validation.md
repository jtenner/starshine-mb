...
The honest remaining state is preset-slot restraint until the missing ordered neighborhoods are representable, plus broader shape-hardening follow-up for hard control-boundary negatives, remaining `call_ref` barriers, and remaining GC/generative-root negatives. Tiny-root repeated `global.get` no-op, repeated `struct.new` and `struct.new_default` generative-root no-ops, the annotated idempotent direct-call positive with ordinary direct-call and `call_indirect` root negatives, before-`if` / then-arm and before-block / straight-line block positives, and before-loop into loop-body, `br_table`, `return`, and `unreachable` boundary coverage are now durable in the direct test surface.
...
| `src/passes/local_cse_test.mbt` | Direct repeated-tree, parent-cancellation, load/store barrier, local-write, before-`if` / then-arm and before-block / straight-line block positives, annotated idempotent direct-call reuse with ordinary direct-call and `call_indirect` root negatives, paired after-`if` / else-arm negative, before-loop into loop-body negative, `br_table` boundary negative, return-boundary negative, unreachable-boundary negative, tiny-root `global.get` no-op, and `struct.new` / `struct.new_default` generative-root no-op tests. |
...
- a remaining `call_ref` negative paired with the now-covered annotated idempotent direct-call positive plus ordinary direct-call and `call_indirect` root negatives
- additional generative GC-root negatives beyond the covered `struct.new` / `struct.new_default` fixtures; `array.new*` likely needs core/binary fixtures because the local direct-pass WAT path does not expose ordinary `array.*` text
...
The source-backed Binaryen positives where a repeated tree before an `if` is reused inside the `then` arm and where a repeated tree before a straight-line block is reused inside that block body are now covered in `src/passes/local_cse_test.mbt`. Paired after-`if` and else-arm negatives keep the `if` fix from widening the reuse window past Binaryen's `LinearExecutionWalker` boundaries, and the block fix remains a straight-line block-body bridge rather than loop or hard-terminator reuse. The implementation stays narrow: it shares eligible outer whole-tree bindings with the `then` or straight-line block body only and does not turn LCSE into CFG-wide CSE. The same direct test file now also protects the loop-boundary rule by proving an expression before a loop is not reused inside the loop body, protects switch-like `br_table`, return, and `unreachable` boundaries by proving expressions before those terminators are not reused after them, protects the tiny-root profitability rule by proving repeated `global.get` roots remain unmaterialized, protects the indirect-call root barrier by proving repeated `call_indirect` roots remain separate, and protects two GC generativity rules by proving repeated `struct.new` and `struct.new_default` roots remain separate.
