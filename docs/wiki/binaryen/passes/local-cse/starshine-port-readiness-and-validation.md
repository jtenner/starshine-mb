...
- additional generative GC-root variants beyond the covered `struct.new` / `struct.new_default` WAT fixtures and core-built `array.new` / `array.new_default` / `array.new_fixed` / `array.new_data` / `array.new_elem` fixtures
