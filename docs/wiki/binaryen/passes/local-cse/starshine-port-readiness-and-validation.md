...
The honest remaining state is preset-slot restraint until the missing ordered neighborhoods are representable, plus broader shape-hardening follow-up for remaining hard-control and GC/generative-root variant negatives. Tiny-root repeated `global.get` no-op, repeated `struct.new`, `struct.new_default`, and core `array.new`, `array.new_default`, and `array.new_fixed` generative-root no-ops, the annotated idempotent direct-call positive with ordinary direct-call, `call_indirect`, and `call_ref` root negatives, before-`if` / then-arm, before-block / straight-line block, and before-`try_table` / try-body positives, and before-loop into loop-body, `br_table`, `return`, `throw`, top-level `unreachable`, and `try_table` body unreachable-boundary coverage are now durable in the direct test surface.
...
- additional hard control-boundary negatives beyond after-`if`, else-arm, before-loop into loop-body, `br_table`, return, throw, top-level `unreachable`, and `try_table` body unreachable
