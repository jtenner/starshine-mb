...
| `src/passes/local_cse_test.mbt` | Direct repeated-tree, parent-cancellation, load/store barrier, local-write, before-`if` / then-arm, before-block / straight-line block, and before-`try_table` / try-body positives, annotated idempotent direct-call reuse with ordinary direct-call and `call_indirect` root negatives, paired after-`if` / else-arm negative, before-loop into loop-body negative, `br_table` boundary negative, return-boundary negative, unreachable-boundary negative, tiny-root `global.get` no-op, and `struct.new` / `struct.new_default` / core `array.new` generative-root no-op tests. |
