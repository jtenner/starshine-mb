...
- The before-`if` into `then`, before-block into straight-line block, and annotated idempotent direct-call missed optimizations are now covered test-first and implemented in the raw/module path, the ordinary direct-call and `call_indirect` root no-ops, before-loop into loop-body, `br_table`, return-boundary, and unreachable-boundary negatives are covered, the tiny-root repeated-`global.get` no-op is covered, and the repeated `struct.new` / `struct.new_default` generative-root no-ops are covered as durable direct tests; keep future O4z LCSE closure criteria focused on the remaining hard control-boundary, `call_ref`, and GC/generative shapes rather than reopening those fixed/covered families.
