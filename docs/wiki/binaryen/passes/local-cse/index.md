...
- The before-`if` into `then` missed optimization is now covered test-first and implemented in the raw/module path, the before-loop into loop-body, `br_table`, return-boundary, and unreachable-boundary negatives are covered, and the tiny-root repeated-`global.get` no-op is covered as a durable direct test; keep future O4z LCSE closure criteria focused on the remaining hard control-boundary, GC/generative, and idempotent-call shapes rather than reopening those fixed/covered families.
