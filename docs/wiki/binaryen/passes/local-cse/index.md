...
- Intervening writes are handled by effect invalidation rather than a single blanket boundary rule: `struct.set`, `array.set`, `array.fill`, `array.copy`, `array.init_data`, `array.init_elem`, `memory.copy`, `memory.fill`, `memory.init`, `table.copy`, `table.fill`, `table.init`, `table.set`, `table.grow`, `global.set`, `data.drop`, and `elem.drop` do not kill reuse of a repeated tree that only reads locals, but the write itself is not a reusable root.
