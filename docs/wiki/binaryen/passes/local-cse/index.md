...
- Fresh/generative GC roots such as `struct.new`, `struct.new_default`, and `array.new*` must not be reused; Starshine now has direct regression coverage for repeated `struct.new`, `struct.new_default`, and core-built `array.new` / `array.new_default` / `array.new_fixed` roots remaining separate.
