...
- Intervening effectful operations and state reads are handled by effect invalidation rather than a single blanket boundary rule: `struct.set`, `array.set`, `array.fill`, `array.copy`, `array.init_data`, `array.init_elem`, `memory.copy`, `memory.fill`, `memory.init`, `memory.grow`, `memory.size`, `table.copy`, `table.fill`, `table.init`, `table.set`, `table.grow`, `table.size`, `global.set`, `data.drop`, and `elem.drop` do not kill reuse of a repeated tree that only reads locals, but the operation itself is not automatically a reusable root.
