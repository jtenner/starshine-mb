...
- Intervening writes are handled by effect invalidation rather than a single blanket boundary rule: `struct.set`, `array.set`, `array.fill`, `array.copy`, and `array.init_data` do not kill reuse of a repeated tree that only reads locals, but the write itself is not a reusable root.
