...
- keep non-relaxed pure SIMD arithmetic/logic, comparison, splat, replace-lane, shuffle/swizzle, any-true/all-true/bitmask, extended integer arithmetic, float min/max, float rounding, widening/narrowing, conversion, and lane-extract roots as implemented Binaryen-parity cases with `v128` or scalar temp locals; keep relaxed SIMD and SIMD memory roots (`v128.load*`, lane-loads, load-splat/load-zero/load-extend, stores, and lane-stores) as conservative deferrals unless a separate nondeterminism/memory-state model is approved
