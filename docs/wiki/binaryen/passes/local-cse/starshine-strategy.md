...
The 2026-05-06 refreshed direct-pass lane is green: `.tmp/pass-fuzz-local-cse` reported 6759/10000 compared cases, 6759 normalized matches, 0 mismatches, and 20 known Binaryen empty-recursion-group command failures. The 2026-06-04 O4z audit lane stayed semantically green on generated inputs (`998` normalized matches, `0` mismatches, and `2` known Binaryen empty-recursion-group command failures) and sampled Starshine pass-local time on `tests/node/dist/starshine-debug-wasi.wasm` at about `63-67 ms` versus Binaryen's `109-110 ms` debug pass time. The 2026-06-07 closeout lane at `.tmp/pass-fuzz-local-cse-final-100000` requested `100000` cases and compared `99710`, with `99709` normalized matches, `0` validation/property/generator failures, `290` tool/Binaryen command failures, and `1` agent-classified semantic-safe size-winning representation mismatch where Binaryen expands an unreachable multi-value `tuple.drop` into scratch locals while Starshine preserves the smaller trapping shape.

The 2026-06-04 audit found direct adjacent-window parity gaps and coverage-positive state/effect cases: Binaryen can reuse a repeated tree computed before an `if` inside the `then` arm, before a straight-line block inside that block body, before a `try_table` inside the try body, across `return_call` / `return_call_indirect` / `return_call_ref` / `throw_ref` into unreachable continuation code, across `br_on_null` / `br_on_non_null` / `br_on_cast` / `br_on_cast_fail` into fallthrough continuation code, and across `struct.set` / `array.set` / `array.fill` / `array.copy` / `array.init_data` / `array.init_elem` / `memory.copy` / `memory.fill` / `memory.init` / `memory.grow` / `memory.size` / `table.get` / `table.copy` / `table.fill` / `table.init` / `table.set` / `table.grow` / `table.size` / `global.set` / `data.drop` / `elem.drop` for local-only repeated expressions. Starshine now covers and implements those narrow raw/module positives while preserving after-`if`, else-arm, loop, `br`, `br_table`, plain return, plain throw, top-level `unreachable`, and `try_table` body hard-terminator boundary negatives. The 2026-06-06 scalar-root follow-ups fixed repeated integer-to-float conversion roots across all `f32.convert_i32/i64_{s,u}` and `f64.convert_i32/i64_{s,u}` variants, `[O4Z-AUDIT-LCSE-PARITY]001` materializes trap-sensitive scalar numeric roots for Binaryen parity across `i32/i64.div_{s,u}`, `i32/i64.rem_{s,u}`, and non-saturating `i32/i64.trunc_f32/f64_{s,u}` repeats, `[O4Z-AUDIT-LCSE-PARITY]002` materializes repeated `ref.test` predicate roots whose temp-local result type is the defaultable scalar `i32`, `[O4Z-AUDIT-LCSE-PARITY]003` materializes repeated `any.convert_extern` / `extern.convert_any` roots with defaultable nullable `anyref` / `externref` temp locals, `[O4Z-AUDIT-LCSE-PARITY]004` materializes non-relaxed pure SIMD roots with `v128` or scalar temp locals while also removing the previous dropped-unreachable/block-debris mismatch family, and `[O4Z-AUDIT-LCSE-PARITY]005` materializes repeated nullable `ref.cast` roots directly plus repeated non-null `ref.cast` and `ref.as_non_null` roots by caching the defaultable nullable operand and replaying the checked operation at reuse sites, `[O4Z-AUDIT-LCSE-PARITY]006` materializes repeated `ref.i31` roots by caching the defaultable `i32` operand and replaying `ref.i31`, `[O4Z-AUDIT-LCSE-PARITY]007` materializes repeated GC heap-read roots (`struct.get`, packed struct gets, `array.get`, packed array gets, and `array.len`) when the result local is defaultable, with a narrow heap-read invalidation bit and a non-default result deferral guard, and `[O4Z-AUDIT-LCSE-PARITY]008` materializes repeated SIMD memory load roots (`v128.load`, lane loads, load-splat, load-zero, and load-extend) while preserving local-only reuse across scalar/SIMD memory writes and invalidating memory-read candidates across writes/grow, and `[O4Z-AUDIT-LCSE-PARITY]009` implements repeated `ref.get_desc` descriptor-read reuse with operand-cache replay for defaultable descriptor operands instead of appending non-default descriptor result locals. A 2026-06-07 follow-up implemented the Binaryen-current nullable `ref.cast_desc_eq` root: Starshine now decodes/encodes the current custom-descriptor opcodes and treats the root as a two-operand defaultable nullable reference-result candidate. A later same-day slice reopened the non-null `ref.cast_desc_eq` gap and implements it with two-operand replay: Starshine caches both defaultable operands at the first occurrence and replays the checked non-null descriptor cast at replacement sites, avoiding invalid non-default result locals while preserving trap behavior. Obsolete/unsupported `ref.test_desc` roots remain tool-blocked and deferred. The same day, a reopened call-boundary slice proved Binaryen reuses local-only arithmetic across `call_indirect` and `call_ref`, including result-dropping indirect calls; Starshine now preserves only local-only candidates across those call forms while still refusing to CSE `call_indirect` / `call_ref` roots or call-result-dependent expressions. A later atomic-boundary slice applies that same local-only precision across linear-memory atomics and `struct.atomic.get*` while keeping atomic roots and atomic-dependent expressions non-reusable. After the closeout was corrected/reopened, relaxed SIMD roots were also implemented as Binaryen behavior parity: the explicit `relaxed_simd_behavior_tests` array covers all 20 locally modeled relaxed SIMD opcodes, and the direct spot/comparison evidence shows Binaryen and Starshine both materialize those repeated roots with `v128` temp locals. The final 2026-06-07 conservative-lanes reopening then added local-only reuse across `ref.null`, `ref.func`, `string.const`, locally modeled string array/encode roots, ordinary GC allocations, descriptor allocations, direct/imported/indirect/reference-call boundaries, and atomic roots while preserving no-CSE root/dependent coverage for strings, allocations, calls, atomics, disjoint memory/table writes, and tool-blocked descriptor-test/string-proposal text-oracle lanes; `.tmp/pass-fuzz-local-cse-conservative-lanes-20260607-10000` compared `9977/10000` cases with `9977` normalized matches and `0` mismatches. A same-day pass-local performance matrix after the conservative-lane changes measured five timing-only samples on the 7.2M debug-WASI artifact plus the eight restored `json-as` example artifacts; every artifact cleared the stricter user-requested `<=1.5x` Binaryen cap, with median ratios from `0.250x` to `0.856x` and no LCSE code change needed. Trap-sensitive reuse remains scoped to identical repeated whole trees under the existing local window/effect invalidation model: if the first evaluation traps, later replacement is unobservable; if it does not trap, the unchanged identical repeat would not trap either. `ref.test`, reference-conversion, `ref.cast`, `ref.as_non_null`, `ref.i31`, `ref.get_desc`, nullable and non-null `ref.cast_desc_eq`, non-relaxed and relaxed SIMD, heap-read, SIMD memory-load, and local-only indirect/reference-call-boundary reuse are likewise reuse over unchanged operands and exact instruction identities, not broad descriptor test reasoning, string-root materialization, atomic-root CSE, arbitrary call-root CSE, or CFG-wide heap/memory GVN widening.

The relaxed-SIMD bullet is deliberately **Binaryen-oracle-scoped**. The feature's ordinary `v128` typing and codec support do not establish a generic evaluation-merging law: the current proposal overview permits host-dependent result sets for some relaxed operations while scoping them with a same-environment projection model. The existing all-20-opcode local-CSE slice remains supported by explicit Binaryen comparison/replay evidence, but any new relaxed-SIMD rewrite needs separate formal-semantics and oracle proof. See [`../../../raw/wasm/2026-07-10-relaxed-simd-execution-semantics-recheck.md`](../../../raw/wasm/2026-07-10-relaxed-simd-execution-semantics-recheck.md) and [`../../../wast/simd-authoring.md`](../../../wast/simd-authoring.md).
...
- keep non-relaxed pure SIMD arithmetic/logic, comparison, splat, replace-lane, shuffle/swizzle, any-true/all-true/bitmask, extended integer arithmetic, float min/max, float rounding, widening/narrowing, conversion, lane-extract roots, and the locally modeled relaxed SIMD roots as implemented Binaryen-parity cases with `v128` or scalar temp locals; keep SIMD memory loads (`v128.load*`, lane-loads, load-splat/load-zero/load-extend) as implemented same-window memory-read parity with scalar/SIMD memory-write invalidation and local-only reuse across those writes
- keep `ref.null`, `string.const`, `ref.func`, and string new-array / encode-array roots unmaterialized while still preserving local-only candidates across them; fresh 2026-06-07 `string.const` evidence was Binaryen-no-op for root materialization, and installed external text tools still reject parts of the stringrefs array/encode proposal surface
- keep the before-`if` / then-arm, before-block / straight-line block, before-`try_table` / try-body, `return_call` / `return_call_indirect` / `return_call_ref` / `throw_ref` unreachable-continuation positives, `br_on_null` / `br_on_non_null` / `br_on_cast` / `br_on_cast_fail` fallthrough-continuation positives, unrelated `local.set` / `local.tee` precision positives, ordinary/imported direct-call plus `call_indirect` / `call_ref` local-only precision positives, string/allocation/atomic local-only barrier positives, `struct.set` / `array.set` / `array.fill` / `array.copy` / `array.init_data` / `array.init_elem` / `memory.copy` / `memory.fill` / `memory.init` / `memory.grow` / `memory.size` / `table.get` / `table.copy` / `table.fill` / `table.init` / `table.set` / `table.grow` / `table.size` / `global.set` / `data.drop` / `elem.drop` local-only reuse positives, try-body hard-terminator negative, annotated idempotent direct-call positive, ordinary/imported/multi-result direct-call plus `call_indirect` and `call_ref` root/dependent negative coverage, disjoint memory/table write no-GVN coverage, descriptor allocation no-CSE coverage, and atomic root no-CSE coverage green while treating only tool-blocked descriptor/string proposal frontiers and non-semantic exact-shape parity as remaining documentation work
...
- active pass implementation and tests
  ...
  - `src/passes/local_cse_test.mbt`
    - covers registry, same-window arithmetic, parent-over-child, load/store and local-write barriers, integer-to-float conversion reuse, trap-sensitive scalar numeric reuse for integer division, integer remainder, and non-saturating float-to-int trunc conversion roots, `ref.test` predicate-root reuse, reference-conversion reuse for `any.convert_extern` / `extern.convert_any`, nullable and non-null `ref.cast` root reuse, `ref.as_non_null` root reuse, `ref.i31` root reuse, `ref.get_desc` operand-cache replay, nullable and non-null `ref.cast_desc_eq` reuse, non-relaxed pure SIMD reuse, relaxed-SIMD root reuse, dropped-unreachable/block-debris cleanup before hard `unreachable`, before-`if` / then-arm reuse, before-block / straight-line block reuse, before-`try_table` / try-body reuse, `return_call` / `return_call_indirect` / `return_call_ref` / `throw_ref` unreachable-continuation reuse, `br_on_null` / `br_on_non_null` / `br_on_cast` / `br_on_cast_fail` fallthrough-continuation reuse, `struct.set` / `array.set` / `array.fill` / `array.copy` local-only reuse, try-body hard-terminator clearing, annotated idempotent direct-call reuse, ordinary/imported/multi-result direct-call, `call_indirect`, and `call_ref` root/dependent no-ops plus local-only reuse across ordinary/imported/indirect/reference-call boundaries and scoped annotated-idempotent direct-call root reuse, after-`if` and else-arm negatives, before-loop into loop-body, `br`/`br_table`, return-boundary, unreachable-boundary, tiny-root `global.get` no-op, and repeated `struct.new` / `struct.new_default` / descriptor `struct.new_desc` / `struct.new_default_desc` / core `array.new` / `array.new_default` / `array.new_fixed` / `array.new_data` / `array.new_elem` generative-root coverage, `string.const`, `ref.null`, and `ref.func` no-op coverage, local-only reuse across those roots, and string new-array / encode-array root deferral plus local-only barrier coverage, and i31 coverage for `i31.get_s` / `i31.get_u` reuse plus `ref.i31` operand-cache replay, plus struct/array heap-read, array length, and packed heap-read root reuse with heap-write invalidation coverage, and descriptor allocation root deferral plus local-only barrier coverage, disjoint memory/table write no-GVN coverage, linear atomic local-only reuse coverage including load/store, RMW, wait/notify/fence, and cmpxchg variants, repeated linear atomic root no-CSE coverage, relaxed-SIMD root reuse coverage, SIMD memory-load reuse coverage across `v128.load`, lane-load, load-splat, load-zero, and load-extend roots, SIMD memory-write invalidation for memory-read roots, and scalar local-only reuse across scalar/SIMD memory stores including `v128.store8_lane` / `v128.store16_lane` / `v128.store32_lane` / `v128.store64_lane`
...
- completed backlog and release note
  - `agent-todo.md`
    - no longer carries an active LCSE item; completed parity/performance evidence lives in this wiki dossier and the 2026-06-04/07 research note
  - `CHANGELOG.md`
...
The implementation covers same-window temp-localizing reuse for repeated local arithmetic trees and nontrapping scalar conversion roots, preserves barrier resets for local writes and non-idempotent calls in the raw path, and is protected by direct pass tests plus fuzz/self-optimize evidence. It also implements Binaryen's adjacent before-`if` into `then`, before-block into straight-line block, before-`try_table` into try-body and try-body hard-terminator clearing, `return_call` / `return_call_indirect` / `return_call_ref` / `throw_ref` unreachable-continuation reuse, `br_on_null` / `br_on_non_null` / `br_on_cast` / `br_on_cast_fail` fallthrough-continuation reuse, `struct.set` / `array.set` / `array.fill` / `array.copy` local-only reuse, local-only reuse across ordinary direct-call, `call_indirect`, and `call_ref` boundaries, integer-to-float conversion root reuse, `ref.test` predicate-root reuse, and annotated idempotent direct-call reuse windows while keeping ordinary direct-call, `call_indirect`, and `call_ref` roots unmaterialized.
...
1. Keep the landed direct tests green
   ...
   - before-`try_table` / try-body, before-block / straight-line block, `return_call` / `return_call_indirect` / `return_call_ref` / `throw_ref` unreachable-continuation positives, the `br_on_null` / `br_on_non_null` / `br_on_cast` / `br_on_cast_fail` fallthrough-continuation positives, ordinary direct-call / `call_indirect` / `call_ref` local-only reuse positives, and the `struct.set` / `array.set` / `array.fill` / `array.copy` / `array.init_data` / `array.init_elem` / `memory.copy` / `memory.fill` / `memory.init` / `memory.grow` / `memory.size` / `table.get` / `table.copy` / `table.fill` / `table.init` / `table.set` / `table.grow` / `table.size` / `global.set` / `data.drop` / `elem.drop` local-only reuse positives
   ...
   - add remaining hard control-boundary negatives and additional GC/generative-root negatives only as focused source-backed slices; repeated `call_indirect` and `call_ref` roots already have direct no-reuse coverage, and their local-only boundary positives are now implemented
