...
- keep `ref.test` predicate-root parity as an implemented scalar-`i32` result case, `any.convert_extern` / `extern.convert_any` parity as implemented defaultable nullable reference-result cases, and `ref.cast` parity as implemented for nullable result locals plus non-null casts replayed from a cached defaultable nullable operand, while leaving `ref.as_non_null`, descriptor reads, descriptor casts/tests, and other non-null/descriptor reference reasoning as documented conservative deferrals unless a separate safe root model is approved
...
- active pass implementation and tests
  ...
  - `src/passes/local_cse_test.mbt`
    - covers registry, same-window arithmetic, parent-over-child, load/store and local-write barriers, integer-to-float conversion reuse, trap-sensitive scalar numeric reuse for integer division, integer remainder, and non-saturating float-to-int trunc conversion roots, `ref.test` predicate-root reuse, reference-conversion reuse for `any.convert_extern` / `extern.convert_any`, nullable and non-null `ref.cast` root reuse, non-relaxed pure SIMD reuse, dropped-unreachable/block-debris cleanup before hard `unreachable`, before-`if` / then-arm reuse, before-block / straight-line block reuse, before-`try_table` / try-body reuse, `return_call` / `return_call_indirect` / `return_call_ref` / `throw_ref` unreachable-continuation reuse, `br_on_null` / `br_on_non_null` / `br_on_cast` / `br_on_cast_fail` fallthrough-continuation reuse, `struct.set` / `array.set` / `array.fill` / `array.copy` local-only reuse, try-body hard-terminator clearing, annotated idempotent direct-call reuse, ordinary direct-call, `call_indirect`, and `call_ref` root no-ops, after-`if` and else-arm negatives, before-loop into loop-body, `br`/`br_table`, return-boundary, unreachable-boundary, tiny-root `global.get` no-op, and repeated `struct.new` / `struct.new_default` / descriptor `struct.new_desc` / `struct.new_default_desc` / core `array.new` / `array.new_default` / `array.new_fixed` / `array.new_data` / `array.new_elem` generative-root coverage, `string.const` and `ref.func` no-op coverage, string new-array / encode-array root deferral coverage, `ref.as_non_null` nullability-root deferral coverage, and i31 coverage split between `i31.get_s` / `i31.get_u` reuse and `ref.i31` non-null-temp deferral, plus struct/array heap-read, array length, and packed heap-read root deferral coverage, and descriptor allocation root deferral coverage, linear atomic boundary coverage including store, wait/notify/fence, and cmpxchg roots, relaxed-SIMD root deferral coverage, SIMD load-root boundary coverage, SIMD lane-load-root boundary coverage across `v128.load8_lane` / `v128.load16_lane` / `v128.load32_lane` / `v128.load64_lane`, and SIMD lane-store local-only boundary coverage across `v128.store8_lane` / `v128.store16_lane` / `v128.store32_lane` / `v128.store64_lane`
