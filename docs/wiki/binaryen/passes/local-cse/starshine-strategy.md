...
- keep linear atomic operations, including load width variants, store width variants, RMW op/width variants, wait/notify/fence, cmpxchg roots, and cmpxchg width variants, and packed shared-GC atomic accessors grouped with shared-GC atomics as documented conservative boundaries rather than adding atomic or memory GVN
...
- keep `ref.null`, `string.const`, `ref.func`, and string new-array / encode-array roots unmaterialized unless a separate string/reference temp-local safety story is approved
