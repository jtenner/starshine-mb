...
- keep non-relaxed pure SIMD arithmetic/logic, comparison, splat, replace-lane, shuffle/swizzle, any-true/all-true/bitmask, extended integer arithmetic, float min/max, float rounding, widening/narrowing, conversion, and lane-extract roots as implemented Binaryen-parity cases with `v128` or scalar temp locals; keep SIMD memory loads (`v128.load*`, lane-loads, load-splat/load-zero/load-extend) as implemented same-window memory-read parity with scalar/SIMD memory-write invalidation and local-only reuse across those writes; keep relaxed SIMD roots conservative unless a separate nondeterminism model is approved
