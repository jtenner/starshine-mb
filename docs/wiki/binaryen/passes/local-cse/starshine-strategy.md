...
- keep repeated `struct.get` / `array.get` / `array.len` roots, including packed signed/unsigned accessors, as a documented heap-read deferral, not a shortcut into heap GVN
