...
- keep `ref.null`, `string.const`, `ref.func`, and string new-array / encode-array roots unmaterialized unless a separate string/reference temp-local safety story is approved; fresh 2026-06-07 binary `string.const` evidence was Binaryen-no-op while installed `wasm-tools` still rejected the stringrefs section
- keep the before-`if` / then-arm, before-block / straight-line block, before-`try_table` / try-body, `return_call` / `return_call_indirect` / `return_call_ref` / `throw_ref` unreachable-continuation positives, `br_on_null` / `br_on_non_null` / `br_on_cast` / `br_on_cast_fail` fallthrough-continuation positives, unrelated `local.set` / `local.tee` precision positives, ordinary direct-call plus `call_indirect` / `call_ref` local-only precision positives, `struct.set` / `array.set` / `array.fill` / `array.copy` / `array.init_data` / `array.init_elem` / `memory.copy` / `memory.fill` / `memory.init` / `memory.grow` / `memory.size` / `table.get` / `table.copy` / `table.fill` / `table.init` / `table.set` / `table.grow` / `table.size` / `global.set` / `data.drop` / `elem.drop` local-only reuse positives, try-body hard-terminator negative, annotated idempotent direct-call positive, and ordinary direct-call plus `call_indirect` and `call_ref` root negative coverage green while hardening the remaining control-boundary and GC/generative-root surfaces one focused slice at a time
...
- active pass implementation and tests
  ...
  - `src/passes/local_cse_test.mbt`
    - covers registry, same-window arithmetic, parent-over-child, load/store and local-write barriers, integer-to-float conversion reuse, trap-sensitive scalar numeric reuse for integer division, integer remainder, and non-saturating float-to-int trunc conversion roots, `ref.test` predicate-root reuse, reference-conversion reuse for `any.convert_extern` / `extern.convert_any`, nullable and non-null `ref.cast` root reuse, `ref.as_non_null` root reuse, `ref.i31` root reuse, `ref.get_desc` operand-cache replay, nullable and non-null `ref.cast_desc_eq` reuse, non-relaxed pure SIMD reuse, relaxed-SIMD root reuse, dropped-unreachable/block-debris cleanup before hard `unreachable`, before-`if` / then-arm reuse, before-block / straight-line block reuse, before-`try_table` / try-body reuse, `return_call` / `return_call_indirect` / `return_call_ref` / `throw_ref` unreachable-continuation reuse, `br_on_null` / `br_on_non_null` / `br_on_cast` / `br_on_cast_fail` fallthrough-continuation reuse, `struct.set` / `array.set` / `array.fill` / `array.copy` local-only reuse, try-body hard-terminator clearing, annotated idempotent direct-call reuse, ordinary direct-call, `call_indirect`, and `call_ref` root no-ops plus local-only reuse across those call boundaries, after-`if` and else-arm negatives, before-loop into loop-body, `br`/`br_table`, return-boundary, unreachable-boundary, tiny-root `global.get` no-op, and repeated `struct.new` / `struct.new_default` / descriptor `struct.new_desc` / `struct.new_default_desc` / core `array.new` / `array.new_default` / `array.new_fixed` / `array.new_data` / `array.new_elem` generative-root coverage, `string.const` and `ref.func` no-op coverage, string new-array / encode-array root deferral coverage, and i31 coverage for `i31.get_s` / `i31.get_u` reuse plus `ref.i31` operand-cache replay, plus struct/array heap-read, array length, and packed heap-read root reuse with heap-write invalidation coverage, and descriptor allocation root deferral coverage, linear atomic local-only reuse coverage including load/store, RMW, wait/notify/fence, and cmpxchg variants, relaxed-SIMD root reuse coverage, SIMD memory-load reuse coverage across `v128.load`, lane-load, load-splat, load-zero, and load-extend roots, SIMD memory-write invalidation for memory-read roots, and scalar local-only reuse across scalar/SIMD memory stores including `v128.store8_lane` / `v128.store16_lane` / `v128.store32_lane` / `v128.store64_lane`
