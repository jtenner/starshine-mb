...
| `src/passes/local_cse_test.mbt` | Direct registry and behavior tests for repeated trees, parent-over-child cancellation, load barriers, tiny-root `global.get` no-ops, `struct.new`, `struct.new_default`, and core `array.new` / `array.new_default` / `array.new_fixed` generative-root no-ops, annotated idempotent direct-call reuse plus ordinary direct-call, `call_indirect`, and `call_ref` root no-ops, `return_call_indirect`, `return_call_ref`, and `throw_ref` unreachable-continuation reuse, local-write window resets, Binaryen's before-`if` into `then`, before-block into straight-line block, and before-`try_table` into try-body positives, paired after-`if` / else-arm negatives, and before-loop into loop-body plus `br` / `br_table` / `return` / `throw` / top-level `unreachable` and `try_table` body unreachable boundary negatives. |
...
- continued source-derived tests for any additional GC-generative allocation variants beyond the covered `struct.new` / `struct.new_default` / `array.new` / `array.new_default` / `array.new_fixed` roots,
