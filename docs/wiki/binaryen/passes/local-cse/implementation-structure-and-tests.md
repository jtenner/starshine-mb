...
| `src/passes/local_cse_test.mbt` | Direct registry and behavior tests for repeated trees, parent-over-child cancellation, load barriers, tiny-root `global.get` no-ops, `struct.new` generative-root no-op, local-write window resets, Binaryen's before-`if` into `then` positive, paired after-`if` / else-arm negatives, and before-loop into loop-body plus `br_table` / `return` / `unreachable` boundary negatives. |
...
- continued regression coverage for the before-`if` into `then` reuse positive fixed after the 2026-06-04 audit,
...
- continued source-derived tests for idempotent direct calls and additional GC-generative allocation rejection beyond the covered `struct.new` root,
