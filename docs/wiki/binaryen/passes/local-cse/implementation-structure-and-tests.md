...
| `src/passes/local_cse_test.mbt` | Direct registry and behavior tests for repeated trees, parent-over-child cancellation, load barriers, tiny-root `global.get` no-ops, `struct.new`, `struct.new_default`, descriptor `struct.new_desc` / `struct.new_default_desc`, and core `array.new` / `array.new_default` / `array.new_fixed` / `array.new_data` / `array.new_elem` generative-root no-ops, annotated idempotent direct-call reuse plus ordinary direct-call, `call_indirect`, and `call_ref` root no-ops, SIMD pure/load/lane-load/load-splat/load-zero/load-extend/store deferrals, `return_call`, `return_call_indirect`, `return_call_ref`, and `throw_ref` unreachable-continuation reuse, `br_on_null` / `br_on_non_null` / `br_on_cast` / `br_on_cast_fail` fallthrough-continuation reuse, `struct.set` / `array.set` / `array.fill` / `array.copy` / `array.init_data` / `array.init_elem` / `memory.copy` / `memory.fill` / `memory.init` / `memory.grow` / `memory.size` plus conservative struct/array heap-read and array-length roots, shared-GC atomic including packed atomic accessors, and linear atomic local-only deferrals including wait/notify/fence / `table.get` / `table.copy` / `table.fill` / `table.init` / `table.set` / `table.grow` / `table.size` / `global.set` / `data.drop` / `elem.drop` local-only reuse, integer equality/eqz, integer division/remainder and trap-sensitive trunc-conversion deferrals, nontrapping numeric conversion, sign-extension, saturating trunc, `i31.get_s` / `i31.get_u` root reuse with `ref.i31` deferral, full-width non-i32 load, narrow-load root reuse, heap-read root deferral, array-length root deferral, packed heap-read root deferral, linear atomic boundary deferral including wait/notify/fence, SIMD pure-root deferral, SIMD load-root deferral, SIMD load-splat/load-zero/load-extend deferrals, SIMD lane-load-root deferral, and SIMD store local-only deferral, `select` and `ref.is_null` root reuse plus `string.const` and `ref.func` no-op coverage, string new-array / encode-array root deferral coverage, `ref.test` / `ref.as_non_null` / `any.convert_extern` / `extern.convert_any` / `ref.cast` / `ref.get_desc` / descriptor test-cast deferrals, local-write window resets, Binaryen's before-`if` into `then`, before-block into straight-line block, and before-`try_table` into try-body positives, paired after-`if` / else-arm negatives, and before-loop into loop-body plus `br` / `br_table` / `return` / `throw` / top-level `unreachable` and `try_table` body unreachable boundary negatives. |
