...
| --- | --- |
| `src/passes/local_cse.mbt` | Active Starshine owner file for direct `local-cse` execution, including the summary, descriptor, raw/module rewrite pipeline, nested adjacent-window support for straight-line block / `try_table` bodies, callee annotation lookup for the narrow idempotent direct-call exception, `return_call`, `return_call_indirect`, `return_call_ref`, and `throw_ref` operand modeling plus `br_on_null` fallthrough stack passthrough, `br_on_non_null` operand modeling, `br_on_cast` / `br_on_cast_fail` fallthrough stack modeling for Binaryen's continuation reuse shapes, `struct.set` / `array.set` / `array.fill` / `array.copy` / `array.init_data` / `array.init_elem` operand modeling for local-only reuse across GC writes, `memory.copy` / `memory.fill` / `memory.init`, pure `ref.eq` operand/result modeling, `table.copy` / `table.fill` / `table.init` / `table.set`, and `global.set` operand modeling, `memory.size` stack-result modeling plus side-effecting `memory.grow` stack-result modeling with memory-size invalidation, side-effecting `table.grow` plus `table.get` / `table.size` stack-result modeling without table-root reuse, and zero-operand `data.drop` / `elem.drop` side-effect modeling for local-only reuse across linear-memory/table/global/segment effects, module-pass entry, and main HotPass rewrite pipeline. |
