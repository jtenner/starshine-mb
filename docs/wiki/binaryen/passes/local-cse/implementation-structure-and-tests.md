...
| `src/passes/local_cse_test.mbt` | Direct registry and behavior tests for repeated trees, parent-over-child cancellation, load barriers, tiny-root `global.get` no-ops, local-write window resets, Binaryen's before-`if` into `then` positive, paired after-`if` / else-arm negatives, and before-loop into loop-body plus return-boundary negatives. |
...
- durable before-loop into loop-body and return-boundary negative coverage,
