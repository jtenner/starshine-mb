...
- `src/passes/LocalCSE.cpp` verified comparison baseline: <https://github.com/WebAssembly/binaryen/blob/version_132/src/passes/LocalCSE.cpp>
...
The verified v132 `Applier` uses the repeated expression's full result type for
the new local. Starshine mirrors that shape for ordinary non-null `ref.cast`
roots in the outer function body, including nominal and exact heap types. It
keeps the original cast before the tee and leaves nested structured-control
rewrites on operand replay, so definite initialization remains local to a
straight-line function-body window.
...
| --- | --- |
| `src/passes/local_cse.mbt` | Active Starshine owner file for direct `local-cse` execution, including the summary, descriptor, call-dependence tracking, allocation/string local-only barrier modeling, raw/module rewrite pipeline, outer-function-window ordinary non-null `ref.cast` result materialization with full nullable/exact/heap result typing, nested adjacent-window support for straight-line block / `try_table` bodies, callee annotation lookup for the narrow idempotent direct-call exception, `return_call`, `return_call_indirect`, `return_call_ref`, and `throw_ref` operand modeling plus `br_on_null` fallthrough stack passthrough, `br_on_non_null` operand modeling, `br_on_cast` / `br_on_cast_fail` fallthrough stack modeling for Binaryen's continuation reuse shapes, `struct.set` / `array.set` / `array.fill` / `array.copy` / `array.init_data` / `array.init_elem` operand modeling for local-only reuse across GC writes, `memory.copy` / `memory.fill` / `memory.init`, pure integer equality / eqz / ordering comparison and bit-op, float arithmetic/equality/comparison, nontrapping numeric conversions, sign-extension operations, saturating trunc conversions, `i31.get_s` / `i31.get_u` scalar accessor roots, full-width and narrow-load root coverage, `select`, `ref.is_null`, and `ref.eq` operand/result modeling, `table.copy` / `table.fill` / `table.init` / `table.set`, and `global.set` operand modeling, `memory.size` stack-result modeling plus side-effecting `memory.grow` stack-result modeling with memory-size invalidation, side-effecting `table.grow` plus `table.get` / `table.size` stack-result modeling without table-root reuse, and zero-operand `data.drop` / `elem.drop` side-effect modeling for local-only reuse across linear-memory/table/global/segment effects, module-pass entry, and main HotPass rewrite pipeline. |
| `src/passes/local_cse_test.mbt` | Direct registry and behavior tests for repeated trees, parent-over-child cancellation, load barriers, tiny-root `global.get` no-ops, case29-shaped nominal non-null cast result reuse, potentially failing cast-before-tee order, exact result-local typing, source-local write invalidation, nested-control operand replay, `struct.new`, `struct.new_default`, descriptor `struct.new_desc` / `struct.new_default_desc`, and core `array.new` / `array.new_default` / `array.new_fixed` / `array.new_data` / `array.new_elem` generative-root no-ops plus local-only barrier coverage, annotated idempotent direct-call reuse plus ordinary/imported/multi-result direct-call, `call_indirect`, and `call_ref` root/dependent no-ops, SIMD pure/load/lane-load/load-splat/load-zero/load-extend/store deferrals, `return_call`, `return_call_indirect`, `return_call_ref`, and `throw_ref` unreachable-continuation reuse, `br_on_null` / `br_on_non_null` / `br_on_cast` / `br_on_cast_fail` fallthrough-continuation reuse, `struct.set` / `array.set` / `array.fill` / `array.copy` / `array.init_data` / `array.init_elem` / `memory.copy` / `memory.fill` / `memory.init` / `memory.grow` / `memory.size` plus conservative struct/array heap-read and array-length roots, shared-GC atomic including packed atomic accessors, and linear atomic local-only deferrals including wait/notify/fence / `table.get` / `table.copy` / `table.fill` / `table.init` / `table.set` / `table.grow` / `table.size` / `global.set` / `data.drop` / `elem.drop` local-only reuse, integer equality/eqz, integer division/remainder and trap-sensitive trunc-conversion deferrals, nontrapping numeric conversion, sign-extension, saturating trunc, `i31.get_s` / `i31.get_u` root reuse with `ref.i31` deferral, full-width non-i32 load, narrow-load root reuse, heap-read root deferral, array-length root deferral, packed heap-read root deferral, linear atomic boundary deferral including repeated-root no-CSE, wait/notify/fence, SIMD pure-root deferral, SIMD load-root deferral, SIMD load-splat/load-zero/load-extend deferrals, SIMD lane-load-root deferral, and SIMD store local-only deferral, `select` and `ref.is_null` root reuse plus `string.const`, `ref.null`, and `ref.func` no-op coverage plus local-only reuse across those roots, string new-array / encode-array root deferral plus local-only barrier coverage, `ref.test` / `ref.as_non_null` / `any.convert_extern` / `extern.convert_any` / `ref.cast` / `ref.get_desc` / descriptor test-cast deferrals, local-write window resets, disjoint memory/table write no-GVN coverage, Binaryen's before-`if` into `then`, before-block into straight-line block, and before-`try_table` into try-body positives, paired after-`if` / else-arm negatives, and before-loop into loop-body plus `br` / `br_table` / `return` / `throw` / top-level `unreachable` and `try_table` body unreachable boundary negatives. |
| `src/passes/local_cse_round2_test.mbt` / `src/cmd/local_cse_fourth_audit_wbtest.mbt` | Direct Core-AST and active-dispatch regressions require nested struct/array atomic writes and ordinary `ArrayStore` writes to invalidate pre-write heap reads. |
| `src/passes/local_cse_waitqueue_test.mbt` / `src/cmd/local_cse_waitqueue_wbtest.mbt` | Valid direct and active-dispatch fixtures require nested `waitqueue.new`, `waitqueue.notify`, and `struct.wait` operations to invalidate borrowed shared-heap reads while preserving module validity. They also document the implementation boundary: top-level raw scanning and HOT effects were already conservative; the repaired path is adjacent block/`try_table` scanning. |
...
- continued regression coverage for the before-`if` into `then`, simple before-block into straight-line block, before-`try_table` into try-body, `return_call` / `return_call_indirect` / `return_call_ref` / `throw_ref` unreachable-continuation reuse, `br_on_null` / `br_on_non_null` / `br_on_cast` / `br_on_cast_fail` fallthrough-continuation reuse, and `struct.set` / `array.set` / `array.fill` / `array.copy` / `array.init_data` / `array.init_elem` / `memory.copy` / `memory.fill` / `memory.init` / `memory.grow` / `memory.size` plus conservative struct/array heap-read and array-length roots, shared-GC atomic including packed atomic accessors, and linear atomic local-only deferrals including wait/notify/fence / `table.get` / `table.copy` / `table.fill` / `table.init` / `table.set` / `table.grow` / `table.size` / `global.set` / `data.drop` / `elem.drop` local-only reuse positives, integer equality/eqz, integer division/remainder and trap-sensitive trunc-conversion deferrals, nontrapping numeric conversion, sign-extension, saturating trunc, `i31.get_s` / `i31.get_u` root reuse plus `ref.i31` non-null-temp deferral, full-width non-i32 load, narrow-load coverage, heap-read root deferral, array-length root deferral, packed heap-read root deferral, linear atomic boundary deferral including wait/notify/fence, SIMD pure-root deferral, SIMD load-root deferral, and SIMD lane-load-root deferral, `select` root coverage, `ref.is_null` pure-null-test reuse, `string.const` and `ref.func` no-op coverage, string new-array / encode-array root deferral coverage, `ref.test` / `ref.as_non_null` / `any.convert_extern` / `extern.convert_any` / `ref.cast` / `ref.get_desc` / descriptor test-cast deferrals, and `ref.eq` pure-reference reuse fixed or covered after the 2026-06-04 audit,
...
- continued source-derived tests for any additional GC-generative allocation variants beyond the covered `struct.new` / `struct.new_default` / descriptor `struct.new_desc` / `struct.new_default_desc` / `array.new` / `array.new_default` / `array.new_fixed` / `array.new_data` / `array.new_elem` roots,
