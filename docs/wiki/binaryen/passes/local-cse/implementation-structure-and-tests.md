...
| `src/passes/local_cse_test.mbt` | Direct registry and behavior tests for repeated trees, parent-over-child cancellation, load barriers, tiny-root `global.get` no-ops, `struct.new`, `struct.new_default`, and core `array.new` / `array.new_default` generative-root no-ops, annotated idempotent direct-call reuse plus ordinary direct-call, `call_indirect`, and `call_ref` root no-ops, local-write window resets, Binaryen's before-`if` into `then`, before-block into straight-line block, and before-`try_table` into try-body positives, paired after-`if` / else-arm negatives, and before-loop into loop-body plus `br_table` / `return` / top-level `unreachable` and `try_table` body unreachable boundary negatives. |
...
- durable before-loop into loop-body, `br_table`, `return`, top-level `unreachable`, and `try_table` body unreachable boundary negative coverage,
- durable tiny-root no-op coverage for repeated `global.get`,
- continued source-derived tests for any additional GC-generative allocation variants beyond the covered `struct.new` / `struct.new_default` / `array.new` / `array.new_default` roots,
