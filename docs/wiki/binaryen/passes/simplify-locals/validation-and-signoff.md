...
## Historical Binaryen-v131 closeout
...
- `[O4Z-AUDIT-SL]` and the follow-up `[AUDIT]002` raw-threshold audit are closed. Current classification and focused boundary coverage live in [`../../pass-manager-threshold-guards.md`](../../pass-manager-threshold-guards.md); large-artifact timing remains under `[WALL]001` unless new evidence pins a regression to `simplify-locals` itself.
