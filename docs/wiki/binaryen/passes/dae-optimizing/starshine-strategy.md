...
- a cheap large-module-safe touched raw cleanup runs after DAE rewrites and strips `nop` plus dropped `i32.const 0` debris only from touched functions, improving generated output even when the full nested cleanup scheduler is size-guarded away.
