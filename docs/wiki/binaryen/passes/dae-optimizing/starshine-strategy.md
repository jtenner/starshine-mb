...
- [`agent-todo.md`](../../../../../agent-todo.md)
  ...
  - `[DAE]002` covers nested `optimizeAfterInlining` replay plus artifact comparison; its narrow scheduler now includes touched-only `optimize-instructions`, `pick-load-signs`, `heap-store-optimization`, `heap2local`, direct `precompute`, `local-cse`, `code-folding`, repeated `merge-blocks`, `remove-unused-brs`, and `remove-unused-names` cleanup, and the nested `local-cse` adapter now traces `func/start/done` so order tests can pin it before `simplify-locals`, but the full `precompute-propagate` + default-function-pipeline replay and artifact parity remain open.
