...
- [`agent-todo.md`](../../../../../agent-todo.md)
  ...
  - `[DAE]002` covers nested `optimizeAfterInlining` replay plus artifact comparison; its narrow scheduler now includes touched-only `local-cse`, repeated `merge-blocks`, `remove-unused-brs`, and `remove-unused-names` cleanup, but the full `precompute-propagate` + default-function-pipeline replay and artifact parity remain open.
