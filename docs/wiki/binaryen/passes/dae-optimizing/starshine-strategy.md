...
- a cheap large-module-safe post-touch raw cleanup runs after DAE rewrites and strips `nop` plus dropped pure leaf debris (`i32.const`, `i64.const`, `f32.const`, `f64.const`, `local.get`, and `global.get` followed by `drop`) across the module once DAE has touched at least one function, while preserving the original DAE touched set for the nested cleanup scheduler.
