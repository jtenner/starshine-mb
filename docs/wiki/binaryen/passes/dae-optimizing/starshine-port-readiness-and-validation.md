...
- one active validation blocker in simple function-type pruning: imported tag payload `TypeIdx` references are not marked live, so the saved wasm-smith `case-000140` imported-tag-only module fails final validation after direct DAE. This is a narrow mandatory fix, not an accepted non-goal;
