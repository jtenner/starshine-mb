...
- closure of the valid current artifact's remaining `+16055` canonical-byte size-losing gap after the Func-164 body/result win; note `1579` attributes the next exact-reference/removable-param chain to filtered Func-37 precompute plus Func-38/41 convergence, while note `1574` shows the general `large-touched-set touched=22` skip owns only `21` canonical bytes;
