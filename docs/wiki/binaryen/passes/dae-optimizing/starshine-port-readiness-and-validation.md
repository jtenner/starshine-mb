...
- a touched-only private `precompute-propagate-prefix` helper that folds SSA-backed default-init and direct local constant facts, derives constants through value-preserving `local.tee` and unbranched single-result block fallthrough while retaining effects and rejecting branch-valued blocks, then reruns plain `precompute`; this closes one real fallthrough family but is still not the complete public `precompute-propagate` pass contract;
...
Research note [`1583`](../../../raw/research/1583-2026-07-13-daeo-post-param-chain-gap-attribution.md) identifies the next direct owner after the exact-parameter chain: Binaryen removes Func `12293`'s dropped result, which unlocks Func `8429` result removal and selected local-copy cleanup. The result-only endpoint is size-losing. Adding selected cleanup is valid and improves raw/canonical/current-tool WAT size, but takes `33027.052ms` versus Binaryen `8083.49ms`; this exceeds the pass-local target and is not retained. The missing code surface is now a payoff-aware broad result-chain scheduler plus a bounded fast cleanup for the proven `structured-pure-copy-call-tail` family, not blanket replay.

Research note [`1584`](../../../raw/research/1584-2026-07-13-daeo-scheduled-mode-specific-blockers.md) splits the large scheduled blockers. `--optimize` reproduces a direct `vacuum` timeout before DAEO. `-O4z` avoids that raw vacuum route but still times out earlier in `ssa-nomerge`. Large `--shrink` remains unmeasured in this slice. Final scheduled artifact signoff therefore needs mode-specific bounded-prefix or performance repairs.
...
- closure of the valid current artifact's remaining `+15995` canonical-byte size-losing gap after note `1580` closed the Func-37/38/41 exact-reference/removable-param chain; note `1574` still shows the general large-touched-set skip is only a minor owner, so the next slice needs a newly attributed source-backed function/body family rather than blanket replay;
