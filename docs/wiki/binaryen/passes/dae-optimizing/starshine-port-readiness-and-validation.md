...
- no full default-function-pipeline replay on all touched functions, no real public `precompute-propagate` sibling, remaining `large-touched-set` / `large-touched-function` nested-cleanup guards, and no byte-parity debug-artifact compare yet;
