...
- a narrow constant-actual materialization slice for exact literals on read-only params, now including scalar memory-load sibling carriers, typed single-result `TypeIdxBlockType` wrappers, a guarded folded-multivalue suffix-only path, and a guarded literal-condition result-if plus dead-zero-if inter-argument bridge. The shared exact-literal machinery now covers that low-definition non-artifact family, while the selected Func323 path remains for its immutable-global condition and artifact cleanup sequence. Arbitrary inter-argument statements, nonliteral constant-condition facts, GC refinement, and result-refinement families remain open;
...
- `moon test src/passes` is signable after note `1563` fixed the shared zero-param multivalue drop-carrier `merge-blocks` abort, note `1564` genericized the folded-multivalue forwarded-default suffix family, and note `1565` added guarded literal-condition/dead-zero-if inter-argument recovery; the high-defined-function DAE fixtures assert durable post-cleanup semantics instead of pre-cleanup body shapes;
