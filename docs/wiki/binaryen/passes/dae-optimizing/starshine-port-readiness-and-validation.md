...
- focused regressions for touched-only nested cleanup order, size-skip tracing, large-module small-touched-set cleanup, the folded-multivalue Func3737 nested-cleanup lane, touched-only `optimize-casts` / `coalesce-locals` / `reorder-locals` behavior, a narrow every-direct-caller-same-literal constant-actual family, the new low-definition folded-multivalue forwarded-default genericization guard, and a small-module fact-discovered dropped-result candidate outside the artifact selected-def list in [`src/passes/dae_optimizing_test.mbt`](../../../../../src/passes/dae_optimizing_test.mbt);
...
- a narrow constant-actual materialization slice for exact literals on read-only params, now including scalar memory-load sibling carriers, typed single-result `TypeIdxBlockType` wrappers, and a guarded folded-multivalue suffix-only recovery path for forwarded defaults; the new suffix recognizer reuses the shared exact-literal machinery for the old Func3737/Func3765 family while still bailing out on the older Func323 inter-argument effect/debris boundary. Broader operand localization, GC refinement, and result-refinement families remain open;
...
- `moon test src/passes` is signable again after note `1563` fixed the shared zero-param multivalue drop-carrier `merge-blocks` abort exposed by the broader large-module lane, note `1564` genericized the folded-multivalue forwarded-default suffix family, and the high-defined-function DAE fixtures now assert durable post-cleanup semantics instead of pre-cleanup body shapes;
