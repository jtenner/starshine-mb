...
- closure of the valid current artifact's `+16350` canonical-byte size-losing gap through an efficient low-candidate/exact-reference worklist; note `1574` shows the `large-touched-set touched=22` skip owns only `21` canonical bytes;
