...
- Current behavior-gap evidence is research note [`1641`](../../../raw/research/1641-2026-07-17-daeo-func8184-null-guard-and-call-argument.md): selected structural cleanup folds exact adjacent `ref.null; ref.is_null` true arms and sinks an exact one-set/one-get nonnull global call argument only across the other argument's pure `local.get`. Func `8184` falls from canonical body `27` to Binaryen-exact `11`; the valid byte-identical endpoint is raw `3203066` / canonical `3263956`, `+1500` over Binaryen v130. Focused/full tests, native build, artifact validation/convergence, and dedicated/regular `1000`-case normalized smokes are green. Funcs `8184` and `8186` are closed; Func `8185 +35`, full closeout, and pre-slot public-pipeline blockers remain active.
