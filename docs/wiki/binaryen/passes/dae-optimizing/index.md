...
- Fresh post-tuple-cleanup direct evidence is green on native commit `cf08ff06f`. Research note [`1568`](../../../raw/research/1568-2026-07-13-daeo-fresh-dedicated-and-regular-compare.md) records the dedicated `dae-optimizing` profile at `10000/10000` normalized and regular GenValid at `100000/100000` normalized, with zero cleanup-normalized matches, raw mismatches, validation/generator/property/command failures, explicit DAE normalizers, `--jobs auto`, and `_build/native/release/build/cmd/cmd.exe`. Fresh wasm-smith, random-all, and scheduled O4z performance/artifact evidence remain open.
