...
- Public-prefix evidence is research note [`1646`](../../../raw/research/1646-2026-07-17-ssa-nomerge-batch-writeback.md): batching rollback-capable SSA no-merge writeback validation replaces a `600s` partial direct run with valid byte-identical `43.846s` / `44.954s` current-artifact repeats and preserves the `100000/100000` dedicated aggregate. Dedicated optimize/shrink/O4z still execute DAEO exactly once in the locked late slot. Large shrink and O4z now pass the former early SSA blocker but expose later `remove-unused-brs` validation and HSO pass-local owners; optimize remains blocked in direct vacuum. All three public artifact modes still stop before DAEO, so release closure remains active under `[WALL]001`.
