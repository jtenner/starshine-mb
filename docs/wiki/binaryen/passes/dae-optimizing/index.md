...
- Current closeout evidence is research note [`1614`](../../../raw/research/1614-2026-07-15-daeo-selected-uniform-null-final-matrix.md): the four required explicit-native Binaryen-v130 compare lanes are complete and classified, public optimize/shrink/O4z execute DAEO exactly once immediately before `inlining-optimizing`, and the retained direct artifact is valid and byte-stable at controlled `1.96x` / `1.96x` Binaryen. Three optimizing-only selected-callee shapes—uniform final `ref.null` argument/parameter materialization, materialized null-test folding, and exact sentinel-wrap conditional folding—close another `57` raw / `57` canonical bytes. The audit remains active on a `+10481` canonical-byte parity gap; Funcs `7008`/`7007` still retain `+1398` combined canonical body bytes and excess local get/set traffic, while Func `7105` retains a `+22` body gap. Those positive bytes are not accepted Starshine wins.
