...
- Current behavior-gap evidence is research note [`1642`](../../../raw/research/1642-2026-07-17-daeo-func8185-immutable-field-delay.md): after final selected local ordering, optimizing-only cleanup delays only the exact one-set/four-get read of an immutable field from an unwritten non-null parameter source to its first use as `local.tee`. Func `8185` reaches the direct Binaryen-v130 `simplify-locals-nostructure -> vacuum` probe at canonical body `2462`; raw and canonical both improve by `2` bytes to `3203064` / `3263954`, leaving `+1498` canonical. Focused/full tests, native build, artifact validation/convergence, and dedicated/regular `1000`-case normalized smokes are green. The remaining Func `8185 +33` versus final Binaryen DAEO, full closeout, and pre-slot public-pipeline blockers remain active.
