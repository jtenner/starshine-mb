...
- The active Starshine port preserves a still-partial scalar signature subset: direct dead-parameter removal, a narrow exact-literal every-direct-caller-same-constant materialization slice on read-only params, adjacent self-recursive forwarding, no-param dropped/uncalled result removal, case-000690 escaped-result self-call operand parameter preservation, and dead-suffix cleanup around root `unreachable`; as of 2026-05-13 that exact-literal slice also recovers candidates when sibling call arguments use the scalar memory-load family and typed single-result `TypeIdxBlockType` wrappers, and note [`0627`](../../../raw/research/0627-2026-05-26-dae003-constant-unread-inventory.md) now maps the supported exact-literal, guarded forwarded-local, unread-parameter, bounded immutable-global, and historical selected default-argument surfaces to source helpers and tests. Notes [`1564`](../../../raw/research/1564-2026-07-12-dae-generic-folded-multivalue-suffix-defaults.md), [`1565`](../../../raw/research/1565-2026-07-13-dae-generic-constant-condition-inter-argument-defaults.md), and [`1566`](../../../raw/research/1566-2026-07-13-dae-generic-immutable-global-inter-argument-defaults.md) now recover folded-multivalue suffix defaults plus direct-literal- and immutable-defined-global-conditioned inter-argument defaults generically; the former Func323 8/9-parameter selected rewrite is gone, leaving only a seven-parameter residual artifact cleanup. The active port now also has a usefully shrinking raw-cleanup slice on DAE-touched functions that strips live integer identity tails, including guarded left-constant forms, with focused wbtest coverage. Complete result-removal scheduling (`[DAE]004`) and broader constant-actual/unread-parameter Binaryen families (`[DAE]003`) are now closed for the current v0.1.0 surface by research notes `0661` and `0687`; future widening should reopen only from a new semantic mismatch, validation failure, or measured DAE-owned regression.
- The saved 1000-case direct frontier after `1f81bbc7` is not currently showing another semantic/signature candidate: all 13 remaining `gen-valid` mismatches in `.tmp/pass-fuzz-dae-690-final2-1000` have exactly one diff hunk limited to unused local declarations, while the two `wasm-smith` failures are unchanged Binaryen/tool `binaryen-rec-group-zero` parser failures. See [`../../../raw/research/0558-2026-05-12-dae-local-declaration-frontier.md`](../../../raw/research/0558-2026-05-12-dae-local-declaration-frontier.md). Newer external-generator evidence in [`1567`](../../../raw/research/1567-2026-07-13-dae-block-fallthrough-propagation-and-imported-tag-type-gap.md) exposes one separate Starshine validation failure: simple function-type pruning marks defined tags but not imported tag payload types, so an imported-tag-only module loses its referenced type. This is an active correctness gap, not accepted drift.
- `[DAE]002` is closed only for the historical guarded scheduler slice, not for full nested behavior parity. Starshine has a touched-function cleanup scheduler for `precompute-propagate-prefix -> dead-code-elimination -> optimize-instructions -> local-cse -> pick-load-signs -> heap-store-optimization -> heap2local -> optimize-casts -> code-pushing -> simplify-locals -> code-folding -> precompute -> merge-blocks -> remove-unused-brs -> remove-unused-names -> merge-blocks -> coalesce-locals -> reorder-locals -> vacuum`, with trace-order and touched-only coverage. The private `precompute-propagate-prefix` helper folds SSA-backed default-init and direct local constant facts, now also deriving fallthrough constants through value-preserving `local.tee` nodes and unbranched single-result blocks while preserving effectful prefixes, then reruns plain `precompute`. Branch-valued blocks remain fail-closed through label-use guards. This is meaningful Binaryen-shaped behavior but still not the complete public upstream sibling or exact default-function replay. The `local-cse` step uses a function-filtered raw adapter, while `coalesce-locals` and `reorder-locals` use function-filtered adapters over existing module passes, so public direct forms remain module-shaped while DAE cleanup touches only DAE-mutated functions.
