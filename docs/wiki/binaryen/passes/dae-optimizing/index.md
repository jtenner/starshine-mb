...
- Current checkpoint evidence is research note [`1624`](../../../raw/research/1624-2026-07-16-daeo-pair-result-final-matrix.md): the four required explicit-native Binaryen-v130 compare lanes are complete and classified, public optimize/shrink/O4z execute DAEO exactly once after late `heap-store-optimization` and immediately before `inlining-optimizing`, and plain `dae` remains separate. Exactly three red-first changes fuse the first `struct.set` into exact two-call and one-call terminal pair producers so both one-use spills disappear, then tee an exact nontrapping `i32.wrap_i64` readback in the selected forwarding traversal. The valid byte-stable artifact improves note `1623` by `10` raw / `28` canonical bytes to raw `3194727` / canonical `3272221`; controlled repeats `16669.816ms`, `16659.436ms`, and `16620.041ms` remain below the absolute `17076.04ms` ceiling. The audit remains active on the `+9765` canonical-byte parity gap; validation, fewer local operations, or byte-identical probes do not excuse a larger unproved output shape.
