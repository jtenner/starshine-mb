...
- Fresh post-tuple-cleanup direct evidence is complete on the rebuilt native binary. Research note [`1568`](../../../raw/research/1568-2026-07-13-daeo-fresh-dedicated-and-regular-compare.md) records the dedicated `dae-optimizing` profile at `10000/10000` normalized and regular GenValid at `100000/100000` normalized. Research note [`1569`](../../../raw/research/1569-2026-07-13-daeo-fresh-wasm-smith-and-random-all.md) records explicit wasm-smith with `9956` compared, `9955` normalized plus `1` cleanup-normalized, zero mismatches, and `44` Binaryen/oracle tool failures; random-all compared `10000/10000`, normalized `9633`, and left `367` byte-identical previously reviewed residuals (`dae-effectful-args=124`, `coverage-forced-portable=243`) classified by agent judgment as measured/source-backed Starshine cleanup wins with aggregate deltas `-110219` raw / `-797486` canonical / `-5465849` WAT bytes and no canonical/WAT-positive case. Every lane used the explicit `_build` native binary, `--jobs auto`, and both DAE normalizers. Scheduled O4z performance/artifact evidence remains open.
