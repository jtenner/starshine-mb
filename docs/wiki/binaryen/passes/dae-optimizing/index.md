...
- Current checkpoint evidence is research note [`1623`](../../../raw/research/1623-2026-07-16-daeo-single-traversal-readback-final-matrix.md): the four required explicit-native Binaryen-v130 compare lanes are complete and classified, public optimize/shrink/O4z execute DAEO exactly once after late `heap-store-optimization` and immediately before `inlining-optimizing`, and plain `dae` remains separate. Exactly three red-first changes fuse the note-`1622` nested pair-store fixpoint exposure into its owning terminal-call transport, reread exact tee-captured aggregate fields without a spill, and convert an immediate aggregate-field readback to a tee while preserving later reads. The valid byte-stable artifact improves note `1622` by `16` raw / `16` canonical bytes to raw `3194737` / canonical `3272249`; controlled quiescent repeats `16608.001ms`, `16524.749ms`, and `16568.615ms` remain below the absolute `17076.04ms` ceiling. The audit remains active on the `+9793` canonical-byte parity gap; validation, fewer local operations, or byte-identical probes do not excuse a larger unproved output shape.
