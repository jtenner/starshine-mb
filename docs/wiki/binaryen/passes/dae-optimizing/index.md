...
- Current closeout evidence is research note [`1620`](../../../raw/research/1620-2026-07-15-daeo-remapped-carrier-cleanup-final-matrix.md): the four required explicit-native Binaryen-v130 compare lanes are complete and classified, public optimize/shrink/O4z execute DAEO exactly once after late `heap-store-optimization` and immediately before `inlining-optimizing`, and plain `dae` remains separate. Exactly three red-first changes sink the first exact two-producer sibling result, both exact three-producer sibling results, and typed terminal complete-block carriers through the existing post-remap counted traversal. The valid byte-stable artifact improves note `1619` by `40` raw / `40` canonical bytes to raw `3194796` / canonical `3272308`; retained controlled pass-local repeats are `16804.456ms` / `16971.806ms`, both below the absolute ceiling. The audit remains active on the `+9852` canonical-byte parity gap; validation, fewer local operations, or byte-identical probes do not excuse a larger unproved output shape.
