...
- Current closeout evidence is research note [`1616`](../../../raw/research/1616-2026-07-15-daeo-type309-call-transport-final-matrix.md): the four required explicit-native Binaryen-v130 compare lanes are complete and classified, public optimize/shrink/O4z execute DAEO exactly once immediately before `inlining-optimizing`, and the retained direct artifact is valid and byte-stable at controlled `1.97x` / `1.96x` Binaryen. Three exact changed-function cleanup lanes remove both surviving Type `309` terminal-self-branch/fallthrough wrappers and sink two one-use producer-call results through pure local arguments into their consumers. The artifact improves note `1615` by `26` raw / `78` canonical bytes to raw `3195283` / canonical `3272837`; Func `7008`'s canonical body falls to `4995` (`+663`) while Func `7007` remains `4541` (`+657`). The audit remains active on a `+10381` canonical-byte parity gap, `+1320` combined selected-pair body bytes, and excess local get/set traffic. Those positive bytes are not accepted Starshine wins.
