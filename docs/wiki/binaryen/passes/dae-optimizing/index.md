...
- Current closeout evidence is research note [`1618`](../../../raw/research/1618-2026-07-15-daeo-complete-call-transport-final-matrix.md): the four required explicit-native Binaryen-v130 compare lanes are complete and classified, public optimize/shrink/O4z execute DAEO exactly once immediately before `inlining-optimizing`, and the retained direct artifact is valid and byte-stable at controlled `1.96x` Binaryen. Three exact changed-function cleanup lanes sink twelve Func `7007` block results through a complete zero/call/constructor suffix, four Func `7008` structured results through two pure caller-local arguments, and one complete nested direct-call value slice including the prefix call required to avoid ambient-stack capture. The artifact improves note `1617` by `73` raw / `76` canonical bytes to raw `3194881` / canonical `3272433`; Func `7008` falls to canonical `4746` (`+414`) and Func `7007` to `4386` (`+502`). The audit remains active on a `+9977` canonical-byte parity gap and `+916` combined selected-pair body bytes. Starshine's lower get/tee counts do not excuse the larger code and module payloads.
