...
- Current closeout evidence is research note [`1619`](../../../raw/research/1619-2026-07-15-daeo-complete-carrier-cleanup-final-matrix.md): the four required explicit-native Binaryen-v130 compare lanes are complete and classified, public optimize/shrink/O4z execute DAEO exactly once after late `heap-store-optimization` and immediately before `inlining-optimizing`, and plain `dae` remains separate. Exactly three red-first changes recover complete Func `7008` `call 7053` carriers, the conditional selector sibling, and a counted post-remap four-field terminal aggregate spill. The valid byte-stable artifact improves note `1618` by `45` raw / `85` canonical bytes to raw `3194836` / canonical `3272348`; controlled pass-local repeats are `16633.600ms` / `16929.736ms`, both below the absolute ceiling. The audit remains active on the `+9892` canonical-byte parity gap; validation, fewer local operations, or byte-identical probes do not excuse a larger unproved output shape.
