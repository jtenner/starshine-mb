...
- Current checkpoint evidence is research note [`1626`](../../../raw/research/1626-2026-07-16-daeo-immutable-selector-blocker-checkpoint.md): one red-first selected-readback change removes Func `7008` local `43` by delaying a constant-arm selector only after proving its source reference non-null and its struct field immutable. The endpoint improves by `4` raw / `4` canonical bytes to raw `3194707` / canonical `3272197`, leaving `+9741` canonical versus Binaryen v130. The requested three-commit matrix stopped at one because sequential loop reuse and array-item transport tied the endpoints, while high-local slot reuse improved raw only; making the newly dead post-remap locals canonical-winning requires a forbidden second selected body compaction/remap traversal, and moving the remaining call-produced carriers lacks safe call-effect evidence in the current owner. Full explicit-native lanes, exact-once public scheduling, plain-DAE separation, and controlled timings `16513.537ms`, `16698.406ms`, and `16730.872ms` remain green under the `17076.04ms` ceiling. The direct gap remains active.
