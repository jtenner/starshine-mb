...
Research note [`1615`](../../../raw/research/1615-2026-07-15-daeo-terminal-local-sinking-final-matrix.md) is current after exact terminal default-aggregate, first-sentinel, and second-sentinel local sinking in Func `7105`. Explicit native SHA-256 `541418778e981eaf9ff96bf70081afced819b3645a04db076fc01da1ce916ba9` drives authoritative `terminal-locals-v130-final-*` lanes with `WASM_OPT_BIN=$BINARYEN_BIN_DIR/wasm-opt`: dedicated `10000/10000` normalized; regular `100000/100000` normalized; wasm-smith `9955` normalized plus `1` cleanup-normalized out of `9956`, with only the unchanged `44` Binaryen/oracle failures; random-all `10000/10000` with `9633` normalized and the same `367` mismatches. All `367` failure-directory names and all `3670` saved failure files are byte-identical to note `1614`, so the established `coverage-forced-portable=243` / `dae-effectful-args=124` measured/source-backed cleanup-win classification and aggregate `-110224` raw / `-797486` canonical / `-5465849` WAT deltas remain unchanged. There are no unknown/risky, size-losing generated, Starshine-validation, or true-semantic residuals. Public optimize, shrink, and `--optimize -O4z` execute DAEO exactly once immediately before `inlining-optimizing`. The direct artifact improves to raw `3195309` / canonical `3272915`; controlled pass-local repeats are `1.98x` / `1.97x` Binaryen. Func `7105` now matches Binaryen's 50-byte body with zero body locals, while DAEO remains open for the `+10459` canonical gap, Funcs `7008`/`7007`, and pre-slot large scheduled blockers.
