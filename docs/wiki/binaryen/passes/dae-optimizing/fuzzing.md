...
Research note [`1599`](../../../raw/research/1599-2026-07-14-daeo-adjacent-local-order-final-matrix.md) is current after notes [`1597`](../../../raw/research/1597-2026-07-14-daeo-adjacent-type-stable-local-order.md) and [`1598`](../../../raw/research/1598-2026-07-14-daeo-adjacent-local-order-prefilter.md) added generic type-stable local ordering and exact-two-definition prefiltering to the selected broad adjacent pair. With explicit native SHA-256 `2dc4e78b621484309c1d4469238605054365895e88734d28967f50bb800dcf98`, Binaryen v130, and both DAE normalizers: dedicated is `10000/10000` normalized; regular is `100000/100000` normalized; wasm-smith is `9955` normalized plus `1` cleanup-normalized out of `9956` comparable cases with only the unchanged `44` Binaryen/oracle failures; random-all completes `10000/10000` with `9633` normalized and `367` mismatches under `--no-reduce-mismatches`. Its failure-directory names and all `2936` `.wasm`/`.wat` semantic artifacts are byte-identical to note `1596`, preserving the `coverage-forced-portable=243` / `dae-effectful-args=124` measured/source-backed Starshine cleanup-win classification and aggregate `-110219` raw / `-797486` canonical / `-5465849` WAT deltas. There are no unknown/risky, size-losing generated, Starshine validation, or true-semantic residuals. Public optimize, shrink, and `--optimize -O4z` execute DAEO exactly once immediately before `inlining-optimizing`. DAEO remains open for the `+12481` canonical artifact gap and pre-slot large scheduled blockers.
