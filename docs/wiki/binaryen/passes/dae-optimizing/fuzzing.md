...
Research note [`1611`](../../../raw/research/1611-2026-07-15-daeo-branch-deadtee-compaction-final-matrix.md) is current after selected branch-only-if rotation, never-read `local.tee` removal, and fused exposed-local compaction on the accepted definitions. Explicit native SHA-256 `0a2c51842501c036bda1e62e82daec2a90a42f25db022bb6e570c6408b4d4a96` drives authoritative `post-deadtee-v130-final-*` lanes with `WASM_OPT_BIN=$BINARYEN_BIN_DIR/wasm-opt`: dedicated `10000/10000` normalized; regular `100000/100000` normalized; wasm-smith `9955` normalized plus `1` cleanup-normalized out of `9956`, with only the unchanged `44` Binaryen/oracle failures; random-all `10000/10000` with `9633` normalized and the same `367` mismatches. All `367` failure-directory names and all `3670` saved failure files are byte-identical to note `1610`, including `2202` semantic artifacts, so the established `coverage-forced-portable=243` / `dae-effectful-args=124` measured/source-backed cleanup-win classification and aggregate `-110224` raw / `-797486` canonical / `-5465849` WAT deltas remain unchanged. There are no unknown/risky, size-losing generated, Starshine-validation, or true-semantic residuals. Public optimize, shrink, and `--optimize -O4z` execute DAEO exactly once immediately before `inlining-optimizing`. The direct artifact improves to raw `3195518` / canonical `3273130`; controlled pass-local repeats are `1.97x` / `1.94x` Binaryen, and DAEO remains open for the `+10674` canonical gap and pre-slot large scheduled blockers.
