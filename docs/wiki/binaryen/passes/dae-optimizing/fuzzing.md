...
Research note [`1618`](../../../raw/research/1618-2026-07-15-daeo-complete-call-transport-final-matrix.md) is current after twelve exact Func `7007` block-result/zero/direct-call/constructor transports, four exact Func `7008` structured-result transports through two pure caller-local arguments, and one complete nested direct-call value-slice transport that includes the prefix call required to avoid ambient-stack capture. Explicit native SHA-256 `2e46318b273bc3c17bf0843f8fcd816e886e8b88e566ab6e64a191ad5ab25021` drives the authoritative final lanes with `WASM_OPT_BIN=$BINARYEN_BIN_DIR/wasm-opt`: dedicated `10000/10000` normalized; regular `100000/100000` normalized; wasm-smith `9955` normalized plus `1` cleanup-normalized out of `9956`, with only the unchanged `44` Binaryen/oracle failures; random-all `10000/10000` with `9633` normalized and the same `367` mismatches. All `367` failure-directory names and all `3670` saved failure files are byte-identical to note `1617`, so the established `coverage-forced-portable=243` / `dae-effectful-args=124` measured/source-backed cleanup-win classification and aggregate deltas remain unchanged. There are no unknown/risky, size-losing generated, Starshine-validation, or true-semantic residuals. Public optimize, shrink, and `--optimize -O4z` execute DAEO exactly once immediately before `inlining-optimizing`; plain `dae` emits no post-component cleanup. The direct artifact improves to raw `3194881` / canonical `3272433`; controlled pass-local repeats are `16757.176ms` / `16741.469ms`, both `1.96x` Binaryen and below the absolute `17076.04ms` ceiling. DAEO remains open for the `+9977` canonical gap, `+916` combined Func `7008`/`7007` canonical body bytes, unproved local-shape differences, and pre-slot large scheduled blockers.
