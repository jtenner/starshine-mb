...
Research note [`1610`](../../../raw/research/1610-2026-07-15-daeo-post-component-vacuum-final-matrix.md) is current after function-filtered vacuum on the accepted coalesced definitions. Explicit native SHA-256 `a160cb5c201c7282a845d353455d862f35763c43f357fd21508b60b1ab31097e` drives authoritative `post-vacuum-v130-final-*` lanes with `WASM_OPT_BIN=$BINARYEN_BIN_DIR/wasm-opt`: dedicated `10000/10000` normalized; regular `100000/100000` normalized; wasm-smith `9955` normalized plus `1` cleanup-normalized out of `9956`, with only the unchanged `44` Binaryen/oracle failures; random-all `10000/10000` with `9633` normalized and the same `367` mismatches. All `367` failure-directory names and all `2202` compared saved semantic artifacts are byte-identical to note `1609`, so the established `coverage-forced-portable=243` / `dae-effectful-args=124` measured/source-backed cleanup-win classification and aggregate `-110224` raw / `-797486` canonical / `-5465849` WAT deltas remain unchanged. There are no unknown/risky, size-losing generated, Starshine validation, or true-semantic residuals. Public optimize, shrink, and `--optimize -O4z` execute DAEO exactly once immediately before `inlining-optimizing`. The direct artifact improves to raw `3196657` / canonical `3274317`; DAEO remains open for the `+11861` canonical gap and pre-slot large scheduled blockers.
