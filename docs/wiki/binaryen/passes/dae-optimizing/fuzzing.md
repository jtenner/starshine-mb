...
Research note [`1619`](../../../raw/research/1619-2026-07-15-daeo-complete-carrier-cleanup-final-matrix.md) is current after complete Func `7008` `call 7053` carrier transport, the conditional selector sibling, and counted post-remap four-field terminal aggregate forwarding. Explicit native SHA-256 `b9c7011d3e0a17bbc8deaa90b55db9d012c4eb230fdba76e2b61cd8c431dda2b` drives the authoritative final lanes with `WASM_OPT_BIN=$BINARYEN_BIN_DIR/wasm-opt`: dedicated `10000/10000` normalized; regular `100000/100000` normalized; wasm-smith `9955` normalized plus `1` cleanup-normalized out of `9956`, with only the unchanged `44` Binaryen/oracle failures; random-all `10000/10000` with `9633` normalized and the same `367` mismatches. All `367` failure-directory names and all `3670` saved files are byte-identical to note `1618`, so the established `coverage-forced-portable=243` / `dae-effectful-args=124` measured/source-backed cleanup-win classification remains unchanged at aggregate Starshine deltas `-110224` raw / `-797486` canonical / `-5465849` WAT bytes with no positive canonical/WAT case. There are no unknown/risky, size-losing generated, Starshine-validation, or true-semantic residuals. Public optimize, shrink, and `--optimize -O4z` execute DAEO exactly once after late HSO and immediately before `inlining-optimizing`; plain `dae` emits no post-component cleanup. The direct artifact improves to raw `3194836` / canonical `3272348`; controlled pass-local repeats are `16633.600ms` / `16929.736ms`, below the absolute `17076.04ms` ceiling. DAEO remains open for the `+9892` canonical gap, unproved selected-function output shapes, and pre-slot large scheduled blockers.
