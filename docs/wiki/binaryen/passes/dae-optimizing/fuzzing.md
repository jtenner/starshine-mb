...
Research note [`1607`](../../../raw/research/1607-2026-07-14-daeo-forwarding-component-final-matrix.md) is current after the forwarding-only component transaction. With explicit native SHA-256 `cb9a2832171faa49b8bf654981a4f4cf1808c9425085328909299d52a8642005`, Binaryen v130, and both DAE normalizers: dedicated is `10000/10000` normalized; regular is `100000/100000` normalized; wasm-smith is `9955` normalized plus `1` cleanup-normalized out of `9956` comparable cases with only the unchanged `44` Binaryen/oracle failures; random-all completes `10000/10000` with `9633` normalized and `367` mismatches under `--no-reduce-mismatches --max-failures 10000`. Its failure-directory names and all `2936` semantic `.wasm`/`.wat` artifacts are byte-identical to note `1606`, preserving the `coverage-forced-portable=243` / `dae-effectful-args=124` measured/source-backed Starshine cleanup-win classification and aggregate `-110219` raw / `-797486` canonical / `-5465849` WAT deltas. There are no unknown/risky, size-losing generated, Starshine validation, or true-semantic residuals. Public optimize, shrink, and `--optimize -O4z` execute DAEO exactly once immediately before `inlining-optimizing`. The named Func `7007` / `7008` / `7010` / `7024` forwarding component is closed; DAEO remains open for the `+12335` canonical artifact gap, a measured `+32` raw pre-canonical localization/encoding residue, and pre-slot large scheduled blockers.
