...
Research note [`1626`](../../../raw/research/1626-2026-07-16-daeo-immutable-selector-blocker-checkpoint.md) is current after one strict red-first immutable-selector transport. Explicit native SHA-256 `260c9b1b6f3ed314adc98a7222af3f9b45b5edb0b92e8055aab2488f9029b33a` drives the authoritative final lanes with `WASM_OPT_BIN=$BINARYEN_BIN_DIR/wasm-opt`: dedicated `10000/10000` normalized; regular `100000/100000` normalized; wasm-smith `9955` normalized plus `1` cleanup-normalized out of `9956`, with only the unchanged `44` Binaryen/oracle failures; random-all `10000/10000` with `9633` normalized and the same `367` mismatches. All `367` failure-directory names, all `3670` saved files, and all `10000` case records are byte-identical to note `1625`, so the established `coverage-forced-portable=243` / `dae-effectful-args=124` measured/source-backed cleanup-win classification remains unchanged at aggregate Starshine deltas `-110224` raw / `-797486` canonical / `-5465849` WAT bytes with no positive canonical/WAT case. There are no unknown/risky, generated size-losing, Starshine-validation, or true-semantic generated residuals. Public optimize, shrink, and `--optimize -O4z` execute DAEO exactly once after late HSO and immediately before `inlining-optimizing`; plain `dae` emits no post-component cleanup. The direct artifact improves to raw `3194707` / canonical `3272197`; CPU-affinity controlled repeats `16513.537ms`, `16698.406ms`, and `16730.872ms` remain below the absolute `17076.04ms` ceiling. The iteration stopped at a concrete single-traversal/canonical-profitability blocker rather than retaining tied or raw-only probes. DAEO remains open for the `+9741` canonical gap, safe call-effect evidence for remaining carriers, and pre-slot large scheduled blockers.
