...
Research note [`1617`](../../../raw/research/1617-2026-07-15-daeo-sibling-terminal-transport-final-matrix.md) is current after exact two-producer and three-producer final-result sinking plus fifty terminal structured-result set/readback removals. Explicit native SHA-256 `86db2642dfbc3b6b1c1e2cdd11517fd146c24529cb73e8fb0f442e88ce004ab2` drives authoritative `sibling-transport-v130-final-*` lanes with `WASM_OPT_BIN=$BINARYEN_BIN_DIR/wasm-opt`: dedicated `10000/10000` normalized; regular `100000/100000` normalized; wasm-smith `9955` normalized plus `1` cleanup-normalized out of `9956`, with only the unchanged `44` Binaryen/oracle failures; random-all `10000/10000` with `9633` normalized and the same `367` mismatches. All `367` failure-directory names and all `3670` saved failure files are byte-identical to note `1616`, so the established `coverage-forced-portable=243` / `dae-effectful-args=124` measured/source-backed cleanup-win classification and aggregate `-110224` raw / `-797486` canonical / `-5465849` WAT deltas remain unchanged. There are no unknown/risky, size-losing generated, Starshine-validation, or true-semantic residuals. Public optimize, shrink, and `--optimize -O4z` execute DAEO exactly once immediately before `inlining-optimizing`. The direct artifact improves to raw `3194954` / canonical `3272509`; controlled pass-local repeats are `1.95x` / `1.96x` Binaryen. DAEO remains open for the `+10053` canonical gap, `+992` combined Func `7008`/`7007` canonical body bytes, unproved local-shape differences, and pre-slot large scheduled blockers.
