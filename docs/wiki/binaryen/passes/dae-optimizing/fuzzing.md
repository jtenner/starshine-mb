...
Research note [`1613`](../../../raw/research/1613-2026-07-15-daeo-selected-aggregate-spill-final-matrix.md) is current after exact two-field aggregate spill collapse, four-field reordered-leader forwarding, and residual four-field spill collapse on the accepted definitions. Explicit native SHA-256 `bd381c2c17f08b8f97213fa04e2e721e5a4367c4864edbb47eaff97498d8b4fb` drives authoritative `aggregate-spill-v130-final-*` lanes with `WASM_OPT_BIN=$BINARYEN_BIN_DIR/wasm-opt`: dedicated `10000/10000` normalized; regular `100000/100000` normalized; wasm-smith `9955` normalized plus `1` cleanup-normalized out of `9956`, with only the unchanged `44` Binaryen/oracle failures; random-all `10000/10000` with `9633` normalized and the same `367` mismatches. All `367` failure-directory names and all `3670` saved failure files are byte-identical to note `1612`, so the established `coverage-forced-portable=243` / `dae-effectful-args=124` measured/source-backed cleanup-win classification and aggregate `-110224` raw / `-797486` canonical / `-5465849` WAT deltas remain unchanged. There are no unknown/risky, size-losing generated, Starshine-validation, or true-semantic residuals. Public optimize, shrink, and `--optimize -O4z` execute DAEO exactly once immediately before `inlining-optimizing`. The direct artifact improves to raw `3195388` / canonical `3272994`; controlled pass-local repeats are `1.92x` / `1.94x` Binaryen, and DAEO remains open for the `+10538` canonical gap and pre-slot large scheduled blockers.
