...
Research note [`1616`](../../../raw/research/1616-2026-07-15-daeo-type309-call-transport-final-matrix.md) is current after exact Type `309` terminal-self-branch folding, branch-free result-block flattening, and one-use producer-call result sinking in Func `7008`. Explicit native SHA-256 `f5f515c1e2bc396c1667896ef3e780ca7ac3aabdd2e8ea55ae36d9351eccab55` drives authoritative `type309-transport-v130-final-*` lanes with `WASM_OPT_BIN=$BINARYEN_BIN_DIR/wasm-opt`: dedicated `10000/10000` normalized; regular `100000/100000` normalized; wasm-smith `9955` normalized plus `1` cleanup-normalized out of `9956`, with only the unchanged `44` Binaryen/oracle failures; random-all `10000/10000` with `9633` normalized and the same `367` mismatches. All `367` failure-directory names and all `3670` saved failure files are byte-identical to note `1615`, so the established `coverage-forced-portable=243` / `dae-effectful-args=124` measured/source-backed cleanup-win classification and aggregate `-110224` raw / `-797486` canonical / `-5465849` WAT deltas remain unchanged. There are no unknown/risky, size-losing generated, Starshine-validation, or true-semantic residuals. Public optimize, shrink, and `--optimize -O4z` execute DAEO exactly once immediately before `inlining-optimizing`. The direct artifact improves to raw `3195283` / canonical `3272837`; controlled pass-local repeats are `1.97x` / `1.96x` Binaryen. DAEO remains open for the `+10381` canonical gap, `+1320` combined Func `7008`/`7007` canonical body bytes, excess local traffic, and pre-slot large scheduled blockers.
