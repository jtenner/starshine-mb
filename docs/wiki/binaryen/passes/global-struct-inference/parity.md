...
`[GSI-PARITY-005]` closed as an explicit v0.1.0 deferral with the current one-compare policy unchanged. The source-backed profitable grouping shape remains one materialized value, or two materialized values where one candidate group is singleton and can be selected with one `ref.eq`. Starshine already covers that exact/subtype-propagated local/param subset and keeps focused negatives for more than two unique values, two equal-sized value groups, and subtype-propagated versions of both families in [`global_struct_inference_test.mbt`](../../../../../src/passes/global_struct_inference_test.mbt).

No size-winning fixture in the current dossier proves that a larger multi-`ref.eq` decision tree should be generated for v0.1.0. Larger trees would add extra compares, code-size pressure, and evaluation-shape risk while moving beyond the documented Binaryen lit/source rule. Future work may reopen this only with a bounded, validation-preserving, size-neutral-or-better fixture and must keep existing ambiguity negatives unless the bound deliberately covers them.

## 2026-06-04 typed repair and refinalization trigger audit

`[GSI-PARITY-006]` closed as an audit/no-op for the v0.1.0 plain-GSI subset. No focused fixture in the current implemented surface requires an explicit Binaryen-style `ReFinalize` layer: direct-global, block-carried direct-global, local/param origin, one-value, singleton-select, packed repair, atomic-get, and descriptor-read replacements all construct result types that are valid at the replacement site before committing the rewrite. The pass therefore continues to use validation-preserving replacement typing instead of a separate typed-AST repair pass.
...
This matrix was the release-gating triage for the then-remaining official `gsi.wast`, shared descriptor, and sibling `gsi-desc-cast` surfaces. It is a docs classification, not compare-harness output. The source inventory was the Binaryen primary-source manifest, the current-main recheck, the local implementation/test map, the sibling desc-cast dossier, and the focused Starshine GSI tests listed above.
