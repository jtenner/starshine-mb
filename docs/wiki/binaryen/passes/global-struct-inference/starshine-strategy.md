...
- in closed-world mode, builds an internal candidate-global/poison fact table that includes immutable top-level origins, excludes mutable and too-broad global declarations, poisons nominal struct types that can enter through imported globals, tables, function results, or tag payloads, poisons function-local and nested global-initializer allocations, propagates poisoned child types to parents, and propagates child candidates upward to parent types
...
The closed-world analysis now builds the first local `typeGlobals`-shaped fact table: top-level immutable candidate globals are grouped by created struct type, mutable and equality-incomparable global declarations are excluded, imported reference boundaries poison every compatible nominal struct type, function-local `struct.new*` allocations poison their type, nested non-top-level global-initializer `struct.new*` allocations poison their type, poisoned child types poison parents, and child candidate globals propagate upward to parent types in deterministic global-index order. Imported globals and tables contribute their reference types; imported functions contribute result types; imported tags contribute payload types. It now reasons about locals and params for exact and subtype-propagated single-candidate origin rewrites, exact and subtype-propagated one-value folds, and exact and subtype-propagated two-value singleton-group selects.
