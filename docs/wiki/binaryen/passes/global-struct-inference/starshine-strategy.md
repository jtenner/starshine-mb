...
| `src/passes/global_struct_inference.mbt:577-1088` | guarded small-module arithmetic/bitwise un-nesting request collection, fresh-global synthesis, initializer repair, and forced reorder-globals repair |
...
| `src/passes/global_struct_inference.mbt:2079-2303` | recursive body walk that rewrites immediate `global.get` + `struct.get*` pairs, exact/subtype-propagated single-candidate `local.get` + `struct.get*` origin pairs, exact/subtype-propagated one-value local folds, and exact/subtype-propagated two-value singleton-group local selects |
| `src/passes/global_struct_inference.mbt:2306-2520` | cheap pre-scan used to skip unchanged functions |
| `src/passes/global_struct_inference.mbt:2523-2684` | public module-pass entrypoint; builds closed-world exact single-candidate and propagated candidate facts when requested, then runs direct-global and local-origin/value/select rewrites |
| `src/passes/global_struct_inference_test.mbt:28-939` | open-world direct-global positives, nullable-trap preservation, packed/default/descriptor constructor coverage, arithmetic/bitwise un-nesting positives, unsafe-global negatives, exact single-candidate local/param positives, exact/subtype-propagated one-value and two-value local positives, and open-world/ambiguous/non-materializable/poisoned/unsafe local-origin negatives |
...
1. broaden non-constant operand un-nesting beyond the current small-module arithmetic/bitwise read-gated subset only if pass-local runtime remains green
