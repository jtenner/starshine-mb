...
- in closed-world mode, builds an internal candidate-global/poison fact table that includes immutable top-level origins, excludes mutable and too-broad global declarations, poisons nominal struct types that can enter through imported globals, tables, function results, tag payloads, or exported defined-function parameters, poisons function-local and nested global-initializer allocations, propagates poisoned child types to parents, and propagates child candidates upward to parent types
