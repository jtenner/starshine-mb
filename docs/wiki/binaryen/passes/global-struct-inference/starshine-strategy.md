...
- in closed-world mode, builds an internal candidate-global/poison fact table that includes immutable top-level origins, excludes mutable and too-broad global declarations, poisons nominal struct types that can enter through imported globals, tables, function results, tag payloads, directly exported defined-function parameters, or defined functions type-compatible with imported or exported function tables, poisons function-local and nested global-initializer allocations, propagates poisoned child types to parents, and propagates child candidates upward to parent types
...
| `src/passes/global_struct_inference.mbt:20-615` | closed-world fact table, allocation and external-boundary poisoning, safe candidate-origin filters, subtype poison/candidate propagation, exact direct candidate extraction, and exact single-candidate extraction |
| `src/passes/global_struct_inference.mbt:616-1370` | guarded small-module arithmetic/bitwise/shift-rotate/unary-numeric/float-binary/float-rounding-sqrt/sign-extension un-nesting request collection, fresh-global synthesis, initializer repair, dynamic packed signed/unsigned repair for fresh-global payloads, and forced reorder-globals repair |
| `src/passes/global_struct_inference.mbt:1371-1558` | candidate field-value harvesting from trusted global initializers and accepted top-level global initializer families |
...
| `src/passes/global_struct_inference.mbt:2560-2893` | recursive body walk that rewrites immediate `global.get` or one-instruction block-carried `global.get` + `struct.get*` / `struct.atomic.get*` pairs, exact/subtype-propagated single-candidate `local.get` + field-read origin pairs, exact/subtype-propagated one-value local folds, and exact/subtype-propagated two-value singleton-group local selects |
| `src/passes/global_struct_inference.mbt:2894-3229` | cheap pre-scan used to skip unchanged functions |
| `src/passes/global_struct_inference.mbt:3257-3441` | public module-pass entrypoint; builds closed-world exact single-candidate and propagated candidate facts when requested, then runs direct-global and local-origin/value/select rewrites |
...
The closed-world analysis now builds the first local `typeGlobals`-shaped fact table: top-level immutable candidate globals are grouped by created struct type, mutable and equality-incomparable global declarations are excluded, external reference boundaries poison every compatible nominal struct type, function-local `struct.new*` allocations poison their type, nested non-top-level global-initializer `struct.new*` allocations poison their type, poisoned child types poison parents, and child candidate globals propagate upward to parent types in deterministic global-index order. Imported globals and tables contribute their reference types; imported functions contribute result types; imported tags contribute payload types; parameters of exported defined functions contribute their declared types because callers can supply instances created outside the optimized module. Imported and exported function tables add the same boundary for every defined function whose nominal function reference can inhabit the table: an embedder can retain or retrieve such a reference and invoke it with externally created values matching its parameter types. The conservative type-compatible scan also covers table initializers and later table mutation without relying on a fragile table-content analysis. It now reasons about locals and params for exact and subtype-propagated single-candidate origin rewrites, exact and subtype-propagated one-value folds, and exact and subtype-propagated two-value singleton-group selects.
...
- [research note 0529](./index.md)
- [research note 0506](./index.md)
- [research note 0344](./index.md)
- [research note 0234](./index.md)
- [research note 0140](./index.md)
