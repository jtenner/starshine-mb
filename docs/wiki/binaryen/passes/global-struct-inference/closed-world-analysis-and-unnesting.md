...
If local code intentionally keeps a smaller subset, the wiki should continue calling that out as a local limitation, not as the Binaryen contract. After the 2026-06-03 O4z audit and follow-up facts/local-origin/value-fold/select slices, that local subset is the open-world direct-global folder plus a subtype-aware closed-world candidate/poison table consumed only by exact local/param one-global rewrites, exact one-value direct-candidate folds, and exact two-value singleton-group `select(ref.eq(...))` rewrites as described in [`./starshine-hot-ir-strategy.md`](./starshine-hot-ir-strategy.md), not the full `typeGlobals` supertype rewrite or un-nesting engine described here.
