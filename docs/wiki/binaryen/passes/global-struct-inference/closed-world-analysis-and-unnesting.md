...
If local code intentionally keeps a smaller subset, the wiki should continue calling that out as a local limitation, not as the Binaryen contract. After the 2026-06-03 O4z audit and follow-up facts/local-origin/value-fold/select/supertype-value/un-nesting slices, that local subset is the open-world direct-global folder plus a subtype-aware closed-world candidate/poison table consumed by exact local/param one-global rewrites, exact and subtype-propagated one-value direct-candidate folds, exact and subtype-propagated two-value singleton-group `select(ref.eq(...))` rewrites, guarded small-module arithmetic/bitwise/shift-rotate/unary-numeric/float-sqrt un-nesting, and direct plus closed-world packed-field repair after fresh-global splitting as described in [`./starshine-hot-ir-strategy.md`](./starshine-hot-ir-strategy.md), not the full unbounded Binaryen engine described here.
