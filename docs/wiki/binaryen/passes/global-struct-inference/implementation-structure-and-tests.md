...
- read-gated small-module non-constant un-nesting for arithmetic, integer bitwise, and integer shift/rotate field operands
...
| Non-constant un-nesting | yes | yes for small-module pure arithmetic, integer bitwise, and integer shift/rotate field operands that are actually read, using fresh immutable globals plus forced `reorder-globals` repair; large modules keep the materializable-only path |
