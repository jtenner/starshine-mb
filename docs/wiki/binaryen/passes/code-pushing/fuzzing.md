...
Use `code-pushing-all` for the pass-specific lane. It is a deterministic composite over eighteen currently aggregate-safe positive families. Targeted `code-pushing-br-if-value` also exists for focused blocker lanes, but it is intentionally excluded from the ordinary aggregate until its lowering/normalization blocker is fixed or narrowly accepted:
...
| `code-pushing-br-if-value` | Targeted-only for this slice: adjacent local-independent pure `local.set` roots before a value-block-target `br_if` with one branch payload, followed by same-block suffix reads after the branch, preserving source order. Excluded from `code-pushing-all` until a larger targeted refresh and aggregate smoke land. The remaining `local.set tmp (br_if ...); drop tmp` drift is now normalized by `local-cleanup-debris`; a 200-case targeted lane is cleanup-normalized green. |
