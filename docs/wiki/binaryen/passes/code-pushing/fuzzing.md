...
| `code-pushing-br-on-non-null-prefix` | Targeted-only for this slice: two adjacent local-independent pure `local.set` roots before a two-result block-label `br_on_non_null` with an explicit prefix payload plus implicit non-null reference payload. Excluded from `code-pushing-all` until a larger targeted refresh and aggregate smoke land. The exact-HOT movement gap is narrowed and the remaining generated local-copy/drop and tuple-local allocation drift is now normalized by `local-cleanup-debris`; a 200-case targeted lane is cleanup-normalized green. |
