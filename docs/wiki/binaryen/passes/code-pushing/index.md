...
- guarded movement of selected `global.get`, local-copy setup shapes, and a narrow non-null `struct.get` heap-read shape across safe intervening roots, with call and tag-based `throw` roots preserved as segment-order barriers before later push points and a source-backed pure-value `throw_ref` / later-`br_if` movement refinement;
...
- EH control where movement can change exceptional observability, including the current `throw_ref` positive movement and tag-based `throw` stationary split.
