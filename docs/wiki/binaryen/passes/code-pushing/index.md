...
- guarded movement of selected `global.get`, local-copy setup shapes, and a narrow non-null `struct.get` heap-read shape across safe intervening roots, with call, tag-based `throw`, and `try_table` roots preserved as segment-order barriers before later push points and a source-backed pure-value `throw_ref` / later-`br_if` movement refinement;
...
- EH control where movement can change exceptional observability, including the current `throw_ref` positive movement and tag-based `throw` / `try_table` stationary split.
