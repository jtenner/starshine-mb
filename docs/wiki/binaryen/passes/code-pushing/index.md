...
- Call and EH control where movement can change observability, including Binaryen's `binaryen-intrinsics/call.without.effects` no-effects call surface: [`0899`](../../../raw/research/0899-2026-06-25-code-pushing-intrinsic-no-effects-boundary.md) records the old missing-import-metadata blocker, and [`0904`](../../../raw/research/0904-2026-06-25-code-pushing-import-identity-metadata.md) lands the exact HOT module-context import identity prerequisite without yet admitting call movement. The current `throw_ref` positive movement, no-payload and payload-bearing tag-based `throw` / `try_table` stationary split including the `catch_all_ref` try-table boundary, no-rethrow legacy `try`/`catch` try-lowered movement characterization, and rethrow-containing HOT stationary boundary remain separate.
