...
- guarded movement of selected `global.get`, local-copy setup shapes, and a narrow non-null `struct.get` heap-read shape across safe intervening roots, with call, no-payload and payload-bearing tag-based `throw`, and `try_table` roots preserved as segment-order barriers before later push points, a source-backed pure-value `throw_ref` / later-`br_if` movement refinement, and a legacy `try`/`catch` WAT fixture that currently observes Binaryen-positive movement through Starshine's try-lowered HOT block path;
...
- EH control where movement can change exceptional observability, including the current `throw_ref` positive movement, no-payload and payload-bearing tag-based `throw` / `try_table` stationary split, and legacy `try`/`catch` try-lowered movement characterization.
