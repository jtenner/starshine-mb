...
| `isPushPoint(...)` over `if`, `switch`, conditional `br`, dropped forms | Diagnostic recognizes these families where HOT representation is local; mutation now targets ordinary void `if`, dropped value-`if` wrappers, and a no-branch-value `br_if` to a void block or loop label; simple no-branch-value `br_table` block-exit windows are protected as a Binaryen-stationary boundary | Add one remaining push-point mutation family at a time, but do not widen switch mutation from the protected simple `br_table` boundary alone |
