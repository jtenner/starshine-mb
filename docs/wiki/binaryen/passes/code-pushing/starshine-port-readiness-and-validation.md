...
5. **Dedicated profile growth**
   - `code-pushing-all` now covers the implemented `if`-arm, after-`if`, dropped-`if`, narrow `br_if`, dropped `br_on_null`, one-result-block `br_on_non_null`, dropped one-result-block `br_on_cast`, dropped one-result-block `br_on_cast_fail`, ordinary-`if` ordered multi-set, dropped-`if` ordered multi-set, `br_if` ordered multi-set, local-copy ordered multi-set, `nop`-separated ordered multi-set, loop-target `br_if`, `drop(const)`-separated ordered multi-set, `drop(local.get)`-separated ordered multi-set, and bounded ordinary-/dropped-`if` `drop(global.get)`-separated ordered multi-set positive families. The targeted-only `code-pushing-br-if-value` leaf covers branch-value `br_if` but remains excluded from the aggregate pending the value-`br_if` lowering gap. Add aggregate leaves when switch/`br_table`, broader conditional branches, broader ordered multi-set movement, or broader atomics/GC/EH/trap-policy slices land.
6. **Preset slice**
...
Current dedicated-profile evidence from [`0810`](../../../raw/research/0810-2026-06-20-code-pushing-dedicated-genvalid-profile.md) through [`0829`](../../../raw/research/0829-2026-06-24-code-pushing-br-on-cast-fail-movement.md), plus the [`0822`](../../../raw/research/0822-2026-06-21-code-pushing-br-table-boundary.md) no-mutation boundary tests and the targeted blocker refresh in [`0833`](../../../raw/research/0833-2026-06-25-code-pushing-br-if-value-lowering-blocker.md): focused generator tests for `code-pushing` profiles passed `3/3` across the `*code-pushing*` filter after adding `code-pushing-br-on-cast-fail`. `moon build --target native --release src/cmd` produced `_build/native/release/build/cmd/cmd.exe` in this checkout. The latest refreshed native `code-pushing-all` lane with `--normalize local-cleanup-debris` compared `10000/10000`, with `5377` normalized matches, `4623` cleanup-normalized matches, `0` raw mismatches, `0` validation/generator/property/command failures, `0` command failures, and selected all seventeen aggregate-safe leaves, including `code-pushing-br-on-cast-fail: 559`, `code-pushing-br-on-cast: 602`, `code-pushing-br-on-non-null: 601`, and `code-pushing-br-on-null: 589`. A 2026-06-25 ordinary non-profile GenValid closeout-progress lane with the same native binary and `--normalize local-cleanup-debris` compared `10000/10000` in `.tmp/pass-fuzz-code-pushing-regular-10000-20260625`, with `10000` normalized matches, `0` cleanup-normalized matches, `0` raw mismatches, `0` validation/generator/property/command failures, `0` command failures, Binaryen cache `102 hits/9898 misses`, and ordinary `binaryen-oracle-portable: 10000` GenValid selection. A 2026-06-25 explicit wasm-smith closeout-progress lane in `.tmp/pass-fuzz-code-pushing-wasm-smith-10000-20260625` compared `9956/10000`, with `9956` normalized matches, `0` cleanup-normalized matches, `0` raw mismatches, `0` validation/generator/property failures, `44` Binaryen/tool command failures (`binaryen-rec-group-zero: 39`, `binaryen-invalid-tag-index: 1`, `binaryen-table-index-out-of-range: 1`, `binaryen-bad-section-size: 3`), cache `wasm-smith 10000 hits/0 misses`, Binaryen `106 hits/9850 misses`, and Binaryen failures `0 hits/44 misses`. A 2026-06-25 broad named `pass-fuzz-stress` closeout-progress lane in `.tmp/pass-fuzz-code-pushing-pass-fuzz-stress-10000-20260625` compared `10000/10000`, with `10000` normalized matches, `0` cleanup-normalized matches, `0` raw mismatches, `0` validation/generator/property/command failures, Binaryen cache `14 hits/9986 misses`, and selected profile counts `pass-fuzz-stress: 10000`. A same-date targeted `code-pushing-br-if-value` refresh in `.tmp/pass-fuzz-code-pushing-br-if-value-refresh-100-20260625` compared `35/100` before the mismatch cap, with `35` raw mismatches and no failures; it keeps that leaf excluded from `code-pushing-all` pending the value-`br_if` temporary-local lowering fix or a narrow normalizer/acceptance decision. These strengthen and sharpen direct compare evidence but do not replace final closeout's required 100000 regular lane and a then-current dedicated-profile lane after remaining source gaps are resolved or accepted. The earlier non-normalized probe stopped after `65` raw mismatches, all classified as bounded local-cleanup drift from Starshine removing standalone `nop`/empty-else debris.
