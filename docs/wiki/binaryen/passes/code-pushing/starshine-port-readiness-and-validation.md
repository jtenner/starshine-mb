...
The prefix-payload `br_on_non_null` probe in [`0834`](../../../raw/research/0834-2026-06-25-code-pushing-br-on-non-null-prefix-payload.md) found a remaining positive behavior gap: Binaryen v130 moves a pure `local.set` after a two-result block-label `br_on_non_null` carrying an explicit `i32` prefix payload plus the implicit non-null reference payload. Starshine's current `BrOnNonNull` movement gate remains limited to the one-result block-label shape, so this prefix-payload family is open implementation work rather than an accepted stationary boundary.
