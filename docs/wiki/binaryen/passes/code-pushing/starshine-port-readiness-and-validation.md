...
The 2026-06-25 loop-label boundary slice in [`0830`](../../../raw/research/0830-2026-06-25-code-pushing-br-on-non-null-loop-boundary.md) found that Binaryen v130 keeps a one-result loop-label `br_on_non_null` window stationary after rewriting the loop-carried reference through a scratch/control wrapper. Starshine now has focused HOT coverage that keeps this loop-label shape stationary. This is a narrow accepted boundary for that reduced `br_on_non_null` loop-label probe only; broader `br_on_*` loop-label, prefix-payload, and reference-carrying forms remain open.
