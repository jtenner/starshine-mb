...
The exact-HOT follow-up in [`0837`](../../../raw/research/0837-2026-06-25-code-pushing-br-on-non-null-prefix-exact-hot.md) inspected the replayed generated HOT and found the missed representation: a two-child prefix-payload `BrOnNonNull` root inside a two-result block that is shared by duplicate top-level drops. Starshine now moves the generated user-local prefix sets after that branch using unique-node accounting for the helper, and focused `*br_on_non_null*` coverage passes `6/6`. The targeted leaf still cannot join `code-pushing-all`: `.tmp/pass-fuzz-code-pushing-br-on-non-null-prefix-exact-20-20260625` compared `20/20` with `20` raw mismatches, and the follow-up `.tmp/pass-fuzz-code-pushing-br-on-non-null-prefix-exact-200-20260625` compared `65/200` before the mismatch cap with `65` raw mismatches. Starshine lowering materializes the post-branch prefix-payload drop through an extra temporary local while Binaryen emits a direct `drop (local.get prefix)`. Treat this as the next parity/normalization blocker, not a Starshine win.
