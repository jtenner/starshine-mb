...
- EH can make movement observable through exceptional paths. The first reduced EH refinements are narrow: Binaryen v130 moves a pure SFA set after a later `br_if` when the intervening root is non-fallthrough `throw_ref`; keeps the set before a tag-based `throw` and before a `try_table` with a catch target; and moves the set after a legacy `try`/`catch` in the reduced WAT probe, which Starshine currently covers only through try-lowered block movement. Broader payload-bearing `throw`, `rethrow`, native HOT `Try`, `try_table`, and richer legacy `try` surfaces remain open.
