...
- EH can make movement observable through exceptional paths. The first reduced EH refinement is narrow: Binaryen v130 moves a pure SFA set after a later `br_if` when the intervening root is non-fallthrough `throw_ref`, but broader `throw`, `rethrow`, `try_table`, and legacy `try` surfaces remain open.
