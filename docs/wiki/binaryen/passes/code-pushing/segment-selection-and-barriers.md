...
- EH can make movement observable through exceptional paths. The first reduced EH refinements are narrow: Binaryen v130 moves a pure SFA set after a later `br_if` when the intervening root is non-fallthrough `throw_ref`; keeps the set before both no-payload and payload-bearing tag-based `throw` probes and before a `try_table` with a catch target; and moves the set after a legacy `try`/`catch` in the reduced WAT probe, which Starshine currently covers only through try-lowered block movement. Broader `rethrow`, native HOT `Try`, richer `try_table`, caught payload/reference forms, and richer legacy `try` surfaces remain open.
