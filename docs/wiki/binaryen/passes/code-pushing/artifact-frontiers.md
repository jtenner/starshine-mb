...
- The next work is back to refreshing the remaining current-tree artifact and
  runtime frontiers, plus checking how much of the exact autogenerated
  standalone `Func 1948` / `Func 1977` WAT drift is still real after stronger
  normalization.
  It is the reopened valid whole-artifact / runtime frontier after `Func 148`.
- One reduced standalone caveat is narrower now too: current-source standalone
  replay once again admits the reopened `Func 1977` local-`38` / local-`33` /
  local-`45` motion family and the repeated `Func 1948` local-`135`-through-
  `145` / `153`-through-`163` motion family at the pass level. The remaining
  exact standalone Starshine-vs-Binaryen WAT drift is therefore mixed with the
  known no-pass / writeback canonicalization noise in those autogenerated
  reconstructions rather than another reduced `code-pushing` fallback cliff.
