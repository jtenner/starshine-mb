...
- The reduced standalone `Func 1977` lower/writeback fallback is now closed too.
- The remaining reduced non-void explicit-exit prefix fence is narrower again:
  earlier unsafe `Block` prefixes are now handled candidate-by-candidate instead
  of as a blanket blocker for later motion.
...
- The new live reducer split is now clearer too:
  ...
  Earlier unsafe explicit-exit `Block` prefixes are not blanket blockers either:
  reduced Binaryen probes still reorder later branch-root, nested-branch-body,
  branch-free parent-escape, and return-continuation parent-err candidates past
  the later `if`.
...
The closing mechanism is narrower now:

- the payload still gets extracted upward through a parent escape block
- the inner carried `if (result ...)` still gets demoted to `if (Void)`
- but the real remaining failure was later recursive repair, not the extraction itself
- a newly synthesized voidified split `local.set` wrapper pair was not being re-run
  through the mixed-depth split-wrapper repair helpers
- that left the mirrored tag-`76` / tag-`77` branch payloads with stale branch depths
  (`br 3` where the new wrapper shape required `br 2`), which is why standalone
  validation underflowed at the carried branch instead of near the final reopened
  alias tail
...
- It is also no longer a reduced standalone invalid-output cliff.
- The next work is back to refreshing the remaining current-tree artifact and
  runtime frontiers. That is the reopened valid whole-artifact / runtime
  frontier after `Func 148`.
- One reduced standalone caveat is classified more sharply now too:
  current-source standalone replay once again admits the reopened `Func 1977`
  local-`38` / local-`33` / local-`45` motion family and the repeated
  `Func 1948` local-`135`-through-`145` / `153`-through-`163` motion family at
  the pass level.
  The remaining exact standalone Starshine-vs-Binaryen WAT drift is no longer
  good direct evidence of a missed `code-pushing` motion:
  - in `Func 1948`, most of the exact drift already exists in the no-pass
    standalone reconstruction baseline
  - in `Func 1977`, the remaining exact drift is mostly Starshine writeback
    canonicalization after the now-correct admitted move
