...
2. **Ordinary `if`, dropped `if`, and narrow `br_if` segment movement**
   ...
   - The `br_if` extension moves the same single-set family after a void-block-target or void-loop-target `br_if` with no branch values, after a dropped void-label `br_on_null`, and after a value-block-target `br_if` with one branch payload, when the branch/guard/payload does not read the local and every read is a same-block / same-loop-body suffix read after the branch.
   - The ordered multi-set extension moves adjacent local-independent SFA sets after an ordinary void `if`, dropped value-`if`, narrow no-branch-value void-block-target / void-loop-target `br_if`, dropped void-label `br_on_null`, or value-block-target `br_if` in source order when the push point does not read any moved local and every read is a suffix read after the push point; broader branch-value and broader `br_on_*` windows remain out of scope.
