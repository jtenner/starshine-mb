...
2. **Non-mutating segment-window inventory**
...
3. **Guarded setup movement across later roots**
...
4. **Starshine-local typed/dead-block flattening near unreachable context**
...
| [`src/passes/code_pushing.mbt`](../../../../../src/passes/code_pushing.mbt) lines 831-975 | Current single-consuming-arm `local.set` into `if` rewrite |
...
1. build on the initial analyzer/segment-window diagnostic inventory from [`0808`](../../../raw/research/0808-2026-06-20-code-pushing-segment-inventory.md);
2. expand safe segment movement before widening value effects;
...
Current Starshine `code-pushing` is active, but `[O4Z-AUDIT-CP]` is not closed under the current release-gating standard. The older direct subset remains supported by its 2026-05 evidence, the 2026-06-20 post-use slice is implemented, and the first analyzer/segment inventory now exists. The next active audit work is a smallest source-backed safe segment-movement slice, with ordered-before / atomics boundaries carried forward from the `version_130` refresh. The pass remains intentionally outside public presets until ordered-neighborhood proof lands.
