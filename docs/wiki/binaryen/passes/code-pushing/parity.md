...
- `Func 1977` is no longer just a "not skipped" contract. The reduced probe and
  ...
  The sharper standalone writeback frontier is closed too: the mirrored
  tag-`76` / tag-`77` decode-ladder wrappers were failing because `hot_lower`
  synthesized a voidified split `local.set` wrapper pair and then stopped before
  recursively re-running the mixed-depth split-wrapper repair on that new
  wrapper. The recursive follow-on fix now shifts that stale carried branch from
