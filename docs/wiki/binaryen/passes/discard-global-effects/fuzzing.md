...
- More fundamentally, the upstream pass clears in-memory `Function.effects` metadata while leaving the module's Wasm text unchanged. A canonical WAT match—or a run with zero meaningful metadata observations—would not prove cleanup correctness. See [`index.md`](index.md), [`metadata-shapes.md`](metadata-shapes.md), and the upstream source capture [`../../../raw/binaryen/2026-05-05-discard-global-effects-current-main-recheck.md`](../../../raw/binaryen/2026-05-05-discard-global-effects-current-main-recheck.md).
