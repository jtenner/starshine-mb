...
# `discard-global-effects` Fuzzing Profile

Recommended smoke lane: run the ordinary mixed-generator compare-pass lane for this pass:

```sh
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass discard-global-effects --out-dir .tmp/pass-fuzz-discard-global-effects --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe
```
