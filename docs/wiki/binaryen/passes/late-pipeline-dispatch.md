...
- Official Binaryen release pages for `version_127` (`2026-03-10`), `version_128` (`2026-03-13`), `version_129` (`2026-04-01`), and `version_130` are all directly reachable on GitHub, and the Chromium refs listing now corroborates that `version_130` is the newest release tag visible in the mirror.
- The `version_130` release horizon now aligns with the current `main` changelog's `v130` section, which is the live follow-up watch for anything beyond the latest tag.
