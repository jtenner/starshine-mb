...
- Binaryen `version_131` leaves the 56-slot / 38-owner top-level scheduler unchanged. Starshine preserves those first 56 slots exactly, then appends four documented extensions: `strip-debug -> coalesce-locals-cfg -> reorder-locals -> vacuum`. The CFG spelling is intentionally late-only: using it in the earlier compatibility slots perturbs inlining/simplification shape and increased the BLAKE3 SIMD artifact, while the post-strip suffix reduces the validated result by 877 bytes. `remove-unused-brs` still appears exactly three times at zero-based indices `13`, `24`, and `39`. The complete O4z expansion is now 60 entries.
...
- The registry contract compares the complete first 56 O4z entries against the Binaryen-v131 roster, asserts all three RUB indices, and separately asserts the four-entry Starshine suffix. Optimize and shrink expose the same 60-entry expansion only when their resolved levels are both 4; lower O/size levels are intentionally different.
