...
- Public `optimize` and `shrink` are level-sensitive. Their O4z expansion includes the accepted late-tail suffix `simplify-globals-optimizing -> remove-unused-module-elements -> string-gathering -> reorder-globals -> directize`; lower levels admit those owners only at the verified Binaryen-v132 thresholds.
...
- Binaryen `version_132` is the current public release horizon. The historical 2026-07-18 v131 audit found registration changes without default scheduling changes. The v132 delta expands constraint analysis and DAE2; post-tag #9010 scheduling is not part of v132.
- The live `main` changelog is now the drift watch for anything beyond `version_132`; classify later fixes by exact tag ancestry rather than treating all `main` changes as released.
