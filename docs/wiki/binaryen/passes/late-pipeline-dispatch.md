...
- The current `main` branch changelog page reachable in that same Chromium mirror also does not currently advertise a newer optimization-pass addition: its `Current Trunk` section leads with a C API worker-threading note. Treat that as a useful freshness check, not a complete pass-history audit, because changelog coverage is narrower than source-level pass enumeration.
