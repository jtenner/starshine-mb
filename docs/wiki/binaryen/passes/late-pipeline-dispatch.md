...
- Binaryen `version_131` leaves the 56-slot / 38-owner top-level scheduler unchanged. Starshine now matches those 56 slots exactly and appends `strip-debug` as its documented sole extension at slot 57. `remove-unused-brs` appears exactly three times at zero-based indices `13`, `24`, and `39`; the former redundant adjacent early slot is removed. The post-`code-folding` neighboring block-exit and EH cleanup shapes are now closed; remaining `[O4Z-PRESET]001` work is outside `merge-blocks` ownership.
...
- The 2026-07-31 `code-folding -> merge-blocks -> remove-unused-brs -> remove-unused-names -> merge-blocks` audit closes both remaining ordered cleanup witnesses at O4z levels `4/4`: after final `strip-debug`, the block-exit fixture is `41` bytes versus Binaryen's `43`, and the EH fixture is byte-identical at `74` bytes. Red-first tests lock redundant inner self-`br_if` removal and unused `catch_ref`/`catch_all_ref` payload conversion.
