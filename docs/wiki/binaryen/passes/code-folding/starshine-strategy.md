...
On 2026-05-10, the refreshed direct lane for `--pass code-folding` at `.tmp/pass-fuzz-code-folding-cf002-embedded-budget` reported `6759/10000` compared cases, `6759` normalized matches, `0` semantic mismatches, and `20` Binaryen empty-recursion-group command failures. The debug artifact `--code-folding` command still differs first at `defined=220 abs=237`, but Starshine now shares the repeated `call $28(local.get $6)` branch-exit tails inside the value-carrying branch payload. The remaining inspected diff is helper-block / value-carrying branch-payload wrapper shape rather than those duplicated calls. The latest replay at `/tmp/starshine-self-optimize-compare-starshine-debug-wasi-841296` reports `333.075ms` Starshine pass-local time vs `176.973ms` Binaryen, which is back inside the repo speed floor (`starshine_time <= 2 * binaryen_time`). The implementation keeps embedded branch/drop payload traversal budgeted so large value-carrying payloads remain conservative until broader helper-block parity work justifies the extra scan cost; see [`../../../raw/research/0522-2026-05-06-code-folding-direct-revalidation.md`](../../../raw/research/0522-2026-05-06-code-folding-direct-revalidation.md) for the prior direct-lane baseline.
