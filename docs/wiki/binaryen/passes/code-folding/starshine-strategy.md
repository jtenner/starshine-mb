...
- `try` / `try_table` bodies are only visited for ordinary non-terminal `if` suffix folding; EH block-exit, terminal-tail, movement, and repair shapes remain conservative bailouts, and the explicit outer-`catch_all` `try_table` terminal-tail bailout was checked against Binaryen `version_129`
