...
The boundary is specifically the one-root case. Binaryen does fold source-backed full multi-root non-block arms such as `(nop) (f32.const 0)` / `(nop) (f32.const 0)` to `drop(condition); nop; f32.const 0`, and Starshine now matches that top-level shape plus narrow embedded typed-wrapper variants for source-backed `select`, `drop`, `call`, and `binary` value parents, covering both full multi-root arms and partial value suffixes where probed.
