...
- exact partial non-block value-arm `if` suffix sharing, one-block/one-non-block `if` value-suffix folding in both then-block and else-block orientations plus both-arm-prefix, full one-block value-arm, full one-block void-arm, full multi-root non-block value-arm, embedded `select`, `drop`, `call`, `unary`, `binary`, `compare`, `convert`, `load`, `local.set`, `local.tee`, `global.set`, `store`, `br` payload, and `return` value-child typed-wrapper shapes, and a source-backed two-unnamed-block value-arm suffix fold with unique prefixes plus a two-unnamed-block void-arm suffix fold
...
- failing-first tests covered Binaryen's one-block/non-block partial value-suffix fold where both arms have distinct prefixes, the full one-block/non-block value-arm fold to `drop(condition)` plus one shared suffix, the full tiny/larger one-block/non-block void-arm fold to `drop(condition)` plus the shared void suffix, the full multi-root non-block value-arm fold, the narrow embedded-`select` full-arm typed-wrapper variant, the embedded-`select` partial value-if suffix typed-wrapper variant, and source-backed embedded value-parent wrappers: partial suffixes under `drop`, `call`, `unary`, `binary`, `compare`, `convert`, `load`, `local.set`, `local.tee`, `global.set`, `store`, `br` payload, and `return`, plus full multi-root suffixes under `call` and `binary`;
- implementation now computes the actual common suffix between the unused block body and non-block arm instead of requiring the whole non-block arm to be the suffix, no longer bails out before the existing full-arm replacement path for result-void one-block/non-block arms, permits full non-block value folds only when the shared suffix has multiple roots, and wraps full multi-root or partial value `if` children in typed blocks for the source-backed `select`, `drop`, `call`, `unary`, `binary`, `compare`, `convert`, `load`, `local.set`, `local.tee`, `global.set`, `store`, `br` payload, and `return` parent subset;
- latest validation: initial `moon test src/passes` for the core value-parent batch failed exactly the five new tests (`1679/1684`); after implementation `moon test src/passes` passed `1684/1684`; `moon fmt`, `moon info`, and full `moon test` passed `4869/4869`; native build produced `_build/native/release/build/cmd/cmd.exe`; direct 1000-case compare at `.tmp/pass-fuzz-code-folding-embedded-value-core-ops-1000` had `998` normalized matches, `0` mismatches, and `2` tool/Binaryen command failures (`command-class.binaryen-rec-group-zero`).
