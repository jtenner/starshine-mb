...
- exact partial non-block value-arm `if` suffix sharing, one-block/one-non-block `if` value-suffix folding in both then-block and else-block orientations plus both-arm-prefix, full one-block value-arm, full one-block void-arm, full multi-root non-block value-arm, and embedded `select` child full multi-root value-arm shapes, and a source-backed two-unnamed-block value-arm suffix fold with unique prefixes plus a two-unnamed-block void-arm suffix fold
...
The one-block and full multi-root `if` breadth continuation is green:

- failing-first tests covered Binaryen's one-block/non-block partial value-suffix fold where both arms have distinct prefixes, the full one-block/non-block value-arm fold to `drop(condition)` plus one shared suffix, the full tiny/larger one-block/non-block void-arm fold to `drop(condition)` plus the shared void suffix, the full multi-root non-block value-arm fold, and the narrow embedded-`select` typed-wrapper variant;
- implementation now computes the actual common suffix between the unused block body and non-block arm instead of requiring the whole non-block arm to be the suffix, no longer bails out before the existing full-arm replacement path for result-void one-block/non-block arms, permits full non-block value folds only when the shared suffix has multiple roots, and wraps full multi-root `if` children of `select` in a typed block;
- latest validation: `moon test src/passes` passed `1668/1668`; `moon info`, `moon fmt`, and full `moon test` passed `4853/4853`; native build produced `_build/native/release/build/cmd/cmd.exe`; direct 1000-case compare at `.tmp/pass-fuzz-code-folding-full-multiroot-value-1000` had `998` normalized matches, `0` mismatches, and `2` tool/Binaryen command failures.
