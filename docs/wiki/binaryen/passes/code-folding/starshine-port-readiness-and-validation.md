...
- exact partial non-block value-arm `if` suffix sharing, one-block/one-non-block `if` value-suffix folding in both then-block and else-block orientations plus both-arm-prefix, full one-block value-arm, and full one-block void-arm shapes, and a source-backed two-unnamed-block value-arm suffix fold with unique prefixes
...
The one-block `if` both-prefix/full-value breadth continuation is green:

- failing-first tests covered Binaryen's one-block/non-block partial value-suffix fold where both arms have distinct prefixes, the full one-block/non-block value-arm fold to `drop(condition)` plus one shared suffix, and the full tiny/larger one-block/non-block void-arm fold to `drop(condition)` plus the shared void suffix;
- implementation now computes the actual common suffix between the unused block body and non-block arm instead of requiring the whole non-block arm to be the suffix, and no longer bails out before the existing full-arm replacement path for result-void one-block/non-block arms;
- latest validation: `moon test src/passes` passed `1665/1665`; `moon fmt`, `moon info`, and full `moon test` passed `4850/4850`; native build produced `_build/native/release/build/cmd/cmd.exe`; direct 1000-case compare at `.tmp/pass-fuzz-code-folding-one-block-full-void-1000` had `998` normalized matches, `0` mismatches, and `2` tool/Binaryen command failures.
