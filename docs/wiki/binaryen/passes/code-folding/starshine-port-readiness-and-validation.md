...
- failing-first tests covered Binaryen's one-block/non-block partial value-suffix fold where both arms have distinct prefixes, the full one-block/non-block value-arm fold to `drop(condition)` plus one shared suffix, the full tiny/larger one-block/non-block void-arm fold to `drop(condition)` plus the shared void suffix, the full multi-root non-block value-arm fold, the narrow embedded-`select` full-arm typed-wrapper variant, the embedded-`select` partial value-if suffix typed-wrapper variant, and source-backed embedded value-parent wrappers: partial suffixes under `drop`, `call`, `unary`, `binary`, `compare`, `convert`, `load`, `local.set`, `local.tee`, `global.set`, `store`, `br` payload, `return`, `ref.is_null`, `table.get`, `table.set` index, `table.copy` dest, `call_indirect`, `memory.grow`, `table.grow`, `table.fill`, `memory.fill`, `memory.copy`, and `memory.init`, plus full multi-root suffixes under `call` and `binary`;
