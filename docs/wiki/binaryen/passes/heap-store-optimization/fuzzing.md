...
- catchable `try_table` / `throw` skip-local-set hazards where exception control can skip the fresh-constructor `local.set` before a later same-local `struct.set`.
