...
The broadened random-all profile exposed a second HSO-owned gap: the raw preflight skipped every allocation-bearing function without a `struct.set`, but Binaryen's `optimizeBlock(...)` still pushes a `local.set(struct.new*)` toward later operations while searching for a future store. Starshine now routes local-write plus struct-allocation functions through HOT even when no `struct.set` exists. A public-pipeline regression locks allocation sinking across dropped `if` and branch-result block roots, and `1000/1000` cases from `local-subtyping-control-refinalize` are now normalized with zero failures. Truly allocation-free functions retain the `no-heap-store-candidates` raw skip.

Final explicit-v131 matrix evidence uses `_build/native/release/build/cmd/cmd.exe` and `.tmp/binaryen-version-131-bin/bin/wasm-opt` with `--require-binaryen-version 131`:

- regular GenValid, seed `0x5eed`: `100000/100000` normalized, zero mismatches or failures, canonical totals exactly equal at `422,113,416` bytes, Binaryen cache `100000/0` hits/misses; evidence `.tmp/pass-fuzz-hso-final-genvalid-100000-20260901/`;
- explicit wasm-smith, seed `0x5eed`: `9956/10000` compared and normalized with zero mismatches or Starshine failures; the remaining `44` are Binaryen-v131 oracle failures (`39` zero-sized recursive groups, `3` bad section sizes, `1` invalid tag index, and `1` table index out of range), all served from the failure cache on the final run; evidence `.tmp/pass-fuzz-hso-final-wasm-smith-10000-20260901/`;
- dedicated nine-leaf aggregate, seed `0x5eed`: `2155` normalized plus `7845` `local-cleanup-debris` matches, zero residual mismatches or failures. All `1068` selected side-effects cases now compare-clean, and canonical totals are `2,707,970` Starshine versus `2,723,733` Binaryen bytes (`7845` Starshine-smaller, `2155` equal); evidence `.tmp/pass-fuzz-hso-final-dedicated-10000-20260901/`;
- current broad `random-all-profiles`, seed `0x5555`: `9748` normalized and `252` raw output-shape residuals, zero failures. The reviewed local-cleanup normalizer accounts for `168`; the remaining `84` are exclusively `remove-unused-brs-cleanup` / `remove-unused-brs-control` inputs whose functions all trace `skip-raw reason=no-heap-store-candidates`. Three-way original/Starshine/Binaryen semantic-v2 replay reports `84/84` `all-equal`, zero blocked or semantic mismatches, while Starshine canonical output is smaller in all `84`. Agent classification is pass-independent HOT/Binaryen expression-scheduling representation drift, not an HSO behavior gap. Evidence `.tmp/pass-fuzz-hso-final-random-all-10000-20260901/`, `.tmp/pass-fuzz-hso-parity-fixed-random-all-after-sink-10000-20260901/`, and `.tmp/pass-fuzz-hso-random-all-envelope-semantic-replay-20260901/`.
...
- `.tmp/pass-fuzz-hso-v131-spot-post-rebase-ordinary-1000`: `1000/1000` exact normalized matches, zero mismatches/failures, Binaryen cache `1000/0` hits/misses;
- `.tmp/pass-fuzz-hso-v131-spot-post-rebase-dedicated-100`: `16` exact plus `84` cleanup-normalized matches under `local-cleanup-debris`, zero mismatches/failures, Binaryen cache `0/100` hits/misses.
