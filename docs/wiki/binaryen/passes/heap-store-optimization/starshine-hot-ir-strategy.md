...
- focused local evidence surfaces
  - [`src/passes/heap_store_optimization_test.mbt:476-4084`](../../../../../src/passes/heap_store_optimization_test.mbt) covers reduced constructor/store, old-field side-effect preservation, later-field call barriers, descriptor immutable/mutable global barriers, descriptor `local.get`, block-wrapped immutable global operands, descriptor `if` operands, descriptor block self-branch operands, branchless descriptor loops plus self-branching loop negatives, readonly-prefix, swap including memory/table growth and size operands, block-wrapped blockers, trapping-load and call-valued negatives, constructor-ping-pong boundaries, branch, raw-prefix, and wrapper-cleanup regressions
