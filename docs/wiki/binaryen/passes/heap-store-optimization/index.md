...
- `try_table` wrappers are a no-fold boundary for the probed ordinary-store, same-effect bulk-root, and cross-family growth-root swap shapes.
  ...
  - Closure note `1109` removes the exact descriptor `ref.cast` blocker by adding exact-cast decode/encode/local coverage and proving the exact probe preserves `struct.set`; remaining closeout blockers are HSO-I performance disposition and HSO-J final validation/compare/O4z/docs/backlog closeout.
