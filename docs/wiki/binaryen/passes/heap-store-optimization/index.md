...
- Allocation-heavy candidate performance is now measured and partially improved, but not closed.
  ...
  - Validation refresh `1132` reran `moon test src/passes` (`3045/3045`) and a post-`1131` 10000-case direct GenValid compare (`10000/10000` normalized, `0` mismatches/failures). HSO-J remains explicitly deferred because HSO-I still has not met the `<=2x` target, been superseded with stronger evidence, or been user-accepted.
