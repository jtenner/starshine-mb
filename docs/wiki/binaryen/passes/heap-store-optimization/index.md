...
- `try_table` wrappers are a no-fold boundary for the probed ordinary-store, same-effect bulk-root, and cross-family growth-root swap shapes.
  ...
  - Residual map `1108` summarizes closeout state after `1107`: the docs/wiki matrix has no broad named source-backed HSO behavior family left unmapped, but the pass is still incomplete until exact descriptor `ref.cast` is runnable and preserves `struct.set`, HSO-I performance is dispositioned, and HSO-J final validation/compare/O4z/docs/backlog closeout is rerun.
