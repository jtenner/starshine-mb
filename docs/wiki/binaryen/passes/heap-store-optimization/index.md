...
- Target-local hazards are checked for each attempted chain movement.
  ...
  - Follow-up `0860` fixed descriptor later-field global conflict handling: Binaryen folds when a later field reads mutable global `$g0` and the moved value writes unrelated `$g1`, but preserves `struct.set` when both touch `$g0`.
