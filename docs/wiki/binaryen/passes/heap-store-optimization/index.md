...
- Rust `wasm_opt::Pass` docs: <https://docs.rs/wasm-opt/latest/wasm_opt/enum.Pass.html>
  - Negative evidence only for this pass during the 2026-04-18 maintenance check: the published enum page says its exposed variants mirror command-line pass names with Rust capitalization conventions, but it still did not expose `HeapStoreOptimization`, so the Debian manpage remains the stronger non-GitHub naming source here.
