...
- `try_table` wrappers are a no-fold boundary for the probed cross-family ordinary-store swap shapes.
  ...
  - Coverage note `0911` confirmed the table-side counterpart: Binaryen preserves `table.size`, `try_table` / `i32.store`, and the later `struct.set`; Starshine already matched. These notes do not supersede the existing block/if/loop/br_table cross-family ordinary-store positives.
