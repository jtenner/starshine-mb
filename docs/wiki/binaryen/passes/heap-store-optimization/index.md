...
- `try_table` wrappers are a no-fold boundary for the probed memory-side cross-family ordinary-store swap shape.
  - Coverage note `0910` confirmed Binaryen preserves `memory.size`, `try_table` / `table.set`, and the later `struct.set`; Starshine already matched. This note does not supersede the existing block/if/loop/br_table cross-family ordinary-store positives.
