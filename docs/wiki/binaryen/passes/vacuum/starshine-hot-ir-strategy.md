...
- `vacuum raw-cleans empty-block call-prefix wrappers before void ifs`, `vacuum raw-unwraps nested empty-block call-prefix parent wrappers`, and `vacuum flattens single-if wrappers without leading empty prefixes`
  - prove the safe subset of the line-`29378` empty-block/call-prefix family: leading empty child blocks can be removed and singleton ancestors from the same empty-block chain can be unwrapped, while the rejected `defined=4370` single-`if` wrapper family still flattens instead of being preserved
