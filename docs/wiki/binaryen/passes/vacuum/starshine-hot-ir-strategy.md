...
- `.tmp/vacuum-current-o4z-neighborhood/slot23-pre22-vacuum-compare-sideeffect-child-final`: after adding the narrower child-only side-effect wrapper cleanup, the full slot23 replay reported Starshine/Binaryen whole-command `674.368ms/577.167ms`, pass-local `1.971ms/145.990ms`, raw `vacuum` `73.420ms`, other traced `544.702ms`, and untraced/runtime `54.275ms`; emitted raw sizes are `3098522/3113007`, while canonicalized compare artifacts are `3113500/3113007`. The focused red-first fixture failed before implementation because Starshine kept the child wrapper; the accepted rule does not unwrap the singleton parent and preserved the seven-function invalid-lower baseline. Direct smoke `.tmp/pass-fuzz-vacuum-1000-after-sideeffect-child` compared `1000/1000`, normalized `1000`, with `0` mismatches/failures. The ordered slot remains open: first diff is still `defined=4481 abs=4508`, now line `24763`, and the new residual is Starshine removing a block that Binaryen preserves.
