...
The current line-`29378` empty-block/call-prefix residual remains open. A 2026-06-30 experiment proved the isolated positive shape with `.tmp/vacuum-probes/empty-block-call-prefix-double-wrapper-before-if.wat`, but the corresponding native raw cleanup regressed the full slot23 replay to `defined=4370 abs=4397` and worsened accepted slot23 raw/canonicalized sizes, so it was reverted. Do not revive broad leading-empty-block or call-prefix wrapper cleanup without a parent/control-shape discriminator that preserves the accepted `defined=4481 abs=4508` progress and avoids the `defined=4370` regression.
