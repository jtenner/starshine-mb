...
- `.tmp/vacuum-current-o4z-neighborhood/slot23-pre22-vacuum-compare-func-batch-final`: after changing batch repair to validate only changed candidate bodies under one reused module environment, the slot23 timing-only replay dropped Starshine whole-command time further to `916.276ms` vs Binaryen `580.744ms`, while preserving the same seven invalid-lower skips. `guard:vacuum-writeback-batch` is now `113.338ms`, aggregate `pipeline` is `460.791ms`, raw `vacuum` is `69.378ms`, Starshine/Binaryen pass-local is `23.454ms/153.775ms`, and raw Starshine/Binaryen size remains `3119563/3113007`. The ordered-neighborhood local-set/call wrapper gap remains open; direct smoke `.tmp/pass-fuzz-vacuum-1000-after-func-batch-writeback` compared `1000/1000` with `1000` normalized and `0` mismatches/failures.
