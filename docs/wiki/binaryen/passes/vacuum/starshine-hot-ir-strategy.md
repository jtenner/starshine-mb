...
Perf tracing now also makes the slot-level cost boundary explicit: the hot pipeline emits an aggregate `pipeline` timer, `vacuum` raw preclean/no-candidate probes emit `raw:vacuum-*` timers, and `vacuum` writeback validation emits either `guard:vacuum-writeback-batch` or per-function `guard:vacuum-writeback:*` fallback timers. The current default batches validation after all candidate rewrites, then repairs invalid functions by restoring their originals and revalidating; the older per-function guard remains as a fallback if batch repair cannot identify a function-local validation error. These timers and repairs protect writeback validity; they do not add a hidden cleanup phase.
