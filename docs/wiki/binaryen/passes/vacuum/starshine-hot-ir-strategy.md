...
Perf tracing now also makes the slot-level cost boundary explicit: the hot pipeline emits an aggregate `pipeline` timer, `vacuum` raw preclean/no-candidate probes emit `raw:vacuum-*` timers, and the `vacuum` per-function writeback-validation guard emits `guard:vacuum-writeback:*` timers. These timers are instrumentation only; they do not add a hidden cleanup phase.
