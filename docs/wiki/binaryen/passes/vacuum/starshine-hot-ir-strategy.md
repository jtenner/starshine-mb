...
- `.tmp/pass-fuzz-vacuum-random-all-profiles-1000-after-raw-nontrap-final`: after adding the guarded raw pure-expression cleanup for call/local-set and large structured-write hazards, broad random all-profiles GenValid compared `1000/1000` cases with `1000` normalized matches, `0` cleanup-normalized matches, and `0` mismatches/failures. This replaces the earlier timeout on the 20-case debug lane, where mismatch reduction on the coverage-forced portable case 7 had hidden the underlying pure-debris parity gap.
- `.tmp/pass-fuzz-vacuum-wasm-smith-1000-after-raw-nontrap-final`: refreshed explicit wasm-smith smoke compared `997/1000` cases with `997` normalized matches, `0` mismatches, and `3` Binaryen/tool parser failures (`case-000029` and `case-000573` empty recursion groups, `case-000662` invalid tag index).
