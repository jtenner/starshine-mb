...
On the exact 3,140-byte depth-1024 wasm fixture, 11-sample command medians move Starshine `10.800ms -> 1.660ms` (`6.506x`, `-84.630%`) versus Binaryen v131 `2.601ms`. Baseline Starshine retained all 1,024 wrappers. The recorded Starshine output emits 66 raw bytes because it preserves the import name section; after `--strip-debug`, current and Binaryen are byte-identical at 48 bytes with SHA-256 `cc435f18e75f3b2e0730373100929101de3b5824cb18200496bc3dfe71e1de18`.
...
- recorded wasm-smith lane: `9934/9956` normalized plus 22 pre-existing byte-identical clean/current Starshine wins and the same 44 Binaryen-v131 tool failures. Twenty-one wins serialize valid empty function bodies instead of Binaryen's one-byte `nop`; case 3694 remains the documented six-byte loop/local-carrier win.

The recorded fresh canonical production attribution is intentionally neutral. Three clean/current no-trace medians are `1705.620/1706.197ms` versus paired Binaryen v131 `711.836ms`; raw preprocessing is `559.636/564.746ms`, batch writeback `237.698/235.353ms`, and every clean/current output is byte-identical at SHA-256 `519d36fcf0e88121774ad188ceafba81b2d25d376517fc63fa02f074462c07a0`. This misses the former isolated `<=2x` threshold of `1423.672ms`, but the user explicitly accepted that direct-pass miss after the complete production O4z comparison below.
