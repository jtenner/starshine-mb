...
# `mark-js-called` Fuzzing Profile

Recommended smoke lane: run the ordinary mixed-generator compare-pass lane for this pass:

```sh
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass mark-js-called --out-dir .tmp/pass-fuzz-mark-js-called --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe
```
