...
**Gap.** Ordinary scalar removal, scalar localization, solved/general constant materialization, and forwarding components use validated plans, but producer-specific suffix/ref-null, GC, results, control reconstruction, and older specialized paths still retain separate application tails. Direct-call and typed-select operand recovery now share canonical ordered `DaeValueSlice` collection; non-scalar result/control carriers are not yet universally represented by that one operand model.
