...
- Dropped-result producer consolidation beyond ordinary-core dispatcher application and optimizing batch transactions. All ordinary-core admissions, including argument-side repair, dispatch `DaeDroppedResultWork`, report callee/result plus caller/drop effects, and require the authoritative current ownership/observer/target-tail boundary guard. Same-parameter complete-result removal now projects exact `DaeRemoveResult` actions, caller rewrites, callee code, type/function sections, effects, invalidations, validation, and rollback into `DaeRewritePlan`; parameter-changing and specialized products retain the family fallback. Exact target evidence replaces the former module-wide tail barrier, while target-less specialized helpers remain fail closed. Obsolete selected schedulers are deleted; specialized result/control producers and unified return repair remain open.
