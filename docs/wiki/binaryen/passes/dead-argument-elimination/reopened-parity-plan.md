...
The reopen remains active only for performance signoff. On the dense-call artifact-scale probe (`3,597,499` bytes, `5,374` defined functions, roughly `251,000` direct calls), cached lifecycle facts and single-owned commit handling reduced Starshine plain DAE from `>1500s` to `191.894s`; the valid output remains byte-identical across the clean optimization runs. Binaryen v131 completes in `0.356s`. Focused medians remain below one second, so the remaining fixed-loop/selected-lane cost is a specific artifact-scale performance blocker rather than a behavior, validity, lifecycle, or residual-classification gap. Close the plan after that gap is fixed or explicitly accepted by release review.
...
**Gap.** Ordinary scalar removal, scalar localization, solved/general/verified-suffix/ref-null constant materialization, and forwarding components use validated plans, but GC, results, control reconstruction, and older specialized paths still retain separate application tails. Direct-call and typed-select operand recovery now share canonical ordered `DaeValueSlice` collection; non-scalar result/control carriers are not yet universally represented by that one operand model.
...
- `DaeParameterAction` — `DaeKeepParameter`, `DaeRemovePureParameter`, `DaeRemoveParameterReplayEffects`, `DaeLocalizeParameterAndRetry`, `DaeMaterializeConstantParameter`, `DaeRefineParameterType`. Ordinary unread analysis emits the removal/localization actions; uniform and general constant producers emit materialization; GC type refinement remains family-owned.
...
- Full immutable validated plan commit/rollback for every signature-changing family. The generic older-family transaction now normalizes touched ownership and attempts `DaeRewritePlan` before constructing a family wrapper. Nonreconstructable signature changes fail closed; fallback is limited to proven body-only products and the explicitly flagged dead-suffix parameter-restoration transaction, which adds back one source-required input outside the shrinking/refining action model. Stale/call/validation failures remain terminal. The scalar plan owns detailed parameter edits; forwarding component plans atomically compose members and triggered ref-null materialization; and every dispatcher family attempts `DaeRewritePlan` first; `DaeFamilyRewritePlan` remains for body-only control/type reconstruction, unsupported phase-local products, ordinary-core fallbacks, reverse/low exact revisits, low exact cleanup, broad immutable-global and adjacent-root revisits, dead-suffix parameter restoration, and boundary cleanup. Family application derives touched ownership from exact body/signature deltas, ignores inactive dead-suffix calls in identity totals while still rescanning active bodies for stale inserted calls, and folds active-boundary unreachable-tail repair into the candidate before final validation. The three plan kinds and their successful applications now carry `DaeCommitEffects`: exact structural touched, per-definition `worth_decided`, semantic worth, parameter/result/body/local/control/type/metadata consequences, invalidation domains, and decision reasons. Work outcomes and the DAEO adapter preserve these effects; a producer owns worth only for definitions it explicitly decided, and no local-count compatibility inference remains. Committed work-item effects also select before/after dependency requeue by invalidation domain; result-domain and metadata-only focused tests pin exact versus local propagation, while missing domains retain the conservative caller/callee fallback. The general remove/constant compatibility entry now reconstructs current original/graph facts and delegates to the scalar plan, including stack-carried-void slice recovery when requested. Verified immediate-suffix and selected ref-null compatibility producers now use that scalar lifecycle and direct effects. The late semantic global-suffix specialization projects its custom caller/callee candidate into `DaeRewritePlan`, validates ordered parameter actions against immutable original ownership and current epochs, and feeds exact producer effects back into core worth selection. Signature-plan effect exactification now reports type-section and metadata changes from actual before/after products rather than setting both fields unconditionally. Ordinary and optimizing-batch GC parameter/result refinement plus every specialized dropped-result candidate now require exact action-bearing `DaeRewritePlan`; signature-changing dropped-result work no longer has a family-plan fallback. The dispatcher now proves the family fallback is body-only by comparing every defined-function signature; any signature-changing control/type candidate that cannot project into `DaeRewritePlan` fails closed with type-reconstruction diagnostics. Remaining body-only control/type helpers still need complete common control-product ownership.
- Constant materialization now uses the common scalar transaction for solved uniform values and every caller of the general remove/constant compatibility entry. Exact-literal/immutable-global work and verified scalar-plan suffixes become `DaeMaterializeConstantParameter` actions, honor boundary ownership, preserve older stack-carried operands, and commit through the complete candidate. The selected exact-literal scan/mutate helper is removed. The former direct suffix/ref-null caller-scope and callee-finalization tails are deleted; both compatibility producers now require exact scalar-plan evidence and atomic validation.
- Localization consolidation beyond the ordinary scalar transaction. The scalar plan owns typed scratch-local insertion, localized removed-slot projection, exact caller epochs, candidate validation, commit, rollback, and stack-carried void-effect recovery. Dispatcher retries reuse supplied original/current facts and report the plan's exact caller effects; the former manual retry and synthesized effect record are deleted. Forwarding/broad-family localization still needs the same lifecycle.
- GC result and remaining parameter-evidence producer consolidation beyond the dispatcher. Ordinary-core GC parameter and result refinement enter distinct work families but project complete candidates into one `DaeRewritePlan` with exact kept/refined action vectors, including simultaneous parameter/result narrowing. The optimizing batch now commits refreshed per-definition combined candidates through the same signature plan and merges their exact effects. GC result closure derives nested caller-to-callee result dependencies and forward requeue from stable active call identities, removing its terminal top-level-call scan and productive cap. Direct `ref.null` results enter the same instruction candidate API as allocations, locals, function references, and typed calls across concrete and abstract heap results, including call-ref/call-indirect arguments and targets from canonical branch-free multi-instruction value slices plus declared `global.get`, typed `struct.get` fields, `array.get` elements, and imported/defined `table.get` reference types, ordinary and descriptor-bearing struct/array constructors, exact `cont.new` / `cont.bind` constructors, `string.const`, all represented string array constructors, `string.concat`, `string.as_wtf16`, `stringview_wtf16.slice`, `ref.i31`, typed `ref.cast`, `ref.cast_desc_eq`, `ref.as_non_null`, `ref.get_desc`, and `extern.convert_any` / `any.convert_extern` result expressions with operand nullability/sharedness preserved while retaining observable effects, allowing null-bottom explicit returns to join with sibling/fallthrough leaves. Terminal and recursively discovered nested explicit returns plus multiple-return fallthrough values recover their complete branch-free operands through canonical `DaeValueSlice` evidence before candidate typing, so multi-instruction producers share one result API across all three paths. Explicit-return discovery also traverses represented legacy `try` protected and catch bodies, so exceptional-region returns constrain the fallthrough LUB; terminal all-return block/loop/if/try-table/legacy-try controls contribute their explicit-return LUB without requiring a fallthrough value; broader deterministic returned-value carriers and common finalization remain.
- Dropped-result producer consolidation beyond ordinary-core dispatcher and optimizing per-definition signature-plan application. All ordinary-core admissions, including argument-side repair, dispatch `DaeDroppedResultWork`, report callee/result plus caller/drop effects, and require the authoritative current ownership/observer/target-tail boundary guard. Shrinking/refining parameter/result signatures now project ordered kept/refined/removed actions, caller rewrites, callee code, type/function sections, exactified effects, invalidations, validation, and rollback into `DaeRewritePlan`, including partial multivalue projection and the optimizing direct-GC batch's per-definition commits. Inferred removed parameters use conservative replay-effects actions; only structurally unsupported products retain the family fallback. Exact target evidence replaces the former module-wide tail barrier, while target-less specialized helpers remain fail closed. Obsolete selected schedulers are deleted; specialized result/control producers and unified return repair remain open.
...
- Migrate remaining parallel GC/result/control/type consumers marked with owning `[O4Z-DAE-*]` TODOs.
...
- Build one returned-value evidence API covering implicit fallthrough, explicit `return`, terminal `block` / `if` / `loop`, typed and type-indexed controls, `select`, static `local.get` types, effect-preserving typed `local.tee` writes, typed aggregate reads including `struct.atomic.get` / `array.atomic.get`, simple nested carriers, typed `try_table` normal fallthrough with catch preservation, typed legacy-`try` normal fallthrough when all catches are nonfallthrough, tuples, flat multivalue lanes, and represented exceptional exits.
...
- Extend the type graph utilities for exactness, nullability, abstract heap types, recursive groups, shared types, and mixed rec groups. Concrete subtype and ancestor traversal now resolves every recursive-group member through flattened type-section slots and converts group-local `RecIdx` supertypes relative to the owning group's flattened base, matching signature lookup/reuse/append. Ordinary equivalent lookup, batch lookup, and selected typeidx-control matching share one metadata-safe simple-function predicate and can reuse grouped members; pruning and mixed-group edit breadth remain open. Unsupported proposal forms must fail closed with a representation-blocker test.
...
**Gap.** All ten work-family labels have handlers, but result/type/body/cleanup invalidations remain conservative and several specialized collectors still perform their own lifecycle. The canonical core now emits deterministic production diagnostics for plain DAE and DAEO, including every counter and all stable reason codes. Additive produced/applied counts now travel in `DaeCommitEffects`, so optimizing unread/direct-GC/dropped-result batches preserve one diagnostic event per committed signature plan rather than one event per aggregate transaction. Ordinary DAEO nested replay contributes exact touched-function and scheduled shared-roster pass counts to the delayed primary record; selected-only, guard, producer-specific mini-roster, convergence-wave, and specialized-loop counters are not yet integrated.
...
**Gap.** Starshine shares the nested roster and has correct slot placement. The arbitrary touched-count, module-count, local-count, instruction-count, encoded-body-size, removed-local, call-free-shape, and folded-callee replay selectors were removed on 2026-07-22. Ordinary replay now consumes merged producer-owned `DaeCommitEffects`: changed boundaries, body localization/materialization, result repair, and exact callers are admitted only when the producer marks them semantically worth optimizing; callsite-only structural edits remain excluded. No local-count or other legacy worth inference remains. Changed-boundary local compaction and immutable-field folding execute transactionally at every cardinality. The former private broad-cleanup roster and folded-callee recursion are deleted, so `precompute-propagate` plus the shared default function roster runs exactly once. The selected literal/global blocker was resolved by the common uniform-value lifecycle; a 101-definition replay and explicit v131 dead-tee/immutable-global cases remain green. The core reports whether the one-wave single-caller stop was consumed; DAEO carries that state through post-cleanup reentry. Final convergence now drains an exact producer-owned seed across represented epoch-bearing work families, applies control/type followups, requeues duplicate-suppressed effect invalidations, runs the shared roster only for committed worth, and retains only valid strictly smaller waves. Recursive whole-pass retry is deleted. Ordinary shared-roster replay now records its exact admitted touched count and each scheduled pass in the primary DAEO diagnostics record. Remaining work is producer-specific worth/invalidation plus selected, specialized-mini-roster, and convergence-wave diagnostic ownership.
...
- Keep the removed broad touched/module/local/instruction guards and private nested roster absent. If a defensive runtime budget is introduced, it must be telemetry-visible and must not change the semantic worth set or exact-once shared-roster execution.
