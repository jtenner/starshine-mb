...
**Gap.** Starshine shares the nested roster and has correct slot placement. The arbitrary `touched_count > 8` and module `>100` replay skips were removed on 2026-07-22, and ordinary replay now derives a semantic worth set from changed signatures, result-change callers, and exact local-count growth for transitional scratch-local cleanup instead of replaying every structurally changed callsite caller or every touched caller with preexisting locals. Producer-owned body-only localization/materialization worth now admits the exact affected definitions without the former encoded-body-size threshold; callsite-only structural edits remain excluded. Local-count and instruction-count do not suppress semantic nested replay: those checks belong only to the earlier broad changed-boundary fast-skip policy, so the authoritative 129-local cleanup transaction runs before the per-function nested roster. The selected literal/global blocker from the first module-guard trial was resolved by the common uniform-value lifecycle. A fresh 101-definition replay passes; explicit Binaryen v131 replays confirm the newly exposed dead-tee removal and immutable-global fold are oracle output, not regressions. Every validated exact changed boundary now enters the filtered roster without the former encoded-size, removed-local, call-free, or folded-callee admission policy. The core reports whether the one-wave single-caller stop was consumed; DAEO carries that state through post-cleanup reentry and final retries, which accept only valid strictly smaller modules and continue until unchanged or non-retainable. Broad-cleanup fast-skip ownership and replacement of the remaining broad-triggered whole-pass retry with exact effect invalidations remain open.
