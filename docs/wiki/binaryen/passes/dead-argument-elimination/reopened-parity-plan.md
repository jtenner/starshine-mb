...
- Dropped-result producer consolidation beyond ordinary-core dispatcher application and optimizing batch transactions. All ordinary-core admissions, including argument-side repair, now preserve source ordering while dispatching `DaeDroppedResultWork` and reporting callee/result plus caller/drop effects; specialized result/control producers, common observer evidence, and unified return repair remain open.
