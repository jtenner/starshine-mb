...
- GC result and remaining parameter-evidence producer consolidation beyond the dispatcher. Ordinary-core GC parameter refinement now enters `DaeGcParameterRefinementWork`, preserves producer effects through the family transaction, and shares application with the optimizing batch; deterministic LUB discovery and old-type local repair remain.
