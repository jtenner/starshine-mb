...
- Dropped-result producer consolidation beyond ordinary-core dispatcher application and optimizing batch transactions. All ordinary-core admissions, including argument-side repair, now preserve source ordering while dispatching `DaeDroppedResultWork` and reporting callee/result plus caller/drop effects; the obsolete selected-definition wrapper, descending cap/order collectors, empty fallback carrier, and rejected bucketed-attempt scaffold are deleted, with useful fact/timing/touched coverage migrated to the live definition helper; specialized result/control producers, common observer evidence, and unified return repair remain open.
...
**Gap.** Starshine shares the nested roster and has correct slot placement. The arbitrary touched-count, module-count, local-count, instruction-count, encoded-body-size, removed-local, call-free-shape, and folded-callee replay selectors were removed on 2026-07-22. Ordinary replay derives a semantic worth set from changed signatures, result-change callers, and exact local-count growth for transitional scratch cleanup instead of replaying every structurally changed callsite caller or every touched caller with preexisting locals; producer-owned body localization/materialization admits its exact definitions, while callsite-only structural edits remain excluded. Changed-boundary local compaction and immutable-field folding execute transactionally at every cardinality. The former private broad-cleanup roster and folded-callee recursion are deleted, so `precompute-propagate` plus the shared default function roster runs exactly once. The selected literal/global blocker was resolved by the common uniform-value lifecycle; a 101-definition replay and explicit v131 dead-tee/immutable-global cases remain green. The core reports whether the one-wave single-caller stop was consumed; DAEO carries that state through post-cleanup reentry and final retries, which accept only valid strictly smaller modules. Replacement of the remaining whole-pass retry and legacy worth inference with exact producer-effect invalidations remains open.
