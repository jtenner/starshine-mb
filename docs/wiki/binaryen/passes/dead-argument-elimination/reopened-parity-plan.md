...
- Full immutable validated plan commit/rollback for every signature-changing family. The ordinary scalar plan owns a preconstructed complete candidate module, an independent type epoch, and exact path-plus-kind direct-call identities. Forwarding component plans now enforce the same type epoch and exact call identities across every member, plus original ownership and member/caller epochs, before validating and committing the whole component; post-component null materialization still needs to move inside that candidate.
...
- Commit recursive forwarding components atomically. The component transaction now rejects stale type/call identities and any member that escaped in the original module before changing a signature; post-component specialization must still be folded into the same validated candidate so a cycle never exposes partial follow-up edits.
...
**Gap.** Starshine shares the nested roster and has correct slot placement, but full replay is still selected by `touched_count > 8`, module-size, local-count, instruction-count, and material-caller thresholds. Those guards are described as optional performance policy, yet they can omit Binaryen-visible optimizing cleanup and the small-module computed queue still has a cross-phase availability split.
