...
| Late selected cleanup | active definitions `311/313`; Func408 pair-if work now uses the touched-definition frontier | 2/4 | the exact carrier recognizer succeeds at arbitrary definition `709`; no identity admission remains for Func408 and artifact output is unchanged |
