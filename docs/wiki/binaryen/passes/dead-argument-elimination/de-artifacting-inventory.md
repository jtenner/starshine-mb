...
| Nested function/module size skips | full-roster replay omitted for modules above 100 definitions or touched functions above 128 locals / 1,000 instructions | 3 | retained optional performance budgets with `guard=performance optional=true` diagnostics; the ordinary replay now uses a semantic worth bitmap with exact caller local-count growth instead of every structurally touched caller or any caller with preexisting locals, but removing the module guard still preempted selected literal/global producer ordering in focused large fixtures | reopen after those producers move into the common lifecycle and a red-first large-module replay preserves both selected fixtures; then measure the retained artifact |
