...
| Selected dropped-result fallbacks | former definitions `3566, 3732, 3814` are removed from production; exact fact-selected candidates now run through `DaeDroppedResultWork` with refreshed graph epochs | 6 | fixed fallback list plus its selected wrapper, descending cap/order collectors, empty fallback carrier, and rejected bucketed-attempt scaffold deleted; live fact/timing/touched tests target the definition helper directly; retain behavior fixtures and reopen only for a classified exact result-dependency gap |
...
| Late selected cleanup | former definitions `311/313/323/327/408/1794` are identity-free | removed | global suffix uses one exact current caller whose body changed from the immutable original, then commits through a family plan against original exposure/current epochs and publishes producer effects; all other late families use touched/dependency/shape evidence |
...
| Nested module / broad-cleanup function size policies | removed | removed | module, touched-count, local-count, and instruction-count cardinality no longer select semantic local compaction or nested replay. Changed-boundary local cleanup is transactional at every size, and the former private broad-cleanup roster/folded-callee recursion is deleted so the shared `precompute-propagate`-first roster runs exactly once. A 101-definition replay and the prior literal/global cases remain green | reopen only if a measured defensive budget can suppress optional work without changing the semantic worth set or exact shared-roster execution |
| Material touched caller threshold | removed for producer-owned body-only worth functions | removed | body localization/materialization effects now admit their exact definitions to the filtered roster without consulting encoded size; callsite-only structural edits remain excluded, and validation plus non-larger per-definition retention stay authoritative | reopen only if a producer can emit body-localization/materialization worth without the affected definition entering this frontier |
| Exact changed-boundary heavy replay policy | removed | removed | every validated changed boundary enters the filtered roster regardless of encoded size, removed-local count, control shape, or folded-callee status. The core reports one-wave single-caller consumption, DAEO carries it across cleanup reentry, and the producer-effect frontier drains represented work families plus touched nested cleanup while retaining only valid strictly smaller waves | reopen only if a changed boundary is missing from the deterministic frontier or convergence performs a second throttled parameter wave |
