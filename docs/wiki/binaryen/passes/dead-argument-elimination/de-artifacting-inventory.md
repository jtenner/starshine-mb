...
| Large uniform constants | broad semantic identity/pure-expression/constructor flow now runs at every module size; separate `>4096` heavy immutable/global prioritization remains | 1/3 | generic correctness path is size-independent; heavy cleanup prioritization still uses cardinality | retain only measured profitability guards |
...
| Nested touched-set skip | broad `touched_count > 8` remains; admitted touched functions now run the exact shared Binaryen-v131 `precompute-propagate -> default O4z function pipeline`, and small call-free functions with any removed local still enter isolated cleanup instead of requiring `>=16` removed locals | 3 | the private DAE roster is removed and artifact/smoke output is unchanged; replace or justify the remaining cardinality gate using exact worth-optimizing evidence and accepted runtime medians |
