...
| Early selected body cleanup | active definitions `236, 267, 268`; production schedules `233`, `237`, and `256` removed | 2/4 | v131 preserves the constant-if and exported loop shapes for `233/256` fixtures, while `237` was dead policy; move remaining reusable cleanup into shared passes |
...
| Late selected cleanup | active definitions `288, 298, 299, 311, 313, 408` and later local/control lanes; production schedules for `246`, `287`, `867`, and exact-literal `326/327/328/1795` removed | 2/4 | removed schedules are byte-identical dead policy; exact direct callees of changed bodies use semantic uniform invalidation; remaining local/control transforms belong in shared touched passes |
