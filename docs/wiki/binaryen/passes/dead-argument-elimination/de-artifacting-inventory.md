...
| Exact changed-boundary heavy replay policy | encoded body `>=2048`, removed-local count `>=16`, exact small call-free cleanup, or folded-callee evidence | 3 | still selects the expensive isolated roster for changed-boundary definitions that were not separately admitted by producer-owned body-only worth; removing it immediately exposed repeated same-pass single-caller pruning, so exact convergence must preserve Binaryen's one-wave throttle rather than merely widening the old loop | replace with dependency-driven boundary/nested convergence plus an explicit one-wave throttle, then measure the retained artifact |
