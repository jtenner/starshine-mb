...
| Late selected cleanup | former definitions `311/313/408` are identity-free | removed | pair-if uses touched shape; terminal-call families use immutable original signature/body shape intersected with actual current body change, batched to local fixpoint |
