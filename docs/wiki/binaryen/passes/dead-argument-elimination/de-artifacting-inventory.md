...
| Dropped-result dispatch | ordinary and high-payoff productive caps plus both `8192` disablements removed; ascending through `4096`, descending above it remains | 1/2 | work is generic and finite, but ordinary ordering still depends on module cardinality | derive ordinary ordering from result dependencies rather than definition count |
