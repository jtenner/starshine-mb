...
| Immutable-global revisit | every module size; exact immediate materializable call-target discovery | removed | the small all-definition `<=4096` wave was deleted; one current-call-fact snapshot is reused across rejected candidates and refreshed only after a commit | reopen only for a minimized exact immutable-global dependency miss |
...
| Nested touched-set skip | full-roster replay is omitted when `touched_count > 8` | 3 | retained optional profitability budget. Mandatory signature/call repair plus validated changed-boundary `precompute-propagate`, `remove-unused-brs`, and `vacuum` fallbacks run first. Removing the budget deletes non-oracle wrapper params and raised the retained artifact to about `62s`; the retained policy preserves the `2,991,168`-byte output and current `25.440s` productive median | reopen only with an exact Binaryen `worthOptimizing` set that preserves the large-wrapper oracle and does not regress the accepted median |
| Nested function/module size skips | full-roster replay omitted for modules above 100 definitions or touched functions above 128 locals / 1,000 instructions | 3 | retained optional performance budgets with `guard=performance optional=true` diagnostics; all mandatory repair and cheap validated fallback cleanup remains active | reopen when touched adapters can prove a no-regression per-function worth/profitability decision on the retained artifact |
