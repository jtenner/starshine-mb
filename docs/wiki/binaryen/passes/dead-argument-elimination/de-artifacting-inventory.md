...
| Nested module / broad-cleanup function size policies | full-roster replay omitted above 100 definitions; touched functions above 128 locals / 1,000 instructions previously skipped the roster too | 3 | local/instruction cardinality no longer suppresses semantic nested replay; those two checks remain only in the earlier broad changed-boundary fast-skip discriminator so oversized cleanup keeps transactional ownership before the roster. The module guard remains a visible optional skip because its removal preempted selected literal/global producers | migrate the broad-cleanup discriminator to exact candidate facts, then repair producer ordering and remove the module skip; measure the retained artifact afterward |
