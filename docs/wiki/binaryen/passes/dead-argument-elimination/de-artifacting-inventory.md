...
| Late selected cleanup | active definitions `288, 298, 299, 311, 313`, plus pair-if work at `408`; Func408 const-local identity is generic and its nop schedule is removed | 2/4 | v131 preserves nops when no DAE boundary changes; original-shape const-local cleanup is generic and 60 bytes smaller |
