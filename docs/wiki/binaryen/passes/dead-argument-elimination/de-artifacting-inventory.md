...
| Nested function/module size skips | full-roster replay omitted for modules above 100 definitions or touched functions above 128 locals / 1,000 instructions | 3 | retained optional performance budgets with `guard=performance optional=true` diagnostics; the ordinary replay now uses a semantic worth bitmap with exact caller local-count growth instead of every structurally touched caller or any caller with preexisting locals, but focused removal trials still exposed owner-order regressions: the module guard preempted selected literal/global producers, and the function-size guard let nested replay consume a 129-local boundary before broad changed-boundary transaction ownership | reopen after those producers move into the common lifecycle and a red-first large-module replay preserves both selected fixtures; then measure the retained artifact |
