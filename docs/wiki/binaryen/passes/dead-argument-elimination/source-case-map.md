...
| call-ref-tail family | Use typed `return_call_ref` evidence; tolerate unreachable targets without assertion. | Call-ref result-slice evidence across concrete and abstract heap signatures and canonical branch-free multi-instruction argument/target slices and declared `global.get` / typed `struct.get` / `array.get` / `table.get` / ordinary and descriptor-bearing struct/array constructor / `cont.new` / `cont.bind` / `string.const` / `string.concat` / `string.as_wtf16` / `stringview_wtf16.slice` / `ref.i31` / typed `ref.cast` / `ref.cast_desc_eq` / `ref.as_non_null` / `ref.get_desc` / `extern.convert_any` / `any.convert_extern` results, including effectful producers whose evaluation is preserved, public call-ref tail tests, and unreachable producer tests. | Exact represented evidence; unowned reference callers remain conservative. | Returned-value evidence. | Reopen only with exact typed-reference ownership proof. |
