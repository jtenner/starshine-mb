...
| `removeReturnValue` / `ReturnUtils::removeReturns` | Remove a result only when all owned calls drop it and tail/unseen constraints permit it; preserve bottom with `call; unreachable`. | Observer graph, dropped-result work, common complete-result action plan for unchanged or all-removed parameters, and specialized dead-suffix plans. | Exact for represented scalar/bottom cases; lifecycle partial. | Ordinary and specialized complete-result removal is plan-owned for unchanged or all-removed parameters; close when partial-parameter repairs share one lane-aware atomic result plan without stale double rewriting. |
