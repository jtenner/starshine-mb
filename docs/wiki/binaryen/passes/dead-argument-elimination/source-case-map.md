...
| `$refine-return-many` | Join multiple explicit returns and fallthrough. | `dae GC result refinement joins multiple explicit returns and fallthrough`, the null-bottom explicit-return regression, and `dae GC result refinement includes legacy try explicit returns`. | Exact. | Returned-value evidence. | Typed exceptional payload, throw/delegate flow, and unrepresented continuation exits remain fail closed. |
