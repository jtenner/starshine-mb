...
| call-ref-tail family | Use typed `return_call_ref` evidence; tolerate unreachable targets without assertion. | Call-ref result-slice evidence across concrete and abstract heap signatures and canonical branch-free multi-instruction argument/target slices and declared `global.get` / typed `struct.get` / `array.get` / struct/array constructor / `string.const` / `ref.i31` / typed `ref.cast` / `ref.as_non_null` results, including effectful producers whose evaluation is preserved, public call-ref tail tests, and unreachable producer tests. | Exact represented evidence; unowned reference callers remain conservative. | Returned-value evidence. | Reopen only with exact typed-reference ownership proof. |
