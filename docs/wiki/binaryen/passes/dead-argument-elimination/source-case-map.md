...
| `ParamUtils::removeParameters` | Remove parameters and all owned direct operands together; preserve unremovable effects by localization; under traps-never-happen, erase trap-only operands; allocate replacement locals and repair nondefaultable locals. | Scalar/component/family candidates plus value-slice replay/localization, TNH-aware trap-only removal, local projection, and exact producer-owned dead-suffix caller cleanup. | Exact or equivalent-smaller for represented forms; lifecycle exact. | Reopen for a new operand/control form, stale plan, invalid caller repair, or a residual without a measured Starshine win. |
| `ParamUtils::localizeCallsTo` | Child-localize every owned target call, then repair nested EH pops. | Typed scratch-local scalar/component work with canonical slices, exact caller epochs, producer effects, and stale requeue. | Exact for represented direct/`return_call` forms; fail closed on unsupported escaping control. | Reopen for a represented localization family that bypasses work outcomes or loses operand/effect order. |
| `refineArgumentTypes` / `TypeUpdating::updateParamTypes` | Strict reference LUB over every owned direct actual, skipping unread parameters; repair old-type writes/tees/locals. | Shared exact-caller value-slice collection, strict LUB, old-type local repair, and common parameter/result signature work outcomes for ordinary and optimizing batches. | Exact for represented behavior; lifecycle exact. | Reopen for a new represented GC producer/type user or stale type-finalization regression. |
| `LUB::getResultsLUB` / `ReFinalize` | Join every returned value, explicit return, and tail return source; update call and parent types. | GC result worklist, canonical fallthrough value slices including external conversion nullability, represented legacy-`try`/typed-control return evidence, common result-action signature plans, and epoch-bearing control/type follow-ups. | Exact for represented source cases; lifecycle exact, with unsupported value-producing exceptional exits fail closed. | Reopen for a new represented return carrier/type user or an atomic-finalization regression. |
| `removeReturnValue` / `ReturnUtils::removeReturns` | Remove a result only when all owned calls drop it and tail/unseen constraints permit it; preserve bottom with `call; unreachable`. | Observer graph, lane-aware dropped-result work, common shrinking/refining parameter/result plans, specialized dead-suffix outcomes, exact changed-function touched accounting, and bottom/control follow-ups. | Exact for represented scalar, partial-multivalue, and bottom cases; lifecycle exact. | Reopen for a new observed/tail surface, stale double rewrite, or unsupported result lane that becomes represented. |
| `OptUtils::optimizeAfterInlining` | In DAEO only, run `precompute-propagate` then the default function optimization roster on `worthOptimizing`. | One shared nested roster, producer-owned worth/effects, pre/post-cleanup dependency seeding, and one epoch-bearing effect frontier in `pass_manager.mbt`. | Exact roster, exact-once ownership, diagnostics, and finite convergence for represented work. | Reopen if an exact source case requires work outside the committed frontier, duplicates cleanup ownership, or loses a cleanup-deleted dependency edge. |
...
| `$refine-return-return` | Refine from an explicit return operand. | `dae GC result refinement joins multiple explicit returns and fallthrough`, public explicit-return tests, and canonical multi-instruction value-slice coverage for external conversions. | Exact for represented branch-free scalar slices. | Returned-value evidence. | Rewrite-requiring control carriers remain fail closed until their complete slice rewrite is owned. |
| `$refine-return-many` | Join multiple explicit returns and fallthrough. | `dae GC result refinement joins multiple explicit returns and fallthrough`, nested branch-free multi-instruction return/fallthrough slices, the null-bottom explicit-return regression, `dae GC result refinement includes legacy try explicit returns`, and `dae GC result refinement joins all-return legacy try`, and typed legacy-try normal-fallthrough refinement with wider catch fallthrough rejected. | Exact. | Returned-value evidence. | Typed exceptional payload joins, branch/delegate reconstruction, and unrepresented continuation exits remain fail closed. |
...
| call-ref-tail family | Use typed `return_call_ref` evidence; tolerate unreachable targets without assertion. | Call-ref result-slice evidence across concrete and abstract heap signatures and canonical branch-free multi-instruction argument/target slices and declared `global.get` / typed `struct.get` / `array.get` / `table.get` / ordinary and descriptor-bearing struct/array constructor / `cont.new` / `cont.bind` / `string.const` / string array constructors / `string.concat` / `string.as_wtf16` / `stringview_wtf16.slice` / `ref.i31` / typed `ref.cast` / `ref.cast_desc_eq` / `ref.as_non_null` / `ref.get_desc` / `extern.convert_any` / `any.convert_extern` results, including effectful producers whose evaluation is preserved, public call-ref tail tests, and unreachable producer tests. | Exact represented evidence; unowned reference callers remain conservative. | Returned-value evidence. | Reopen only with exact typed-reference ownership proof. |
