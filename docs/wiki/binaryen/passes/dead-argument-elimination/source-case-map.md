...
| `refineArgumentTypes` / `TypeUpdating::updateParamTypes` | Strict reference LUB over every owned direct actual, skipping unread parameters; repair old-type writes/tees/locals. | Shared exact-caller value-slice collection, strict LUB, old-type local repair, and ordinary combined parameter/result signature plan. | Exact focused behavior; lifecycle partial. | Ordinary GC work is action-plan-owned; close when the optimizing batch and every remaining repair enter the common type finalizer. |
| `LUB::getResultsLUB` / `ReFinalize` | Join every returned value, explicit return, and tail return source; update call and parent types. | GC result worklist, canonical fallthrough value slices including external conversion nullability, returned-value scanners across represented legacy `try` protected/catch regions plus typed normal fallthrough when every catch is nonfallthrough, common result-action signature plan, and control/type follow-ups. | Exact on represented source cases; lifecycle partial. | Result-only refinement is plan-owned; close when one returned-value carrier owns all represented controls and combined parameter/result plus every type user is finalized atomically. |
| `removeReturnValue` / `ReturnUtils::removeReturns` | Remove a result only when all owned calls drop it and tail/unseen constraints permit it; preserve bottom with `call; unreachable`. | Observer graph, dropped-result work, common shrinking/refining parameter/result action plan attempted by every dispatcher family, including partial multivalue projections and optimizing unread/direct-GC/dropped-result batches, and specialized dead-suffix plans. | Exact for represented scalar/bottom cases; lifecycle partial. | Ordinary and specialized shrinking parameter/result repair is plan-owned, including partial parameter and multivalue result projection; close when represented bottom/control reconstruction shares the same lane-aware atomic result plan without stale double rewriting. |
