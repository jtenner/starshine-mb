...
| `ParamUtils::removeParameters` | Remove parameters and all owned direct operands together; preserve unremovable effects by localization; allocate replacement locals and repair nondefaultable locals. | Scalar/component/family candidates plus value-slice replay/localization and local projection. | Behavior exact or equivalent-smaller; lifecycle partial. | Close when all remaining producer-specific candidate builders emit their own effects and no compatibility local-growth inference remains. |
| `ParamUtils::localizeCallsTo` | Child-localize every owned target call, then repair nested EH pops. | Typed scratch-local callsite rewrite plus component/localization paths. | Exact for represented direct/`return_call` forms; fail closed on unsupported escaping control. | Close when every broad/component localization enters one epoch-bearing work item and exact caller epoch. |
| `refineArgumentTypes` / `TypeUpdating::updateParamTypes` | Strict reference LUB over every owned direct actual, skipping unread parameters; repair old-type writes/tees/locals. | Shared strict LUB and old-type local repair. | Exact focused behavior; lifecycle partial. | Close when initial multi-target collection and every producer use the canonical evidence API and common type finalizer. |
...
| --- | --- | --- | --- | --- | --- |
| TNH-1 trapping unused `struct.get` actual | With traps-never-happen enabled, remove the unused trapping actual and parameter. | No focused DAE test currently passes `HotPipelineOptions.traps_never_happen` into DAE. | **Coverage and behavior gap.** `dae_run_module_pass` currently receives no TNH option. | Parameter value-slice / effect policy. | Release-blocking. Add a red public/pass-manager fixture and thread TNH into plain DAE and DAEO operand-removal analysis. |
...
| `$various-params-yes` | Refine two live reference parameters independently while preserving an intervening scalar lane. | Direct-call common-LUB and multi-slot parameter tests. | Exact. | Shared strict-LUB API. | Initial multi-target collector remains to consolidate. |
...
| `$various-params-null` | Refine one parameter to non-null bottom evidence and retain nullable type for the other; unrelated locals stay untouched. | Nullable typed-select/LUB tests and local-remap safety tests. | Exact. | Parameter evidence and local repair. | Reopen on abstract heap hierarchy changes. |
...
| `$refine-return-many` | Join multiple explicit returns and fallthrough. | Same white-box and public mixed-return/fallthrough tests. | Exact. | Returned-value evidence. | Wider exceptional exits remain fail closed. |
...
| `$update-null` / `$call-update-null` | Rewrite null arm type while joining multiple explicit-return leaves at their nullable common parent. | Nullable explicit-return join and ref.null repair tests. | Exact. | Returned-value LUB and source rewrite. | Reopen on new null/branch carrier forms. |
...
- The one source-backed behavior gap found by this mapping is **TNH-aware unused-operand removal**: DAE is not passed `traps_never_happen` today.
...
| B1, B2, B3, A4, C2 | **2. release-blocking lifecycle risk** | Behavior is broadly represented, but forwarding/broad localization and some producer entry points still depend on phase-local builders or inferred effects. |
...
| H3, H4, I1, I2 | **2. release-blocking lifecycle risk** | Family validation exists, but compatibility effects, conservative fallback invalidation, incomplete diagnostics, and parallel phase loops remain. |
| J1, J2, J3, J4 | **2. release-blocking lifecycle risk** | Shared roster and semantic worth exist, but legacy inference and whole-pass retry remain. |
...
2. Finish producer-owned effects and remove compatibility local-growth worth inference.
...
6. Replace DAEO whole-pass retry with effect-frontier convergence.
7. Reconcile this map, the completion matrix, inventory, backlog, and release note before final long gates.
