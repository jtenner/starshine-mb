...
| DAE-H1 | Original/current boundary separation | DAE scanner | both | one immutable `DaeOriginalBoundarySnapshot` owns original type indices, visibility, params, call summaries, unreachable-prefix facts, and all result/dead-suffix evidence; current facts remain separately collected | generic-partial | complete snapshot projection plus white-box row projections | stale/current mixing cases | all DAE profiles | focused/public/pass suites green; 100-case plain dedicated and DAEO random-all counts unchanged | original collection is coherent, but current boundary records and incremental invalidation remain fragmented | coherent current boundary graph and consumer projections | epochs | original legality immutable; current facts refresh by invalidation | new legality fact |
