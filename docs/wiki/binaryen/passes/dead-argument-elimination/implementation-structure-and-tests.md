...
The completion work also establishes a stable reason-coded diagnostic vocabulary without changing optimizer output. `DaeDecisionReason` assigns durable text codes to boundary visibility, tail dependencies, parameter/result observations, operand preservation/localization/trap decisions, GC refinement outcomes, stale facts, validation rollback, profitability, and work-budget rejection. Boundary candidates and operand rewrite dispositions carry their primary reason. `DaeDiagnostics` is consumed by the common dispatcher: producer-owned commit effects increment produced/applied, parameter/result removal or refinement, materialization, caller result repair, localization, type-section edit, stale-requeue, and validation-rollback counters. Focused white-box tests lock the boundary-surface mapping, operand/GC classifications, reason aggregation, and a real uniform-materialization commit. Selected-only/guard, scratch-local reuse, effect replay, nested-roster counters, and migration of every specialized decision site remain open in [`completion-matrix.md`](./completion-matrix.md).
...
`DaeCommitEffects` is the semantic sidecar for scalar, component, and family plans. It separates `structurally_touched` from `worth_optimizing`, records per-definition `worth_decided`, and carries parameter/refinement, result/removal, materialization/localization, caller drop/bottom repair, control refinalization, local projection, type-section, metadata, invalidation-domain, and decision-reason effects. Successful plan applications and `DaeWorkCommitted` preserve the sidecar; exact-literal worklist applications merge it across batches, `dae_run_core(...)` merges committed effects, and the DAEO pass-manager adapter trusts producer-owned worth only for definitions whose decision is explicit. Unrelated legacy changes merged into the same wave still use compatibility inference. Exact local-count growth remains only as a labeled compatibility bridge for older scalar producers that still return a bare module candidate. Focused tests distinguish callsite-only caller edits from callee worth, mixed producer/legacy ownership, scratch-local localization from unchanged callers, ordinary and immutable-global callee materialization, forwarding-component membership, and unrelated functions.

The ordinary private scalar unread-parameter path is the first production consumer. It builds caller rewrites and the callee/local projection without mutation, preserves direct `return_call` composition, supports imports before definitions and repeated calls across multiple callers, reuses, appends, or safely replaces the replacement function type, omits unwritten removed parameter slots, retains written removed slots as remapped body locals, incorporates an already-proved speculative callee-source cleanup into the same transaction, reports exact touched callers/callee, and then crosses the common application boundary. Common constant materialization, scalar scratch localization, ordinary-core GC parameter refinement, and ordinary-core dropped-result removal now cross the dispatcher/family transaction. Remaining GC/result producers, forwarding/broad localization, control reconstruction, and selected legacy lanes are explicitly open in [`completion-matrix.md`](./completion-matrix.md).

The ordinary core no longer performs full-module forward rescans. `DaeBoundaryDependencyGraph` projects deterministic caller and callee edges from the current graph, while `DaeBoundaryWorklist` seeds every definition once, suppresses duplicate pending entries, and requeues the touched transaction plus its before/after caller/callee neighborhood. The graph owns direct `return_call` dependencies and deterministic Tarjan SCCs normalized by the lowest definition in each component. Parameter-forwarding dependencies are extracted from exact current direct/`return_call` operand slices whose source is one of the caller's parameters. Each edge records caller and callee parameter indices, compatible source type, direct-versus-tail composition, and caller/callee signature/body epochs, so reordered multi-parameter forwarding is represented by slots rather than only by function adjacency. Their symmetric closure provides deterministic forwarding-family components that include one-way incoming forwarders without merging unrelated direct-call boundaries. `DaeParameterFlowState` now merges forwarded-only, uniform materializable value, conflict, and blocked states monotonically over the current forwarding/SCC evidence. Solved per-slot values are attached to `DaeUniformActualWork`, and the candidate runner recomputes them after every commit. Uniform-value invalidation remains exactly the current direct callers rather than a caller/callee union. Stable current call identities now separately populate active non-tail undropped result observers, active direct-tail dependencies, their combined potential result-forwarding frontier, and inverse per-caller result dependencies; dropped calls are excluded from observer evidence. Committed work-item effects now select before/after dependency rows by invalidation domain: result changes consume the exact undropped/direct-tail frontier, metadata-only changes remain local, and missing producer domains use the conservative caller/callee fallback. Common work families publish narrow semantic domain sets: parameter/materialization/localization families omit result/control propagation, result families omit local propagation, and control-only reconstruction omits calls/signature/result/local/type propagation. Legacy producers remain conservative until their family migrations. `DaeWorkItem` names one of ten candidate families plus its target, component identity, exact component members, solved values, and module/call/signature/body epochs. Family queues are component ordered, duplicate resistant, and reject/requeue stale items. Queue frontiers retain Binaryen's source-backed one-wave single-caller profitability stop: work discovered by a frontier is processed by the next frontier, where further ordinary parameter removal may be suppressed while result and cleanup convergence continues. `DaeCoreRunResult` reports whether the ordinary frontier consumed that throttle. Because older selected lanes can still own the only boundary edit after the frontier, DAEO also recognizes the exact net shape only when one definition's parameter list shrinks, every other parameter/result signature is unchanged, and that callee had exactly one direct caller; mixed boundary waves are rejected. Cleanup reentry initializes both unread batching and unprofitable-removal state from the carried value. Termination comes from queue exhaustion rather than a numeric iteration budget.

`dae_process_work_item(...)` is now the common family dispatch boundary. It receives the immutable original snapshot, current module/graph/dependencies, one epoch-bearing item, accumulated scheduler state, and diagnostics; stale targets either refresh as one current work item or terminate if the definition no longer exists. Unread-parameter and localization-retry work constructs its candidate through the exact-definition transactional path. Forwarding work resolves the item's exact member set, builds and validates one component plan against the supplied graph and epochs, and reports one atomic common commit. Ordinary work items use the graph's forwarding-family closure; the established production collector can attach its exact seed members to the same item, preserving its discovery order while the transaction expands the proven forwarded parameter nodes. The production forwarding wrapper enters `dae_process_work_item(...)` and reconstructs the changed-definition report from those nodes, so cleanup and traces retain their established scope. Uniform literal work now also executes in the dispatcher against current exact call facts. `DaeWorkItem` carries immutable-global and immediate-suffix specialization policy explicitly, stale refresh preserves both, and the exact-literal candidate runner uses the dispatcher for ordinary literals, verified immediate suffixes, and immutable globals. `DaeExactLiteralWorklistApplication` reports merged producer effects as well as touched/productive definitions. Its runner processes the complete supplied exact frontier and no longer accepts an external productive limit or an inert call-facts parameter. The core immutable-global revisit submits exact definition frontiers to that runner with the immutable original boundary snapshot; the former selected bitmap scan/mutate helper and the pass-manager's duplicate folded-callee recursion are deleted. Solved forwarded values remain disabled for the immutable-global policy-bearing lane so it cannot preempt the later forwarding-component transaction; the exact current-call/global proof remains authoritative. GC parameter, GC result, and dropped-result work now execute as distinct dispatcher families. GC uses a shared owned-boundary wrapper around `dae_try_refine_direct_gc_types(...)`, whose parameter and result modes can be selected independently. Ordinary-core GC parameter discovery now constructs `DaeGcParameterRefinementWork`, commits through the dispatcher/family plan, and reports producer-owned refinement/worth effects back to core aggregation. The GC result closure routes each candidate through `DaeGcResultRefinementWork`, derives callee-first postorder from inverse exact result dependencies—including nested control carriers—refreshes graph/dependency epochs after commits, and requeues exact result forwarders. Direct GC-parameter actual evidence accepts allocation, exact `ref.func`, direct/tail call, typed indirect/tail-indirect, and typed call-ref/tail-call-ref producers; parsed concrete and decoded canonical abstract reference parameters enter the same strict-refinement proof, while exact function-reference values retain their original `ref.func` operands and only narrow the callee signature. Single-result evidence recursively joins multiple nested explicit returns with the fallthrough value, uses the shared abstract/concrete reference subtype lattice across both parsed `HeapTypeRefType` and decoded canonical `AbsHeapTypeRefType` encodings, derives exact function-reference results from `ref.func`, follows direct and typed `return_call`, `return_call_indirect`, and `return_call_ref` result signatures, and admits self-called functions when recursive results do not themselves flow out. It no longer scans only the final top-level instruction or accepts a productive cap. All ordinary-core dropped-result admissions—including the three zero-parameter predicates and argument-side repair—retain their distinct source-order predicates but now construct `DaeDroppedResultWork`, commit through the same dispatcher/family plan, and report result-removal worth for the target plus exact caller-drop effects. The dependency graph's stable-callsite observer partition distinguishes dropped calls, active non-tail undropped consumers, and direct-tail dependencies. Ordinary dropped-result work now also requires the authoritative current boundary guard: the target remains owned, all exact direct observers drop, it is not a direct-tail callee, and the target itself has no outgoing direct/indirect/reference tail. The lower rewrite evidence inspects only the exact target body when its definition is known, so an unrelated tail no longer suppresses a legal result removal; target-less specialized callers retain the aggregate fail-closed fallback. `DaeDroppedResultWork` consumes the current exact direct/dropped-call candidate and the existing complete result/callsite repair. The fact-selected dropped-result phase runs at every module size and processes dispatcher-backed waves to exhaustion instead of the former `32` / `14` / `8` productive caps or the former `8192` disablement. The obsolete selected-definition wrapper, descending attempt/order collectors, empty fallback-neighborhood carrier, and rejected bucketed-attempt scaffold are deleted; fact-guard, application-timing, and exact caller-touched tests now exercise the live definition helper directly. Its former ascending-through-`4096` / descending-above-`4096` split is gone: exact current callers order candidates caller-first so wrappers precede candidate callees, with deterministic lowest-definition fallback only for a remaining cycle. The high-payoff wrapper/callee chain and its post-result cleanup now do the same: the `8192` gates, two-chain transaction cap, and two-candidate cleanup cap are removed. A 513-definition regression proves the same exact payoff family without artifact-scale padding. The adjacent high-payoff local-cleanup lane also no longer uses the `4097..8199` pair band or preceding-eight root window. A semantic one-parameter call-sink seed runs through exact uniform work first; the resulting applied-boundary definitions become the explicit cross-phase roots for adjacent pair cleanup. This preserves the computed-cycle ordering proof, makes the adjacent fixture pass at definitions `500/504/505`, and avoids an overbroad all-pair experiment that produced only a two-byte artifact delta while raising runtime to about 150 seconds. `DaePostCleanupBoundaryRevisitWork` also uses the exact unread-parameter transaction. The optimizing pass-manager runs one touched-definition revisit worklist before post-cleanup core convergence, then merges the revisit and core touched sets. Every validated changed boundary and producer-owned body-only worth definition enters the filtered nested roster; module/touched/local/instruction cardinality, encoded body size, removed-local count, call-free shape, and folded-callee evidence no longer admit or reject work. Changed-boundary local compaction and immutable-field folding retain each exact definition transactionally, then the one shared `precompute-propagate`-first roster runs exactly once. The former private broad-cleanup roster and folded-callee recursion are deleted. When cleanup changes the module, final optimizing retries carry the consumed single-caller throttle and iterate until unchanged or non-retainable; each accepted candidate must validate and strictly reduce encoded size. Control-reconstruction and type-repair retry items also reuse the exact result/control/type reconstruction transaction, so all ten family labels now terminate through concrete no-change or commit outcomes rather than deferred placeholders. The computed/unread runner consumes only `DaeWorkOutcome`, refreshes graph/SCC/dependency facts after commits, and requeues definitions through producer-owned calls/signature/result/body/control/locals/types/metadata/nested-cleanup domains across both old and new graphs. Legacy applications without domains retain the conservative caller/callee fallback.
...
Broad exact-literal discovery uses two definition-independent original-shape seeds: a private single-parameter identity and a scalar allocation/store constructor with at least two parameters, one allocation-result local, exact allocator/tee/return shape, one unwritten read per parameter, and enough stores to consume the fields. The original shape is intentionally immutable while values are solved from the current module, because earlier core work can erase an identity result/body before its now-unread parameter is removed. Plain DAE runs this broad solved-value frontier at the final large-module core boundary. DAEO runs it after forwarding-component and SCC transactions, preserving the existing component proofs instead of preempting them. This phase-local ordering was required: unrestricted pre-component admission regressed the computed-cycle carrier, while unrestricted all-uniform post-component admission changed five established cleanup/payoff families. The accepted original-shape seeds close the recursive high-definition fixture for both variants without either failure family. Their post-core uniform-flow runner is no longer disabled at `defined <= 1024`; the same semantic candidate source and exact worklist now run at every module size, while focused/full tests and the retained artifact remain unchanged. The former fixed reverse definitions `4593, 4592, 4591, 4589, 4588, 4587, 4586, 4584` no longer appear in production.

White-box tests cover successful caller/callee/type commit, unrelated-function preservation, exact touched reporting, stale target signature/body epochs, stale caller body epochs, stale call epochs, changed call counts, changed direct-versus-tail composition, changed type resolution, validation rollback, direct `return_call` planning, dependency projection, deterministic two- and three-member SCCs, independent components, direct-tail dependencies, reordered typed per-slot forwarding, monotone multi-slot uniform convergence, solved-value candidate and dispatcher commits, component/family order, epoch rejection, deterministic duplicate-free requeue, invalidation closure, unread, uniform-literal, and atomic forwarding dispatch commits, deferred no-op termination, stale refresh, permanently dead stale targets, full forwarding-component commit, stale-member rollback, and whole-component validation rollback. The GC-result dependency postorder is finite through duplicate suppression rather than the former depth-`16` cutoff or a productive cap. A twenty-function result chain and a nested block-carried forwarder both refine from the allocation leaf through every exact result-dependent caller, validating the complete output.

The former selected dropped-result fallback definitions `3566`, `3732`, and `3814` are deleted. The generic fact worklist produces byte-identical output on the direct artifact without them. Small reverse exact-literal work, low forwarded-constant revisits, broad nullable exact-literal batches, large immutable/global revisits, direct GC result-caller closure, low-callee revisits, and shifted-wrapper revisits no longer stop at the former `8`, `64`, `64`, `512`, `16`, `14`, or `21` productive caps. The broad nullable batch also scans the full current definition range rather than truncating discovery at definition `4096`, and it now runs after ordinary reverse processing at every module size rather than only when `original_defined > 4096`. This placement preserves the earlier proof frontier while removing the module-size correctness gate. Admitted work now terminates through finite signature reduction and no-progress. The fixed wrapper definitions `128, 131, 133, ... 196` are also gone: exact current wrapper bodies and resolved types derive a deterministic duplicate-free callee order. The older caller-prefix `64` and shifted-wrapper range `64..128` production lanes are now deleted entirely; ordinary dependency work plus that full exact wrapper-shape order produce byte-identical artifact output without either band. The private broad-module exact-parameter chain for definitions `37`, `38`, and `41`, including its touched-definition `164` trigger and bespoke prefix cleanup, is deleted as obsolete: the ordinary direct-GC batch already discovers the same boundaries, the focused broad-module behavior remains green, and the direct artifact is byte-identical without the lane. Selected unread definitions `505/538/3799` and the mid/high exact-literal lists are also deleted. Broad exact-literal semantic admission now includes a one-parameter, no-local, nontrapping pure binary expression over the parameter and a materializable constant; this generically closes the former arbitrary definition ranges without admitting trapping arithmetic. Phase-local cleanup invalidation is now explicit: direct callees of bodies changed by a cleanup phase seed a narrow exact-uniform frontier. That replaces the selected exact-literal/default-argument definitions `326/327/328/1795` without the eight regressions caused by an unrestricted whole-module post-shape scan. The artifact-only `3737/3765` suffix bridge, including caller `281` and hard-coded `i32.const 1024`, is deleted entirely; generic forwarding/uniform and unread processing already owns the output byte-identically. The `def_idx == 505` rewritten-control cleanup switch is also gone: the same branch-safe constant/control simplification now applies uniformly after every validated parameter rewrite. The selected zero-tee list `215/260/261/1137/1978` and its private split helper are deleted. Binaryen v131 preserves the original tee shape on equivalent arbitrary-definition direct-DAEO fixtures, and the selected lane was unproductive on the current artifact. All definition-`237` production scheduling is also removed after proving the complete family unproductive on the current artifact and unnecessary for every focused/full suite. The detailed `Func237` helpers remain test-only characterization until their generic control/local pieces are absorbed by owning passes. Selected production schedules for definitions `246`, `287`, and `867` are likewise removed after disablement proved byte-identical artifact output and no focused/full regression; their helper-level characterization remains available without selecting production by identity. Definitions `233` and `256` are removed for a stronger reason: explicit Binaryen-v131 direct-DAEO probes preserve the constant-result `if` and exported loop-carrier shapes, so the Starshine identity rewrites were non-oracle cleanup. Definition `236` is now generic rather than deleted behavior: any changed, straight-line, call-free function with at least one removed local enters isolated nested cleanup. The focused fixture reaches Binaryen's exact `i32.const 5`, while call-bearing and structured candidates retain the phase-local profitability frontier. Definitions `267/268` are also generic: after caller/callee parameter rewriting, constant integer equality followed by `if` folds monotonically until no such control remains, replacing the selected eight-step loop with finite instruction-count reduction. Func408's single-use constant-local rewrite is now seeded by immutable original shape—one parameter, one local, one direct call, and an exact constant set/get slice—rather than a definition number. On the retained artifact this also finds the same shape in additional functions, removes ten redundant locals, reduces canonical output by 60 bytes to `2,991,168`, validates, and is byte-idempotent. Func408's selected nop cleanup is removed separately: explicit v131 direct-DAEO output preserves both top-level and nested nops when no boundary changes, and removing the identity lane leaves the improved artifact byte-identical. Definitions `288/298/299` follow the same boundary rule: their focused targets are exported, v131 preserves the switch-carrier, nested return-wrapper, loop-carrier, and inverted-result-if shapes without a DAE boundary change, and deleting all schedules leaves the improved artifact byte-identical. The large fixed Func288 local-slot map remains only as white-box characterization, not production policy. Func408 pair-if reconstruction is also identity-free: its exact carrier/body recognizer now runs only over the current touched-definition frontier, and the public fixture succeeds at arbitrary definition `709` while the artifact stays byte-identical. The former Func311/Func313 terminal-call families are seeded from immutable original signature/body shape intersected with an exact current-body change, then all admitted functions converge in one batched local fixpoint. Their internal zero-equality branch swaps and constant-local inlining also run to pattern exhaustion rather than the former `4`, `4`, and `9` local caps. Their fixtures now pass at arbitrary definitions `711` and `713`; an unrestricted all-definition experiment found smaller output but raised productive artifact time from about 54 seconds to 139 seconds, so it was rejected as phase-overbroad cleanup rather than retained as an unmeasured parity claim. The former Func327/Func1794 post-raw lanes are unified similarly: terminal callers are admitted when their callee boundary changed or their current terminal body contains an exact foldable constant equality/if, then const-if folding, terminal local inlining, typed default-wrapper lowering, and direct-callee uniform invalidation converge generically. Func327 fixtures pass at arbitrary definition `727`; the former `2` and `8` wrapper caps are gone. The last numeric production admission, Func323, is also removed: exact single-direct-caller facts identify the component, the caller's body must differ from the immutable original, and the existing immutable-global suffix recognizer proves the removable operands. Its fixture passes at arbitrary caller/callee definitions `813/823`. Unrestricted admission exposed a 723-byte artifact reduction but increased productive runtime to about 152 seconds by admitting a large unchanged caller; that expansion was rejected, and exact changed-caller admission restores byte-identical output in about 58 seconds. Remaining cardinality/prefix admission policies are tracked separately.

## Default DAE/DAEO test budget

The default `dae_optimizing_test.mbt` lane is behavior-focused and bounded. Small public fixtures retain registry, ownership, signature, operand, result, GC, control, cleanup, convergence, and transactional behavior. Synthetic definition thresholds, aggregate boundary sweeps, and artifact-scale modules with roughly `1024..8193` definitions remain in the same source file for explicit replay, but carry descriptive `#skip` annotations and do not run under ordinary `moon test`.

Those manual fixtures still protect useful scheduler and artifact provenance when invoked deliberately with `--include-skipped` plus a focused filter or index. Their semantic pieces remain represented in bounded public tests and direct white-box scheduler tests, so routine pass development does not need to parse and optimize tens of thousands of padding functions. Keep new module-cardinality, threshold, aggregate-seed, and artifact-scale regressions in this explicit lane instead of widening the default runtime again.

The 2026-07-22 audit moved `40` additional aggregate, duplicate-composite, synthetic-threshold, and artifact-derived public replays out of the default path. The file now declares `382` tests: `320` run by default and `62` are explicit manual tests. Final warm focused execution completes in `13s` on JS and `5s` on wasm-gc instead of exceeding `16m`; the first post-edit rebuild runs also stayed bounded at `25..26s`. Both default runs currently report `319/320` because the pre-existing `dae-optimizing drops root-return dead suffix debris after touched rewrite` expectation remains red after the scalar transaction refactor; this behavior failure stays visible under `[O4Z-DAE]001` and was not hidden behind a skip. Adjacent DAE white-box, pass-manager, GenValid, registry/preset, optimize, and fuzz-manifest selections each complete in `0..1s` after the build is warm.
