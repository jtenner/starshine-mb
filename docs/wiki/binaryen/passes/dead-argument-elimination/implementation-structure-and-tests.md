...
`DaeCommitEffects` is the semantic sidecar for scalar, component, and family plans. It separates `structurally_touched` from `worth_optimizing` and records parameter/refinement, result/removal, materialization/localization, caller drop/bottom repair, control refinalization, local projection, type-section, metadata, invalidation-domain, and decision-reason effects. Successful plan applications and `DaeWorkCommitted` preserve the sidecar; `dae_run_core(...)` merges committed effects, and the DAEO pass-manager adapter trusts producer-owned worth on covered definitions instead of reconstructing it from generic touched state. Exact local-count growth remains only as a labeled compatibility bridge for older scalar producers that still return a bare module candidate. Focused tests distinguish callsite-only caller edits from callee worth, scratch-local localization from unchanged callers, callee materialization, forwarding-component membership, and unrelated functions.
