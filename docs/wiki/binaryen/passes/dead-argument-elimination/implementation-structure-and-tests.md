...
Broad exact-literal discovery now includes two generic low-risk families independent of definition identity: a private direct callee with exactly one parameter and one equal result, no locals, and body `local.get 0`; and a fully-uniform scalar allocation/store constructor with at least two parameters, one allocation-result local, exact allocator/tee/return shape, one unwritten read per parameter, and enough scalar stores to consume the fields. Current direct callers must agree on every materialized value. Definition-`4200` regressions cover both the identity and a three-parameter constructor. The first experiment admitting every fully-uniform multi-parameter body reproduced three public regressions, including the known invalid nondefaultable-local family; restricting admission to the source-backed scalar constructor shape restored the complete public suite. General forwarding-through-wrapper uniformity remains on the de-artifacting ledger until the new per-slot dependencies have a monotone SCC solver.

White-box tests cover successful caller/callee/type commit, unrelated-function preservation, exact touched reporting, stale target signature/body epochs, stale caller body epochs, stale call epochs, changed call counts, changed direct-versus-tail composition, changed type resolution, validation rollback, direct `return_call` planning, dependency projection, deterministic two- and three-member SCCs, independent components, direct-tail dependencies, reordered typed per-slot forwarding, component/family order, epoch rejection, deterministic duplicate-free requeue, invalidation closure, unread, uniform-literal, and atomic forwarding dispatch commits, deferred no-op termination, stale refresh, permanently dead stale targets, full forwarding-component commit, stale-member rollback, and whole-component validation rollback. Current totals are `240/240` DAE white-box, `345/345` public DAE/DAEO, `6201/6201` pass-package, and `9679/9679` full deterministic tests.
