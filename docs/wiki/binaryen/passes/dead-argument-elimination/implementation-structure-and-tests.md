...
The optimizing computed/unread retry now has an exact-definition entry point and runs through the epoch-bearing family work-item queue. For the established small-module phase, this removes the selected-all bitmap scan and the `defined * 2 + 1` productive-attempt cap, reuses the current type/call snapshot for each item, rejects and refreshes stale queued work, refreshes SCC/dependency facts after every localized commit, and requeues only invalidated caller/callee neighbors. Broad-module availability remains a lifecycle-ordering blocker: an experiment that ran this queue before broad component/nested phases preempted existing low exact-parameter and computed-cycle proofs, so the release path still requires one shared cross-phase queue rather than a cardinality-only widening.
