...
The ordinary core no longer performs full-module forward rescans. `DaeBoundaryDependencyGraph` projects deterministic caller and callee edges from the current graph, while `DaeBoundaryWorklist` seeds every definition once, suppresses duplicate pending entries, and requeues the touched transaction plus its before/after caller/callee neighborhood. Each item is evaluated against current graph epochs and every commit refreshes the graph. Queue frontiers retain Binaryen's source-backed one-wave single-caller profitability stop: work discovered by a frontier is processed by the next frontier, where further ordinary parameter removal may be suppressed while result and cleanup convergence continues. Termination comes from queue exhaustion rather than a numeric iteration budget.

The optimizing computed/unread retry now has an exact-definition entry point and a dependency-worklist runner. For the established small-module phase, this removes the selected-all bitmap scan and the `defined * 2 + 1` productive-attempt cap, reuses the current type/call snapshot for each item, refreshes after every localized commit, and requeues only invalidated caller/callee neighbors. Broad-module availability remains a lifecycle-ordering blocker: an experiment that ran this queue before broad component/nested phases preempted existing low exact-parameter and computed-cycle proofs, so the release path still requires one shared cross-phase queue rather than a cardinality-only widening.
