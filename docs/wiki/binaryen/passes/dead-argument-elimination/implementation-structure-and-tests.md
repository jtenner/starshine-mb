...
`dae_process_work_item(...)` is now the common family dispatch boundary. It receives the immutable original snapshot, current module/graph/dependencies, one epoch-bearing item, accumulated scheduler state, and diagnostics; stale targets either refresh as one current work item or terminate if the definition no longer exists. Current unread-parameter and localization-retry work constructs its candidate through the existing exact-definition transactional path, while unmigrated family labels terminate as explicit deferred outcomes rather than spinning. The computed/unread runner now consumes only `DaeWorkOutcome`, refreshes graph/SCC/dependency facts after commits, and requeues invalidated definitions through the common lifecycle.
