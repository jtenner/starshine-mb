...
`dae_try_rewrite_verified_uniform_const_candidate(...)` is the shared exact-constant candidate boundary for Func867 and the forwarded Func3765 suffix specialization. Uniform-constant collection accepts a removed-parameter mask so unrelated complex actuals do not become evidence requirements; the helper verifies every removed actual against the exact expected materialized constant before delegating to `dae_try_rewrite_candidate(...)`. Func3765 is scheduled after the optimizing-only Func3737 folded-multivalue repair so the forwarded `1024` is current evidence. The earlier Func3737 scanner remains separate because generic value-slice recovery still cannot own its pre-wrapper caller shape. Whitebox `dae verified uniform-constant candidate uses shared boundary rewrite transactionally` covers multi-instruction kept operands, forwarded locals, exact mismatch rollback, caller/callee application, signature projection, and touched metadata. Slice-183 validation reached whitebox `197/197`, public DAE `303/303`, pass suites `5205/5205`, and full tests `8657/8657`; fresh 10000-case counts remain `5000`, `9136`, and `9633` normalized for dedicated direct, direct random-all, and normalized optimizing random-all respectively.
