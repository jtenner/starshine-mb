...
The ordinary core no longer performs full forward waves or applies the old single-caller "unprofitable removal" stop. `DaeBoundaryDependencyGraph` projects deterministic caller and callee edges from the current graph, while `DaeBoundaryWorklist` seeds every definition once, suppresses duplicate pending entries, and requeues the touched transaction plus its before/after caller/callee neighborhood. Each item is evaluated against current graph epochs, every commit refreshes the graph, and convergence is queue exhaustion rather than a numeric iteration budget. This also allows a newly unread caller parameter to be removed after a callee signature rewrite.
