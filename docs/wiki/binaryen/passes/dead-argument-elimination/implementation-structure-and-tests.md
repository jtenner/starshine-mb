...
The ordinary core no longer performs full-module forward rescans. `DaeBoundaryDependencyGraph` projects deterministic caller and callee edges from the current graph, while `DaeBoundaryWorklist` seeds every definition once, suppresses duplicate pending entries, and requeues the touched transaction plus its before/after caller/callee neighborhood. The graph owns direct `return_call` dependencies and deterministic Tarjan SCCs normalized by the lowest definition in each component. Parameter-forwarding dependencies are now extracted from exact current direct/`return_call` operand slices whose source is one of the caller's parameters; uniform-value dependencies are exactly the current direct callers rather than a caller/callee union. Result/type/body/cleanup domains remain conservative until their family migrations. `DaeWorkItem` names one of ten candidate families plus its target, component, and module/call/signature/body epochs. Family queues are component ordered, duplicate resistant, and reject/requeue stale items. Queue frontiers retain Binaryen's source-backed one-wave single-caller profitability stop: work discovered by a frontier is processed by the next frontier, where further ordinary parameter removal may be suppressed while result and cleanup convergence continues. Termination comes from queue exhaustion rather than a numeric iteration budget.

`dae_process_work_item(...)` is now the common family dispatch boundary. It receives the immutable original snapshot, current module/graph/dependencies, one epoch-bearing item, accumulated scheduler state, and diagnostics; stale targets either refresh as one current work item or terminate if the definition no longer exists. Unread-parameter and localization-retry work constructs its candidate through the exact-definition transactional path. Forwarding work now resolves the item's current direct-call SCC, builds and validates one component plan against the supplied graph and epochs, and reports one atomic common commit. Other unmigrated family labels terminate as explicit deferred outcomes rather than spinning. The computed/unread runner consumes only `DaeWorkOutcome`, refreshes graph/SCC/dependency facts after commits, and requeues invalidated definitions through the common lifecycle. The broader production forwarding seed may also contain one-way incoming forwarders outside the owning SCC; that family-specific closure remains separate until work items can carry exact forwarding-component membership without shrinking the established transaction.
...
Broad exact-literal discovery now includes a generic low-risk family independent of definition identity: a private direct callee with exactly one parameter and one equal result, no locals, and body `local.get 0`. When all current direct callers provide one uniform literal, the existing count-checked constant transaction removes the boundary at any definition index. A `4200`-definition regression proves this path is not selected by the old high-definition list. More complex multi-parameter forwarding chains remain on the de-artifacting ledger.

White-box tests cover successful caller/callee/type commit, unrelated-function preservation, exact touched reporting, stale target signature/body epochs, stale caller body epochs, stale call epochs, changed call counts, changed direct-versus-tail composition, changed type resolution, validation rollback, direct `return_call` planning, dependency projection, deterministic two- and three-member SCCs, independent components, direct-tail dependencies, component/family order, epoch rejection, deterministic duplicate-free requeue, invalidation closure, unread and atomic SCC-forwarding dispatch commits, deferred no-op termination, stale refresh, permanently dead stale targets, full forwarding-component commit, stale-member rollback, and whole-component validation rollback.
