...
| --- | --- | --- |
| summaries | import/defined counts, type lookup, simple size scan, refs/roots, shape flags, current caller size for combined-size action checks | no full Binaryen cost/trivial/flexible/depth model |
| eligibility | tiny, one-use private, narrow shrinking-trivial two-parameter binary-wrapper, ordered direct-call-wrapper, narrow shrinking-trivial three-parameter `select`-wrapper, narrow shrinking-trivial parameter-passthrough memory/table/SIMD/GC operation-wrapper defined callee (`i32.store`, `i64.store`, `f32.store`, `f64.store`, `i32.store8`, `i32.store16`, `i64.store8`, `i64.store16`, `i64.store32`, `v128.store`, `v128.store8_lane`, `v128.store16_lane`, `v128.store32_lane`, `v128.store64_lane`, supported SIMD plus GC heap-operation wrappers, `table.set`, `table.grow`, `memory.fill`, `memory.copy`, `memory.init`, `table.fill`, `table.copy`, or `table.init`), or first speed-focused flexible subset (`optimize_level >= 3`, `shrink_level == 0`, no direct calls, no loops, `size <= 20`); block type must be void/single-result; skip `try_table` and return-call-containing callees; honor internal full-inline suppression from `no-inline` / `no-full-inline` | remaining `Shrinks` classes, partial splitter, deeper flexible policy, multi-result support, partial-inlining-specific no-inline behavior |
| rewrite | direct `call` / `return_call` replacement, param/body-local append, local remap, simple return-to-branch, combined-size action guard before copying | incomplete nested `return_call*`, label collision, nondefaultable-local, metadata/name repair |
...
| optimizing approximation | trace marker, public touched-only `precompute-propagate`, touched-function filtered cleanup lane, touched unused-local compaction, unreachable-root collapse | propagating prefix is now public/shared; the remaining gap is exact option-specific Binaryen default-pipeline expansion |
...
- repeated parameter-passthrough binary, direct-call, `select`, and memory/table/SIMD/GC operation wrappers inline as narrow shrinking-trivial heuristic subsets;
- a repeated-param `MayNotShrink` wrapper stays under default plain `inlining` and inlines when run through `HotPipelineOptions::new(optimize_level=3)`;
...
| Active backlog | `agent-todo.md` accepts `[INL]001`, `[INL]002`, `[INL]003`, `[INL]004`, and `[INL]007` for v0.1.0; deferred breadth `[INL]005` and residual `[INL]006` are v0.2.0-only |
