...
| optimizing approximation | trace marker, private touched-only `precompute-propagate-prefix`, touched-function filtered cleanup lane, touched unused-local compaction, unreachable-root collapse | not exact public `precompute-propagate` + option-specific Binaryen default pipeline |
