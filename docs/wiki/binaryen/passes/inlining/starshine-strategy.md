...
- narrow shrinking-trivial two-parameter binary wrappers, three-parameter `select` wrappers, and two-parameter scalar store wrappers (`i32.store`, `i64.store`, `f32.store`, `f64.store`, `i32.store8`, `i32.store16`, `i64.store8`, `i64.store16`, and `i64.store32`);
...
- Full Binaryen heuristic classes beyond the narrow parameter-passthrough binary, `select`, and scalar-store `Shrinks` subsets: remaining `Shrinks`, `MayNotShrink`, flexible size, loop policy, optimize/shrink-level sensitivity.
