...
- Full Binaryen heuristic classes beyond the narrow parameter-passthrough binary, `select`, and memory/table/SIMD/GC operation `Shrinks` subsets: remaining `Shrinks`, `MayNotShrink`, flexible size, loop policy, optimize/shrink-level sensitivity.
