...
A first `maxBits`-style unsigned subset folds when an `and` mask bounds the left operand below the compared constant. Starshine now preserves effectful i32 and i64 masked operands by dropping the evaluated masked value before the folded constant. First recursive-width facts also handle direct i32/i64 unsigned-right-shift results with constant shift amounts `1..31` / `1..63`, including direct child `and`/`shr_u` facts such as `(x & 1023) >>> 4`; direct unsigned load widths (`i32.load8_u`, `i32.load16_u`, `i64.load8_u`, `i64.load16_u`, `i64.load32_u`) also feed this compare fold while preserving the trapping load as a drop. Starshine also covers the first direct i32 sign-extension equality range facts for `i32.extend8_s` / `i32.extend16_s` compared with constants outside the signed lane range. The corresponding i64 `extend8_s` / `extend16_s` / `extend32_s` equality spellings are a source-backed keep-spelling boundary because Binaryen `version_130` kept the probed out-of-range comparisons; direct i32 signed relational comparisons over out-of-range sign-extension results are likewise a source-backed keep-spelling boundary.
