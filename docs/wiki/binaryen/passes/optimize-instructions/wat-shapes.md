...
A first `maxBits`-style unsigned subset folds when an `and` mask bounds the left operand below the compared constant. Starshine now preserves effectful i32 and i64 masked operands by dropping the evaluated masked value before the folded constant. First recursive-width facts also handle direct i32/i64 unsigned-right-shift results with constant shift amounts `1..31` / `1..63`, including direct child `and`/`shr_u` facts such as `(x & 1023) >>> 4`; direct unsigned load widths (`i32.load8_u`, `i32.load16_u`, `i64.load8_u`, `i64.load16_u`, `i64.load32_u`) also feed this compare fold while preserving the trapping load as a drop. A narrow straight-line local scan now carries those unsigned max facts through direct `local.set` / `local.tee` roots to later `local.get` compares, so `local.set (x & 255); local.get < 256` can fold and local-carried positive `shr_u` facts can rewrite signed relational spellings to unsigned spellings. The same nonnegative bounded-value proof now folds the first signed relational out-of-range cases such as `gt_s` above the maximum or `le_s` above the maximum, and rewrites in-range signed relational spellings to unsigned spellings when the rhs constant is nonnegative. Starshine also covers the first direct i32 sign-extension equality range facts for `i32.extend8_s` / `i32.extend16_s` compared with constants outside the signed lane range. The corresponding i64 `extend8_s` / `extend16_s` / `extend32_s` equality spellings are a source-backed keep-spelling boundary because the historical Binaryen `version_130` probe kept the out-of-range comparisons; direct i32 signed relational comparisons over out-of-range sign-extension results are likewise a source-backed keep-spelling boundary.
...
(i32.gt_s (i32.and (local.get $x) (i32.const 255)) (i32.const 300)) ;; -> i32.const 0
(i32.le_s (i32.load8_u (local.get $p)) (i32.const 300)) ;; -> drop(load); i32.const 1
(i32.lt_s (i32.and (local.get $x) (i32.const 255)) (i32.const 128)) ;; -> i32.lt_u
(i64.ge_s (i64.shr_u (local.get $y) (i64.const 8)) (i64.const 42)) ;; -> i64.ge_u
(local.set $n (i32.and (local.get $x) (i32.const 255)))
(i32.lt_u (local.get $n) (i32.const 256)) ;; -> i32.const 1
```

For i32 and i64, effectful direct masked operands are preserved with an explicit drop before the folded `i32.const`; for example, `((call $effect) & 255) == 256` becomes `drop(i32.and(call $effect, 255)); i32.const 0`, and the i64 spelling uses `i64.and` under the same drop. Effectful direct i32/i64 `shr_u` bounded operands likewise become `drop(i32.shr_u(...)); i32.const result` or `drop(i64.shr_u(...)); i32.const result`, including a direct nested mask child under the dropped shift. Unsigned load operands follow the same trap-preserving shape: `i32.load8_u(ptr) == 256` becomes `drop(i32.load8_u(ptr)); i32.const 0`, and `i32.load8_u(ptr) <=_s 300` becomes `drop(i32.load8_u(ptr)); i32.const 1`. The remaining boundaries are dynamic/zero shift amounts, signed range proofs beyond this nonnegative bounded subset, local-scanner facts, select/phi/extension width facts, and broader recursive `maxBits` expressions unless covered by later slices; i64 sign-extension equality and direct i32 sign-extension signed-relational compares are explicitly source-backed as keep-spelling for the probed out-of-range forms.
