...
- a narrowed stack-carried call raw boundary: the public/raw pipeline now admits the simple no-param direct-call plus pure `local.get`/constant commutative integer binop form so `local.get + call` can reach HOT and canonicalize to call-first spelling, and admits flat tiny `memory.copy` forms whose destination/source operands are no-param direct calls or pure local/constant operands so the existing HOT load/store lowering can match Binaryen's effectful-call shapes. Broader stack-carried effect/control/localizing cases remain blocked.
...
The O4z/raw pipeline also has explicit pre-lift skip gates for currently expensive or representation-sensitive shapes. The 2026-06-19 OI-C audit made those gates trace-accountable for large local-heavy functions, lowered instruction-count blowups, stack-carried effect barriers, load/call mixes, call/local-write mixes, and structured call/branch meshes. The 2026-06-24 raw-gate narrowing admits the exact straight-line `pure local.get/const; no-param direct call; commutative integer binop` subset and flat tiny `memory.copy` sequences with no-param direct-call/local/constant operands through the stack-carried-effect gate; it also admits the exact `i32.const; nonzero-offset scalar load; drop; call` load/call offset-fold shape. Broader stack-carried effects and broader load/call mixes remain skipped.
