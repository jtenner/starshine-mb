...
This is still narrower than Binaryen's full scanner. Starshine now has narrow direct unsigned `maxBits` compare facts, including direct `and` / `shr_u` expressions, unsigned load result widths, and first nonnegative signed-relational out-of-range folds, plus direct i32 sign-extension equality range facts, but it does not yet model CFG joins, loop-carried fact merging, i64 sign-extension equality, signed relational range facts beyond this subset, signed-to-unsigned spelling canonicalization, or broad compare proofs through this substrate.
