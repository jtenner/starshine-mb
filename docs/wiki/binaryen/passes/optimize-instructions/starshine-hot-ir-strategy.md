...
- a narrowed stack-carried call raw boundary: the public/raw pipeline now admits the simple no-param direct-call plus pure `local.get`/constant commutative integer binop form so `local.get + call` can reach HOT and canonicalize to call-first spelling, admits flat tiny bulk-memory forms whose operands independently use no-param direct calls, pure local/constant operands, or direct calls with pure local/constant arguments so the existing HOT tiny-copy/byte-fill lowerings can match Binaryen's effectful-call shapes, and no longer rejects a straight-line function merely because it mixes tiny `memory.copy` with byte `memory.fill`. Broader stack-carried effect/control/localizing cases remain blocked.
...
- constant-size `1` `memory.fill` to `i32.store8`, with constant fill values canonicalized to the low byte and flat stack-carried local/no-param-call plus direct-call value forms with pure local/constant arguments admitted through the raw gate
