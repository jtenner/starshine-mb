...
- stored-value/load-result cleanup for redundant masks in either `and` operand order plus constant stored-value truncation before `i32.store8` / `i32.store16` and, as documented, `i64.store8` / `i64.store16` / `i64.store32`, direct `i32.wrap_i64` i32-store widening, direct reinterpret-store representation rewrites (`f32.store(f32.reinterpret_i32 x)` -> `i32.store x`, etc.), direct one-use full-width reinterpret-load result rewrites (`f32.reinterpret_i32(i32.load p)` -> `f32.load p`, etc.), and one-use i32-load plus `i64.extend_i32_*` rewrites to matching i64 loads (`i64.load32_*`, `i64.load8_*`, `i64.load16_*`) where the loaded value semantics are identical; representation-load rewrites preserve the original load memarg offset and alignment, while local-carried/shared load-result spellings are now documented keep-spelling boundaries
