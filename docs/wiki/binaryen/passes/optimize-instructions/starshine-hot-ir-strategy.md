...
- first `ref.as_non_null` basics from OI-I: `ref.as_non_null(ref.null)` rewrites to `unreachable`, `ref.as_non_null(ref.i31(x))` rewrites to `ref.i31(x)` while preserving effectful payloads such as `ref.i31(call $effect)`, `ref.as_non_null(ref.func f)` rewrites to `ref.func f`, `ref.as_non_null(local.get)` rewrites to the original `local.get` for declared non-null reference locals, and exact `ref.cast(unreachable)` collapses to `unreachable` so stacked cast shapes lower validly
