...
- definitely successful `ref.cast` / `ref.test` cases beyond the covered constructor, exact local, `(ref i31)` local-supertype, and absolute `struct` / `array` local-supertype subsets plus broader failed cast/test cases beyond the local `ref.i31`, absolute struct/array sibling miss proofs, and nullable-target non-null aggregate-local miss coverage
