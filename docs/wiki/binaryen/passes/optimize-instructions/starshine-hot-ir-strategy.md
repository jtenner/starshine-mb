...
- OI-J descriptor-cast raw-skip families include semantic direct-call descriptor-miss, exact-success, and exactification/narrowing scanners plus pure-reference/direct-descriptor-call miss scanners. Exact `ref.func` + `call_ref` descriptor producers are covered for descriptor misses and for same-heap exactification: when the reference producer already returns non-null the scanner rebuilds inexact/nullable casts as non-null exact, and when the reference producer's static result may be null the scanner exactifies nullable inexact casts to nullable exact. Both paths require the direct `ref.func` target and the static `call_ref` type to return an exact non-null descriptor for the cast target with matching descriptor-argument count, then directize the descriptor producer while preserving source order. Standalone `ref.get_desc(ref.as_non_null(block ...))` null-check movement covers branch-free nullable blocks plus escaping direct `br`, value `br_if`, and value `br_table` block children under finite label/source-order preconditions. Exact-success skipping with `call_ref` descriptor producers, return_call_ref/indirect descriptor producers, multivalue, control/EH, broader TNH/IIT, broader effectful/trapping operands, escaping `br_on_*` standalone children, nested control/EH, and `ref.test_desc` tooling remain open.
