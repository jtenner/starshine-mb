...
- OI-J descriptor-cast raw-skip families include semantic direct-call descriptor-miss, exact-success, and exactification/narrowing scanners plus pure-reference/direct-descriptor-call miss scanners. Exact `ref.func` + `call_ref` descriptor producers are covered for descriptor misses, same-heap exactification, the default-mode exact-success boundary, and the no-argument/local-argument TNH exact-success localization subset: descriptor misses fold to direct descriptor calls and `unreachable`, inexact/nullable same-heap casts directize the descriptor producer while rebuilding the residual cast as exact, default exact-success casts directize the descriptor producer but keep the explicit exact `ref.cast_desc_eq`, and `--traps-never-happen` exact-success casts directize the descriptor child, remove the cast, preserve reference-before-descriptor source order, and drop the descriptor result while returning the exact reference. These paths require the direct `ref.func` target and the static `call_ref` type to return an exact non-null descriptor for the cast target with matching descriptor-argument count, then preserve source order. Standalone `ref.get_desc(ref.as_non_null(...))` null-check movement covers direct calls and typed-table call_ref children after directization to `call_indirect` when the call has nontrapping leaf operands and any single reference result, no-argument exact `ref.func` + `call_ref` children after directization, branch-free nullable blocks, and escaping direct `br`, value `br_if`, and value `br_table` block children under finite label/source-order preconditions. IIT removal/localization with `call_ref` descriptor-cast producers, return_call_ref/indirect descriptor producers, broader call_ref standalone children beyond direct ref.func and typed-table targets, multivalue, control/EH, broader effectful/trapping operands, escaping `br_on_*` standalone children, nested control/EH, and `ref.test_desc` tooling remain open.
