...
- tiny `memory.copy` lowering for constant sizes `1`/`2`/`4`/`8`, including direct-core memory64 copy fixtures for `i64` address preservation and a flat stack-carried no-param-call/local/constant operand raw-gate escape that preserves destination-before-source evaluation order
- constant/local-value `memory.fill` lowering for selected sizes (`1`, constant `2`/`4`/`8`, and local.get `2`/`4`/`8`), including size-1 constant low-byte canonicalization, a flat stack-carried byte-fill raw-gate escape for local/no-param-call values, and direct-core memory64 fill fixtures after the local typechecker length fix
- stored-value/load-result cleanup for redundant masks in either `and` operand order plus constant stored-value truncation before `i32.store8` / `i32.store16` and, as documented, `i64.store8` / `i64.store16` / `i64.store32`, direct `i32.wrap_i64` i32-store widening, direct reinterpret-store representation rewrites (`f32.store(f32.reinterpret_i32 x)` -> `i32.store x`, etc.) with source memargs preserved, direct one-use full-width reinterpret-load result rewrites (`f32.reinterpret_i32(i32.load p)` -> `f32.load p`, etc.), and one-use i32-load plus `i64.extend_i32_*` rewrites to matching i64 loads (`i64.load32_*`, `i64.load8_*`, `i64.load16_*`) where the loaded value semantics are identical; representation-load rewrites preserve the original load memarg offset and alignment, while local-carried/shared load-result and reinterpret-store spellings are now documented keep-spelling boundaries
...
- a narrowed stack-carried call raw boundary: the public/raw pipeline now admits the simple no-param direct-call plus pure `local.get`/constant commutative integer binop form so `local.get + call` can reach HOT and canonicalize to call-first spelling, admits flat tiny `memory.copy` forms whose destination/source operands are no-param direct calls or pure local/constant operands so the existing HOT load/store lowering can match Binaryen's effectful-call shapes, and admits flat byte `memory.fill` forms whose destination/value operands use the same local/constant/no-param-call operand proof. Broader stack-carried effect/control/localizing cases remain blocked.
- a narrowed public-pipeline `load-call-optimize-instructions-noop` boundary: the exact constant-address/nonzero-offset scalar-load/drop/direct-call fixture now escapes and folds the offset, while broader mixed plain-load plus call functions still skip the pass
...
- `tuple.extract(tuple.make(...))` forwarding for one-use tuples: pure non-selected siblings are omitted, and the covered single-result effectful-sibling case preserves earlier sibling effects, localizes the selected lane when later effects exist, drops later sibling effects, reloads the selected value, and remains explicit through a focused `simplify-locals-nostructure` neighbor replay
...
- flat stack-carried tiny `memory.copy` sequences with no-param direct-call/local/constant destination/source operands now escape the raw stack-effect gate and lower to the same load/store pairs while preserving destination-before-source evaluation order
...
- constant-size `1` `memory.fill` to `i32.store8`, with constant fill values canonicalized to the low byte and flat stack-carried local/no-param-call value forms admitted through the raw gate
...
- any future source-backed nonconstant wider `memory.fill` materialization beyond the covered direct `local.get` subset; call-backed and computed `i32.add` values for sizes `2`/`4`/`8` are now explicit keep-spelling boundaries under Binaryen `version_130`, while size-1 local/no-param-call values are covered
