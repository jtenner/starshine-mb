...
- stored-value/load-result cleanup for redundant masks in either `and` operand order plus constant stored-value truncation before `i32.store8` / `i32.store16` and, as documented, `i64.store8` / `i64.store16` / `i64.store32`, direct `i32.wrap_i64` i32-store widening, direct reinterpret-store representation rewrites (`f32.store(f32.reinterpret_i32 x)` -> `i32.store x`, etc.), and direct one-use full-width reinterpret-load result rewrites (`f32.reinterpret_i32(i32.load p)` -> `f32.load p`, etc.)
