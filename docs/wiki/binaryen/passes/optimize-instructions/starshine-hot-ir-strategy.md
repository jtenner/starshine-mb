...
- a narrowed stack-carried call raw boundary: the public/raw pipeline now admits the simple no-param direct-call plus pure `local.get`/constant commutative integer binop form so `local.get + call` can reach HOT and canonicalize to call-first spelling, admits flat tiny `memory.copy` forms whose destination/source operands independently use no-param direct calls, pure local/constant operands, or the exact one-pure-argument direct-call form so the existing HOT load/store lowering can match Binaryen's effectful-call shapes, and admits flat byte `memory.fill` forms whose destination/value operands use the same local/constant/no-param-call or one-pure-argument direct-call operand proof. Broader stack-carried effect/control/localizing cases remain blocked.
...
- flat stack-carried tiny `memory.copy` sequences whose destination/source operands independently use local/constant operands, no-param direct-call operands, or the exact `pure local/constant; one-param one-result direct call` form now escape the raw stack-effect gate and lower to the same load/store pairs while preserving destination-before-source evaluation order
...
- constant-size `1` `memory.fill` to `i32.store8`, with constant fill values canonicalized to the low byte and flat stack-carried local/no-param-call plus exact one-pure-argument direct-call value forms admitted through the raw gate
...
- any future source-backed nonconstant wider `memory.fill` materialization beyond the covered direct `local.get` subset; call-backed and computed `i32.add` values for sizes `2`/`4`/`8` are now explicit keep-spelling boundaries under Binaryen `version_130`, while size-1 local/no-param-call and exact one-pure-argument direct-call values are covered
