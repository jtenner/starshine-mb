...
- tiny `memory.copy` lowering for constant sizes `1`/`2`/`4`/`8`, including direct-core memory64 copy fixtures for `i64` address preservation and flat stack-carried raw-gate escapes for local/constant operands, no-param direct-call operands, and the exact one-pure-argument direct-call address form, preserving destination-before-source evaluation order
...
- flat stack-carried tiny `memory.copy` sequences with local/constant operands, no-param direct-call operands, or the exact `pure local/constant; one-param one-result direct call` address operands now escape the raw stack-effect gate and lower to the same load/store pairs while preserving destination-before-source evaluation order
