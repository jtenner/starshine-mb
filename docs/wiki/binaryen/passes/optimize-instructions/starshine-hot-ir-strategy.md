...
- nonconstant-size `memory.copy` / `memory.fill`, because a dynamic size expression is not part of the exact tiny lowering proof; call-backed and local dynamic-size operands are now explicit keep-spelling boundaries under Binaryen `version_130`
