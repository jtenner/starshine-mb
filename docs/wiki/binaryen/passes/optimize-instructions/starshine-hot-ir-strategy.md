...
- known-non-null and literal/disjoint-i31 equality basics from OI-I: `ref.is_null(ref.i31)`, `ref.is_null(ref.func)`, and `ref.is_null(local.get)` for declared non-null reference locals fold to `i32.const 0`; `ref.eq(ref.i31, null)` / `ref.eq(null, ref.i31)` and null equality against declared non-null local refs fold to `i32.const 0`; `ref.eq` between immediate `ref.i31(i32.const)` operands folds to `i32.const 1` for equal payloads or `i32.const 0` for unequal payloads; and `ref.eq` between a definitely non-null `i31` value and a local whose declared heap cannot be `i31` folds to `i32.const 0`; the validator now accepts non-null `ref.is_null` operands so those fixtures can be authored through WAT
