...
- non-constant `eqz` / compare-to-zero rewrites, same-local integer compare folding, pure i32/i64 plus effect-preserving i32 masked unsigned-compare folds, and relational constant plus guarded operand canonicalization
...
- a narrowed stack-carried call raw boundary: the public/raw pipeline now admits the simple no-param direct-call plus pure `local.get`/constant commutative integer binop form so `local.get + call` can reach HOT and canonicalize to call-first spelling, while stack-carried effectful-call `memory.copy` remains explicitly blocked because preserving already-evaluated call-result operands through load/store lowering needs a localizing proof
...
- pure i32/i64 masked unsigned-compare folding when an `and` with a nonnegative mask bounds the value below an out-of-range constant, plus the i32 effect-preserving variant that drops an evaluated masked value before the folded constant
