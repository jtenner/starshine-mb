...
- OI-J descriptor-cast raw-skip families include semantic direct-call descriptor-miss, exact-success, and exactification/narrowing scanners plus pure-reference/direct-descriptor-call miss scanners. Exact `ref.func` + `call_ref` descriptor producers are covered for descriptor misses, same-heap exactification, the default-mode exact-success boundary, and the no-argument/local-argument TNH exact-success localization subset; typed-table `call_ref` reference producers are also covered for TNH exact-success localization after directization to `call_indirect` under nontrapping leaf call/table operands, and descriptor-side typed-table `call_ref` producers are covered for strict-subtype descriptor misses by directizing to `call_indirect`, preserving the nontrapping descriptor inputs and table index, then dropping the descriptor result before `unreachable`; they are also covered for the TNH exact-success subset by directizing to `call_indirect`, preserving the direct reference call before the indirect descriptor call, and dropping the descriptor result: descriptor misses fold to direct descriptor calls and `unreachable`, inexact/nullable same-heap casts directize the descriptor producer while rebuilding the residual cast as exact, default exact-success casts directize the descriptor producer but keep the explicit exact `ref.cast_desc_eq`, and `--traps-never-happen` exact-success casts directize the descriptor child, remove the cast, preserve reference-before-descriptor source order, and drop the descriptor result while returning the exact reference. The `ref.func` descriptor-producer paths require the direct `ref.func` target and the static `call_ref` type to return an exact non-null descriptor for the cast target with matching descriptor-argument count; the typed-table TNH paths require either a directized reference `call_indirect` type returning an exact non-null target reference plus a direct descriptor call that exactly describes the target, or a direct reference call returning an exact non-null target plus a directized descriptor `call_indirect` type returning an exact non-null descriptor for the target, then preserve source order. Standalone `ref.get_desc(ref.as_non_null(...))` null-check movement covers direct calls and typed-table call_ref children after directization to `call_indirect` when the call has nontrapping leaf operands and any single reference result, no-argument exact `ref.func` + `call_ref` children after directization, branch-free nullable blocks, and escaping direct `br`, value `br_if`, and value `br_table` block children under finite label/source-order preconditions. IIT removal/localization with `call_ref` descriptor-cast producers, descriptor-side typed-table exactification and broader indirect producers, return_call_ref descriptor producers, broader call_ref standalone children beyond direct ref.func and typed-table targets, multivalue, control/EH, broader effectful/trapping operands, escaping `br_on_*` standalone children, nested control/EH, and `ref.test_desc` tooling remain open.
- The pure-local both-direct-call descriptor-miss bridge is not arity-specialized. `RawOiDescriptorCallValueGraphFact` finds maximal nontrapping argument runs; `RawOiDescriptorDirectCallMissFact` resolves each direct call and requires its function parameter count to equal the observed run length. A proven strict-subtype reference result plus exact target descriptor result rewrites to `ref-args; call; drop; desc-args; call; drop; unreachable` in source order. The absorbed 84-case probe matrix covered `1..9 × 1..9` and the three tested ten-argument edges; retain representative matrix tests, not an arity whitelist.
- duplicate-branch collapse in then-regions
- `tuple.extract(tuple.make(...))` forwarding for one-use tuples: pure non-selected siblings are omitted, selected trapping lanes remain evaluated as the returned value, selected trapping lanes with one or two earlier effectful siblings preserve those effects before the exact selected load, selected trapping lanes with a later effectful sibling are stored before the sibling call and reloaded after it, the two-earlier-plus-one-later through two-earlier-plus-eight-later selected trapping fixtures store the selected load after both earlier drops and before the later drops/reload, and the covered single-result effectful-sibling case preserves earlier sibling effects, preserves trapping load siblings as dropped effects, localizes the selected lane when later effects exist, drops later sibling effects, reloads the selected value, and remains explicit through focused `simplify-locals-nostructure` neighbor replays for both the first later-effect subset and an earlier+later effectful-sibling direct-HOT subset; full `simplify-locals` and dedicated `tuple-optimization` on the public multivalue-block probe remain boundaries where Binaryen uses tuple scratch but Starshine keeps the block/drop spelling, and direct-HOT replay of the full-neighbor shape currently hits `InvalidChildRef`; multi-result selected lanes through the selected-fifth variant and multi-result non-selected siblings are explicit tuple-scratch localization boundaries
...
- pure and effect-preserving i32/i64 masked unsigned-compare folding when an `and` with a nonnegative mask bounds the value below an out-of-range constant, plus first i32/i64 `shr_u` bounded unsigned-compare folds for constant shift amounts `1..31` / `1..63`, dropping evaluated masked/shifted/loaded values before the folded constant and rewriting in-range nonnegative signed relational compares to unsigned spellings
...
- non-`1`/`2`/`4`/`8`/`16` constant-size `memory.copy` sequences and any future multi-store copy shapes; size `3` is now an explicit Binaryen-backed keep-spelling boundary, while size `16` is covered by the one-load/one-store SIMD proof
- nonconstant-size `memory.copy` / `memory.fill`, because a dynamic size expression is not part of the exact tiny lowering proof; call-backed, local, and global dynamic-size operands are now explicit keep-spelling boundaries under Binaryen `version_130`
- any future source-backed nonconstant wider `memory.fill` materialization beyond the covered direct `local.get` subset and constant size-16 repeated-byte fill case; call-backed and computed `i32.add` values for sizes `2`/`4`/`8`, plus call-backed and local.get-backed size-`16`, are now explicit keep-spelling boundaries under Binaryen `version_130`, while size-1 local/no-param-call and pure-argument direct-call values are covered
