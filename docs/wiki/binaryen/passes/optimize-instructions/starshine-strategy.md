...
- `eqz` rewrites such as subtraction/addition compare lowering, direct double-`eqz` to `ne zero` cleanup, and direct zero-compare `eqz` inversion, and integer-relational `eqz(compare)` inversion while intentionally preserving literal-constant `eqz` nodes to match Binaryen's direct pass output
