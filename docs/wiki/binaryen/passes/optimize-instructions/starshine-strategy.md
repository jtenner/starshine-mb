...
- the OI-J standalone `ref.get_desc` null-check movement family for direct locals, branch-free blocks, self-contained branch/cast blocks, branch-free select/effect-safe if nullable children, and direct-call children with no parameters or nontrapping leaf operands, preserving the nullable child as the `ref.get_desc` operand in default/TNH-focused tests
