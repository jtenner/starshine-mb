...
- `eqz` rewrites such as subtraction/addition compare lowering and direct double-`eqz` to `ne zero` cleanup while intentionally preserving literal-constant `eqz` nodes to match Binaryen's direct pass output
