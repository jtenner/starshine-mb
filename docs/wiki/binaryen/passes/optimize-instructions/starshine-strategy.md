...
- the adjacent OI-J direct-call descriptor exactification/narrowing family under semantic preconditions: ordered pure `local.get*` or `i32.const` ref-call args, direct same-heap non-null ref call, ordered pure `local.get*` or `i32.const` descriptor-call args, direct exact target descriptor call, and source-order-preserving `ref.cast_desc_eq` retention that rebuilds inexact or nullable source casts as non-null exact without skipping the descriptor check
