...
- the OI-J TNH typed-table `call_ref` exact descriptor-success family under semantic preconditions: after call_ref directization, ordered nontrapping leaf reference-call operands plus a nontrapping table index feed a `call_indirect` whose type returns an exact non-null target reference; a direct descriptor call exactly describes the target; default and IIT keep the exact descriptor cast, while `--traps-never-happen` preserves source order as `call_indirect; call desc; drop` without the descriptor equality trap
