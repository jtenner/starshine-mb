...
- broader memory and bulk-memory lowering beyond the covered tiny-copy/fill, stored-value, load-result, offset-fold, and narrow raw-gate escapes; tiny copy/fill now starts from `OiBulkMemoryFact`, but broader bulk-memory ordering/localization, zero-size trap-relaxed cleanup, multi-store materialization, and effect/control operand movement are still open
