...
- `call_ref` directization facts beyond the covered direct/ref.func, constant-index and call-indexed table.get, select, and fallthrough-known subsets with zero arguments or localized single-result arguments; the current target and scalar-operand facts cover those subsets, and `OiRefProducerFact` now gives descriptor-cast logic a shared call_ref signature/result fact, while richer descriptor-producer target/operand facts, broader argument-order, fallthrough type-lattice beyond the current branch-free block/local.tee ref facts, and multi-result argument select-of-`ref.func` directization remain open (the multi-result case is a documented tuple-scratch localization boundary for both `call_ref` and `return_call_ref`)
