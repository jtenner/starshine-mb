...
- the sibling OI-J pure-reference/direct-descriptor-call exact descriptor-miss family under semantic preconditions: pure strict-subtype `local.get` reference operand, ordered nontrapping `local.get*`, scalar constant (`i32.const`/`i64.const` with the shared scanner predicate also covering `f32.const`/`f64.const`), `global.get`, branch-free value-block descriptor-call args, no-parameter direct value-call descriptor-call args, parameterized direct value-call descriptor-call args with ordered nontrapping leaf operands, scalar-producer `local.tee` descriptor-call args, or stack-form trapping integer div/rem scalar descriptor-call args with nontrapping leaves, direct descriptor call with matching parameter count and exact target descriptor result, and source-order-preserving `desc args; call desc; drop; unreachable` output
...
- the adjacent OI-J direct-call descriptor exactification/narrowing family under semantic preconditions: ordered nontrapping `local.get*`, scalar constant (`i32.const`/`i64.const` with the same scanner predicate covering `f32.const`/`f64.const`), or `global.get` ref-call args, direct same-heap non-null ref call, ordered nontrapping `local.get*`, scalar constant, or `global.get` descriptor-call args, direct exact target descriptor call, and source-order-preserving `ref.cast_desc_eq` retention that rebuilds inexact or nullable source casts as non-null exact without skipping the descriptor check
...
- broad reference-typed and GC rewrites beyond the many narrow OI-I/OI-J/OI-K subsets already covered; OI-J now covers direct-call exact descriptor misses with ordered nontrapping local/scalar-constant/global arguments, pure-reference/direct-descriptor-call exact descriptor misses with ordered nontrapping local/scalar-constant/global descriptor arguments plus branch-free value-block, no-parameter direct value-call, parameterized direct value-call descriptor-call arguments with ordered nontrapping leaf operands, scalar-producer local.tee descriptor-call arguments, and stack-form trapping integer div/rem scalar descriptor-call arguments with nontrapping leaves, pure-local direct-call exact descriptor successes, and same-heap direct-call descriptor exactification/narrowing with ordered nontrapping local/scalar-constant/global arguments as semantic families rather than arity whitelists, but nullable/inexact descriptor success/skip breadth outside that narrowing shape, `ref.get_desc` null-check movement/localization, TNH/IIT mode breadth, broader effectful/trapping call arguments beyond the covered nontrapping leaves, branch-free descriptor-call block arguments, no-parameter direct value calls, parameterized direct value calls with nontrapping leaf operands, scalar-producer local.tee value arguments, and stack-form integer div/rem chunks, indirect/call_ref/return_call* and multivalue producers, escaping control/EH descriptor operands, `ref.test_desc`, and descriptor BrOn representation/tooling remain open; the OI-K default-array get subset now keeps negative and huge `array.new_default` lengths to preserve allocation traps before folding constant-index gets
