...
- `call_ref` directization facts beyond the covered direct/ref.func, constant-index and call-indexed table.get, select, and fallthrough-known subsets with zero arguments or single-result arguments; the current target and scalar-operand facts cover those subsets, including bounded direct `ref.func` target fallthrough through `local.tee`, non-conversion `ref.as_non_null`, and block wrappers, plus the Binaryen v130 `visitCallRef` argument-order rule that interposes the target/drop block only at the last operand while leaving earlier operands direct, and `OiRefProducerFact` now gives descriptor-cast logic a shared call_ref signature/result fact, while richer descriptor-producer target/operand facts, broader argument-order beyond that last-operand fallthrough path, fallthrough type-lattice beyond the current branch-free block/local.tee/explicit-unreachable-if/branch-free-loop/ref.cast/ref.as_non_null/single-result-`br_on_*` ref facts, exact-type-sensitive `ref.cast` target fallthrough, and multi-result argument select-of-`ref.func` directization remain open (the multi-result case is a documented tuple-scratch localization boundary for both `call_ref` and `return_call_ref`)
