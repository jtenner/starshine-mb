...
- broad reference-typed and GC rewrites beyond the many narrow OI-I/OI-J/OI-K subsets already covered; OI-J now covers pure-local direct-call exact descriptor misses and exact descriptor successes as semantic families rather than arity whitelists, and direct-call descriptor exactification/narrowing includes ordered scalar constants (now including focused `i64.const` coverage) and `global.get` arguments in addition to local.get arguments, but nullable/inexact descriptor success/skip breadth outside that narrowing shape, `ref.get_desc` null-check movement/localization, TNH/IIT mode breadth, effectful/trapping or broader non-local call arguments, indirect/call_ref/return_call* and multivalue producers, escaping control/EH descriptor operands, `ref.test_desc`, and descriptor BrOn representation/tooling remain open; the OI-K default-array get subset now keeps negative and huge `array.new_default` lengths to preserve allocation traps before folding constant-index gets
