...
- the sibling OI-J pure-reference/direct-descriptor-call exact descriptor-miss family under semantic preconditions: pure strict-subtype `local.get` reference operand, ordered nontrapping `local.get*`, scalar constant (`i32.const`/`i64.const` with the shared scanner predicate also covering `f32.const`/`f64.const`), `global.get`, branch-free value-block descriptor-call args, no-parameter direct value-call descriptor-call args, or parameterized direct value-call descriptor-call args with ordered nontrapping leaf operands, direct descriptor call with matching parameter count and exact target descriptor result, and source-order-preserving `desc args; call desc; drop; unreachable` output
