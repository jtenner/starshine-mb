...
- non-constant `eqz` / compare-to-zero rewrites, same-local integer compare folding, first pure i32 masked unsigned-compare folds, and relational constant canonicalization
...
- pure i32 masked unsigned-compare folding when an `i32.and` nonnegative mask proves the value is below an out-of-range constant
