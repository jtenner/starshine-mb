...
- a whole-function local prescan equivalent beyond the narrow fallthrough sign facts, direct, straight-line-local, and `i32.wrap_i64` `maxBits` compare folds/spelling rewrites, direct sign-extension equality range folds, and source-backed sign-extension relational plus dynamic-shift maxBits keep-spelling boundaries
