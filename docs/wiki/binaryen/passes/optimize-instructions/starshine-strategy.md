...
- `eqz` rewrites such as subtraction/addition compare lowering, direct double-`eqz` to `ne zero` cleanup, direct zero-compare `eqz` inversion, integer-relational `eqz(compare)` inversion, and float equality/inequality `eqz(compare)` inversion while intentionally preserving literal-constant `eqz` nodes and float relational `eqz(compare)` nodes to match Binaryen's direct pass output
