...
- nested boolean-`if` normalization, `eqz` wrapping, the narrow direct De Morgan `i32.and(i32.eqz X, i32.eqz Y)` to `i32.eqz(i32.or X Y)` cleanup, direct boolean constant `select` cleanup for i32 `1/0` and `0/1` arms, and direct `i32.eqz` select-condition cleanup by swapping select arms
...
- `eqz` rewrites such as subtraction/addition compare lowering, direct double-`eqz` to `ne zero` cleanup, direct zero-compare `eqz` inversion, integer-relational `eqz(compare)` inversion, float equality/inequality `eqz(compare)` inversion, and the narrow De Morgan `i32.and(i32.eqz X, i32.eqz Y)` to `i32.eqz(i32.or X Y)` boolean cleanup while intentionally preserving literal-constant `eqz` nodes and float relational `eqz(compare)` nodes to match Binaryen's direct pass output; direct i32 boolean constant `select` arms now fold `1/0` to `i32.ne(cond, 0)` and `0/1` to `i32.eqz(cond)`, and direct `i32.eqz` select conditions now swap select arms and use the inner condition
