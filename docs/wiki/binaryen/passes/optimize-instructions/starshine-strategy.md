...
- broader memory and bulk-memory lowering beyond the covered tiny-copy/fill, stored-value, load-result, offset-fold, trap-relaxed same-local, localtee/get, and block-left fallthrough `memory.copy`, zero-size bulk drops including flat stack-carried call operands under trap-relaxed modes, and narrow raw-gate escapes; tiny copy/fill and the first trap-relaxed cleanup decisions now start from `OiBulkMemoryFact`, but broader bulk-memory ordering/localization, non-flat stack-carried zero-size operands, multi-store materialization, and effect/control operand movement are still open
