...
The 2026-07-06 recursive Binaryen-porting track is replacing narrow scanner families with explicit source-backed facts before widening behavior. `src/passes/optimize_instructions.mbt` now has a Binaryen `LocalScanner`-shaped `OptimizeInstructionsLocalFacts` store for integer sign-extension and unsigned max facts, plus the first OI-J reference/descriptor/null-trap fact layer: `OiRefFact`, `OiNullTrapFact`, `OiDescriptorTypeFact`, and `OiDescriptorOperandFact`. Those OI-J facts currently drive direct known-null `ref.get_desc` trapping, known-non-null `ref.is_null` / `ref.as_non_null` decisions, exact-described-heap descriptor reasoning, descriptor erase/trap decisions, and TNH descriptor localization/exactness guards. `OiCallRefTargetFact` now represents direct `ref.func`, table/index, fallthrough-block `ref.func`, and select-of-`ref.func` targets for call_ref/return_call_ref directization, with `OiCallRefOperandFact` owning the scalar argument localization proof those target families need. OI-G now has `OiTinyMemoryCopyFact` for tiny memory.copy size/load/store choices, `OiTinyMemoryFillFact` for Binaryen v130 tiny memory.fill size/value/shrink boundaries, and `OiMemoryAccessFact` / `OiAtomicAccessFact` for load result types, narrow-store value widths, i64 narrow-store classification, and memarg rewriting. OI-M/localization now has `OiTupleLaneFact` and `OiTupleExtractFact` for one-use `tuple.extract(tuple.make(...))` lane/result/effect-order facts, plus `OiOrderedOperandFact` for scalar ordered-effect preservation in the forward-selected and ordered-effects helpers. This is intentionally partial: the remaining OI-J arity/name scanner families, broader `ref.test_desc` / `ref.cast_desc_eq` facts, descriptor-producer call_ref facts, broader memory/bulk ordering/localization facts, broader tuple-scratch/multivalue localization facts, and final-repair fact families are still active backlog work.
...
- `call_ref` directization facts beyond the covered direct/ref.func, constant-index and call-indexed table.get, select, and fallthrough-known subsets with zero arguments or localized single-result arguments; the current target and scalar-operand facts cover those subsets, while descriptor-producer, broader argument-order, fallthrough type-lattice, and multi-result argument select-of-`ref.func` directization remain open (the multi-result case is a documented tuple-scratch localization boundary for both `call_ref` and `return_call_ref`)
