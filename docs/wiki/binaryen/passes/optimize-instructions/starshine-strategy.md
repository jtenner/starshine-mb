...
- the OI-J standalone `ref.get_desc` null-check movement family for direct locals, branch-free blocks, self-contained branch/cast blocks, and now branch-free select plus effect-safe if nullable children with exactly one nullable reference result, preserving the nullable child as the `ref.get_desc` operand in default/TNH-focused tests
