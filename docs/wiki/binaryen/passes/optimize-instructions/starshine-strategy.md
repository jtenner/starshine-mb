...
- non-constant `eqz` / compare-to-zero rewrites, same-local integer compare folding, pure and effect-preserving i32/i64 masked unsigned-compare folds plus a first pure/effect-preserving i32 `shr_u` bounded unsigned-compare fold, and relational constant canonicalization
...
- pure and effect-preserving i32/i64 masked unsigned-compare folding when an `and` with a nonnegative mask proves the value is below an out-of-range constant, plus the first i32 `shr_u` bounded unsigned-compare fold for constant shift amounts `1..31`, dropping effectful masked/shifted values before the replacement constant
