...
- the adjacent OI-J pure-local direct-call descriptor exactification/narrowing family under semantic preconditions: ordered `local.get*` ref args, direct same-heap non-null ref call, ordered `local.get*` descriptor args, direct exact target descriptor call, and source-order-preserving `ref.cast_desc_eq` retention that rebuilds inexact or nullable source casts as non-null exact without skipping the descriptor check
