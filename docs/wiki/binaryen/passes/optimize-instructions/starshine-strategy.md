...
- tuple extraction parity beyond the one-use tuple.make subset with pure siblings or covered single-result effectful sibling drop/localization; the covered single-result effectful-sibling localization now has first `simplify-locals-nostructure` neighbor coverage, local-carried / multi-use tuple extraction is a documented keep-spelling boundary for the probed Binaryen `version_130` shape, and multi-result non-selected siblings plus multi-result selected children, including the selected-second lane, remain documented tuple-scratch localization boundaries
