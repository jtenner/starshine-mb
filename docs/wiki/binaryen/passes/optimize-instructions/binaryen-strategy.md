...
- widen direct one-use `i32.wrap_i64` values under i32 stores to matching i64 stores, preserving the original store memarg offset and alignment
- keep probed local-carried/shared `i32.wrap_i64` values before narrow i32 stores; the 2026-06-25 `version_130` oracle keeps the `local.tee` shape instead of widening the store
