...
`oi-live-nonzero-memory-cross-copy-boundary` is implemented as an OI-specific smoke transform. It appends a private nonzero i32 destination memory, guards source and destination `memory.size != 0`, performs a reachable length-1 `memory.copy` from source memory 0 to that private destination memory at byte 17, and load/drops the private destination byte. The private destination keeps source-visible memory behavior stable while introducing a real cross-memory copy mutation shape. The focused transform test failed red-first on the missing id/constructor, and the script command-shape test failed until `scripts/lib/oi-parity-sweep.ts` forwarded the new id. The grouped count-216 run `.tmp/oi-g-cross-copy-boundary-count216b-20260629` compares 216/216 with 181 normalized matches, 35 mismatches, zero validation/generator/property/command failures, Binaryen cache hits/misses 173/43, zero canonical size-losing failure dirs, and five raw-size-larger Starshine dirs. The new transform samples twelve cases: eleven match and one repeats the known store-mask low-byte-mask residual. Representative case 000211 shows both tools lower the cross-memory copy to load8/store8 forms (`memory.copy` 0/0, `i32.load8_u` 2/2, `i32.store8` 2/2), with canonical Starshine/Binaryen sizes 86/90 and all outputs validating; the diff is the unrelated `i64.store8` mask where Binaryen keeps `i64.and 255` and Starshine removes it. This is private-destination cross-memory copy/read smoke evidence only, not randomized-address, memory64 cross-copy, multi-memory atomics, live trap-order closure, or OI-J descriptor evidence.
