...
6. `pass-oi-tuple` with randomized trigger-biased selected-lane constructors, broader existing-producer tuple wrappers, broader live effect/trap variants, trapping select operands, and true tuple-scratch probes beyond the current seed-indexed direct/local-carried/all-result/helper-call-produced/local.tee-produced/select-produced/selected-trapping/selected-effectful/effectful-sibling/trapping-sibling/effectful-select-operand multivalue selected-lane trigger smoke plus synthetic selected-lane transform.
