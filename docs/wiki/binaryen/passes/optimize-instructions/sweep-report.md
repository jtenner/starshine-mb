...
These profiles are accepted by `--gen-valid-profile` and have stable labels. `pass-oi-default-scalar`, `pass-oi-local-facts`, `pass-oi-memory-bulk`, `pass-oi-ref-gc`, and `pass-oi-tuple` now emit trigger smoke modules, so they are no longer only broad bounded configs or one fixed trigger module; the remaining profiles are still smoke configs. OI-G is the first of these to move beyond three labels: its profile now includes memory64 dynamic/live boundary/direct tiny-bulk, existing-producer call-wrapped bulk, store-mask, dynamic-i32 bulk, multi-memory bulk, seed-derived random-address copy/read, non-primary-memory random-address copy/read, private-destination cross-memory random-address copy/read, memory64 private-destination cross-memory random-address copy/read, source-visible destination-restore cross-memory copy/read, and memory64 source-visible destination-restore cross-memory copy/read labels. OI-M now has thirteen direct/local-carried/all-result/helper-call-produced/local.tee-produced/select-produced/selected-trapping/selected-effectful/effectful-sibling/trapping-sibling/effectful-select-operand/trapping-select-operand/multi-selected-effectful multivalue selected-lane labels; the count3 grouped sweep at `.tmp/oi-m-tuple-trigger-profile-sweep-count3-20260629`, the count4 grouped sweep at `.tmp/oi-m-tuple-call-produced-sweep-count4-20260629`, the count20 grouped sweep at `.tmp/oi-m-tuple-effectful-sibling-sweep-count20-20260629`, the count24 grouped sweep at `.tmp/oi-m-tuple-trapping-sibling-sweep-count24-20260629`, the count28 grouped sweep at `.tmp/oi-m-tuple-localtee-produced-sweep-count28-20260629`, the count32 grouped sweep at `.tmp/oi-m-tuple-select-produced-sweep-count32-20260629`, the count36 grouped sweep at `.tmp/oi-m-tuple-selected-trapping-sweep-count36-20260629`, the count40 grouped sweep at `.tmp/oi-m-tuple-selected-effectful-sweep-count40-20260629`, the count44 grouped sweep at `.tmp/oi-m-tuple-effectful-select-operand-sweep-count44-20260629`, the count48 grouped sweep at `.tmp/oi-m-tuple-trapping-select-operand-sweep-count48-20260629`, and the count52 grouped sweep at `.tmp/oi-m-tuple-multi-selected-effectful-sweep-count52-20260629` all had zero validation/generator/property/command failures. The count52 sweep compared 52/52 with 52 raw mismatches and sampled the new multi-selected-effectful label three times; manual validation checked all 416 direct+grouped raw/canonical Binaryen/Starshine wasm artifacts, and all direct/grouped residual dirs are sampled raw/canonical size-winning Starshine block-result spelling candidates rather than tuple-scratch, broad existing-producer, broad local.tee/select/effect/trap-producer, broad live trapping, broad effectful/trapping select-operand, multi-result reconstruction closure, or broad effectful family closure. GenValid manifests and compare-pass result artifacts expose `profile_case_label` / `genValidProfileCaseCounts` / `genValidProfileCaseLabel` metadata for OI-D/OI-E/OI-G/OI-I/OI-M, letting agents group raw mismatches without manually opening every WAT first. The next generator-quality slice should turn the seed-indexed OI-D/OI-E/OI-I/OI-M selectors into randomized trigger-biased constructors and continue reducing/classifying OI-G grouped store-mask, commuted-compare, raw-size, multi-memory, broader cross-memory restore/mutation, and atomic-address issues before scaling counts. The current `pass-oi-ref-gc` selector is deliberately non-descriptor because the previous descriptor-bearing broad config produced wasm-tools baseline validation failures in compare-pass; OI-J descriptor/exactness/TNH/IIT remains blocked pending a dedicated profile or compatible oracle path.
...
6. `pass-oi-tuple` with randomized trigger-biased selected-lane constructors, broader existing-producer tuple wrappers, broader live effect/trap variants, broader effectful/trapping select operands, and true tuple-scratch probes beyond the current seed-indexed direct/local-carried/all-result/helper-call-produced/local.tee-produced/select-produced/selected-trapping/selected-effectful/effectful-sibling/trapping-sibling/effectful-select-operand/trapping-select-operand multivalue selected-lane trigger smoke plus synthetic selected-lane transform.
