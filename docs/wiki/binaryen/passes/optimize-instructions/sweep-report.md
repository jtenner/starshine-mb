...
The latest grouped run is `.tmp/oi-g-atomic-xchg-count350-20260629`, which broadens the same atomic RMW transform with byte-preserving xchg restore smoke. Red-first `metamorphic_wbtest.mbt` coverage failed while no `atomic.rmw*.xchg` probes were emitted, then passed after `src/fuzz/metamorphic.mbt` added `i32.atomic.rmw8.xchg_u` and `i64.atomic.rmw8.xchg_u` sequences that atomically load the target byte and exchange the same value back at fixed, dynamic end-byte, and module-shape-derived in-page addresses. The count350 run compares 196/350 transform-applicable cases with 175 normalized matches, 21 mismatches, zero validation/generator/property/command failures, and Binaryen cache hits/misses 189/7; `oi-live-nonzero-memory-atomic-rmw-boundary` samples 10/10 matches, and sampled input case 000016 contains 24 xchg RMW opcodes. Manual validation/size review validates all 84 Binaryen/Starshine raw/canonical failure artifacts and finds zero validation failures, zero raw-size-larger Starshine dirs, zero canonical-WASM-larger dirs, and zero normalized-WAT-larger dirs. Remaining mismatches are store-mask low-byte-mask or commuted/synthetic cleanup candidates, not xchg or atomic-RMW drift. OI-G remains open for stronger seeded/randomized atomic addresses, wait/notify, full-width/source-visible xchg closure, true shared-memory ordering, true live trap/effect ordering, grouped mismatch reduction, raw-size policy beyond fixed encoder residuals, exact cross-memory runtime evidence where needed, and OI-J descriptor/exactness/TNH/IIT remains blocked.
