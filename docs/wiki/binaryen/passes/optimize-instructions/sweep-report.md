...
The latest OI-J include-disabled boundary refresh at `.tmp/oi-j-nondescriptor-boundary-count15-20260630` compared 15/15 with 9 normalized matches, 6 raw mismatches, and zero validation/generator/property/command failures. It confirms the current OI-J row still exercises only the non-descriptor `pass-oi-ref-gc` smoke profile, not descriptor/exactness/TNH/IIT. Manual validation accepted all 24 residual raw/canonical artifacts. Label split: `local-carried-null-test-cast` matched 6/6; `direct-null-test-cast-eq` remains sampled non-descriptor Starshine-win drift across 4 residuals with deltas `-12/-28/-344`, `ref.is_null=8/4`, `drop=38/30`, `local.get=2/2`, and `unreachable=4/4`; `branch-cast-boundaries` remains canonical-neutral/raw-smaller structural drift across 2 residuals with deltas `-8/0/-8` and matching branch/control counts (`br_on_cast=4/4`, `br_on_cast_fail=4/4`, `drop=16/16`, `unreachable=2/2`). Keep OI-J blocked/open until true descriptor-compatible inputs or an oracle path exist.
