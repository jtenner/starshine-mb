...
The latest grouped run is `.tmp/oi-g-randomized-atomic-rmw-count350-20260629`, which broadens the no-op atomic store/RMW/cmpxchg transform from fixed byte 0/16/32768, dynamic end-byte, and non-primary/private-memory smoke to include two module-shape-derived in-page byte addresses. Red-first `metamorphic_wbtest.mbt` coverage failed while those module-derived atomic addresses were absent, then passed after `src/fuzz/metamorphic.mbt` derived stable in-page probes from defined-function and memory counts for memory32 and memory64 forms. The count350 run compares 198/350 transform-applicable cases with 177 normalized matches, 21 mismatches, zero validation/generator/property/command failures, and Binaryen cache hits/misses 189/9; `oi-live-nonzero-memory-atomic-rmw-boundary` samples 10/10 matches. Manual validation/size review validates all 84 Binaryen/Starshine raw/canonical failure artifacts and finds zero validation failures, zero raw-size-larger Starshine dirs, zero canonical-WASM-larger dirs, and zero normalized-WAT-larger dirs. Remaining mismatches are store-mask low-byte-mask or commuted/synthetic cleanup candidates, not module-derived atomic RMW/cmpxchg address drift. OI-G remains open for stronger seeded/randomized atomic addresses, xchg, wait/notify, true shared-memory ordering, true live trap/effect ordering, grouped mismatch reduction, raw-size policy beyond fixed encoder residuals, exact cross-memory runtime evidence where needed, and OI-J descriptor/exactness/TNH/IIT remains blocked.
