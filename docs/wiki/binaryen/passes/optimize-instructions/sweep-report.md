...
The latest grouped run is `.tmp/oi-g-nonprimary-atomic-rmw-count350-20260629`, which broadens the no-op atomic store/RMW/cmpxchg transform from primary-memory-only smoke to non-primary/private-memory smoke. Red-first `metamorphic_wbtest.mbt` coverage failed while the transform still guarded memory 0, then passed after `src/fuzz/metamorphic.mbt` selected an existing non-primary memory or appended a private nonzero memory. The count350 run compares 194/350 transform-applicable cases with 174 normalized matches, 20 mismatches, zero validation/generator/property/command failures, and Binaryen cache hits/misses 187/7; `oi-live-nonzero-memory-atomic-rmw-boundary` samples 10/10 matches. Manual validation/size review validates all 80 Binaryen/Starshine raw/canonical failure artifacts and finds zero validation failures, zero raw-size-larger Starshine dirs, zero canonical-WASM-larger dirs, and zero normalized-WAT-larger dirs. Remaining mismatches are store-mask low-byte-mask or commuted/synthetic cleanup candidates, not non-primary atomic RMW/cmpxchg drift. OI-G remains open for randomized atomic addresses, xchg, wait/notify, true shared-memory ordering, true live trap/effect ordering, grouped mismatch reduction, raw-size policy beyond fixed encoder residuals, exact cross-memory runtime evidence where needed, and OI-J descriptor/exactness/TNH/IIT remains blocked.
