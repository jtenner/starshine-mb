...
| --- | --- |
| `src/passes/coalesce_locals.mbt` | Active module-pass owner with action scan, value-aware interference, exact-type coloring, local-index rewrite, redundant-copy cleanup, structured ineffective-write cleanup with branch-aware effective-write marking, effective-copy weighting, copy-connected-first coloring order, preferred-first concrete GC-ref packing for narrow direct-`struct.get` multi-set locals, immediate `nop; drop` debris cleanup after ineffective tee rewriting, nested nonlocal block-escape reachability for structured cleanup, label-aware structured branch liveness for terminal/branch continuations, dead-write cleanup, structured param-slot reuse, top-level tail param reuse after ineffective dead structured writes, non-loop structured `local.tee` coalescing, structured self-copy cleanup, bounded structured copy-chain forwarding and derived branch-carrier consume-forwarding into dead slots with destination-read-after-source-write rejection, source-write/destination-read interference restoration after copy/consume relaxation, path-disjoint branch-result slot reuse with same-path clobber-read guards, conservative loop unused-local and unread/write-only scratch coalescing, safe unbranched loop-wrapper flattening, label-depth-aware dead-tail sentinel handling, loop-backedge copy preference with earlier-backedge rejection, same-source fanout coloring, a legacy-EH definite-assignment proof across `if`/`try` joins, centralized branch/control-transfer barriers including GC branches and stack-switching exits, a terminal-get-aware linear copy-chain fast path, a `4096` flattened-local guard for the dense non-loop coloring matrices, local-name-section invalidation, and the `[AUDIT006-D]` TypeIdx/RecIdx invariant comment. |
| `src/passes/coalesce_locals_test.mbt` | Focused direct-pass and ordered-neighborhood tests for registration, non-overlap merge, different-value overlap, later reread liveness, structured param-slot reuse, non-loop structured `local.tee` coalescing, structured self-copy cleanup, structured ineffective copy-set cleanup, branch-aware side-carrier effective writes, effective-copy/copy-connected side-carrier coloring, structured-scalar slot-order preference, structured branch copy-chain forwarding, derived branch-carrier consume-forwarding, destination-read-after-source-write guarding, source-write/destination-read interference restoration, top-level tail param reuse after ineffective dead structured writes, path-disjoint branch-result param-slot reuse, legacy-EH copy dominance across bypassing arms, reads before assignment, protected-body throws, and sibling catches, loop unused-local, unread-local scratch, adjacent copy-chain coalescing, non-adjacent copy-through coalescing, loop-backedge copy preference plus nested-old-value-read and earlier-backedge rejection, all represented GC branch barriers, same-source inter-block fanout coloring, unbranched loop-wrapper flattening, local-branch parent-tail preservation, concrete-ref direct-struct-get packing after unused leading and trailing locals, ineffective tee debris before immediate drop, nested block-escape live branch-carrier writes, return-arm dead-write cleanup, branch-to-outer-block-continuation live-write preservation, the 4097-local dense-coloring boundary, redundant-copy cleanup, ineffective-write cleanup, `local-subtyping -> coalesce-locals -> local-cse -> simplify-locals`, and `reorder-locals -> coalesce-locals -> reorder-locals`. |
| `src/validate/gen_valid.mbt` | Defines the eleven-leaf `coalesce-locals-all` aggregate: straight-line, structured, loop copy-through, entry values, copy preference, tee copies, inter-block, unreachable, legacy EH, large unused locals, and exact types. |
| `src/validate/gen_valid_tests.mbt` | Retains the original three core local-copy trigger checks and aggregate-size assertion. |
| `src/validate/gen_valid_coalesce_locals_tests.mbt` | Proves all eleven family leaves are aggregate members and each deterministically emits a validating local-traffic fixture. |
