...
| `src/passes/optimize.mbt` | `coalesce-locals` remains the Binaryen-compatible spelling; `coalesce-locals-cfg` is the Starshine late O4z convergence spelling appended after `strip-debug -> simplify-locals-nostructure`, followed by `reorder-locals -> vacuum`. |
