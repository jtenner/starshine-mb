...
| --- | --- |
| `src/passes/coalesce_locals.mbt` | Active module-pass owner with the established raw action/value-aware paths plus a bounded CFG loop path. The CFG path lifts through HOT IR, reconstructs per-block local actions with owning node ids, requires raw/HOT action-count agreement, uses basic-block live-in/live-out sets, interferes every executing write with currently live locals, colors exact types only, and validates/rolls back changed functions. Per-definition ineffective sets become `drop(value)` and ineffective tees become their non-structured value node through the public HOT mutation helpers; value effects and traps remain. It remains bounded by the 512-local O4z safety frontier and deliberately omits Binaryen's equal-value overlap relaxation. |
| `src/passes/coalesce_locals_test.mbt` | Focused direct-pass and ordered-neighborhood tests, now including CFG loop positives for disjoint write-first ranges, negatives for overlapping live ranges, ineffective-write/live-parameter clobber guards, overwritten-set load preservation, and overwritten-tee value preservation. The O4z late alias is exercised separately from the earlier bounded compatibility slots. |
...
| `src/validate/gen_valid_wbtest.mbt` | Retains the original three core local-copy trigger checks and aggregate-size assertion. |
| `src/validate/gen_valid_coalesce_locals_wbtest.mbt` | Proves all eleven family leaves are aggregate members and each deterministically emits a validating local-traffic fixture. |
| `src/passes/optimize.mbt` | `coalesce-locals` remains the Binaryen-compatible spelling; `coalesce-locals-cfg` is the Starshine late O4z convergence spelling. After `strip-debug`, two `simplify-locals-nostructure -> coalesce-locals-cfg -> reorder-locals -> vacuum` waves reach the measured fixed point. |
| `src/passes/pass_manager.mbt` | Keeps the earlier O4z `coalesce-locals` slots on their bounded raw dispatcher and sends `coalesce-locals-cfg` through the full validated module pass, preventing the new coloring shape from perturbing earlier inlining/simplification profitability. |
