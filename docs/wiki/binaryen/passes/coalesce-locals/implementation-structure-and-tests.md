...
| `src/validate/gen_valid.mbt` | Defines the `coalesce-locals-all` pass-specific GenValid aggregate plus `coalesce-locals-straight-line`, `coalesce-locals-structured`, and `coalesce-locals-loop-copy-through` leaves for dedicated pass-fuzz closeout. |
| `src/validate/gen_valid_tests.mbt` | Validates the CL GenValid profile names/aliases and proves each leaf emits validating local-copy triggers, including a loop-local copy-through shape. |
