...
- The 2026-08-25 production `json-as` semantic closeout found a validating wrong-code family after `ssa-nomerge -> coalesce-locals`. Function-body hybridization across 670 changed `naive/array` bodies isolated absolute function 1244. A body scratch local was assigned from parameter 0, read as a stack-carried call operand, overwritten from parameter 1, and read again for the same call. Coloring the scratch into parameter 1's slot destroyed the still-needed entry value and changed the first call operand; the artifact later trapped out of bounds. Ordinary coalescing now reuses the established stack-carried-overwritten-local/call detector and fails closed for that exact lifetime family. The focused regression is `coalesce-locals intentionally fails closed for stack-carried scratch overwrite calls`. Current native SHA-256 `6c6da9cc5334ba824e073e18013388670e9eca0ae6bd02cf70b5cc75a3bbe852` optimizes and independently validates all 105 pinned naive/SWAR/SIMD artifacts, and exact four-worker no-cache `as-test` passes `105/105` with zero failures or timeouts; aggregate output is `26,228,860` bytes.
