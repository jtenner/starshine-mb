...
- The 2026-08-25 production `json-as` semantic closeout found a validating wrong-code family after `ssa-nomerge -> coalesce-locals`. Function-body hybridization across 670 changed `naive/array` bodies isolated absolute function 1244. A body scratch local was assigned from parameter 0, read as a stack-carried call operand, overwritten from parameter 1, and read again for the same call. Coloring the scratch into parameter 1's slot destroyed the still-needed entry value and changed the first call operand; the artifact later trapped out of bounds. Ordinary coalescing now reuses the established stack-carried-overwritten-local/call detector and fails closed for that exact lifetime family. The focused regression is `coalesce-locals intentionally fails closed for stack-carried scratch overwrite calls`. Native SHA-256 `4948a7ca00a5b2495f9e0d1710c70f06d1f984cf53f1d06e1a3c39e642eb916b` optimizes and independently validates all 105 pinned naive/SWAR/SIMD artifacts, and exact four-worker no-cache `as-test` passes `105/105` with zero failures or timeouts; aggregate output is `26,228,860` bytes.
