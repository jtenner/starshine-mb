...
- The 2026-08-27 follow-up replaces dense `Array[Array[Bool]]` interference and structured write-live hazard matrices with one symmetric triangular `BitSet`, indexed by precomputed row offsets in exactly `N * (N - 1) / 2` bits. Dense copy-weight matrices are likewise gone: `CLCopyWeights` stores only weighted adjacency edges, coloring aggregates those edges by already-assigned slot, removed-copy scoring visits each undirected edge once, and bounded copy reachability traverses adjacency directly. Exact duplicate coloring orders are evaluated once. Deep dense-tee interval analysis now builds branch-depth and escape summaries in one postorder visit instead of recursively rescanning every nested subtree; separate `never_falls_through` and outer escape-range facts preserve the subtle `block { br 1 }` parent-escape relation. The initially naive bit-matrix-only experiment was 123.802ms slower than the prior pass median, but the complete indexed representation plus sparse weights and exact control summaries reaches a final 3,002.950ms pass median, 425.409ms / 12.41% below the accepted 3,428.359ms baseline and `0.966x` Binaryen v131's 3,108.410ms pass median. Whole command is 4,205.134ms / `1.731x` Binaryen. The accepted 4,719,475-byte output remains byte-identical at SHA-256 `dda5cf2d4db0998e4ac777b6fa1089456ba5a7f8c5cf1861c86984603fc624ec`; final native SHA-256 is `02bdbe741b1be767422f41ed40bc90ee93878f0dd4ec670edccaff454d4e457a`.
- Follow-up validation is 22/22 white-box tests, 90/90 focused behavior tests, 7,035/7,035 pass tests, and 10,719/10,719 full Moon tests. The regular 10,000-case lane remains `634` direct + `9,366` cleanup-normalized with zero residuals or validation/property/generator/command failures, 10,000/10,000 deterministic bytes, 10,000/10,000 codec idempotence, external `wasm-tools` validation, and zero canonical size losses. The dedicated aggregate remains `3,750` direct + `5,000` cleanup-normalized + `1,250` pre-existing 16-byte loop-wrapper cleanup wins, with zero Starshine failures and zero canonical size losses. Runtime-callable self semantics remain exact 100/100.

- The 2026-08-27 production pathology closeout reduces direct `coalesce-locals` on the canonical 4,977,401-byte module from `554.787s` pass-local / about `555.976s` command to final medians of `3.428s` pass-local / `4.582s` no-trace command. Verified Binaryen v131 measures `3.108s` pass-local / `2.429s` command in the same one-warmup/three-sample serial protocol, so Starshine is `1.103x` pass-local and `1.886x` whole-command, clearing the v0.1.1 `<=2x` absolute gate by `276.900ms`. Final traced/no-trace outputs are byte-identical and stable at 4,719,475 bytes, SHA-256 `dda5cf2d4db0998e4ac777b6fa1089456ba5a7f8c5cf1861c86984603fc624ec`; they are also byte-identical to the accepted pre-fast-path output. The repair replaces dense or repeated work with sparse live-member interference, linear retained-copy indexing, bounded copy-graph reachability, prefiltered hazard candidates, one structured write/live traversal, direct-region consume candidates, a may/must-proved single-body-local parameter fast path, and a two-action-scan tail-reuse fact index. The parameter fast path fails closed on loops/EH, mixed overwrite joins, reads before definite assignment, parameter reads after possible overwrite, multiple writes, and immediate-drop tees; direct copied parameters retain source orientation, while call-crossing value producers preserve the established Binaryen-shaped alternate-slot choice. Final native SHA-256 is `82cc09a70b29d4ab66081f52fe165dc0883b2efa98cc36e6449b258c231ca086`.
- Final correctness evidence for that closeout is 17/17 white-box tests, 90/90 focused behavior tests, 7,030/7,030 pass tests, and 10,714/10,714 full Moon tests. The regular 10,000-case lane has `634` direct plus `9,366` cleanup-normalized matches, zero residuals/failures, 10,000/10,000 byte-stable determinism checks, 10,000/10,000 codec-idempotence checks, external `wasm-tools` validation, and zero canonical size losses (`9,366` smaller / `634` equal). The dedicated `coalesce-locals-all` lane has `3,750` direct, `5,000` cleanup-normalized, and `1,250` inspected pre-existing `coalesce-locals-loop-copy-through` Starshine wins: Starshine removes one unbranched loop wrapper plus `nop`, saving exactly 16 bytes per case and 20,000 bytes aggregate. A clean-HEAD replay emits byte-identical output for the sampled residual, so this family predates the performance repair. The runtime-callable self-semantic lane is 100/100 exact with zero blocked, mismatching, validation, generator, property, or command cases.

- The 2026-08-25 production `json-as` semantic closeout found a validating wrong-code family after `ssa-nomerge -> coalesce-locals`. Function-body hybridization across 670 changed `naive/array` bodies isolated absolute function 1244. A body scratch local was assigned from parameter 0, read as a stack-carried call operand, overwritten from parameter 1, and read again for the same call. Coloring the scratch into parameter 1's slot destroyed the still-needed entry value and changed the first call operand; the artifact later trapped out of bounds. Ordinary coalescing now reuses the established stack-carried-overwritten-local/call detector and fails closed for that exact lifetime family. The focused regression is `coalesce-locals intentionally fails closed for stack-carried scratch overwrite calls`. Current native SHA-256 `797cb22884706dd376ed142eb7620813481e01e9cf5f85c464e5db2210b96e91` optimizes and independently validates all 105 pinned naive/SWAR/SIMD artifacts, and exact four-worker no-cache `as-test` passes `105/105` with zero failures or timeouts; aggregate output is `26,228,860` bytes.
- A 2026-08-14 loop-coalescing slice adds a CFG-backed path for defaultable functions within the existing 512-local safety boundary. It lifts the function to HOT IR, uses real basic-block live-in/live-out sets, reconstructs local actions in evaluation order, and greedily colors exact-typed locals from a conservative interference matrix. All simultaneously live entry values interfere; every executing write, including an ineffective one, interferes with currently live locals so dead writes cannot become clobbers after coloring. CFG action liveness now also identifies overwritten definitions: ineffective `local.set` nodes become `drop(value)`, and ineffective `local.tee` nodes become their value expression, preserving evaluation/traps while removing only the dead assignment. Structured value children remain fail-closed. The implementation deliberately omits Binaryen's equal-value overlap relaxation. Raw/HOT local-action counts must agree or the path fails closed, and changed functions retain the existing validation/rollback protocol. The public `coalesce-locals-cfg` spelling owns an explicit post-`strip-debug` O4z convergence suffix, leaving the earlier Binaryen-v131-compatible `coalesce-locals` slots unchanged. Two late `simplify-locals-nostructure -> coalesce-locals-cfg -> reorder-locals -> vacuum` waves now run to the measured local fixed point. On BLAKE3 SIMD, dominant function 8 remains at 52 body locals while CFG dead-write cleanup reduces local tees from 1,698 to 1,246 and validated O4z falls from 44,017 to 42,032 bytes (`-1,985`); json-as `bool/naive` reaches 114,460 bytes (`-213`) and BLAKE3 SWAR reaches 22,214 bytes (`-192`). A subsequent bounded SimplifyLocals stack-carried-get slice lowers the same artifact to 41,796 bytes without changing the 52-local coloring, and two late SSA-normalization plus local-cleanup waves reach 41,621 bytes, exact dropped pure SIMD shuffle cleanup reaches 41,109 bytes, and O4z-only bounded small-function stackification reaches 40,903 bytes. Verified Binaryen v131 remains smaller at 39,484 bytes, leaving 1,419 bytes / 3.6%. Regular GenValid is exact at `10000/10000`; the dedicated aggregate reproduces the established `8125` exact matches plus `1875` three-byte Starshine cleanup wins (`-5625` aggregate), with zero validation/property/generator/command failures.
