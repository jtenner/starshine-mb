...
| Port-batch classification | `docs/wiki/ir2/registry-map.md` | Records `type-finalizing` among boundary-only type/global/signature shaping names |
...
- pass parity: `moon build --target native --release src/cmd` followed by `bun fuzz compare-pass --pass type-finalizing ... --jobs auto --starshine-bin _build/native/release/build/cmd/cmd.exe` or the nearest harness spelling accepted by the local Binaryen toolchain
