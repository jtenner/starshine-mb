...
- `moon build --target native --release src/cmd` followed by `bun fuzz compare-pass --pass type-finalizing ... --jobs auto --starshine-bin _build/native/release/build/cmd/cmd.exe` once the local harness accepts the active pass name;
