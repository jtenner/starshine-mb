...
# `remove-exports` Fuzzing Profile

Recommended smoke lane: run the ordinary mixed-generator compare-pass lane for this pass:

```sh
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass remove-exports --out-dir .tmp/pass-fuzz-remove-exports --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe
```
