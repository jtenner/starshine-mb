...
- [`late-pipeline-dispatch.md`](late-pipeline-dispatch.md) - Compact `-O4z` / `shrink` tail roster and dispatch note, now also recording that Debian/docs.rs/README package surfaces are all incomplete in different ways, that the Chromium refs page still shows `version_129` as the newest directly reachable release tag, that the current `main` changelog on both Chromium and official GitHub still lacks a newer documented optimization-pass addition, and that the living ordered-audit summary now names the exact five remaining generated-artifact blockers instead of only the broader failing-pass cluster.
