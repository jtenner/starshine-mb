...
- That same check also found additional upstream-only pass additions in the Chromium-hosted public release notes: `--minimize-rec-groups` is already present by `version_119`, `--string-lifting` and `TypeRefiningGUFA` by `version_124`, `ReorderTypes` is called out by `version_125`, and `--remove-relaxed-simd` plus `--strip-toolchain-annotations` by `version_126`. They are intentionally absent from this catalog because this page tracks Starshine's implemented pass subset, not the full upstream Binaryen catalog, and the reachable non-GitHub sources for this run should be treated as a conservative lower bound rather than a complete pass-history audit.
