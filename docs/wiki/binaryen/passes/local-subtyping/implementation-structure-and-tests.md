...
6. **Dominance pre-scan**
   - The pass admits non-null candidates only when a raw scan proves all gets follow a dominating write in the straight-line root, branch-free `block` bodies, branch-free block writes that dominate later outer gets, terminal `br` / `br_table` block bodies whose gets are already dominated before the branch, branch-free `loop` bodies entered after prior writes, the source-backed loop tail-`br_if` backedge subset, branch-free nested `if` arms inside those dominated regions, branch-free root `if` arms, conditional-`return` branches that skip the later write/get path, or root terminal `return`s after dominated gets. Writes inside a nested loop, if arm, or branch-terminated block are not propagated to the outer post-construct state; branch-free block writes are propagated because local Binaryen v130 narrows that shape. Focused fallback tests pin local Binaryen v130/Starshine validation boundary behavior where loop writes, all-arm `if` writes, block writes with branch flow before a later outside get, `br_if` paths that can skip a later block write, and direct block-return flow narrow only to nullable child for a later outside get.
...
- full structural get-aware non-null repair beyond straight-line roots, branch-free blocks, terminal `br` / `br_table` dominated-get blocks, branch-free block-write post-state, branch-free loop entry bodies plus the narrow tail-`br_if` backedge subset, nested branch-free block-if arms, and branch-free root if arms;
...
- dominance-sensitive non-null propagation for loops with broader backedges/post-state joins, EH, broader `if` join/post-state cases, and branch/return/throw control flow beyond the terminal-branch dominated-get, conditional-return branch, and root terminal-return subsets; current loop write, all-arm `if` write, branch-skipped write, branch-flow block post-state, and direct block-return cases are source-backed nullable/tooling fallbacks;
