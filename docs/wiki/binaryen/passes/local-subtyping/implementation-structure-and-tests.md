...
| `src/passes/local_subtyping_test.mbt` dominance fixtures | Straight-line, branch-free block/loop, loop tail-`br_if` backedge, branch-free block-write post-state, nested branch-free block-if, source-backed nullable loop/if/branch-flow/branch-skipped-write post-state, and branch-free root-if dominance tests cover non-null positives and nullable fallbacks. |
...
- dominance-sensitive non-null propagation for loops with broader backedges/post-state joins, EH, broader `if` join/post-state cases, and branch/return/throw control flow; current loop write, all-arm `if` write, branch-skipped write, and branch-flow block post-state cases are source-backed nullable fallbacks;
