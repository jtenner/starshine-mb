...
| `src/passes/local_subtyping_test.mbt` dominance fixtures | Straight-line, branch-free block/loop, loop tail-`br_if` backedge, terminal `br` / `br_table` dominated-get, branch-free block-write post-state, nested branch-free block-if, branch-free root-if, conditional-return skip, root terminal-return, root/block terminal-throw, try_table body dominated-get including terminal body `throw`, source-backed nullable loop/if/branch-flow/try-table-body/branch-skipped-write post-state, and direct block-return validator-boundary tests cover non-null positives and nullable fallbacks. |
...
- dominance-sensitive non-null propagation for loops with broader backedges/post-state joins, EH, broader `if` join/post-state cases, broader `try_table`/EH flow, and branch/return/throw control flow beyond the terminal-branch dominated-get, conditional-return branch, and root terminal-return, root/block terminal-throw, try_table body dominated-get subsets including terminal body `throw`; current loop write, all-arm `if` write, branch-skipped write, branch-flow block post-state, try-table body post-state, and direct block-return cases are source-backed nullable/tooling fallbacks;
