...
| `src/passes/local_subtyping_test.mbt` dominance fixtures | Straight-line, branch-free block/loop, loop tail-`br_if` backedge, branch-free block-write post-state, nested branch-free block-if, branch-free root-if, conditional-return skip, source-backed nullable loop/if/branch-flow/branch-skipped-write post-state, and direct block-return validator-boundary tests cover non-null positives and nullable fallbacks. |
...
- full structural get-aware non-null repair beyond straight-line roots, branch-free blocks, branch-free block-write post-state, branch-free loop entry bodies plus the narrow tail-`br_if` backedge subset, nested branch-free block-if arms, and branch-free root if arms;
...
- dominance-sensitive non-null propagation for loops with broader backedges/post-state joins, EH, broader `if` join/post-state cases, and branch/return/throw control flow beyond the conditional-return branch subset; current loop write, all-arm `if` write, branch-skipped write, branch-flow block post-state, and direct block-return cases are source-backed nullable/tooling fallbacks;
