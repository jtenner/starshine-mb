...
6. **Dominance pre-scan**
   - The pass admits non-null candidates only when a raw scan proves all gets follow a dominating write in the straight-line root, branch-free `block` bodies, branch-free nested `if` arms inside those blocks, or branch-free root `if` arms. Writes inside a nested block or if arm are not propagated to the outer post-construct state.
...
| `src/passes/local_subtyping_test.mbt:157-406` | Straight-line, branch-free block, nested branch-free block-if, and branch-free root-if dominance tests cover non-null positives and nullable fallbacks. |
...
- full structural get-aware non-null repair beyond straight-line roots, branch-free blocks, nested branch-free block-if arms, and branch-free root if arms;
