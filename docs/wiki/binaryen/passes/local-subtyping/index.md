...
- The current implementation uses write-site evidence plus a raw `local.get` pre-scan for straight-line roots, non-null `local.tee` assignment/use validation, branch-free `block` bodies and block writes that dominate later outer gets, branch-free `loop` bodies entered after a dominating write, branch-free nested `if` arms inside those dominated regions, and branch-free root `if` arms entered after a dominating write. It also has source-backed nullable fallback guards for branch-free loop writes before an outside get, all-arm `if` writes before an outside get, and block writes followed by branch flow before an outside get; `try_table`, branch/return/throw dominance beyond this fallback, broader loop/control-flow dominance and loop post-state non-null propagation, broader `if` post-state joins, and broad expression-type repair are still open.
