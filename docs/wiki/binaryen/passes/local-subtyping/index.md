...
- The tracked remaining Binaryen delta is now explicit and targeted rather than a broad hidden family list, so the active implementation can keep moving while the ref-catch handler-flow representation/tooling blocker and the two nondefaultable-local validator/tooling boundaries stay visible.
