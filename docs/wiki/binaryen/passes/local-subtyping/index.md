...
- The 2026-07-04 behavior-family matrix refreshed Binaryen `version_130` primary sources for `LocalSubtyping.cpp`, `local-subtyping.wast`, and `local-structural-dominance.h`, confirmed they are byte-identical to the earlier `version_129` sources used by this dossier, and reduced the previous broad blocker list. Implemented/protected families cover body-local assignment LUB narrowing, parameter preservation, conservative non-reference/nondefaultable no-rewrite handling, straight-line/unnamed/branch-free structured dominance, terminal-control and try-body dominated-get subsets, source-backed nullable post-state fallbacks, and cleanup-normalized residual routing. The follow-up retag representation slice shows emitted Starshine `local.get` / `local.tee` instructions have no separate expression-type metadata to repair and adds a tee-parent validation guard. The iterative-refinalization slice implements the Binaryen `multiple-iterations` local-get assignment chain by repeating module rewrite/re-lift until stable, protected by `local-subtyping iterates after local.get assignment refinalization`. The select/LUB refinalization slice implements the source-backed `multiple-iterations-refinalize` adjacent-local-get select shape, protected by `local-subtyping iterates after select LUB refinalization`. The call_ref refinalization slice implements the represented zero-param adjacent-local-get call_ref and bottom-call-ref shapes, protected by `local-subtyping iterates after call_ref result refinalization` and `local-subtyping replaces bottom call_ref refinalization with unreachable value`. The ref-catch handler-flow slice proved local Binaryen v130 narrows `catch_ref` / `catch_all_ref` skipped-write locals to nullable exact-child types. The follow-up raw-assignment slice closes that concrete residual without HOT lifting ref-catch result-flow functions: `src/passes/local_subtyping.mbt` now uses a conservative raw assignment collector for ref-catch functions, unknown producers fall back to the declared local type, non-null dominance is disabled on that path, and focused tests require nullable exact-child narrowing plus validating output for both `catch_ref` and `catch_all_ref`. The direct block-return refresh shows Binaryen v130 still narrows the reduced shape to a non-null child local, but `wasm-tools validate --features all` rejects that output with `uninitialized local: 1`; `src/validate/validate.mbt` now has matching boundary coverage. The raw-unreachable tee/get refresh likewise shows Binaryen v130 narrowing the exact-function local to non-null, but `wasm-tools` rejects that output with `uninitialized local: 1`; rebuilt Starshine keeps the local nullable exact and emits validating wasm, now protected by a WAT-parsed focused test. LS remains open for the direct block-return nondefaultable-local validator/tooling boundary if the non-null output becomes valid or repairable, the raw-unreachable-before-write nondefaultable-local validator/tooling boundary exposed by Binaryen's tee/get retag lit shape, and final refreshed closeout evidence. Reopen ref-catch under LS only for a reduced case outside the current raw-assignment subset, such as a catch-payload result join or an unknown producer that must narrow without falling back to the declared type.
...
- The tracked remaining Binaryen delta is now explicit and targeted rather than a broad hidden family list, so the active implementation can keep moving while the two nondefaultable-local validator/tooling boundaries and any future ref-catch cases outside the raw-assignment subset stay visible.
