...
- The tracked remaining Binaryen delta is now explicit and targeted rather than a broad hidden family list, so the active implementation can keep moving while retagging, refinalization, EH handler flow, and the validator/tooling boundary stay visible.
