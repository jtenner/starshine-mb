...
3. use a conservative read-before-write scan over straight-line roots and branch-free `block` bodies to decide whether nullable-to-non-null narrowing is safe;
...
- The current implementation uses write-site evidence plus a raw `local.get` pre-scan for straight-line roots and branch-free `block` bodies; loop, `if`, `try_table`, branch/return/throw dominance and expression-type repair are still open.
