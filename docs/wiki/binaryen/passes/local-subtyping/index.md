...
3. use a conservative read-before-write scan over straight-line roots, branch-free `block` bodies including writes propagated to later outer gets, terminal `br` / `br_table` blocks whose gets are already dominated before the branch, `loop` bodies entered after prior writes including the narrow safe `br_if` backedge-at-tail subset, branch-free nested `if` arms inside dominated regions, branch-free root `if` arms, conditional-`return` branches that skip the later write/get path, and root/block terminal `return`, if-arm nested block terminal `return`/`throw`, root/block terminal `throw`, or a `try_table` body (non-throwing or ending in terminal body `return`/`throw`) after dominated gets to decide whether nullable-to-non-null narrowing is safe, while keeping source-backed nullable fallbacks for loop/if/branch-flow post-state boundaries, direct block-return tooling boundaries, try-table body post-state boundaries, and branch paths that can skip writes;
