...
7. repeat the module rewrite/re-lift loop until no declarations change, so dependent `local.get` assignments and select/LUB assignments see the rebuilt local and expression types;
8. rebuild the module only if something changed.
...
- The tracked remaining Binaryen delta is now explicit and targeted rather than a broad hidden family list, so the active implementation can keep moving while `call_ref` refinalization probes, EH handler flow, and the two validator/tooling boundaries stay visible.
