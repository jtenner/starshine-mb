...
3. use a conservative read-before-write scan over straight-line roots, branch-free `block` bodies, branch-free nested `if` arms inside those blocks, and branch-free root `if` arms to decide whether nullable-to-non-null narrowing is safe;
...
- The current implementation uses write-site evidence plus a raw `local.get` pre-scan for straight-line roots, branch-free `block` bodies, branch-free nested `if` arms inside those dominated blocks, and branch-free root `if` arms entered after a dominating write; loop, `try_table`, branch/return/throw dominance, broader `if` post-state joins, and expression-type repair are still open.
