...
Starshine still lacks two precise Binaryen-relevant surfaces:
...
2. a validator/tooling solution for the raw-unreachable-before-write nondefaultable-local tee/get family exposed by Binaryen's retag lit shape. The 2026-07-04 refresh reduced this to an exact nullable function-local tee/get shape: Binaryen v130 narrows it to non-null exact, `wasm-tools` rejects that output with `uninitialized local: 1`, and rebuilt Starshine keeps a nullable exact local that validates. Reopen ref-catch under LS only for a reduced case outside the raw-assignment subset, such as a catch-payload result join or unknown producer that cannot safely fall back to the declared local type.
...
- **full Binaryen parity**: no, because two nondefaultable-local validator/tooling boundaries remain open and final direct-pass evidence must be refreshed after this code change.
...
1. the direct block-return nondefaultable-local validator/tooling boundary once Starshine validation can prove Binaryen's unreachable-tail shape;
2. the raw-unreachable-before-write nondefaultable-local validator/tooling boundary once Starshine validation can prove Binaryen's tee/get retag lit shape;
