...
5. closing or rerouting the ordered GC/local neighborhood blocker. The direct ordered attempt `.tmp/pass-fuzz-local-subtyping-gc-local-neighborhood-10000-20260703` for `heap2local -> optimize-casts -> local-subtyping -> coalesce-locals -> local-cse` timed out after 3600s with only `200` partial cases (`18` matches, `182` mismatches). A sampled failure shows downstream local declaration/count drift after the neighborhood rather than direct LS narrowing behavior, so final LS closeout needs a smaller neighborhood strategy, downstream local-cleanup parity, or explicit owner classification.
