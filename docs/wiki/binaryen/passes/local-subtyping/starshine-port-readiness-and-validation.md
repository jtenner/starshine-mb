...
5. repeat the rewrite/re-lift loop while body-local declarations keep changing, so later `local.get` assignment evidence can observe earlier declaration rewrites;
6. rebuild the module only when a body local changes.
...
1. focused `multiple-iterations-refinalize` select/LUB and `call_ref` / bottom-call-ref probes beyond the now-implemented local-get assignment chain;
...
1. remaining repeated-refinement probes beyond the local-get chain, especially select/block LUB and `call_ref` cases where one narrowed local sharpens a later assignment;
