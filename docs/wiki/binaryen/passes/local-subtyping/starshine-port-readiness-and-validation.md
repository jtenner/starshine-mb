...
Those three residuals are the next parity slices, not the current shipped pass. Reopen broad retagging only if Starshine adds emitted local expression type metadata or lowers a stale typed HOT graph after declaration rewrites.
