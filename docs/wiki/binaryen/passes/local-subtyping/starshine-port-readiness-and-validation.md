...
The shipped implementation is narrower than Binaryen: it uses write-site evidence to narrow body locals and now has first straight-line plus branch-free `block`, branch-free `loop`, nested block/loop-`if`, and root `if` non-null dominance slices, but it does not yet do full structural get-aware dominance repair, get/tee expression retagging, or iterative refinalization.
...
3. allow nullable-to-non-null narrowing only for the current straight-line, branch-free `block`, branch-free `loop`, nested branch-free region-`if`, and branch-free root `if` subset where every raw `local.get` of that body local follows a dominating write and no `try_table`, branch, return, throw, or broader join/post-state case invalidates the simple proof;
...
| `src/passes/local_subtyping.mbt:143-581` | subtype helpers, assignment collection, candidate narrowing, straight-line/branch-free block/loop/nested-if/root-if dominance scanning, and function rewrite. |
| `src/passes/local_subtyping.mbt:582-613` | active module-pass entrypoint and module rebuild. |
...
| `src/passes/local_subtyping_test.mbt:41-431` | active pass tests for registry lookup, the shipped narrowing cases, straight-line non-null dominance, branch-free block/loop dominance/fallback, nested branch-free block-if dominance, and branch-free root-if dominance/fallback. |
...
- non-null fallback based on structural dominance across loops with backedges or post-state joins, EH regions, broader `if` join/post-state cases, and blocks/ifs with branch/return/throw flow;
...
The current audit slices add and validate focused direct tests: `local.tee` assignments feed narrowing; a straight-line non-null write before all gets can narrow a nullable body local to a non-null child type; branch-free `block` and `loop` bodies entered after a dominating write can contain non-null gets; a branch-free nested `if` inside such a dominated region can contain non-null gets; a branch-free root `if` entered after a dominating write can contain non-null gets; and early gets before the write, including gets inside an earlier block or `if` arm, fall back to the nullable child type. The straight-line, block, loop, nested block-if, and root-if non-null positives failed before implementation and now pass with optimized-module validation.

Then grow coverage in this order:

1. structured-control nullable-to-non-null positives and failures beyond branch-free `block`/`loop` bodies and branch-free root `if` arms;
