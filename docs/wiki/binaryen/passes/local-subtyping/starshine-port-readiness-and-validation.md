...
1. iterative refinalization/reanalysis after one local declaration rewrite exposes a sharper assigned value type;
...
1. repeated refinement after a narrowing change, especially select/block LUB and `call_ref` cases where one narrowed local sharpens a later assignment;
