...
3. allow nullable-to-non-null narrowing only for the current straight-line, branch-free `block`, branch-free block-write post-state, terminal `br` / `br_table` dominated-get block, branch-free `loop` entry, narrow loop tail-`br_if` backedge, nested branch-free region-`if`, branch-free root `if`, and conditional-`return` branch subset where every raw `local.get` of that body local follows a dominating write and no `try_table`, branch-skipped write, direct return/post-state validator boundary, throw, broader loop backedge, or broader join/post-state case invalidates the simple proof; source-backed loop write, all-arm `if` write, branch-skipped write, branch-flow block post-state, and direct block-return cases fall back to nullable child rather than non-null;
...
| `src/passes/local_subtyping_test.mbt` | active pass tests for registry lookup, the shipped narrowing cases, non-null `local.tee` assignment/use validation, straight-line non-null dominance, branch-free block/loop dominance/fallback, loop tail-`br_if` backedge dominance, terminal `br` / `br_table` dominated-get narrowing, branch-free block-write post-state propagation, nested branch-free block-if dominance, conditional-return dominance, source-backed nullable loop/if/branch-flow/branch-skipped-write/direct-return post-state fallback, parameter preservation, and branch-free root-if dominance/fallback. |
