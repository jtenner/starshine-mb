...
The shipped implementation is narrower than Binaryen: it uses write-site evidence to narrow body locals and now has first straight-line plus branch-free `block` non-null dominance slices, but it does not yet do full structural get-aware dominance repair, get/tee expression retagging, or iterative refinalization.
...
3. allow nullable-to-non-null narrowing only for the current straight-line and branch-free `block` subset where every raw `local.get` of that body local follows a dominating write and no loop, `if`, `try_table`, branch, return, or throw invalidates the simple proof;
...
| `src/passes/local_subtyping.mbt:143-451` | subtype helpers, assignment collection, candidate narrowing, straight-line/branch-free block dominance scanning, and function rewrite. |
| `src/passes/local_subtyping.mbt:452-515` | active module-pass entrypoint and module rebuild. |
...
| `src/passes/local_subtyping_test.mbt:41-280` | active pass tests for registry lookup, the shipped narrowing cases, straight-line non-null dominance, and branch-free block dominance/fallback. |
...
- non-null fallback based on structural dominance across loops, `if`, EH regions, and blocks with branch/return/throw flow;
...
The current audit slices add and validate focused direct tests: `local.tee` assignments feed narrowing; a straight-line non-null write before all gets can narrow a nullable body local to a non-null child type; a branch-free `block` entered after a dominating write can contain non-null gets; and early gets before the write, including gets inside an earlier block, fall back to the nullable child type. The straight-line and block non-null positives failed before implementation and now pass with optimized-module validation.

Then grow coverage in this order:

1. structured-control nullable-to-non-null positives and failures beyond branch-free `block` bodies;
