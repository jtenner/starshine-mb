...
| FinalOptimizer `tablify(...)` and `visitSwitch(...)` | `RemoveUnusedBrs.cpp` lowers dense same-value equality `br_if` runs to `br_table`, requiring unique constants, no values, side-effect-free shared condition, range heuristics, fresh default labels, and uniquification; late `visitSwitch` collapses one-target switches when condition/value reorder is safe. | Dense `br_if` ladder to `br_table`; late single-target switch collapse. | Starshine now covers the no-payload dense `tablify(...)` family: `i32.eq` and `i32.eqz`, local/local.tee selector sharing, distinct branch targets, unique constants, Binaryen range thresholds, min-offset selector subtraction, and wrapper default blocks; existing br_table wrapper cleanup subsets remain covered. | Late `visitSwitch` one-target collapse parity remains separate from the completed tablify slice and now blocks final RUB closeout under `[O4Z-AUDIT-RUB-P]`; branch-hint metadata behavior remains `[O4Z-AUDIT-RUB-N]`; payload/value branch ladders stay conservative. | `[O4Z-AUDIT-RUB-I]` complete; `[O4Z-AUDIT-RUB-P]` open for late visitSwitch; `[O4Z-AUDIT-RUB-N]` for hints | Baseline `br_table`. |
