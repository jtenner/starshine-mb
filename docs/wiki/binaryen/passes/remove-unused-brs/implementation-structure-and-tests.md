...
| JumpThreader | `RemoveUnusedBrs.cpp` collects no-value scope uses via `operateOnScopeNameUsesAndSentTypes`, redirects through one-child named blocks, redirects child jumps to a following simple `br`, and turns simple jumps to child-plus-`unreachable` into direct `unreachable`; `remove-unused-brs_trap.wast` proves trap-sensitive behavior. | Trivial jump retargeting and branch-to-trap direct trap rewrite. | Starshine now covers the branch-to-trap direct rewrite: when a void block is immediately followed by `unreachable`, simple childless `br` instructions targeting that block become `unreachable`; `br_if` and `br_table` targets stay conservative. Starshine also has wrapper cleanup and continuation retargeting subsets, but not general JumpThreader parity. | Need branch retargeting through named shells, child-to-child jump chains, scope sent-type preservation, and the now-release-baseline type-equality relaxation. | `[O4Z-AUDIT-RUB-M]` complete for branch-to-trap; `[O4Z-AUDIT-RUB-G]` remains for general JumpThreader | Baseline; branch-to-trap is in local `version_130`, not optional drift. |
