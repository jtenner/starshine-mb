...
| Early `optimizeSwitch(...)` | `RemoveUnusedBrs.cpp` trims trailing default targets, removes leading defaults by subtracting an offset, handles default-only / one-target / two-option tables, and lowers very large mostly-default tables to nested `if`; `remove-unused-brs_shrink-level=1.wast` has `simple-switch*` cases. | Early switch cleanup before final optimizer. | Starshine now covers the safe no-payload early switch subset: trailing-default trimming, leading-default offsetting with `i32.sub`, default-only lowering to `drop(selector); br`, and one-explicit-target/two-option lowering to branch-if structure. It still has br_table continuation-wrapper retargeting and repeated equality-ladder-to-`br_table`. | Remaining blocker: Binaryen's large mostly-default nested-`if` lowering is covered as a fail-closed test for the nested stack-style local HOT shape; widening the nested traversal regressed existing long two-arm branch-drain behavior, so the safe implementation leaves that table intact until a narrower traversal proof lands. Value-carrying switches also stay conservative to avoid exposing older br_table-sensitive payload rewrites in the same RUB cycle. | `[O4Z-AUDIT-RUB-B]` safe subset complete; large/value blockers remain documented for RUB-O/future switch work. | Baseline `br_table`; no optional gate. |
