...
The July 17 DAEO-prefix repair adds rollback-capable changed-function batch validation plus fail-closed multivalue-carrier, nullable-return/non-null-result, and giant-table convergence guards. The final explicit-native regular lane is `10000/10000` with `1520` normalized plus `8480` cleanup-normalized and zero mismatches/failures. The dedicated runtime sample reproduces the accepted `115`-mismatch side-effect-free family with zero validation failures and Node runtime `89` equal results / `26` equal traps / `0` semantic mismatches. Direct current-artifact RUB falls from `580.178s` and invalid output to valid deterministic `3.239s` / `3.068s` repeats; three productive applications reach a byte-identical fixed point. The recorded measurements used historical Binaryen v130; new comparison signoff uses Binaryen 132.
