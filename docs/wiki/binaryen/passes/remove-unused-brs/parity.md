...
- `[O4Z-AUDIT-RUB-H]` adjacent `br_if` merge parity is now implemented for the shrink-mode no-payload safe subset: child-form and stack-form adjacent `br_if`s to the same target merge into one `br_if` whose condition is `i32.or(cond1, cond2)`, guarded by a local proof that speculating the later condition is side-effect/trap-safe. The former broad O4z raw no-op gate is removed, so this behavior is observable under scheduled `-O4z` options without a special candidate exception. `[O4Z-AUDIT-RUB-Q]` later added the non-shrink no-payload adjacent `br_if; br` cleanup and note `1377` added the same-value self-target block-tail subset for repeated `local.get`/constant values; adjacent same-target value-carrying branch merges beyond that self-target same-value shape are now a source-backed no-value-assert boundary in note `1382`; broad Binaryen cost/effect/model equality reopens only with a focused HOT structural-equality/invalidation proof; branch-hint `applyOrTo`/`clear` and `never-unconditionalize` remain under the `[O4Z-AUDIT-RUB-N]` / `[O4Z-AUDIT-RUB-X]` metadata/pass-option boundary.
...
- Perf and bailout coverage:
  [`../../../../../src/passes/remove_unused_brs_test.mbt`](../../../../../src/passes/remove_unused_brs_test.mbt) and [`../../../../../src/passes_perf_long/remove_unused_brs_perf_test.mbt`](../../../../../src/passes_perf_long/remove_unused_brs_perf_test.mbt)
...
3. Reopen only for a true semantic mismatch, validation failure, loss of a measured win, a newly size-losing/unclassified family, or upstream post-v131 behavior drift.
...
- `[O4Z-AUDIT-RUB-B]` landed the safe early switch subset from Binaryen `optimizeSwitch(...)`:
  ...
  - no-payload large mostly-default nested stack-style tables lower to Binaryen-style nested `if` form through the ordinary candidate path now that the broad O4z raw no-op gate is removed
