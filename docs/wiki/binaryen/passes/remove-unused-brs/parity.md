...
- The uniform multivalue-drop `+1` family is repaired as a stronger Starshine lowering. Exact unused literal multivalue blocks consumed only by result drops flatten without Binaryen's four-scratch-local shell. Focused evidence is `1000/1000` measured wins, uniformly `-12` bytes. The aggregate remains `5655/10000` exact but improves to a `-81681`-byte residual delta; all `3125` size-different residuals are now smaller Starshine outputs. Direct parity remains open for `1220` equal-size reference-typing/output-shape gaps and classification of the smaller compound residuals.
...
1. Repair the equal-sized broader-reference result typing in constant-branch and multi-function leaves.
2. Classify the smaller control, cleanup, switch, GC, and multivalue-drop residuals; the multivalue singleton is already a measured `-12` Starshine win, while compound families still need owner-by-owner confirmation.
