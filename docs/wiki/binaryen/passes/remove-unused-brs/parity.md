...
- The current tree already covers much more than dead tail stripping:
  ...
  - JumpThreader safe subsets: branch-to-trap rewriting for simple branches to void blocks followed by `unreachable`, no-payload `br_if` retargeting through one-child named block shells, no-payload `br_if` retargeting to a following simple jump, the narrow direct childless-`br` one-child shell subset from `[O4Z-AUDIT-RUB-Q]`, the no-payload `br_table` target/default retarget subset from `[O4Z-AUDIT-RUB-Q]`/`[O4Z-AUDIT-RUB-V]` for zero-child or one-selector-child HOT tables with arity-zero targets, including pure ten-target shell tables while keeping mostly-default switch-owned tables conservative, the audited stack-source WAT table subset that lowers to selector-child HOT tables, the audited `if`-arm subset for direct-branch normalization plus small child-form no-payload `br_table` retargeting, and the no-payload `try_table` catch-target child-to-following-simple-jump subset
  - Binaryen-style `sinkBlocks(...)` safe subset for void block/loop rotation plus void, one-result, and multi-result block/single-if sinking into the unique multi-root label-using arm, with condition-label-use and both-arm-use negatives
  ...
  - GC `br_on_*` cleanup for the safe local subset: definitely taken/not-taken `br_on_null` and `br_on_non_null`, definitely successful `br_on_cast`, definitely not-taken `br_on_cast_fail`, no-payload non-null disjoint-family `br_on_cast` / `br_on_cast_fail` failure rewrites, no-payload nullable-source/non-null-target `br_on_cast` splitting to `br_on_non_null` plus appended `ref.null`, branch-taking prefix-payload variants for definitely taken `br_on_null` / `br_on_non_null`, definitely successful `br_on_cast`, and definitely failing non-null disjoint `br_on_cast_fail`, and child-form ordinary `br_on_cast*` unreachable-input cleanup with payload drops, with explicit fail-closed boundaries for fallthrough-producing payload cases, payload-bearing cast splits, descriptor variants (closed as a representation blocker in note `1396`), broader fallthrough/cast insertion, stack-form unreachable-input cleanup (closed as a child-form exposure/raw-proof blocker in note `1396`), and nullable disjoint `SuccessOnlyIfNull`
  ...
  - adjacent same-target no-payload branch cleanup, including non-shrink `br_if; br` condition dropping and shrink-mode `br_if`/`br_if` merging with `i32.or` when the later condition is locally safe to speculate
  ...
  - the approved note-`1392` Starshine-win cleanup for side-effect-free dead structured trap/control shells after constant-`br_if` folding or same-target `br_table` selector-drop/payload-forwarding; RUB-R covers scalar and multivalue representatives, RUB-S adds exact `v128.const` payload coverage, and `remove_unused_brs_prune_dead_suffix_after_nonfallthrough(...)` remains the explicit helper owner
- RUB-S raw-size triage in notes `1394`/`1395` closed with a split outcome. The actionable raw-size owner was residual public stack-form `v128` same-target `br_table` payloads: every positive raw regression in the saved `1000-max100` dedicated failure set originally had Starshine `br_table` residue where Binaryen had none, led by a reduced `v128` case that was `80` raw bytes in Starshine versus `65` in Binaryen. The implementation fixed that owner by treating exact `v128.const` SIMD payloads as reorder-safe for one-target value-switch collapse; the reduced case is now Starshine `61` raw bytes versus Binaryen `65`, and saved-lane reruns have `0` residual Starshine `br_table` cases. The remaining no-table positives are intentionally blocked on result-suffix preservation: the leading `case-000073` (`+38`) reduces to a void structured block with `return` in a result-typed function followed by a dead result suffix, which `remove_unused_brs_root_suffix_required_after_void_structured_terminal(...)` keeps to avoid the note-`1388` validation failure family.
...
- `[O4Z-AUDIT-RUB-Q]` is the user-directed complete-family revisit beyond the v0.1.0 closeout. Its first implementation slice added a narrow no-payload direct-`br` JumpThreader subset for direct childless `br` nodes reached through ordinary body/child traversal and retargeted through one-child named block shells. Its second implementation slice added small child-form no-payload `br_table` target/default retargeting through one-child block shells and child-block-to-following-simple-jump shapes, while explicitly keeping value-carrying/sent-type tables conservative. Its third implementation slice audited `if` arms: direct no-payload branches in the probed shape were already covered via existing branch-exit normalization to `br_if`, and small child-form no-payload `br_table` targets now retarget through then/else regions. Its fourth implementation slice widened no-payload table retargeting to small/medium zero-child or one-selector-child HOT tables with at most eight explicit targets, covering audited stack-source WAT table forms while keeping larger mostly-default switch-lowering interactions blocked; note `1375` then raised the defensive table cap from eight to nine explicit targets after a broad 32-target probe regressed the existing large mostly-default switch fixtures, and note `1396` replaced the blanket larger-table cap with a mostly-default-shape guard so pure ten-target shells retarget while ten-target mostly-default tables stay conservative. Its fifth slice audited payload/sent-type JumpThreader directly against Binaryen `version_130`: `JumpThreader::visitExpression(...)` records only `Type::none` sent-type uses, so payload-carrying `br`, `br_if`, and `br_table` retargeting is a source-backed non-goal with boundary coverage rather than an implementation gap. Its sixth slice added no-payload `try_table` catch-target retargeting for the child-block-to-following-simple-jump shape and boundary coverage for `catch_ref`. Its seventh slice closed the analogous one-child-shell catch-target retarget with a narrow raw-gate admission plus HOT catch-label retarget helper. Its eighth slice broadened the GC `br_on_cast` safe subset for Binaryen's `SuccessOnlyIfNonNull` result: no-payload nullable-source/non-null-target casts now lower to `br_on_non_null` plus appended `ref.null`, while descriptor, broader fallthrough/cast-insertion, unreachable-input, and payload-bearing split cases remain open boundaries. Its ninth slice broadened `sinkBlocks(...)` from void-only single-if sinks to the one-result matching block/if subset and added a temporary multi-result boundary pending HOT repeated-root/arm repair; its thirteenth slice removed that boundary by supporting matching multi-result repeated-root block/if sinks. Its fourteenth slice added the locally safe GC branch-taking prefix-payload subset for definitely taken `br_on_null` / `br_on_non_null`, definitely successful `br_on_cast`, and definitely failing non-null disjoint `br_on_cast_fail`, while keeping fallthrough-producing payload rewrites and payload-bearing cast splits blocked on child-localization/fallthrough-result proof. Its fifteenth slice probed the fallthrough-producing stack-payload `SuccessOnlyIfNonNull` split and documented it as a narrower ChildLocalizer/scratch-local blocker with boundary coverage. Its sixteenth slice audited descriptor `br_on_cast_desc_eq*` as a representation blocker and implemented the child-form ordinary `br_on_cast*` unreachable-input subset with payload drops before the replacement `unreachable`, while keeping public stack-form unreachable-input cleanup as a candidate-lowering boundary. Its tenth slice closed the no-payload adjacent `br_if` + unconditional same-target `br` cleanup for child-form and stack-form HOT shapes, preserving condition effects with `drop(condition)` and adding a value-carrying boundary backed by Binaryen's no-value asserts. Its eleventh slice audited payload/value `tablify(...)` ladders and child-less stack-payload switches: value-carrying equality ladders are now a source-backed Binaryen `version_130` non-goal for `tablify(...)`, while public stack-style one-target value switches lift to child-form payloads and remain covered by the existing late `visitSwitch(...)` collapse; the only switch residue is the local child-less stack-payload HOT shape. Its twelfth slice added the no-payload non-null disjoint-family `br_on_cast` / `br_on_cast_fail` definite-failure subset, while keeping nullable disjoint `SuccessOnlyIfNull` as a source-backed Binaryen `version_130` TODO boundary. Note `1376` added explicit public-pipeline evidence that legacy `try` lowers away before RUB and does not expose a live HOT `Try`/`TryTable` caught-throw shape. Note `1377` added the local repeated-`local.get`/constant subset of Binaryen's redundant self-target `br_if` value cleanup, with a `local.tee` invalidation boundary. Note `1378` audited the remaining final value-legality guards for `selectify(...)`, `restructureIf(...)`, and `optimizeSetIf(...)`: Starshine now has focused boundaries for condition-invalidated selectification, dropped self-target prefix selectification when the condition invalidates the fallthrough rest, and conditional `br_if` set-if arms; unreachable-condition selectify coverage is classified as a HOT-lift command/tooling boundary until polymorphic unreachable conditions lift cleanly. Notes `1379`-`1383` narrow the prior residue: old-`try` catch-region bodies, child-less stack-payload switches, unreachable-condition selectify, table retargeting beyond the `<= 9` switch-interaction guard, remaining GC descriptor/fallthrough/localizer cases, and adjacent/value final-optimizer residue are accepted representation/tooling/source boundaries with explicit reopening criteria, not broad open transform-family work. Notes `1385`-`1388` add raw-gate/pass-local timing, refreshed normalized direct-compare progress, the first dedicated `remove-unused-brs-all` GenValid aggregate, a completed final regular lane, and the first dedicated-profile validation reduction. The current bounded timing lane is pass-local faster than Binaryen on the O4z startup repro; regular GenValid `10000`, explicit wasm-smith `10000`, broad `pass-fuzz-stress` `10000`, and the rerun regular GenValid `100000` are mismatch-free under the accepted cleanup normalizers. The prior regular `100000` timeout is superseded by `.tmp/pass-fuzz-remove-unused-brs-rub-q-regular-100000-normalized-rerun-long` (`100000/100000`, `14604` normalized, `85396` cleanup-normalized, `0` mismatches/failures). RUB-Q is now closed by note `1392` under the approved-substitute clause: the stale dedicated validation blocker was superseded by current-binary evidence, and the remaining dedicated-profile drift is the accepted side-effect-free dead structured shell / pure selector-drop / branch-payload forwarding cleanup family rather than a validation or runtime semantic failure. RUB-R makes that accepted Starshine win explicit with focused tests for constant-`br_if` dead-shell deletion and same-target value/multivalue `br_table` payload forwarding, and documents `remove_unused_brs_prune_dead_suffix_after_nonfallthrough(...)` as the intended owner. Do not restore Binaryen dead shells merely to make dedicated normalized output greener; reopen only under note `1392` criteria. Evidence and commands live in [`../../../raw/research/1359-2026-06-29-remove-unused-brs-direct-br-jumpthreader.md`](../../../raw/research/1359-2026-06-29-remove-unused-brs-direct-br-jumpthreader.md), [`../../../raw/research/1360-2026-06-29-remove-unused-brs-brtable-jumpthreader.md`](../../../raw/research/1360-2026-06-29-remove-unused-brs-brtable-jumpthreader.md), [`../../../raw/research/1361-2026-06-29-remove-unused-brs-if-arm-jumpthreader.md`](../../../raw/research/1361-2026-06-29-remove-unused-brs-if-arm-jumpthreader.md), [`../../../raw/research/1362-2026-06-29-remove-unused-brs-stack-source-brtable-jumpthreader.md`](../../../raw/research/1362-2026-06-29-remove-unused-brs-stack-source-brtable-jumpthreader.md), [`../../../raw/research/1363-2026-06-29-remove-unused-brs-payload-jumpthreader-boundary.md`](../../../raw/research/1363-2026-06-29-remove-unused-brs-payload-jumpthreader-boundary.md), [`../../../raw/research/1364-2026-06-29-remove-unused-brs-trytable-catch-jumpthreader.md`](../../../raw/research/1364-2026-06-29-remove-unused-brs-trytable-catch-jumpthreader.md), [`../../../raw/research/1365-2026-06-29-remove-unused-brs-trytable-catch-shell-jumpthreader.md`](../../../raw/research/1365-2026-06-29-remove-unused-brs-trytable-catch-shell-jumpthreader.md), [`../../../raw/research/1366-2026-06-29-remove-unused-brs-gc-nullable-cast-split.md`](../../../raw/research/1366-2026-06-29-remove-unused-brs-gc-nullable-cast-split.md), [`../../../raw/research/1367-2026-06-29-remove-unused-brs-result-sinkblocks.md`](../../../raw/research/1367-2026-06-29-remove-unused-brs-result-sinkblocks.md), [`../../../raw/research/1368-2026-06-29-remove-unused-brs-adjacent-brif-br.md`](../../../raw/research/1368-2026-06-29-remove-unused-brs-adjacent-brif-br.md), [`../../../raw/research/1369-2026-06-29-remove-unused-brs-tablify-switch-boundaries.md`](../../../raw/research/1369-2026-06-29-remove-unused-brs-tablify-switch-boundaries.md), [`../../../raw/research/1370-2026-06-29-remove-unused-brs-gc-disjoint-cast.md`](../../../raw/research/1370-2026-06-29-remove-unused-brs-gc-disjoint-cast.md), [`../../../raw/research/1371-2026-06-29-remove-unused-brs-multivalue-sinkblocks.md`](../../../raw/research/1371-2026-06-29-remove-unused-brs-multivalue-sinkblocks.md), [`../../../raw/research/1372-2026-06-29-remove-unused-brs-gc-prefix-branch-payload.md`](../../../raw/research/1372-2026-06-29-remove-unused-brs-gc-prefix-branch-payload.md), [`../../../raw/research/1373-2026-06-29-remove-unused-brs-gc-fallthrough-payload-split-boundary.md`](../../../raw/research/1373-2026-06-29-remove-unused-brs-gc-fallthrough-payload-split-boundary.md), [`../../../raw/research/1374-2026-06-29-remove-unused-brs-gc-unreachable-and-descriptor-audit.md`](../../../raw/research/1374-2026-06-29-remove-unused-brs-gc-unreachable-and-descriptor-audit.md), [`../../../raw/research/1375-2026-06-29-remove-unused-brs-medium-brtable-jumpthreader-guard.md`](../../../raw/research/1375-2026-06-29-remove-unused-brs-medium-brtable-jumpthreader-guard.md), [`../../../raw/research/1376-2026-06-29-remove-unused-brs-legacy-try-exposure-boundary.md`](../../../raw/research/1376-2026-06-29-remove-unused-brs-legacy-try-exposure-boundary.md), [`../../../raw/research/1377-2026-06-29-remove-unused-brs-redundant-self-brif-value.md`](../../../raw/research/1377-2026-06-29-remove-unused-brs-redundant-self-brif-value.md), and [`../../../raw/research/1378-2026-06-29-remove-unused-brs-final-value-legality-audit.md`](../../../raw/research/1378-2026-06-29-remove-unused-brs-final-value-legality-audit.md), [`../../../raw/research/1379-2026-06-29-remove-unused-brs-stack-representation-boundaries.md`](../../../raw/research/1379-2026-06-29-remove-unused-brs-stack-representation-boundaries.md), [`../../../raw/research/1380-2026-06-29-remove-unused-brs-gc-remaining-boundary-closure.md`](../../../raw/research/1380-2026-06-29-remove-unused-brs-gc-remaining-boundary-closure.md), [`../../../raw/research/1381-2026-06-29-remove-unused-brs-large-brtable-jumpthreader-boundary.md`](../../../raw/research/1381-2026-06-29-remove-unused-brs-large-brtable-jumpthreader-boundary.md), [`../../../raw/research/1382-2026-06-29-remove-unused-brs-final-optimizer-boundary-closeout.md`](../../../raw/research/1382-2026-06-29-remove-unused-brs-final-optimizer-boundary-closeout.md), [`../../../raw/research/1383-2026-06-29-remove-unused-brs-boundary-consolidation.md`](../../../raw/research/1383-2026-06-29-remove-unused-brs-boundary-consolidation.md), [`../../../raw/research/1384-2026-06-29-remove-unused-brs-boundary-closeout-validation.md`](../../../raw/research/1384-2026-06-29-remove-unused-brs-boundary-closeout-validation.md), [`../../../raw/research/1385-2026-06-29-remove-unused-brs-raw-gate-performance-refresh.md`](../../../raw/research/1385-2026-06-29-remove-unused-brs-raw-gate-performance-refresh.md), and [`../../../raw/research/1386-2026-06-29-remove-unused-brs-signoff-refresh.md`](../../../raw/research/1386-2026-06-29-remove-unused-brs-signoff-refresh.md).
...
- `[O4Z-AUDIT-RUB-E]` EH caught-throw parity is now implemented for the local `version_130` `visitThrow(...)` safe `try_table` subset: exact tag catches rewrite `throw` to payload-carrying `br`, `catch_all` without ref rewrites payload children to `drop`s followed by `br`, and tag mismatch / `catch_ref` / `catch_all_ref` / earlier catch-ref-before-catch-all cases remain explicit fail-closed exnref-transport boundaries. Legacy old-`try` mixed-control stays a local representation blocker because the public WAT path lowers legacy `try` away before HOT; note `1376` locks this with an explicit no-live-`HotOp::Try` boundary test.
- `[O4Z-AUDIT-RUB-H]` adjacent `br_if` merge parity is now implemented for the shrink-mode no-payload safe subset: child-form and stack-form adjacent `br_if`s to the same target merge into one `br_if` whose condition is `i32.or(cond1, cond2)`, guarded by a local proof that speculating the later condition is side-effect/trap-safe. The O4z raw no-op gate now admits simple stack-condition candidates so this behavior is observable under `-O4z`-style options. `[O4Z-AUDIT-RUB-Q]` later added the non-shrink no-payload adjacent `br_if; br` cleanup and note `1377` added the same-value self-target block-tail subset for repeated `local.get`/constant values; adjacent same-target value-carrying branch merges beyond that self-target same-value shape are now a source-backed no-value-assert boundary in note `1382`; broad Binaryen cost/effect/model equality reopens only with a focused HOT structural-equality/invalidation proof; branch-hint `applyOrTo`/`clear` and `never-unconditionalize` remain under the `[O4Z-AUDIT-RUB-N]` / `[O4Z-AUDIT-RUB-X]` metadata/pass-option boundary.
...
- `[O4Z-AUDIT-RUB-B]` landed the safe early switch subset from Binaryen `optimizeSwitch(...)`:
  ...
  - child-less local stack-payload value switch shapes remain conservative because ordinary lifted value switches carry payloads as `br_table` children; note `1379` adds focused representation coverage for that public-path fact
...
- `[O4Z-AUDIT-RUB-G]` landed the conditional no-payload JumpThreader safe subset:
  ...
  - this older reopening list was narrowed by `[O4Z-AUDIT-RUB-Q]` and `[O4Z-AUDIT-RUB-V]`: direct unconditional `br` and no-payload `br_table` retargeting are now implemented for the safe arity-zero subset, including pure ten-target shell tables; payload sent-type retargeting is a source-backed JumpThreader non-goal for Binaryen `version_130`; try_table catch retargeting is covered for the no-payload safe subset; and note `1396` keeps mostly-default switch-owned larger tables conservative with a focused ten-target boundary
...
- `[O4Z-AUDIT-RUB-D]` landed the `sinkBlocks(...)` safe subset:
  ...
  - `[O4Z-AUDIT-RUB-Q]` later broadened that sinker to one-result matching block/if types in note `1367`, then to matching multi-result repeated-root block/if types in note `1371`
  ...
  - remaining boundaries: direct unreachable-condition sink assertions when ordinary DCE erases the shape first, and exact Binaryen branch-hint/refinalization metadata behavior
...
- `[O4Z-AUDIT-RUB-F]` landed the GC BrOn safe subset of Binaryen `optimizeGC(...)`:
  ...
  - single-ref-child `br_on_cast` becomes a direct branch when the operand type already matches the target, `br_on_cast_fail` is removed when the same proof makes the fail edge definitely not taken, the no-payload nullable-source/non-null-target `br_on_cast` split is now covered by `[O4Z-AUDIT-RUB-Q]` research note `1366`, and branch-taking prefix payloads for definite `br_on_*` outcomes are covered by note `1372`
  - fallthrough-producing payload cases, payload-bearing cast splits, descriptor variants, broader fallthrough-type/local.tee cast insertion, and public stack-form unreachable-input rewrites remain explicit fail-closed boundaries rather than hidden no-ops; note `1373` adds a focused stack-payload `SuccessOnlyIfNonNull` boundary proving Binaryen needs ChildLocalizer/scratch-local repair for the fallthrough payload drop path, note `1374` implements the child-form ordinary unreachable-input subset while keeping descriptor BrOn representation-blocked, note `1379` adds public stack-form unreachable-input boundary coverage, and note `1380` closes the remaining GC entries as exact blockers/non-goals with reopening criteria
...
- `[O4Z-AUDIT-RUB-J]` landed the locally representable Binaryen `FinalOptimizer::restructureIf(...)` family:
  ...
  - extra target uses and effectful fallthrough arms remain preserved; branch-hint metadata and `never-unconditionalize` stay under the `[O4Z-AUDIT-RUB-N]` / `[O4Z-AUDIT-RUB-X]` metadata/pass-option boundary
- `[O4Z-AUDIT-RUB-L]` landed the locally representable Binaryen `FinalOptimizer::optimizeSetIf(...)` family:
  ...
  - branch-hint metadata transfer and public `never-unconditionalize` behavior remain explicit `[O4Z-AUDIT-RUB-N]` / `[O4Z-AUDIT-RUB-X]` boundaries
- `[O4Z-AUDIT-RUB-N]` / `[O4Z-AUDIT-RUB-X]` now document and close the local metadata/pass-option boundary rather than treating it as an implementable RUB rewrite slice:
  ...
  - note `1397` rechecked source and closes RUB-X as a product-level representation/pass-option blocker; reopening requires a code-metadata representation/parser/lowerer/binary or opaque-section policy plus remap tests and per-rewrite pass-option tests
- `[O4Z-AUDIT-RUB-K]` is now implemented for the direct value-`if` `selectify` family: Starshine keeps the existing one-result, scalar select-emission, reorder/effect, and local-read-safety gates, and direct selectification now mirrors Binaryen's shrink-level cost threshold for costly integer div/rem and trapping int-convert arms. Focused coverage locks the speed-mode preservation, shrink-mode selectification of a one-costly-arm `i32.rem_s` case, and note `1378`'s condition-invalidation boundary. Branch-hint metadata and public `never-unconditionalize` remain the `[O4Z-AUDIT-RUB-N]` / `[O4Z-AUDIT-RUB-X]` unsupported boundary.
- `[O4Z-AUDIT-RUB-P]` is now implemented for the local HOT subset of Binaryen's late `FinalOptimizer::visitSwitch(...)` one-target value-switch collapse. Lifted value-carrying `br_table`s whose explicit and default targets are all the same now rewrite to `drop(selector); br payload` when the selector is const/local.get and payload values are locally reorder-safe; RUB-S extends the payload proof to exact `v128.const` SIMD values. Effectful selector/value pairs and local child-less stack-payload switch forms remain conservative. The RUB-Q `1369` audit strengthened the positive local-selector test to show public stack-style value-switch WAT lifts to child-form payloads before this collapse, so the child-less shape is a local representation boundary rather than a public WAT gap. Focused tests cover the positive local-selector collapse, v128 payload collapse, and an effectful call/call reorder negative. RUB-P slice evidence: focused `remove_unused_brs_test.mbt` passed `170/170`, `moon test src/passes` passed `2595/2595`, full `moon test` passed `5906/5906`, and direct compare `.tmp/pass-fuzz-remove-unused-brs-rub-p-10000` compared `9977/10000` with `0` mismatches and `23` Binaryen/tool command failures (`22` rec-group-zero, `1` bad-section-size).
- `[O4Z-AUDIT-RUB-O]` final signoff is closed as of the post-RUB-P 2026-06-19 run:
  - RUB-P slice tests and build already passed before the RUB-P commit: focused `remove_unused_brs_test.mbt`, `moon test src/passes`, `moon info`, `moon fmt`, full `moon test`, and native `src/cmd` release build
  - post-RUB-P direct final 100000 at `.tmp/pass-fuzz-remove-unused-brs-rub-o-post-rub-p-final-100000` compared `99751/100000`, with `57148` normalized matches, `42601` cleanup-normalized matches, and `2` raw mismatches
  - the two raw mismatches are agent-classified Starshine wins: `case-027803-wasm-smith` removes a pure SIMD dropped expression before unconditional `unreachable` (Binaryen 69 bytes, Starshine 61 bytes), and `case-048903-wasm-smith` removes pure `memory.size` and `f64.abs` drops before unconditional `br` (Binaryen 68 bytes, Starshine 54 bytes); no side-effecting work is removed
  - command failures are Binaryen/tool failures: `249` total (`219` rec-group-zero, `12` bad-section-size, `11` command-failed, `6` table-index-out-of-range, `1` invalid-tag-index)
  - post-RUB-P generated O4z slot14/slot40 full and extracted direct replays in `.tmp/rub-o-post-rub-p-slots` validate with `wasm-tools`
  - RUB-N/RUB-X remain the documented unsupported policy boundary: branch-hint metadata and `remove-unused-brs-never-unconditionalize` pass-option behavior lack local representation/API support.
...
- The post-RUB-X bounded perf check in note `1398` keeps direct `remove-unused-brs` pass-local runtime within the repo target on both sampled repros: the O4z startup sample median is `0.715 ms` Starshine vs `0.686 ms` Binaryen (`1.039x`, mean ratio `1.000x`), and the slot42 sample median is `1.154 ms` vs `0.930 ms` (`1.255x`). The slot42 sample is the correctness-aligned perf sample (`Normalized WAT equal: yes`, `Canonical function compare equal: yes`); whole-command time remains slower there (`1.609x`) because non-pass traced/untraced overhead dominates.
