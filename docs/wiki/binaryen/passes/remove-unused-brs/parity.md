...
1. Run the focused profile and full dedicated evidence only after the deterministic implementation surface is complete; the latest root-admission repair has not yet been compared by explicit user request.
2. Classify any residual focused leaf before widening implementation further, with particular attention to dropped multivalue scratch allocation and nested wrapper cleanup.
3. Keep the repaired random-all families closed and preserve the `689` measured smaller unread-tee and terminal-return cleanups.
