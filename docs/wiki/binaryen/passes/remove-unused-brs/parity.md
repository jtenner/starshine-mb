...
- The nested wrapper-cleanup blocker is now repaired without the rejected broad suffix pruning. A shared CFG query proves whether one region root can reach the next, including an opcode-contract fallback for embedded value controls absent from the top-level CFG. RUB applies only the exact outward-branch/result-wrapper filler cleanup after ordinary rewrites. Focused wrapper evidence is `1000/1000` exact, and the wrapper leaf in the fresh `remove-unused-brs-all` development aggregate is `1250/1250` exact. The aggregate improves to `5655/10000` exact with zero failures and a `-41684`-byte residual delta, eliminating the former uniformly size-losing blocker. Direct parity remains open for the `625` one-byte multivalue-drop gaps, `1220` equal-size reference-typing/output-shape gaps, and classification of the `2500` smaller compound-profile residuals.
...
- The 2026-06-09 merge-blocks/RUB result-fallback audit is covered by [research note 0721](./index.md):
...
1. Repair the uniform `remove-unused-brs-multivalue-drop` `+1` family and the equal-sized broader-reference result typing in constant-branch and multi-function leaves.
2. Classify the smaller control, cleanup, switch, and GC compound residuals; retain only measured Starshine wins and align mixed families where no benefit is proven.
3. Keep wrapper-cleanup, result-refinalize, selectify, and single-target-br-table leaves exact-green, and preserve the `689` measured random-all unread-tee and terminal-return wins.
4. Rerun the fresh regular, dedicated, random-all, and wasm-smith v131 lanes, then reconcile the extra scheduled RUB slot separately.
...
- The generated `_build/wasm/debug/build/cmd/cmd.wasm` ordered `-O4z` audit from `2026-04-18` no longer leaves an open hard corruption slot for RUB on Binaryen-produced predecessor states:
  ...
  - slot `40` is retired by [`0108`](../late-pipeline-dispatch.md)
...
- Durable owner: [research note 0070](./index.md)
- Carried-wrapper follow-up: [research note 0076](./index.md)
- Follow-up health rerun: [research note 0079](../../../tooling/pass-fuzz-compare.md)
