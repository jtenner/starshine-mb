...
- The current tree already covers much more than dead tail stripping:
  ...
  - GC `br_on_*` cleanup for the safe local subset: definitely taken/not-taken `br_on_null` and `br_on_non_null`, definitely successful `br_on_cast`, definitely not-taken `br_on_cast_fail`, no-payload non-null disjoint-family `br_on_cast` / `br_on_cast_fail` failure rewrites, no-payload nullable-source/non-null-target `br_on_cast` splitting to `br_on_non_null` plus appended `ref.null`, branch-taking prefix-payload variants for definitely taken `br_on_null` / `br_on_non_null`, definitely successful `br_on_cast`, and definitely failing non-null disjoint `br_on_cast_fail`, and child-form ordinary `br_on_cast*` unreachable-input cleanup with payload drops, with explicit fail-closed boundaries for fallthrough-producing payload cases, payload-bearing cast splits, descriptor variants, broader fallthrough/cast insertion, stack-form unreachable-input cleanup, and nullable disjoint `SuccessOnlyIfNull`
...
- `[O4Z-AUDIT-RUB-G]` landed the conditional no-payload JumpThreader safe subset:
  ...
  - this older reopening list was narrowed by `[O4Z-AUDIT-RUB-Q]`: direct unconditional `br` and small/medium no-payload `br_table` retargeting are now implemented, payload sent-type retargeting is a source-backed JumpThreader non-goal for Binaryen `version_130`, try_table catch retargeting is covered for the no-payload safe subset, and note `1381` narrows large-table retargeting beyond the `<= 9` guard to a predicate/proof blocker protected by a ten-target boundary test
...
- `[O4Z-AUDIT-RUB-O]` final signoff is closed as of the post-RUB-P 2026-06-19 run:
  ...
  - RUB-N remains the only documented unsupported boundary: branch-hint metadata and `remove-unused-brs-never-unconditionalize` pass-option behavior lack local representation/API support.
