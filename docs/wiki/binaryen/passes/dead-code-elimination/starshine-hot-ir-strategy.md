...
- the first real legacy-`try` DCE reachability subset for legacy `try` without `pop`, legacy catch-arm HOT preservation, HOT legacy-`pop` representation/query support, the first represented nested-pop DCE repair, plus the remaining stack-switching handler-label tooling boundaries
