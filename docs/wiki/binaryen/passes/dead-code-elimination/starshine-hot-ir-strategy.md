...
- the first real legacy-`try` DCE reachability subset for legacy `try` without `pop`, legacy catch-arm HOT preservation, plus the remaining `pop`/catch-payload repair and stack-switching handler-label tooling boundaries
