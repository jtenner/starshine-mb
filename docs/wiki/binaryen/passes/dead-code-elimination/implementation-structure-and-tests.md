...
The latest blocker-enabling steps add a distinct legacy `pop` placeholder: WAST AST `Pop(ValueType)` and public `@lib.Instruction::Pop(ValType)` preserve `(pop i32)` through parser/AST-to-lib lowering and typechecking as a produced value. Typechecking now tracks a narrow per-catch payload context so `Pop` rejects outside legacy catches and rejects payload type mismatches. Binary module encode/decode supports a narrow contextual payload bridge: direct, tag-type-matching catch-body `Pop` pseudo-instructions are stripped before writing binary and reinjected from tag payload signatures after module decode, while uncontextualized instruction encoding still reports `CannotEncodeLegacyPop`. The high-level `wast_to_binary_module` and `wast_text_binary_roundtrip` entrypoints now allow only the simplest direct single-payload consumer text shape, such as `(drop (pop i32))`, and still reject broader Binaryen legacy `pop` text with an explicit diagnostic instead of admitting an incomplete full catch-payload implementation.

Remaining blockers are narrower but still release-relevant for full parity: high-level legacy-pop WAST ingestion is only the narrow direct single-payload consumer subset, binary catch-payload flow only covers direct contextual pseudo-pops rather than broader nested/interleaved forms, and DCE still lacks broader Binaryen pop movement/repair coverage beyond the represented repair fixtures. HOT lift/lower now preserves individual legacy catch tags and catch-all arm boundaries for the represented no-`pop` surface, so exact catch-arm shape is no longer the blocker it was after the first real-`Try` slice. Reopen for full parity when the remaining `pop`/payload and stack-switching surfaces exist, then extend repair deliberately rather than treating the representation slices as complete parity.
