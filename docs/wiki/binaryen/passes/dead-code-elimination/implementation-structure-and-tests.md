...
Remaining blockers are narrower but still release-relevant for full parity: high-level legacy-pop WAST ingestion is only the narrow direct complete-prefix consumer subset plus the `$call-pop-catch`-style ingress case where a complete prefix flows into an immediate nonfallthrough branch argument. That ingress now lets the local DCE pipeline remove the dead call after the raw candidate/trimming helpers learned to inspect legacy `try` bodies and catches. The WAST-lowered path now repairs a surviving contiguous root `pop` prefix before a branch, or before a void-return terminator, into fresh local carriers (`pop; local.set; local.get; drop; br/return`), narrowing the Binaryen local.set/local.get repair-shape gap without claiming exact nested block output parity. The return repair is deliberately guarded by the enclosing function result count so result-return payload flow is not discarded. Binary catch-payload flow only covers complete direct contextual pseudo-pop prefixes rather than broader nested/interleaved forms, and DCE still lacks broader Binaryen pop movement/repair coverage beyond the represented repair fixtures plus these contiguous root-pop-prefix-before-nonfallthrough text-path repairs. HOT lift/lower now preserves individual legacy catch tags and catch-all arm boundaries for the represented no-`pop` surface, so exact catch-arm shape is no longer the blocker it was after the first real-`Try` slice. Reopen for full parity when the remaining `pop`/payload and stack-switching surfaces exist, then extend repair deliberately rather than treating the representation slices as complete parity.
