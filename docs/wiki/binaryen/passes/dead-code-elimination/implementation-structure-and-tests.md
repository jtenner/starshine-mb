...
The latest blocker-enabling steps add a distinct legacy `pop` placeholder: WAST AST `Pop(ValueType)` and public `@lib.Instruction::Pop(ValType)` preserve `(pop i32)` through parser/AST-to-lib lowering and typechecking as a produced value. Typechecking now tracks a narrow per-catch payload context so `Pop` rejects outside legacy catches and rejects payload type mismatches. Binary module encode/decode supports a narrow contextual payload bridge: complete direct, tag-type-matching catch-body `Pop` pseudo-instruction prefixes are stripped before writing binary and reinjected from tag payload signatures after module decode, while uncontextualized instruction encoding and incomplete multi-payload prefixes still report `CannotEncodeLegacyPop`. The high-level `wast_to_binary_module` and `wast_text_binary_roundtrip` entrypoints now allow only complete direct payload-prefix consumer text shapes, including single-payload `(drop (pop i32))`, direct `pop -> local.set` or `pop -> local.tee`, and multi-payload prefixes immediately consumed by `drop`/`local.set`/`local.tee`, and still reject broader Binaryen legacy `pop` text with an explicit diagnostic instead of admitting an incomplete full catch-payload implementation. The boundary is now guarded by focused negative tests for interrupted multi-payload prefixes, nested catch-payload `pop`, interleaved module binary `Pop`, and nested module binary `Pop` so future `$call-pop-catch` admission has to be deliberate.

Remaining blockers are narrower but still release-relevant for full parity: high-level legacy-pop WAST ingestion is only the narrow direct complete-prefix consumer subset plus `$call-pop-catch`-style ingress cases where a complete prefix flows into an admitted nonfallthrough branch argument. That ingress now lets the local DCE pipeline remove the dead call after the raw candidate/trimming helpers learned to inspect legacy `try` bodies and catches. The WAST-lowered path now repairs a surviving contiguous root `pop` prefix before a branch with either a void target or a single matching value target, before a narrow `br_table` selector statement (`i32.const`, `local.get`, or `local.get; i32.const; i32.and`) whose targets are all void or have the same single matching value target, or before a void-return terminator into fresh local carriers (`pop; local.set; local.get; drop; br/br_table/return` for void targets, and `pop; local.set; local.get; br/br_table` for the single-value branch target), narrowing the Binaryen local.set/local.get repair-shape gap without claiming exact nested block output parity. Focused coverage also locks that those fresh carriers are allocated after the function parameter plus existing body-local index budget, so repair locals do not clobber pre-existing locals. The return repair is deliberately guarded by the enclosing function result count so result-return payload flow is not discarded; broader multi-value/type-index branch payload forms remain fail-closed at the WAST legacy-pop gate until their branch-payload repair has label-type-aware representation. Arbitrary multi-instruction `br_table` selector flow is still not broadly admitted. Binary catch-payload flow only covers complete direct contextual pseudo-pop prefixes rather than broader nested/interleaved forms, and DCE still lacks broader Binaryen pop movement/repair coverage beyond the represented repair fixtures plus these contiguous root-pop-prefix-before-nonfallthrough text-path repairs. HOT lift/lower now preserves individual legacy catch tags and catch-all arm boundaries for the represented no-`pop` surface, so exact catch-arm shape is no longer the blocker it was after the first real-`Try` slice. Reopen for full parity when the remaining `pop`/payload and stack-switching surfaces exist, then extend repair deliberately rather than treating the representation slices as complete parity.
