...
- full later reuse of already-computed best casts across Binaryen's fallthrough, adjacent-block, repeated-cast, and best-subtype cases beyond the current direct-root repeated/best-cast subset
...
- active narrow static-folding pass plus first root-linear later-reuse subsets for `ref.cast`, nullable-source `ref.as_non_null`, direct `local.tee` sources, repeated-cast source protection, and simple narrower-subtype best-cast selection
