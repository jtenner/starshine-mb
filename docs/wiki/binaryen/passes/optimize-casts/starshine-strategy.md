...
- strict earlier duplication of later `ref.cast` / `ref.as_non_null` values to earlier `local.get`s beyond the current empty, `nop`, dropped nontrapping i32 pure-tree, dropped local-read, nontrapping pure separate-index `local.set`, and nested outer-ref.as-over-nullable-ref.cast subset, including wider local-write windows and all effect/trap/control barriers
...
- active narrow static-folding pass plus first later-reuse subsets for `ref.cast`, nullable-source `ref.as_non_null`, direct `local.tee` sources, repeated-cast and unrelated-cast source protection, simple narrower-subtype best-cast selection including narrower-then-broader cast source retargeting and nullable-cast-to-`ref.as_non_null` source reuse, nested outer-ref.as-over-nullable-ref.cast motion, branch-free void fallthrough blocks, dropped branch-free value blocks, single-root branch-free value-block sources, nonlinear-control fact clearing, and strict empty, `nop`, dropped nontrapping i32 pure-tree, dropped-local-read, and nontrapping pure separate-index `local.set` early cast/as-non-null motion
