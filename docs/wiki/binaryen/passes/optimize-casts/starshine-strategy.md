...
- full later reuse of already-computed best casts across Binaryen's broader fallthrough, adjacent-block, repeated-cast, and best-subtype cases beyond the current direct-root plus branch-free void-block fallthrough subsets
...
- active narrow static-folding pass plus first later-reuse subsets for `ref.cast`, nullable-source `ref.as_non_null`, direct `local.tee` sources, repeated-cast source protection, simple narrower-subtype best-cast selection, and branch-free void fallthrough blocks
