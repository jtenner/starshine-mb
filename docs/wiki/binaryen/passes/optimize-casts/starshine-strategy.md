...
- strict earlier duplication of later `ref.cast` / `ref.as_non_null` values to earlier `local.get`s or earlier cast sources beyond the current empty, `nop`, dropped nontrapping i32 pure-tree, dropped local-read, nontrapping pure separate-index `local.set`, nested outer-ref.as-over-nullable-ref.cast, adjacent separate-root nullable-cast/ref.as, direct narrower-cast-into-broader-cast-source, and earliest-get best-selection subsets, including wider local-write windows, broader `move-cast-*` chains/refinalization, and all effect/trap/control barriers
- full later reuse of already-computed best casts across Binaryen's broader fallthrough, adjacent-block, repeated-cast, and best-subtype cases beyond the current direct-root, narrower-then-broader source-retargeting, branch-free void-block, dropped branch-free value-block, and single-root branch-free value-block-source fallthrough subsets
