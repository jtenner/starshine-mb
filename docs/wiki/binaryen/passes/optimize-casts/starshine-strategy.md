...
The pass now also has a dedicated GenValid aggregate, `optimize-casts-all`, with singleton leaves for later reuse, early motion, barrier negatives, best-cast/subtype selection, `ref.as_non_null`, static folds, and the ordered GC/local neighborhood. The aggregate records each sampled leaf in the manifest `selected_profile` field. After the initialized non-defaultable local fix, a bounded `100`-case aggregate smoke is normalized-green, but it is not a closeout lane yet because the required larger direct/dedicated/broad/wasm-smith evidence and source review are still missing and because several strict early-motion and broader local-flow families remain only partially implemented. The early-motion subset still excludes calls, trapping/effectful roots, loads, cross-region fact propagation, same-local writes/tees, local-write windows beyond nontrapping pure separate-index `local.set` roots and the current dropped separate-local `local.tee` subset, local.tee shapes beyond the current later-reuse alias plus separate-local source-tee/dropped-tee cases, and broader GC/control expression trees until each is source-backed and barrier-tested. It now scans nested structured-control regions independently, so a branch-local block can use the same strict local window without leaking facts across its enclosing control root.
