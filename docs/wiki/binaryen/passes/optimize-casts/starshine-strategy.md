...
- active narrow static-folding pass plus first later-reuse subsets for `ref.cast`, nullable-source `ref.as_non_null`, direct `local.tee` sources, repeated-cast and unrelated-cast source protection, simple narrower-subtype best-cast selection including narrower-then-broader cast source retargeting, branch-free void fallthrough blocks, and dropped branch-free value blocks
