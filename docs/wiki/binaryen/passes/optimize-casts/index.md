...
- Despite the broad CLI name, Binaryen `version_129` uses it for a much narrower job: improve how nearby `ref.cast` and `ref.as_non_null` values are reused through locals. Starshine's current direct pass is narrower/different: it removes provably redundant GC casts, folds statically known `ref.test` outcomes, folds descriptor tests/casts, rewrites guaranteed-success `br_on_cast` / `br_on_cast_fail` forms while preserving trap-vs-branch behavior, and now implements a conservative later-reuse subset with direct `local.get` / `local.tee` sources, same-local write barriers, repeated-cast and unrelated-cast source protection, simple narrower-subtype best-cast selection including narrower-then-broader cast source retargeting and nullable-cast-to-`ref.as_non_null` source reuse, branch-free void fallthrough blocks, dropped branch-free value blocks, and a strict early-motion subset across an empty window, `nop`, dropped constants, dropped `i32.eqz`-of-constant, or dropped `i32.add`/`i32.sub`/`i32.mul`/`i32.and`/`i32.or`/`i32.xor`/`i32.shl`/`i32.shr_s`/`i32.shr_u`/`i32.rotl`/`i32.rotr`-of-constants only.
