...
- The pass shares its upstream implementation file with [`../simplify-globals-optimizing/index.md`](../simplify-globals-optimizing/index.md).
...
- `agent-todo.md` currently has **no dedicated `simplify-globals` slice**, so this folder is also the first durable pass-specific explanation for the plain variant in this repo.
...
- The dossier now also has a source-confirmed implementation/test-map page, closing the biggest remaining owner-file and proof-surface gap in this folder.
...
- Keep the relationship to [`../simplify-globals-optimizing/index.md`](../simplify-globals-optimizing/index.md) explicit instead of silently teaching the plain pass only through the optimizing variant.
