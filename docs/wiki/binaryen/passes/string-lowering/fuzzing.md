...
# `string-lowering` Fuzzing Profile

Recommended smoke lane: run the ordinary mixed-generator compare-pass lane for this pass:

```sh
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass string-lowering --out-dir .tmp/pass-fuzz-string-lowering --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe
```
