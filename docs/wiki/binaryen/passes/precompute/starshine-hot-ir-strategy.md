...
- `precompute_control_is_const_self_br_if_noop(...)`
  - recognizes constant false self-branching loops as no-ops and also recognizes constant true-or-false void blocks whose `br_if` exits to the same zero-arity block label
- `precompute_loop_is_const_true_self_br_if_with_result_tail(...)`
  - rewrites a root constant-true self-branching void loop to an unconditional self-branch and replaces the following result-producing dead tail with `unreachable`, matching Binaryen's direct `precompute` shape for the reduced GenValid family
