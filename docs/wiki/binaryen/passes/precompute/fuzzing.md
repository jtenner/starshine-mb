...
`precompute` now has a dedicated pass-specific GenValid profile: `precompute-all`. Under the modern pass closeout standard, `[O4Z-AUDIT-PC]` is still not closed. The 2026-06-20 final-evidence refresh has green current `10000`-case regular GenValid, required `10000`-case dedicated `precompute-all`, and `10000`-case broad `pass-fuzz-stress` lanes with PC normalizers and `_build/native/release/build/cmd/cmd.exe`, but the regular `100000` final lane has not run. The explicit wasm-smith `10000` lane found one mismatch where Binaryen erases a reachable `atomic.fence` before a branch-to-end and Starshine preserves it as an ordering barrier; this is now a tested Starshine correctness boundary that still needs an explicit final closeout decision. The O4z no-op gate is decided as an accepted v0.1.0 fail-closed boundary, not as full O4z PC-slot parity.
...
1. regular GenValid: `--count 100000 --seed 0x5eed --pass precompute` with the documented PC cleanup normalizers;
2. explicit wasm-smith: `--wasm-smith --count 10000 --seed 0x5eed --pass precompute` with the documented PC cleanup normalizers and the reachable-`atomic.fence` boundary classified explicitly;
...
4. broad named GenValid: `--count 10000 --seed 0x5555 --pass precompute --gen-valid-profile pass-fuzz-stress` with the documented PC cleanup normalizers unless a literal random all-profiles profile exists by then.

Current refreshed evidence from [`../../../raw/research/0794-2026-06-20-precompute-final-evidence-refresh.md`](../../../raw/research/0794-2026-06-20-precompute-final-evidence-refresh.md): regular `10000` green (`1547` normalized, `8453` cleanup-normalized, `0` mismatches); dedicated `precompute-all` `10000` green (`5413` normalized, `4587` cleanup-normalized, all seven leaves sampled); broad `pass-fuzz-stress` `10000` green (`1529` normalized, `8471` cleanup-normalized); explicit wasm-smith `10000` compared `9956/10000` with `9952` normalized, `3` cleanup-normalized, `44` Binaryen/oracle command failures, and one `atomic.fence` preservation mismatch. Report requested count, compared count, normalized and cleanup-normalized matches, raw mismatches, generator/validation/property failures, command-failure classes, cache counters, selected-profile counts, and pass-local timings separately for each lane.
