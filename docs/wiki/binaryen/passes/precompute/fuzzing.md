...
`precompute` now has a dedicated pass-specific GenValid profile: `precompute-all`. Under the modern pass closeout standard, `[O4Z-AUDIT-PC]` is still not closed because the profile has not yet been used in the required `10000`-case dedicated closeout lane and the regular `100000`, wasm-smith, and broad `pass-fuzz-stress` lanes still need current green evidence. The O4z no-op gate is now decided as an accepted v0.1.0 fail-closed boundary, not as full O4z PC-slot parity.
