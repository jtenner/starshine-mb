...
1. bounded regular GenValid is still not green: after the constant-true-loop fix, `.tmp/pass-fuzz-precompute-true-loop-hot-fix-direct-100` compared `100/100` with PC normalizers but still had `20` raw mismatches. The remaining inspected subfamilies are constant-false self-branching loop cleanup (currently a focused-test-backed Starshine size win needing compare closeout handling) and mixed root `block (br self)` / `nop` debris around those loop/control shapes;
