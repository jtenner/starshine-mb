...
`precompute` remains open for the v0.1.0 `-O4z` per-pass audit. The older branch-heavy 10000-case direct compare evidence is still useful, the dedicated profile gap is closed by `precompute-all`, and the first O4z recovery slice now allows only changed `raw-scalar-folds` results under `optimize_level >= 4 && shrink_level >= 1`. The remaining O4z no-op surface is now an explicit v0.1.0 release boundary rather than an undecided blocker: HOT-only cleanup, load/call ownership hazards, large lowered functions, br_table/parser stack hazards, unchanged raw no-candidate cases, and changed non-scalar repair reasons stay fail-closed as `o4z-precompute-noop` until a focused artifact/fixture reopens them. The native path decision is now explicit for this checkout: after `moon build --target native --release src/cmd`, use `_build/native/release/build/cmd/cmd.exe` for precompute compare lanes because `target/native/release/build/cmd/cmd.exe` remains absent. The bounded regular GenValid mismatch family has now been reduced: constant self-exiting `block br_if` debris and constant-true self-branching loop result tails are fixed in pass code, while the remaining constant-false loop / mixed root-debris family is classified as a focused-test-backed Starshine size win with explicit PC normalizer coverage. The 2026-06-20 final-evidence refresh records current green `10000` regular GenValid, `10000` dedicated `precompute-all`, and `10000` broad `pass-fuzz-stress` lanes with PC normalizers and the explicit `_build` native binary. It also exposes one explicit wasm-smith mismatch, `case-006523-wasm-smith`, where Binaryen erases a reachable `atomic.fence` before a branch-to-end and Starshine preserves the ordering barrier. That divergence is now tested as a Starshine correctness boundary and documented in [`../../../raw/research/0794-2026-06-20-precompute-final-evidence-refresh.md`](../../../raw/research/0794-2026-06-20-precompute-final-evidence-refresh.md), but it must remain visible until accepted for final closeout. The pass is still not closed under the current standard because the final regular GenValid `100000` lane has not run and the atomic-fence wasm-smith boundary needs an explicit closeout decision. The O4z boundary is accepted for v0.1.0, but it must remain visible in the final report as a fail-closed release boundary rather than a claim of full O4z PC-slot optimization parity.
...
The durable modern status refresh is [`../../../raw/research/0785-2026-06-20-precompute-modern-signoff-refresh.md`](../../../raw/research/0785-2026-06-20-precompute-modern-signoff-refresh.md), and the latest final-evidence refresh is [`../../../raw/research/0794-2026-06-20-precompute-final-evidence-refresh.md`](../../../raw/research/0794-2026-06-20-precompute-final-evidence-refresh.md). Treat the pass as active/open until the final closeout lanes are recorded and the atomic-fence wasm-smith boundary is accepted or otherwise resolved; the O4z no-op boundary is decided but must stay visible as a release-boundary caveat.
