...
`precompute` remains open for the v0.1.0 `-O4z` per-pass audit. The older branch-heavy 10000-case direct compare evidence is still useful, and no new reduced direct-pass semantic bug was found in the 2026-06-20 refresh. The dedicated profile gap is now closed by `precompute-all`, and the first O4z recovery slice now allows only changed `raw-scalar-folds` results under `optimize_level >= 4 && shrink_level >= 1`. The pass is still not closed under the current standard for three remaining reasons:

1. the modern four-lane final closeout matrix has not yet been rerun with current code: regular GenValid `100000`, explicit wasm-smith `10000`, dedicated `precompute-all` `10000`, and broad `pass-fuzz-stress` `10000`;
2. broader O4z `precompute` work still deliberately returns `o4z-precompute-noop` for HOT-only cleanup, load/call ownership hazards, large lowered functions, br_table/parser stack hazards, raw no-candidate cases, and non-scalar raw repair reasons; current O4z slot evidence must therefore either recover more safe work or explicitly document an approved release boundary with reopening criteria;
3. `moon build --target native --release src/cmd` currently writes the native binary at `_build/native/release/build/cmd/cmd.exe` in this checkout, while the repo-standard final closeout commands still name `target/native/release/build/cmd/cmd.exe`; final signoff needs that path decision made explicit.
...
The O4z raw scalar follow-up is [`../../../raw/research/0788-2026-06-20-precompute-o4z-raw-scalar-recovery.md`](../../../raw/research/0788-2026-06-20-precompute-o4z-raw-scalar-recovery.md). It adds focused O4z tests proving scalar raw folds now run while HOT-only `br_table` cleanup remains fail-closed, and it records the `_build/native/...` explicit-native compare smoke.

The durable modern status refresh is [`../../../raw/research/0785-2026-06-20-precompute-modern-signoff-refresh.md`](../../../raw/research/0785-2026-06-20-precompute-modern-signoff-refresh.md). Treat the pass as active/open until the remaining O4z slot decision/evidence, native path decision, and final closeout lanes are recorded.
