...
- Current Starshine models only a small scalar/control subset of the upstream behavior, with a conservative raw stack-level shortcut for scalar/no-candidate functions, module-proven immutable `global.get` constants, mutable/global no-candidate reads, and dropped flat nontrapping scalar/global expressions to reduce direct-pass HOT lift/lower work.
