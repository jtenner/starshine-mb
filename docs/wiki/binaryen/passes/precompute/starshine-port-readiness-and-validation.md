...
- The remaining regular GenValid mismatch family from `.tmp/pass-fuzz-precompute-true-loop-hot-fix-direct-100/failures/` needs reduction, classification, and a TDD implementation or explicit evidence-backed boundary before final closeout. The self-exiting-block and constant-true loop result-tail subgaps are fixed; constant-false self-branching loop cleanup and mixed root `block (br self)` / `nop` debris are still open for closeout handling.
