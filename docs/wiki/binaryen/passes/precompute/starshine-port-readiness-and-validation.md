...
- preserve Binaryen-visible dead-root `nop` sentinels before trailing `unreachable`, normalize empty void bodies to one `nop` at the direct pass boundary, and remove exact constant self-exiting void blocks when both branch and fallthrough leave the same zero-arity block;
...
- Whether `[O4Z-AUDIT-PC]` should recover more safe `precompute` behavior under the current O4z gate or keep the remaining `o4z-precompute-noop` surfaces as explicitly approved v0.1.0 release boundaries with reopening criteria.
- The remaining regular GenValid mismatch family from `.tmp/pass-fuzz-precompute-self-brif-fix-direct-100/failures/` needs reduction, classification, and a TDD implementation or explicit evidence-backed boundary before final closeout. The first self-exiting-block subgap is fixed; constant-false self-branching loop cleanup and constant-true loop/dead-tail cleanup are still open for closeout handling.
