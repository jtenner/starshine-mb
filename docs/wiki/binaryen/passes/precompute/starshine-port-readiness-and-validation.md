...
- under the current O4z raw gate, run only changed raw scalar folds and intentionally no-op HOT-only cleanup plus hazardous/non-scalar raw paths until self-opt ownership hazards are narrowed or explicitly accepted as a release boundary;
...
- The bounded regular GenValid mismatch family from `.tmp/pass-fuzz-precompute-true-loop-hot-fix-direct-100/failures/` now has explicit closeout handling: self-exiting-block and constant-true loop result-tail subgaps are fixed in pass code, while constant-false self-branching loop cleanup and mixed root `block (br self)` / `nop` debris are classified as Starshine no-op-control cleanup wins covered by the PC normalizer. The final-evidence refresh raises regular GenValid to `10000/10000` with `0` mismatches, but final closeout still needs the required `100000` regular lane.
- The explicit wasm-smith lane found one reachable-`atomic.fence` mismatch. Starshine currently keeps this as a correctness boundary based on the local atomics docs; final closeout needs an explicit decision to accept this divergence or a deliberate policy-changing implementation slice.
- The dedicated GenValid profile name and family split are now fixed as `precompute-all`; final closeout still needs the current `10000`-case dedicated lane and selected-profile counts.
