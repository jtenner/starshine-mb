...
- use a conservative raw stack-level fast path for no-candidate functions, adjacent scalar folds, module-proven immutable `global.get` constants, and dropped flat nontrapping scalar expressions;
...
| `drop(pure-value)` | Replaced with `nop` or spliced away when the value is side-effect-free and nontrapping; flat scalar drop tails can use the raw shortcut, while dead exact roots before a trailing `unreachable` keep one Binaryen-style `nop`. | Local HOT/raw cleanup, not the full Binaryen child-retention algorithm. |
