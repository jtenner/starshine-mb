...
- Added a `rse-gc.wast` `needs-refinalize`-style raw fixture and retargeting for `struct.get`: after redundant `local.tee` removal exposes a strict-subtype receiver, the following `struct.get $A` can become `struct.get $B` so the field result reflects the refined subtype.
...
- Refreshed direct compare-pass parity at `.tmp/pass-fuzz-rse-rse002-struct-get-refinalize`: `6759/10000` compared, `6759` normalized matches, `0` mismatches, and `20` Binaryen/tool command failures.
