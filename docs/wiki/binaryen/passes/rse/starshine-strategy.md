...
- Added raw identity-preserving refinement wrappers for `ref.as_non_null`, `ref.cast`, and `ref.cast_desc_eq`, so concrete-heap refined gets now retarget through both straight-line and branch-merge shapes instead of stopping at the wrapper node; follow-up tests now cover `ref.cast` and `ref.cast_desc_eq` via programmatic lowered-instruction fixtures because the WAT parser does not yet expose ordinary `ref.cast` syntax.
...
- Added an explicit conservative loop-exit skip regression where `block (loop ... br_if 1 ... local.set) ...; i32.const 1; local.set` keeps the post-loop set instead of trusting loop-local facts that an outer exit can bypass.
- Added a HOT fallback block-exit safety regression and conservative post-block fact drop so direct HOT execution no longer folds a final same-const set when `br_if` can bypass the block-local write; the raw lowered path remains the precise block/if/GC-branch label-exit lane.
- Refreshed direct compare-pass parity at `.tmp/pass-fuzz-rse-rse002-hot-block-exits`: `6759/10000` compared, `6759` normalized matches, `0` mismatches, and `20` Binaryen/tool command failures.
