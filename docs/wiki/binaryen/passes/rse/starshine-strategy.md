...
6. Future direct parity should run `moon build --target native --release src/cmd` followed by `bun fuzz compare-pass ... --pass redundant-set-elimination --jobs auto --starshine-bin _build/native/release/build/cmd/cmd.exe`; historical evidence includes `.tmp/pass-fuzz-redundant-set-elimination` from 2026-05-06 with 6759 comparable matches, 0 semantic mismatches, and 20 Binaryen empty-recursion-group command failures.
