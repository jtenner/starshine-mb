...
```sh
bun scripts/pass-fuzz-compare.ts --count 10000 --seed 0x5eed --pass llvm-memory-copy-fill-lowering --out-dir .tmp/pass-fuzz-llvm-memory-copy-fill-lowering --jobs auto --starshine-bin target/native/release/build/cmd/cmd.exe
```
