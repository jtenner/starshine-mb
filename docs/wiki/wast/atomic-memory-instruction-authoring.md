...
- The generator's `[FZG]017` row is strong current-Starshine core/binary/validator evidence for atomics, not evidence that arbitrary WAST text can author those shapes or that Starshine exactly matches every proposal atomic/shared-memory distinction. In particular, it should not hide the local policy split between standalone `AtomicFence` and the shared-memory gate on `MemArg` atomics.
