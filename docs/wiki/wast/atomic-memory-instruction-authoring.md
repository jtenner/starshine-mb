...
Do not use this page as the owner for every instruction whose name contains `atomic`. Shared-Everything Threads also has **shared-GC aggregate atomics**. Starshine now carries shared type metadata plus struct/array atomic get/RMW/cmpxchg across core, binary, validation, HOT, and selected WAST forms; route that boundary through [`../wasm-shared-everything-threads-boundary.md`](../wasm-shared-everything-threads-boundary.md) and [`gc-aggregate-instruction-authoring.md`](gc-aggregate-instruction-authoring.md), not through `MemArg` rules. Relaxed Atomics remains a separate active Phase-2 proposal: Starshine now has the `SeqCst` / `AcqRel` linear-atomic carrier and codec slice, but `pause`, high-level text, dedicated generator gating, and complete proposal/runtime signoff remain open.
...
Current Starshine source/tests document ordered struct atomic gets plus struct/array RMW/cmpxchg WAST forms, while core/binary/validation/HOT additionally carry array atomic gets. Aggregate set and wait/notify remain absent. Existing optimizer folds are pass-specific proofs, not general permission to move or delete aggregate atomic operations.
...
The exact local map is in [`src/binary/decode.mbt`](../../../src/binary/decode.mbt), [`atomic_rmw_op_id(...)`](../../../src/binary/encode.mbt), and [`atomic_cmpxchg_op_id(...)`](../../../src/binary/encode.mbt). [`src/binary/tests.mbt`](../../../src/binary/tests.mbt) locks `0xFE 0x03 0x01` as `AtomicFence(AcqRel)` and rejects reserved order byte `0x02`.
...
3. **Preserve directional memory ordering and trap behavior.** Ordered loads contribute reads/acquires, ordered stores contribute writes/releases, RMW/cmpxchg can carry both directions, and `SeqCst` is stronger ordering. Wait operations remain read/trap-sensitive. `atomic.fence` has no stack effect or memory operand, but carries order and must not move or disappear without a memory-model proof.
