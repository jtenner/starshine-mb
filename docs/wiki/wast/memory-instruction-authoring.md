...
Linear-memory atomic instructions are deliberately routed to [`atomic-memory-instruction-authoring.md`](atomic-memory-instruction-authoring.md): Starshine core/binary/validator/generator supports them, but current WAST keywords and parser cases do not expose `i32.atomic.load`, `memory.atomic.wait32`, `atomic.fence`, and related text syntax. Shared-GC `struct.atomic.get*` is a separate aggregate-instruction surface owned by [`gc-aggregate-instruction-authoring.md`](gc-aggregate-instruction-authoring.md). The companion page [`memory-argument-authoring.md`](memory-argument-authoring.md) owns `offset=`, `align=`, default memory `0`, memory32/memory64 address widths, and the current WAST gap around explicit nonzero memory indices. [`resource-declaration-authoring.md`](resource-declaration-authoring.md) owns `(memory ...)` declarations, imports, exports, and the current text-surface caveat that WAST memory declarations lower through the `i32` limits path. The validator-side memory64/table64 matrix now lives in [`../validate/memory-table-address-widths.md`](../validate/memory-table-address-widths.md). Custom Page Sizes is a separate active-proposal memory-type dimension; current Starshine memory instructions, resource declarations, and pass helpers assume ordinary page-size semantics because `MemType` has no page-size field. Route that through [`../wasm-custom-page-sizes-boundary.md`](../wasm-custom-page-sizes-boundary.md). Memory Control is another separate active-proposal family: `memory.discard`, possible commit/protection/mapping modes, and BYOB host-memory ideas are not current Starshine memory instructions and must route through [`../wasm-memory-control-boundary.md`](../wasm-memory-control-boundary.md). This page owns the **instruction stack shapes**, **resource-index relationships**, **data-count requirement**, **side-effect/trap behavior**, and **Starshine layer map**.

The current source set combines the Core multi-memory, data/data-count, and address-width refreshes with Starshine WAST parser/lowerer/printer, core instruction, binary codec, validator, generator, arbitrary WAST, and HOT-IR effect surfaces. The focused [`../wasm-multi-memory-boundary.md`](../wasm-multi-memory-boundary.md) page provides the selected-memory/status route for `MemIdx` and `MemArg` carriers. The 2026-06-05 Custom Page Sizes bridge [`../wasm-custom-page-sizes-boundary.md`](../wasm-custom-page-sizes-boundary.md) adds a no-support boundary for proposal memories whose page size is not represented locally. The focused [`../wasm-memory-control-boundary.md`](../wasm-memory-control-boundary.md) page records the no-support boundary for discard/commit/protection/mapping-style runtime memory-management proposal surfaces. The 2026-06-04 refresh in [`../validate/memory-table-address-widths.md`](../validate/memory-table-address-widths.md) confirms the official Core 3.0 pages dated 2026-06-03 use positional address-width rules and anchors the current Starshine code-map caveats.
...
`memory.size` pushes the current size in pages. `memory.grow` consumes a page delta and pushes the previous size, or a failure sentinel at runtime. For memory64, Starshine's validator uses the selected memory's address type for both the `memory.size` result and `memory.grow` operand/result via [`typecheck_memory_size(...)`](../../../src/validate/typecheck.mbt#L2552-L2558) and [`typecheck_memory_grow(...)`](../../../src/validate/typecheck.mbt#L2561-L2571). Today those page counts are ordinary Starshine pages because no custom page-size carrier exists in `MemType`; do not interpret this as Custom Page Sizes proposal support.

Current WAST lowering emits `MemorySize(MemIdx(0))` and `MemoryGrow(MemIdx(0))`; direct core/binary fixtures are required for nonzero memory-index coverage until the WAST surface grows explicit memory-index syntax. Use [`../wasm-multi-memory-boundary.md`](../wasm-multi-memory-boundary.md) when the question is selected-memory support across Starshine layers. Use [`../validate/memory-table-address-widths.md`](../validate/memory-table-address-widths.md) when the question is whether a selected memory's `I64Limits` changes stack typing, not how to spell the fixture in WAST.
...
4. **After data-segment rewrites, validate data-count and data users.** A pass that deletes or remaps data segments must update `MemoryInit`, `DataDrop`, `ArrayNewData`, `ArrayInitData`, active data modes, and `DataCntSec` together; use [`data-segment-authoring.md`](data-segment-authoring.md) for the full segment rewrite checklist and the current local data-count precheck gap.
5. **Do not overclaim WAST text support.** Core/binary/generator support is broader than WAST text for nonzero memory indices and linear-memory atomics. Use direct core or binary fixtures when testing those surfaces today, and use [`atomic-memory-instruction-authoring.md`](atomic-memory-instruction-authoring.md) for linear-memory atomic stack/effect/signoff rules.
...
- Current Starshine validation types `memory.fill` length as `i32` even for memory64; official validation uses the selected memory address type for that length operand. The 2026-06-04 refresh in [`../validate/memory-table-address-widths.md`](../validate/memory-table-address-widths.md) is the current focused source for this divergence, while the 2026-05-20 memory64 note remains the earlier detailed source bridge.
- Custom Page Sizes is not represented in Starshine memory instructions today. Any fixture or pass claim that depends on a page being 1 byte or some non-64KiB size is future-port evidence and must start from [`../wasm-custom-page-sizes-boundary.md`](../wasm-custom-page-sizes-boundary.md).
- Memory Control is not represented in Starshine memory instructions today. `memory.discard`, possible lazy commit/protection/mapping operations, and BYOB host-memory behavior are active proposal evidence only; do not treat `memory.fill 0`, `data.drop`, memory64, shared memory, or Custom Page Sizes as substitutes. Route through [`../wasm-memory-control-boundary.md`](../wasm-memory-control-boundary.md).
...
- Current address-width refresh: [`../validate/memory-table-address-widths.md`](../validate/memory-table-address-widths.md), [`../validate/memory-table-address-widths.md`](../validate/memory-table-address-widths.md)
