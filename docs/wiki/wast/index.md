...
- [`atomic-memory-instruction-authoring.md`](atomic-memory-instruction-authoring.md) — linear-memory threads-proposal atomic loads/stores/RMW/wait/notify plus standalone `atomic.fence`; core/binary/validator/generator support with a current WAST keyword/parser gap, the `MemArg` shared-memory versus no-memory fence split, plus routing notes that keep shared-GC `struct.atomic.get*` on the aggregate page.
