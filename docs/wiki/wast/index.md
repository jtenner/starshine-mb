...
- **Aggregate instruction text:** many official `array.*` and `struct.set` families currently need core/binary/generator fixtures, but focused shared-GC `struct.atomic.get*` WAST text now exists with `seq_cst` / `acq_rel` order spellings. Full Shared-Everything Threads status, shared heap types, array atomics, aggregate RMW/cmpxchg, and wait/notify route through [`../wasm-shared-everything-threads-boundary.md`](../wasm-shared-everything-threads-boundary.md). Keep initializer admission distinct from text support: official Core and Starshine's current gate both admit `array.new`, `array.new_default`, and `array.new_fixed` in constant expressions after array-type resolution, but Starshine still has no high-level `array.*` WAST instruction text. Route the exact boundary through [`gc-aggregate-instruction-authoring.md`](gc-aggregate-instruction-authoring.md), [`../validate/constant-expressions.md`](../validate/constant-expressions.md), and the [`2026-07-10 reconciliation`](../raw/wasm/2026-07-10-constant-expression-array-constructor-reconciliation.md).
