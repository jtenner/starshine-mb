...
| Struct atomic reads | `struct.atomic.get`, `struct.atomic.get_s`, `struct.atomic.get_u` | Yes, using canonical `seq_cst` / `acq_rel` order spellings | Yes | Preserve packed signedness and ordering; route proposal status through [`../wasm-shared-everything-threads-boundary.md`](../wasm-shared-everything-threads-boundary.md). |
| Struct atomic RMW/cmpxchg | `struct.atomic.rmw.*`, `struct.atomic.rmw.cmpxchg` | Yes for the represented operator/type/field forms | Yes | These are ordered read-write operations; never infer free movement from parser or validator success. |
...
| Core array constructors | `array.new`, `array.new_default`, `array.new_fixed`, `array.new_data`, `array.new_elem` | No | Yes | Use core/binary/generated fixtures; data/element-backed forms also touch segment index spaces. |
| More Array Constructors proposal | `array.new_array`, `array.new_memory`, `array.new_table` | No | No | Active Phase-1 proposal only; route standards/status and future implementation requirements through [`../wasm-more-array-constructors-boundary.md`](../wasm-more-array-constructors-boundary.md). |
...
| Array atomic get/RMW/cmpxchg | `array.atomic.get*`, `array.atomic.rmw.*`, `array.atomic.rmw.cmpxchg` | RMW/cmpxchg text is present; atomic-get text remains narrower than core | Yes | Use WAST for represented RMW/cmpxchg forms and core/binary fixtures for text gaps; preserve element index, mutability, result, trap, and order semantics. |
...
;; no ArrayNewArray / ArrayNewMemory / ArrayNewTable proposal variants today
ArrayGet(TypeIdx)
ArrayGetS(TypeIdx)
ArrayGetU(TypeIdx)
ArraySet(TypeIdx)
...
The reviewed local surface now includes struct aggregate RMW/cmpxchg and array aggregate RMW/cmpxchg WAST forms, while the core/binary/validator/HOT layer also carries array atomic gets. Remaining text/proposal gaps include aggregate `set`, aggregate wait/notify, broad shared heap-type declaration syntax, and WAST exposure for every core-carried array atomic get form. The focused proposal boundary is [`../wasm-shared-everything-threads-boundary.md`](../wasm-shared-everything-threads-boundary.md).
...
| `array.new`, `array.new_default`, `array.new_fixed` | Listed as constant instructions. | Accepted when `validate_const_instr(...)` resolves the type as an array; ordinary constant-expression typechecking still checks operands, defaultability, fixed arity, and expected result. | Use core/binary/generated initializer fixtures; this does not imply current WAST `array.*` text support. |
