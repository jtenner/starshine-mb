...
- `validate-invalid-text` uses inline `assert_malformed`, `assert_invalid`, and `assert_unlinkable` strategies to prove the parser/lowerer/validator stage split. Its dynamic helper currently adds a separate GenValid-WAT mutation lane: generate valid WAT, verify the base parses/lowers/validates, remove the final closing paren, quote the mutated text as `assert_malformed`, and classify the result with the same static evaluator.
