...
- `validate-invalid-text` uses inline `assert_malformed`, `assert_invalid`, and `assert_unlinkable` strategies to prove the parser/lowerer/validator stage split. Its dynamic helper adds a separate GenValid-WAT mutation lane: generate valid WAT, verify the base parses/lowers/validates, then mutate the printed text into quoted `assert_malformed` missing-final-paren / bad-opcode-token specimens or a raw `assert_invalid` duplicate-export specimen, classify the result with the same static evaluator, and report simple source feature facts in suite details.
