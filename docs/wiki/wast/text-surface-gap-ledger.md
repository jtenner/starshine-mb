...
| Ordinary non-tail reference calls | `call_ref` | Missing as ordinary WAST text. `return_call_ref` text is supported. | Core, binary, validator, and generator support ordinary `call_ref`; `ref.func` declaration rules remain separate. | Use core/binary/generated fixtures; route standards/status and local layer claims through [`../wasm-typed-function-references-boundary.md`](../wasm-typed-function-references-boundary.md), call semantics through [`function-call-and-module-authoring.md`](function-call-and-module-authoring.md), and tail forms through [`tail-call-authoring.md`](tail-call-authoring.md). |
| GC aggregate mutation, arrays, shared-GC atomics, and proposal array constructors | Core `struct.set`, Core `array.new*`, `array.get*`, `array.set`, `array.len`, `array.fill`, `array.copy`, `array.init_*`; active-proposal `array.new_array`, `array.new_memory`, `array.new_table`; struct/array aggregate atomics | Ordinary `struct.set` and Core `array.*` text remain missing. Supported aggregate-atomic text now includes `struct.atomic.get*`, struct RMW/cmpxchg, and array RMW/cmpxchg; array atomic-get text remains narrower than core, and aggregate set/wait/notify are absent. | Core, binary, validator, HOT/effects, valid-generator, and passes carry shared type metadata plus struct/array atomic get/RMW/cmpxchg. More Array Constructors remains unsupported. | Use WAST for represented aggregate-atomic forms and core/binary/generated fixtures for remaining arrays/atomics; route ordinary aggregates through [`gc-aggregate-instruction-authoring.md`](gc-aggregate-instruction-authoring.md), More Array Constructors through [`../wasm-more-array-constructors-boundary.md`](../wasm-more-array-constructors-boundary.md), and Shared-Everything status through [`../wasm-shared-everything-threads-boundary.md`](../wasm-shared-everything-threads-boundary.md). |
| Linear-memory atomics | `i32.atomic.load`, atomic stores/RMW/cmpxchg, `memory.atomic.wait*`, `memory.atomic.notify`, `atomic.fence` | The v132 WAST surface covers the represented linear-memory atomic forms, including named/numeric memory selection and `seqcst` / `acqrel` / `relaxed` order spellings. Unsupported proposal forms remain text gaps. | Core/binary/validator/generator support the current local subset. `atomic.fence` is no-memory/no-`MemArg`; `MemArg` atomics require an existing selected memory plus ordinary memarg/stack validity, but not sharedness during local typechecking. `[FZG]017` still deliberately selects shared memory. | Use WAST for represented forms and core/binary/generated fixtures for the remaining gaps; route through [`atomic-memory-instruction-authoring.md`](atomic-memory-instruction-authoring.md) and [`../wasm-linear-memory-threads-boundary.md`](../wasm-linear-memory-threads-boundary.md). Do not confuse this with supported shared-GC `struct.atomic.get*` text on [`gc-aggregate-instruction-authoring.md`](gc-aggregate-instruction-authoring.md) or with full Shared-Everything Threads proposal work on [`../wasm-shared-everything-threads-boundary.md`](../wasm-shared-everything-threads-boundary.md). |
| Memory64/table64 declarations and selected resource text | memory64/table64 resource declarations, nonzero selected memories in text memory ops, broad multi-memory text | Narrow current WAST declaration and memarg support; many memory64/table64 positives need core/binary/generator fixtures. | Core/resource/binary and parts of validator represent `I64Limits` and address-width behavior; selected-memory `MemIdx` / `MemArg` support is broader in core/binary/validator/generator than in WAST text; some validator table/memory instruction gaps remain documented. | Route declaration text through [`resource-declaration-authoring.md`](resource-declaration-authoring.md), memargs through [`memory-argument-authoring.md`](memory-argument-authoring.md), selected-memory status through [`../wasm-multi-memory-boundary.md`](../wasm-multi-memory-boundary.md), runtime stack widths through [`memory-instruction-authoring.md`](memory-instruction-authoring.md) / [`table-instruction-authoring.md`](table-instruction-authoring.md), and validator status through [`../validate/memory-table-address-widths.md`](../validate/memory-table-address-widths.md). |
...
- Using Binaryen WAT examples with branch hints, ordinary `call_ref`, Core arrays, More Array Constructors, or atomics as Starshine text fixtures without checking `src/wast/keywords.mbt` and the focused WAST page.
