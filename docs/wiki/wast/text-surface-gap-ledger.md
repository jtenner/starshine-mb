...
| GC aggregate mutation, arrays, broader shared-GC atomics, and proposal array constructors | Core `struct.set`, Core `array.new*`, `array.get*`, `array.set`, `array.len`, `array.fill`, `array.copy`, `array.init_*`; active-proposal `array.new_array`, `array.new_memory`, `array.new_table`; `array.atomic.*`, aggregate atomic set/RMW/cmpxchg, wait/notify | Missing as high-level WAST text except for focused `struct.atomic.get*`. Struct constructors/gets, i31, conversions, descriptor constructors, `ref.get_desc`, and focused `struct.atomic.get*` are supported. | Core, binary, validator, valid-generator, and several passes use many ordinary aggregate instructions; More Array Constructors has no local WAST/core/binary/validator/generator support; only the struct-atomic-get slice is documented locally for Shared-Everything proposal-shaped aggregate atomics. | Use core/binary/generated fixtures for current Core arrays; route ordinary aggregate shapes through [`gc-aggregate-instruction-authoring.md`](gc-aggregate-instruction-authoring.md), route More Array Constructors proposal names through [`../wasm-more-array-constructors-boundary.md`](../wasm-more-array-constructors-boundary.md), and route full Shared-Everything proposal status through [`../wasm-shared-everything-threads-boundary.md`](../wasm-shared-everything-threads-boundary.md). For data/element-backed Core arrays also use [`data-segment-authoring.md`](data-segment-authoring.md), [`element-segment-authoring.md`](element-segment-authoring.md), and [`../validate/data-count-and-code-data-indices.md`](../validate/data-count-and-code-data-indices.md). |
| Linear-memory atomics | `i32.atomic.load`, atomic stores/RMW/cmpxchg, `memory.atomic.wait*`, `memory.atomic.notify`, `atomic.fence` | Missing as WAST keywords/parser cases. | Core/binary/validator/generator support the current local subset. `atomic.fence` is no-memory/no-`MemArg`; `MemArg` atomics require an existing selected memory plus ordinary memarg/stack validity, but not sharedness during local typechecking. `[FZG]017` still deliberately selects shared memory. | Use core/binary/generated fixtures; route through [`atomic-memory-instruction-authoring.md`](atomic-memory-instruction-authoring.md) and its [2026-07-10 reconciliation](../raw/wasm/2026-07-10-linear-atomics-unshared-validation-reconciliation.md). Do not confuse this with supported shared-GC `struct.atomic.get*` text on [`gc-aggregate-instruction-authoring.md`](gc-aggregate-instruction-authoring.md) or with full Shared-Everything Threads proposal work on [`../wasm-shared-everything-threads-boundary.md`](../wasm-shared-everything-threads-boundary.md). |
