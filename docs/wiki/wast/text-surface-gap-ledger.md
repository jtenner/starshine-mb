...
| Official name/custom annotations and expression-level code metadata | `@name`, `@custom`, `@metadata.code.*`, branch hints | Current `(@...)` lane attaches only to function definitions/imports and lowers to `FuncAnnotationSec`; it does not implement official name/custom placement or expression metadata effects. | Binary custom/name sections and function annotations are separate local surfaces; Binaryen metadata behavior is documented from Binaryen sources. | Route local function annotations through [`code-metadata-and-function-annotations.md`](code-metadata-and-function-annotations.md) and binary name/custom behavior through [`../binary/custom-and-name-sections.md`](../binary/custom-and-name-sections.md). |
