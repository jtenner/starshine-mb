...
- **Do not hide unsupported name maps behind tests that only parse WAST.** If a test needs type/table/memory/global/tag/elem/data debug names after binary lowering, assert the corresponding `NameSec` map explicitly.
