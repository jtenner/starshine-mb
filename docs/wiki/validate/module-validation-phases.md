...
The most important beginner trap is unreachable-code stack polymorphism. When code is unreachable, Starshine can synthesize `BotValType` for missing operands, matching the official validation model. But values pushed after an unreachable point are still real values. The focused contract, examples, and regression map live in [`stack-polymorphism-and-bottom.md`](stack-polymorphism-and-bottom.md). The separate runtime meaning of a wasm trap, host `WebAssembly.RuntimeError`, `mayTrap` effect facts, and equal-trap runtime smoke evidence lives in [`runtime-trap-semantics.md`](runtime-trap-semantics.md). Tests such as `validate_module rejects concrete stack junk after return inside block`, `validate_module rejects wrong concrete loop result after infinite inner loop`, and the end-of-body stack-shape diagnostics lock this boundary.
