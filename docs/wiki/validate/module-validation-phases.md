...
| `memsec` | Validates memory limits, memory64 address widths, and shared-memory maximum requirements. | Extends `mems` incrementally. | `validate_memsec`, `MemType` validation; focused resource-section rules live in [`resource-sections-and-limits.md`](resource-sections-and-limits.md), WAST memory declaration caveats live in [`../wast/resource-declaration-authoring.md`](../wast/resource-declaration-authoring.md), memory argument address-width effects in [`../wast/memory-argument-authoring.md`](../wast/memory-argument-authoring.md), and runtime memory stack shapes plus the current `memory.fill` memory64 length caveat in [`../wast/memory-instruction-authoring.md`](../wast/memory-instruction-authoring.md). |
| `tagsec` | Validates exception tag type indices and Starshine's current local empty-result rule. Current Core 3.0 tagtype validity is broader, while `throw` / `catch` / `catch_ref` still require empty-result tag expansions at use sites. | Extends `tags` incrementally. | `validate_tagsec`, `TagType` validation; focused resource-section rules live in [`resource-sections-and-limits.md`](resource-sections-and-limits.md), WAST catch/throw authoring details live in [`../wast/exception-tag-authoring.md`](../wast/exception-tag-authoring.md), and the current source split is [`../raw/wasm/2026-06-04-exception-tag-current-refresh.md`](../raw/wasm/2026-06-04-exception-tag-current-refresh.md). |
| `globalsec` | Validates global types and constant initializers. Later globals see earlier globals only. | Extends `globals` incrementally. | `validate_globalsec`, `validate_const_expr`; focused resource-section rules live in [`resource-sections-and-limits.md`](resource-sections-and-limits.md), the focused initializer and immutable-`global.get` rules live in [`constant-expressions.md`](constant-expressions.md), and WAST global declarations, imports, exports, and mutability syntax live in [`../wast/resource-declaration-authoring.md`](../wast/resource-declaration-authoring.md). |
| `elemsec` | Validates passive/declarative/active element modes, element payload types, table targets, and constant offsets. | Extends `elems`. | `validate_elemsec`; focused resource-section rules live in [`resource-sections-and-limits.md`](resource-sections-and-limits.md), with core/binary segment shapes in [`../binary/data-element-and-datacount-sections.md`](../binary/data-element-and-datacount-sections.md). |
| `datasec` | Validates passive/active data modes, memory targets, and constant offsets. | Extends `datas`. | `validate_datasec`; focused resource-section rules live in [`resource-sections-and-limits.md`](resource-sections-and-limits.md), and WAST fixture guidance lives in [`../wast/data-segment-authoring.md`](../wast/data-segment-authoring.md). |
| `datacnt` | Checks declared data-count equality with the data section. | Reports `DataCountSection` diagnostics. | `validate_datacnt`; the equality-versus-body-requirement split is summarized in [`resource-sections-and-limits.md`](resource-sections-and-limits.md). |
| `datacnt_requirement` | Rejects covered data-index body users when the data-count section is absent. Current official WebAssembly requires data-count for any code-section data index, so `array.new_data` / `array.init_data` are a documented local gap in this precheck. | Reports `FunctionBody` diagnostics with absolute imported-prefix function indices. | `validate_bulk_memory_data_count_requirement`; tests for `data count section required`; focused guide in [`data-count-and-code-data-indices.md`](data-count-and-code-data-indices.md) and recheck [`../raw/wasm/2026-06-04-data-count-code-data-index-recheck.md`](../raw/wasm/2026-06-04-data-count-code-data-index-recheck.md). |
...
| `ref_func_declarations` | Builds the declared-function bitmap, then checks every `ref.func` in globals/tables/elements/code against it. The 2026-06-04 current refresh confirms the official `refs` rule still includes optional start, while Starshine intentionally excludes `start_sec` locally. | Reports section-specific diagnostics, with body uses reported as `FunctionBody`. | [`./ref-func-declarations.md`](./ref-func-declarations.md). |
...
| --- | --- | --- | --- |
| `ref.func` declared references | The official module `refs` set includes module-level declaration sources, including optional start; the official formula also names memories, though ordinary Starshine memory definitions do not currently carry function indices directly. The current 2026-06-03 Core 3.0 pages reconfirmed this on 2026-06-04. | Starshine computes a declaration bitmap from exports, global/table initializer `ref.func`, element payloads, and element expressions, but excludes `start_sec`. | Keep [`./start-section.md`](./start-section.md), [`./ref-func-declarations.md`](./ref-func-declarations.md), and the negative start regression aligned until a deliberate policy change lands. |
| Resultful exception tags | Current Core 3.0 tag-type validation accepts a type use expanding to `func params -> results`, but `throw`, `catch`, and `catch_ref` still require selected tags to expand as `func params -> epsilon` at the EH instruction. | Starshine rejects resultful tag imports and tag-section entries earlier in `Validate for TagType`, so declaration-only resultful tags cannot currently reach code-body validation. | Keep the placement matrix in [`../wast/exception-tag-authoring.md`](../wast/exception-tag-authoring.md#tag-result-shape-split-declaration-versus-eh-use-site) visible: future widening should move declaration-only positives into section validation while preserving use-site rejections in instruction validation. |
| Structured `name` section | Core custom sections are not normally semantic validation blockers; current WebAssembly 3.0 name-section subsections cover module/function/local/type/field/tag names. The active Phase-2 Extended Name Section proposal covers label/table/memory/global/elem/data name maps, but it is not finished/Core evidence yet. | Starshine rejects raw `name` custom sections in `custom_secs`, validates official maps, and also validates proposal-facing/local label/table/memory/global/elem/data name maps against their index spaces. | Link [`../wasm-extended-name-section-boundary.md`](../wasm-extended-name-section-boundary.md) and [`../binary/custom-and-name-sections.md`](../binary/custom-and-name-sections.md) when changing name metadata or pass rewrite rules; keep the official-Core-versus-proposal/local subsection caveat visible. Tag-name rewrites should also follow [`../wast/exception-tag-authoring.md`](../wast/exception-tag-authoring.md). |
| Data-count requirement diagnostics | Current official binary module validation requires a data-count section when any data index occurs in code. Bulk-memory `memory.init` / `data.drop` are the common examples, and GC `array.new_data` / `array.init_data` are also data-index users in Starshine's core instruction set. | Starshine has a distinct `datacnt_requirement` phase so missing data-count use in a body reports as `FunctionBody` with absolute `FuncIdx`, but that phase currently scans only `MemoryInit` / `DataDrop`; `ArrayNewData` / `ArrayInitData` without `DataCntSec` are validator-gap evidence. | Route detailed changes through [`data-count-and-code-data-indices.md`](data-count-and-code-data-indices.md); keep invalid-fuzzer expected family and imported-prefix tests updated; if the scanner widens, add core/binary fixtures for both GC array data users and update [`../wast/data-segment-authoring.md`](../wast/data-segment-authoring.md). |
| Import declaration versus host matching | The official model separates module import declarations from runtime external-value matching at instantiation. The 2026-06-04 current refresh also keeps Core 3.0 memory matching as address-type-plus-limits, without Starshine's local shared-memory flag. | `validate_importsec(...)` validates the declared external types and extends local index spaces, while [`src/validate/match.mbt`](../../../src/validate/match.mbt) owns the reusable external-type compatibility relation; there is no active public host-linking contract here. Local `MemType` matching additionally requires shared-flag equality. | Keep [`import-export-and-external-type-matching.md`](import-export-and-external-type-matching.md) linked when changing import validation, future linker APIs, or `Match for ExternType`. |
| Memory64 `memory.fill` length | Official validation types `memory.fill` destination and length with the selected memory address type, while the byte value is `i32`. The 2026-06-04 refresh confirmed this on the Core 3.0 pages dated 2026-06-03. | Starshine currently types the destination as selected-memory `at` but still pops `len:i32`, so memory64 positive fixtures need a direct validator follow-up before they can be accepted locally. | Keep the focused matrix in [`memory-table-address-widths.md`](memory-table-address-widths.md), [`../wast/memory-instruction-authoring.md`](../wast/memory-instruction-authoring.md), [`../raw/wasm/2026-06-04-memory-table-address-width-validation-refresh.md`](../raw/wasm/2026-06-04-memory-table-address-width-validation-refresh.md), and the older [`../raw/wasm/2026-05-20-memory64-bulk-memory-validation-refresh.md`](../raw/wasm/2026-05-20-memory64-bulk-memory-validation-refresh.md) linked when changing memory64 bulk-memory typing. |
| Table64 table-operation widths | Official validation uses the selected table address type for `table.get` / `set` / `size` / `grow`, both destination and length in `table.fill`, mixed-width `table.copy`, and active element offsets; `table.init` keeps element source and length as `i32`. The 2026-06-04 refresh confirmed this on the Core 3.0 pages dated 2026-06-03. | Starshine currently hard-codes `i32` for get/set/size/grow and indirect-call table indices, uses address widths for `table.copy` and `table.init` destination, and only widens the `table.fill` destination while keeping `len:i32`. | Keep the focused matrix in [`memory-table-address-widths.md`](memory-table-address-widths.md), [`../wast/table-instruction-authoring.md`](../wast/table-instruction-authoring.md), [`../raw/wasm/2026-06-04-memory-table-address-width-validation-refresh.md`](../raw/wasm/2026-06-04-memory-table-address-width-validation-refresh.md), and the older [`../raw/wasm/2026-05-20-table64-table-instruction-validation-refresh.md`](../raw/wasm/2026-05-20-table64-table-instruction-validation-refresh.md) linked when changing table64 instruction typing. |
| Local/proposal `StringRefsSec` | Current Core WebAssembly 3.0 does not define section id `14` as stable core WebAssembly; the active Phase-1 Reference-Typed Strings proposal does define a draft `stringrefs` section with that id. | Starshine validates `string.const`, validates supported array-backed string helpers, and carries local stringrefs roundtrip plumbing that mirrors the draft subset. | Do not describe `StringRefsSec` as stable core Wasm; route fixture stack/storage details through [`../wast/string-instruction-authoring.md`](../wast/string-instruction-authoring.md), the current refresh [`../raw/wasm/2026-06-04-stringref-proposal-current-refresh.md`](../raw/wasm/2026-06-04-stringref-proposal-current-refresh.md), and the shared feature-status vocabulary in [`../wasm-feature-status-and-proposal-boundaries.md`](../wasm-feature-status-and-proposal-boundaries.md). |
...
4. If a pass deletes data segments or any data-index instruction (`memory.init`, `data.drop`, `array.new_data`, or `array.init_data`), is `DataCntSec` still correct, are data-index users repaired, and do WAST/core fixture expectations still match [`../wast/data-segment-authoring.md`](../wast/data-segment-authoring.md)?
