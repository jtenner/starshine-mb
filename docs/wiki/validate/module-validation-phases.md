...
| `ref_func_declarations` | Builds the declared-function bitmap, then checks every `ref.func` in globals/tables/elements/code against it. The 2026-05-20 refresh confirms the official `refs` rule still includes optional start, while Starshine intentionally excludes `start_sec` locally. | Reports section-specific diagnostics, with body uses reported as `FunctionBody`. | [`./ref-func-declarations.md`](./ref-func-declarations.md). |
