...
Use this page as the validator-side companion to the binary layout map in [`../binary/module-section-map.md`](../binary/module-section-map.md). The binary page explains wire-order and section ids; this page explains why Starshine validates in semantic dependency order. For the compact navigation table of current local/spec splits—resultful tags, data-count scanner coverage, memory64/table64 operand-width gaps, constant-expression allow-list differences, structured-name metadata, import matching, and stringref proposal boundaries—use [`local-spec-divergence-ledger.md`](local-spec-divergence-ledger.md).
...
| `tagsec` | Validates exception tag type indices and Starshine's current local empty-result rule. Current Core 3.0 tagtype validity is broader, while `throw` / `catch` / `catch_ref` still require empty-result tag expansions at use sites. | Extends `tags` incrementally. | `validate_tagsec`, `TagType` validation; focused resource-section rules live in [`resource-sections-and-limits.md`](resource-sections-and-limits.md), WAST catch/throw authoring details live in [`../wast/exception-tag-authoring.md`](../wast/exception-tag-authoring.md), and the current source split is [`../wast/exception-tag-authoring.md`](../wast/exception-tag-authoring.md). |
...
| `ref_func_declarations` | Builds the declared-function bitmap, then checks every `ref.func` in globals/tables/elements/code against it. Current Core 3.0 and Starshine both exclude the optional start field from `refs`. | Reports section-specific diagnostics, with body uses reported as `FunctionBody`. | [`./ref-func-declarations.md`](./ref-func-declarations.md). |
...
- Current exception/tag-result refresh: [`../wast/exception-tag-authoring.md`](../wast/exception-tag-authoring.md)
...
- Current import/export matching source bridge: [`import-export-and-external-type-matching.md`](import-export-and-external-type-matching.md)
- Current GC type/subtyping bridge: [`type-section-and-subtyping.md`](type-section-and-subtyping.md)
