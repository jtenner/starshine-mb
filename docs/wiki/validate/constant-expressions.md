...
| Valid-generator widening | [`gen_valid_const_expr_for_global(...)`](../../../src/validate/gen_valid.mbt), `gen_valid_const_expr_from_context(...)`, `allow_const_expr_variants`, `ConstExprVariants`; current focused coverage includes the shared `ref.i31` path for global and table initializer targets. The GenValid context allow-list records active data and active element offsets as i32-only constant-expression targets: numeric `i32.const` or immutable imported `i32 global.get`, not reference initializers. |
