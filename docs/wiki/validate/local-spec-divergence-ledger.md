...
| Constant-expression allow-list | Core 3.0 defines a current constant-instruction family and notes the list can grow; `array.new`, `array.new_default`, and `array.new_fixed` are included, and table-initializer `global.get` visibility is narrower than ordinary global initializers. | [`validate_const_instr(...)`](../../../src/validate/validate.mbt) matches that three-constructor Core array subset and is broader for several scalar/local proposal operations; environment order makes optional core table initializers see imported immutable globals but not later locals. WAST still lacks `array.*` instruction text. | [`constant-expressions.md`](constant-expressions.md), [`../wast/gc-aggregate-instruction-authoring.md`](../wast/gc-aggregate-instruction-authoring.md). | Mark broader accepted forms as Starshine-local unless the feature-status page says otherwise; use direct core/binary/generator fixtures for the admitted array constructors and keep WAST text absence distinct from validator admission. |
