...
The current resource-section contract is maintained on this page and its focused current-source bridges; it is grounded in WebAssembly Core validation, binary, and syntax pages plus Starshine's validator and invalid-fuzzer evidence. The focused [`../wasm-custom-page-sizes-boundary.md`](../wasm-custom-page-sizes-boundary.md) makes an additional memory-type boundary explicit: Starshine validates address-width limits and local sharedness, but current `MemType` has no page-size field to validate or match. The 2026-06-04 import/export matching refresh in [`import-export-and-external-type-matching.md`](import-export-and-external-type-matching.md) keeps Core 3.0 memory matching as address-type-plus-limits and documents Starshine's `shared`-flag equality as a local/proposal extension. The focused living router for Threads/shared-memory claims is [`../wasm-linear-memory-threads-boundary.md`](../wasm-linear-memory-threads-boundary.md), backed by the 2026-06-04 linear-memory threads/shared-memory refresh in [`../wasm-linear-memory-threads-boundary.md`](../wasm-linear-memory-threads-boundary.md): stable Core 3.0 is not the owner for Starshine's `shared` flag, while the threads proposal/draft is the right source for shared memories and the shared-memory maximum requirement. The 2026-06-04 constant-expression refresh in [`constant-expressions.md`](constant-expressions.md) sharpens the context split for initializer `global.get`: globals can see imports plus earlier globals, optional table initializers are imported-only, and Starshine's table phase runs before local globals. The 2026-06-04 element refresh in [`../wast/element-segment-authoring.md`](../wast/element-segment-authoring.md) confirms the mode-sensitive active/passive/declarative element validation split and keeps Starshine's WAST declarative-mode loss separate from validator/core support. The 2026-06-04 address-width refresh in [`memory-table-address-widths.md`](memory-table-address-widths.md) confirms that accepting memory64/table64 limits at the resource layer is not enough to claim every memory/table instruction is address-width-complete; the focused living matrix is [`memory-table-address-widths.md`](memory-table-address-widths.md). The 2026-06-04 data/data-count refresh in [`../binary/data-element-and-datacount-sections.md`](../binary/data-element-and-datacount-sections.md) and focused recheck in [`data-count-and-code-data-indices.md`](data-count-and-code-data-indices.md) similarly separate Starshine's local bulk-memory-only missing-data-count precheck from the current official rule for any code-section data index; the reusable guide is [`data-count-and-code-data-indices.md`](data-count-and-code-data-indices.md). The 2026-06-04 exception refresh in [`../wast/exception-tag-authoring.md`](../wast/exception-tag-authoring.md) separates Starshine's strict tag-section result check from current Core 3.0's broader tagtype validity and narrower EH-instruction empty-result requirements.
...
- Current linear-memory threads/shared-memory refresh: [`../wasm-linear-memory-threads-boundary.md`](../wasm-linear-memory-threads-boundary.md)
...
- Current memory/table address-width refresh: [`memory-table-address-widths.md`](memory-table-address-widths.md), [`memory-table-address-widths.md`](memory-table-address-widths.md)
...
- Current import/export matching refresh: [`import-export-and-external-type-matching.md`](import-export-and-external-type-matching.md)
