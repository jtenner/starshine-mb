...
- Follow-up work is now about widening specific families rather than rebuilding the stack shape: richer binary corruption families (for example malformed LEBs and UTF-8 corruption), more binary validator-rejected families beyond the current function/start/export/code/datacount/name coverage, even deeper per-family AST invalid breadth inside already-covered validator families, richer GC-shaped valid generation, and any newly exposed downstream parity holes from the broader generator.
