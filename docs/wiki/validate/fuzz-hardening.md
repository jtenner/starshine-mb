...
- The current checked-in fuzz runner is now strong on valid-module and AST-invalid validator coverage, while the binary, text, and spec-seed rejection lanes are still future work.
- The active runnable suites are currently `validate-valid`, `validate-invalid-ast`, `binary-roundtrip`, `wast-roundtrip`, `wat-roundtrip`, and `cmd-harness`.
- The remaining future validator invalid suite ids are now reserved in the CLI as `validate-invalid-binary`, `validate-invalid-text`, and `validate-invalid-spec-seed` until their later implementation slices land.
...
- The AST invalid lane is now back in-tree, but the binary, text, and spec-seed rejection suites still need explicit implementations.
