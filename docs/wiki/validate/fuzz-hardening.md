...
- The currently landed AST-invalid strategy set now covers duplicate export names, invalid start signatures, missing datacount for `memory.init`, undeclared `ref.func`, out-of-range function-name indices, out-of-range function-section type indices, function sections without code sections, mutable `global.get` uses inside constant global initializers, non-constant active data offsets, out-of-range imported function type indices, mutable `global.get` uses inside table initializers, non-constant active element offsets, shared memories without maxima, invalid tag type indices, descriptor metadata on non-struct function types, and datacount sections without data sections.
...
- The currently landed binary-invalid strategy set covers trailing garbage, truncated modules, duplicate type sections, wrong section order, out-of-range function-section type indices, and a second class of validator-rejected encoded modules built from deterministic AST invalid mutations: invalid start signatures, duplicate export names, function sections without code sections, datacount sections without data sections, and out-of-range function name indices.
...
- The currently landed text-invalid strategy set now covers a malformed quoted module with a missing closing paren, a malformed quoted module with trailing garbage, an invalid result-stack module, an invalid mutable-`global.get` const-initializer module, and two valid-but-unlinkable inline modules that depend on unknown function or memory imports.
...
- The curated spec-seed registry now includes multiple committed assertions per stage family instead of only one example of each: two malformed quote cases from `tests/spec/const.wast`, two invalid type-mismatch cases from `tests/spec/f32.wast`, and two unlinkable unknown-import cases from `tests/spec/imports.wast`.
...
- Follow-up work is now about widening specific families rather than rebuilding the stack shape: richer binary corruption families (for example malformed LEBs and UTF-8 corruption), more binary validator-rejected families beyond the current function/start/export/code/datacount/name coverage, deeper per-family AST invalid breadth inside already-covered validator families, richer GC-shaped valid generation, and any newly exposed downstream parity holes from the broader generator.
