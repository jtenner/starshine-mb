...
- The `gen-valid` module generator is now materially broader than the earlier function-only/active-segment core: natural generation can now mix function/table/memory/global/tag imports, randomize counts beyond the old single-import/single-segment ceiling, vary active offsets and data payloads, emit passive data segments plus passive/declarative element segments with both `funcs` and expr-backed element kinds, widen general ref usage beyond just `funcref`/`externref` into abstract GC-family refs like `anyref`, `eqref`, `i31ref`, `structref`, and `arrayref`, and reach bulk memory/table instruction families like `memory.copy`, `memory.fill`, `memory.init`, `data.drop`, `table.copy`, `table.fill`, `table.init`, and `elem.drop`; the checked-in coverage-forced path now deliberately adds concrete struct/array type defs plus a rec-group shape, typed ref-heavy globals/locals/tables, and deterministic GC/ref instruction families such as `struct.new_default`, `struct.get`, `struct.set`, `array.new_default`, `array.get`, `array.set`, `array.len`, `ref.test`, `ref.cast`, `ref.eq`, `ref.as_non_null`, `call_ref`, and `br_on_null` in one validating module.
