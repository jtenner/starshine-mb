...
- Follow-up work is now about widening specific families rather than rebuilding the stack shape: richer binary corruption families (for example malformed LEBs and UTF-8 corruption), more AST-invalid families across the remaining table/element/import/tag/type corners, richer GC-shaped valid generation, and any newly exposed downstream parity holes from the broader generator.
