...
- The currently landed AST-invalid strategy set now covers duplicate export names plus out-of-range export func indices, invalid start signatures plus out-of-range start function indices, missing datacount for both `memory.init` and `data.drop`, undeclared `ref.func`, out-of-range function-name, memory-name, table-name, and global-name indices, out-of-range function-section and imported-function type indices, function sections without code sections, code sections without function sections, mutable `global.get` uses inside constant global initializers and table initializers, non-constant active data and element offsets, shared memories without maxima, invalid tag type indices, descriptor metadata on non-struct function types, and datacount sections without data sections.
...
- The currently landed binary-invalid strategy set covers trailing garbage, truncated modules, duplicate type sections, wrong section order, bad magic headers, bad version headers, out-of-range function-section type indices, and a second class of validator-rejected encoded modules built from deterministic AST invalid mutations: invalid start signatures, duplicate export names, function sections without code sections, code sections without function sections, datacount sections without data sections, and out-of-range function, table, and global name indices.
...
- The currently landed text-invalid strategy set now covers a malformed quoted module with a missing closing paren, a malformed quoted module with trailing garbage, a malformed quoted module with an invalid float-const token, an invalid result-stack module, an invalid mutable-`global.get` const-initializer module, an invalid parameterized start-function module, and three valid-but-unlinkable inline modules that depend on unknown function, memory, or global imports.
...
- The curated spec-seed registry now includes multiple committed assertions per stage family instead of only one example of each: three malformed quote cases from `tests/spec/const.wast`, three invalid type-mismatch cases from `tests/spec/f32.wast`, and three unlinkable cases from `tests/spec/imports.wast`, including both unknown-import and incompatible-import-type failures.
...
- Follow-up work is now about widening specific families rather than rebuilding the stack shape: richer binary corruption families beyond the current header/order/duplication/core set (for example malformed LEBs, section-size corruption, and UTF-8 corruption), more binary validator-rejected families beyond the current function/start/export/code/datacount/name coverage, even deeper per-family AST invalid breadth inside already-covered validator families, richer GC-shaped valid generation, and any newly exposed downstream parity holes from the broader generator.
