...
| Optimizer passes | Local inlining/no-inline code reads `FuncAnnotationSec` marker names it owns. The separate Starshine `apply-compiler-facts` stage consumes trusted exact-target assertions, not proposal metadata. | No pass consumes proposal `compilation_priority`, `instr_freq`, call-target percentages, `never_opt`, or `always_opt` semantics. |
